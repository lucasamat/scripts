...
def GetEquipmentMaster(PerPage, PageInform, A_Keys, A_Values):
	...
	for par in parent_obj:
		...
		if str(ibase_status) == "INCOMPLETE" and str(ibase_greenbook) =="DISPLAY":
			...
			#ibase_icon = '<a class = "cust_exc" title="Missing iBase Attributes"><img src="/mt/appliedmaterials_sit/Additionalfiles/Icons/Exclamation.svg"></a>'
			ibase_missing_flag = True
			for key,col in ibase_attributes.items():
				if not col:
					err_title += "Attribute "+str(key)+" is missing from the iBase \n"
			ibase_icon = '<a class = "cust_exc" title="'+str(err_title)+'"><img src="/mt/appliedmaterials_sit/Additionalfiles/Icons/Exclamation.svg"></a>'
			...
			ibase_icon = '<a class = "cust_exc" title="Complete"><img src="/mt/appliedmaterials_sit/Additionalfiles/Green_Tick.svg"></a>'
			err_title +="Complete"
	...
	if ibase_missing_flag:
		...
		filter_control_function += ('$(".cpq_notification_div .cpq_cust_warning").html("<img src=\'/mt/APPLIEDMATERIALS_SIT/Additionalfiles/warning1.svg\' class=\'wid20_px\'><p>'+str(missing_text)+'</p>").css("display","block").parent().css("display","block").parent().prev().children("div").removeClass("collapsed");$("#notify").addClass("in");$("#notify").css("height","auto");')
	else:
	...
	for quer_key, quer_value in enumerate(Dict_formation):
		...
		if Dict_formation.get(quer_value) != "":
			...
			if str(quer_values).find(",") == -1:
				...
				elif str(quer_value) in ('WARRANTY_START_DATE','WARRANTY_END_DATE'):
					ATTRIBUTE_VALUE_STR += str(quer_value)+" = convert(varchar, '"+str(quer_values)+"', 101) and "
				else:
...
def GetEventsMasterFilter(ATTRIBUTE_NAME, ATTRIBUTE_VALUE,PerPage,PageInform):
	...
	if TreeTopSuperParentParam == "Product Offerings":
		...
		parent_obj = Sql.GetList(
			"select top "
			+ str(PerPage)
			+ " * from ( select ROW_NUMBER() OVER( ORDER BY "+str(orderby)+") AS ROW, QUOTE_REV_PO_GRNBK_PM_EVEN_ASSEMBLIES_RECORD_ID,EQUIPMENT_DESCRIPTION,EQUIPMENT_ID,ASSEMBLY_ID,GREENBOOK,FABLOCATION_ID,DEVICE_NODE,PROCESS_TYPE,GOT_CODE,PM_ID,PM_NAME,MNTEVT_LEVEL,KIT_ID,KIT_NAME,KIT_NUMBER,SSCM_PM_FREQUENCY,PM_FREQUENCY,INCLUDED from SAQGPA (NOLOCK) where "+str(ATTRIBUTE_VALUE_STR)+" QUOTE_RECORD_ID = '"
			+ str(ContractRecordId)
			+ "' and QTEREV_RECORD_ID = '"
			+ str(RevisionRecordId)
			+ "' and SERVICE_ID = '"
...
def GetCovObjMaster(PerPage, PageInform, A_Keys, A_Values):    
	...
	if TreeSuperParentParam == "Product Offerings" or (TreeTopSuperParentParam== "Comprehensive Services" and TreeParentParam=="Add-On Products"):
		...
		if TreeTopSuperParentParam == "Product Offerings" :
			...
			if TreeParentParam == "Receiving Equipment":
				...
				Trace.Write('6871----6886-------J1'+str(TreeParam))
				service_id = Product.GetGlobal("addon_service_id")
	...
	for par in parent_obj:
		...
		if str(ibase_status) == "INCOMPLETE":
			...
			ibase_icon = '<a class = "cust_exc" title="Missing iBase Attributes for assemblies"><img src="/mt/appliedmaterials_sit/Additionalfiles/Icons/Exclamation.svg"></a>'
			ibase_missing_flag = True
		else:
			ibase_icon = '<a class = "cust_exc" title="Complete"><img src="/mt/appliedmaterials_sit/Additionalfiles/Green_Tick.svg"></a>'
		Action_str = (
...
def GetCovObjMasterFilter(ATTRIBUTE_NAME, ATTRIBUTE_VALUE,PerPage,PageInform):
	...
	for par in parent_obj:
		data_dict = {}
		data_id = str(par.QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID)
		Trace.Write('Qstr-WARRANTY_END_DATE_ALERT_J ----'+str(par.WARRANTY_END_DATE_ALERT))
		ibase_status = str(par.IBASE_ATTSTS)
		if str(ibase_status) == "INCOMPLETE":
			ibase_icon = '<a class = "cust_exc" title="Missing iBase Attributes for Assemblies"><img src="/mt/appliedmaterials_sit/Additionalfiles/Icons/Exclamation.svg"></a>'
		else:
		...
		if can_delete.upper() == "TRUE":
			if '<li><a class="dropdown-item" id="deletebtn" data-target="#cont_CommonModalDelete" data-toggle="modal" onclick="CommonDelete(this, \'SAQSCO#'+ data_id +'\', \'WARNING\')" href="#">DELETE</a></li>' not in Action_str:
				Action_str += '<li><a class="dropdown-item" id="deletebtn" data-target="#cont_CommonModalDelete" data-toggle="modal" onclick="CommonDelete(this, \'SAQSCO#'+ data_id +'\', \'WARNING\')" href="#">DELETE</a></li>'
		if can_clone.upper() == "TRUE":
			Action_str += '<li><a class="dropdown-item" data-target="#" data-toggle="modal" onclick="Material_clone_obj(this)" href="#">CLONE</a></li>'
	...
	for key, col_name in enumerate(list(Columns)):
		...
		if filter_level_data == "select":
			...
			for string in StringValue_list:
				string = string.strip()
				if string == "ACQUIRED":
					string_value = string.replace("ACQUIRED","<img title='Acquired' src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/Green_Tick.svg> ACQUIRED")
				...
				if string == "ASSEMBLY MISSING":
					string_value = string.replace("ASSEMBLY MISSING","<img title='Assembly Missing' src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/Orange1_Circle.svg> ASSEMBLY MISSING")
