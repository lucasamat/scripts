...
class SyncQuoteAndCustomTables:
    ...
    def _get_custom_fields_detail(self):		
        return {
            ...
            'CreatedBy':self.quote.GetCustomField('CreatedBy').Content,
            'RequestedDate':datetime.datetime.strptime(self.quote.GetCustomField('RequestedDate').Content, '%Y-%m-%d').date(),
            'PayerAddress':self.quote.GetCustomField('PayerAddress').Content,
            'SalesPersonEmail':self.quote.GetCustomField('SalesPersonEmail').Content,
            'StateAbbreviation':self.quote.GetCustomField('StateAbbreviation').Content,
            'OpportunityName':self.quote.GetCustomField('OpportunityName').Content,
        }
                ...
                if not quote_obj:					
                    ...
                    if custom_fields_detail.get("STPAccountID"):
                        ...
                        if account_obj:							
                            ...
                            if custom_fields_detail.get("OpportunityId"):
                                ...
                                #1018
                                get_par_record_id = Sql.GetFirst("select PAR_ACCOUNT_ID,PAR_ACCOUNT_RECORD_ID from SAACNT WHERE ACCOUNT_ID = '{}'".format(custom_fields_detail.get("STPAccountID")))
                    ...
                    if payload_json_obj:
                        ...
                        if payload_json.get('SAEMPL'):
                            ...
                            employee_object = Sql.GetFirst("SELECT FIRST_NAME,LAST_NAME,EMPLOYEE_ID,EMPLOYEE_NAME,EMPLOYEE_RECORD_ID FROM SAEMPL WHERE EMPLOYEE_ID = '{employee_id}'".format(employee_id= custom_fields_detail.get('EmployeeResponsibleID')))
                        ...
                        if contract_quote_obj:
                            ...
                            try:
                                if payload_json.get("SAQTSV"):
                                    ...
                                    saqtsv_update = """UPDATE A SET A.CONTRACT_VALID_FROM = B.CONTRACT_START_DATE,A.CONTRACT_VALID_TO = B.CONTRACT_END_DATE FROM SAQTSV A INNER JOIN {} B on A.QUOTE_RECORD_ID = B.QUOTE_RECORD_ID and A.SERVICE_ID = B.SERVICE_ID
