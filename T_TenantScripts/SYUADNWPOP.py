...
def POPUPLISTVALUEADDNEW(
    ...
    if Question_obj is not None:
        ...
        # ADD FAB POPUP STARTS JOE
            ...
            if (("Sending Account -" in TreeParam) or ("Receiving Account -" in TreeParam)) and TreeParentParam == 'Fab Locations':
                ...
                QueryCountObj = Sql.GetFirst(
                        ...
                        ObjectName,
                        sales_org_id,
                        "WHERE MAFBLC.SALESORG_ID = '"+str(sales_org_id)+"' " + where_string if where_string else ""
        ...
        ##Added the below code for the receiving fab popup...
            ...
            Pagination_M = Sql.GetFirst(
                "SELECT COUNT(PRLPBE.CpqTableEntryId) as count FROM {} (NOLOCK) WHERE GREENBOOK = '{}' AND PRICEBOOK_TYPE = 'POSS' ".format(
            ...
            if where_string:
                where_string += " AND"
            where_string += """ GREENBOOK = '{}' AND PRICEBOOK_TYPE = 'POSS' """.format(
        ...
        ##involved party equipment ends
            ...
            Trace.Write("check=3291==getDocType.DOCTYP_ID"+str(getDocType.DOCTYP_ID))
            Trace.Write("check=3291==getDocType"+str(getDocType))
            pagination_where = where_string + " AND" if where_string!="" else ""
            Pagination_M = Sql.GetFirst("select count(MAADPR.CpqTableEntryId) as count from MAADPR WHERE "+str(pagination_where)+" PRDOFR_ID = '"+str(TreeSuperParentParam)+"' AND PRDOFR_DOCTYP = '"+str(getDocType.DOCTYP_ID)+"' AND MNL_ADDON = 1  AND COMP_PRDOFR_ID NOT IN (SELECT SERVICE_ID FROM SAQTSV where QUOTE_RECORD_ID ='"+str(contract_quote_record_id)+"' AND QTEREV_RECORD_ID ='"+str(quote_revision_record_id)+"') ")
            ...
            if where_string:
                where_string += " AND"
            where_string += """ PRDOFR_ID = '{}' AND PRDOFR_DOCTYP = '{}' AND MNL_ADDON = 1 AND COMP_PRDOFR_ID NOT IN (SELECT SERVICE_ID FROM SAQTSV where QUOTE_RECORD_ID ='{}' AND QTEREV_RECORD_ID = '{}' AND PAR_SERVICE_ID ='{}'  )""".format(str(TreeSuperParentParam),str(getDocType.DOCTYP_ID),contract_quote_record_id,quote_revision_record_id,str(TreeSuperParentParam))
            table_data = Sql.GetList(
            ...
            if QryCount==0:
                ...
                if A_Keys != "" and A_Values != "":
                    ...
                    for key, value in zip(A_Keys, A_Values):
                        if value.strip():
                            if where_string:
                                where_string += " AND "
                            where_string += "SACRVC.{Key} LIKE '%{Value}%' ".format(Key=key, Value=value)
                Trace.Write("checking_whr_str-- "+str(where_string))
                DIVNAME = "VIEW_DIV_ID"
                new_value_dict = {}
                ObjectName = "SACRVC"
                ...
                # LEFT JOIN SACVNT (NOLOCK) ON SACRVC.BUKRS = SACVNT.BUKRS AND SACRVC.HKONT = SACVNT.HKONT AND SACRVC.BELNR = SACVNT.BELNR AND SACRVC.ZUONR = SACVNT.ZUONR AND SACRVC.GJAHR = SACVNT.GJAHR AND SACRVC.BUZEI = SACVNT.BUZEI AND SACRVC.MANDT = SACVNT.MANDT
                Pagination_M = Sql.GetFirst("select count(SACRVC.CpqTableEntryId) as count from SACRVC (NOLOCK)  WHERE {} REPLACE(LTRIM(REPLACE(KUNAG,'0',' ')),' ','0') = '{}'".format(where_string + " AND " if where_string else "",account_id))
                ...
                table_data = Sql.GetList(
                    "select {} from SACRVC (NOLOCK) WHERE {} REPLACE(LTRIM(REPLACE(KUNAG,'0',' ')),' ','0') = '{}' ".format(
                        ", ".join(ordered_keys), where_string if where_string else "",str(account_id)
                        # "WHERE " + where_string if where_string else "",
                        # order_by,
                        # pagination_condition,
                    )
                ...
                table_data = Sql.GetList(
                "select {} from SACRVC (NOLOCK) WHERE {} REPLACE(LTRIM(REPLACE(KUNAG,'0',' ')),' ','0') = '{}' ".format(
                    ", ".join(ordered_keys), where_string if where_string else "", str(account_id),str(TreeParentParam),str(ADDON_PRD_ID), contract_quote_record_id,quote_revision_record_id
                )
                )
                QueryCountObj = Sql.GetFirst("select count(*) as cnt from SACRVC(NOLOCK) LEFT JOIN SACVNT (NOLOCK) ON SACRVC.BUKRS = SACVNT.BUKRS AND SACRVC.HKONT = SACVNT.HKONT AND SACRVC.BELNR = SACVNT.BELNR AND SACRVC.ZUONR = SACVNT.ZUONR AND SACRVC.GJAHR = SACVNT.GJAHR AND SACRVC.BUZEI = SACVNT.BUZEI AND SACRVC.MANDT = SACVNT.MANDT WHERE {} REPLACE(LTRIM(REPLACE(KUNAG,'0',' ')),' ','0') = '{}'".format(where_string if where_string else "",account_id))
            ...
            if TreeParam in ("Comprehensive Services","Product Offerings","Complementary Products"):
                ...
                if get_sales_org :
                    ...
                    additional_where = " AND SALESORG_ID='{}' AND MNL_ADDON = 0 AND TRG_ADDON = 0".format(get_sales_org.SALESORG_ID)
        ...
        ##Add receiving equipment popup.....
            ...
            # elif TreeParam == 'Add-On Products':
            # 	where_string += " QUOTE_RECORD_ID = '{quo_rec_id}' AND QTEREV_RECORD_ID = '{qurev_rec_id}' AND SERVICE_ID = '{TreeSuperParentParam}' AND GREENBOOK = '{TreeParentParam}' AND {restrict_tools} QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID NOT IN (SELECT QTEREVFEQ_RECORD_ID FROM SAQSCO WHERE QUOTE_RECORD_ID = '{quo_rec_id}' and SERVICE_ID = 'Z0123' AND GREENBOOK = '{TreeParentParam}' AND QTEREV_RECORD_ID  = '{qurev_rec_id}')".format(
            ...
            # 	)
                ...
                if TreeParam == "Add-On Products":
                    #changed condition for defect #1608 issue 1
            ...
            if TreeParentParam == "Add-On Products" and TreeParam !="":
                ...
                if offset_skip_count != 0:
                    ...
                    offset_val=offset_skip_count
                # QUOTE_RECORD_ID = '{quo_rec_id}' AND QTEREV_RECORD_ID = '{qurev_rec_id}' AND SERVICE_ID = '{TreeSuperParentParam}' AND GREENBOOK = '{TreeParentParam}' AND SAQSCO.EQUIPMENT_ID NOT IN (SELECT EQUIPMENT_ID FROM SAQSCO WHERE QUOTE_RECORD_ID = '{quo_rec_id}' and SERVICE_ID ='{addon_service_id}' AND QTEREV_RECORD_ID ='{qurev_rec_id}')
                #Added EQUIPMENT_ID in sql for defect #1608
                table_data = Sql.GetList("SELECT QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID,EQUIPMENT_ID,EQUIPMENTCATEGORY_DESCRIPTION,SERIAL_NO,CUSTOMER_TOOL_ID,GREENBOOK,EQUIPMENT_STATUS,PLATFORM,FABLOCATION_ID,WARRANTY_START_DATE,WARRANTY_END_DATE,WARRANTY_END_DATE_ALERT FROM SAQSCO WHERE {where_string}  order by EQUIPMENT_ID ASC offset {offset_skip_count} rows fetch next {per_page} rows only".format(per_page=PerPage,where_string=where_string,quo_rec_id=Quote.GetGlobal("contract_quote_record_id"),addon_service_id= Product.GetGlobal("addon_service_id"),TreeParam = TreeParam,qurev_rec_id = quote_revision_record_id,offset_skip_count=offset_val,TreeSuperParentParam = TreeSuperParentParam, TreeParentParam = TreeParentParam))
            else:    
                table_data = Sql.GetList(
                    "select {} from {} (NOLOCK) {} {} {} ".format(
                        ", ".join(ordered_keys),
        ...
        # UNMAPPED EQUIPMENTS ENDED
            ...
            if Objd_Obj is not None:
                ...
                sec_str += '<div class="col-md-12 padlftrhtnone"><div class="row pad-10 bg-lt-wt brdr"> <img style="height: 40px; margin-top: -1px; margin-left: -1px; float: left;" src="/mt/APPLIEDMATERIALS_SIT/Additionalfiles/customer_info_icon.svg"/><div class="product_txt_div_child secondary_highlight" style="display: block;text-align: left;"><div class="product_txt_child"><abbr title="Key">Contacts</abbr></div><div class="product_txt_to_top_child"><abbr title="ALL">Select a valid Contract manager record below to replace it to the list of Contracts associated with your Quote</abbr></div></div></div></div>'
