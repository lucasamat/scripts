...
class EntitlementView():
    ...
    def entitlement_view(
        ...
        if EntitlementType != "NO_ENTITLEMENT":
            ...
            if GetCPSVersion :
                if GetCPSVersion.KB_VERSION is not None and GetCPSVersion.KB_VERSION != Fullresponse["kbKey"]["version"]:
                    ...
                    # sec_str += '<div id="Headerbnr" class="mart_col_back disp_blk"><div class="col-md-12" id="PageAlert_not"><div class="row modulesecbnr brdr" data-toggle="collapse" data-target="#Alert_notifcatio6" aria-expanded="true">NOTIFICATIONS<i class="pull-right fa fa-chevron-down"></i><i class="pull-right fa fa-chevron-up"></i></div><div id="Alert_notifcatio6" class="col-md-12 alert-notification brdr collapse in"><div class="col-md-12 alert-info"><label title=" Information : The quote configuration has changed. Please click on the complete stage button to regenerate the quote items"><img src="/mt/APPLIEDMATERIALS_SIT/Additionalfiles/infocircle1.svg" alt="Info"> Information : The quote configuration has changed. Please click on the complete stage button to regenerate the quote items</label></div></div></div></div>'
                else:
                    sec_str += ''
            else:
                Trace.Write("GETCPS VERSION EMPTY!")

            # sec_str += '<div id="Alert_notifcatio6" class="col-md-12 alert-notification brdr collapse in"><div class="col-md-12 alert-info"><label title=" Information : The quote configuration has changed. Please click on the complete stage button to regenerate the quote items"><img src="/mt/APPLIEDMATERIALS_SIT/Additionalfiles/infocircle1.svg" alt="Info"> Information : The quote configuration has changed. Please click on the complete stage button to regenerate the quote items</label></div></div>'
                                    ...
                                    if DType == "Drop Down":										
                                        ...
                                        if standard_attr_values and val.ENTITLEMENT_ID == str(attrSysId): 
                                            ...
                                            for value in standard_attr_values:
                                                ...
                                                #1399 ends
                                                    #Trace.Write("inside----"+str(value.SYSTEM_ID)+'--'+str(attrSysId))		 		
                                                    disallow_style = "style = 'display:none'"
                                                else:	
                                                    disallow_style = ""
        ...
        if quote_status:
            if quote_status.REVISION_STATUS in ("PRI-PRICING","APR-APPROVAL PENDING","CFG-ACQUIRING","CFG-ON HOLD - COSTING","PRR-ON HOLD PRICING","APR-RECALLED","APR-REJECTED","APR-APPROVED","OPD-PREPARING QUOTE DOCUMENTS","OPD-CUSTOMER REJECTED","OPD-CUSTOMER ACCEPTED","APR-SUBMITTED TO CUSTOMER","LGL-PREPARING LEGAL SOW","LGL-LEGAL SOW REJECTED","LGL-LEGAL SOW ACCEPTED","CBC-PREPARING CBC","CBC-CBC COMPLETED","BOK-CONTRACT CREATED","BOK-CONTRACT BOOKED","CFG-ON HOLD TKM"):
                dbl_clk_function = ""
        date_field = ""
        new_value_dict = ""
        api_name = ""
