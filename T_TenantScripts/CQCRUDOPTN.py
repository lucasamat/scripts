...
class ContractQuoteOfferingsModel(ContractQuoteCrudOpertion):
    ...
    def CreateEntitlements(self,OfferingRow_detail):		
        ...
        elif Fullresponse['complete'] == 'false' and Fullresponse['consistent'] == 'true' :
            configuration_status = 'INCOMPLETE'
        elif Fullresponse['complete'] == 'false' and Fullresponse['consistent'] == 'false' :
        ...
        if ProductVersionObj:
            ...
            Trace.Write("insert saqtse=="+str(insert_qtqtse_query))
    ...
    def _create(self):
        ...
        if self.action_type == "ADD_FAB":
            ...
            for key,val in enumerate(list(self.values)):
                ...
                #A055S000P01-20280
                if APPLIED_CREDITS:
                    total_credits_used = float(APPLIED_CREDITS[key]) + float(credit_applied)
                else:
                    Trace.Write("editwithoutAPPLIED_CREDITS")
                    total_credits_used = float(credit_applied)
                #A055S000P01-20280
                #total_credits_used = float(APPLIED_CREDITS[key]) + float(credit_applied)
                Trace.Write("total_credits_used--- "+str(total_credits_used))
                if float(total_credits_used) > float(credit_details.ZGBAL_AMT):
                    ...
                    if APPLIED_CREDITS!='':
                        ...
                        if int(credit_details.UNBL_INGL_CURR)== 0 or int(credit_details.UNBL_INGL_CURR)=='':
                            unapplied = float(credit_details.WRBTR)-float(APPLIED_CREDITS[key]) if APPLIED_CREDITS[key]!='' else float(credit_details.WRBTR)
                        else:
                            unapplied = float(credit_details.UNBL_INGL_CURR)-float(APPLIED_CREDITS[key]) if APPLIED_CREDITS[key]!='' else float(credit_details.UNBL_INGL_CURR)
                        if str(credit_details.CRTAPP_INGL_CURR) == "":
                            credit_details.CRTAPP_INGL_CURR = 0
                        Sql.RunQuery("UPDATE SACRVC SET CRTAPP_INGL_CURR = '{}', UNBL_INGL_CURR = '{}' WHERE CpqTableEntryId = '{}'".format(float(credit_details.CRTAPP_INGL_CURR)+ float(APPLIED_CREDITS[key]), unapplied, id))
                        # except Exception as e:
            ...
            if credit_applicable != "No":
                ...
                row_values = {
                    ...
                    "DIVISION_RECORD_ID": get_div_rec.DIVISION_RECORD_ID if get_div_rec.DIVISION_RECORD_ID  else ''
                }
                #2124 end
        ...
        if self.values:
            ...
            if self.all_values and auto_equp_insert is None:	     
                ...
                if self.tool_type=="TEMP_TOOL": 
                    ...
                    query_string = "SELECT EQUIPMENT_RECORD_ID FROM MAEQUP (NOLOCK) WHERE {Query_str} ACCOUNT_RECORD_ID = '{acc}' AND FABLOCATION_ID = '{fab}' AND ISNULL(SERIAL_NO, '') <> '' AND ISNULL(GREENBOOK, '') <> '' AND  EQUIPMENT_RECORD_ID NOT IN  (SELECT EQUIPMENT_RECORD_ID FROM SAQFEQ (NOLOCK) WHERE QUOTE_RECORD_ID = '{QuoteRecordId}' AND QTEREV_RECORD_ID = '{RevisionRecordId}' AND FABLOCATION_ID = '{fab}' )".format(
                            Query_str= qury_str,
                            ...
                            """
                                INSERT SAQFEQ (
                                    ...
                                    SUBSTRATE_SIZE_GROUP 
                                        ...
                                        MAEQUP.MNT_PLANT_ID,
                                        MAEQUP.MNT_PLANT_NAME,
                                        {warranty_start},NULL AS WARRANTY_END_DATE,{salesorg_id},{salesorg_name},{salesorg_recid},
                                        MAEQUP.CUSTOMER_TOOL_ID,
                                        '{UserName}' AS CPQTABLEENTRYADDEDBY,
                                        GETDATE() as CPQTABLEENTRYDATEADDED,
                                        {UserId} as CpqTableEntryModifiedBy,
                                        ...
                                        AND SYSPBT.BATCH_GROUP_RECORD_ID = '{BatchGroupRecordId}'                        
                                """.format(
                                fab_id=" '"+str(get_fab_details.FABLOCATION_ID)+"' AS FABLOCATION_ID" if self.tool_type=="TEMP_TOOL" else " MAEQUP.FABLOCATION_ID ",
                                fab_name=" '"+str(get_fab_details.FABLOCATION_NAME)+"' AS FABLOCATION_NAME" if self.tool_type=="TEMP_TOOL" else " MAEQUP.FABLOCATION_NAME ",
                                fab_recid = " '"+str(get_fab_details.FABLOCATION_RECORD_ID)+"' AS FABLOCATION_RECORD_ID" if self.tool_type=="TEMP_TOOL" else " MAEQUP.FABLOCATION_RECORD_ID ",
...
class ContractQuoteCoveredObjModel(ContractQuoteCrudOpertion):
    ...
    def _insert_quote_service_covered_object(self,record_ids='', **kwargs):		
        ...
        if self.tree_param.endswith("W"):
            ...
            """
                INSERT SAQSCO (
                    ...
                    CpqTableEntryDateModified					
                        ...
                        JOIN MAEQUP (NOLOCK) ON MAEQUP.EQUIPMENT_ID = SAQFEQ.EQUIPMENT_ID
                        WHERE 
            ...
            service_addon_object =Sql.GetList("Select SAQSAO.*,SAQTSV.QUOTE_SERVICE_RECORD_ID from SAQSAO (nolock) inner join SAQTSV on SAQTSV.QUOTE_RECORD_ID=SAQSAO.QUOTE_RECORD_ID and SAQTSV.QTEREV_RECORD_ID=SAQSAO.QTEREV_RECORD_ID and SAQTSV.SERVICE_ID = SAQSAO.ADNPRD_ID where SAQSAO.QUOTE_RECORD_ID= '{QuoteRecordId}' AND SAQSAO.QTEREV_RECORD_ID = '{RevisionRecordId}' AND SAQSAO.ACTIVE ='TRUE' AND SAQSAO.ADNPRD_ID = '{service_id}' AND SAQSAO.SERVICE_ID NOT IN (SELECT SERVICE_ID FROM SAQTSV WHERE QUOTE_RECORD_ID ='{QuoteRecordId}' AND QTEREV_RECORD_ID = '{RevisionRecordId}' AND SERVICE_ID = '{service_id}' AND SAQTSV.SERVICE_ID=SAQSAO.ADNPRD_ID)".format(QuoteRecordId=self.contract_quote_record_id,RevisionRecordId=self.quote_revision_record_id, service_id =Product.GetGlobal('addon_service_id') ))
            #tableInfo = SqlHelper.GetTable("SAQTSE")
            Trace.Write('addservice'+str(service_addon_object))
            x = datetime.datetime.today()
            x= str(x)
            y = x.split(" ")
            for OfferingRow_detail in service_addon_object:
                Trace.Write("equipments_chk_ "+str(OfferingRow_detail))
                CQADDONPRD._addon_equipment_insert(OfferingRow_detail,self.tree_parent_level_0,record_ids)
            Trace.Write("INSERT DONE FOR Z0123")
        else:
            ...
            """
                INSERT SAQSCO (
                    ...
                    CpqTableEntryDateModified
                        ...
                        WHERE 
                        SAQFEQ.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQFEQ.QTEREV_RECORD_ID = '{RevisionRecordId}'
                        AND SYSPBT.BATCH_GROUP_RECORD_ID = '{BatchGroupRecordId}' AND SAQFEQ.TEMP_TOOL = 'True')A                       
                """.format(
                        TreeParam=self.tree_param,
                        TreeParentParam=self.tree_parent_level_0,
                        QuoteRecordId=self.contract_quote_record_id,
    ...
    def _insert_quote_service_covered_assembly(self, **kwargs):
        ...
        if str(TreeParam) in ("Z0009","Z0010","Z0128"):
            ...
            if str(TreeParam) == "Z0009":
                ...
                if get_quote_type.QUOTE_TYPE!="Tool based":
                    check_assembly = True
            if check_assembly:
                #Sql.RunQuery("UPDATE SAQSCO SET IBASE_ATTSTS = 'INCOMPLETE' from ( SELECT DISTINCT B.EQUIPMENT_ID FROM SAQSCO A JOIN SAQGPA B ON B.EQUIPMENT_ID = A.EQUIPMENT_ID AND B.QUOTE_RECORD_ID = A.QUOTE_RECORD_ID AND B.QTEREV_RECORD_ID = A.QTEREV_RECORD_ID  WHERE A.QUOTE_RECORD_ID = '"+str(self.contract_quote_record_id)+"' AND A.QTEREV_RECORD_ID = '"+str(self.quote_revision_record_id)+"' AND A.SERVICE_ID = '"+str(TreeParam)+"' AND A.IBASE_ATTSTS = 'COMPLETE' AND ISNULL(B.PROCESS_TYPE,'') = '' AND ISNULL(B.DEVICE_NODE,'') = '') equp WHERE QUOTE_RECORD_ID = '"+str(self.contract_quote_record_id)+"' AND QTEREV_RECORD_ID = '"+str(self.quote_revision_record_id)+"' AND SERVICE_ID = '"+str(TreeParam)+"' AND SAQSCO.EQUIPMENT_ID = equp.EQUIPMENT_ID ")
                Sql.RunQuery("UPDATE SAQSCO SET IBASE_ATTSTS = 'INCOMPLETE' FROM (SELECT DISTINCT A.EQUIPMENT_ID FROM SAQSCA A JOIN MAEQUP B ON  B.PAR_EQUIPMENT_ID = A.EQUIPMENT_ID AND B.EQUIPMENT_ID = A.ASSEMBLY_ID WHERE A.QUOTE_RECORD_ID = '"+str(self.contract_quote_record_id)+"' AND A.QTEREV_RECORD_ID = '"+str(self.quote_revision_record_id)+"' AND A.SERVICE_ID = '"+str(TreeParam)+"' AND ISNULL(B.PROCESS_TYPE,'') = '') equp WHERE QUOTE_RECORD_ID = '"+str(self.contract_quote_record_id)+"' AND QTEREV_RECORD_ID = '"+str(self.quote_revision_record_id)+"' AND SERVICE_ID = '"+str(TreeParam)+"' AND SAQSCO.EQUIPMENT_ID = equp.EQUIPMENT_ID  AND SAQSCO.IBASE_ATTSTS = 'COMPLETE'")
        ...
        if self.tree_param:##A055S000P01-18658 code starts... calling the script to receive and update Real Time Event/KIT for the quote..
            ...
            Sql.RunQuery("""UPDATE MAEAPM_INBOUND SET PM_LEVEL = 'Scheduled Maintenance' where QUOTE_ID = '{}' and PM_LEVEL = 'Sched Maint'""".format(self.contract_quote_id))
        ...
        try:
            ...
            if self.pmevents_changes_insert == "Yes":
                ...
                self._process_query("""INSERT SAQSKP (
                        ...
                        SAQSCA.PAR_SERVICE_RECORD_ID
                        FROM SAQFEQ (NOLOCK)
                        JOIN {SAQSCA} SAQSCA (NOLOCK) ON SAQSCA.QUOTE_RECORD_ID = SAQFEQ.QUOTE_RECORD_ID AND SAQSCA.EQUIPMENT_RECORD_ID = SYSPBT.BATCH_RECORD_ID AND SAQSCA.QTEREV_RECORD_ID = SYSPBT.QTEREV_RECORD_ID
    ...
    def applied_preventive_maintainence(self, **kwargs):##function to insert got code ,events and child tables for event based and flex event based quote type...
        ...
        if(kwargs.get('applied_preventive_maintainence_quote_type_changed') == "Yes" or  kwargs.get('pmevents_changes_insert') == "Yes")and kwargs.get('tools_from_ui') != "Yes":
            ...
            if (kwargs.get('applied_preventive_maintainence_quote_type_changed') == "Yes" or kwargs.get('pmevents_changes_insert') == "Yes"):
                Trace.Write("self.contract_quote_id"+str(self.contract_quote_id)+"self.quote_revision_id"+str(self.quote_revision_id))
                ScriptExecutor.ExecuteGlobal('QTPOSTPKIT',{'QUOTE_ID':str(self.contract_quote_id),'REVISION_ID':str(self.quote_revision_id),'LEVEL':str(self.entitlement_level),'VALUE':str(self.entitlement_level_value)})
                Sql.RunQuery("""UPDATE MAEAPM_INBOUND SET PM_LEVEL = 'Scheduled Maintenance' where QUOTE_ID = '{}' and PM_LEVEL = 'Sched Maint'""".format(self.contract_quote_id))
            ...
            self._process_query(
                """INSERT SAQGPA (
                        ...
                        saqgpm_backup_table.KIT_NUMBER,
                        saqgpm_backup_table.KITNUMBER_RECORD_ID,
            ...
            Sql.RunQuery("""UPDATE SAQGPA SET FABLOCATION_ID = MAEQUP.FABLOCATION_ID,
                            ...
                            FROM SAQGPA(NOLOCK) JOIN MAEQUP(NOLOCK) ON MAEQUP.PAR_EQUIPMENT_RECORD_ID = SAQGPA.EQUIPMENT_RECORD_ID WHERE SAQGPA.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQGPA.QTEREV_RECORD_ID = '{RevisionRecordId}' AND SAQGPA.SERVICE_ID = '{TreeParam}' AND ISNULL(SAQGPA.FABLOCATION_RECORD_ID,'') = '' """.format(QuoteRecordId=self.contract_quote_record_id,RevisionRecordId=self.quote_revision_record_id,TreeParam=self.real_time_integration_service if self.real_time_integration_service else self.tree_param))
            ...
            Sql.RunQuery("""UPDATE SAQGPA
                SET PROCESS_TYPE = MAEQUP.PROCESS_TYPE,
                    ...
                    MAEQUP.EQUIPMENT_ID = SAQGPA.ASSEMBLY_ID WHERE SAQGPA.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQGPA.QTEREV_RECORD_ID = '{RevisionRecordId}' AND SAQGPA.SERVICE_ID = '{TreeParam}'  """.format(QuoteRecordId=self.contract_quote_record_id,RevisionRecordId=self.quote_revision_record_id,TreeParam=self.real_time_integration_service if self.tree_param == "Revisions" else self.tree_param))
            ...
            self._process_query("""INSERT SAQRGG (
                    ...
                    JOIN SAQSGB(NOLOCK) ON SAQSGB.QUOTE_RECORD_ID = SYSPBT.QUOTE_RECORD_ID AND SYSPBT.QTEREV_RECORD_ID = SAQSGB.QTEREV_RECORD_ID AND SAQSCO.GREENBOOK_RECORD_ID = SAQSGB.GREENBOOK_RECORD_ID
                    JOIN SAGTCD(NOLOCK) ON MAEQUP.GOT_CODE = SAGTCD.GOT_CODE
                    JOIN SAQTRV(NOLOCK) ON SAQTRV.QUOTE_RECORD_ID = SAQSCO.QUOTE_RECORD_ID AND SAQTRV.QTEREV_RECORD_ID = SAQSCO.QTEREV_RECORD_ID 
            ...
            drop_saqsco_backup_table = SqlHelper.GetFirst("sp_executesql @T=N'IF EXISTS (SELECT ''X'' FROM SYS.OBJECTS WHERE NAME= ''"+str(saqsco_backup_table)+"'' ) BEGIN DROP TABLE "+str(saqsco_backup_table)+" END  ' ")
            ##HPQC DEFECT 630 code starts...
            saqsco_temp_table_insert = SqlHelper.GetFirst("sp_executesql @T=N'SELECT SAQSCO.QUOTE_RECORD_ID,SAQSCO.QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID,SAQSCO.QTEREV_RECORD_ID,SAQSCO.QUOTE_ID,SAQSCO.QTEREV_ID,SAQSCO.SERVICE_RECORD_ID,SAQSCO.SERVICE_ID,SAQSCO.GREENBOOK,SAQSCO.GREENBOOK_RECORD_ID,SAQSCO.EQUIPMENT_ID,SAQSCO.EQUIPMENT_DESCRIPTION,SAQSCO.EQUIPMENT_RECORD_ID INTO "+str(saqsco_backup_table)+" FROM SAQSCO(NOLOCK) WHERE SAQSCO.QUOTE_RECORD_ID = ''{}'' AND SAQSCO.QTEREV_RECORD_ID = ''{}'' AND  SAQSCO.SERVICE_ID = ''{}'' '".format(self.contract_quote_record_id,self.quote_revision_record_id,self.real_time_integration_service if self.real_time_integration_service else self.tree_param))
            ##HPQC DEFECT 630 code ends...
            ...
            self._process_query(
                """INSERT SAQGPA (
                    ...
                    JOIN {saqsco_backup_table} (NOLOCK)  saqsco_backup_table ON saqgpm_backup_table.QUOTE_RECORD_ID = saqsco_backup_table.QUOTE_RECORD_ID AND saqgpm_backup_table.QTEREV_RECORD_ID = saqsco_backup_table.QTEREV_RECORD_ID AND saqgpm_backup_table.SERVICE_RECORD_ID = saqsco_backup_table.SERVICE_RECORD_ID AND SYSPBT.BATCH_RECORD_ID = saqsco_backup_table.EQUIPMENT_RECORD_ID and saqsco_backup_table.GREENBOOK = saqgpm_backup_table.GREENBOOK
                    JOIN MAEQUP(NOLOCK) ON MAEQUP.PAR_EQUIPMENT_RECORD_ID = saqsco_backup_table.EQUIPMENT_RECORD_ID AND MAEQUP.GOT_CODE = saqgpm_backup_table.GOT_CODE
                    JOIN MAEAPM_INBOUND(NOLOCK) ON MAEAPM_INBOUND.EQUIPMENT_ID = saqsco_backup_table.EQUIPMENT_ID AND MAEAPM_INBOUND.ASSEMBLY_ID = MAEQUP.EQUIPMENT_ID AND MAEAPM_INBOUND.PM_NAME = saqgpm_backup_table.PM_NAME AND MAEAPM_INBOUND.KIT_ID = saqgpm_backup_table.KIT_ID  AND MAEAPM_INBOUND.QUOTE_ID = saqsco_backup_table.QUOTE_ID AND MAEAPM_INBOUND.QTEREV_ID = saqsco_backup_table.QTEREV_ID AND MAEAPM_INBOUND.SERVICE_ID = saqsco_backup_table.SERVICE_ID AND ISNULL(MAEAPM_INBOUND.KIT_NUMBER,'') = ISNULL(saqgpm_backup_table.KIT_NUMBER,'')
                    WHERE SYSPBT.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SYSPBT.BATCH_GROUP_RECORD_ID = '{BatchGroupRecordId}' AND SYSPBT.
            ...
            if self.values:
                ...
                if self.all_values:        
                    ...
                    #ADDED FOR ADD ON PRODUCT SCENARIO
                        ...
                        # if Product.GetGlobal("addon_service_id") == "Z0123":
                        #commented codes for defect #1736
                        ...
                        # else:
                        # 	query_string = "SELECT QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID FROM SAQFEQ (NOLOCK) WHERE QUOTE_RECORD_ID = '{QuoteRecordId}' AND QTEREV_RECORD_ID = '{RevisionRecordId}' AND {Qury_Str} QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID NOT IN (SELECT QTEREVFEQ_RECORD_ID FROM SAQSCO WHERE  QUOTE_RECORD_ID = '{QuoteRecordId}' AND QTEREV_RECORD_ID = '{RevisionRecordId}' AND SERVICE_ID = '{service_id}' AND PAR_SERVICE_ID !='') ".format(QuoteRecordId=self.contract_quote_record_id,RevisionRecordId=self.quote_revision_record_id,service_id= Product.GetGlobal("addon_service_id"),TreeParam = self.tree_param,Qury_Str=qury_str)
                    elif self.tree_param == 'Z0004':
                        query_string = "SELECT QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID FROM SAQFEQ (NOLOCK) LEFT JOIN SAQSCO(NOLOCK) ON SAQSCO.EQUIPMENT_ID = SAQFEQ.EQUIPMENT_ID AND SAQSCO.TEMP_TOOL = SAQFEQ.TEMP_TOOL WHERE QUOTE_RECORD_ID = '{QuoteRecordId}' AND QTEREV_RECORD_ID = '{RevisionRecordId}' AND EQUIPMENTCATEGORY_ID = 'Y' AND {Qury_Str} SAQSCO.SERVICE_ID ='{TreeParam}' AND ISNULL(SAQSCO.EQUIPMENT_RECORD_ID,'')='' ".format(QuoteRecordId=self.contract_quote_record_id,RevisionRecordId=self.quote_revision_record_id,TreeParam = self.tree_param,Qury_Str=qury_str)
                    else:
                    ...
                    table_count_data = Sql.GetFirst(query_string_for_count)
                    Trace.Write("table_count"+str(table_count_data))
                    if table_count_data is not None:
                        table_total_rows = table_count_data.count
                    if table_total_rows:
                ...
                primaryQueryItems = SqlHelper.GetFirst(""+str(parameter.QUERY_CRITERIA_1)+" SYSPBT(BATCH_RECORD_ID, BATCH_STATUS, QUOTE_ID, QUOTE_RECORD_ID, BATCH_GROUP_RECORD_ID,QTEREV_RECORD_ID,QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID) SELECT SAQFEQ.EQUIPMENT_RECORD_ID as BATCH_RECORD_ID, ''IN PROGRESS'' as BATCH_STATUS, SAQFEQ.QUOTE_ID, SAQFEQ.QUOTE_RECORD_ID, ''"+str(batch_group_record_id)+"'' as BATCH_GROUP_RECORD_ID,''"+str(self.quote_revision_record_id)+"'' as QTEREV_RECORD_ID,SAQFEQ.QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID FROM SAQFEQ (NOLOCK) JOIN splitstring(''"+record_ids+"'') ON ltrim(rtrim(NAME)) = SAQFEQ.QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID'")
                
                Trace.Write("test"+str(parameter.QUERY_CRITERIA_1)+" SYSPBT(BATCH_RECORD_ID, BATCH_STATUS, QUOTE_ID, QUOTE_RECORD_ID, BATCH_GROUP_RECORD_ID,QTEREV_RECORD_ID,QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID) SELECT SAQFEQ.EQUIPMENT_RECORD_ID as BATCH_RECORD_ID, ''IN PROGRESS'' as BATCH_STATUS, SAQFEQ.QUOTE_ID, SAQFEQ.QUOTE_RECORD_ID, ''"+str(batch_group_record_id)+"'' as BATCH_GROUP_RECORD_ID,''"+str(self.quote_revision_record_id)+"'' as QTEREV_RECORD_ID,SAQFEQ.QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID FROM SAQFEQ (NOLOCK) JOIN splitstring(''"+record_ids+"'') ON ltrim(rtrim(NAME)) = SAQFEQ.QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID'")
                ...
                if service_entitlement_obj is not None:
                    ...
                    for value in re.finditer(pattern_tag, updateentXML):
                        ...
                        if quote_type_attribute_id:
                            ...
                            quote_type_attribute_value = quote_type_attribute_value[0].upper()
                        ...
                        if pm_event_attribute_id and self.tree_param != 'Z0009' and self.tree_param != 'Z0010':
                            ...
                            if (self.tree_param in ("Z0035","Z0091","Z0091W","Z0009","Z0004") and "Included - All PM" in pm_event_attribute_value):
...
class ContractQuoteNoficationModel(ContractQuoteCrudOpertion):
    ...
    def _get(self):
        ...
        if get_quote_id:
            ...
            # 	gettransactionmessage += '<div class="col-md-12" id="dirty-flag-warning"><div class="col-md-12 alert-warning"><label> <img src="/mt/APPLIEDMATERIALS_TST/Additionalfiles/warning1.svg" alt="Warning">The quote revision is on hold due to missing TKM Kit IDs. Your quote will automatically transition out of the hold status once all TKM Kit IDs are available in SSCM.</label></div></div>'
