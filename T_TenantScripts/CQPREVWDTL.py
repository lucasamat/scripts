...
def popups(params):
    ...
    sec_str += """<div class="drop-boxe" style="display: none;">
                ...
                <div class="col-md-2 p-0">
                    <img src="/mt/APPLIEDMATERIALS_SIT/Additionalfiles/info_icon.svg" class="img-responsive center-block">
    ...
    for sect in Oppp_SECT:
        ...
        try:
            if CurrentTabName == "Quote":
                Trace.Write("checkng_qoute"+str(Quote))
                rev_status = Sql.GetFirst("SELECT QUOTE_REVISION_RECORD_ID,REVISION_STATUS FROM SAQTRV (NOLOCK) WHERE QUOTE_RECORD_ID= '"+str(Quote)+"' AND ACTIVE = 1")
        except Exception, e:
        ...
        if sec_html_btn is not None:
            ...
            if CurrentTabName == "Quote":
                if rev_status.REVISION_STATUS == "CFG-CONFIGURING":
                    pass
                else:
                    edit_action = ''
            else:
        ...
        for sefl in Oppp_SEFL:
            ...
            if col_name:
                ...
                ##to get date from datetime for CONTRACT_VALID_FROM and CONTRACT_VALID_TO ends
                    ...
                    try:
                        ...
                        sec_str += (
                            "<div class='col-md-3 pad-0'> <textarea type='text' id ='"+str(sefl_api)+"' title = '"+  eval("col_name." + str(sefl_api))+"' value = '"
                            + eval("col_name." + str(sefl_api))
                            + "' 'title':userInput}, incrementalTabIndex, enable: isEnabled' class='form-control txtArea remove_yellow bor-left bor-right' style='height: 28px;border-top: 0 !important;border-bottom: 1px solid #ddd !important;' id='' title='' tabindex='' disabled=''>"
                            + eval("col_name." + str(sefl_api))
                            + "</textarea></div>"
            ...
            permission_chk_query = Sql.GetFirst("""SELECT DISTINCT SYOBJD.OBJECT_NAME, SYOBJD.FIELD_LABEL,case when SYOBJD.EDITABLE_ONINSERT ='TRUE' then 'EDITABLE' 
                Else 'READ ONLY' end AS PERMISSION 
                FROM SYOBJD (NOLOCK)
                ...
                WHERE SYSEFL.RECORD_ID = '{0}' AND SYSEFL.SECTION_RECORD_ID = '{2}'""".format(str(sefl.RECORD_ID), str(User.Id),str(sect.RECORD_ID)))
...
def constructCBC(Qt_rec_id, Quote, MODE):
    ...
    get_dynamic_div =Sql.GetFirst("SELECT REVISION_STATUS FROM SAQTRV(NOLOCK) WHERE QUOTE_RECORD_ID = '{quote_recid}' AND QTEREV_RECORD_ID = '{quote_revision_recid}' AND REVISION_STATUS IN ('CBC-CBC COMPLETED','BOK-CONTRACT CREATED','BOK-CONTRACT BOOKED') ".format(quote_recid=Quote,quote_revision_recid=quote_revision_record_id))
    get_trans_type = Sql.GetFirst("SELECT TRANSACTION_TYPE FROM SAQTRV(NOLOCK) WHERE QUOTE_RECORD_ID = '{quote_recid}' AND QTEREV_RECORD_ID = '{quote_revision_recid}' AND TRANSACTION_TYPE IN ('A-QUOTE') ".format(quote_recid=Quote,quote_revision_recid=quote_revision_record_id))
    get_poes_type =Sql.GetFirst("SELECT POES FROM SAQTMT WHERE MASTER_TABLE_QUOTE_RECORD_ID  = '{quote_recid}' AND QTEREV_RECORD_ID = '{quote_revision_recid}' AND POES IN ('True') ".format(quote_recid=Quote,quote_revision_recid=quote_revision_record_id))
    saqrit_status = []
    saqico_status = []
    status_query = Sql.GetList("SELECT SAQRIT.STATUS AS SAQRIT_STATUS,SAQICO.STATUS AS SAQICO_STATUS FROM SAQRIT (NOLOCK) INNER JOIN SAQICO (NOLOCK) ON SAQRIT.QUOTE_RECORD_ID = SAQICO.QUOTE_RECORD_ID AND SAQRIT.QTEREV_RECORD_ID = SAQICO.QTEREV_RECORD_ID  WHERE SAQRIT.QUOTE_RECORD_ID = '{quote_rec_id}' AND SAQRIT.QTEREV_RECORD_ID = '{quote_rev_rec_id}'".format(quote_rec_id = Quote,quote_rev_rec_id = quote_revision_record_id))
    revision_query =  Sql.GetFirst("SELECT * FROM SAQTRV (NOLOCK) WHERE QUOTE_RECORD_ID = '" + str(Quote) + "' AND QTEREV_RECORD_ID = '" + str(quote_revision_record_id) + "' ")
    ...
    for sect in dynamic_sect:
        if get_dynamic_div:
            ...
            static =''
        elif "CFG-ON HOLD TKM" in saqrit_status or "CFG-ON HOLD TKM" in saqico_status:
            static = ''
        else:
            if (str(revision_query.REVISION_STATUS).upper() in ("CBC-CBC COMPLETED","BOK-CONTRACT CREATED","BOK-CONTRACT BOOKED","LGL-LEGAL SOW REJECTED","LGL-PREPARING LEGAL SOW")) or (str(revision_query.REVISION_STATUS).upper() in ("CFG-ACQUIRING","PRR-ON HOLD PRICING","CFG-ON HOLD - COSTING","PRI-PRICING") and str(TreeParam).upper() not in ("QUOTE ITEMS","BILLING","APPROVALS","QUOTE DOCUMENTS")) or (str(revision_query.REVISION_STATUS).upper() in ("APR-APPROVED","APR-REJECTED","APR-RECALLED","APR-APPROVAL PENDING","OPD-PREPARING QUOTE DOCUMENTS","OPD-CUSTOMER ACCEPTED","OPD-CUSTOMER REJECTED") and str(TreeParam).upper() not in ("APPROVALS","QUOTE DOCUMENTS")):
                static = ""
            else:
                if (str(revision_query.REVISION_STATUS).upper() in("LGL-LEGAL SOW ACCEPTED","CBC-PREPARING CBC")):
                    static = '<div id="ctr_drop" class="btn-group dropdown cbc_ctr_drop"><div class="dropdown"><i data-toggle="dropdown" class="fa fa-sort-desc dropdown-toggle"></i><ul class="dropdown-menu left" aria-labelledby="dropdownMenuButton"><li class="edit_list"><a id="'+str(sect.RECORD_ID)+'" class="dropdown-item" href="#" onclick="cbcEDIT(this)">EDIT</a></li></ul></div></div>'
                else:
                    static = ''
        sec_str += '<div id="container" class="wdth100 margtop10 ' + str(sect.RECORD_ID) + '">'
        sec_str += '<div onclick="dyn_main_sec_collapse_arrow(this)" data-target="" id="dyn1577" data-toggle="collapse" class="g4 dyn_main_head master_manufac add_level glyphicon glyphicon-chevron-down pointer"><label data-bind="html: hint" class="onlytext"><div class="height_auto">'+str(static)+''+str(sect.SECTION_NAME)+'</div></label> </div>'
        ...
        checklist_vals = Sql.GetList("select TOP 1000 CHECKLIST_ID,CHECKLIST_DESCRIPTION,SERVICE_CONTRACT,SPECIALIST_REVIEW,COMMENT FROM SAQCBC(NOLOCK) WHERE QUOTE_RECORD_ID = '{quote_recid}' AND QTEREV_RECORD_ID = '{quote_revision_recid}' ORDER BY convert(float,CHECKLIST_ID) ASC".format(quote_recid=Quote,quote_revision_recid=quote_revision_record_id))
        for value in checklist_vals:
            ...
            elif sect.SECTION_NAME =="IF THE CUSTOMER HAS A MASTER SERVICE AGREEMENT" and str(value.CHECKLIST_ID) in ['5','6','7','8','9','10','11','11.1','11.2','11.3']:
                if '.' in str(value.CHECKLIST_ID):				
                    sec_str +='<tr class ="cbc_child">'
                    sec_str += ('<td><input id="CHECKLIST_ID" type="text" value="'+str(value.CHECKLIST_ID)+'" title="'+str(value.CHECKLIST_ID)+'" class="form-control related_popup_css fltlt" disabled></td>')
                    sec_str += ('<td><abbr id="CHECKLIST_DESCRIPTION" title="'+str(value.CHECKLIST_DESCRIPTION)+'" class="form-control related_popup_css fltlt" disabled>'+str(value.CHECKLIST_DESCRIPTION)+'</abbr></td>')
            ...
            elif sect.SECTION_NAME =="VALIDATE CANCELLATION FOR CONVENIENCE DAYS IN CRM ARE RECORDED ACCORDING TO TS&CS IN THE BOOKING PACKAGE.  DO NOT INCLUDE POSS CANCELLATION TERMS IN EITHER OF THE CPQ FIELDS.  MAKE ENTRIES BASED ON THE FOLLOWING RULES:" and str(value.CHECKLIST_ID) in ['12','13','14','15','16','17']:
                if '.' in str(value.CHECKLIST_ID):				
                    sec_str +='<tr class ="cbc_child">'
                    sec_str += ('<td><input id="CHECKLIST_ID" type="text" value="'+str(value.CHECKLIST_ID)+'" title="'+str(value.CHECKLIST_ID)+'" class="form-control related_popup_css fltlt" disabled></td>')
                    sec_str += ('<td><abbr id="CHECKLIST_DESCRIPTION" title="'+str(value.CHECKLIST_DESCRIPTION)+'" class="form-control related_popup_css fltlt" disabled>'+str(value.CHECKLIST_DESCRIPTION)+'</abbr></td>')
    ...
    Section = Sql.GetFirst(
            "SELECT RECORD_ID,SECTION_NAME FROM SYSECT (NOLOCK) WHERE PRIMARY_OBJECT_NAME = 'SAQCBC' AND SECTION_NAME = 'AUDIT INFORMATION' "
        )
   
    sec_str += '<div id = "cbc_notes"></div><div id="container" class="wdth100 margtop10 g4 audit_sect ' + str(Section.RECORD_ID) + '">'
    sec_str += (
        '<div class="dyn_main_head master_manufac glyphicon pointer   glyphicon-chevron-down" onclick="dyn_main_sec_collapse_arrow(this)" data-target=".sec_'
        + str(Section.RECORD_ID)
        + '" data-toggle="collapse"><label class="onlytext"><label class="onlytext"><div>'
    ...
    Oppp_SEFL = Sql.GetList(
        "SELECT TOP 1000 FIELD_LABEL, API_FIELD_NAME,API_NAME FROM SYSEFL (NOLOCK) WHERE SECTION_RECORD_ID = '" + str(Section.RECORD_ID) + "' ORDER BY DISPLAY_ORDER"
    )
    for sefl in Oppp_SEFL:
        sec_str += '<div id="sec_' + str(Section.RECORD_ID) + '" class= "sec_' + str(Section.RECORD_ID) + ' collapse in">'
        sec_str += "<div style='height:30px;border-left: 0;border-right: 0;border-bottom:1px solid  #dcdcdc;' data-bind='attr: {'id':'mat'+stdAttrCode(),'class': isWholeRow() ? 'g4  except_sec removeHorLine iconhvr' : 'g1 except_sec removeHorLine iconhvr' }' id='mat1578' class='g4  except_sec removeHorLine iconhvr'>"
        sec_str += (
            "<div class='col-md-5'> <abbr data-bind='attr:{'title':label}' title='"
            + str(sefl.FIELD_LABEL)
        ...
        syobjd_obj = Sql.GetFirst("SELECT DATA_TYPE FROM SYOBJD (NOLOCK) WHERE API_NAME = '{}' and OBJECT_NAME ='{}'".format(sefl_api,object_name))
        data_type = syobjd_obj.DATA_TYPE
        col_name = Sql.GetFirst("SELECT TOP 1 * FROM SAQCBC (NOLOCK) WHERE QUOTE_RECORD_ID = '" + str(Quote) + "' ORDER BY CpqTableEntryDateModified DESC")
        if col_name:
            if sefl_api == "CpqTableEntryModifiedBy":
                current_obj_value = col_name.CpqTableEntryModifiedBy
                current_user = Sql.GetFirst(
                ...
                current_user = current_user.USERNAME if current_user else ""
                sec_str += (
                    "<div class='col-md-3 pad-0'> <input type='text' value = '"
                    + str(current_user)
                    + "' 'title':userInput}, incrementalTabIndex, enable: isEnabled' class='form-control' style='height: 28px;border-top: 0 !important;border-bottom: 0 !important;' id='' title='' tabindex='' disabled=''> </div>"
...
def editcbc(Qt_rec_id, Quote, MODE):
    ...
    get_all_checklist=Sql.GetFirst("SELECT COUNT(*) as cnt FROM SAQCBC (NOLOCK) WHERE QUOTE_RECORD_ID = '{quote_rec_id}' AND QTEREV_RECORD_ID = '{quote_rev_recid}' AND SERVICE_CONTRACT=0 AND SPECIALIST_REVIEW=0 AND CHECKLIST_ID NOT IN('4.1','4.2','4.3','4.4','11.1','11.2','11.3','28.1','28.2','28.3')".format(quote_rec_id = Quote,quote_rev_recid = quote_revision_record_id))
...
def countcbc(Qt_rec_id, Quote, MODE):
    popupquery=Sql.GetFirst("SELECT COUNT(*) as cnt FROM SAQCBC (NOLOCK) WHERE QUOTE_RECORD_ID = '{quote_rec_id}' AND QTEREV_RECORD_ID = '{quote_rev_recid}' AND SERVICE_CONTRACT='False' AND SPECIALIST_REVIEW='FALSE' AND CHECKLIST_ID NOT IN('4.1','4.2','4.3','4.4','11.1','11.2','11.3','28.1','28.2','28.3')".format(quote_rec_id = Quote,quote_rev_recid = quote_revision_record_id))
    popupquery_value = popupquery.cnt
    #A055S000P01-17166 start
...
def constructlegalsow(Qt_rec_id, Quote, MODE):    
    ...
    for sect in Oppp_SECT:
        ...
        for sefl in Oppp_SEFL:
            ...
            if permission_chk_query:
                if str(permission_chk_query.PERMISSION) == "EDITABLE" and str(col_name.REVISION_STATUS).upper() != "APR-APPROVED" and str(col_name.REVISION_STATUS).upper() != "PRI-PRICING" and str(col_name.REVISION_STATUS).upper() != "CFG-ACQUIRING" and str(col_name.REVISION_STATUS).upper() != "CFG-ON HOLD - COSTING"and str(col_name.REVISION_STATUS).upper() != "PRR-ON HOLD PRICING"and str(col_name.REVISION_STATUS).upper() != "APR-APPROVAL PENDING"and str(col_name.REVISION_STATUS).upper() != "OPD-PREPARING QUOTE DOCUMENTS" and str(col_name.REVISION_STATUS).upper() != "OPD-CUSTOMER ACCEPTED" and str(col_name.REVISION_STATUS).upper() != "LGL-LEGAL SOW REJECTED" and str(col_name.REVISION_STATUS).upper() != "LGL-LEGAL SOW ACCEPTED"and str(col_name.REVISION_STATUS).upper() != "CBC-PREPARING CBC"and str(col_name.REVISION_STATUS).upper() != "CBC-CBC COMPLETED"and str(col_name.REVISION_STATUS).upper() != "BOK-CONTRACT CREATED"and str(col_name.REVISION_STATUS).upper() != "BOK-CONTRACT BOOKED":
                    edit_lock_icon = "fa fa-pencil"
                else:
                    edit_lock_icon = "fa fa-lock"  
            else:
