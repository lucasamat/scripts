...
if str(TempTable_Seq) != 'None':
	...
	try:
		...
		if Check_flag == 1:
			...
			""
			+ str(Parameter1.QUERY_CRITERIA_1)
			+ "  A SET EXCHANGE_RATE = 1 / CONVERT(DECIMAL(13,5),REPLACE(EXCHANGE_RATE,''-'','''')) FROM "+str(Table_Name)+" (NOLOCK) A WHERE  EXCHANGE_RATE LIKE ''%-%'' '")
			...
			primaryQueryItems = SqlHelper.GetList("select distinct EXCHANGE_RATE_DATE AS sap_part_number,INTEGRATION_STATUS from "+str(Table_Name)+" (nolock) where isnull(INTEGRATION_STATUS,'')<>'' AND   timestamp = "+str(timestamp_sessionid)+" ")
			...
			if len(primaryQueryItems) > 0:
				...
				if len(Error_list) > 0:
					...
					msg.Subject = "AMAT INTEGRATION MATMAS EXCEPTIONS - CPQ(T-Tenant)"
					msg.IsBodyHtml = True
					msg.Body = Error_Info
