...
def insert_quote_billing_plan():
            ...
            if item_billing_plan_obj_year.BILLING_YEAR:
                ...
                if item_billing_plans_obj:
                    ...
                    for service_obj in services_obj:
                        ...
                        get_billing_type = Sql.GetFirst("SELECT BILLING_TYPE from SAQRIT where SERVICE_ID = '{}' and QUOTE_RECORD_ID = '{}' AND SERVICE_ID NOT IN ('Z0101','A6200','Z0108','Z0110')".format(service_obj.SERVICE_ID,contract_quote_record_id))
    ...
    if SUM_YEAR2:
        M1_Y2 =''
        Trace.Write('year 2--405------>'+str(SUM_YEAR2.MONTH_1))
...
def _insert_subtotal_by_offerring_quote_table():
    ...
    #QC 213,452 start
    get_quotetotal = Sql.GetFirst("SELECT SALESORG_ID,BANK_NAME,INCOTERM_NAME,NET_VALUE_INGL_CURR as netprice,ESTVAL_INGL_CURR as est_val,TAX_AMOUNT_INGL_CURR as taxtotal,TOTAL_AMOUNT_INGL_CURR as totamt,DOC_CURRENCY from SAQTRV where QUOTE_RECORD_ID = '{contract_quote_record_id}' and QTEREV_RECORD_ID ='{quote_revision_record_id}' ".format(contract_quote_record_id=contract_quote_record_id,quote_revision_record_id=quote_revision_record_id))
    get_total_doc_val = Sql.GetFirst("SELECT SUM(ESTIMATED_VALUE) as est_vals,SUM(NET_VALUE) as net_val,SUM(TOTAL_AMOUNT)  as tot_quote,SUM(TAX_AMOUNT) as tax_amt  from SAQRIT where QUOTE_RECORD_ID = '{contract_quote_record_id}' and QTEREV_RECORD_ID ='{quote_revision_record_id}' ".format(contract_quote_record_id=contract_quote_record_id,quote_revision_record_id=quote_revision_record_id))
    ...
    if get_address_details:
        ...
        if get_address_details.DESCRIPTION:
            Quote.GetCustomField('QT_SA_COMCODE').Content =  str(get_address_details.DESCRIPTION).title()
        else:
    ...
    if get_quotetotal:
        ...
        Quote.GetCustomField('doc_net_price').Content = str(get_total_doc_val.net_val)
        Quote.GetCustomField('tot_est').Content = str(get_total_doc_val.est_vals)
        if str(get_total_doc_val.tax_amt):
            Quote.GetCustomField('taxtotal').Content = str(get_total_doc_val.tax_amt)
        if str(get_total_doc_val.tot_quote):
            Quote.GetCustomField('TL_NET_VALUE').Content = str(get_total_doc_val.tot_quote)
...
def language_select():
    ...
    if get_service_details:
        ...
        get_fpm_service = ''
    if get_quote_status:
        if str(get_quote_status.REVISION_STATUS).upper() in ("APR-APPROVED","OPD-PREPARING QUOTE DOCUMENTS","OPD-CUSTOMER ACCEPTED","APR-SUBMITTED TO CUSTOMER"):
            Trace.Write("If")
...
if str(parts_list) == 'True' and str(billing_matrix) == 'True':
    ...
    Trace.Write('881--1302-FPPM QUote--')
    Quote.GetCustomField('INCLUDE_ITEMS').Content = 'YES'
    Quote.GetCustomField('ITEM_DELIVERY_SCHEDULE').Content = 'YES'
    Quote.GetCustomField('PARTS_DELIVERY_SCHEDULE').Content = 'YES'
    ApiResponse = ApiResponseFactory.JsonResponse(fpm_quote_doc())
