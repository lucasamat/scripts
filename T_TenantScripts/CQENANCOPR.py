...
class AncillaryProductOperation:
    def __init__(self):		
        ...
        self.equip_insert_flag = equip_insert_flag #2127 
    ...
    def _do_opertion(self):
        if self.action_type == "INSERT_SERVICE":
            ...
            #self._insert_fab()
            #2127 starts
            if self.equip_insert_flag != 'True':
                self._insert_grn()
            #2127 ends
    ...
    def _insert_service_offering(self):
        material_obj = Sql.GetFirst("SELECT MATERIAL_RECORD_ID,SAP_DESCRIPTION,MATERIALCONFIG_TYPE FROM MAMTRL (NOLOCK) WHERE SAP_PART_NUMBER = '{}'".format(self.ancillary_obj))
        get_existing_record = Sql.GetFirst("SELECT count(CpqTableEntryId) as cnt FROM SAQTSV (NOLOCK) WHERE QUOTE_RECORD_ID = '{}' AND QTEREV_RECORD_ID = '{}' AND SERVICE_ID ='{}' AND PAR_SERVICE_ID = '{}'".format(self.contract_quote_record_id, self.contract_quote_revision_record_id ,self.ancillary_obj, self.service_id))
    ...
    def _insert_grn(self):
        addtional_where = ""
        get_service_details = Sql.GetFirst("SELECT * FROM SAQTSV (NOLOCK) WHERE QUOTE_RECORD_ID = '{}' AND QTEREV_RECORD_ID = '{}' AND SERVICE_ID ='{}' AND PAR_SERVICE_ID = '{}'".format(self.contract_quote_record_id, self.contract_quote_revision_record_id ,self.ancillary_obj, self.service_id))
        #if self.fab:
    ...
    def _equipment_insert(self):
        ...
        #2127 starts
        if self.equip_insert_flag == 'True':
            existing_greenbook_query = Sql.GetList("SELECT * FROM SAQSGB (NOLOCK) WHERE QUOTE_RECORD_ID = '{}' AND QTEREV_RECORD_ID = '{}' AND SERVICE_ID ='{}' AND PAR_SERVICE_ID = '{}'".format(self.contract_quote_record_id, self.contract_quote_revision_record_id ,self.ancillary_obj, self.service_id))
            existing_greenbook_list = str(tuple(set([i.GREENBOOK for i in existing_greenbook_query]))).replace(",)",')')
            addtional_where = " AND GREENBOOK in {}".format(existing_greenbook_list)
        #2127 ends
        Sql.RunQuery(
                """
                INSERT SAQSCO (
    ...
    def _insert_service_ent(self):
        get_service_details = Sql.GetList("SELECT SAQTSV.* FROM SAQTSV (NOLOCK) INNER JOIN MAMTRL (NOLOCK) ON SAP_PART_NUMBER = SERVICE_ID WHERE QUOTE_RECORD_ID = '{}' AND QTEREV_RECORD_ID = '{}'  AND PAR_SERVICE_ID = '{}' AND MATERIALCONFIG_TYPE != 'SIMPLE MATERIAL'".format(self.contract_quote_record_id, self.contract_quote_revision_record_id , self.service_id))
        # anc_insert_dict ={}
        for addon in get_service_details:
            ...
            get_existing_record = Sql.GetFirst("SELECT count(CpqTableEntryId) as cnt FROM SAQTSE (NOLOCK) WHERE QUOTE_RECORD_ID = '{}' AND QTEREV_RECORD_ID = '{}' AND SERVICE_ID ='{}' AND PAR_SERVICE_ID = '{}'".format(self.contract_quote_record_id, self.contract_quote_revision_record_id ,addon.SERVICE_ID, self.service_id))
                ...
                for attrs in overallattributeslist:
                    ...
                    if PRODUCT_ATTRIBUTES:
                        ...
                        if PRODUCT_ATTRIBUTES.ATT_DISPLAY_DESC in ('Drop Down','Check Box') and ent_disp_val:
                            ...
                            get_display_val = Sql.GetFirst("SELECT STANDARD_ATTRIBUTE_DISPLAY_VAL  from STANDARD_ATTRIBUTE_VALUES (NOLOCK) S INNER JOIN ATTRIBUTE_DEFN (NOLOCK) A ON A.STANDARD_ATTRIBUTE_CODE=S.STANDARD_ATTRIBUTE_CODE WHERE S.STANDARD_ATTRIBUTE_CODE = '{}' AND A.SYSTEM_ID = '{}' AND S.STANDARD_ATTRIBUTE_VALUE = '{}' ".format(STANDARD_ATTRIBUTE_VALUES.STANDARD_ATTRIBUTE_CODE,attrs,  attributevalues[attrs] ) )
    ...
    def _update_entitlement_values(self,anc_service = '',ent_table = ''):
        try:
            ...
            check_ancillary = Sql.GetFirst("SELECT COUNT(SERVICE_ID) AS CNT FROM SAQTSE (NOLOCK) WHERE QUOTE_RECORD_ID ='{}' AND QTEREV_RECORD_ID = '{}' AND PAR_SERVICE_ID ='{}' AND SERVICE_ID ='{}'".format(self.contract_quote_record_id, self.contract_quote_revision_record_id ,self.service_id, anc_service) )
            if check_ancillary.CNT > 0 and self.tablename == 'SAQSGE':
                get_parent_xml = Sql.GetList("SELECT * FROM {} (NOLOCK) WHERE {}".format(ent_table, self.where_string) )
                get_anc_xml_dict = {}
                for parent in get_parent_xml:
                    ...
                    getall_recid = Sql.GetFirst("SELECT * FROM {} (NOLOCK) WHERE QUOTE_RECORD_ID ='{}' AND QTEREV_RECORD_ID = '{}' AND SERVICE_ID ='{}' AND PAR_SERVICE_ID = '{}' AND GREENBOOK = '{}' {}".format(ent_table,self.contract_quote_record_id, self.contract_quote_revision_record_id, anc_service ,self.service_id, parent.GREENBOOK, joinstr) )
                    if getall_recid:
    ...
    def service_rollup(self,anc_service,AttributeID,NewValue):	
        ...
        try:
            get_greenbook_value = Sql.GetList("SELECT * FROM SAQSGE (NOLOCK) WHERE QUOTE_RECORD_ID ='{}' AND QTEREV_RECORD_ID = '{}'  AND SERVICE_ID = '{}'".format(self.contract_quote_record_id, self.contract_quote_revision_record_id,self.service_id) )
            ...
            if self.tablename == 'SAQSGE' and len(get_greenbook_value) > 0:
                ...
                if flag_list :
                    ...
                    elif anc_service == 'Z0100' and AttributeID == 'AGS_Z0100_PQB_QTITST' and 'STR-OFFBGB OBJ-GREQ' in  flag_list:
                        NewValue = 'STR-OFFBGB OBJ-GREQ'
    ...
    def _update_service_entitlement(self):
        ...
        try:
            ...
            get_c4c_quote_id = Sql.GetFirst("select * from SAQTMT (NOLOCK) where MASTER_TABLE_QUOTE_RECORD_ID = '{}' AND QTEREV_RECORD_ID = '{}'".format(self.contract_quote_record_id,self.contract_quote_revision_record_id))
            ent_temp = "ENT_BKP_"+str(get_c4c_quote_id.C4C_QUOTE_ID)
            ent_temp_drop = Sql.GetFirst("sp_executesql @T=N'IF EXISTS (SELECT ''X'' FROM SYS.OBJECTS WHERE NAME= ''"+str(ent_temp)+"'' ) BEGIN DROP TABLE "+str(ent_temp)+" END  ' ")
            where_cond = "QUOTE_RECORD_ID = '{}' AND QTEREV_RECORD_ID = '{}' AND SERVICE_ID = '{}'".format(self.contract_quote_record_id,self.contract_quote_revision_record_id , self.service_id)
            where_cond = where_cond.replace("'","''")
            ...
            GetXMLsecField=Sql.GetList("SELECT * FROM {} (NOLOCK) ".format(ent_temp))
                ...
                if val.ENTITLEMENT_ID:
                    ...
                    if val.ENTITLEMENT_DISPLAY_VALUE:
                        try:
                            ...
                            if str(val.ENTITLEMENT_ID) == "AGS_Z0046_PQB_PPCPRM" :
                            ...
                            elif str(val.ENTITLEMENT_ID) in ('AGS_Z0100_PQB_QTETYP','AGS_Z0100_PQB_QTITST','AGS_Z0100_PQB_BILTYP'):
                                ServiceId = 'Z0100'
                                ...
                                if 'AGS_Z0046' in val.ENTITLEMENT_ID:
                                    ...
                                    AttributeID_Pass = val.ENTITLEMENT_ID
                                #1356 STARTS
                                elif 'AGS_Z0103' in val.ENTITLEMENT_ID:
                                    ServiceId = 'Z0103'
                                    AttributeID_Pass = val.ENTITLEMENT_ID
                                #1356 ENDS
    ...
    def _delete_entitlement_tables_anc(self):
        #Log.Info("delete_obj_list--"+str(self.tablename))
        delete_obj_list =[]
        ...
        for obj in delete_obj_list:
            Sql.RunQuery("DELETE FROM {obj} WHERE {where} AND PAR_SERVICE_ID IN (SELECT SERVICE_ID FROM {obj} (NOLOCK)  WHERE {par_where}) AND SERVICE_ID NOT IN (SELECT ADNPRD_ID FROM SAQSAO (NOLOCK) WHERE QUOTE_RECORD_ID='{quote_rec_id}' and SERVICE_ID = '{service_id}' AND QTEREV_RECORD_ID = '{revision_rec_id}' )".format(obj = obj, where=  ancillary_where, par_where = self.where_string ,quote_rec_id= self.contract_quote_record_id,revision_rec_id = self.contract_quote_revision_record_id,service_id = self.service_id ))
    ...
    def _entitlement_rolldown(self):
        try:
            ...
            get_ancillaryservice = Sql.GetFirst("select * from SAQTSE (NOLOCK) WHERE QUOTE_RECORD_ID = '{}' AND QTEREV_RECORD_ID = '{}' AND PAR_SERVICE_ID = '{}'".format(self.contract_quote_record_id, self.contract_quote_revision_record_id , self.service_id))
            ...
            if get_ancillaryservice :				
                ...
                qtqsge_query_anc= """INSERT SAQSGE (KB_VERSION,QUOTE_ID,QUOTE_NAME,QUOTE_RECORD_ID,QTEREV_RECORD_ID,QTEREV_ID,SERVICE_DESCRIPTION,SERVICE_ID,SERVICE_RECORD_ID,SALESORG_ID,SALESORG_NAME,SALESORG_RECORD_ID,	
                    ...
                    JOIN SAQSGB (NOLOCK) ON SAQTSE.PAR_SERVICE_ID = SAQSGB.PAR_SERVICE_ID  AND SAQTSE.QUOTE_RECORD_ID = SAQSGB.QUOTE_RECORD_ID AND SAQSGB.QTEREV_RECORD_ID = SAQTSE.QTEREV_RECORD_ID AND SAQTSE.SERVICE_ID = SAQSGB.SERVICE_ID
                    ...
                    JOIN SAQSGE (NOLOCK) ON SAQSGE.SERVICE_ID = SAQSCO.PAR_SERVICE_ID AND SAQSCO.QUOTE_RECORD_ID = SAQSGE.QUOTE_RECORD_ID AND SAQSCO.QTEREV_RECORD_ID = SAQSGE.QTEREV_RECORD_ID AND SAQSGB.GREENBOOK_RECORD_ID = SAQSGE.GREENBOOK_RECORD_ID
                ...
                qtqsce_anc_query="""
                    ...
                    JOIN SAQSGE (NOLOCK) ON SAQSCO.PAR_SERVICE_ID = SAQSGE.PAR_SERVICE_ID AND SAQSCO.SERVICE_ID = SAQSGE.SERVICE_ID AND SAQSCO.QUOTE_RECORD_ID = SAQSGE.QUOTE_RECORD_ID  AND SAQSCO.QTEREV_RECORD_ID = SAQSGE.QTEREV_RECORD_ID AND SAQSCO.GREENBOOK = SAQSGE.GREENBOOK
                    JOIN SAQSCE (NOLOCK) ON SAQTSE.PAR_SERVICE_ID = SAQSCE.SERVICE_ID AND SAQSCO.QUOTE_RECORD_ID = SAQSCE.QUOTE_RECORD_ID AND SAQSCO.QTEREV_RECORD_ID = SAQSCE.QTEREV_RECORD_ID  AND SAQSCO.EQUIPMENT_ID = SAQSCE.EQUIPMENT_ID
                        ...
                        for offering in get_ancillary:
                            ...
                            for rec_table in ['SAQSGE','SAQSCE','SAQGPE','SAQTSE','SAQSAE']:
                                ScriptExecutor.ExecuteGlobal("CQENTLNVAL", {'action':'ENTITLEMENT_COLUMN_UPDATE',
                                                                            'partnumber':offering.SERVICE_ID,
    ...
    def _delete_entitlement_tables(self):
        ...
        ancillary_where = re.sub(r'AND SERVICE_ID\s*\=\s*\'[^>]*?\'', '', self.where_string )
        for obj in delete_obj_list:
            delete_query = "DELETE FROM {obj} WHERE {where} AND PAR_SERVICE_ID IN (SELECT SERVICE_ID FROM {obj} (NOLOCK) WHERE {par_where} AND CONFIGURATION_STATUS ='INCOMPLETE') AND SERVICE_ID NOT IN (SELECT ADNPRD_ID FROM SAQSAO (NOLOCK) WHERE QUOTE_RECORD_ID='{quote_rec_id}' and SERVICE_ID = '{service_id}' AND QTEREV_RECORD_ID = '{revision_rec_id}' ); ".format(obj = obj, where=  ancillary_where, par_where = self.where_string, quote_rec_id=self.contract_quote_record_id,revision_rec_id = self.contract_quote_revision_record_id, service_id = self.service_id )
            QueryResult += delete_query
        QueryResult += "commit;"
        Log.Info("_delete_entitlement_tables QueryResult--> "+str(QueryResult))
    ...
    def _delete_operation(self):
        ...
        for obj in delete_obj_list:
            ...
            delete_query = "DELETE FROM {} WHERE {} AND SERVICE_ID = '{}' AND SERVICE_ID NOT IN (SELECT ADNPRD_ID FROM SAQSAO (NOLOCK) WHERE QUOTE_RECORD_ID='{}' and SERVICE_ID = '{}' AND QTEREV_RECORD_ID = '{}' ); ".format(obj,ancillary_where,self.ancillary_obj, self.contract_quote_record_id, self.service_id,self.contract_quote_revision_record_id )
            QueryResult += delete_query
        QueryResult += "commit;"
        Log.Info("_delete_operation QueryResult--> "+str(QueryResult))
        Sql.RunQuery(QueryResult)
        
        ##deleting higher table records based on below level 
        if self.tablename != "SAQTSE":
            obj_list = ["SAQSCA","SAQSCO","SAQSGB"]
            QueryResult1 = "begin transaction;"
            for rec in obj_list:
                if rec == "SAQSCA":
                ...
                if rec == "SAQSCA" and get_count.cnt == 0:
                    for del_obj in ['SAQSCO','SAQSCE']:
                        delete_query = "DELETE FROM {} WHERE QUOTE_RECORD_ID = '{}' AND QTEREV_RECORD_ID = '{}' AND SERVICE_ID ='{}' AND PAR_SERVICE_ID = '{}' {}; ".format(del_obj,self.contract_quote_record_id, self.contract_quote_revision_record_id ,self.ancillary_obj, self.service_id,addtional_where )
                        QueryResult1 += delete_query
                    ...
                    for del_obj in ['SAQTSE','SAQTSV']:
                        delete_query = "DELETE FROM {} WHERE QUOTE_RECORD_ID = '{}' AND QTEREV_RECORD_ID = '{}' AND SERVICE_ID ='{}' AND PAR_SERVICE_ID = '{}' ; ".format(del_obj,self.contract_quote_record_id, self.contract_quote_revision_record_id ,self.ancillary_obj, self.service_id )
                        QueryResult1 += delete_query
            QueryResult1 += "commit;"
            Log.Info("_delete_operation QueryResult1--> "+str(QueryResult1))
            Sql.RunQuery(QueryResult1)
...
#2127 STARTS
try:
    equip_insert_flag = Param.equip_insert_flag
except:
    equip_insert_flag = ""
#2127 ENDS
