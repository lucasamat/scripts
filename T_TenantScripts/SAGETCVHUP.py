...
try:
	...
	if LOGIN_CREDENTIALS is not None:
		...
		primaryQueryItems = SqlHelper.GetFirst(
			""
			+ str(Parameter1.QUERY_CRITERIA_1)
			+ " SACRVC SET ZGBAL_AMT = SACRVC_INBOUND.ZGBAL_AMT, ZDOC_AMT = SACRVC_INBOUND.ZDOC_AMT  FROM SACRVC_INBOUND (NOLOCK) JOIN SACRVC(NOLOCK)  ON SACRVC_INBOUND.BUKRS = SACRVC.BUKRS AND SACRVC_INBOUND.HKONT = SACRVC.HKONT AND SACRVC_INBOUND.BELNR = SACRVC.BELNR AND SACRVC_INBOUND.ZUONR = SACRVC.ZUONR AND SACRVC_INBOUND.GJAHR = SACRVC.GJAHR AND SACRVC_INBOUND.BUZEI = SACRVC.BUZEI AND SACRVC_INBOUND.MANDT = SACRVC.MANDT WHERE ISNULL(CRTAPP_INGL_CURR,0)=0 '"	)
