...
try:
	...
	primaryQueryItems = SqlHelper.GetFirst( ""+ str(Parameter.QUERY_CRITERIA_1)+ " SYINPL (INTEGRATION_PAYLOAD,INTEGRATION_KEY,CpqTableEntryDateModified,INTEGRATION_NAME)  select ''"+str(Final_xml)+ "'',''"+ str(QUOTE_ID)+ "'',getdate(),''CPQ to CRM Quote Replication_new'' ' ")
	...
	LOGIN_CREDENTIALS = SqlHelper.GetFirst("SELECT 'S0022234957' as Username,'Welcome@123' as Password")
	if LOGIN_CREDENTIALS is not None:
		...
		result= '''<?xml version="1.0" encoding="UTF-8"?><soapenv:Envelope	xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">	<soapenv:Body>{Final_xml}</soapenv:Body></soapenv:Envelope>'''.format( Final_xml = Final_xml)
		

		LOGIN_CRE = SqlHelper.GetFirst("SELECT URL FROM SYCONF where EXTERNAL_TABLE_NAME ='CPQ_TO_CRM_QUOTE_ASYNC'")
		crm_response = webclient.UploadString('https://e250404-iflmap.hcisbt.us3.hana.ondemand.com/cxf/QTPOSTWBZP', str(result))
