...
try:
        ...
        for json_data in Jsonquery:
            ...
            if len(rebuilt_data) != 0:      
                ...
                if Check_flag == 1 and Saqico_Flag == 1:
                    ...
                    msg.Subject = "Pricing Initiated - AMAT CPQ(T-Tenant)"
                    msg.IsBodyHtml = True
                    msg.Body = Error_Info
                    ...
                    copyEmail8 = MailAddress("Pricing-PrjDev-X@amat.com")
                    ...
                    primaryQueryItems = SqlHelper.GetFirst(
                        ""
                    + str(Parameter1.QUERY_CRITERIA_1)
                    + "  SAQICO SET STATUS=''CFG-ON HOLD TKM'' FROM SAQICO (NOLOCK) JOIN SAQGPA (NOLOCK) ON SAQICO.QUOTE_ID = SAQGPA.QUOTE_ID AND SAQICO.QTEREV_ID = SAQGPA.QTEREV_ID AND SAQICO.SERVICE_ID = SAQGPA.SERVICE_ID AND SAQICO.EQUIPMENT_ID = SAQGPA.EQUIPMENT_ID WHERE SAQICO.QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND SAQICO.QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"'' AND SAQICO.LINE IN (SELECT DISTINCT LINE FROM SAQICO_INBOUND WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' AND QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' ) AND ISNULL(SAQICO.STATUS,'''') NOT IN (''PRR-ON HOLD PRICING'',''CFG-ON HOLD - COSTING'',''ASSEMBLY IS MISSING'') AND ISNULL(SAQGPA.KIT_ID,'''')<>'''' AND ISNULL(SAQGPA.KIT_NUMBER,'''')='''' '")
                    ...
                    # Set message subject and body
                    msg.Subject = "Pricing Completed - AMAT CPQ(T-Tenant)"
                    msg.IsBodyHtml = True
                    msg.Body = Error_Info
                    ...
                    for Gbk in Greenbkquery:
                        ...
                        if len(tbl_info) > 0:
                            ...
                            # Set message subject and body
                            sub = "On Hold - Costing Quote("+str(Gbk.GREENBOOK)+")- AMAT CPQ (T-Tenant)"
                            msg.Subject = sub
                            msg.IsBodyHtml = True
                            msg.Body = Error_Info
                    ...
                    if Saqico_Flag == 0:
                        ...
                        # Set message subject and body
                        msg.Subject = "SSCM to CPQ - SAQICO Error Notification(T-Tenant)"
                        msg.IsBodyHtml = True
                        msg.Body = Error_Info
    ...
    # Set message subject and body
    msg.Subject = "SSCM to CPQ - Pricing Error Notification(T-Tenant)"
    msg.IsBodyHtml = True
    msg.Body = Error_Info
