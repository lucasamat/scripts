...
def entitlemnt_attr_update(partnumber,entitlement_table, where):
	get_equipment_count = Sql.GetFirst("select count(*) as cnt FROM {} (NOLOCK) {}".format(entitlement_table, where))
	...
	get_equipment = Sql.GetList("SELECT * FROM {} (NOLOCK) {}".format(entitlement_table, where))        
	if entitlement_table != 'SAQSAE':
		if get_equipment:
			QueryResult = "begin transaction;"
			while cnt <= get_total_count:
				get_equipment = SqlHelper.GetList("select * FROM {} (NOLOCK) {} ORDER BY CpqTableEntryId ASC OFFSET {} ROWS FETCH NEXT {} ROWS ONLY ".format(entitlement_table, where,fetch_count,end_count))
