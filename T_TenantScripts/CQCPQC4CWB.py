...
def writeback_to_c4c(writeback,contract_quote_record_id,quote_revision_record_id,input_data = None):
    ...
    if writeback == "quote_header":
        revision_obj = Sql.GetFirst("select ISNULL(NET_VALUE_INGL_CURR,0) AS NET_VALUE_INGL_CURR,SALESORG_ID,DOCTYP_ID,DISTRIBUTIONCHANNEL_ID,DIVISION_ID,QTEREV_ID,REVISION_DESCRIPTION,REVISION_STATUS,CONVERT(varchar, CONTRACT_VALID_FROM, 23) as CONTRACT_VALID_FROM,CONVERT(varchar, CONTRACT_VALID_TO , 23) as CONTRACT_VALID_TO FROM SAQTRV (NOLOCK) WHERE QUOTE_RECORD_ID = '{}' AND QTEREV_RECORD_ID = '{}' AND ACTIVE = 1 ".format(contract_quote_record_id,quote_revision_record_id))
        ...
        if revision_obj:
            ...
            if revision_obj.NET_VALUE_INGL_CURR <= 0:
                Trace.Write("not netvalue")
                quote_header_data = '{\"BuyerPartyID\":"'+str(quote_obj.ACCOUNT_ID)+'", \"EmployeeResponsiblePartyID\":"'+str(c4c_employee_id)+'", \"SalesUnitPartyID\":"'+str(revision_obj.SALESORG_ID)+'", \"DistributionChannelCode\":"'+str(revision_obj.DISTRIBUTIONCHANNEL_ID)+'", \"DivisionCode\":"'+str(revision_obj.DIVISION_ID)+'", \"ZWB_ContractValidFrom_KUT\":"'+str(valid_from)+'", \"ZWB_ContractValidTo_KUT\":"'+str(valid_to)+'", \"ZWB_QuoteRevisionID_KUT\":"'+str(revision_obj.QTEREV_ID)+'", \"ZWB_RevisionDescription_KUT\":"'+str(revision_obj.REVISION_DESCRIPTION)+'", \"ZQuoteRevisionStatus\":"'+str(revision_status_code.get(revision_obj.REVISION_STATUS))+'", \"ZWB_TotalQuotecurrencyCode_KUT\":"USD"}'
            else:
                Trace.Write("net value")
                quote_header_data = '{\"BuyerPartyID\":"'+str(quote_obj.ACCOUNT_ID)+'", \"EmployeeResponsiblePartyID\":"'+str(c4c_employee_id)+'", \"SalesUnitPartyID\":"'+str(revision_obj.SALESORG_ID)+'", \"DistributionChannelCode\":"'+str(revision_obj.DISTRIBUTIONCHANNEL_ID)+'", \"DivisionCode\":"'+str(revision_obj.DIVISION_ID)+'", \"ZWB_ContractValidFrom_KUT\":"'+str(valid_from)+'", \"ZWB_ContractValidTo_KUT\":"'+str(valid_to)+'", \"ZWB_QuoteRevisionID_KUT\":"'+str(revision_obj.QTEREV_ID)+'", \"ZWB_RevisionDescription_KUT\":"'+str(revision_obj.REVISION_DESCRIPTION)+'", \"ZQuoteRevisionStatus\":"'+str(revision_status_code.get(revision_obj.REVISION_STATUS))+'", \"ZWB_TotalQuoteContent_KUT\":"'+str(revision_obj.NET_VALUE_INGL_CURR)+'", \"ZWB_TotalQuotecurrencyCode_KUT\":"USD"}'
            ...
            Log.Info("###Quote Header Request data for quoterecid -"+str(contract_quote_record_id)+"and revisionrecid-"+str(quote_revision_record_id)+" are "+str(requestdata))
        ...
        if revision_obj:
            ...
            Log.Info("###OPP Header Request data for quoterecid -"+str(contract_quote_record_id)+"and revisionrecid-"+str(quote_revision_record_id)+" are "+str(requestdata))
        ...
        Trace.Write('Involved Parties replace request data--->'+str(requestdata))
