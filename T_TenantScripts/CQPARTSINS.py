...
class SyncFPMQuoteAndHanaDatabase:
    ...
    def prepare_backup_table(self):
        ...
        if self.response:
            ...
            if record_count >0:
                ...
                Log.Info("Insert Spare_PARTS Start ==>"+str(date_time))
                self._insert_spare_parts()
                now = datetime.datetime.now()# current date and time
                date_time = now.strftime("%m/%d/%Y, %H:%M:%S") 
                Log.Info("Insert Spare_PARTS End ==>"+str(date_time))
                now = datetime.datetime.now()# current date and time
                date_time = now.strftime("%m/%d/%Y, %H:%M:%S") 
                Log.Info("Update Spare_PARTS Start ==>"+str(date_time))
                self.update_records_saqspt()  
                now = datetime.datetime.now()# current date and time
                date_time = now.strftime("%m/%d/%Y, %H:%M:%S") 
                Log.Info("Update Spare_PARTS End ==>"+str(date_time))
    ...
    def bulk_pricing(self):
        ...
        Partnumber_list=SqlHelper.GetList("SELECT * FROM SAQSPT WHERE QUOTE_RECORD_ID = '"+str(self.quote_record_id)+"' AND QTEREV_RECORD_ID = '"+str(self.quote_revision_id)+"' AND PRICING_STATUS='ERROR'")
        Trace.Write((Partnumber_list))
        Trace.Write(Part_num_reprice)
    ...
    def cqpartiflw_iflow(self):
        Log.Info("PARTS==>"+(str(Parameter["Part_Number"])))
    ...
    def req_data(self):
        ...
        if (self.domain).lower() == 'appliedmaterials_tst':
            ...
            self.requestdata = "client_id=application&grant_type=client_credentials&username=a6ac4b1c-0f75-41f6-adc7-a235e46ebc61&password=i31TFt3Prut7bVqzb0-.Oszlv.&scope=fpmsafeaccess"
            self.authorization = "Basic YTZhYzRiMWMtMGY3NS00MWY2LWFkYzctYTIzNWU0NmViYzYxOmkzMVRGdDNQcnV0N2JWcXpiMC0uT3N6bHYu"
...
try:
    ...
    current_prod = "SALES"
try:
    parameter["Service_id"] = Param.CPS_Columns["Service_id"]
except Exception:
...
if Parameter["Action"] == 'Price':
    ...
    fpm_obj.cqpartiflw_iflow()
elif Parameter["Action"] == 'Bulk_Reprice':
    ...
    now = datetime.datetime.now()# current date and time
    date_time = now.strftime("%m/%d/%Y, %H:%M:%S") 
    Log.Info("Spare_PARTS Start ==>"+str(date_time))
    fpm_obj.prepare_backup_table()
    end_time = time.time() 
    now = datetime.datetime.now()# current date and time
    date_time = now.strftime("%m/%d/%Y, %H:%M:%S") 
    Log.Info("Spare_PARTS end==> "+str(date_time))
