...
class Entitlements:
    ...
    def EntitlementSave(self, subtabName, NewValue, AttributeID, AttributeValCode,SectionRecordId,EquipmentId,calc_factor,costimpact,priceimapct,getmaualipval,ENT_IP_DICT,scheduled_parts):
                                        ...
                                        if prdvalue["possibleValues"]:
                                            for i in prdvalue["possibleValues"]:
                                                ...
                                                if i['selectable'] == 'false' and 'valueLow' in i.keys():
                                                    ...
                                                    if (prdvalue["id"] in ('AGS_Z0091_NET_PRMALB','AGS_Z0091W_NET_PRMALB','AGS_Z0035_NET_PRMALB','AGS_Z0035W_NET_PRMALB') and self.treeparam in ('PDC','MPS') and get_attribute_value.STANDARD_ATTRIBUTE_DISPLAY_VAL == 'Included - Monthly and Above') or (prdvalue["id"] in ('AGS_Z0091_KPI_TGTKPI','AGS_Z0091W_KPI_TGTKPI','AGS_Z0092W_KPI_TGTKPI','AGS_Z0092_KPI_TGTKPI','AGS_Z0090_KPI_TGTKPI' ) and self.treeparam != 'IMPLANT' and get_attribute_value.STANDARD_ATTRIBUTE_DISPLAY_VAL in ('Max wafer Output <= 4%','Max Wafer Output >4%')) or(prdvalue["id"] in ('AGS_Z0091_KPI_TGTKPI','AGS_Z0091W_KPI_TGTKPI','AGS_Z0092W_KPI_TGTKPI','AGS_Z0092_KPI_TGTKPI','AGS_Z0090_KPI_TGTKPI' ) and self.treeparam not in ('MDP','DDP') and get_attribute_value.STANDARD_ATTRIBUTE_DISPLAY_VAL in ('Excursion Detection') ) or (prdvalue["id"] in ('AGS_Z0010_PQB_USRFEE') and self.treeparam not in ('CMP') and get_attribute_value.STANDARD_ATTRIBUTE_DISPLAY_VAL == 'AGS_Z0010_PQB_USRFEE_002' )  :
                                                        dropdowndisallowlist.append(str(prdvalue["id"])+'_'+str(i['valueLow'])	)
                                                    else:
                                                        dropdownallowlist.append(str(prdvalue["id"])+'_'+str(i['valueLow'])	)
                                                    # commented for value not defined error
                                                    # or (prdvalue["id"] in ('AGS_Z0091_KPI_PRPFGT','AGS_Z0091W_KPI_PRPFGT') and self.treeparam in ('PDC') and value.SYSTEM_ID == 'AGS_Z0091_KPI_PRPFGT_002' )
                                                    ###
                                                        #dropdownallowlist_selected.append(str(prdvalue["id"])+'}}'+str(i['valueLow'])	)
            ...
            if "calc" in AttributeID:
                ...
                for key,dict_val in ENT_IP_DICT.items():
                    ...
                    if key != 'undefined' and dict_val.split("||")[3] != 'undefined'  and str((dict_val).split("||")[0]).strip() != 'undefined':
                        ...
                        elif key == "AGS_{}_TSC_CUOWPN".format(serviceId) and serviceId in ("Z0091",'Z0092','Z0004','Z0009','Z0006','Z0090') :#1771,#1866
        ...
        if cps_conflict:
            ...
            # 	# msg_text = (
            # 	# 	'<div class="col-md-12" id="entitlement-info"><div class="col-md-12 alert-info"><label> <img src="/mt/APPLIEDMATERIALS_SIT/Additionalfiles/infocircle1.svg" alt="Info"> '
            # 	# 	+ str(Fullresponse['conflicts'][0]['explanation'])
            # 	# 	+ "</label></div></div>"
            # 	# )
            try:
                msg_text = '<div class="emp_notifiy" style="display: none;"><div class="col-md-12 page_alert_notifi" id="PageAlert"><div class="row modulesecbnr brdr" onclick="call_vertical_scrl()" data-toggle="collapse" data-target=".alertnotify" aria-expanded="true">NOTIFICATIONS<i class="pull-right fa fa-chevron-down "></i><i class="pull-right fa fa-chevron-up"></i></div><div id="alertnotify" class="col-md-12 alertnotify alert-notification  brdr collapse in"><div class="col-md-12" id="entitlement-info"><div class="col-md-12 alert-info"><label> <img src="/mt/APPLIEDMATERIALS_SIT/Additionalfiles/infocircle1.svg" alt="Info"> '+str(cps_conflict)+' </label></div></div> </div></div></div>'
            except Exception as e:
