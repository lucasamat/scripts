...
class SYLDRTLIST:
    def MDYNMICSQLOBJECT(self, RECORD_ID, PerPage, PageInform, SubTab, PR_CURR, TP, equipment_id,line_item):         
        ...
        getyears = col_year = addon_equipments = footer_tot = ""
        ...
        Page_start = (
            ...
            ObjectName
        ) = dbl_clk_function = col = text = texts = Qury_str = QuryCount_str = table_ids = lookup_str = curr_symbol_obj = curr_symbol = SAQICO_dbl_clk_function = line = ""
        ...
        Quote.GetCustomField('BILLING_MESSAGE').Content = ''
        get_billing_types = get_ttl_amt = get_dt_amt = get_billing_cycle_weekly = pricing_manual = '' 
        ...
        if obj_obj is not None:
            ...
            if Currenttab == 'Quotes':
                ...
                if  quote_rec_id:                  
                    ...
                    getRevision = Sql.GetFirst("SELECT REVISION_STATUS,WORKFLOW_STATUS FROM SAQTRV (NOLOCK) WHERE QUOTE_RECORD_ID = '"+str(quote_rec_id)+"' AND QTEREV_RECORD_ID = '"+str(quote_revision_record_id)+"'  ")
                    if getQuote:
                    ...
                    if str(getQuotetype).upper() == "ZWK1 - SPARES" and  str(TreeParam) in ['Quote Items','Quote Preview','Cart Items','Contract Preview']:
                        ...
                        if RECORD_ID == "SYOBJR-00029" and SubTab.upper() =='INCLUSIONS':							
                            rem_list_sp = ["NEW_PART","GREENBOOK"]
                            Columns = [ele for ele in  eval(Columns) if ele not in rem_list_sp]
                            Columns.extend(['UNIT_PRICE_INGL_CURR','EXTENDED_PRICE_INGL_CURR'])
                            Columns = str(Columns)
            ...
            try:
                Trace.Write("test352")
                if (str(getRevision.REVISION_STATUS).upper() in ("CBC-CBC COMPLETED","BOK-CONTRACT CREATED","BOK-CONTRACT BOOKED","LGL-LEGAL SOW ACCEPTED","LGL-LEGAL SOW REJECTED","LGL-PREPARING LEGAL SOW","CBC-PREPARING CBC")) or (str(getRevision.REVISION_STATUS).upper() in ("CFG-ACQUIRING","PRR-ON HOLD PRICING","CFG-ON HOLD - COSTING","PRI-PRICING") and str(TreeParam).upper() not in ("QUOTE ITEMS","BILLING","APPROVALS","QUOTE DOCUMENTS")) or (str(getRevision.REVISION_STATUS).upper() in ("APR-APPROVED","APR-REJECTED","APR-RECALLED","APR-APPROVAL PENDING","OPD-PREPARING QUOTE DOCUMENTS","OPD-CUSTOMER ACCEPTED","APR-SUBMITTED TO CUSTOMER","OPD-CUSTOMER REJECTED") and str(TreeParam).upper() not in ("APPROVALS","QUOTE DOCUMENTS")):
                    Trace.Write("test354")
                    Action_permission["Edit"] = "False"
                    Action_permission["Delete"] = "False"
                else:
                    Trace.Write("test358")
                    Action_permission["Edit"] = "True"
                    Action_permission["Delete"] = "True"
                if getRevision.REVISION_STATUS not in ('BOK-CONTRACT BOOKED','BOK-CONTRACT CREATED','CBC-CBC COMPLETED') and str(TreeParam).upper() == "REVISIONS":
                    Action_permission["Edit"] = "True"
                    Action_permission["Delete"] = "False"
            except Exception, e:
                Trace.Write("error360"+str(e))            
            ...
            if Wh_OBJECT_NAME == 'SAQIBP' and SubTab != 'Billing Plan':
                get_billing_days = Sql.GetFirst("SELECT BILLING_DAY FROM SAQIBP (NOLOCK) where SERVICE_ID = '{}' and QUOTE_RECORD_ID = '{}' and QTEREV_RECORD_ID ='{}'".format(TreeParam,contract_quote_record_id,quote_revision_record_id))
                get_billing_type = Sql.GetFirst("SELECT BILLING_TYPE from SAQRIT where SERVICE_ID = '{}' and QUOTE_RECORD_ID = '{}' and QTEREV_RECORD_ID ='{}'".format(TreeParam,contract_quote_record_id,quote_revision_record_id))
                if get_billing_days:
                    get_billing_days = get_billing_days.BILLING_DAY
                else:
                    get_billing_days = "Last day of Month"
                if get_billing_type:
                ...
                try:
                    if SubTab:
                        ...
                        if get_billing_days == "15th of Month":						
                            end = int(SubTab.split(' ')[-1]) * 12 + 1
                            Trace.Write('end-445--'+str(end))
                            start = end - 12 
                            Trace.Write('start--446-'+str(start))
                        else:
                ...
                if str(TreeParam) == "Billing":
                    ...
                    try:
                        if TreeParam not in ("Z0009","Z0123","Z0006","Z0007"):
                            get_round_val = 2
                            ...
                            get_year_max = Sql.GetList("SELECT max(CpqTableEntryId) as cpqid,SUM("+get_ttl_amt+") as billval from SAQIBP where QUOTE_RECORD_ID= '"+str(contract_quote_record_id)+"' and QTEREV_RECORD_ID ='"+str(quote_revision_record_id)+"'  and BILLING_YEAR= '"+str(SubTab)+"' and SERVICE_ID= '"+str(TreeParam)+"' GROUP BY GREENBOOK,SERVICE_ID,EQUIPMENT_ID,LINE")
                            for val in get_year_max:
                                get_total_amt = Sql.GetFirst("SELECT  LINE,EQUIPMENT_ID,GREENBOOK,"+get_ttl_amt+" as billval,ANNBILAMT_INGL_CURR from SAQIBP where QUOTE_RECORD_ID= '"+str(contract_quote_record_id)+"' and BILLING_YEAR= '"+str(SubTab)+"' and SERVICE_ID= '"+str(TreeParam)+"' and CpqTableEntryId = '"+str(val.cpqid)+"' and QTEREV_RECORD_ID ='"+str(quote_revision_record_id)+"' GROUP BY EQUIPMENT_ID,SERVICE_ID,ANNBILAMT_INGL_CURR,"+get_ttl_amt+",GREENBOOK,LINE")
                                get_diff = get_total_amt.ANNBILAMT_INGL_CURR-val.billval
        ...
        if Columns != "" and Obj_Name != "":
            ...
            if Qstn_REC_ID != "" and Wh_API_NAME != "":
                ...
                if str(Qury_str) == "" and str(QuryCount_str) == "":
                    ...
                    if str(RECORD_ID) == "SYOBJR-98795":                                           
                        ...
                        if TreeParam == "Quote Preview":                            
                            ...
                            "select top "
                                + str(PerPage)
                                + " QUOTE_ITEM_COVERED_OBJECT_RECORD_ID, EQUIPMENT_ID,SERVICE_ID,"+col_year+",SERIAL_NO, GREENBOOK,FABLOCATION_ID, SALES_DISCOUNT_PRICE,LINE,CONTRACT_ID,CONVERT(VARCHAR(10),CONTRACT_VALID_FROM,101) AS [CONTRACT_VALID_FROM],CONVERT(VARCHAR(10),CONTRACT_VALID_TO,101) AS [CONTRACT_VALID_TO],CpqTableEntryId from ( select  * from SAQICO (NOLOCK) where QUOTE_ID = '"
                        ...
                        if Product.GetGlobal("TreeParentLevel2") == "Quote Items":                            
                            imgstr = '<img title="Acquired" src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/Green_Tick.svg>'
                            acquiring_img_str = '<img title="Acquiring" src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/Cloud_Icon.svg>'
                            exclamation = '<img title="Approval Required" src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/clock_exe.svg>'
                            error = '<img title="Error" src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/exclamation_icon.svg>'
                            partially_priced = '<img title="Partially Priced" src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/Red1_Circle.svg>'
                            assembly_missing = '<img title="Assembly Missing" src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/Orange1_Circle.svg>'
                            on_hold_tkm = '<img title="ON-HOLD-TKM" src=/mt/APPLIEDMATERIALS_UAT/Additionalfiles/icons/on_hold_tkm.svg>'
                            on_hold_costing = '<img title="CFG-ON HOLD - COSTING" src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/icons/pricing_on_hold.svg>'
                            on_hold_pricing = '<img title="PRR-ON HOLD PRICING" src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/pricing_on_hold.svg>'
                            offline_pricing = '<img title="OFFLINE PRICING" src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/manual_pricing.svg>'
                            ...
                            Qury_str = (
                                    "SELECT DISTINCT TOP "
                                    + str(PerPage)
                                    + " CASE WHEN STATUS = 'ACQUIRED' THEN '"+ imgstr +"' WHEN STATUS = 'CFG-ON HOLD TKM' THEN '"+ on_hold_tkm +"' WHEN STATUS = 'APPROVAL REQUIRED' THEN '"+ exclamation +"' WHEN STATUS = 'OFFLINE PRICING' THEN '" +offline_pricing+ "' WHEN STATUS = 'CFG-ON HOLD - COSTING' THEN '"+on_hold_costing+"' WHEN STATUS = 'PRR-ON HOLD PRICING' THEN '"+on_hold_pricing+"' WHEN STATUS = 'ERROR' THEN '"+ error +"' WHEN STATUS = 'PARTIALLY PRICED' THEN '"+ partially_priced +"' WHEN STATUS = 'ASSEMBLY IS MISSING' THEN '"+ assembly_missing +"' ELSE '"+ acquiring_img_str +"' END AS STATUS, QUOTE_ITEM_COVERED_OBJECT_RECORD_ID,LINE,EQUIPMENT_ID,SERVICE_ID,SERIAL_NO,GREENBOOK,FABLOCATION_ID,TECHNOLOGY,KPU,TARGET_PRICE,NET_PRICE,"+col_year+",EQUIPMENT_RECORD_ID,SERVICE_RECORD_ID,FABLOCATION_RECORD_ID,EQUIPMENTCATEGORY_RECORD_ID,QUOTE_RECORD_ID,MNT_PLANT_RECORD_ID,SALES_DISCOUNT_PRICE,SALESORG_RECORD_ID,GREENBOOK_RECORD_ID,CONTRACT_ID,CONVERT(VARCHAR(10),CONTRACT_VALID_FROM,101) AS [CONTRACT_VALID_FROM],CONVERT(VARCHAR(10),CONTRACT_VALID_TO,101) AS [CONTRACT_VALID_TO],CpqTableEntryId,ASSEMBLY_ID from ( select ROW_NUMBER() OVER(order by LINE,"+ str(Wh_API_NAMEs) +") AS ROW, * from SAQICO (nolock)  where QUOTE_RECORD_ID ='"+str(RecAttValue)
                            ...
                            if TreeParentParam == "Quote Items":
                                Qury_str = (
                                    "select top "
                                        + str(PerPage)
                                        + " CASE  WHEN STATUS = 'ACQUIRED' THEN '"+ imgstr +"' WHEN STATUS = 'CFG-ON HOLD TKM' THEN '"+ on_hold_tkm +"' WHEN STATUS = 'APPROVAL REQUIRED' THEN '" +exclamation+ "' WHEN STATUS = 'CFG-ON HOLD - COSTING' THEN '"+on_hold_costing+"' WHEN STATUS = 'PRR-ON HOLD PRICING' THEN '"+on_hold_pricing+"' WHEN STATUS = 'OFFLINE PRICING' THEN '"+offline_pricing+"' WHEN STATUS = 'ERROR' THEN '"+ error +"' WHEN STATUS = 'PARTIALLY PRICED' THEN '"+ partially_priced +"' WHEN STATUS = 'ASSEMBLY IS MISSING' THEN '"+ assembly_missing +"' ELSE '"+ acquiring_img_str +"' END AS STATUS, QUOTE_ITEM_COVERED_OBJECT_RECORD_ID, EQUIPMENT_ID,SERVICE_ID,"+col_year+",SERIAL_NO, GREENBOOK,FABLOCATION_ID,TECHNOLOGY,KPU, SALES_DISCOUNT_PRICE,CONTRACT_ID,CONVERT(VARCHAR(10),CONTRACT_VALID_FROM,101) AS [CONTRACT_VALID_FROM],CONVERT(VARCHAR(10),CONTRACT_VALID_TO,101) AS [CONTRACT_VALID_TO],CpqTableEntryId,ASSEMBLY_ID from ( select * from SAQICO (NOLOCK) where QUOTE_ID = '"
                                        + str(qt_rec_id.QUOTE_ID)
                                        + "' AND SERVICE_ID = '"
                            ...
                            #A055S000P01-4578 starts
                                ...
                                Qury_str = (
                                    "select top "
                                        + str(PerPage)
                                        + " CASE WHEN STATUS = 'ACQUIRED' THEN '"+ imgstr +"' WHEN STATUS = 'CFG-ON HOLD TKM' THEN '"+ on_hold_tkm +"' WHEN STATUS = 'APPROVAL REQUIRED' THEN '" +exclamation+ "' WHEN STATUS = 'CFG-ON HOLD - COSTING' THEN '"+on_hold_costing+"' WHEN STATUS = 'PRR-ON HOLD PRICING' THEN '"+on_hold_pricing+"' WHEN STATUS = 'ERROR' THEN '"+ error +"' WHEN STATUS = 'OFFLINE PRICING' THEN '"+ offline_pricing +"' WHEN STATUS = 'PARTIALLY PRICED' THEN '"+ partially_priced +"' WHEN STATUS = 'ASSEMBLY IS MISSING' THEN '"+ assembly_missing +"'  ELSE '"+ acquiring_img_str +"' END AS  STATUS,QUOTE_ITEM_COVERED_OBJECT_RECORD_ID,LINE,SERVICE_ID,CNTYER,OFRING,OBJECT_ID,OBJECT_TYPE,EQUPID,ASSEMBLY_ID,GOT_CODE,EQNODE,PROCES,PM_ID,MNTEVT_LEVEL,KIT_ID,KIT_NUMBER,FABLOC,GRNBOK,QUANTITY,TOLCFG,SPQTEV,SPSPCT,SVSPCT,WTYSTE,WTYEND,WTYDAY,INWRTY,CNTDAY,BILTYP,TNTVGC,TENVGC,TAXVGC,ATGKEC,ATGKEP,NWPTOC,NWPTOP,CONSCP,CONSPI,NONCCI,NONCPI,AIUICI,AIUIPI,SBTCST,SBTPRC,AMNCCI,AMNPPI,FCWISS,ITCTAS,ITTPMF,TRGPRC,SLSPRC,BDVPRC,CELPRC,BCHPGC,BCHDPT,TGADJP,YOYPCT,USRPRC,CNTPRC,CpqTableEntryId from ( select  ROW_NUMBER() OVER( ORDER BY LINE) AS ROW, * from SAQICO (NOLOCK) where QUOTE_ID = '"
                                        + str(qt_rec_id.QUOTE_ID)
                                        + "'  AND QTEREV_RECORD_ID = '"+str(quote_revision_record_id)+"') m where m.ROW BETWEEN "
                                        + str(Page_start)
                                        + " and "
                    ...
                    ## involved parties equipmemt starts
                        ...
                        if delivery_date_column:                        
                            ...
                            column_before_delivery_pivot_changes = "QTEREVSPT_RECORD_ID,PART_DESCRIPTION,PART_NUMBER,MATPRIGRP_ID,UOM_ID,SALESUOM_ID,SCHEDULED_MODE,DELIVERY_MODE,CUSTOMER_ANNUAL_QUANTITY,DELIVERY_QUANTITY,DELIVERY_SCHEDULE,CUSTOMER_PART_NUMBER,QUANTITY,CONVERT(VARCHAR(10),FORMAT(DELIVERY_SCHED_DATE,'MM-dd-yyyy'),101) AS [DELIVERY_SCHED_DATE]"	
                            if Qustr:
                        ...
                        if billing_date_column:
                            get_billing_cylce = par_ser_id = ''                
                            pivot_columns = ",".join(['[{}]'.format(billing_date) for billing_date in billing_date_column])
                            contract_quote_current_record_id = Quote.GetGlobal("contract_quote_current_record_id")
                            Trace.Write('pivot_columns-contract_quote_current_record_id---'+str(contract_quote_current_record_id))
                            Trace.Write('pivot_columns-Qustr---'+str(pivot_columns))
                            get_current_record = Sql.GetFirst("select isnull(PAR_SERVICE_ID, '') as PAR_SERVICE_ID from SAQRIB (nolock) where QUOTE_BILLING_PLAN_RECORD_ID = '"+str(contract_quote_current_record_id)+"'")
                            if get_current_record:
                                if str(get_current_record.PAR_SERVICE_ID) != '':
                                    par_ser_id = str(get_current_record.PAR_SERVICE_ID)
                            get_billing_type = Sql.GetFirst("SELECT BILLING_TYPE from SAQRIT where SERVICE_ID = '{}' and QUOTE_RECORD_ID = '{}' AND QTEREV_RECORD_ID = '{}'".format(TreeParam,RecAttValue,quote_revision_record_id))
                            column_before_pivot_changes = "LINE,ANNBILAMT_INGL_CURR,CONTRACT_END_DATE,CONTRACT_START_DATE,UOM,STATUS,SAP_PART_NUMBER,OBJECT_TYPE,OBJECT_ID,LINE_TYPE,KIT_NUMBER,KIT_NAME,FABLOCATION_ID,COUNT_OF_ASSEMBLY,AGS_POSS_ID,UPDATED_CRM,TTLMRG_INGL_CURR,TEMP_TOOL,PRIPERKIT_INGL_CURR,PM_ID,MNTEVT_LEVEL,GOT_CODE,CUSTOMER_TOOL_ID,ASSEMBLY_ID,SERVICE_ID,EQUIPMENT_ID,SERIAL_NUMBER,GREENBOOK,BILLING_VALUE_INGL_CURR,ANNUAL_BILLING_AMOUNT,QUOTE_RECORD_ID,GREENBOOK_RECORD_ID,QTEREV_RECORD_ID,EQUIPMENT_RECORD_ID,SERVICE_RECORD_ID,CONVERT(VARCHAR(10),WARRANTY_END_DATE,101) AS [WARRANTY_END_DATE],CONVERT(VARCHAR(10),FORMAT(BILLING_DATE,'MM-dd-yyyy'),101) AS [BILLING_DATE],ESTVAL_INGL_CURR"

                            if get_billing_type:
                                get_billing_types = get_billing_type.BILLING_TYPE
                                if str(get_billing_types).upper() in ('FIXED','MILESTONE'):
                                    get_ttl_amt = 'BILLING_VALUE_INGL_CURR'
                                ...
                                if str(TreeParentParam)== "Billing" and TreeParam == "Z0117":
                                    ...
                                    if get_billing_cycle:
                                        ...
                                        for m in re.finditer(pattern_tag, updateentXML):
                                            ...
                                            get_ent_val= re.findall(pattern_name,sub_string)
                                            if get_ent_id:
                                                get_billing_cylce = str(get_ent_val[0])
                                        if get_billing_cylce == "ANNUALLY":
                                            Qustr += " AND QTEREV_RECORD_ID = '{}' AND SERVICE_ID = '{}' AND ISNULL(PAR_SERVICE_ID,'') = '{}' AND BILLING_DATE = '{}'".format(quote_revision_record_id,TreeParam, par_ser_id,billing_date_column[0])
                                        else:
                                            Qustr += " AND QTEREV_RECORD_ID = '{}' AND SERVICE_ID = '{}' AND ISNULL(PAR_SERVICE_ID,'') = '{}' AND BILLING_DATE BETWEEN '{}' AND '{}'".format(quote_revision_record_id,TreeParam, par_ser_id,billing_date_column[0], billing_date_column[-1])
                                else:
                                    Qustr += " AND BILLING_YEAR = '{}' AND QTEREV_RECORD_ID = '{}' AND SERVICE_ID = '{}' AND ISNULL(PAR_SERVICE_ID,'') = '{}' AND BILLING_DATE BETWEEN '{}' AND '{}'".format(SubTab,quote_revision_record_id,TreeParam, par_ser_id,billing_date_column[0], billing_date_column[-1])
                            else:
                                Qustr += " AND BILLING_YEAR = '{}' AND QTEREV_RECORD_ID = '{}' AND SERVICE_ID = '{}' AND ISNULL(PAR_SERVICE_ID,'') = '{}' AND BILLING_DATE BETWEEN '{}' AND '{}'".format(SubTab,quote_revision_record_id,TreeParam, par_ser_id,billing_date_column[0], billing_date_column[-1])
                            Trace.Write(str(TreeParam)+'--TreeParam---Qustr---1925--'+str(Qustr)+'---cols'+str(column_before_pivot_changes))
                            if TreeParam in ("Z0123","Z0010","Z0006","Z0007"):
                                pivot_query_str = """
                    ...
                    ##involved parties source fab ends
                        ...
                        Qustr += " AND QTEREV_RECORD_ID = '"+str(quote_revision_record_id)+"' AND PAR_SERVICE_ID = '"+str(TreeSuperParentParam)+"' AND GREENBOOK = '"+str(TreeParentParam)+"' "
                        Qury_str = (
                            "select DISTINCT top "
                            + str(PerPage)
                            + " "
                        ...
                        QuryCount_str = "select count(DISTINCT SERVICE_ID) as cnt from " + str(ObjectName) + " S (nolock) INNER JOIN MAADPR MA ON MA.COMP_PRDOFR_ID=S.SERVICE_ID " + str(Qustr)                        
                        #QuryCount_str = "select count(*) as cnt from " + str(ObjectName) + " (nolock) " + str(Qustr)
                ...
                if RECORD_ID == "SYOBJR-94443" and str(current_tab) == "Tab":                    
                    ...
                    acquiring_img_str = '<img title="Acquiring" src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/Cloud_Icon.svg>'
                    cps_pricing_img = ""
                    #cps_pricing_img ="<a href=''#'' onclick=''cps_pricing_call(this)''><img src=''/mt/APPLIEDMATERIALS_SIT/Additionalfiles/info.png''  style=''height: 15px; width: 15px; background-size: 20px 20px;''> </a>" 
                    error = '<img title="Error" src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/exclamation_icon.svg>'
                    Qury_str = (
                        "SELECT DISTINCT TOP "
                        + str(PerPage)
                        + " QUOTE_ITEM_FORECAST_PART_RECORD_ID, CASE WHEN PRICING_STATUS = 'ACQUIRED' THEN '"+ imgstr +"'  WHEN PRICING_STATUS = 'ERROR' THEN '" +error+ "' ELSE '"+ acquiring_img_str +"' END AS PRICING_STATUS,SERVICE_ID,CONCAT('"+cps_pricing_img+ "',PART_NUMBER) AS PART_NUMBER,MATPRIGRP_ID,PART_DESCRIPTION,BASEUOM_ID,SCHEDULE_MODE,DELIVERY_MODE,UNIT_PRICE,UNIT_PRICE_INGL_CURR,EXTENDED_PRICE,EXTPRI_INGL_CURR,ANNUAL_QUANTITY,CUSTOMER_PART_NUMBER_RECORD_ID,BASEUOM_RECORD_ID,MATPRIGRP_RECORD_ID,QTEITM_RECORD_ID,QUOTE_RECORD_ID,QTEREV_RECORD_ID,SALESORG_RECORD_ID,SERVICE_RECORD_ID,PART_RECORD_ID,SALESUOM_RECORD_ID,CpqTableEntryId,TAX_AMOUNT_INGL_CURR,SRVTAXCLA_DESCRIPTION,TAX_PERCENTAGE from ( select TOP "+ str(PerPage)+" ROW_NUMBER() OVER(order by "+ str(Wh_API_NAMEs) +") AS ROW, * from SAQIFP (nolock)  where QUOTE_RECORD_ID ='"+str(RecAttValue)
                        +"' AND QTEREV_RECORD_ID = '"
                        +str(quote_revision_record_id)
                        +"') m where m.ROW BETWEEN "
                        + str(Page_start)
                    ...
                    # 			icon = '<img title="Acquired" src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/Green_Tick.svg>'
                    # 		else:
                    # 			icon = '<img title="Acquiring" src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/Cloud_Icon.svg>'
                    # else:
                    ...
                    for pricing_status in SAQICO_status:
                        ...
                        if "ACQUIRED" in price_status and ('ACQUIRING' not in price_status and 'APPROVAL REQUIRED' not in price_status and 'ERROR' not in price_status):
                            ...
                            icon = '<img title="Error" src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/exclamation_icon.svg>'
                            ...
                            icon = '<img title="Approval Required" src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/clock_exe.svg>'
                            ...
                            icon = '<img title="Acquiring" src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/clock_exe.svg>'
                        else:
                            icon = '<img title="Error" src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/exclamation_icon.svg>'
                ...
                ##involved parties source fab ends
                    ...
                    if str(TreeSuperParentParam) == "Product Offerings":
                        Qury_str = (
                            ...
                            + " (nolock) "
                            + str(Qustr)
                            + " AND SERVICE_ID = '"+str(TreeParam)+"' AND PM_FREQUENCY_EDITABLE = 'True'  ) m where m.ROW BETWEEN "
                            + str(Page_start)
                            + " and "
                            + str(Page_End)
                            + ""
                        ...
                        Qury_str = (
                            ...
                            + " (nolock) "
                            + str(Qustr)
                            + " AND SERVICE_ID = '"+str(TopTreeSuperParentParam)+"' AND GREENBOOK = '"+str(TreeSuperParentParam)+"' AND GOT_CODE = '"+str(TreeParentParam)+"' AND PM_ID = '"+str(TreeParam)+"' AND PM_FREQUENCY_EDITABLE = 'True'  ) m where m.ROW BETWEEN "
                            + str(Page_start)
                            + " and "
                            + str(Page_End)
                            + ""
                ...
                for ik in Query_Obj:				                  
                    ...
                    if primary_view != "":                        
                        if str(current_tab).upper() == "PROFILE" and (ObjectName != "SYPROD"):
                            ...
                            if get_activerev and str(get_activerev.REVISION_STATUS) not in ("BOK-CONTRACT BOOKED","BOK-CONTRACT CREATED","CBC-CBC COMPLETED"):
                        ...
                        if str(current_tab).upper() == "PROFILE" and (ObjectName == "SYPROF"):
                            ...
                            if get_activerev and str(get_activerev.REVISION_STATUS) not in ("BOK-CONTRACT BOOKED","BOK-CONTRACT CREATED","CBC-CBC COMPLETED"):
                                ...
                                Action_str += ''
                        elif ObjectName == "SAQRSP":
                            contract_quote_rec_id = Quote.GetGlobal("contract_quote_record_id")
                            quote_revision_rec_id = Quote.GetGlobal("quote_revision_record_id")
                            reprice_btn = Sql.GetFirst("SELECT PRICING_STATUS FROM SAQRSP WHERESERVIE_ID='Z0100' AND QUOTE_RECORD_ID = '"+str(contract_quote_rec_id)+"' and CpqTableEntryId = '"+str(value1234)+"'")
                            
                            qt_status =Sql.GetFirst("""SELECT REVISION_STATUS FROM SAQTRV WHERE  QUOTE_RECORD_ID = '{ContractRecordId}' AND QTEREV_RECORD_ID = '{quote_revision_record_id}'""".format(ContractRecordId =contract_quote_rec_id,quote_revision_record_id =quote_revision_rec_id))


                            if qt_status.REVISION_STATUS in ('CFG-CONFIGURING','CFG-ACQUIRING','PRR-ON HOLD PRICING','PRI-PRICING','APR-APPROVAL PENDING','APR-RECALLED','APR-REJECTED'):
                                Action_str += '<li><a id = "" class="dropdown-item" href="#" " onclick="Repricing_CPS(this)">REPRICE</a></li>'
                            ...
                            if qt_status.REVISION_STATUS in ('CFG-CONFIGURING','CFG-ACQUIRING','PRR-ON HOLD PRICING','PRI-PRICING','APR-APPROVAL PENDING','APR-RECALLED','APR-REJECTED'):
                                Action_str += '<li><a id = "" class="dropdown-item" href="#" " onclick="Repricing(this)">REPRICE</a></li>'
                            ...
                            if docnode_action_btn:
                                if (str(docnode_action_btn.DOCUMENT_DESCRIPTION) == "" or docnode_action_btn.DOCUMENT_DESCRIPTION == "") and str(getRevision.REVISION_STATUS) not in ("BOK-CONTRACT BOOKED","BOK-CONTRACT CREATED","CBC-CBC COMPLETED"):
                                    Trace.Write("edit=====")
                        ...
                        # elif str(current_tab).upper() == "ROLE":
                            ...
                            if ObjectName== "SAQICT" :
                                ...
                                saqdlt_query = Sql.GetList("SELECT C4C_PARTNERFUNCTION_ID FROM SAQDLT (NOLOCK) WHERE QUOTE_RECORD_ID= '{quote_rec_id}' AND QTEREV_RECORD_ID = '{qte_rev_rec_id}' AND MEMBER_ID = '{UserName}'".format(quote_rec_id=Quote.GetGlobal("contract_quote_record_id"),qte_rev_rec_id=Quote.GetGlobal("quote_revision_record_id"),UserName=UserName))
                                roles_list = ("PRICING PERSON","BD","BD MANAGER","BD HEAD")
                                if saqdlt_query:
                                    for role in saqdlt_query:
                                        if str(role.C4C_PARTNERFUNCTION_ID).upper().strip() in roles_list:
                                            Action_str += '<li><a class="dropdown-item" href="#" onclick="Commonteree_view_RL(this)">VIEW</a></li>'
                                            pricing_manual = 'True'
                                            break
                                        else:
                                            Action_str += ""
                                            pricing_manual = 'False'
                                else:
                                    Action_str += ""
                                    pricing_manual = 'False'       
                    ...
                    if str(Action_permission.get("Delete")).upper() == "TRUE":
                        ...
                        if related_list_delete_permission:
                            ...
                            ##A055S000P01-10136
                                ...
                                if str(ik.CPQ_PARTNER_FUNCTION).upper() in ("SHIP TO","BILL TO","PAYER") and getRevision.WORKFLOW_STATUS == "CONFIGURE":
                                    Action_str += '<li><a class="dropdown-item" id="ADDNEW__SYOBJR_98798_SYOBJ_00910" href="#"   data-toggle="modal" data-target="#replace_account" onclick="localStorage.setItem(\'replacingAccount\',$(this).closest(\'tr\').find(\'td:nth-child(3)\').text()); $(\'#replace_account\').css(\'display\',\'block\');" >REPLACE</a></li>'
                                    quote_contract_recordId = Quote.GetGlobal("contract_quote_record_id")
                                    quote_revision_record_id = Quote.GetGlobal("quote_revision_record_id")
                            ...
                            elif str(ObjectName)=="SAQSPT" and  qt_status.REVISION_STATUS in ('CFG-CONFIGURING','CFG-ACQUIRING','PRR-ON HOLD PRICING','PRI-PRICING','APR-APPROVAL PENDING','APR-RECALLED','APR-REJECTED') :								
                                Action_str += (
                                    '<li><a class="dropdown-item" href="#" id="deletebtn" onclick="'
                                    + str(onclick)
                                    + '" data-target="#cont_CommonModalDelete" data-toggle="modal">DELETE</a></li>'
                    ...
                    for inm in ik:
                        ...
                        if RECORD_ID != 'SYOBJR-98869':
                            if value1234 == "ACQUIRED" or value1234 == "PRICED":
                                if RECORD_ID == 'SYOBJR-00005' or RECORD_ID == 'SYOBJR-00029':
                                    Trace.Write("ACQ@@@")
                                    value1234 = value1234.replace(value1234,"<img title='"+str(value1234).title()+"' src='/mt/APPLIEDMATERIALS_SIT/Additionalfiles/Green_Tick.svg'> ")
                                else:
                            ...
                            if value1234 == "ACQUIRING":
                                if RECORD_ID == 'SYOBJR-00005':
                                    value1234 = value1234.replace("ACQUIRING","<img title='Acquiring' src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/Cloud_Icon.svg>")
                                else:
                            ...
                            if value1234 == "ERROR":
                                if RECORD_ID == 'SYOBJR-00005' or RECORD_ID == 'SYOBJR-00029':
                                    value1234 = value1234.replace("ERROR","<img title='Error' src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/exclamation_icon.svg>")
                                else:
                            ...
                            if value1234 == "CFG-ON HOLD - COSTING":
                                Trace.Write("cmng_inside_on_hold_1 "+str(value1234))
                                value1234 = value1234.replace("CFG-ON HOLD - COSTING","<img title='CFG-ON HOLD - COSTING' src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/icons/on_hold_tkm.svg>")
                            if value1234 ==  "PRR-ON HOLD PRICING":
                                                
                                value1234 = value1234.replace("PRR-ON HOLD PRICING","<img title='PRR-ON HOLD PRICING' src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/pricing_on_hold.svg>")
                            if value1234 ==  "OFFLINE PRICING":
                                value1234 = value1234.replace("OFFLINE PRICING","<img title='OFFLINE PRICING' src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/manual_pricing.svg>")
                            if value1234 == "NOT PRICED":
                                if RECORD_ID == 'SYOBJR-00005' or RECORD_ID == 'SYOBJR-00029':
                                    value1234 = value1234.replace("NOT PRICED","<img title='NOT PRICED' class ='img-noprice' src=/mt/appliedmaterials_sit/Additionalfiles/no-price.svg>")
                            ...
                            if value1234 ==  "CFG-ON HOLD TKM":
                                value1234 = value1234.replace("CFG-ON HOLD TKM","<img title='ON-HOLD-TKM' src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/icons/on_hold_tkm.svg>")
                        else:
                            value1234 = value1234
                        ...
                        if str(cur_api_name) is not None and (
                            ...
                            try:
                                if cur_api_name_obj.CURRENCY_INDEX != "":
                                    if str(ObjectName) == "SAQIBP":
                                        Trace.Write('3433-->'+str(cur_api_name_obj.CURRENCY_INDEX)+'<-->'+str(value123))
                                        contract_quote_record_id = Product.GetGlobal("contract_quote_record_id")
                            ...
                            if value1234 is not None:
                                if value1234 != "":
                                    ...
                                    my_format = "{:,." + str(decimal_place) + "f}"
                                    value1234 = str(my_format.format(round(float(value1234), int(decimal_place))))
                                    if str(value123) == "ANNBILAMT_INGL_CURR" and str(ObjectName) == "SAQIBP":
                                        value1234 = value1234
                                    else:											
                                        value1234 = value1234 + " " + curr_symbol
                        if str(cur_api_name) is not None and (
                        ...
                        if value123 in lookup_disply_list:
                            ...
                            if str(ObjectName) == "USERS":
                                value1234 = str(ObjectName) + "-" + str(key_value).rjust(5, "0")
                            elif str(ObjectName) == 'SAQDOC' and key_value == 'Pending':
                                value1234 = 'Pending'
                            else:
                                value1234 = str(ObjectName) + "-" + str(OBJ_CpqTableEntryId).rjust(5, "0")
                            tab_obj1 = Sql.GetFirst(
                                "SELECT PG.TAB_NAME,PG.TAB_RECORD_ID FROM SYSECT (nolock) SE INNER JOIN SYPAGE (nolock) PG on SE.PAGE_RECORD_ID = PG.RECORD_ID WHERE SE.PRIMARY_OBJECT_NAME='"
                                + str(ObjectName)
                                + "' and SE.SECTION_NAME ='BASIC INFORMATION'"
                            ...
                            img_list = ['PO_NOTES','PRICING_STATUS','STATUS','EQUIPMENT_STATUS','REVISION_STATUS']
                            ...
                            if value123 in img_list:
                                ...
                                if not re.match(r'[A-Za-z0-9]',value1234):
                                    ...
                                    Trace.Write("========>>>><<<<<<M<MM<< "+str(value1234)+ " - "+str(data_type_val))
                                    if str(data_type_val) == "DATE":
                            ...
                            if value123 in checkbox_list:
                                ...
                                if (value123 == "SET_NAME" or value123 == "SETMAT_NAME") and (
                                    ...
                                    if not re.match(r'[A-Za-z0-9]',value1234):                                                              
                                        ...
                                        value1234 = value1234.replace('"', "&quot;")
                                        value1234 = value1234.replace("<p>", " ")
                                        value1234 = value1234.replace("</p>", " ")#Trace.Write(str(value123)+'3107--3051---'+str(value1234))
                                    if value123 in [
                                        "ERROR",
                                        "MINIMUM_PRICE",
                                        ...
                                        if str(value123) == "WARRANTY_END_DATE" and RECORD_ID != "SYOBJR-00009":
                                            ...
                                            contract_quote_record_id = Product.GetGlobal("contract_quote_record_id")
                                            get_billing_type = Sql.GetFirst("SELECT BILLING_TYPE from SAQRIT where QUOTE_RECORD_ID='{}' and QTEREV_RECORD_ID='{}' and SERVICE_ID='{}'".format(contract_quote_record_id,quote_revision_record_id,TreeParam))
                                            if str(get_billing_type.BILLING_TYPE).upper() in ("FIXED","MILESTONE"):
                                                curr_symbol_obj = Sql.GetFirst(
                                            ...
                                            if curr_symbol_obj:
                                                ...
                                                try:
                                                    ...
                                                    value1234
                                                Trace.Write('value1234--3718----'+str(value1234)+str(curr_symbol_obj.DISPLAY_DECIMAL_PLACES))											
                                            if getdate_indication:												
                                                if getdate_indication > getdate_indication_billing:
                                                    ...
                                                    if str(value1234) == "0.00":
                                                        ...
                                                        if str(value1234) != '':
                                                            ...
                                                            new_dict[value123] = (
                                                                '<input  type= "text" id ="' + key_value + '" class= "billclassnoedit"  value="' + value1234 + '" style="border: 0px solid;"  disabled>'
                                                            )
                                            else:
                                                #Trace.Write('value1234--3740---'+str(value1234))
                                                if str(value1234) != '':												
                                                    new_dict[value123] = (
                                                        '<input  type= "text" id ="' + key_value + '" class= "billclassedit"  value="' + value1234 + '" style="border: 0px solid;"  disabled>'
                                                    )
                                            ...
                                            if str(value123) != "CUSTOMER_ANNUAL_QUANTITY":
                                                ...
                                                if value123 in precentage_columns:
                                                    ...
                                                    if value123 in img_list:
                                                        ...
                                                        new_dict[value123] = ('<a href="#" onclick="Commonteree_view_RL(this)"  title="' + str(value1234).upper() + '">' + str(value1234).upper() + '</a>')
                                                    # for redirecting the left tree node while viewing record from listgrid - start    
                                                    elif ObjectName in value1234: 
                                                        Trace.Write("CHKZ_J_9__"+str(value1234))
                                                    ...
                                                    # for redirecting the left tree node while viewing record from listgrid - end       
                                                        ...
                                                        try:
                                                            if RECORD_ID == 'SYOBJR-00009' and value123 == 'DISCOUNT':
                                                                ...
                                                                Trace.Write("CHKZ_J_3__"+str(value1234))
                                                                new_dict[value123] = ('<a href="#" onclick="Commonteree_view_RL(this)" >' + str(value1234).upper() + '</a>')
                                                                #new_dict[value123] = ('<abbr id ="' + key_value + '"  title="' + str(value1234).upper() + '">' +str(value1234).upper() + "</abbr>")
                                                            ...
                                                            Trace.Write("CHKZ_J_1"+value1234)
            ...
            if TreeParam == "Quote Preview" or TreeParam == "Contract Preview" or subTab == "NSO Catalog":
                table_header += ''
            else:
            ...
            for invs in list(eval(Columns)):
                ...
                if invs in billing_date_column: # Billing Matrix - Pivot - Start
                    ...
                    if TreeParam == 'Quote Items' or invs not in ignorecol:
            ...
            filter_control_function += (
                ...
                + str(table_ids)
                + '").bootstrapTable("load", data[0] );if (table_id == "SYOBJR_00009_E5504B40_36E7_4EA6_9774_EA686705A63F"){annualgrid_show();} if (document.getElementById(\''
                + str(table_id)
                ...
                + str(table_id)
                + "_NumberofItem').innerHTML = data[2]; $('#SYOBJR_98882_ACE2CB08_73E0_4295_AC8C_78845C297161 #INCLUDED').removeAttr('disabled'); } if(document.getElementById('"
                + str(table_id)
            ...
            # if RECORD_ID == "SYOBJR-00009":
            # 	filter_control_function +=('$("#SYOBJR_00009_E5504B40_36E7_4EA6_9774_EA686705A63F tr").addClass("annual_fixed_tr"); $("#cust_fields_div").css("display","none");console.log("filter====="); $("#SYOBJR_00009_E5504B40_36E7_4EA6_9774_EA686705A63F").wrapAll("<div class=\"benchmarkdiv\"></div>"); $( ".JCLRgrips" ).remove(); var grouping_list = ["contractual_info_column_toggle","object_info_column_toggle","warranty_info_column_toggle","cat_info_column_toggle","price_info_column_toggle"]; grouping_list.forEach(function(rec_id){	if (rec_id == "object_info_column_toggle"){ var columns = ["GOT_CODE","ASSEMBLY_ID","PM_ID","PM_LEVEL","KIT_NAME","KIT_NUMBER","KPU"];} else if (rec_id == "contractual_info_column_toggle"){ var columns = ["QUANTITY","GREENBOOK","FABLOCATION_ID","CNTYER","CONTRACT_VALID_FROM","CONTRACT_VALID_TO","TENVGC","TNTVGC","TNTMGC","BILTYP"];} else if (rec_id == "warranty_info_column_toggle"){ var columns = ["WTYSTE","WTYEND","WTYDAY"];} else if (rec_id == "cat_info_column_toggle"){ var columns = ["ATGKEC","ATGKEP","NWPTOC","NWPTOP","AMNCCI"];} else if (rec_id == "price_info_column_toggle"){ var columns = ["TRGPRC","SLSPRC","BDVPRC","CELPRC","USRPRC","TGADJP"];} $.each(columns, function(index, column) { get_index = $("#SYOBJR_00009_E5504B40_36E7_4EA6_9774_EA686705A63F ").find("th[data-field=\"+column+\"]").index()+1+8; $("#SYOBJR_00009_E5504B40_36E7_4EA6_9774_EA686705A63F th[data-field=\"+column+\"]").hide(); $("#SYOBJR_00009_E5504B40_36E7_4EA6_9774_EA686705A63F td:nth-child("+get_index+")").hide();});});grouping_list.forEach(function(rec_id){ $("#"+rec_id).removeClass("glyphicon-minus-sign").addClass("glyphicon-plus-sign"); $("#"+rec_id).closest("th").attr("colspan",1);}); $("#SYOBJR_00009_E5504B40_36E7_4EA6_9774_EA686705A63F tbody tr td:nth-child(8)").css("cssText", "border-left:1px solid #dcdcdc !important");')                                      
            ...
            for key, invs in enumerate(list(eval(Columns))):
                ...
                # if RECORD_ID == 'SYOBJR-34575':
                # 	rowspan = 'rowspan="3"'
                Trace.Write("invs_chk-- "+str(invs))

                if key == 0 and invs != "INCLUDED":
                    if invs in primary_link_popup:
                        ...
                        if str(current_tab).upper() == "APP" and current_prod.upper() == "SYSTEM ADMIN":
                            ...
                            elif RECORD_ID == "SYOBJR-00010" or RECORD_ID == "SYOBJR-98882":
                        ...
                        if str(qstring) == "Purchase Order Notes" or str(qstring) == "Equipment Status":
                            ...
                            if (str(TreeParam) != 'Quote Preview' and str(TreeParam) != 'Contract Preview' and  str(TreeParam) != 'Billing Matrix' and str(current_tab).upper() != "APP") and RECORD_ID not in ("SYOBJR-98875","SYOBJR-98873","SYOBJR-00010","SYOBJR-98881","SYOBJR-00005","SYOBJR-98877"):
                                ...
                                Trace.Write("CHKNG_J_05---3744-- "+str(qstring))
                                table_header += (
                                    '<th  data-field="'
                                    + str(invs)
                                    + '" data-filter-control="input" data-title-tooltip="'
                            ...
                            elif RECORD_ID in ("SYOBJR-00010","SYOBJR-98882"):
                                Trace.Write("CHKNG_EMPTY HYPERLINK")
                                table_header += (
                                    '<th  data-field="'
                                    + str(invs)
                ...
                elif invs == "LINE" and RECORD_ID == "SYOBJR-98872":
                    Trace.Write("Inside Items Tab LINE")
                    table_header += (
                        '<th  data-field="'
                        + str(invs)
                        + '" data-filter-control="input" data-title-tooltip="'
                ...
                # 		table_header += '<th colspan="11" '+rowspan_level1+'  data-align="right"><div>CONTRACT INFO<button style="border:none;" class="glyphicon glyphicon-minus-sign" id="contractual_info_column_toggle" onclick="quote_items_column_toggle(this)"></button></div></th>'
                ...
                # 		table_header += '<th colspan="10" '+rowspan_level1+'  data-align="right"><div>OBJECT INFO<button style="border:none;" class="glyphicon glyphicon-minus-sign" id="object_info_column_toggle" onclick="quote_items_column_toggle(this)"></button></div></th>'
                # 	if str(invs) in right_align_list:
                # 		align = 'right'
                # 	elif str(invs) in center_align_list:
                # 		align = 'center'
                ...
                # 		table_header += '<th colspan="5" '+rowspan_level1+'  data-align="right"><div>WTY/SPLIT INFO<button style="border:none;" class="glyphicon glyphicon-minus-sign" id="warranty_info_column_toggle" onclick="quote_items_column_toggle(this)"></button></div></th>'
                # 	if str(invs) in right_align_list:
                # 		align = 'right'
                # 	elif str(invs) in center_align_list:
                # 		align = 'center'
                ...
                # 		table_header += '<th colspan="11" '+rowspan_level1+'  data-align="right"><div>CAT 4 INFO<button style="border:none;" class="glyphicon glyphicon-minus-sign" id="cat_info_column_toggle" onclick="quote_items_column_toggle(this)"></button></div></th>'
                # 	if str(invs) in right_align_list:
                # 		align = 'right'
                # 	elif str(invs) in center_align_list:
                # 		align = 'center'
                ...
                # 		table_header += '<th colspan="10" '+rowspan_level1+'  data-align="right"><div>PRICE INFO<button style="border:none;" class="glyphicon glyphicon-minus-sign" id="price_info_column_toggle" onclick="quote_items_column_toggle(this)"></button></div></th>'
                # 	if str(invs) in right_align_list:
                # 		align = 'right'
                # 	elif str(invs) in center_align_list:
                # 		align = 'center'
                ...
                # 	continue
                    ...
                    if str(invs) in right_align_list:
                        ...
                        if RECORD_ID == 'SYOBJR-00007' and str(invs) == 'BILLING_VALUE_INGL_CURR':                            
                        ...
                        if RECORD_ID == 'SYOBJR-00010' and str(invs) == 'SERVICE_ID' and TreeParam != "Quote Preview":
                            ...
                            table_header += (
                                '<th  data-field="'
                                + str(invs)
                                + '" data-filter-control="input" data-align="center" title="'
                                + str(hovertext)
                                + '" data-sortable="true" '
                                + str(visible)
                                + ' '
                        ...
                        if str(qstring) == "Key": 							
                            ...
                            tooltip = qstring
                            qstringlink = '<a onclick="openBillingMatrixDateChangeModal(\'{Value}\')" href="#">{Value}</a>'.format(Value=qstring.replace('-','/'))
                            ...
                            table_header += (
                                '<th  data-field="'
                                + str(invs)
                                + '" data-filter-control="input" class="cust_billing_name" title="'
                                + str(hovertext)
                                + '" data-sortable="true" '
                                + rowspan
                                +'>'
        ...
        restrict_dbl_click = ["SYOBJR-98882"]
        if CHL_STS_OBJ is not None: # Not Needed
            ...
            dbl_clk_function += (
                ...
                + str(cls)
                + '").text().trim(); reco_id=reco_id.split(","); localStorage.setItem("multiedit_save_date", reco_id); } localStorage.setItem("multiedit_data_clicked", reco_id); localStorage.setItem("table_id_RL_edit", "'
                + str(table_id)
                + '"); value = value.replace(/<\/?[^>]+(>|$)/g,"");cpq.server.executeScript("SYBLKETRLG", {"TITLE":field, "VALUE":value, "CLICKEDID":"'
                + str(table_id)
            ...
            if str(RECORD_ID) == "SYOBJR-98882":#A055S000P01-18196
                ...
                dbl_clk_function += ('$("'
                    + str(table_ids)
                    + '").on("all.bs.table", function (e, name, args) { if($("'+str(table_ids)+' input[name=\'btSelectAll\']:checkbox").is(":checked")) { if(localStorage.getItem("CommonNodeTreeSuperParentParam") == "Comprehensive Services" || localStorage.getItem("CommonTreeParentParam") == "Complementary Products" || localStorage.getItem("CommonTreeParam") == "Add-On Products"){$("button#delete_parts").css("display","block"); }localStorage.setItem("selectall","yes");}else{$("button#delete_parts").css("display","none");localStorage.setItem("selectall","no");} if($("'+str(table_ids)+' input[name=\'btSelectItem\']:checked").length > 1){ if(localStorage.getItem("CommonNodeTreeSuperParentParam") == "Comprehensive Services" || localStorage.getItem("CommonTreeParentParam") == "Complementary Products" || localStorage.getItem("CommonTreeParam") == "Add-On Products"){$("button#delete_parts").css("display","block");}if (localStorage.getItem("selectall") != "yes"){localStorage.setItem("selectall","no")};} else{$("button#delete_parts").css("display","none");} $(".bs-checkbox input").addClass("custom");});'
                )
                dbl_clk_function += (
                ...
                dbl_clk_function += (
                    ...
                    + str(table_ids)
                    + '").on("all.bs.table", function (e, name, args) { $(".bs-checkbox input").addClass("custom");if ($("'+str(table_ids)+' input[name=\'btSelectAll\']:checkbox").is(":checked")) { if(localStorage.getItem("CommonNodeTreeSuperParentParam") == "Comprehensive Services" || localStorage.getItem("CommonTreeParentParam") == "Complementary Products" || localStorage.getItem("CommonTreeParam") == "Add-On Products"){$("button#delete_parts").css("display","block"); }localStorage.setItem("selectall","yes");}if (!$("'+str(table_ids)+' input[name=\'btSelectAll\']:checkbox").is(":checked")){$("button#delete_parts").css("display","none");localStorage.setItem("selectall","no");} if($("'+str(table_ids)+' input[name=\'btSelectItem\']:checked").length > 1){ if(localStorage.getItem("CommonNodeTreeSuperParentParam") == "Comprehensive Services" || localStorage.getItem("CommonTreeParentParam") == "Complementary Products" || localStorage.getItem("CommonTreeParam") == "Add-On Products"){$("button#delete_parts").css("display","block");}if (localStorage.getItem("selectall") != "yes"){localStorage.setItem("selectall","no")};} if(!$("'+str(table_ids)+' input[name=\'btSelectItem\']:checked").length > 1){$("button#delete_parts").css("display","none");}$(".bs-checkbox input").after("<span class=\'lbl\'></span>"); }); $("'
                    + str(table_ids)
                    + '\ th.bs-checkbox div.th-inner").before("<div class=\'pad0brdbt\' >SELECT</div>"); $(".bs-checkbox input").addClass("custom"); $(".bs-checkbox input").after("<span class=\'lbl\'></span>"); function onClickCell(event, field, value, row, $element) { if(localStorage.getItem("InlineEdit")=="YES"){ return ;}var reco_id=""; var reco = []; reco = localStorage.getItem("multiedit_checkbox_clicked"); if (reco === null || reco === undefined ){ reco = []; } if (reco.length > 0){reco = reco.split(",");} if (reco.length > 0){ reco.push($element.closest("tr").find("td:'
                    + str(cls)
                    + '").text().trim());  data1 = $element.closest("tr").find("td:'
        ...
        filter_clas_name = ""
        cv_list = []		
        footer_str, footer = get_billing_cylce = "", ""
        ...
        if ObjectName == "SAQIBP":
            ...
            if get_current_record:
                if str(get_current_record.PAR_SERVICE_ID) != '':
                    par_ser_id = str(get_current_record.PAR_SERVICE_ID)
            get_billing_cycle = Sql.GetFirst("select ENTITLEMENT_XML from SAQITE where QUOTE_RECORD_ID = '{qtid}' AND QTEREV_RECORD_ID = '{qt_rev_id}' and SERVICE_ID = '{get_service}'".format(qtid =ContractRecordId,qt_rev_id=quote_revision_record_id,get_service = str(TreeParam).strip()))
            if get_billing_cycle:
                updateentXML = get_billing_cycle.ENTITLEMENT_XML
                pattern_tag = re.compile(r'(<QUOTE_ITEM_ENTITLEMENT>[\w\W]*?</QUOTE_ITEM_ENTITLEMENT>)')
                pattern_id = re.compile(r'<ENTITLEMENT_ID>(AGS_'+str(TreeParam)+'_PQB_BILCYC)</ENTITLEMENT_ID>')
                ...
                for m in re.finditer(pattern_tag, updateentXML):
                    ...
                    if get_ent_id:
                        get_billing_cylce = str(get_ent_val[0])
            if TreeParam == "Z0117" and get_billing_cylce == "ANNUALLY":
                gettotaldateamt = Sql.GetList("SELECT BILLING_VALUE_INGL_CURR=SUM(BILLING_VALUE_INGL_CURR),ESTVAL_INGL_CURR = SUM(ESTVAL_INGL_CURR),ANNBILAMT_INGL_CURR = SUM(ANNBILAMT_INGL_CURR),BILLING_DATE FROM SAQIBP WHERE BILLING_DATE in ('{billing_date_column}') and QUOTE_RECORD_ID ='{cq}' AND QTEREV_RECORD_ID='{revision_rec_id}' AND SERVICE_ID = '{service_id}' AND ISNULL(PAR_SERVICE_ID, '') = '{par_ser_id}' group by BILLING_DATE ".format(cq=str(ContractRecordId),revision_rec_id = quote_revision_record_id,billing_date_column="','".join(billing_date_column),service_id = TreeParam, par_ser_id=par_ser_id))
                #gettotaldateamt = Sql.GetList("SELECT BILLING_VALUE=SUM(BILLING_VALUE),ESTVAL_INGL_CURR = SUM(ESTVAL_INGL_CURR),ANNUAL_BILLING_AMOUNT = SUM(ANNUAL_BILLING_AMOUNT),BILLING_DATE FROM SAQIBP WHERE BILLING_DATE = '{billing_date_column}' and BILLING_YEAR='{subtab}' AND QUOTE_RECORD_ID ='{cq}' AND QTEREV_RECORD_ID='{revision_rec_id}' AND SERVICE_ID = '{service_id}' GROUP BY BILLING_DATE".format(cq=str(ContractRecordId),revision_rec_id = quote_revision_record_id,billing_date_column=billing_date_column[0],service_id = TreeParam,subtab=subTab))
                
                #gettotaldateamt = Sql.GetList("SELECT BILLING_VALUE=SUM(BILLING_VALUE),ESTVAL_INGL_CURR = SUM(ESTVAL_INGL_CURR),ANNUAL_BILLING_AMOUNT = SUM(ANNUAL_BILLING_AMOUNT),BILLING_DATE FROM SAQIBP WHERE BILLING_DATE in ('{billing_date_column}') and QUOTE_RECORD_ID ='{cq}' AND QTEREV_RECORD_ID='{revision_rec_id}' AND SERVICE_ID = '{service_id}' group by BILLING_DATE ".format(cq=str(ContractRecordId),revision_rec_id = quote_revision_record_id,billing_date_column="','".join(billing_date_column),service_id = TreeParam))
            elif str(get_billing_types).upper() == "MILESTONE" and TreeParam not in ("Z0006","Z0007","Z0009"):
                if TreeParam != 'Z0105':
                    par_ser_id = ''
                gettotaldateamt = Sql.GetList("SELECT BILLING_VALUE_INGL_CURR=SUM(BILLING_VALUE_INGL_CURR),ESTVAL_INGL_CURR = SUM(ESTVAL_INGL_CURR),ANNBILAMT_INGL_CURR = SUM(ANNBILAMT_INGL_CURR),BILLING_DATE FROM SAQIBP WHERE BILLING_DATE in ('{billing_date_column}') and QUOTE_RECORD_ID ='{cq}' AND QTEREV_RECORD_ID='{revision_rec_id}' AND SERVICE_ID = '{service_id}' AND BILLING_YEAR = '{BillingYear}' AND ISNULL(PAR_SERVICE_ID, '') = '{par_ser_id}' group by BILLING_DATE, BILLING_YEAR ".format(cq=str(ContractRecordId),revision_rec_id = quote_revision_record_id,billing_date_column="','".join(billing_date_column),service_id = TreeParam,BillingYear=SubTab, par_ser_id=par_ser_id))
                #gettotaldateamt = Sql.GetList("SELECT BILLING_VALUE=SUM(BILLING_VALUE),ESTVAL_INGL_CURR = SUM(ESTVAL_INGL_CURR),ANNUAL_BILLING_AMOUNT = SUM(ANNUAL_BILLING_AMOUNT),BILLING_DATE FROM SAQIBP WHERE BILLING_DATE = '{billing_date_column}' and BILLING_YEAR='{subtab}' AND QUOTE_RECORD_ID ='{cq}' AND QTEREV_RECORD_ID='{revision_rec_id}' AND SERVICE_ID = '{service_id}' GROUP BY BILLING_DATE".format(cq=str(ContractRecordId),revision_rec_id = quote_revision_record_id,billing_date_column=billing_date_column[0],service_id = TreeParam,subtab=subTab))
                
            elif str(get_billing_types).upper() == "MILESTONE" and TreeParam in ("Z0006","Z0007"):
                if TreeParam != 'Z0105':
                    par_ser_id = ''
                #gettotaldateamt = Sql.GetList("SELECT BILLING_VALUE=SUM(BILLING_VALUE),ESTVAL_INGL_CURR = SUM(ESTVAL_INGL_CURR),ANNUAL_BILLING_AMOUNT = SUM(ANNUAL_BILLING_AMOUNT),BILLING_DATE FROM SAQIBP WHERE BILLING_DATE in ('{billing_date_column}') and QUOTE_RECORD_ID ='{cq}' AND QTEREV_RECORD_ID='{revision_rec_id}' AND SERVICE_ID = '{service_id}' group by BILLING_DATE ".format(cq=str(ContractRecordId),revision_rec_id = quote_revision_record_id,billing_date_column="','".join(billing_date_column),service_id = TreeParam))                
                gettotaldateamt = Sql.GetList("SELECT BILLING_VALUE_INGL_CURR=SUM(BILLING_VALUE_INGL_CURR),ESTVAL_INGL_CURR = SUM(ESTVAL_INGL_CURR),ANNBILAMT_INGL_CURR = SUM(ANNBILAMT_INGL_CURR),BILLING_DATE FROM SAQIBP WHERE BILLING_DATE in ('{billing_date_column}') and QUOTE_RECORD_ID ='{cq}' AND QTEREV_RECORD_ID='{revision_rec_id}' AND SERVICE_ID = '{service_id}' AND ISNULL(PAR_SERVICE_ID, '') = '{par_ser_id}' group by BILLING_DATE ".format(cq=str(ContractRecordId),revision_rec_id = quote_revision_record_id,billing_date_column="','".join(billing_date_column),service_id = TreeParam, par_ser_id=par_ser_id))
                ...
                if TreeParam != 'Z0105':
                    par_ser_id = ''
                gettotaldateamt = Sql.GetList("SELECT BILLING_VALUE_INGL_CURR=SUM(BILLING_VALUE_INGL_CURR),ESTVAL_INGL_CURR = SUM(ESTVAL_INGL_CURR),ANNBILAMT_INGL_CURR = SUM(ANNBILAMT_INGL_CURR),BILLING_DATE FROM SAQIBP WHERE BILLING_DATE in ('{billing_date_column}') and QUOTE_RECORD_ID ='{cq}' AND QTEREV_RECORD_ID='{revision_rec_id}' AND SERVICE_ID = '{service_id}' AND BILLING_YEAR = '{BillingYear}' AND ISNULL(PAR_SERVICE_ID, '') = '{par_ser_id}' group by BILLING_DATE, BILLING_YEAR".format(cq=str(ContractRecordId),revision_rec_id = quote_revision_record_id,billing_date_column="','".join(billing_date_column),service_id = TreeParam, BillingYear=SubTab, par_ser_id=par_ser_id))
            Trace.Write('###5037-->'+str(decimal_place))
            if gettotaldateamt:
                if not decimal_place:
                    decimal_place = 2
                my_format = "{:,." + str(decimal_place) + "f}"
                for val in gettotaldateamt:
                    if val.ANNBILAMT_INGL_CURR and not gettotalamt:
                        gettotalamt = str(my_format.format(round(float(val.ANNBILAMT_INGL_CURR), int(decimal_place))))  
            Trace.Write('###5043')	
            if gettotaldateamt:
                my_format = "{:,." + str(decimal_place) + "f}"
            
                Trace.Write('curr_symbol--->'+str(curr_symbol))
                curr_symbol_bill = Sql.GetFirst("SELECT GLOBAL_CURRENCY from SAQTRV where QUOTE_REVISION_RECORD_ID= '{revision_rec_id}' and QUOTE_RECORD_ID='{cq}'".format(cq=str(ContractRecordId),revision_rec_id = quote_revision_record_id))
                footer_tot += '<th colspan="1" class="text-left">{}</th>'.format(curr_symbol_bill.GLOBAL_CURRENCY)
                ...
                for val in gettotaldateamt:
                    if str(get_billing_types).upper() == "VARIABLE":
                        ...
                        getamt = str(my_format.format(round(float(val.BILLING_VALUE_INGL_CURR), int(decimal_place))))
                        footer_tot += '<th class="text-right">{}</th>'.format(getamt)
            ...
            if filter_level_data == "select" and col_name not in checkbox_list:                
                    ...
                    if str(col_name) == "EXCHANGE_RATE_DATE":						
                        ...
                        if str(RECORD_ID) == "SYOBJR-98788":
                            ...
                            for document_type in doc_type:
                                ...
                                elif TreeParam == "Complementary Products":
                                    xcdStr = (
                                        "SELECT DISTINCT TOP 10000000 "
                                        + col_name
                                        + " FROM "
                                        + str(ObjectName)
                                        + " (nolock) JOIN (SELECT distinct PRDOFR_ID FROM MAADPR )M ON SERVICE_ID = M.PRDOFR_ID  where "
                                        + str(Wh_API_NAME)
                                        + " = '"
                                        + str(RecAttValue)
                                        + "'"
                                ...
                                elif TreeParam == "Comprehensive Services":
                                    xcdStr = (
                                        "SELECT DISTINCT TOP 10000000 "
                                        + col_name
                                        + " FROM "
                ...
                for string in StringValue_list:
                    ...
                    if RECORD_ID != 'SYOBJR-98869':
                        if string == "ACQUIRED" or string == "PRICED":
                            ...
                            string_value = string.replace(string,"<img title='"+str(string).title()+"' src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/Green_Tick.svg> ")
                            Trace.Write("string_value--"+str(string_value))
                        if string == "APPROVAL REQUIRED":
                            string_value = string.replace("APPROVAL REQUIRED","<img title='Approval Required' src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/clock_exe.svg> APPROVAL REQUIRED")
                        if string == "ACQUIRING":                        
                            string_value = string.replace("ACQUIRING","<img title='Acquiring' src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/Cloud_Icon.svg> ACQUIRING")
                        if string == "ERROR":
                            string_value = string.replace("ERROR","<img title='Error' src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/exclamation_icon.svg> ERROR")
                        if string == "ASSEMBLY IS MISSING":
                            string_value = string.replace("ASSEMBLY IS MISSING","<img title='Assembly Missing' src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/Orange1_Circle.svg> ASSEMBLY IS MISSING")
                        if string == "OFFLINE PRICING":
                            string_value = string.replace("OFFLINE PRICING","<img title='OFFLINE PRICING' src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/manual_pricing.svg> OFFLINE PRICING")
                        if string == "CFG-ON HOLD - COSTING":
                            string_value = string.replace("CFG-ON HOLD - COSTING","<img title='CFG-ON HOLD - COSTING' src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/icons/on_hold_tkm.svg CFG-ON HOLD - COSTING")
                        if string == "PRR-ON HOLD PRICING":
                            string_value = string.replace("PRR-ON HOLD PRICING","<img title='PRR-ON HOLD PRICING' src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/pricing_on_hold.svg>")
                        if string ==  "CFG-ON HOLD TKM":
                            Trace.Write("Coming inside ON HOLD TKM")
                            string_value = string.replace("CFG-ON HOLD TKM","<img title='ON-HOLD-TKM' src=/mt/APPLIEDMATERIALS_UAT/Additionalfiles/icons/on_hold_tkm.svg>")
                        if string == "PARTIALLY PRICED":
                            string_value = string.replace("PARTIALLY PRICED","<img title='Partially Priced' src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/Red1_Circle.svg> PARTIALLY PRICED")
                    else:
                        string_value = string
                    if (string != "ACQUIRED" and string != "APPROVAL REQUIRED" and string != "ERROR" and string != "ASSEMBLY IS MISSING" and string != "CFG-ON HOLD - COSTING" and string != "PARTIALLY PRICED" and string != "ACQUIRING" and string != "PRICED" and string != "PRR-ON HOLD PRICING" and string != "OFFLINE PRICING") or string != "CFG-ON HOLD TKM":                        
                        string_value = string
                ...
                try:
                    ...
                    if col_name in billing_date_column:
                        Trace.Write('col_name-->'+str(col_name)+'<----billing_date_column-->'+str(billing_date_column))
                        my_format = "{:,." + str(decimal_place) + "f}"
                        tovalue = 0.00
                        getamt = ""                        
                        ...
                        if table_list:
                            Trace.Write('col_name-->'+str(col_name))
                            currency_obj = re.search(r'>(.+?)<', table_list[0].get(col_name))
                            Trace.Write('currency_obj-->'+str(currency_obj))
                            if currency_obj:
        ...
        if ObjectName == 'SAQIBP' and TreeParam != 'Quote Items':
            ...
            footer_str = '<tfoot><tr><th colspan="31" id= "getbill1year" class="text-left">{}</th>{}</tr><tr></tr></tfoot>'.format("GRAND TOTAL", footer_tot)
        ...
        #HPQC 1976 billing restrict
        get_rev_status = Sql.GetFirst("SELECT WORKFLOW_STATUS FROM SAQTRV (NOLOCK) WHERE QUOTE_RECORD_ID = '{}' AND QUOTE_REVISION_RECORD_ID = '{}'".format(Quote.GetGlobal("contract_quote_record_id"), Quote.GetGlobal("quote_revision_record_id")))
        if str(TreeParentParam) == "Billing" and get_rev_status.WORKFLOW_STATUS in ('CONFIGURE','PRICING REVIEW','PRICING'):
            
            dbl_clk_function += (
                "try {var bildict = [];$('#SYOBJR_00007_26B8147E_C59C_4010_AA3A_38176869E305').on('click-row.bs.table', function (e, row, $element) { $('#SYOBJR_00007_26B8147E_C59C_4010_AA3A_38176869E305').find(':input(:disabled)').prop('disabled', false);$('#billingmatrix_save').css('display','block');$('#billingmatrix_cancel').css('display','block');$('#generatingbillingmatrix').css('display','none'); $('.billclassedit').parent().css('background-color','lightyellow');$('#SYOBJR_00007_26B8147E_C59C_4010_AA3A_38176869E305  tbody  tr td input').css('background-color','lightyellow');$('#billingmatrix_save').css('display','block');$('#billingmatrix_cancel').css('display','block'); var BillingmatrixBtn = $('.secondary_highlight_panel').find('button#REFRESH_MATRIX'); var billsave = $('.secondary_highlight_panel').find('button#billingmatrix_save'); var billcan = $('.secondary_highlight_panel').find('button#billingmatrix_cancel'); if (BillingmatrixBtn.length == 1){ BillingmatrixBtn.remove() } $('#billingmatrix_save').css('display','block'); $('#billingmatrix_cancel').css('display','block');$('#input#billeditval_disable').attr('disabled', true);$('#SYOBJR_00007_26B8147E_C59C_4010_AA3A_38176869E305 tbody tr td input').change(function () {console.log('on change function--');var getbillamt = $(this).val();console.log('getbillamt-------',getbillamt);localStorage.setItem('getbillamt', getbillamt);var equipid_index = $('#SYOBJR_00007_26B8147E_C59C_4010_AA3A_38176869E305').find(\"[data-field=\'EQUIPMENT_ID\']\").index()+1;var equipid = $(this).closest('tr').find('td:nth-child('+equipid_index+')').text();var line_equipid_index = $('#SYOBJR_00007_26B8147E_C59C_4010_AA3A_38176869E305').find(\"[data-field=\'LINE\']\").index()+1; var line_equipid = $(this).closest('tr').find('td:nth-child('+line_equipid_index+')').text();var annualamt_total_index = $('#SYOBJR_00007_26B8147E_C59C_4010_AA3A_38176869E305').find(\"[data-field=\'ANNBILAMT_INGL_CURR\']\").index()+1; var annualamt_total = $(this).closest('tr').find('td:nth-child('+annualamt_total_index+')').text();console.log('eduip-----',equipid);var test = $(this).closest('td').index();var getindex = test+1;var getheaderdate = $('#SYOBJR_00007_26B8147E_C59C_4010_AA3A_38176869E305 thead th:nth-child('+getindex+')').text();var getannualtotal = $('#SYOBJR_00007_26B8147E_C59C_4010_AA3A_38176869E305 thead th:nth-child('+getindex+')').text();console.log('getannualtotal----',getannualtotal);console.log('annualamt_total----',annualamt_total);var concate_data = equipid+ ' * '+getheaderdate+ '* '+getbillamt+' * '+annualamt_total+' * '+line_equipid;if(!bildict.includes(concate_data)){bildict.push(concate_data)};getbilldictdata = JSON.stringify(bildict);localStorage.setItem('getbilldictdata', getbilldictdata);}); })}catch (err){console.log('catch-----')}"
            )
        if str(TreeParentParam) == "Billing" and get_rev_status.WORKFLOW_STATUS not in ('CONFIGURE','PRICING REVIEW','PRICING'):
            dbl_clk_function += ""
        #HPQC 1976 billing restrict end
        ...
        service_query= Sql.GetFirst("""select count(*) as cnt from SAQSCO(NOLOCK) WHERE QUOTE_RECORD_ID = '{ContractRecordId}'AND QTEREV_RECORD_ID = '{quote_revision_record_id}' AND PAR_SERVICE_ID ='{TreeSuperParentParam}'AND SERVICE_ID='Z0123'  """.format(ContractRecordId =contract_quote_record_id,quote_revision_record_id =quote_revision_record_id,TreeSuperParentParam=TreeSuperParentParam))
        if int(service_query.cnt) >0:
            Trace.Write("service_query.cnt=="+str(service_query.cnt))
            Trace.Write("service_query=="+str(service_query))
            service_query_result='True'
        else:
            service_query_result='False'
        Trace.Write("service_query_result_CHKSZZZ -- "+str(service_query_result))
        if (str(getRevision.REVISION_STATUS).upper() in ("CBC-CBC COMPLETED","BOK-CONTRACT CREATED","BOK-CONTRACT BOOKED","LGL-LEGAL SOW ACCEPTED","LGL-LEGAL SOW REJECTED","LGL-PREPARING LEGAL SOW","CBC-PREPARING CBC")) or (str(getRevision.REVISION_STATUS).upper() in ("CFG-ACQUIRING","PRR-ON HOLD PRICING","CFG-ON HOLD - COSTING","PRI-PRICING") and str(TreeParam).upper() not in ("QUOTE ITEMS","BILLING","APPROVALS","QUOTE DOCUMENTS")) or (str(getRevision.REVISION_STATUS).upper() in ("APR-APPROVED","APR-REJECTED","APR-RECALLED","APR-APPROVAL PENDING","OPD-PREPARING QUOTE DOCUMENTS","OPD-CUSTOMER ACCEPTED","APR-SUBMITTED TO CUSTOMER","OPD-CUSTOMER REJECTED") and str(TreeParam).upper() not in ("APPROVALS","QUOTE DOCUMENTS")):
            Nso_Included = "True"
        else:
            Nso_Included = "False"
    ...
    def MDYNMICSQLOBJECTFILTER(
        ...
        try:
            getRevision = Sql.GetFirst("SELECT REVISION_STATUS,WORKFLOW_STATUS FROM SAQTRV (NOLOCK) WHERE QUOTE_RECORD_ID = '"+str(quote_rec_id)+"' AND QTEREV_RECORD_ID = '"+str(quote_revision_rec_id)+"'  ")
        except:
            getRevision = ""				
        Tree_Enable = ""
        Tree_Enable = Sql.GetFirst(
        ...
        if obj_obj is not None:
            ...
            try:
                Trace.Write("test5887")
                if (str(getRevision.REVISION_STATUS).upper() in ("CBC-CBC COMPLETED","BOK-CONTRACT CREATED","BOK-CONTRACT BOOKED","LGL-LEGAL SOW ACCEPTED","LGL-LEGAL SOW REJECTED","LGL-PREPARING LEGAL SOW","CBC-PREPARING CBC")) or (str(getRevision.REVISION_STATUS).upper() in ("CFG-ACQUIRING","PRR-ON HOLD PRICING","CFG-ON HOLD - COSTING","PRI-PRICING") and str(TreeParam).upper() not in ("QUOTE ITEMS","BILLING","APPROVALS","QUOTE DOCUMENTS")) or (str(getRevision.REVISION_STATUS).upper() in ("APR-APPROVED","APR-REJECTED","APR-RECALLED","APR-APPROVAL PENDING","OPD-PREPARING QUOTE DOCUMENTS","OPD-CUSTOMER ACCEPTED","APR-SUBMITTED TO CUSTOMER","OPD-CUSTOMER REJECTED") and str(TreeParam).upper() not in ("APPROVALS","QUOTE DOCUMENTS")):
                    Trace.Write("test5889")
                    Action_permission["Edit"] = "False"
                    Action_permission["Delete"] = "False"
                else:
                    Trace.Write("test5893")
                    Action_permission["Edit"] = "True"
                    Action_permission["Delete"] = "True"
                if getRevision.REVISION_STATUS not in ('BOK-CONTRACT BOOKED','BOK-CONTRACT CREATED','CBC-CBC COMPLETED') and str(TreeParam).upper() == "REVISIONS":
                    Action_permission["Edit"] = "True"
            ...
            if Wh_OBJECT_NAME == 'SAQIBP':
                ...
                item_delivery_plans_obj = Sql.GetList("""SELECT FORMAT(DELIVERY_SCHED_DATE, 'MM-dd-yyyy') as DELIVERY_SCHED_DATE FROM (SELECT ROW_NUMBER() OVER(ORDER BY DELIVERY_SCHED_DATE)
                                    AS ROW, * FROM (SELECT DISTINCT DELIVERY_SCHED_DATE
                                                        FROM SAQSPD (NOLOCK) WHERE QUOTE_RECORD_ID = '{}' AND QTEREV_RECORD_ID = '{}' 
                                                        GROUP BY DELIVERY_SCHED_DATE,QTEREVSPT_RECORD_ID) IQ) OQ WHERE OQ.ROW BETWEEN {} AND {}""".format(
                                                            contract_quote_record_id, Quote.GetGlobal("quote_revision_record_id"), 1, 52))
                count = 0
                if item_delivery_plans_obj:
                    delivery_date_column = [item_delivery_plans_obj.DELIVERY_SCHED_DATE for item_delivery_plans_obj in item_delivery_plans_obj]
                    for delivery_data in delivery_date_column:
        ...
        if Columns != "" and Obj_Name != "":
            ...
            if billing_date_column:
                column_before_pivot_change = col
                column_before_pivot_change = column_before_pivot_change.replace('ESTVAL_INGL_CURR','ISNULL(ESTVAL_INGL_CURR, 0.00) AS ESTVAL_INGL_CURR')
                col += ","+ ",".join(billing_date_column)
            ...
            if Qstn_REC_ID != "" and Wh_API_NAME != "":
                ...
                if (
                    ...
                    if SortColumn == "" and SortColumnOrder == "":                        
                        ...
                        get_datatype = Sql.GetFirst("SELECT DATA_TYPE FROM SYOBJD WHERE OBJECT_NAME = '"+str(ObjectName)+"' AND API_NAME = '"+str(SortColumn)+"'")
                        if get_datatype.DATA_TYPE == "AUTO NUMBER":
                            Wh_API_NAMEs = " CpqTableEntryId " + str(SortColumnOrder).upper()
                        else:
                ...
                ATTRIBUTE_VALUE_STR = ""
                if ObjectName =='SAQSPT' or ObjectName == 'SAQRSP':
                        ...
                        if quer_values.find(",") == -1:
                            ...
                            if str(picklist_data).upper() == "TRUE":
                                if str(quer_values).upper() == "TRUE":
                                    ...
                                    elif 'PRR-ON HOLD PRICING' in quer_values:
                                        quer_values = "PRR-ON HOLD PRICING"
                                    elif 'CFG-ON HOLD TKM' in quer_values:
                                        quer_values = "CFG-ON HOLD TKM"
                                ...
                                elif ObjectName == 'SAQIBP' and (str(quer_value) in 'BILLING_VALUE_INGL_CURR' or 'BILLING_DATE' in str(quer_value) or '20' in str(quer_value)):                                    
                                    if (str(quer_values).strip() != '' or str(quer_values) != None):
                                        contract_quote_record_id = Quote.GetGlobal("contract_quote_record_id")
                                        quote_revision_record_id = Quote.GetGlobal("quote_revision_record_id")
                                        getLines = Sql.GetList(
                                            "SELECT DISTINCT LINE FROM SAQIBP (nolock) WHERE QUOTE_RECORD_ID = '"
                                            + str(contract_quote_record_id)
                                            + "' AND QTEREV_RECORD_ID ='"
                                            + str(quote_revision_record_id)
                                            + "' AND SERVICE_ID ='"
                                            ...
                                            + "' AND BILLING_DATE ='"
                                            + str(SortColumn)
                                            + "' AND BILLING_VALUE_INGL_CURR LIKE '%"
                                            + str(quer_values)
                                            + "%'" 
                                        )
                                        ...
                                        line = ",".join(lines)                                        
                                        ATTRIBUTE_VALUE_STR += " LINE IN (" + line + ") AND "
                                    else:
                                        line = ""
                                else:
                                    if  RECORD_ID == 'SYOBJR-98869' and TreeParam == "Revisions":
                                        quer_values = quer_values[-1]
                                        ATTRIBUTE_VALUE_STR += str(quer_value) + " like '%" + str(quer_values) + "%' and "
                                    else:
                                        try:
                                            ATTRIBUTE_VALUE_STR += str(quer_value) + " like '%" + str(quer_values) + "%' and "
                                        except Exception:
                                            ATTRIBUTE_VALUE_STR += str(quer_value) + " like '%" + quer_values + "%' and "
                        else:                            
                            ...
                            if str(quer_value) == "EXCLUSIVE_MATERIAL":
                                ...
                                for i in range(0,len(quer_values)):
                                    ...
                                    elif 'CFG-ON HOLD TKM' in quer_values[i]:
                                        quer_values[i] = "CFG-ON HOLD TKM"
                                quer_values = tuple(quer_values)
                ...
                if ATTRIBUTE_VALUE_STR != "":
                    ...
                    if RECORD_ID == "SYOBJR-95868":                        
                        ...
                        if TreeParam == "Quote Preview":                            
                            ...
                            "select top "
                                + str(PerPage)
                                + " QUOTE_ITEM_COVERED_OBJECT_RECORD_ID, EQUIPMENT_LINE_ID, EQUIPMENT_ID,SERVICE_ID,DISCOUNT,"+col_year+",SERIAL_NO, GREENBOOK,FABLOCATION_ID, SALES_DISCOUNT_PRICE,LINE,NET_VALUE,CONTRACT_ID,CONVERT(VARCHAR(10),CONTRACT_VALID_FROM,101) AS [CONTRACT_VALID_FROM],CONVERT(VARCHAR(10),CONTRACT_VALID_TO,101) AS [CONTRACT_VALID_TO],CpqTableEntryId from ( select  ROW_NUMBER() OVER( ORDER BY EQUIPMENT_LINE_ID) AS ROW, * from SAQICO (NOLOCK) where QUOTE_ID = '"
                                + str(qt_rec_id.QUOTE_ID)
                                + "' AND QTEREV_RECORD_ID = '"+str(quote_revision_record_id)+"') m where m.ROW BETWEEN "
                                + str(Page_start)
                                + " and "
                        ...
                        if Product.GetGlobal("TreeParentLevel2") == "Quote Items":  
                            ...
                            if TreeParam == "Quote Items":
                                ...
                                Qury_str = (
                                    ...
                                    "select top "
                                        + str(PerPage)
                                        + " CASE WHEN STATUS = 'ACQUIRED' THEN '"+ imgstr +"' WHEN STATUS = 'CFG-ON HOLD TKM' THEN '"+ on_hold_tkm +"' WHEN STATUS = 'APPROVAL REQUIRED' THEN '" +exclamation+ "' WHEN STATUS = 'OFFLINE PRICING' THEN '" +offline_pricing+ "' WHEN STATUS = 'CFG-ON HOLD - COSTING' THEN '"+on_hold_costing+"' WHEN STATUS = 'PRR-ON HOLD PRICING' THEN '"+on_hold_pricing+"' WHEN STATUS = 'ERROR' THEN '"+ error +"' WHEN STATUS = 'PARTIALLY PRICED' THEN '"+ partially_priced +"' WHEN STATUS = 'ASSEMBLY IS MISSING' THEN '"+ assembly_missing +"'  ELSE '"+ acquiring_img_str +"' END AS  STATUS,QUOTE_ITEM_COVERED_OBJECT_RECORD_ID,LINE,CNTYER,OFRING,OBJECT_ID,OBJECT_TYPE,EQUPID,ASSEMBLY_ID,GOT_CODE,EQNODE,PROCES,PM_ID,MNTEVT_LEVEL,KIT_ID,KIT_NUMBER,FABLOC,GRNBOK,QUANTITY,TOLCFG,SPQTEV,SPSPCT,SVSPCT,WTYSTE,WTYEND,WTYDAY,INWRTY,CNTDAY,BILTYP,TNTVGC,TENVGC,TAXVGC,ATGKEC,ATGKEP,NWPTOC,NWPTOP,CONSCP,CONSPI,NONCCI,NONCPI,AIUICI,AIUIPI,SBTCST,SBTPRC,AMNCCI,AMNPPI,FCWISS,ITCTAS,ITTPMF,TRGPRC,SLSPRC,BDVPRC,CELPRC,BCHPGC,BCHDPT,TGADJP,YOYPCT,USRPRC,CNTPRC,CpqTableEntryId from ( select  ROW_NUMBER() OVER( ORDER BY LINE) AS ROW, * from SAQICO (NOLOCK) where "+ str(ATTRIBUTE_VALUE_STR)+" QUOTE_ID = '"
                                        + str(qt_rec_id.QUOTE_ID)
                                        + "') m where m.ROW BETWEEN "
                                        + str(Page_start)
                                        + " and "
                            ...
                            ## code is not using anywhere    TreeSuperParentParam
                                ...
                                Qury_str = (
                                    "select top "
                                        + str(PerPage)
                                        + " CASE  WHEN STATUS = 'ACQUIRED' THEN '"+ imgstr +"' WHEN STATUS = 'CFG-ON HOLD TKM' THEN '"+ on_hold_tkm +"' WHEN STATUS = 'APPROVAL REQUIRED' THEN '" +exclamation+ "' WHEN STATUS = 'CFG-ON HOLD - COSTING' THEN '"+on_hold_costing+"' WHEN STATUS = 'OFFLINE PRICING' THEN '"+offline_pricing+"' WHEN STATUS = 'PRR-ON HOLD PRICING' THEN '"+on_hold_pricing+"' WHEN STATUS = 'ERROR' THEN '"+ error +"' WHEN STATUS = 'PARTIALLY PRICED' THEN '"+ partially_priced +"' WHEN STATUS = 'ASSEMBLY IS MISSING' THEN '"+ assembly_missing +"' ELSE '"+ acquiring_img_str +"' END AS STATUS, QUOTE_ITEM_COVERED_OBJECT_RECORD_ID, SERVICE_ID, EQUIPMENT_ID,LINE_ITEM_ID,YEAR_1, SERIAL_NO,GREENBOOK,FABLOCATION_ID,TECHNOLOGY,KPU,TARGET_PRICE, SALES_DISCOUNT_PRICE,NET_VALUE,CONTRACT_ID,CONVERT(VARCHAR(10),CONTRACT_VALID_FROM,101) AS [CONTRACT_VALID_FROM],CONVERT(VARCHAR(10),CONTRACT_VALID_TO,101) AS [CONTRACT_VALID_TO],CpqTableEntryId,ASSEMBLY_ID from ( select  ROW_NUMBER() OVER( ORDER BY "+ str(Wh_API_NAMEs)
                                        +") AS ROW, * from SAQICO (NOLOCK) where "+ str(ATTRIBUTE_VALUE_STR)+" QUOTE_ID = '"
                                        + str(qt_rec_id.QUOTE_ID)
                                        + "' AND SERVICE_ID = '"
                                        + str(LineAndEquipIDList)
                        ...
                        if Product.GetGlobal("TreeParentLevel2") == "Quote Items":
                            ...
                            try:
                                ...
                                ServiceId = TreeParentParam.split("-")[1].strip()                            
                            Qury_str = ("SELECT DISTINCT TOP " + str(PerPage) + " CASE WHEN STATUS = 'ACQUIRED' THEN '"+ imgstr +"' WHEN STATUS = 'CFG-ON HOLD TKM' THEN '"+ on_hold_tkm +"' WHEN STATUS = 'OFFLINE PRICING' THEN '" +offline_pricing+ "' WHEN STATUS = 'APPROVAL REQUIRED' THEN '"+ exclamation +"' WHEN STATUS = 'CFG-ON HOLD - COSTING' THEN '"+on_hold_costing+"' WHEN STATUS = 'PRR-ON HOLD PRICING' THEN '"+on_hold_pricing+"' WHEN STATUS = 'ERROR' THEN '"+ error +"' WHEN STATUS = 'PARTIALLY PRICED' THEN '"+ partially_priced +"' WHEN STATUS = 'ASSEMBLY IS MISSING' THEN '"+ assembly_missing +"' ELSE '"+ acquiring_img_str +"' END AS  STATUS,QUOTE_ITEM_COVERED_OBJECT_RECORD_ID,LINE,CNTYER,OFRING,OBJECT_ID,OBJECT_TYPE,EQUPID,ASSEMBLY_ID,GOT_CODE,EQNODE,PROCES,PM_ID,MNTEVT_LEVEL,KIT_ID,KIT_NUMBER,FABLOC,GRNBOK,QUANTITY,TOLCFG,SPQTEV,SPSPCT,SVSPCT,WTYSTE,WTYEND,WTYDAY,INWRTY,CNTDAY,BILTYP,TNTVGC,TENVGC,TAXVGC,ATGKEC,ATGKEP,NWPTOC,NWPTOP,CONSCP,CONSPI,NONCCI,NONCPI,AIUICI,AIUIPI,SBTCST,SBTPRC,AMNCCI,AMNPPI,FCWISS,ITCTAS,ITTPMF,TRGPRC,SLSPRC,BDVPRC,CELPRC,BCHPGC,BCHDPT,TGADJP,YOYPCT,USRPRC,CNTPRC,CpqTableEntryId from ( select ROW_NUMBER() OVER(order by EQUIPMENT_LINE_ID) AS ROW, * from SAQICO (nolock)  where QUOTE_RECORD_ID ='"+str(RecAttValue) +"' AND QTEREV_RECORD_ID = '"+str(quote_revision_record_id)+"' and GREENBOOK = '"+str(TreeParam)+"' and SERVICE_ID = '"+str(ServiceId)+"' and FABLOCATION_ID = '"+str(Product.GetGlobal("TreeParentLevel0"))+"') m where m.ROW BETWEEN " + str(Page_start) + " AND " + str(Page_End) )
                            ...
                            if TreeParam == "Quote Items":
                                ...
                                Qury_str = (
                                    "select top "
                                        + str(PerPage)
                                        + " CASE WHEN STATUS = 'ACQUIRED' THEN '"+ imgstr +"' WHEN STATUS = 'CFG-ON HOLD TKM' THEN '"+ on_hold_tkm +"' WHEN STATUS = 'APPROVAL REQUIRED' THEN '" +exclamation+ "' WHEN STATUS = 'CFG-ON HOLD - COSTING' THEN '"+on_hold_costing+"' WHEN STATUS = 'PRR-ON HOLD PRICING' THEN '"+on_hold_pricing+"' WHEN STATUS = 'OFFLINE PRICING' THEN '"+offline_pricing+"' WHEN STATUS = 'ERROR' THEN '"+ error +"'  WHEN STATUS = 'PARTIALLY PRICED' THEN '"+ partially_priced +"' WHEN STATUS = 'ASSEMBLY IS MISSING' THEN '"+ assembly_missing +"'  ELSE '"+ acquiring_img_str +"' END AS  STATUS,QUOTE_ITEM_COVERED_OBJECT_RECORD_ID,LINE,CNTYER,OFRING,OBJECT_ID,OBJECT_TYPE,EQUPID,ASSEMBLY_ID,GOT_CODE,EQNODE,PROCES,PM_ID,MNTEVT_LEVEL,KIT_ID,KIT_NUMBER,FABLOC,GRNBOKQUANTITY,TOLCFG,SPQTEV,SPSPCT,SVSPCT,WTYSTE,WTYEND,WTYDAY,INWRTY,CNTDAY,BILTYP,TNTVGC,TENVGC,TAXVGC,ATGKEC,ATGKEP,NWPTOC,NWPTOP,CONSCP,CONSPI,NONCCI,NONCPI,AIUICI,AIUIPI,SBTCST,SBTPRC,AMNCCI,AMNPPI,FCWISS,ITCTAS,ITTPMF,TRGPRC,SLSPRC,BDVPRC,CELPRC,BCHPGC,BCHDPT,TGADJP,YOYPCT,USRPRC,CNTPRC,CpqTableEntryId from ( select  ROW_NUMBER() OVER( ORDER BY "+ str(Wh_API_NAMEs)
                                        +") AS ROW, * from SAQICO (NOLOCK) where "+ str(ATTRIBUTE_VALUE_STR)+" QUOTE_ID = '"
                                        + str(qt_rec_id.QUOTE_ID)
                                        + "' AND QTEREV_RECORD_ID = '"+str(quote_revision_record_id)+"') m where m.ROW BETWEEN "
                                        + str(Page_start)
                                ...
                                Qury_str = (
                                    "select top "
                                        + str(PerPage)
                                        + " CASE  WHEN STATUS = 'ACQUIRED' THEN '"+ imgstr +"' WHEN STATUS = 'CFG-ON HOLD TKM' THEN '"+ on_hold_tkm +"' WHEN STATUS = 'APPROVAL REQUIRED' THEN '" +exclamation+ "' WHEN STATUS = 'CFG-ON HOLD - COSTING' THEN '"+on_hold_costing+"' WHEN STATUS = 'PRR-ON HOLD PRICING' THEN '"+on_hold_pricing+"' WHEN STATUS = 'OFFLINE PRICING' THEN '"+offline_pricing+"' WHEN STATUS = 'ERROR' THEN '"+ error +"' WHEN STATUS = 'PARTIALLY PRICED' THEN '"+ partially_priced +"' WHEN STATUS = 'ASSEMBLY IS MISSING' THEN '"+ assembly_missing +"' ELSE '"+ acquiring_img_str +"' END AS STATUS, QUOTE_ITEM_COVERED_OBJECT_RECORD_ID, EQUIPMENT_LINE_ID, SERVICE_ID, EQUIPMENT_ID,LINE_ITEM_ID,YEAR_1, SERIAL_NO,GREENBOOK,FABLOCATION_ID,TECHNOLOGY,KPU, TARGET_PRICE, SALES_DISCOUNT_PRICE, CEILING_PRICE,NET_VALUE,CONTRACT_ID,CONVERT(VARCHAR(10),CONTRACT_VALID_FROM,101) AS [CONTRACT_VALID_FROM],CONVERT(VARCHAR(10),CONTRACT_VALID_TO,101) AS [CONTRACT_VALID_TO],CpqTableEntryId from ( select  ROW_NUMBER() OVER( ORDER BY "+ str(Wh_API_NAMEs)
                                        +") AS ROW, * from SAQICO (NOLOCK) where  QUOTE_ID = '"
                                        + str(qt_rec_id.QUOTE_ID)
                                        + "' AND SERVICE_ID = '"
                                        + str(LineAndEquipIDList)
                        ...
                        if Qustr:
                            if str(TreeParentParam)== "Billing":
                                ...
                                Qustr += " AND QTEREV_RECORD_ID = '{}' AND ISNULL(PAR_SERVICE_ID,'') = '{}' AND BILLING_DATE BETWEEN '{}' AND '{}'".format(quote_revision_record_id,par_ser_id,billing_date_column[0], billing_date_column[-1])                               
                        pivot_query_str = """
                                    SELECT ROW_NUMBER() OVER(ORDER BY EQUIPMENT_ID)
                                    AS ROW, *
                                        FROM (
                        ...
                        # )
                        QuryCount_str = (
                            "SELECT COUNT(QUOTE_REVISION_RECORD_ID) AS cnt FROM SAQTRV (nolock) WHERE "+str(ATTRIBUTE_VALUE_STR)+" QUOTE_RECORD_ID = '"
                            + str(contract_quote_record_id)
                            + "'"
                        )
                    elif RECORD_ID == 'SYOBJR-00010':
                        ...
                        error = '<img title="Error" src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/exclamation_icon.svg>'
                        cps_pricing_img = ""                     
                        #cps_pricing_img ='<a href="#" onclick=""><img src="/mt/APPLIEDMATERIALS_SIT/Additionalfiles/info.png"></a>' 
                        Qury_str = (
                            "SELECT DISTINCT TOP "
                            + str(PerPage)
                            + "QUOTE_ITEM_FORECAST_PART_RECORD_ID, CASE WHEN PRICING_STATUS = 'ACQUIRED' THEN '"+ imgstr +"' WHEN PRICING_STATUS = 'APPROVAL REQUIRED' THEN '" +exclamation+ "' WHEN PRICING_STATUS = 'ERROR' THEN '" +error+ "' ELSE '"+ acquiring_img_str +"' END AS PRICING_STATUS,SERVICE_ID,CONCAT('"+cps_pricing_img+ "',PART_NUMBER) AS PART_NUMBER,PART_DESCRIPTION,MATPRIGRP_ID,BASEUOM_ID,SCHEDULE_MODE,DELIVERY_MODE,UNIT_PRICE,UNIT_PRICE_INGL_CURR,EXTENDED_PRICE,EXTPRI_INGL_CURR,ANNUAL_QUANTITY,CUSTOMER_PART_NUMBER_RECORD_ID,BASEUOM_RECORD_ID,MATPRIGRP_RECORD_ID,QTEITM_RECORD_ID,QUOTE_RECORD_ID,QTEREV_RECORD_ID,SALESORG_RECORD_ID,SERVICE_RECORD_ID,PART_RECORD_ID,SALESUOM_RECORD_ID,CpqTableEntryId,TAX_AMOUNT_INGL_CURR,SRVTAXCLA_DESCRIPTION,TAX_PERCENTAGE from ( select TOP 10 ROW_NUMBER() OVER(order by "+ str(Wh_API_NAMEs) +") AS ROW, * from SAQIFP (nolock)  where "+ str(ATTRIBUTE_VALUE_STR)+" QUOTE_RECORD_ID ='"+str(RecAttValue)
                            +"' AND QTEREV_RECORD_ID = '"+str(quote_revision_record_id)+"' ) m where m.ROW BETWEEN "
                            + str(Page_start)
                            + " AND "
                            + str(Page_End)+" ORDER BY PRICING_STATUS ASC"
                    ...
                    ##involved parties equipmemt ends
                        ...
                        QuryCount_str = "select count(DISTINCT SERVICE_ID) as cnt from " + str(ObjectName) + " (nolock) " + str(Qustr)
                    ...
                    elif RECORD_ID == "SYOBJR-98882":
                        Qustr = " where "+str(ATTRIBUTE_VALUE_STR)+" "+ str(Wh_API_NAME) + " = '" + str(RecAttValue) + "' AND GREENBOOK = '"+str(TreeParentParam)+"' AND QTEREV_RECORD_ID = '"+str(quote_revision_record_id)+"'"
                    else:     
                        Qustr = " where "+ATTRIBUTE_VALUE_STR+" "+ str(Wh_API_NAME) + " = '" + str(RecAttValue) + "' AND QTEREV_RECORD_ID = '"+str(quote_revision_record_id)+"'"
                        Qustr = Qustr.replace("DEFAULT","[DEFAULT]") 
                        Qustr = Qustr.replace("PRIMARY","[PRIMARY]")
                        Qustr = Qustr.replace("[PRIMARY] CONTACT","PRIMARY CONTACT")
                    ...
                    if RECORD_ID == "SYOBJR-95868":                        
                        ...
                        if RECORD_ID == "SYOBJR-93122":
                            ...
                            if TreeParam == "Quote Preview":                                
                                ...
                                "select top "
                                    + str(PerPage)
                                    + " QUOTE_ITEM_COVERED_OBJECT_RECORD_ID, EQUIPMENT_LINE_ID, EQUIPMENT_ID,SERVICE_ID,LINE_ITEM_ID,"+col_year+",SERIAL_NO, GREENBOOK,FABLOCATION_ID, LINE, SALES_DISCOUNT_PRICE, NET_VALUE,CONTRACT_ID,CONVERT(VARCHAR(10),CONTRACT_VALID_FROM,101) AS [CONTRACT_VALID_FROM],CONVERT(VARCHAR(10),CONTRACT_VALID_TO,101) AS [CONTRACT_VALID_TO],CpqTableEntryId from ( select  ROW_NUMBER() OVER( ORDER BY EQUIPMENT_LINE_ID) AS ROW, * from SAQICO (NOLOCK) where QUOTE_ID = '"
                                    + str(qt_rec_id.QUOTE_ID)
                                    + "' AND QTEREV_RECORD_ID = '"+str(quote_revision_record_id)+"') m where m.ROW BETWEEN "
                                    + str(Page_start)
                                    + " and "
                            ...
                            if Product.GetGlobal("TreeParentLevel2") == "Quote Items":
                                ...
                                if TreeParam == "Quote Items":
                                    ...
                                    Qury_str = (
                                        "select top "
                                            + str(PerPage)
                                            + " CASE WHEN STATUS = 'ACQUIRED' THEN '"+ imgstr +"' WHEN STATUS = 'CFG-ON HOLD TKM' THEN '"+ on_hold_tkm +"' WHEN STATUS = 'APPROVAL REQUIRED' THEN '" +exclamation+ "' WHEN STATUS = 'CFG-ON HOLD - COSTING' THEN '"+on_hold_costing+"' WHEN STATUS = 'OFFLINE PRICING' THEN '"+offline_pricing+"' WHEN STATUS = 'PRR-ON HOLD PRICING' THEN '"+on_hold_pricing+"' WHEN STATUS = 'ERROR' THEN '"+ error +"'  WHEN STATUS = 'PARTIALLY PRICED' THEN '"+ partially_priced +"' WHEN STATUS = 'ASSEMBLY IS MISSING' THEN '"+ assembly_missing +"'  ELSE '"+ acquiring_img_str +"' END AS  STATUS,QUOTE_ITEM_COVERED_OBJECT_RECORD_ID,LINE,CNTYER,OFRING,OBJECT_ID,OBJECT_TYPE,EQUPID,ASSEMBLY_ID,GOT_CODE,EQNODE,PROCES,PM_ID,MNTEVT_LEVEL,KIT_ID,KIT_NUMBER,FABLOC,GRNBOK,QUANTITY,TOLCFG,SPQTEV,SPSPCT,SVSPCT,WTYSTE,WTYEND,WTYDAY,INWRTY,CNTDAY,BILTYP,TNTVGC,TENVGC,TAXVGC,ATGKEC,ATGKEP,NWPTOC,NWPTOP,CONSCP,CONSPI,NONCCI,NONCPI,AIUICI,AIUIPI,SBTCST,SBTPRC,AMNCCI,AMNPPI,FCWISS,ITCTAS,ITTPMF,TRGPRC,SLSPRC,BDVPRC,CELPRC,BCHPGC,BCHDPT,TGADJP,YOYPCT,USRPRC,CNTPRC,CpqTableEntryId from ( select  ROW_NUMBER() OVER( ORDER BY "+ str(wh)
                                            +") AS ROW, * from SAQICO (NOLOCK) where  QUOTE_ID = '"
                                            + str(qt_rec_id.QUOTE_ID)
                                            + "' AND QTEREV_RECORD_ID = '"+str(quote_revision_record_id)+"') m where m.ROW BETWEEN "
                                            + str(Page_start)
                            ...
                            Qury_str = (
                                "SELECT DISTINCT TOP "
                                + str(PerPage)
                                + "QUOTE_ITEM_FORECAST_PART_RECORD_ID, CASE WHEN PRICING_STATUS = 'ACQUIRED' THEN '"+ imgstr +"' WHEN PRICING_STATUS = 'APPROVAL REQUIRED' THEN '" +exclamation+ "' WHEN PRICING_STATUS = 'ERROR' THEN '" +error+ "' ELSE '"+ acquiring_img_str +"' END AS PRICING_STATUS,SERVICE_ID,CONCAT('"+cps_pricing_img+ "',PART_NUMBER) AS PART_NUMBER,PART_DESCRIPTION,MATPRIGRP_ID,BASEUOM_ID,SCHEDULE_MODE,DELIVERY_MODE,UNIT_PRICE,UNIT_PRICE_INGL_CURR,EXTENDED_PRICE,EXTPRI_INGL_CURR,ANNUAL_QUANTITY,CUSTOMER_PART_NUMBER_RECORD_ID,BASEUOM_RECORD_ID,MATPRIGRP_RECORD_ID,QTEITM_RECORD_ID,QUOTE_RECORD_ID,QTEREV_RECORD_ID,SALESORG_RECORD_ID,SERVICE_RECORD_ID,PART_RECORD_ID,SALESUOM_RECORD_ID,CpqTableEntryId,TAX_AMOUNT_INGL_CURR,SRVTAXCLA_DESCRIPTION,TAX_PERCENTAGE from ( select ROW_NUMBER() OVER(order by "+ str(Wh_API_NAMEs) +") AS ROW, * from SAQIFP (nolock)  where QUOTE_RECORD_ID ='"+str(RecAttValue)
                                +"' AND QTEREV_RECORD_ID = '"+str(quote_revision_record_id)+"' ) m where m.ROW BETWEEN "
                                + str(Page_start)
                                + " AND "
                                + str(Page_End)+" ORDER BY PRICING_STATUS ASC"
                            ...
                            if "ACQUIRED" in price_status and ('ACQUIRING' not in price_status and 'APPROVAL REQUIRED' not in price_status and 'ERROR' not in price_status):
                                ...
                                icon = '<img title="Acquiring" src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/exclamation_icon.svg'
                            elif "APPROVAL REQUIRED" in price_status and ('ACQUIRED' not in price_status and 'ERROR' not in price_status and 'ACQUIRING' not in price_status):
                                icon = '<img title="Acquiring" src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/clock_exe.svg>'
                            elif "ACQUIRING" in price_status and 'ACQUIRED' not in price_status and 'ERROR' not in price_status and 'APPROVAL REQUIRED' not in price_status:
                                icon = '<img title="Acquiring" src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/Cloud_Icon.svg>'
                ...
                if (Qury_str) == "" and str(QuryCount_str) == "":  
                    ...
                    if str(RECORD_ID) == "SYOBJR-00007":
                        ...
                        if Qustr:
                            if str(TreeParentParam)== "Billing":
                                Qustr += " AND BILLING_YEAR = '{}' AND SERVICE_ID = '{}' AND ISNULL(PAR_SERVICE_ID,'') = '{}' AND BILLING_DATE BETWEEN '{}' AND '{}'".format(SubTab,TreeParam,par_ser_id,billing_date_column[0], billing_date_column[-1])
                            else:
                                Qustr += " AND ISNULL(PAR_SERVICE_ID,'') = '{}' AND BILLING_DATE BETWEEN '{}' AND '{}'".format(par_ser_id,billing_date_column[0], billing_date_column[-1])
                        if TreeParam == "Z0123":
                                pivot_query_str = """
                                        SELECT ROW_NUMBER() OVER(ORDER BY EQUIPMENT_ID)
                                        AS ROW, *
                        ...
                        if delivery_date_column:                        
                            pivot_columns = ",".join(['[{}]'.format(delivery_date) for delivery_date in delivery_date_column])
                            Trace.Write('8648---delivery_date_column--pivot_columns----'+str(pivot_columns))
                            Qustr = " where " + str(ATTRIBUTE_VALUE_STR)+" "+ str(Wh_API_NAME) + " = '" + str(RecAttValue) + "'"
                            ...
                            column_before_delivery_pivot_changes = 'QTEREVSPT_RECORD_ID,PART_DESCRIPTION,PART_NUMBER,MATPRIGRP_ID,UOM_ID,SALESUOM_ID,SCHEDULED_MODE,DELIVERY_MODE,CUSTOMER_ANNUAL_QUANTITY,DELIVERY_QUANTITY,DELIVERY_SCHEDULE,CUSTOMER_PART_NUMBER,QUANTITY,DELIVERY_SCHED_DATE,CPQTABLEENTRYMODIFIEDBY,CPQTABLEENTRYDATEMODIFIED,QUOTE_REV_PO_PART_DELIVERY_SCHEDULES_RECORD_ID,CPQTABLEENTRYADDEDBY,CPQTABLEENTRYDATEADDED'
                            #column_before_delivery_pivot_changes = 'QTEREVSPT_RECORD_ID,PART_DESCRIPTION,PART_NUMBER,CUSTOMER_PART_NUMBER,QUANTITY,DELIVERY_SCHED_DATE,CPQTABLEENTRYMODIFIEDBY,CPQTABLEENTRYDATEMODIFIED,QUOTE_REV_PO_PART_DELIVERY_SCHEDULES_RECORD_ID,CPQTABLEENTRYADDEDBY,CPQTABLEENTRYDATEADDED'
                            if Qustr:
                                Qustr += " AND DELIVERY_SCHED_DATE  BETWEEN '{}' AND '{}'".format(delivery_date_column[0], delivery_date_column[-1])
                            pivot_query_str = """
                                        SELECT ROW_NUMBER() OVER(ORDER BY QTEREVSPT_RECORD_ID )
                        ...
                        if str(RECORD_ID)=="SYOBJR-98871" and 'CONTACT_NAME' in Qustr:
                            Qustr = Qustr.replace("'%","N'%")
                        Qury_str = (
                            "select top "
                            + str(PerPage)
                            + " "
                        ...
                        QuryCount_str = "select count(*) as cnt from " + str(ObjectName) + " (nolock) " + Qustr
                try:
            ...
            for ik in Query_Obj:				                  
                ...
                if str(current_tab).upper() == "PROFILE" and (ObjectName == "SYPROF"):
                    ...
                    if ObjectName != "SAQIBP" and ObjectName != "SAQTRV" and ObjectName != "SAQSCN" and ObjectName != "SAQDOC" and ObjectName != "SAQSPT":
                        ...
                        quote_revision_rec_id = Quote.GetGlobal("quote_revision_record_id")
                        docnode_action_btn = Sql.GetFirst("SELECT * FROM SAQDOC WHERE QUOTE_RECORD_ID = '{}' AND QTEREV_RECORD_ID = '{}' AND QUOTE_DOCUMENT_RECORD_ID = '{}'".format(Quote.GetGlobal("contract_quote_record_id"),quote_revision_rec_id,ik.QUOTE_DOCUMENT_RECORD_ID))
                        if (str(docnode_action_btn.DOCUMENT_DESCRIPTION) == "") and str(getRevision.REVISION_STATUS) not in ("BOK-CONTRACT BOOKED","BOK-CONTRACT CREATED","CBC-CBC COMPLETED"):
                            Action_str += '<li><a id = "" class="dropdown-item" href="#" " onclick="doc_edit_desc(this)">EDIT DESC</a></li>'	
                        # if docnode_action_btn:
                        #     Trace.Write("act_btn=="+str(docnode_action_btn.DATE_SUBMITTED))						
                        #     if str(docnode_action_btn.DATE_SUBMITTED) != "":
                ...
                if str(Action_permission.get("Delete")).upper() == "TRUE":
                    onclick = "CommonDelete(this, '" + str(ObjectName) + "', 'WARNING')"
                    if ObjectName == "SAQDLT" or ObjectName == "SAQFBL" or ObjectName == "SAQTSV":
                        Action_str += (
                                    '<li><a class="dropdown-item" href="#" id="deletebtn" onclick="'
                                    + str(onclick)
                                    + '" data-target="#cont_CommonModalDelete" data-toggle="modal">DELETE</a></li>'
                                )
                    elif ObjectName == "SAQSPT" and  qt_status.REVISION_STATUS in ('CFG-CONFIGURING','CFG-ACQUIRING','PRR-ON HOLD PRICING','PRI-PRICING','APR-APPROVAL PENDING','APR-RECALLED','APR-REJECTED'):
                        Action_str += (
                                    '<li><a class="dropdown-item" href="#" id="deletebtn" onclick="'
                                    + str(onclick)
                                    + '" data-target="#cont_CommonModalDelete" data-toggle="modal">DELETE</a></li>'
                    ...
                    # HPQC 1245 START
                    elif ObjectName in ["SAQTSV","SAQFBL","SAQSAO"]:
                        if ('<li><a class="dropdown-item" href="#" id="deletebtn" onclick="'+ str(onclick)+ '" data-target="#cont_CommonModalDelete" data-toggle="modal">DELETE</a></li>') not in Action_str:
                            Action_str += (
                                    '<li><a class="dropdown-item" href="#" id="deletebtn" onclick="'
                                    + str(onclick)
                                    + '" data-target="#cont_CommonModalDelete" data-toggle="modal">DELETE</a></li>'
                ...
                for inm in ik:
                    ...
                    if RECORD_ID != 'SYOBJR-98869':
                        ...
                        if value1234 == "PARTIALLY PRICED":
                            value1234 = value1234.replace("PARTIALLY PRICED","<img title='Partially Priced' src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/Red1_Circle.svg> PARTIALLY PRICED")
                        if value1234 == "OFFLINE PRICING":
                            value1234 = value1234.replace("OFFLINE PRICING","<img title='OFFLINE PRICING' src=/mt/APPLIEDMATERIALS_SIT/Additionalfiles/manual_pricing.svg> OFFLINE PRICING")
                        if value1234 == "NOT PRICED":
                            if RECORD_ID == 'SYOBJR-00005' or RECORD_ID =='SYOBJR-00029':
                                    value1234 = value1234.replace("NOT PRICED","<img title='NOT PRICED' class ='img-noprice' src=/mt/appliedmaterials_sit/Additionalfiles/no-price.svg>")
                        if value1234 ==  "CFG-ON HOLD TKM":
                    ...
                    if value123 in lookup_disply_list:
                        ...
                        if str(ObjectName) == "USERS":                            
                            ...
                            value1234 = str(ObjectName) + "-" + str(OBJ_CpqTableEntryId_New).rjust(5, "0")
                        ...
                        if value123 in checkbox_list:
                            ...
                            if value123 in [
                                ...
                                if str(value123) == "WARRANTY_END_DATE" and RECORD_ID != "SYOBJR-00009":
                                    ...
                                    Trace.Write("getdate_indication_billing-->"+str(value123))
                                    Trace.Write("getdate_indication--->"+str(value1234))                                       
                                    getdate_indication_billing = datetime.strptime(str(value123), '%m-%d-%Y')
                                    try:
                                        getdate_indication = datetime.strptime(str(value1234), '%m-%d-%Y') 
                                    except:
                                        getdate_indication = ''
                                    contract_quote_record_id = Product.GetGlobal("contract_quote_record_id")
                                    curr_symbol_obj = Sql.GetFirst(
                                    ...
                                    if curr_symbol_obj:
                                        ...
                                        try:
                                            ...
                                            value1234
                                    if getdate_indication:
                                        #Trace.Write('value1234--9929--'+str(value1234))                                         
                                        if getdate_indication > getdate_indication_billing:
                                            new_dict[value123] = (
                                                '<input  type= "text" id ="' + key_value + '" class= "billclassedit billclassedit_bg"  value="' + value1234 + '" style="border: 0px solid;"  disabled>'
                                            )
                                    ...
                                    if str(value123) != "CUSTOMER_ANNUAL_QUANTITY":                                                
                                            ...
                                            if value123 in precentage_columns:                                                    
                                                ...
                                                if value123 in img_list:
                                                    ...
                                                    if RECORD_ID == 'SYOBJR-00009' and value123 == 'QUOTE_ITEM_COVERED_OBJECT_RECORD_ID':
                                                        new_dict[value123] = ('<abbr id="'+key_value+'" title="'+str(value1234).upper()+'">'+str(value1234).upper()+'</abbr>')
                                                    elif value123 == "QUOTE_REV_DEAL_TEAM_MEMBER_ID":
                                                        new_dict[value123] = ('<a href="#" onclick="Commonteree_view_RL(this)" title="' + str(value1234).upper() + '">' + str(value1234).upper() + '</a>')
                                                        #new_dict[value123] = ('<abbr id ="' + key_value + '"  title="' + str(value1234).upper() + '">' +str(value1234).upper() + "</abbr>")
                ...
                if ObjectName == "SAQIBP":
                    if line != "":
                        lineQuery = " LINE IN (" + line + ") AND "
                
                footer_str, footer = "", ""
                footer_tot = gettotaldateamt = ""
                if ObjectName == "SAQIBP" and TreeParam != 'Quote Items':
                    ContractRecordId = Product.GetGlobal("contract_quote_record_id")
                    contract_quote_current_record_id = Quote.GetGlobal("contract_quote_current_record_id")
                    par_ser_id = ""
                    ...
                    if get_current_record:
                        if str(get_current_record.PAR_SERVICE_ID) != '':
                            par_ser_id = str(get_current_record.PAR_SERVICE_ID)
                    if TreeParam == "Z0117":
                        gettotaldateamt = Sql.GetList("SELECT BILLING_VALUE_INGL_CURR=SUM(BILLING_VALUE_INGL_CURR),ESTVAL_INGL_CURR = SUM(ESTVAL_INGL_CURR),ANNBILAMT_INGL_CURR = SUM(ANNBILAMT_INGL_CURR),BILLING_DATE FROM SAQIBP WHERE BILLING_DATE = '{billing_date_column}' and QUOTE_RECORD_ID ='{cq}' AND QTEREV_RECORD_ID='{revision_rec_id}' AND SERVICE_ID = '{service_id}' AND ISNULL(PAR_SERVICE_ID,'')='{par_ser_id}' GROUP BY BILLING_DATE".format(cq=str(ContractRecordId),revision_rec_id = quote_revision_record_id,billing_date_column=billing_date_column[0],service_id = TreeParam, par_ser_id=par_ser_id))
                    else:
                        gettotaldateamt = Sql.GetList("SELECT BILLING_VALUE_INGL_CURR=SUM(BILLING_VALUE_INGL_CURR),ESTVAL_INGL_CURR = SUM(ESTVAL_INGL_CURR),ANNBILAMT_INGL_CURR = SUM(ANNBILAMT_INGL_CURR),BILLING_DATE FROM SAQIBP WHERE BILLING_DATE in {billing_date_column} and QUOTE_RECORD_ID ='{cq}' AND QTEREV_RECORD_ID='{revision_rec_id}' AND SERVICE_ID = '{service_id}' AND {lineQry} ISNULL(PAR_SERVICE_ID,'')='{par_ser_id}' AND BILLING_YEAR = '{BillingYear}' group by BILLING_DATE,BILLING_YEAR ".format(cq=str(ContractRecordId),revision_rec_id = quote_revision_record_id,billing_date_column=str(tuple(billing_date_column)),service_id = TreeParam,BillingYear=SubTab, par_ser_id=par_ser_id, lineQry = lineQuery))
                    if gettotaldateamt:
                        my_format = "{:,." + str(decimal_place) + "f}"
                        for val in gettotaldateamt: 
                            gettotalamt = str(my_format.format(round(float(val.ANNBILAMT_INGL_CURR), int(decimal_place))))  
    ...
    def SPARE_PARTS_ORDERING(self,select_obj_str, Qustr,PerPage, Page_start, Page_End,Wh_API_NAMEs):
        ...
        if "CpqTableEntryId" in str(Wh_API_NAMEs) or "QUOTE_RECORD_ID" in str(Wh_API_NAMEs):
            ...
            no_child_count = Sql.GetFirst("select COUNT(*) AS CNT from SAQSPT (nolock) "+str(Qustr)+" AND (PAR_PART_NUMBER IS NULL OR PAR_PART_NUMBER ='') "+str(notin_condition))
            ...
            if no_child_count > 1000:
                ...
                while fetch_count < no_child_count: #fetching only 1000 records since we are not able to get more than 1000 records at a time
                    parts = Sql.GetList("SELECT "+str(select_obj_str)+",CpqTableEntryId from SAQSPT (nolock) "+str(Qustr)+" AND (PAR_PART_NUMBER IS NULL OR PAR_PART_NUMBER ='') "+str(notin_condition)+" ORDER BY CpqTableEntryId ASC OFFSET "+str(fetch_count)+" ROWS FETCH NEXT 1000 ROWS ONLY ")
                    fetch_count +=1000
                ...
                parts = Sql.GetList("SELECT TOP 1000 "+str(select_obj_str)+",CpqTableEntryId from SAQSPT (nolock) "+str(Qustr)+" AND (PAR_PART_NUMBER IS NULL OR PAR_PART_NUMBER ='') "+str(notin_condition)+" ORDER BY CpqTableEntryId ASC ")
                ordered_values.extend(parts) #appending parts without having any child parts
            Query_Obj = []
...
if ACTION == "PRODUCT_ONLOAD": 
    ...
    if RECORD_ID:
        ...
        ApiResponse = ApiResponseFactory.JsonResponse(ObjSYLDRTLIST.MDYNMICSQLOBJECTFILTER(RECORD_ID, ATTRIBUTE_NAME, ATTRIBUTE_VALUE, PerPage, PageInform, SortColumn, SortColumnOrder, PR_CURR, TP,subTab,line_item,equipment_id))
