...
try:
    if 'Param' in globals():    
        if hasattr(Param, 'CPQ_Columns'): 
            ...
            primaryQueryItems = SqlHelper.GetFirst( ""+ str(Parameter.QUERY_CRITERIA_1)+ " SYINPL (INTEGRATION_PAYLOAD,INTEGRATION_NAME,SESSION_ID,CPQTABLEENTRYDATEADDED,CPQTABLEENTRYADDEDBY)  select N''"+str(Final_data)+ "'',''INSTALLED BASE'','"+ str(timestamp_sessionid)+ "',Getdate(),''"+str(addby)+ "'' ' ")
    ...
    Log.Info("MAPOSTINBS ERROR---->:" + str(sys.exc_info()[1]))
    Log.Info("MAPOSTINBS ERROR LINE NO---->:" + str(sys.exc_info()[-1].tb_lineno))
