...
class FpmPriceUpdate:
    def __init__(self):
        self.domain=self.sales_org_id=self.sales_recd_id=self.qt_rev_id=self.quote_revision_id=self.quote_record_id=self.global_curr=self.global_curr_recid=self.service_id=self.batch_group_record_id=self.get_billing_type_val=self.exch_rate=self.exch=self.quote_id=self.tax_percentage=''
    ...
    def fetch_priceinfo(self):
        for i in self.price:    
            ...
            if self.service_id in('Z0100','Z0101'):
                self.insert_data.append((str(Guid.NewGuid()).upper(), Itemidinfo[0], Itemidinfo[-5],self.ancliary_netPrice, 'IN PROGRESS', QUOTE, self.quote_record_id, self.batch_group_record_id,str(Taxrate),str(core_credit_amount),i["taxValue"],self.ancliary_netValue,i["grossValue"],i["freightValue"],self.ancliary_netPrice,i["netPriceUnit"],i["netPriceUnitValue"],self.quote_revision_id))
            else:
                self.insert_data.append((str(Guid.NewGuid()).upper(), Itemidinfo[0], Itemidinfo[-5], i["netPrice"], 'IN PROGRESS', QUOTE, self.quote_record_id, self.batch_group_record_id,str(Taxrate),str(core_credit_amount),i["taxValue"],i["netValue"],i["grossValue"],i["freightValue"],i["netPrice"],i["netPriceUnit"],i["netPriceUnitValue"],self.quote_revision_id))
    ...
    def fpm_batchupdate(self):
        ...
        if GetEquipment_count.CNT > 0:
            ...
            getstatus=SqlHelper.GetFirst("""SELECT COUNT(PRICING_STATUS) AS CNT FROM SAQIFP WHERE QUOTE_RECORD_ID='{QuoteRecordId}' AND PRICING_STATUS='ERROR' AND QTEREV_RECORD_ID='{rev}'""".format(QuoteRecordId=self.quote_record_id,rev =self.quote_revision_id))
            if getstatus.CNT>0:
                ...
                getstatus=SqlHelper.GetFirst("""SELECT COUNT(PRICING_STATUS) AS CNT FROM SAQIFP WHERE QUOTE_RECORD_ID='{QuoteRecordId}' AND PRICING_STATUS IN('ERROR','ACQUIRING','NOT PRICED') AND QTEREV_RECORD_ID='{rev}'""".format(QuoteRecordId=self.quote_record_id,rev =self.quote_revision_id))
                if getstatus.CNT==0:
...
Log.Info("CPS Princing Start ==>"+str(start_time))
