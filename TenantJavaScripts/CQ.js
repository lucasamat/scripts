...
function modelEquipmentPaginationFunc(showResultCount, recordEnd, tableId) {
	...
	//A055S000P01-20958-Start-A
	else if (tableId == "ADDNEW__SYOBJR_00040_SYOBJ_00937"){
        ele_id = "add_receiving_equipment"
        footer_id = "add_receiving_equipment_footer"
    }
	//A055S000P01-20958-End-A
	else {
...
function modelFabPaginationFunc(showResultCount, recordEnd, tableId) {
	...
	if (tableId == "ADDNEW__SYOBJR_98857_SYOBJ_01033") {
		...
		ele_id = "fablocation_addnew"
		footer = "fablocation_footer" //A055S000P01-20958-A
	}
		...
		cpq.server.executeScript("SYUADNWPOP", {
			...
			try {
				...
				$('#nso_addnew > tbody > tr').each(function () {
					...
					$(this).find('td:nth-child(' + gb_index + ')').html(html_text);
					var html_text = $(this).find('td:nth-child(' + division_index + ')').text();
					$(this).find('td:nth-child(' + division_index + ')').html(html_text);
					var html_text = $(this).find('td:nth-child(' + bu_index + ')').text();
					$(this).find('td:nth-child(' + bu_index + ')').html(html_text);
					var html_text = $(this).find('td:nth-child(' + poss_nso_index + ')').text();
					$(this).find('td:nth-child(' + poss_nso_index + ')').html(html_text);
					var html_text = $(this).find('td:nth-child(' + poss_desc_index + ')').text();
					$(this).find('td:nth-child(' + poss_desc_index + ')').html(html_text);
				})
			}
			// INC08786210 - Ends - M
			if (tableId == "ADDNEW__SYOBJR_98789_SYOBJ_00919") {
                $('#' + footer).html(data6)
            }
            else{
				$('#' + footer).html(data6) //A055S000P01-20958-M
            }
			eval(data5);
		});
	} catch (e) {
...
function addCredits(ele) {
	...
	$('#add_credits_add_new').find('[type="checkbox"]:checked').map(function () {
		if ($(this).attr('name') == 'btSelectAll') {
			...
			// INC08795842 - Starts - A
			var edit_index = $("#add_credits_add_new").find("[data-field='CREDITVOUCHER_RECORD_ID']").index() + 1;
			var sales_order_no = $(this).closest('tr').find('td:nth-child(' + edit_index + ')').text();
			// INC08795842 - Ends - A
			...
			credit_applied[sales_order_no] = applied_credit;
			if (sel_val != '') {
	...
	// 	if (!(credit_note in credit_note)){
	// 		credit_note += " "+credit_note+" "
	// 	}
	// });
	edit_index = $("#add_credits_add_new").find("[data-field='ZAFNOTE']").index() + 1;
	credit_note = $("#add_credits_add_new > tbody > tr > td:nth-child(" + edit_index + ")").text();
	try {
...
function addcoveredobj(ele, CoveredRecordId) {
	...
	try {
		cpq.server.executeScript("CQCRUDOPTN", {
			...
			'tools_from_ui': 'Yes' //INC08696637 - A
		}, function () {
			//$("#MM_ALL_REFRESH").click();
			$('#cont_viewModalSection').css('display', 'none');
		...
		cpq.server.executeScript("CQSTSVADTN", { 'CurrentTab': CurrentTab, 'quote_item_insert': quote_item_insert, 'Text': prog_bar }, function (datasetres) {
			...
			if (datasetres[1] != '') {
				$('.notificatsection').css('display','block');
                    ...
                    $('.cpq_cust_warning').css('display','block');
                    $('.alert-warning').css('display','block');
                    if (!($('.cpq_cust_warning').text().includes("MISSING ATTRIBUTES FROM IBASE")) == true) {
                        $('.cpq_cust_warning').html(datasetres[1]);
                    }
			}
			if (dataset != '') {
				//if (dataset[0] != ''){
				if (que == 'REJECTED' && editbtn_disp == 'True') {
                    ...
                    else if (datasetres[1].includes('REQUIRED EVENT IS MISSING') && prog_bar == "COMPLETE STAGE") {
                        $(".cpq_notification_div .cpq_cust_warning.alert-warning").css("display", "block").parent().css("display", "block").parent().prev().children("div").removeClass("collapsed");
                        $("#notify").addClass("in");
                        $("#notify").css("height", "auto");
                    }
					...
					$('div[id^=dyn] div#ctr_drop').css('display', 'none');
					if (TreeParam != "Quote Documents") {
						$("div#ctr_drop").css('display', 'block');		//A055S000P01-20974 - Start
						$(".btn-group").css('display','inline-block'); 	//A055S000P01-20974 - End
					}
					$("div#ctr_drop.cbc_ctr_drop").css('display', 'block');
					$('#recall_for_edit').hide();
					$('.segment_table').find('li').find('a:contains("EDIT")').remove();
            ...
            //INC08614363 - A
			if(prog_bar == "COMPLETE STAGE" && datasetres[4]==true){
                $('.continue_cmpt_stage').css('display','block').parent().css('display','block').parent().prev().children("div").removeClass("collapsed");
                $("#notify").addClass("in");
                $("#notify").css("height", "auto");
            }
			//INC08614363 - A
			//INC08824195 - A
			if(datasetres[5]!=''){
				if (!($('.cpq_cust_notify').text().includes("NEW COST MODEL VERSIONS AVAILABLE")) == true) {
					$('.cpq_cust_notify').html(datasetres[5]);
					$('.cpq_cust_notify').css('display','block');
					$('#cartContainer > div > div > div.row.padt-10.notificatsection').css('display','block');
					$("#notify").addClass("in");
                	$("#notify").css("height", "auto");
				}
            }
		...
		if (get_pick_annualitems == 'Contract Information') {
			...
			cpq.server.executeScript("CQANULEDIT", { 'ACTION': 'CAT4_ENTITLMENT', 'values': lines }, function (dataset) {
				...
				for (k in obj_dataset) {
					...
					$('#SYOBJR_00009_E5504B40_36E7_4EA6_9774_EA686705A63F tbody tr').each(function() {
						if (k == ($(this).find("td:nth-child(4) abbr").html() || $(this).find("td:nth-child(4)").text())) {
							va = obj_dataset[k]
							btnFlag = (va.length > 0 && btnFlag == false) ? true : btnFlag //CR095B
				...
				if (btnFlag) {
					$('#bulkedit_annualized_items').hide(); //CR095B
...
function annualizedSave() {
	...
	cpq.server.executeScript("CQUPPRWLFD", { 'Records': Records,'Mode':'Inline Edit'}, function (data0) {
		localStorage.setItem('Annualized_Values', '');
		annualizedCancel();
		primary_banner();
		subTabDetails('Annualized Items', 'Related', 'SAQITM', 'SYOBJR-00010');
		cpq.server.executeScript("CQBLKANEDT", { 'ACTION': "WRITEBACK"});
	});
...
function add_receving_equipment(ele) {
	...
	try {
		cpq.server.executeScript("CQCRUDOFTS", {
			...
			$('#COMMON_TABS > ul > li').removeClass('active');
			$('div#COMMON_TABS').find("li a:contains('Receiving Equipment')").parent().addClass('active');
			// INC08632845 - End - M
		});
	} catch (e) {
		console.log(e);
...
function annualgrid_show() {
	...
	var get_pick_annualitems = $("#anual_grid_picklist").val();
	if (get_pick_annualitems == 'Contract Information') {
		$('#bulkedit_annualized_items').hide();
		// A055S000P01-20868 Start - M
		var restrictedColumns = ['QUOTE_ITEM_COVERED_OBJECT_RECORD_ID', 'ATGKEC', 'ATGKEP', 'NWPTOC', 'NWPTOP', 'CONSCP', 'CONSPI', 'NONCCI', 'NONCPI', 'AIUICI', 'AIUIPI', 'SBTCST', 'SBTPRC', 'AMNCCI', 'AMNPPI', 'FCWISS', 'ITTPMF', 'TRGPRC', 'SLSPRC', 'BDVPRC', 'CELPRC', 'TGADJP', 'YOYPCT', 'USRPRC', 'CNTPRC', 'WTYDAY', 'INWRTY', 'CNTDAY', 'BILTYP', 'OBJECT_ID', 'TNTVGC', 'TENVGC', 'BCHPGC', 'BCHDPT', 'OBJECT_ID', 'TNTVGC', 'TENVGC', 'BCHPGC', 'BCHDPT', 'ATKNCI', 'ATKNPI', 'CUSTOMER_TOOL_ID', 'PLTFRM', 'TECHNOLOGY', 'PRODID,ITWSGP', 'CNTCVG_ENT', 'AIUPPE'];
		// A055S000P01-20868 End - M

	}
	else if (get_pick_annualitems == 'Pricing Review - Manual Information') {
		if ($('.segment_revision_sale_id_text').text() == "PRR-RECALCULATING"){
			$('#bulkedit_annualized_items').hide();
		}
		else {
			$('#bulkedit_annualized_items').show();
		}
		//CR095B-20534
		var restrictedColumns = ['QUOTE_ITEM_COVERED_OBJECT_RECORD_ID', 'SPQTEV', 'SPSPCT', 'BILTYP', 'INWRTY', 'SVSPCT', 'WTYSTE',
			'WTYEND', 'WTYDAY', 'TNTVGC', 'TENVGC', 'TAXVGC', 'TGADJP', 'YOYPCT', 'USRPRC', 'CNTPRC', 'ITTPMF','TAXVGC','ITCSTI','AICPPE','QTCNST','QTCNED','ITCTAS','FCWISS'];
		cpq.server.executeScript("CQBLKANEDT", { 'ACTION': 'BUTTON VISIBILITY' }, function (dataset) {
			if(dataset=="True"){
				$('#bulkedit_annualized_items').css('display','block');
			}
		});
	}
	else if (get_pick_annualitems == 'Pricing - Summary Information') {
		...
		var restrictedColumns = ['QUOTE_ITEM_COVERED_OBJECT_RECORD_ID', 'SPQTEV', 'SPSPCT', 'SVSPCT', 'WTYSTE', 'WTYEND', 'WTYDAY',
			'INWRTY', 'BILTYP', 'TNTVGC', 'TENVGC', 'TAXVGC', 'ATGKEC', 'ATGKEP', 'NWPTOC', 'NWPTOP', 'CONSCP', 'CONSPI', 'NONCCI', 'NONCPI', 'AIUICI', 'AIUIPI', 'SBTCST', 'SBTPRC', 'AMNCCI',
			'AMNPPI', 'FCWISS', 'ITSRNO', 'ITCSTI', 'AICPPE', 'QTCNST', 'QTCNED', 'ITTPMF','TAXVGC','CUSTOMER_TOOL_ID','PLTFRM','TECHNOLOGY','PRODID','ITWSGP','CNTCVG_ENT','ATKNCI','ATKNPI','BDVPRC'];
...
//CR095B starts here.
function bulkEditAnnualizedItems() {
	$('#SYOBJR_98883_E5504B40_36E7_4EA6_9774_EA686705A63FQI').remove();
	$('#annualized_bulkedit_reset_criteria').addClass('blkedt_disabled').removeAttr('onclick');
	$('#annualized_bulkedit_save').addClass('blkedt_disabled').removeAttr('onclick');
	try {
		cpq.server.executeScript("CQBLKANEDT", { 'ACTION': 'ITEMS_QUERY', 'RECORD_ID': 'SYOBJR-98883' }, function (dataset) {
			$('#annualized_item_tab > div.annual_item_tab_details').html(dataset.TABLE_CONTENT);
			var dataCount = dataset.TABLE_DATA.length;
			$('#' + dataset.TABLE_ID).bootstrapTable({
				data: dataset.TABLE_DATA
			});
			if (!dataCount) {
				$("#" + dataset.TABLE_ID + " >  tbody").html('<tr><td colspan = 40><div id="noRecDisp" class="noRecord">No Records to Display</div></td></tr>');
			}
			$('#SYOBJR_98883_E5504B40_36E7_4EA6_9774_EA686705A63FQI').wrapAll('<div class="benchmarkdiv"></div>');
			$('#SYOBJR_98883_E5504B40_36E7_4EA6_9774_EA686705A63FQI').parent().after(dataset.FOOTER);
			eval(dataset.RELATED_DROP_STR);
			$('#SYOBJR_98883_E5504B40_36E7_4EA6_9774_EA686705A63FQI > thead > tr').on('keyup change',function () {
				var btnFlag = false;
				Object.values(getAnnSearchDict()).forEach(function (ele) {
					if (ele != '') {
						btnFlag = true;
						return false;
					}
				...
				if (btnFlag) {
					$('#annualized_bulkedit_reset_criteria').removeClass('blkedt_disabled').attr('onclick', 'bulkEditAnnualizedItems()');
				}
				else {
					$('#annualized_bulkedit_reset_criteria').addClass('blkedt_disabled').removeAttr('onclick');
				}
			});
			$('#annualized_bulkedit_popup > #edit_decrip').on("click", function(event){
				if(!$(event.target).closest(".multiselect-qtlist-cntryofacc").length){
					$('[id*="selectallmultichkbox_"]').hide();
				}
			});
			costPriceImpactsGrid();
		});
	}
...
function getAnnSearchDict() {
	var search_dict = {};
	var table_id = 'SYOBJR_98883_E5504B40_36E7_4EA6_9774_EA686705A63FQI'
	//A055S000P01-20922-Start-M
	$("#" + table_id + ' .filter-control').each(function () {
		values = this.firstElementChild.className;
		var data_field = $(this).parent().parent().attr('data-field');
		if (values.indexOf('control') !== -1 && values.indexOf('multiselect-qtlist-cntryofacc') === -1) {
			search_dict[data_field] = $(this).find('input').val().replaceAll('\t','');
			
		}
		else if (data_field == "TOLCFG") {
			search_dict[data_field] = $(this).find('#listchekSec').text().replaceAll(',', '|').replaceAll('\t','');
		}
		else {
			search_dict[data_field] = $(this).find('#listchekSec').text().replaceAll('\t','');
		}
	});
	//A055S000P01-20922-End-M
	delete search_dict.FILTER;
	delete search_dict.ACTIONS;
	return search_dict;
}

function searchQueryGrid() {
	var TABLE_ID = 'SYOBJR_98883_E5504B40_36E7_4EA6_9774_EA686705A63FQI'
	var pageCount = $('#' + TABLE_ID + '_PageCountValue');
	var startVal = '1'
	cpq.server.executeScript("CQBLKANEDT", { "ACTION": "QUERY_FILTER", "RECORD_ID": "SYOBJR-98883", "SEARCH_DICT": getAnnSearchDict(), "PER_PAGE": $(pageCount).val(), "RECORD_START": startVal, "SORT_COLUMN": localStorage.getItem('#' + TABLE_ID + '_SortColumn'), "SORT_ORDER": localStorage.getItem('#' + TABLE_ID + '_SortColumnOrder') }, function (dataset) {
		try {
			$('#' + TABLE_ID).bootstrapTable('load', dataset.TABLE_DATA);
		}
		catch (e) {
		...
		var dataCount = dataset.TABLE_DATA.length;
		var startEnd = $('#' + TABLE_ID + '_NumberofItem');
		var totalRecords = $('#' + TABLE_ID + '_totalItemCount');
		var pageNo = $('#' + TABLE_ID + '_page_count');
		var pageCount = $('#' + TABLE_ID + '_PageCountValue');
		var pageCountVal = pageCount.val();
		$(totalRecords).text(dataset.COUNT);
		if (dataCount) {
			if (parseInt(dataCount) < parseInt(pageCountVal)) {
				var endCount = dataCount
			}
			else {
				var endCount = pageCountVal
			}
			endCount = '1 - ' + endCount + ' of'
			$(startEnd).text(endCount);
			$(totalRecords).text(dataset.COUNT);
			$(pageNo).text('1');
		}
		else {
			$("#" + TABLE_ID + " >  tbody").html('<tr><td colspan = 40><div id="noRecDisp" class="noRecord">No Records to Display</div></td></tr>');
			$(startEnd).text('0 - 0 of');
			$(totalRecords).text('0');
			$(pageNo).text('1');
		}
		costPriceImpactsGrid()
...
function costPriceImpactsGrid() {
	cpq.server.executeScript("CQBLKANEDT", { "ACTION": "IMPACTS_GRID", "SEARCH_DICT": getAnnSearchDict() }, function (dataset) {
		$('.annual_item_query').html(dataset.TABLE_CONTENT);
		$('#' + dataset.TABLE_ID).bootstrapTable({
			data: dataset.TABLE_DATA
		});
		eval(dataset.RELATED_DROP_STR);
		count_index = $("#impacts_grid").find("[data-field='COST_ROW_COUNT']").index() + 1;
		cst_val_index = $("#impacts_grid").find("[data-field='COST_IMPACT_VALUE']").index() + 1;
		prc_val_index = $("#impacts_grid").find("[data-field='PRICE_IMPACT_VALUE']").index() + 1;
		$("#impacts_grid").find("tbody > tr").each(function (ele) {
			var count_value = $(this).find("td:nth-child(" + count_index + ")").text();
			if (count_value != '0') {
				$(this).find("td:nth-child(" + cst_val_index + ")").addClass('impact_value_edit').html('<input type = "number" class = "light_yellow">').attr('onkeyup', 'impactCalculate(this)');
				$(this).find("td:nth-child(" + prc_val_index + ")").addClass('impact_value_edit').html('<input type = "number" class = "light_yellow">').attr('onkeyup', 'impactCalculate(this)');
			}
		});
	});
}

function queriedItemsFilter(action, RECORD_ID, TABLE_ID) {
	var startEnd = $('#' + TABLE_ID + '_NumberofItem');
	var totalRecords = $('#' + TABLE_ID + '_totalItemCount');
	var pageNo = $('#' + TABLE_ID + '_page_count');
	var pageCount = $('#' + TABLE_ID + '_PageCountValue');
	var startVal = $(startEnd).text().split('-')[0].trim();
	var endVal = $(startEnd).text().split('-')[1].replace('of', '').trim();
	var pageNumber = 0;
	if (action == "sort" || action == "countchange") {
		recordStart = 1;
		pageNumber = 1;
	}
	else if (action == "first") {
		recordStart = parseInt("1")
		pageNumber = 1;
	}
	else if (action == "previous") {
		recordStart = parseInt(startVal) - parseInt($(pageCount).val())
		recordStart = (recordStart > 0) ? recordStart : "1"
		pageNumber = parseInt($(pageNo).text()) - 1
	}
	else if (action == "next") {
		recordStart = parseInt(startVal) + parseInt($(pageCount).val())
		recordStart = (recordStart > parseInt($(totalRecords).text())) ? parseInt($(totalRecords).text()) : recordStart
		pageNumber = parseInt($(pageNo).text()) + 1
	}
	else if (action == "last") {
		recordStart = parseInt($(totalRecords).text()) - parseInt($(pageCount).val()) + 1
	}
	recordEnd = parseInt(recordStart) + parseInt($(pageCount).val()) - 1
	recordEnd = (recordEnd > parseInt($(totalRecords).text())) ? $(totalRecords).text() : recordEnd
	recordStart = (recordStart < parseInt($(pageCount).val())) ? parseInt("1") : recordStart
	var restrictFlag = true;
	if (action == "last" && parseInt(recordStart) + parseInt($(pageCount).val()) == parseInt($(totalRecords).text())) {
		restrictFlag = false;
	}
	if (parseInt(startVal) == parseInt(recordStart) && action != "countchange") {
		restrictFlag = false;
	}
	if (parseInt(recordStart) == parseInt($(totalRecords).text())) {
		restrictFlag = false;
	}
	if (restrictFlag || action == "sort") {
		cpq.server.executeScript("CQBLKANEDT", { "ACTION": "QUERY_FILTER", "SEARCH_DICT": getAnnSearchDict(), "PER_PAGE": $(pageCount).val(), "RECORD_START": recordStart, "SORT_COLUMN": localStorage.getItem('#' + TABLE_ID + '_SortColumn'), "SORT_ORDER": localStorage.getItem('#' + TABLE_ID + '_SortColumnOrder') }, function (dataset) {
			try { $('#' + TABLE_ID).bootstrapTable('load', dataset.TABLE_DATA); }
			catch (e) { console.log(e); }
			var length = dataset.TABLE_DATA.length
			//recordEnd = (length > recordEnd) ? length : recordEnd;
			if (length > recordEnd){
				if (length > parseInt($(pageCount).val())){
					recordEnd = parseInt($(pageCount).val());
				}
				else {
					recordEnd = length;
				}
			...
			$('#' + TABLE_ID + '_NumberofItem').text(recordStart + " - " + recordEnd + " of ");
			$('#' + TABLE_ID + '_totalItemCount').text(dataset.COUNT);
			if (action == "last") {
				pageNumber = Math.floor(parseInt(dataset.COUNT) / parseInt($(pageCount).val()));
			}
			else if (action == "sort") {
				pageNumber = Math.ceil(parseInt(recordStart) / parseInt($(pageCount).val()));
			}
			$('#' + TABLE_ID + '_page_count').text(pageNumber);
			if (action == "sort") {
				costPriceImpactsGrid();
			}
			if(length==0){
				$("#" + TABLE_ID + " >  tbody").html('<tr><td colspan = 40><div id="noRecDisp" class="noRecord">No Records to Display</div></td></tr>');
				$('#' + TABLE_ID + '_NumberofItem').text('0 - 0 of');
				$('#' + TABLE_ID + '_totalItemCount').text('0');
				$('#' + TABLE_ID + '_page_count').text('0');
			}
...
function bulkeditSave() {
	var costImpIndex = $("#impacts_grid").find("[data-field='COST_IMPACT_VALUE']").index() + 1;
	var totalCostImpIndex = $("#impacts_grid").find("[data-field='TOTAL_COST_IMPACT']").index() + 1;
	var priceImpIndex = $("#impacts_grid").find("[data-field='PRICE_IMPACT_VALUE']").index() + 1;
	var totalPriceImpIndex = $("#impacts_grid").find("[data-field='TOTAL_PRICE_IMPACT']").index() + 1;
	var apiIndex = $("#impacts_grid").find("[data-field='COST_IMPACT_FIELD']").index() + 1;
	var impactValues = [];
	$('#impacts_grid > tbody > tr').each(function () {
		var valDict = {};
		valDict.COST_IMPACT = $(this).find('td:nth-child(' + costImpIndex + ') > input').val();
		valDict.PRICE_IMPACT = $(this).find('td:nth-child(' + priceImpIndex + ') > input').val();
		valDict.TOTAL_COST_IMPACT = $(this).find('td:nth-child(' + totalCostImpIndex + ')').text()
		valDict.TOTAL_PRICE_IMPACT = $(this).find('td:nth-child(' + totalPriceImpIndex + ')').text()
		valDict.API_NAME = $(this).find('td:nth-child(' + apiIndex + ') > abbr').attr('name');
		impactValues.push(valDict);
	});
	cpq.server.executeScript("CQBLKANEDT", { "ACTION": "SAVE", "SEARCH_DICT": getAnnSearchDict(), "IMPACT_VALUES": impactValues }, function (dataset) {
		$('#bulkedit_annualized_items').hide();
		subTabDetails('Annualized Items', 'Related', 'SAQITM', 'SYOBJR-00010');
		primary_banner();
		if ($('.segment_revision_sale_id_text').text() == 'PRR-PRICING REVIEWED' || $('.segment_revision_sale_id_text').text() == 'PRR-ON HOLD PRICING') {
			$('#bulkedit_annualized_items').show();
		}
	});
}

function impactCalculate(ele) {
	var elementText = $(ele).find('input').val();
	var hasValue = false;
	if (elementText != '' && elementText != undefined) {
		$(ele).addClass('cust_usd');
		$(ele).next("td").text(parseFloat($(ele).prev("td").text() * elementText).toFixed(2)).addClass('cust_usd pos-relative');
	}
	else {
		$(ele).removeClass('cust_usd');
		$(ele).next("td").text('').removeClass('cust_usd pos-relative');
	}
	$('#impacts_grid > tbody > tr').each(function () {
		if ($(this).find('td:nth-child(3) > input').val() || $(this).find('td:nth-child(7) > input').val()) {
			hasValue = true;
			return false;
		}
	})
	if (hasValue) {
		$('#annualized_bulkedit_save').removeClass('blkedt_disabled').attr('onclick', 'bulkeditSave()').attr('data-dismiss', 'modal');
	}
	else {
		$('#annualized_bulkedit_save').addClass('blkedt_disabled').removeAttr('onclick').removeAttr('data-dismiss');
	}
}

function pricingReviewed() {
	cpq.server.executeScript("CQBLKANEDT", { "ACTION": "PRICING REVIEWED" }, function (dataset) {
		pricing_flag = dataset[1];
		if (pricing_flag == true){
			localStorage.setItem("get_button_price", "show_button");
			$('#CALCULATE_QItems').css('display', 'block');
			$("#step1").removeClass("disabled active").addClass("complete");
			$("#step1 .bar_number").css("display", "none");
			...
			$("#step2 .bar_tick_img").css("display", "block");
			$("#step3").removeClass("disabled").addClass("active");
			$("#step4").addClass("disabled").removeClass("active");
			primary_banner();
			$('#bulkedit_annualized_items').hide();
		}
