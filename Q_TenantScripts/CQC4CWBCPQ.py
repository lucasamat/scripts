...
def salesteam_insert(employee,quote_object,quote_rev_id,quote_revision_record_id,quote_record_id):
    ...
    if created_by_master_rec:
        salesteam_object = Sql.GetFirst("SELECT C4C_PARTNERFUNCTION_ID FROM SAQDLT (NOLOCK) WHERE QUOTE_RECORD_ID = '{}' AND C4C_PARTNERFUNCTION_ID = 'CREATED BY' AND QTEREV_RECORD_ID = '{}'".format(quote_object.MASTER_TABLE_QUOTE_RECORD_ID,quote_revision_record_id))
        if not salesteam_object:
            quote_employee_obj = Sql.GetFirst("SELECT MEMBER_ID,MEMBER_NAME,MEMBER_RECORD_ID FROM SAQDLT (NOLOCK) WHERE QUOTE_RECORD_ID = '{}' AND C4C_PARTNERFUNCTION_ID = 'EMPLOYEE RESPONSIBLE' AND QTEREV_RECORD_ID = '{}' ".format(quote_object.MASTER_TABLE_QUOTE_RECORD_ID,quote_revision_record_id))
            if quote_employee_obj is not None:
                ...
                Sql.RunQuery("UPDATE SAQTMT SET OWNER_ID = '{owner_id}',OWNER_NAME='{owner_name}',OWNER_RECORD_ID = '{owner_rec_id}' WHERE MASTER_TABLE_QUOTE_RECORD_ID = '{quote_rec_id}' AND QTEREV_RECORD_ID = '{quote_rev_rec_id}'".format(owner_id = quote_employee_obj.MEMBER_ID,owner_name = quote_employee_obj.MEMBER_NAME, owner_rec_id = quote_employee_obj.MEMBER_RECORD_ID, quote_rec_id = quote_object.MASTER_TABLE_QUOTE_RECORD_ID,quote_rev_rec_id = quote_revision_record_id))

                #Log.Info("chk_owner_update--- "+str("UPDATE SAQTMT SET OWNER_ID = '{owner_id}',OWNER_NAME='{owner_name}',OWNER_RECORD_ID = '{owner_rec_id}' WHERE MASTER_TABLE_QUOTE_RECORD_ID = '{quote_rec_id}' AND QTEREV_RECORD_ID = '{quote_rev_rec_id}'".format(owner_id = quote_employee_obj.MEMBER_ID,owner_name = quote_employee_obj.MEMBER_NAME, owner_rec_id = quote_employee_obj.MEMBER_RECORD_ID, quote_rec_id = quote_object.MASTER_TABLE_QUOTE_RECORD_ID,quote_rev_rec_id = quote_revision_record_id)))
