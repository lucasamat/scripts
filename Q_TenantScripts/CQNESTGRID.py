...
def GetEquipmentMaster(PerPage, PageInform, A_Keys, A_Values):
    ...
    for par in parent_obj:
        ...
        if str(ibase_status) == "INCOMPLETE" and str(ibase_greenbook) =="DISPLAY":
            ...
            #ibase_icon = '<a class = "cust_exc" title="Missing iBase Attributes"><img src="/mt/appliedmaterials_uat/Additionalfiles/Icons/Exclamation.svg"></a>'
            ibase_missing_flag = True
            for key,col in ibase_attributes.items():
                if not col:
                    err_title += "Attribute "+str(key)+" is missing from the iBase \n"
            ibase_icon = '<a class = "cust_exc" title="'+str(err_title)+'"><img src="/mt/appliedmaterials_uat/Additionalfiles/Icons/Exclamation.svg"></a>'
            ...
            ibase_icon = '<a class = "cust_exc" title="Complete"><img src="/mt/appliedmaterials_uat/Additionalfiles/Green_Tick.svg"></a>'
            err_title +="Complete"
    ...
    if ibase_missing_flag:
        ...
        filter_control_function += ('$(".cpq_notification_div .cpq_cust_warning").html("<img src=\'/mt/APPLIEDMATERIALS_UAT/Additionalfiles/warning1.svg\' class=\'wid20_px\'><p>'+str(missing_text)+'</p>").css("display","block").parent().css("display","block").parent().prev().children("div").removeClass("collapsed");$("#notify").addClass("in");$("#notify").css("height","auto");')
    else:
...
def UpdateBreadcrumb():    
    ...
    Action_Str += '</abbr></a><span class="angle_symbol"><img src="/mt/appliedmaterials_uat/images/productimages/BREADCRUMB_ICON_TRANS.PNG"></span></li>'
...
def GetCovObjMaster(PerPage, PageInform, A_Keys, A_Values):    
    ...
    for par in parent_obj:
        ...
        if str(ibase_status) == "INCOMPLETE":
            ...
            ibase_icon = '<a class = "cust_exc" title="Missing iBase Attributes for assemblies"><img src="/mt/appliedmaterials_uat/Additionalfiles/Icons/Exclamation.svg"></a>'
            ibase_missing_flag = True
        else:
            ibase_icon = '<a class = "cust_exc" title="Complete"><img src="/mt/appliedmaterials_uat/Additionalfiles/Green_Tick.svg"></a>'
        Action_str = (
...
def GetCommonParentContract(PerPage, PageInform, A_Keys, A_Values):    
    ...
    imgstr = '<img title="Acquired" src=/mt/APPLIEDMATERIALS_UAT/Additionalfiles/Green_Tick.svg>'
    acquiring_img_str = '<img title="Acquiring" src=/mt/APPLIEDMATERIALS_UAT/Additionalfiles/Cloud_Icon.svg>'
    exclamation = '<img title="Approval Required" src=/mt/APPLIEDMATERIALS_UAT/Additionalfiles/clock_exe.svg>'
    error = '<img title="Error" src=/mt/APPLIEDMATERIALS_UAT/Additionalfiles/exclamation_icon.svg>'
    partially_priced = '<img title="Partially Priced" src=/mt/APPLIEDMATERIALS_UAT/Additionalfiles/Red1_Circle.svg>'
    assembly_missing = '<img title="Assembly Missing" src=/mt/APPLIEDMATERIALS_UAT/Additionalfiles/Orange1_Circle.svg>'
    ...
    for par in parent_obj:
        ...
        if 'BUNDLE' in str(par.SERVICE_ID):            
            ...
            if 'ACQUIRING' in status_list:
                icon = '<img title="Acquiring" src=/mt/APPLIEDMATERIALS_UAT/Additionalfiles/Cloud_Icon.svg>'
                ...
                icon = '<img title="Error" src=/mt/APPLIEDMATERIALS_UAT/Additionalfiles/exclamation_icon.svg>'
                ...
                icon = '<img title="Approval Required" src=/mt/APPLIEDMATERIALS_UAT/Additionalfiles/clock_exe.svg>'
                ...
                icon = '<img title="Acquired" src=/mt/APPLIEDMATERIALS_UAT/Additionalfiles/Green_Tick.svg>'
            ...
            if 'SPARE' not in GetQuoteType.CONTRACT_TYPE:
                ...
                if item_lines_obj:
                    ...
                    if "ACQUIRED" in line_items_status_list and ('ACQUIRING' not in line_items_status_list and 'APPROVAL REQUIRED' not in line_items_status_list and 'ERROR' not in line_items_status_list):
                        ...
                        icon = '<img title="Acquiring" src=/mt/APPLIEDMATERIALS_UAT/Additionalfiles/clock_exe.svg>'
                        status_update=SqlHelper.GetFirst("sp_executesql @statement = N'UPDATE CTCITM SET PRICING_STATUS=''ACQUIRING'' WHERE CONTRACT_RECORD_ID =''"+str(Quote.GetGlobal("contract_record_id"))+"'' AND SERVICE_ID LIKE ''%BASE%'' '")
            else:
    ...
    for key, col_name in enumerate(list(Columns)):
        ...
        if filter_level_data == "select":
            ...
            for string in StringValue_list:
                string = string.strip()
                if string == "ACQUIRED":
                    string_value = string.replace("ACQUIRED","<img title='Acquired' src=/mt/APPLIEDMATERIALS_UAT/Additionalfiles/Green_Tick.svg> ACQUIRED")
                if string == "APPROVAL REQUIRED":
                    string_value = string.replace("APPROVAL REQUIRED","<img title='Approval Required' src=/mt/APPLIEDMATERIALS_UAT/Additionalfiles/clock_exe.svg> APPROVAL REQUIRED")
                if string == "ACQUIRING":                        
                    string_value = string.replace("ACQUIRING","<img title='Acquiring' src=/mt/APPLIEDMATERIALS_UAT/Additionalfiles/Cloud_Icon.svg> ACQUIRING")
                if string == "ERROR":
                    string_value = string.replace("ERROR","<img title='Error' src=/mt/APPLIEDMATERIALS_UAT/Additionalfiles/exclamation_icon.svg> ERROR")
                    status_update=SqlHelper.GetFirst("sp_executesql @statement = N'UPDATE CTCITM SET PRICING_STATUS=''ERROR'' WHERE CONTRACT_RECORD_ID =''"+str(Quote.GetGlobal("contract_record_id"))+"'' AND SERVICE_ID LIKE ''%BUNDLE%'' '")
                if string == "ASSEMBLY MISSING":
                    string_value = string.replace("ASSEMBLY MISSING","<img title='Assembly Missing' src=/mt/APPLIEDMATERIALS_UAT/Additionalfiles/Orange1_Circle.svg> ASSEMBLY MISSING")
                if string == "PARTIALLY PRICED":
                    string_value = string.replace("PARTIALLY PRICED","<img title='Partially Priced' src=/mt/APPLIEDMATERIALS_UAT/Additionalfiles/Red1_Circle.svg> PARTIALLY PRICED")
