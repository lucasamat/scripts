...
try:
	...
	def GetNewRequest(targetUrl, Btoken): 
		...
		newRequest.Headers.Add("Environment-Identifier", 'Q')
	...
	S = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE SAQICO SET MODVRS_DIRTY_FLAG = SAQICA.MODVRS_DIRTY_FLAG  FROM TOOL_VERSION_INBOUND(NOLOCK) A JOIN SAQICA (NOLOCK) SAQICA ON A.EQUIPMENT_ID = SAQICA.EQUIPMENT_ID AND A.ASSEMBLY_ID = SAQICA.ASSEMBLY_ID JOIN SAQICO(NOLOCK) ON SAQICA.QUOTE_ID = SAQICO.QUOTE_ID AND SAQICA.QTEREV_ID = SAQICO.QTEREV_ID AND SAQICA.LINE = SAQICO.LINE AND SAQICA.EQUIPMENT_ID = SAQICO.EQUIPMENT_ID WHERE ISNULL(SAQICA.MODVRS_DIRTY_FLAG,''FALSE'')=''TRUE''  '")
