...
try:
    ...
    S=SqlHelper.GetFirst("sp_executesql @T=N'update MAFBLC_INBOUND set city= replace(city,''&#92;&#39;'',''''''''  )  ' ")
    S=SqlHelper.GetFirst("sp_executesql @T=N'update MAFBLC_INBOUND set city= replace(city,''&#92;&#92;'',''\\''  )  ' ")
    S=SqlHelper.GetFirst("sp_executesql @T=N'update MAFBLC_INBOUND set city= replace(city,''&#92;&#34;'',''\"''  )  ' ")
    
    S=SqlHelper.GetFirst("sp_executesql @T=N'update MAFBLC_INBOUND set FAB_LOCATION_NAME= replace(FAB_LOCATION_NAME,''&#92;&#39;'',''''''''  )  ' ")
    S=SqlHelper.GetFirst("sp_executesql @T=N'update MAFBLC_INBOUND set FAB_LOCATION_NAME= replace(FAB_LOCATION_NAME,''&#92;&#92;'',''\\''  )  ' ")
    S=SqlHelper.GetFirst("sp_executesql @T=N'update MAFBLC_INBOUND set FAB_LOCATION_NAME= replace(FAB_LOCATION_NAME,''&#92;&#34;'',''\"''  )  ' ")
    
    S=SqlHelper.GetFirst("sp_executesql @T=N'update MAFBLC_INBOUND set ADDRESS_1= replace(ADDRESS_1,''&#92;&#39;'',''''''''  )  ' ")
    S=SqlHelper.GetFirst("sp_executesql @T=N'update MAFBLC_INBOUND set ADDRESS_1= replace(ADDRESS_1,''&#92;&#92;'',''\\''  )  ' ")
    S=SqlHelper.GetFirst("sp_executesql @T=N'update MAFBLC_INBOUND set ADDRESS_1= replace(ADDRESS_1,''&#92;&#34;'',''\"''  )  ' ")
    
    S=SqlHelper.GetFirst("sp_executesql @T=N'update MAFBLC_INBOUND set AGS_SEMI_FAB= replace(AGS_SEMI_FAB,''&#92;&#39;'',''''''''  )  ' ")
    S=SqlHelper.GetFirst("sp_executesql @T=N'update MAFBLC_INBOUND set AGS_SEMI_FAB= replace(AGS_SEMI_FAB,''&#92;&#92;'',''\\''  )  ' ")
    S=SqlHelper.GetFirst("sp_executesql @T=N'update MAFBLC_INBOUND set AGS_SEMI_FAB= replace(AGS_SEMI_FAB,''&#92;&#34;'',''\"''  )  ' ")
    ...
    + str(User.Id)
    + "'',MAFBLC.CpqTableEntryDateModified = GetDate(),MAFBLC.FABCATEGORY_ID = MAEQCT.EQUIPMENTCATEGORY_ID,MAFBLC.FABCATEGORY_NAME = MAEQCT.EQUIPMENTCATEGORY_DESCRIPTION,MAFBLC.FABCATEGORY_RECORD_ID = MAEQCT.EQUIPMENT_CATEGORY_RECORD_ID,MAFBLC.MNT_PLANT_ID = MAPLNT.PLANT_ID,MAFBLC.MNT_PLANT_NAME = MAPLNT.PLANT_NAME,MAFBLC.MNT_PLANT_RECORD_ID = MAPLNT.PLANT_RECORD_ID,FAB_BLUEBOOK= MAFBLC_INBOUND.BLUEBOOK,FAB_LOCATION_NAME = MAFBLC_INBOUND.FAB_LOCATION_NAME,ACCOUNT_ID = SAACNT.ACCOUNT_ID,ACCOUNT_NAME = SAACNT.ACCOUNT_NAME,ACCOUNT_RECORD_ID = SAACNT.ACCOUNT_RECORD_ID,CITY = MAFBLC_INBOUND.CITY,ADDRESS_1 = MAFBLC_INBOUND.ADDRESS_1,COUNTRY = MAFBLC_INBOUND.COUNTRY,COUNTRY_RECORD_ID = SACTRY.COUNTRY_RECORD_ID,POSTAL_CODE= MAFBLC_INBOUND.POSTAL_CODE,AGS_FAB_NAME = AGS_SEMI_FAB,STATE = SACYST.STATE,STATE_RECORD_ID = SACYST.STATE_RECORD_ID,SALESORG_ID=MAFBLC_INBOUND.SALES_ORG_ID,SALESORG_NAME=SASORG.SALESORG_NAME,SALESORG_RECORD_ID = SASORG.SALES_ORG_RECORD_ID FROM MAFBLC_INBOUND(NOLOCK)  JOIN MAFBLC (NOLOCK) ON MAFBLC_INBOUND.FAB_LOCATION_ID = MAFBLC.FAB_LOCATION_ID JOIN MAPLNT (NOLOCK) ON MAFBLC_INBOUND.MNT_PLANT_ID = MAPLNT.PLANT_ID JOIN MAEQCT (NOLOCK) ON MAFBLC_INBOUND.FABCATEGORY_ID = MAEQCT.EQUIPMENTCATEGORY_ID LEFT JOIN SAACNT SAACNT(NOLOCK) ON MAFBLC_INBOUND.ACCOUNT_ID = SAACNT.ACCOUNT_ID LEFT JOIN SACTRY (NOLOCK) ON MAFBLC_INBOUND.COUNTRY = SACTRY.COUNTRY LEFT JOIN SACYST (NOLOCK) ON MAFBLC_INBOUND.STATE = SACYST.STATE LEFT JOIN SASORG(NOLOCK) ON MAFBLC_INBOUND.SALES_ORG_ID = SASORG.SALESORG_ID WHERE  MAFBLC_INBOUND.TIMESTAMP='"+ str(timestamp_sessionid)+ "'  AND ISNULL(MAFBLC_INBOUND.INTEGRATION_STATUS,'''') ='''' AND ISNULL(PROCESS_STATUS,'''')=''READY FOR UPLOAD'' '"	)

    # MAFBLC Add
    primaryQueryItems = SqlHelper.GetFirst(
        ""
        + str(Parameter.QUERY_CRITERIA_1)
        + "   MAFBLC (ACCOUNT_ID,ACCOUNT_NAME,ACCOUNT_RECORD_ID,ADDRESS_1,ADDRESS_2,CITY,COUNTRY,COUNTRY_RECORD_ID,DISTRIBUTIONCHANNEL_ID,DISTRIBUTIONCHANNEL_RECORD_ID,DIVISION_ID,DIVISION_RECORD_ID,FAB_LOCATION_ID,FAB_LOCATION_NAME, MNT_PLANT_ID,MNT_PLANT_RECORD_ID,POSTAL_CODE,SALESORG_ID,SALESORG_NAME,SALESORG_RECORD_ID,STATE,STATE_RECORD_ID,AGS_FAB_NAME,STATUS,CATEGORY_TYPE,MNT_PLANT_NAME,PHONE,FABCATEGORY_ID,FABCATEGORY_NAME,FABCATEGORY_RECORD_ID,FAB_BLUEBOOK,FAB_LOCATION_RECORD_ID,CPQTABLEENTRYADDEDBY,CPQTABLEENTRYDATEADDED)SELECT SUB_MAFBLC.*, CONVERT(VARCHAR(4000),NEWID()),''"
        + str(User.UserName)
        + "'',GETDATE() FROM (SELECT DISTINCT SAACNT.ACCOUNT_ID,SAACNT.ACCOUNT_NAME,SAACNT.ACCOUNT_RECORD_ID,MAFBLC_INBOUND.ADDRESS_1,MAFBLC_INBOUND.ADDRESS_2,MAFBLC_INBOUND.CITY,MAFBLC_INBOUND.COUNTRY,SACTRY.COUNTRY_RECORD_ID,SADSCH.DISTRIBUTIONCHANNEL_ID,SADSCH.DISTRIBUTION_CHANNEL_RECORD_ID AS DISTRIBUTIONCHANNEL_RECORD_ID,	SADIVN.DIVISION_ID,SADIVN.DIVISION_RECORD_ID,MAFBLC_INBOUND.FAB_LOCATION_ID,MAFBLC_INBOUND.FAB_LOCATION_NAME, MAPLNT.PLANT_ID AS MNT_PLANT_ID,MAPLNT.PLANT_RECORD_ID AS MNT_PLANT_RECORD_ID,MAFBLC_INBOUND.POSTAL_CODE,SASORG.SALESORG_ID,SASORG.SALESORG_NAME,SASORG.SALES_ORG_RECORD_ID AS SALESORG_RECORD_ID,SACYST.STATE,SACYST.STATE_RECORD_ID,MAFBLC_INBOUND.AGS_SEMI_FAB,MAFBLC_INBOUND.STATUS,MAFBLC_INBOUND.CATEGORY_TYPE,MAPLNT.PLANT_NAME AS MNT_PLANT_NAME,MAFBLC_INBOUND.PHONE,MAEQCT.EQUIPMENTCATEGORY_ID AS FABCATEGORY_ID,MAEQCT.EQUIPMENTCATEGORY_DESCRIPTION AS  FABCATEGORY_NAME, MAEQCT.EQUIPMENT_CATEGORY_RECORD_ID AS FABCATEGORY_RECORD_ID,MAFBLC_INBOUND.BLUEBOOK FROM MAFBLC_INBOUND(NOLOCK) LEFT JOIN SAACNT(NOLOCK) ON MAFBLC_INBOUND.ACCOUNT_ID = SAACNT.ACCOUNT_ID LEFT JOIN SACTRY (NOLOCK) ON MAFBLC_INBOUND.COUNTRY = SACTRY.COUNTRY LEFT JOIN SADSCH(NOLOCK) ON MAFBLC_INBOUND.DISTRIBUTIONCHANNEL_ID = SADSCH.DISTRIBUTIONCHANNEL_ID LEFT JOIN SADIVN(NOLOCK) ON MAFBLC_INBOUND.DIVISION_ID = SADIVN.DIVISION_ID LEFT JOIN MAPLNT(NOLOCK) ON MAFBLC_INBOUND.MNT_PLANT_ID = MAPLNT.PLANT_ID JOIN SASORG(NOLOCK) ON MAFBLC_INBOUND.SALES_ORG_ID = SASORG.SALESORG_ID LEFT JOIN MAEQCT(NOLOCK)  ON MAFBLC_INBOUND.FABCATEGORY_ID = MAEQCT.EQUIPMENTCATEGORY_ID LEFT JOIN SACYST (NOLOCK) ON MAFBLC_INBOUND.STATE = SACYST.STATE WHERE  MAFBLC_INBOUND.TIMESTAMP='"+ str(timestamp_sessionid)+ "'  AND ISNULL(MAFBLC_INBOUND.INTEGRATION_STATUS,'''') ='''' AND ISNULL(MAFBLC_INBOUND.PROCESS_STATUS,'''') =''READY FOR UPLOAD'' )SUB_MAFBLC LEFT JOIN MAFBLC(NOLOCK) ON SUB_MAFBLC.FAB_LOCATION_ID = MAFBLC.FAB_LOCATION_ID WHERE MAFBLC.FAB_LOCATION_ID IS NULL '" )
    ...
    if len(Error_list) > 0:
        ...
        msg.Subject = "FAB LOCATION EXCEPTIONS - AMAT CPQ(Q-Tenant)"
        msg.IsBodyHtml = True
        msg.Body = Error_Info
