...
def Dynamic_Status_Bar(quote_item_insert,Text):
    ...
    get_ibase_missing = Sql.GetFirst("SELECT COUNT(CpqTableEntryId) as CNT FROM SAQSCO WHERE QUOTE_RECORD_ID = '{}' AND QTEREV_RECORD_ID = '{}' AND IBASE_ATTSTS = 'INCOMPLETE_'  ".format(Quote.GetGlobal("contract_quote_record_id"),quote_revision_record_id))
    ...
    if str(Text) == 'COMPLETE STAGE' and (str(TabName) == "Quotes" or str(TabName) == "Quote") and current_prod == "Sales" and  get_quality_required.CNT == 0 and get_ibase_missing.CNT==0:
        ...
        if get_workflow_statusquery and get_workflow_statusquery.WORKFLOW_STATUS not in ("PRI-PRICING","APPROVALS","LEGAL SOW","QUOTE-DOCUMNETS","CLEAN BOOKING CHECKLIST","BOOKED"):
            Trace.Write('136----')
    ...
    if get_workflow_status:
        ...
        if get_workflow_status.WORKFLOW_STATUS == "PRICING" and get_workflow_status.REVISION_STATUS =="PRI-PRICING" and str(Text) == 'COMPLETE STAGE' and fpm_flag==0:
            ...
            if 'ACQUIRING' not in acquired_items_status:
                Trace.Write('272--->')
                bill_plan_years = Sql.GetList("SELECT DISTINCT BILLING_YEAR,SERVICE_ID from SAQIBP WHERE QUOTE_RECORD_ID = '"+str(Quote.GetGlobal("contract_quote_record_id"))+"' AND QTEREV_RECORD_ID = '"+str(quote_revision_record_id)+"' ")
                billing_message= []
                if bill_plan_years:
                    for val in bill_plan_years:
                        ...
                        item_bill_type = Sql.GetFirst("SELECT BILLING_TYPE,SUM("+bill_years_gl_curr+") as year_gc from SAQRIT where QUOTE_RECORD_ID= '"+str(Quote.GetGlobal("contract_quote_record_id"))+"' and QTEREV_RECORD_ID ='"+str(quote_revision_record_id)+"' and SERVICE_ID='"+str(service_id)+"' group by BILLING_TYPE")
                        if item_bill_type:
    ...
    if quote_item_insert == 'yes' and Text == "COMPLETE STAGE" and status not in ("CFG_CONFIGURING_STATUS","PRICING","GENERATE SOW","COMPLETESOW","APPROVALS","LEGAL SOW","QUOTE DOCUMENTS","BOOKED","CLEAN BOOKING CHECKLIST","LEGAL SOW ACCEPT")  and  get_quality_required.CNT == 0 and get_ibase_missing.CNT==0 and servicelevel_entitilement_obj.CNT == 0:
        ...
        Log.Info('quote_item_insert--'+str(quote_item_insert))
        ...
        if status not in ("GENERATE SOW","COMPLETESOW","APPROVALS","LEGAL SOW","QUOTE DOCUMENTS","BOOKED","CLEAN BOOKING CHECKLIST","LEGAL SOW ACCEPT"):
            ...
            if quote_line_item_obj :
                ...
                if quote_revision_obj:
                    Log.Info("===> QTPOSTACRM called from CQSTSVADTN")
                    revision_flag = "True"
