...
def constructquoteinformation(Qt_rec_id, Quote, MODE):    
    ...
    for sect in Oppp_SECT:
        ...
        for sefl in Oppp_SEFL:
            ...
            if permission_chk_query and col_name:
...
def constructCBC(Qt_rec_id, Quote, MODE):
    ...
    for sect in dynamic_sect:
        if get_dynamic_div:
            Trace.Write("get_dynamic_div")
            static =''
        elif get_trans_type:
            Trace.Write("get_trans_type")
            static =''
        elif get_poes_type:
            Trace.Write("get_poes_type")
            static =''
        elif "CFG-ON HOLD TKM" in saqrit_status or "CFG-ON HOLD TKM" in saqico_status:
            Trace.Write("saqrit_status")
            static = ''
        else:
...
def savecbc(Qt_rec_id, Quote_rec_id, MODE):
    ...
    if getfpm_quote_type:
        ...
        if get_quote_items:
            Log.Info("===> QTPOSTACRM called from CQPREVWDTL 111")
    ...
    if revision_document_type_object:
        if revision_document_type_object.DOCTYP_ID == "ZTBC" and revision_document_type_object.DOCTYP_ID != "":
            Log.Info("===> QTPOSTACRM called from CQPREVWDTL 222")
