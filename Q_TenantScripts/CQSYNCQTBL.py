...
class SyncQuoteAndCustomTables:
    ...
    def _get_custom_fields_detail(self):		
        return {
            ...
            'SIGFPOpportunityName':self.quote.GetCustomField('SIGFPOpportunityName').Content,
                ...
                if not quote_obj:					
                    ...
                    if custom_fields_detail.get('ContractManagerID'):
                            ...
                            if Employee_obj:
                                ...
                                contract_quote_data.update({"OWNER_ID":custom_fields_detail.get('ContractManagerID'), 
                    ...
                    if payload_json_obj:
                        ...
                        if payload_json.get("SAQTIP"):
                            ...
                            for employees in payload_json.get("SAQTIP"):
                                ...
                                involved_party_update_info = {
                                    "QUOTE_INVOLVED_PARTY_RECORD_ID": str(Guid.NewGuid()).upper(),
                                    "QUOTE_ID": contract_quote_data.get("QUOTE_ID"),
                                    "QUOTE_NAME": contract_quote_data.get("MASTER_TABLE_QUOTE_RECORD_ID"),
                                    "SALESORG_ID": salesorg_data.get("SALESORG_ID"),
                                    "SALESORG_NAME": salesorg_data.get("SALESORG_NAME"),
                        ...
                        #             "SALESORG_ID": salesorg_data.get("SALESORG_ID"),
                        #             "SALESORG_NAME": salesorg_data.get("SALESORG_NAME"),
                        #             "CPQ_PARTNER_FUNCTION": employees.get("CPQ_PARTNER_FUNCTION"),
                        #             "PARTY_ID": employees.get("PARTY_ID"),
                        #             "PARTY_NAME": employees.get("PARTY_NAME"),
                        #             "ADDRESS": employees.get("ADDRESS"),
                        ...
                        #             "PRIMARY": employees.get("PRIMARY")
                        #         }
                        #         quote_involved_party_contact_table_info.AddRow(involved_party_update_info)
                        #         Sql.Upsert(quote_involved_party_contact_table_info)
                                    ...
                                    for fab_location_id, value in equipment_fab_data.items():
                                        ...
                                        equipment_temp_insert = Sql.RunQuery("""
                                                                ...
                                                                SELECT A.*, CONVERT(VARCHAR(4000),NEWID()) as QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID, '{UserName}' as CPQTABLEENTRYADDEDBY, GETDATE() as CPQTABLEENTRYDATEADDED, {UserId} as CpqTableEntryModifiedBy, GETDATE() as CpqTableEntryDateModified FROM (
                                                                    SELECT '{quote_revision_id}' AS QTEREV_RECORD_ID,'{quote_rev_id}' AS QTEREV_ID,EQUIPMENT_DESCRIPTION, EQUIPMENT_ID, EQUIPMENT_RECORD_ID, '{FabLocationId}' as FABLOCATION_ID, FABLOCATION_NAME, FABLOCATION_RECORD_ID, MNT_PLANT_ID,'' as MNT_PLANT_NAME, MNT_PLANT_RECORD_ID, PLATFORM, '{QuoteId}' as QUOTE_ID, '{QuoteName}' as QUOTE_NAME, '{QuoteRecordId}' as QUOTE_RECORD_ID, SALESORG_ID, SALESORG_NAME, SALESORG_RECORD_ID, SERIAL_NO, SUBSTRATE_SIZE, TECHNOLOGY, EQUIPMENTCATEGORY_ID, EQUIPMENTCATEGORY_RECORD_ID, EQUIPMENTCATEGORY_DESCRIPTION, EQUIPMENT_STATUS, PBG,KPU, WARRANTY_END_DATE, WARRANTY_START_DATE, CUSTOMER_TOOL_ID,  GREENBOOK, GREENBOOK_RECORD_ID,'True' as TEMP_TOOL,DEVICE_NODE,DEVICE_TYPE,PROCESS_TYPE,WAFER_SIZE_GROUP,GENERATION,END_PRODUCT_TECHNOLOGY,SUBSTRATE_SIZE_GROUP FROM MAEQUP (NOLOCK)
                                                                    JOIN (SELECT NAME FROM SPLITSTRING('{EquipmentIds}'))B ON MAEQUP.EQUIPMENT_ID = NAME 
                                                                    ) A
                                                                """.format(UserId=User.Id,UserName=User.Name,QuoteId=contract_quote_obj.QUOTE_ID, QuoteName=contract_quote_obj.QUOTE_NAME,QuoteRecordId=Quote.GetGlobal("contract_quote_record_id"), FabLocationId=fab_location_id, EquipmentIds=",".join(value),quote_revision_id=Quote.GetGlobal("quote_revision_record_id"),quote_rev_id=quote_rev_id))
                                ...
                                for fab_location_id, value in equipment_data.items():			
                                    ...
                                    equipment_insert = Sql.RunQuery("""
                                                                ...
                                                                SELECT A.*, CONVERT(VARCHAR(4000),NEWID()) as QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID, '{UserName}' as CPQTABLEENTRYADDEDBY, GETDATE() as CPQTABLEENTRYDATEADDED, {UserId} as CpqTableEntryModifiedBy, GETDATE() as CpqTableEntryDateModified FROM (
                                                                    ...
                                                                    FROM MAEQUP (NOLOCK)
                                                                    JOIN (SELECT NAME FROM SPLITSTRING('{EquipmentIds}'))B ON MAEQUP.EQUIPMENT_ID = NAME WHERE FABLOCATION_ID = '{FabLocationId}'
                                                                    ) A
                                                                """.format(UserId=User.Id,UserName=User.Name,QuoteId=quote_id, QuoteName=contract_quote_obj.QUOTE_NAME,QuoteRecordId=quote_record_id, FabLocationId=fab_location_id, EquipmentIds=",".join(value),quote_revision_id=quote_revision_id,quote_rev_id=quote_rev_id,salesId=get_sales_org_data.SALESORG_ID,salesname=get_sales_org_data.SALESORG_NAME,salesrecordid=get_sales_org_data.SALESORG_RECORD_ID))
