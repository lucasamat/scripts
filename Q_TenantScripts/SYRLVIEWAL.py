# =========================================================================================================================================
#   __script_name : SYRLVIEWAL.PY
#   __script_description :  THIS SCRIPT IS USED TO GET THE VALUES FOR THE POPUP FROM LIST GRID DURING VIEW FOR CONTAINERS WITHOUT A TAB
...
def POPUPLISTVALUE(VALUE, TABLEID, OPERATION, RETURN, IDVALUE, LOOKUPOBJ):
    ...
    if OPERATION == "EDIT":
        ...
        func1 = (
            ...
            + "|"
            + str(TABLEID)
            + '" class="btnconfig" onclick="cont_relatedlist_openview(this);">CANCEL</button>'
        )
        func2 = "cont_relatedlist_SAVE(this)"
        func3 = ""
        func4 = ""
        btn3 = ""
    else:
        btn1 = "EDIT"
        func1 = '<button type="button" class="btnconfig" data-dismiss="modal">BACK TO LIST</button>'
    ...
    if RETURN == "1":
        ...
        Trace.Write(attrval_obj.API_NAME)
        ...
        Trace.Write("The value is here----------> " + str(dict(result)))
        new_value_dict = {API_Names["API_NAME"]: API_Names["FORMULA_RESULT"] for API_Names in result}
    Trace.Write("wwwwwwwwwwwwwww-----> " + str(dict(new_value_dict)))
    ...
    if popup_lable_obj is not None:
        ...
        Trace.Write("candelete------>" + str(candelete))
        if canedit == 0:
            canedit = "FALSE"
            func4 = ""
        elif canedit == 1:
    ...
    if Question_obj is not None:
        Trace.Write(str(record_field) + "query" + str(value))
        ...
        sec_str += func1
        Trace.Write(str(Lock_val) + "caneditcanedit" + str(canedit))
        if str(canedit).upper() == "TRUE" and str(Lock_val).upper() != "TRUE":
            sec_str += (
                '<button type="button" id="'
                + str(VALUE)
                + "|"
        ...
        sec_str += '<table style="width:100%; margin-top: 8px;">'
            ...
            for val in Sqq_obj:
                ...
                if nam in lookup_val:
                    for key, value in lookup_list.items():
                        Trace.Write("dddddddddddd " + str(value))
                        if key == nam:
                            ids = value.strip()
                Trace.Write("idsidsidsidsidsids " + str(ids))
                ...
                if header_obj != "":
                    try:
                        ...
                        Trace.Write("try" + str(sec))
                    except:
                        Trace.Write("except" + str(res))
                if Custom_obj != "":
                    try:
                        res = eval(str("Custom_obj." + str(nam)))
                        Trace.Write("resresresresresresres" + str(res))
                    except:
                        Trace.Write("except" + str(res))


                if str(disable) == "disabled":
                    Trace.Write("Locked Read Only")
                    right_icon = '<i class="fa fa-lock" aria-hidden="true"></i>'
                else:
                    Trace.Write("Pencil Editable True")
                    right_icon = '<i class="fa fa-pencil" aria-hidden="true"></i>'

                sec_str += (
                    '<tr class="iconhvr" style="border-bottom: 1px solid #dcdcdc; '
                    + str(add_style)
                    + '"><td style="width: 350px;"><label style="float: left;position: relative; padding-left: 15px;">'
                    + str(lab)
                    + '</label></td><td style="width: 40px;"><a href="#" data-placement="top" data-toggle="popover" data-content="'
                    + str(lab)
                    + '" style="color:#ccc; text-align:center; width: 10%;"><i style="float:left;" class="fa fa-info-circle"></i>'
                )
                if str(val.REQUIRED).upper() == "TRUE" or val.REQUIRED == "1":
                    sec_str += ""
                    # sec_str+='<span class="req-field" style="margin-left: 3px; float:left; margin-top: -7px;">*</span>'
                sec_str += "</a></td>"
                if data_type == "AUTO NUMBER":
                    Trace.Write("TABLEID_TABLEID" + str(TABLEID))
                    RECORD_ID = CPQID.KeyCPQId.GetCPQId(str(TABLEID), str(res))
                    Trace.Write("RECORD_ID_RECORD_ID_RECORD_ID" + str(RECORD_ID))
                    sec_str += (
                        '<td><input id="'
                        + str(nam)
                        + '" type="text" value="'
                        + str(RECORD_ID)
                        + '" class="form-control" style="width: 184px;" disabled></td>'
                    )
                    ...
                    if sec == "" and nam in lookup_val and readonly != "readonly":
                        sec_str += (
                            ...
                            + '" type="text" value="'
                            + str(res)
                            + '" class="form-control" style="float: left; width: 184px;     background-color: lightyellow !important; border: 1px solid #dcdcdc;" readonly>'
                        )
                        sec_str += (
                            '<input class="popup" id="'
                            + str(ids)
                            + '" onclick="cont_lookup_popup_relatedlist(this)" style="float:left;" type="image"  src="../mt/default/images/customer_lookup.gif"></td>'
                        )
                    else:
                        sec_str += (
                            '<td><input id="'
                            ...
                            + '" type="text" value="'
                            + str(res)
                            + '" class="form-control" style="float: left; width: 184px;" disabled></td>'
                        )
                elif data_type == "CHECKBOX":
                    if str(res) == "True" or str(res) == "1":
                        Trace.Write("resresresresresresres ififif")
                        sec_str += (
                            '<td><input id="'
                            + str(nam)
                            + '" type="'
                        ...
                        Trace.Write("resresresresresresres elseelseesle")
                        sec_str += (
                            '<td><input id="'
                            + str(nam)
                            + '" type="'
                ...
                elif data_type == "PICKLIST" and popup_lable_obj is not None:
                    if popup_lable_obj.NAME == "AVAILABLE FULFILLMENT COUNTRIES FOR MATERIAL":
                        ...
                        if str(nam) == "COUNTRIES":
                            ...
                            sec_str += (
                                '<td style="position: relative; color: #555;" onclick="showCheckboxes()"><select id="select_id" value="'
                                + str(res)
                                + '" type="text" class="form-control" style="width: 184px; float: left; font-size: 13px;" '
                                + disable
                                + ' > "add" </select><input id="'
                                + str(nam)
                            ...
                            sec_str += '<div id="checkboxes" style="display: none; position: absolute; left: -49px; overflow: auto; height: 180px; top: 28px; width: auto; border: 1px solid; background-color: #ffff;">'
                        ...
                        sec_str += (
                            ...
                            + '" value="'
                            + str(res)
                            + '" type="text" class="form-control pop_up_brd_rad" style="width: 184px; float: left; font-size: 13px;" '
                            + disable
                            + " >"
                        )
                        Sql_Quality_Tier = Sql.GetFirst(
                    ...
                    sec_str += (
                        ...
                        + '" value="'
                        + str(res)
                        + '" type="text" class="form-control" style="width: 155px; float:left;" '
                        + disable
                        + ' ><span style="padding: 4px 6px; width: 0;" class="input-group-addon" onclick="'
                        + str(datepicker)
                    ...
                    sec_str += (
                        ...
                        + '" type="number" value="'
                        + str(res)
                        + '" class="form-control" style="width: 184px;" '
                        + disable
                        + "></td>"
                    )
                else:
                ...
                sec_str += (
                    '<td style="float: right; border-bottom: 0px;"><div class="col-md-12 editiconright"><a href="#" id="'
                    + str(VALUE)
                    + "|"
                    + str(TABLEID).strip()
                    + '" onclick="'
        ...
        sec_str += '<div style="text-align: center;">No matching records found </div><div class="modal-footer"><button type="button" class="btnconfig" style="float:right;" data-dismiss="modal">Close</button></div>'
