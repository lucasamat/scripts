...
def Dynamic_Status_Bar(quote_item_insert,Text):
	...
	if (str(TabName) == "Quotes" or str(TabName) == "Quote") and current_prod == "Sales":
		...
		Sql.RunQuery("UPDATE SAQICO SET STATUS='' WHERE QUOTE_RECORD_ID = '{}' AND QTEREV_RECORD_ID = '{}'".format(Quote.GetGlobal("contract_quote_record_id"),quote_revision_record_id))
