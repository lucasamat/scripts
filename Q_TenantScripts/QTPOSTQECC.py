...
try:
	...
	for crmifno in input_data:	
		...
		QuoteInvolvedPartiesquery = SqlHelper.GetFirst("select replace(replace(STUFF((SELECT  ' '+final_xml FROM (SELECT DISTINCT  '<QUOTE_INVOLVED_PARTY>'+'<QUOTE_ID>"+str(CRMQT.c4c_quote_id)+"</QUOTE_ID><SOLD_TO>'+ ISNULL((SELECT REPLACE(ACCOUNT_ID,'STP-','') FROM SAQTMT(NOLOCK) WHERE QUOTE_ID = '"+str(QUOTE_ID)+"' ),'') + '</SOLD_TO><BILL_TO>'+ISNULL((SELECT TOP 1 PARTY_ID FROM SAQTIP(NOLOCK) WHERE QUOTE_ID = '"+str(QUOTE_ID)+"' AND CPQ_PARTNER_FUNCTION = 'BILL TO' AND QTEREV_ID = '"+str(REVISION_ID)+"' ),'')+'</BILL_TO><SHIP_TO>'+ISNULL((SELECT TOP 1 PARTY_ID FROM SAQTIP(NOLOCK) WHERE QUOTE_ID = '"+str(QUOTE_ID)+"' AND CPQ_PARTNER_FUNCTION = 'SHIP TO' AND QTEREV_ID = '"+str(REVISION_ID)+"' AND  ISNULL([PRIMARY],'FALSE')='TRUE' ),'')+'</SHIP_TO><PAYER>'+ISNULL((SELECT TOP 1 PARTY_ID FROM SAQTIP(NOLOCK) WHERE QUOTE_ID = '"+str(QUOTE_ID)+"' AND QTEREV_ID = '"+str(REVISION_ID)+"' AND CPQ_PARTNER_FUNCTION = 'PAYER' ),'')+'</PAYER><EMPLOYEERESP>'+ISNULL((SELECT TOP 1 CONVERT(VARCHAR(10),EMPLOYEE_ID) FROM SAQDLT(NOLOCK)A JOIN SAEMPL (NOLOCK) ON MEMBER_ID = EMPLOYEE_ID WHERE QUOTE_ID = '"+str(QUOTE_ID)+"' AND PARTNERFUNCTION_ID = 'ER' AND QTEREV_ID = '"+str(REVISION_ID)+"' AND  ISNULL([PRIMARY],'FALSE')='TRUE'  ),'')+'</EMPLOYEERESP><CONT_MNGR>'+ISNULL((SELECT TOP 1 CONVERT(VARCHAR(10),EMPLOYEE_ID) FROM SAQDLT(NOLOCK)A JOIN SAEMPL (NOLOCK) ON MEMBER_ID = EMPLOYEE_ID WHERE QUOTE_ID = '"+str(QUOTE_ID)+"' AND C4C_PARTNERFUNCTION_ID = 'CONTRACT MANAGER' AND QTEREV_ID = '"+str(REVISION_ID)+"'  AND  ISNULL([PRIMARY],'FALSE')='TRUE'  ),'')+'</CONT_MNGR><SALES_PERSON>'+ISNULL((SELECT TOP 1 CONVERT(VARCHAR(10),EMPLOYEE_ID) FROM SAQDLT(NOLOCK)A JOIN SAEMPL (NOLOCK) ON MEMBER_ID = EMPLOYEE_ID WHERE QUOTE_ID = '"+str(QUOTE_ID)+"' AND C4C_PARTNERFUNCTION_ID = 'Sales Employee' AND QTEREV_ID = '"+str(REVISION_ID)+"' AND  ISNULL([PRIMARY],'FALSE')='TRUE'  ),'')+'</SALES_PERSON>' AS final_xml  FROM SAQTIP(NOLOCK) WHERE QUOTE_ID = '"+str(QUOTE_ID)+"' AND QTEREV_ID = '"+str(REVISION_ID)+"')A FOR XML PATH ('')  ), 1, 1, ''),'&lt;','<'),'&gt;','>')AS RESULT")
		
		QuoteInvolvedPartiesquery1 = SqlHelper.GetFirst("select replace(replace(STUFF((SELECT  ' '+final_xml FROM (SELECT DISTINCT  '<SHIP_TO>'+ISNULL(PARTY_ID,'')+'</SHIP_TO>' AS final_xml  FROM SAQTIP(NOLOCK) WHERE QUOTE_ID = '"+str(QUOTE_ID)+"' AND QTEREV_ID = '"+str(REVISION_ID)+"' AND ISNULL([PRIMARY],'FALSE')='FALSE' AND CPQ_PARTNER_FUNCTION = 'SHIP TO'  )A FOR XML PATH ('')  ), 1, 1, ''),'&lt;','<'),'&gt;','>')AS RESULT")
		...
		while Check_flag1 == 1:
			...
			if str(table1) != "None":
				...
				if str(QTYPE.quote_type) == "ZWK1":
					...
					Quoteitemquery = SqlHelper.GetFirst("SELECT REPLACE((select replace(replace(STUFF((SELECT  ' '+final_xml FROM ( SELECT '<QUOTE_ITEM>'+'<LINE>'+ISNULL(convert(varchar(100),SAQITM.LINE),'')+'</LINE>'+'<SERVICE_ID>'+SAQITM.SERVICE_ID+'</SERVICE_ID>'+'<NET_VALUE>'+ISNULL(convert(varchar,SAQITM.EXTENDED_PRICE),'')+'</NET_VALUE>'+'<PLANT_ID>'+ISNULL(convert(varchar,SAQITM.PLANT_ID),'')+'</PLANT_ID>'+'<CONTRACT_VALID_FROM>'+REPLACE(ISNULL(convert(varchar(11),SAQITM.LINE_ITEM_FROM_DATE,121),''),'-','')+'</CONTRACT_VALID_FROM>'+'<CONTRACT_VALID_TO>'+REPLACE(ISNULL(convert(varchar(11),SAQITM.LINE_ITEM_TO_DATE,121),''),'-','')+'</CONTRACT_VALID_TO>'+ ISNULL(ET_XML,'<QUOTE_ITEM_ENTITLEMENT></QUOTE_ITEM_ENTITLEMENT>')+ISNULL(REPLACE(REPLACE(STUFF((SELECT ' '+ JSON FROM (SELECT '<QUOTE_ITEM_PARTS>'+'<LINE>'+ISNULL(convert(varchar(100),SUB_SAQITM.LINE),'')+'</LINE>'+'<CUSTOMER_PART_NUMBER>'+ISNULL(CUSTOMER_PART_NUMBER,'')+'</CUSTOMER_PART_NUMBER>'+'<PART_NUMBER>'+ISNULL(PART_NUMBER,'')+'</PART_NUMBER>'+'<SCHEDULED_MODE>'+ISNULL(SCHEDULE_MODE,'')+'</SCHEDULED_MODE>'+'<DELIVERY_MODE>'+ISNULL(DELIVERY_MODE,'')+'</DELIVERY_MODE>' +'<CUSTOMER_ANNUAL_QUANTITY>'+ISNULL(convert(varchar,ANNUAL_QUANTITY),'')+'</CUSTOMER_ANNUAL_QUANTITY>'+'<UNIT_PRICE>'+ISNULL(convert(varchar,UNIT_PRICE),'')+'</UNIT_PRICE>'+'<CORE_CREDIT_PRICE>'+ISNULL(convert(varchar,CORE_CREDIT_PRICE),'')+'</CORE_CREDIT_PRICE>'+'<EXCHANGE_ELIGIBLE>'+ISNULL(convert(varchar,case when EXCHANGE_ELIGIBLE='true' then 1 else 2 end),'2')+'</EXCHANGE_ELIGIBLE>'+'<CUSTOMER_PARTICIPATE>'+ISNULL(convert(varchar,CUSTOMER_PARTICIPATE),'')+'</CUSTOMER_PARTICIPATE>'+'<CUSTOMER_ACCEPT_PART>'+ISNULL(convert(varchar,CUSTOMER_ACCEPT_PART),'')+'</CUSTOMER_ACCEPT_PART>'+'<RETURN_TYPE>'+ISNULL(convert(varchar,RETURN_TYPE),'')+'</RETURN_TYPE><ODCC_FLAG>'+ISNULL(convert(varchar,ODCC_FLAG),'')+'</ODCC_FLAG>'+'</QUOTE_ITEM_PARTS>' AS JSON FROM (SELECT ISNULL(convert(varchar(100),SAQITM.LINE),'') AS LINE,PART_NUMBER,ANNUAL_QUANTITY,CASE WHEN DELIVERY_MODE='ONSITE' THEN '1' WHEN DELIVERY_MODE='OFFSITE' THEN '2' ELSE DELIVERY_MODE END AS DELIVERY_MODE, CASE WHEN SCHEDULE_MODE='SCHEDULED' THEN '1' WHEN SCHEDULE_MODE='UNSCHEDULED' THEN '2' WHEN SCHEDULE_MODE='TSL SHARED' THEN '3' WHEN SCHEDULE_MODE='TSL NON-SHARED' THEN '4' WHEN SCHEDULE_MODE='LOW QUANTITY ONSITE' THEN '5' WHEN SCHEDULE_MODE='LOW QTY ONSITE' THEN '5' WHEN SCHEDULE_MODE='ON REQUEST' THEN '6' ELSE SCHEDULE_MODE END AS SCHEDULE_MODE ,EXCHANGE_ELIGIBLE,CASE WHEN ISNULL(CUSTOMER_PARTICIPATE,'FALSE')='true' THEN 'Y' ELSE 'N' END AS CUSTOMER_PARTICIPATE,CASE WHEN ISNULL(CUSTOMER_ACCEPT_PART,'FALSE')='true' THEN 'Y' ELSE 'N' END AS CUSTOMER_ACCEPT_PART ,SAQIFP.UNIT_PRICE_INGL_CURR AS UNIT_PRICE,CORE_CREDIT_PRICE,CUSTOMER_PART_NUMBER,ODCC_FLAG,CASE WHEN ISNULL(RETURN_TYPE,'')='MANDATORY' THEN '1' WHEN ISNULL(RETURN_TYPE,'')='OPTIONAL' THEN '2' ELSE RETURN_TYPE END AS RETURN_TYPE FROM SAQRIT(NOLOCK) JOIN SAQIFP (NOLOCK) ON SAQRIT.QUOTE_ID = SAQIFP.QUOTE_ID AND SAQRIT.LINE = SAQIFP.LINE AND SAQRIT.QTEREV_ID = SAQIFP.QTEREV_ID  WHERE SAQRIT.QUOTE_ID = '"+str(QUOTE_ID)+"' AND SAQRIT.QTEREV_ID = '"+str(REVISION_ID)+"' AND SAQITM.QUOTE_ID  = SAQRIT.QUOTE_ID AND SAQITM.LINE = SAQRIT.LINE )SUB_SAQITM )A FOR XML PATH ('')  ), 1, 1,'' ),'&lt;','<'),'&gt;','>'),'<QUOTE_ITEM_PARTS></QUOTE_ITEM_PARTS>')+'</QUOTE_ITEM>' AS final_xml  FROM "+str(SAQITM)+" SAQITM(NOLOCK) WHERE SAQITM.QUOTE_ID = '"+str(QUOTE_ID)+"' )A FOR XML PATH ('')  ), 1, 1, ''),'&lt;','<'),'&gt;','>')AS RESULT),'','' ) AS A ")
	...
	primaryQueryItems = SqlHelper.GetFirst( ""+ str(Parameter.QUERY_CRITERIA_1)+ " SYINPL (INTEGRATION_PAYLOAD,INTEGRATION_KEY,CpqTableEntryDateModified,INTEGRATION_NAME)  select ''"+str(data)+ "'',''"+ str(QUOTE_ID)+ "'',getdate(),''CPQ to ECC Quote Replication'' ' ")
	...
	webclient.Headers.Add("Environment-Identifier", "Q")

	crm_response = webclient.UploadString(str(LOGIN_CRE.URL),Final_xml)	
	Log.Info("789 ecc_response --->"+str(crm_response))	
