...
try:
	...
	for crmifno in input_data:	
		...
		sessionid2 = SqlHelper.GetFirst("SELECT NEWID() AS A")
		timestamp_sessionid2 = "'" + str(sessionid2.A) + "'"
		...
		SAQSCO_SEL = SqlHelper.GetFirst("sp_executesql @T=N'select DISTINCT A.QUOTE_ID,D.EQUIPMENT_ID,A.SERVICE_ID,B.SALESORG_ID,C.REGION,A.QTEREV_ID,E.CONTRACT_VALID_FROM,E.CONTRACT_VALID_TO,C.ACCOUNT_NAME,(SELECT DISTINCT FABLOCATION_NAME FROM MAEQUP(NOLOCK) WHERE MAEQUP.EQUIPMENT_ID = A.EQUIPMENT_ID) AS FAB_NAME,(SELECT DISTINCT VALDRV_WAFERNODE FROM MAEQUP(NOLOCK) WHERE MAEQUP.EQUIPMENT_ID = A.EQUIPMENT_ID) AS TECH_NODE_RANGE,D.LINE,A.CMLAB_ENT,A.CNSMBL_ENT, A.CNTCVG_ENT,A.NCNSMB_ENT,A.PMEVNT_ENT,A.PMLAB_ENT,A.PRMKPI_ENT,A.WETCLN_ENT,QTETYP AS QUOTE_TYPE,E.PM_ID,E.GOT_CODE,E.GREENBOOK,E.PROCESS_TYPE,E.DEVICE_NODE,E.MNTEVT_LEVEL,E.KIT_ID,CNTYER AS CONTRACT_YEAR,A.SWPKTA INTO "+str(SAQSCO)+" from SAQICO(NOLOCK) A JOIN SAQTRV B(NOLOCK) ON A.QUOTE_ID = B.QUOTE_ID AND A.QTEREV_ID = B.QTEREV_ID JOIN SAQTMT C(NOLOCK) ON B.QUOTE_ID = C.QUOTE_ID AND B.QTEREV_ID = C.QTEREV_ID JOIN SAQRIO D(NOLOCK) ON A.QUOTE_ID = D.QUOTE_ID AND A.QTEREV_ID = D.QTEREV_ID AND A.LINE = D.LINE JOIN SAQRIT(NOLOCK) E ON A.QUOTE_ID = E.QUOTE_ID AND A.QTEREV_ID = E.QTEREV_ID AND A.LINE = E.LINE WHERE A.QUOTE_ID = ''"+str(Qt_id)+"'' AND A.QTEREV_ID=''"+str(REVISION_ID) +"'' AND A.SERVICE_ID IN (SELECT DISTINCT SERVICE_ID FROM PRSPRV(NOLOCK) WHERE ISNULL(SSCM_COST,''FALSE'')=''TRUE'' ) AND ISNULL(A.STATUS,'''')=''''  ' ")
		
		SAQSCA_SEL = SqlHelper.GetFirst("sp_executesql @T=N'select DISTINCT A.QUOTE_ID,A.EQUIPMENT_ID,A.SERVICE_ID,A.ASSEMBLY_ID,CONVERT(VARCHAR(100),NULL) AS COVERAGE,CONVERT(VARCHAR(100),NULL) AS WETCLEAN,CONVERT(VARCHAR(100),NULL) AS PERFGUARANTEE,CONVERT(VARCHAR(100),NULL) AS PMLABOR,CONVERT(VARCHAR(100),NULL) AS CMLABOR,CONVERT(VARCHAR(100),NULL) AS PM_EVENTS,CONVERT(VARCHAR(100),NULL) AS CONSUMABLE,CONVERT(VARCHAR(100),NULL) AS NON_CONSUMABLE,A.QTEREV_ID,A.LINE,CONVERT(VARCHAR(100),NULL) AS SWAP_KIT INTO "+str(SAQSCA)+" from SAQICA(NOLOCK) A JOIN "+str(SAQSCO)+" (NOLOCK)B ON A.QUOTE_ID = B.QUOTE_ID AND A.QTEREV_ID = B.QTEREV_ID AND A.LINE= B.LINE AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND CNTYEAR =CONTRACT_YEAR  WHERE A.QUOTE_ID = ''"+str(Qt_id)+"'' AND A.QTEREV_ID=''"+str(REVISION_ID) +"'' AND A.SERVICE_ID IN (SELECT DISTINCT SERVICE_ID FROM PRSPRV(NOLOCK) WHERE ISNULL(SSCM_COST,''FALSE'')=''TRUE'' )  ' ")
		
		SAQGPA_SEL = SqlHelper.GetFirst("sp_executesql @T=N'select DISTINCT QUOTE_ID,EQUIPMENT_ID,SERVICE_ID,ASSEMBLY_ID,PM_FREQUENCY, PM_ID,GOT_CODE,GREENBOOK,PROCESS_TYPE,DEVICE_NODE,CONVERT(VARCHAR(100),NULL) AS COVERAGE,CONVERT(VARCHAR(100),NULL) AS WETCLEAN,CONVERT(VARCHAR(100),NULL) AS PERFGUARANTEE,CONVERT(VARCHAR(100),NULL) AS PMLABOR,CONVERT(VARCHAR(100),NULL) AS CMLABOR,CONVERT(VARCHAR(100),NULL) AS PM_EVENTS,CONVERT(VARCHAR(100),NULL) AS CONSUMABLE,CONVERT(VARCHAR(100),NULL) AS NON_CONSUMABLE,QTEREV_ID,KIT_ID,CONVERT(VARCHAR(100),NULL) AS SWAP_KIT INTO "+str(SAQGPA)+" from SAQGPA(NOLOCK) WHERE QUOTE_ID = ''"+str(Qt_id)+"'' AND QTEREV_ID=''"+str(REVISION_ID) +"'' AND SERVICE_ID IN (SELECT DISTINCT SERVICE_ID FROM PRSPRV(NOLOCK) WHERE ISNULL(SSCM_COST,''FALSE'')=''TRUE''  ) AND ISNULL(SAQGPA.INCLUDED,''FALSE'') = ''TRUE''  ' ")
		
		SAQGPM_SEL = SqlHelper.GetFirst("sp_executesql @T=N'select DISTINCT QUOTE_ID,SERVICE_ID, PM_ID,GOT_CODE,GREENBOOK,PROCESS_TYPE,DEVICE_NODE,QTEREV_ID,MNTEVT_LEVEL,KIT_ID INTO "+str(SAQGPM)+" from SAQGPM(NOLOCK) WHERE QUOTE_ID = ''"+str(Qt_id)+"'' AND QTEREV_ID=''"+str(REVISION_ID) +"'' AND SERVICE_ID IN (SELECT DISTINCT SERVICE_ID FROM PRSPRV(NOLOCK) WHERE ISNULL(SSCM_COST,''FALSE'')=''TRUE''  )' ")
		...
		#Swap Kit
		S = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET SWAP_KIT=SWPKTA FROM  "+str(SAQSCA)+" A(NOLOCK) JOIN "+str(SAQSCO)+" B(NOLOCK) ON A.QUOTE_ID = B.QUOTE_ID AND A.QTEREV_ID = B.QTEREV_ID AND A.LINE = B.LINE AND A.EQUIPMENT_ID = B.EQUIPMENT_ID '")
		
		S = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET SWAP_KIT=SWPKTA FROM  "+str(SAQGPA)+" A(NOLOCK) JOIN "+str(SAQSCO)+" B(NOLOCK) ON A.QUOTE_ID = B.QUOTE_ID AND A.QTEREV_ID = B.QTEREV_ID AND A.GOT_CODE = B.GOT_CODE AND ISNULL(A.PROCESS_TYPE,'''') = ISNULL(B.PROCESS_TYPE,'''') AND ISNULL(A.DEVICE_NODE,'''') = ISNULL(B.DEVICE_NODE,'''')  '")
		
		SAQSAP_SEL = SqlHelper.GetFirst("sp_executesql @T=N'select QUOTE_ID,EQUIPMENT_ID,SERVICE_ID,ASSEMBLY_ID,PM_FREQUENCY, PM_ID INTO "+str(SAQSAP)+" from SAQSAP(NOLOCK) WHERE QUOTE_ID = ''"+str(Qt_id)+"'' AND QTEREV_ID=''"+str(REVISION_ID) +"'' AND SERVICE_ID IN (SELECT DISTINCT SERVICE_ID FROM PRSPRV(NOLOCK) WHERE ISNULL(SSCM_COST,''FALSE'')=''TRUE''  )' ")
		...
		PMSA_EVENT = SqlHelper.GetFirst(
			"SELECT replace ('{\"QTQICA\": ['+STUFF((SELECT ','+ JSON FROM (SELECT DISTINCT '{\"SESSION_ID\" : \"'+SESSION_ID+'\",\"QUOTE_ID\" : \"'+QUOTE_ID+'\",\"EQUIPMENT_ID\" : \"'+EQUIPMENT_ID+'\",\"CONTRACT_VALID_FROM\" : \"'+CONTRACT_VALID_FROM+'\",\"CONTRACT_VALID_TO\" : \"'+CONTRACT_VALID_TO+'\",\"SERVICE_ID\" : \"'+SERVICE_ID+'\",\"SALESORG_ID\" : \"'+SALESORG_ID+'\",\"REGION\" : \"'+REGION+'\",\"ASSEMBLY_ID\" : \"'+ASSEMBLY_ID+'\",\"LABOR_COVERAGE\" : \"'+LABOR_COVERAGE+'\",\"PREVENTIVE_MAINTENANCE\" : \"'+PREVENTIVE_MAINTENANCE+'\",\"CORRECTIVE_MAINTENANCE\" : \"'+CORRECTIVE_MAINTENANCE+'\",\"PERFORMANCE_GUARANTEE\" : \"'+PERFORMANCE_GUARANTEE+'\",\"WET_CLEAN\" : \"'+WET_CLEAN+'\",\"PM_EVENTS\" : \"'+PM_EVENTS+'\",\"ACCOUNT_NAME\" : \"'+ACCOUNT_NAME+'\",\"FAB_NAME\" : \"'+FAB_NAME+'\",\"CONSUMABLES\" : \"'+CONSUMABLE+'\",\"CONTRACT_YEAR\" : \"'+CONTRACT_YEAR+'\",\"LINE\" : \"'+LINE+'\",\"NON_CONSUMABLE\" : \"'+NON_CONSUMABLE+'\",\"TECH_NODE_RANGE\" : \"'+TECH_NODE_RANGE+'\",\"SWAP_KIT\" : \"'+SWAP_KIT+'\",\"PM_NAME\" : \"'+PM_NAME+'\",\"PM_PER_YEAR\" : \"'+PM_PER_YEAR+'\"}' AS JSON from (SELECT DISTINCT  "+str(timestamp_sessionid2)+" + 'Flex' as SESSION_ID, B.QUOTE_ID+'-'+ CONVERT(VARCHAR,B.QTEREV_ID) AS QUOTE_ID,ISNULL(SALESORG_ID,'') AS SALESORG_ID,ISNULL(REGION,'') AS REGION, ISNULL(B.EQUIPMENT_ID,'') AS EQUIPMENT_ID,ISNULL(B.SERVICE_ID,'') AS SERVICE_ID,ISNULL(C.ASSEMBLY_ID,'') AS ASSEMBLY_ID,ISNULL( COVERAGE,'' ) AS LABOR_COVERAGE,ISNULL(PMLABOR,'') AS PREVENTIVE_MAINTENANCE,ISNULL(CMLABOR,'') AS CORRECTIVE_MAINTENANCE,ISNULL(PERFGUARANTEE,'') AS PERFORMANCE_GUARANTEE,ISNULL(WETCLEAN,'') AS WET_CLEAN,ISNULL(PM_EVENTS,'') AS PM_EVENTS, ISNULL(C.PM_ID,'') AS PM_NAME,ISNULL(CONVERT(VARCHAR(50),C.PM_FREQUENCY),'') AS PM_PER_YEAR,CONVERT(VARCHAR(11),B.CONTRACT_VALID_FROM,121) AS CONTRACT_VALID_FROM,CONVERT(VARCHAR(11),B.CONTRACT_VALID_TO,121) AS CONTRACT_VALID_TO,ISNULL(CONSUMABLE,'') AS CONSUMABLE,ISNULL(NON_CONSUMABLE,'') AS NON_CONSUMABLE,ISNULL(ACCOUNT_NAME,'') AS ACCOUNT_NAME,ISNULL(FAB_NAME,'') AS FAB_NAME,ISNULL(TECH_NODE_RANGE,'') AS TECH_NODE_RANGE,convert(varchar,B.LINE) as LINE,CONTRACT_YEAR,ISNULL(SWAP_KIT,'') AS SWAP_KIT FROM "+str(SAQSCO)+" B(NOLOCK) JOIN "+str(SAQGPM)+" SAQGPM ON B.QUOTE_ID = SAQGPM.QUOTE_ID AND B.QTEREV_ID = SAQGPM.QTEREV_ID AND B.SERVICE_ID = SAQGPM.SERVICE_ID AND B.GOT_CODE = SAQGPM.GOT_CODE  AND ISNULL(B.PROCESS_TYPE,'') = ISNULL(SAQGPM.PROCESS_TYPE,'') AND ISNULL(B.DEVICE_NODE,'') = ISNULL(SAQGPM.DEVICE_NODE,'') AND B.MNTEVT_LEVEL = SAQGPM.MNTEVT_LEVEL AND CASE WHEN ISNULL(B.PM_ID,'')='' THEN SAQGPM.PM_ID ELSE B.PM_ID END = SAQGPM.PM_ID JOIN "+str(SAQGPA)+" C(NOLOCK) ON B.QUOTE_ID = C.QUOTE_ID AND B.SERVICE_ID = C.SERVICE_ID AND B.GOT_CODE = C.GOT_CODE  AND ISNULL(B.PROCESS_TYPE,'') = ISNULL(C.PROCESS_TYPE,'') AND ISNULL(B.DEVICE_NODE,'') = ISNULL(C.DEVICE_NODE,'') AND SAQGPM.PM_ID = C.PM_ID AND B.EQUIPMENT_ID = C.EQUIPMENT_ID WHERE B.QUOTE_ID = '"+str(Qt_id)+"' AND ISNULL(QUOTE_TYPE,'TOOL BASED') IN ('EVENT BASED', 'FLEX EVENT BASED') ) t 	) A FOR XML PATH ('')  ), 1, 1, '')+']}','amp;#','#') AS RESULT "
		)
		
		TKM_EVENT = SqlHelper.GetFirst(
			"SELECT replace ('{\"QTQICA\": ['+STUFF((SELECT ','+ JSON FROM (SELECT DISTINCT '{\"SESSION_ID\" : \"'+SESSION_ID+'\",\"QUOTE_ID\" : \"'+QUOTE_ID+'\",\"EQUIPMENT_ID\" : \"'+EQUIPMENT_ID+'\",\"CONTRACT_VALID_FROM\" : \"'+CONTRACT_VALID_FROM+'\",\"CONTRACT_VALID_TO\" : \"'+CONTRACT_VALID_TO+'\",\"SERVICE_ID\" : \"'+SERVICE_ID+'\",\"SALESORG_ID\" : \"'+SALESORG_ID+'\",\"REGION\" : \"'+REGION+'\",\"ASSEMBLY_ID\" : \"'+ASSEMBLY_ID+'\",\"LABOR_COVERAGE\" : \"'+LABOR_COVERAGE+'\",\"PREVENTIVE_MAINTENANCE\" : \"'+PREVENTIVE_MAINTENANCE+'\",\"CORRECTIVE_MAINTENANCE\" : \"'+CORRECTIVE_MAINTENANCE+'\",\"PERFORMANCE_GUARANTEE\" : \"'+PERFORMANCE_GUARANTEE+'\",\"WET_CLEAN\" : \"'+WET_CLEAN+'\",\"PM_EVENTS\" : \"'+PM_EVENTS+'\",\"ACCOUNT_NAME\" : \"'+ACCOUNT_NAME+'\",\"FAB_NAME\" : \"'+FAB_NAME+'\",\"CONSUMABLES\" : \"'+CONSUMABLE+'\",\"CONTRACT_YEAR\" : \"'+CONTRACT_YEAR+'\",\"LINE\" : \"'+LINE+'\",\"KIT_ID\" : \"'+KIT_ID+'\",\"NON_CONSUMABLE\" : \"'+NON_CONSUMABLE+'\",\"SWAP_KIT\" : \"'+SWAP_KIT+'\",\"TECH_NODE_RANGE\" : \"'+TECH_NODE_RANGE+'\",\"PM_NAME\" : \"'+PM_NAME+'\",\"PM_PER_YEAR\" : \"'+PM_PER_YEAR+'\"}' AS JSON from (SELECT DISTINCT  "+str(timestamp_sessionid1)+" + 'TKMKIT' as SESSION_ID, B.QUOTE_ID+'-'+ CONVERT(VARCHAR,B.QTEREV_ID) AS QUOTE_ID,ISNULL(SALESORG_ID,'') AS SALESORG_ID,ISNULL(REGION,'') AS REGION, ISNULL(B.EQUIPMENT_ID,'') AS EQUIPMENT_ID,ISNULL(B.SERVICE_ID,'') AS SERVICE_ID,ISNULL(C.ASSEMBLY_ID,'') AS ASSEMBLY_ID,ISNULL( COVERAGE,'' ) AS LABOR_COVERAGE,ISNULL(PMLABOR,'') AS PREVENTIVE_MAINTENANCE,ISNULL(CMLABOR,'') AS CORRECTIVE_MAINTENANCE,ISNULL(PERFGUARANTEE,'') AS PERFORMANCE_GUARANTEE,ISNULL(WETCLEAN,'') AS WET_CLEAN,ISNULL(PM_EVENTS,'') AS PM_EVENTS, ISNULL(C.PM_ID,'') AS PM_NAME,ISNULL(CONVERT(VARCHAR(50),C.PM_FREQUENCY),'') AS PM_PER_YEAR,CONVERT(VARCHAR(11),B.CONTRACT_VALID_FROM,121) AS CONTRACT_VALID_FROM,CONVERT(VARCHAR(11),B.CONTRACT_VALID_TO,121) AS CONTRACT_VALID_TO,ISNULL(CONSUMABLE,'') AS CONSUMABLE,ISNULL(NON_CONSUMABLE,'') AS NON_CONSUMABLE,ISNULL(ACCOUNT_NAME,'') AS ACCOUNT_NAME,ISNULL(FAB_NAME,'') AS FAB_NAME,ISNULL(TECH_NODE_RANGE,'') AS TECH_NODE_RANGE,convert(varchar,B.LINE) as LINE,ISNULL(CONVERT(VARCHAR(100),B.KIT_ID),'') AS KIT_ID,CONTRACT_YEAR,ISNULL(SWAP_KIT,'') AS SWAP_KIT FROM "+str(SAQSCO)+" B(NOLOCK) JOIN "+str(SAQGPM)+" SAQGPM ON B.QUOTE_ID = SAQGPM.QUOTE_ID AND B.QTEREV_ID = SAQGPM.QTEREV_ID AND B.SERVICE_ID = SAQGPM.SERVICE_ID AND B.GOT_CODE = SAQGPM.GOT_CODE  AND ISNULL(B.PROCESS_TYPE,'') = ISNULL(SAQGPM.PROCESS_TYPE,'') AND ISNULL(B.DEVICE_NODE,'') = ISNULL(SAQGPM.DEVICE_NODE,'') AND CASE WHEN ISNULL(B.KIT_ID,'')='' THEN SAQGPM.KIT_ID ELSE B.KIT_ID END = SAQGPM.KIT_ID JOIN "+str(SAQGPA)+" C(NOLOCK) ON B.QUOTE_ID = C.QUOTE_ID AND B.SERVICE_ID = C.SERVICE_ID AND B.GOT_CODE = C.GOT_CODE  AND ISNULL(B.PROCESS_TYPE,'') = ISNULL(C.PROCESS_TYPE,'') AND ISNULL(B.DEVICE_NODE,'') = ISNULL(C.DEVICE_NODE,'') AND SAQGPM.KIT_ID = C.KIT_ID AND B.EQUIPMENT_ID = C.EQUIPMENT_ID WHERE B.QUOTE_ID = '"+str(Qt_id)+"' AND B.SERVICE_ID in ('Z0010','Z0128' )) t 	) A FOR XML PATH ('')  ), 1, 1, '')+']}','amp;#','#') AS RESULT "
		)
		
		StatusUpdate = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE SAQICO SET STATUS=''ASSEMBLY IS MISSING'' FROM SAQICO (NOLOCK) LEFT JOIN "+str(SAQSCA)+"  SAQSCA(NOLOCK) ON SAQSCA.QUOTE_ID = SAQICO.QUOTE_ID AND SAQSCA.EQUIPMENT_ID = SAQICO.EQUIPMENT_ID AND SAQSCA.SERVICE_ID = SAQICO.SERVICE_ID AND SAQSCA.QTEREV_ID = SAQICO.QTEREV_ID WHERE SAQICO.QUOTE_ID = ''"+str(Qt_id)+"'' AND SAQICO.QTEREV_ID= ''"+str(REVISION_ID) +"'' AND SAQSCA.EQUIPMENT_ID IS NULL AND SAQICO.SERVICE_ID IN (SELECT DISTINCT SERVICE_ID FROM PRSPRV(NOLOCK) WHERE ISNULL(SSCM_COST,''FALSE'')=''TRUE'' ) AND QTETYP=''TOOL BASED'' AND ISNULL(SAQICO.STATUS,'''')='''' '")
		
		StatusUpdate = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE SAQICO SET STATUS=''ASSEMBLY IS MISSING'' FROM SAQICO (NOLOCK) LEFT JOIN "+str(SAQGPA)+"  SAQSCA(NOLOCK) ON SAQSCA.QUOTE_ID = SAQICO.QUOTE_ID AND SAQSCA.SERVICE_ID = SAQICO.SERVICE_ID AND SAQSCA.QTEREV_ID = SAQICO.QTEREV_ID WHERE SAQICO.QUOTE_ID = ''"+str(Qt_id)+"'' AND SAQICO.QTEREV_ID= ''"+str(REVISION_ID) +"'' AND SAQSCA.ASSEMBLY_ID IS NULL AND SAQICO.SERVICE_ID IN (SELECT DISTINCT SERVICE_ID FROM PRSPRV(NOLOCK) WHERE ISNULL(SSCM_COST,''FALSE'')=''TRUE'' ) AND QTETYP IN (''EVENT BASED'' ,''FLEX EVENT BASED'') AND ISNULL(SAQICO.STATUS,'''')='''' '")
		...
		if str(table).upper() != "NONE" and str(type(table.RESULT)) == "<type 'str'>":
			...
			primaryQueryItems = SqlHelper.GetFirst( ""+ str(Parameter.QUERY_CRITERIA_1)+ " SYINPL (INTEGRATION_PAYLOAD,SESSION_ID,INTEGRATION_NAME)  select ''"+str(table.RESULT)+ "'','"+ str(timestamp_sessionid)+ "',''CPQ_TO_SSCM_LOAD_TEST'' ' ")
		...
		if str(PMSA_EVENT).upper() != "NONE" and str(type(PMSA_EVENT.RESULT)) == "<type 'str'>":
			...
			primaryQueryItems = SqlHelper.GetFirst( ""+ str(Parameter.QUERY_CRITERIA_1)+ " SYINPL (INTEGRATION_PAYLOAD,SESSION_ID,INTEGRATION_NAME)  select ''"+str(PMSA_EVENT.RESULT)+ "'','"+ str(timestamp_sessionid)+ "'+''Flex'',''CPQ_TO_SSCM_LOAD_TEST'' ' ")
		...
		if str(TKM_EVENT).upper() != "NONE" and str(type(TKM_EVENT.RESULT)) == "<type 'str'>":
			...
			#Log.Info("thried stage --->")
			Parameter = SqlHelper.GetFirst("SELECT QUERY_CRITERIA_1 FROM SYDBQS (NOLOCK) WHERE QUERY_NAME = 'SELECT' ")

			primaryQueryItems = SqlHelper.GetFirst( ""+ str(Parameter.QUERY_CRITERIA_1)+ " SYINPL (INTEGRATION_PAYLOAD,SESSION_ID,INTEGRATION_NAME)  select ''"+str(TKM_EVENT.RESULT)+ "'','"+ str(timestamp_sessionid)+ "'+''Flex'',''CPQ_TO_SSCM_LOAD_TEST'' ' ")
		...
		if str(table_Z0100).upper() != "NONE" and str(type(table_Z0100.RESULT)) == "<type 'str'>":
			...
			#Log.Info("fourth stage --->")

			Parameter = SqlHelper.GetFirst("SELECT QUERY_CRITERIA_1 FROM SYDBQS (NOLOCK) WHERE QUERY_NAME = 'SELECT' ")

			primaryQueryItems = SqlHelper.GetFirst( ""+ str(Parameter.QUERY_CRITERIA_1)+ " SYINPL (INTEGRATION_PAYLOAD,SESSION_ID,INTEGRATION_NAME)  select ''"+str(table_Z0100.RESULT)+ "'','"+ str(timestamp_sessionid)+ "',''CPQ_TO_SSCM_LOAD_TEST'' ' ")
		...
		if "Successfully" in crm_response1:
			...
			StatusUpdate = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE SAQICO SET STATUS=''ACQUIRING'' FROM SAQICO (NOLOCK) JOIN "+str(SAQSCA)+"  SAQSCA(NOLOCK) ON SAQSCA.QUOTE_ID = SAQICO.QUOTE_ID AND SAQSCA.EQUIPMENT_ID = SAQICO.EQUIPMENT_ID AND SAQSCA.SERVICE_ID = SAQICO.SERVICE_ID WHERE SAQSCA.QUOTE_ID = ''"+str(Qt_id)+"'' AND SAQICO.QTEREV_ID= ''"+str(REVISION_ID) +"'' AND QTETYP=''TOOL BASED'' AND ISNULL(SAQICO.STATUS,'''')='''' '")
			
			StatusUpdate = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE SAQICO SET STATUS=''ACQUIRING'' FROM SAQICO (NOLOCK) JOIN "+str(SAQGPA)+"  SAQSCA(NOLOCK) ON SAQSCA.QUOTE_ID = SAQICO.QUOTE_ID AND SAQSCA.SERVICE_ID = SAQICO.SERVICE_ID WHERE SAQSCA.QUOTE_ID = ''"+str(Qt_id)+"'' AND SAQICO.QTEREV_ID= ''"+str(REVISION_ID) +"'' AND QTETYP IN (''EVENT BASED'' ,''FLEX EVENT BASED'') AND ISNULL(SAQICO.STATUS,'''')='''' '")

			Emailinfo = SqlHelper.GetFirst("SELECT QUOTE_ID,CPQ,SSCM,CPQ-SSCM AS REMANING FROM (SELECT SAQICO.QUOTE_ID,COUNT(DISTINCT SAQICO.EQUIPMENT_ID) AS CPQ,COUNT(DISTINCT SAQSCA.EQUIPMENT_ID) AS SSCM  FROM SAQICO (NOLOCK) LEFT JOIN "+str(SAQSCA)+" SAQSCA(NOLOCK) ON SAQSCA.QUOTE_ID = SAQICO.QUOTE_ID AND SAQSCA.EQUIPMENT_ID = SAQICO.EQUIPMENT_ID AND SAQSCA.SERVICE_ID = SAQICO.SERVICE_ID WHERE SAQICO.QUOTE_ID = '"+str(Qt_id)+"' AND SAQICO.QTEREV_ID = '"+str(REVISION_ID) +"' AND QTETYP='TOOL BASED' AND SAQICO.SERVICE_ID IN (SELECT DISTINCT SERVICE_ID FROM PRSPRV(NOLOCK) WHERE ISNULL(SSCM_COST,'FALSE')='TRUE' ) group by SAQICO.Quote_ID UNION SELECT SAQICO.QUOTE_ID,COUNT(DISTINCT SAQICO.EQUIPMENT_ID) AS CPQ,COUNT(DISTINCT SAQSCA.EQUIPMENT_ID) AS SSCM  FROM SAQICO (NOLOCK) LEFT JOIN "+str(SAQGPA)+" SAQSCA(NOLOCK) ON SAQSCA.QUOTE_ID = SAQICO.QUOTE_ID AND SAQSCA.SERVICE_ID = SAQICO.SERVICE_ID WHERE SAQICO.QUOTE_ID = '"+str(Qt_id)+"' AND SAQICO.QTEREV_ID = '"+str(REVISION_ID) +"' AND QTETYP IN ('EVENT BASED' ,'FLEX EVENT BASED') AND SAQICO.SERVICE_ID IN (SELECT DISTINCT SERVICE_ID FROM PRSPRV(NOLOCK) WHERE ISNULL(SSCM_COST,'FALSE')='TRUE' ) group by SAQICO.Quote_ID )SUB_SAQICO ")
			...
			msg.Subject = "Quote Successfully Sent to SSCM(Q-Tenant)"
			msg.IsBodyHtml = True
			msg.Body = Error_Info
		...
		if "Status: 400" in  str(crm_response1):
			...
			# Set message subject and body
			msg.Subject = "CPQ to SSCM - Triggering Error Notification(Q-Tenant)"
			msg.IsBodyHtml = True
			msg.Body = Error_Info
