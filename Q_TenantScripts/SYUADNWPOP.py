...
def POPUPLISTVALUEADDNEW(
    ...
    if Question_obj is not None:
        ...
        # ADD FAB POPUP STARTS JOE
        elif str(ObjectName) == "SAQFBL" and str(CurrentTab) == "Quotes" or TABLEID == "ADDNEW__SYOBJR_00038_SYOBJ_00919":
            TreeParam = Product.GetGlobal("TreeParam")
            TreeParentParam = Product.GetGlobal("TreeParentLevel0")  
            ...
            if (("Sending Account -" in TreeParam) or ("Receiving Account -" in TreeParam)) and TreeParentParam == 'Fab Locations':
                ...
                where_string += """ AND FAB_LOCATION_ID NOT IN (SELECT FABLOCATION_ID FROM SAQFBL (NOLOCK) WHERE QUOTE_RECORD_ID = '{contract_quote_record_id}' AND QTEREV_RECORD_ID = '{quote_revision_record_id}') AND ACCOUNT_RECORD_ID IN (SELECT ACCOUNT_RECORD_ID FROM SAACNT WHERE PAR_ACCOUNT_RECORD_ID = (SELECT PAR_ACCOUNT_RECORD_ID FROM SAOPQT WHERE QUOTE_RECORD_ID = '{contract_quote_record_id}' ))""".format(
                ...
                table_data = Sql.GetList(
                    "select  {} from {} (NOLOCK) {} {} {}".format(", ".join(ordered_keys),
                        ObjectName,
                        "WHERE MAFBLC.SALESORG_ID = '{}'".format(sales_org_id) + where_string if where_string else "",
                        order_by,pagination_condition
        ...
        ##Added the below code for the sending fab popup....
            ...
            for key, invs in enumerate(list(ordered_keys)):
                ...
                if key == 0:
                    sec_str += (
                        '<th data-field="'
                        + str(invs)
                        + '" data-formatter="addSendingFabFtsLink" data-sortable="true" data-title-tooltip="'
                        + str(qstring)
                        + '" data-filter-control="input">'
                        + str(qstring)
                        + "</th>"
            ...
            if QryCount == 0:
                ...
                pagedata = str(Page_start) + " - " + str(Page_End)+ " of "
        ##Added the below code for the sending equipment add popup....
        elif str(ObjectName) == "SAQASE" and str(CurrentTab) == "Quotes":
            TreeParam = Product.GetGlobal("TreeParam")
            TreeParentParam = Product.GetGlobal("TreeParentLevel0")  
            account_id = Product.GetGlobal("stp_account_id")
            where_string = ""
            ...
            new_value_dict = {}
            ObjectName = "MAEQUP"
            table_id = "fts_equipments_addnew"
            Header_details = {
            ...
            for invsk in list(Header_details):
                ...
                filter_control_function += (
                    ...
                    + str(table_ids)
                    + '").bootstrapTable("load", date_field  ); $("button#country_save").attr("disabled",true); $("#fts_equipments_addnew").after("<div id=\'noRecDisp\' class=\'noRecord\'>No Records to Display</div>"); $(".noRecord:not(:first)").remove(); } } catch(err) { if(date_field.length > 0) { $("'
                    + str(table_ids)
                    + '").bootstrapTable("load", date_field  ); $("button#country_save").attr("disabled",false); } else{ $("'
                    + str(table_ids)
                    + '").bootstrapTable("load", date_field  ); $("button#country_save").attr("disabled",true); } } ; });  });'
            ...
            pop_val = {}
            where_string += """ ACCOUNT_ID = '{}' AND FABLOCATION_ID = '{}' AND SALESORG_ID = '{}' AND ISNULL(SERIAL_NO, '') <> '' AND ISNULL(GREENBOOK, '') <> '' AND {} EQUIPMENT_RECORD_ID NOT IN (SELECT SND_EQUIPMENT_RECORD_ID FROM SAQASE (NOLOCK) WHERE QUOTE_RECORD_ID = '{}' AND SNDFBL_ID = '{}' AND QTEREV_RECORD_ID = '{}' )""".format(
                account_id,
        ...
        ##Added the below code for the receiving fab popup...
            ...
            if A_Keys != "" and A_Values != "":
                ...
                A_Values = list(A_Values)
                for key, value in zip(A_Keys, A_Values):
                    if key == "QUOTE_REV_SENDING_ACC_FAB_LOCATION_RECORD_ID":
                        key = "SAQSAF.CpqTableEntryId"
                    Trace.Write('Value--->'+str(value))
                    if value.strip():
                        if where_string:
                            where_string += " AND "
            ...
            new_value_dict = {}
            ObjectName = "SAQSAF"
            table_id = "add_receiving_fab"            

            Header_details = {
                "QUOTE_REV_SENDING_ACC_FAB_LOCATION_RECORD_ID": "KEY",
                "SNDFBL_ID": "FAB ID",
                "SNDFBL_NAME": "FAB NAME",
        ...
        elif str(ObjectName) == "SAQSCN" and str(CurrentTab) == "Quotes":
            # Trace.Write("Parent_Param" + str(TreeParentParam))
            where_string = ""
            if A_Keys != "" and A_Values != "":
                A_Keys = list(A_Keys)
                A_Values = list(A_Values)
                for key, value in zip(A_Keys, A_Values):
                    if key == "PRICEBOOK_ENTRIES_RECORD_ID":
                        key = "PRLPBE.CpqTableEntryId"
                    if value.strip():
                        if where_string:
                            where_string += " AND "
                        where_string += "{Key} LIKE '%{Value}%'".format(Key=key, Value=value)
            ...
            sec_str += '<div class="col-md-12 padlftrhtnone" id="btnhide"><div class="row pad-10 bg-lt-wt brdr"><img style="height: 40px; margin-top: -1px; margin-left: -1px; float: left;" src="/mt/APPLIEDMATERIALS_TST/Additionalfiles/Secondary Icon.svg"/><div class="product_txt_div_child secondary_highlight" style="display: block;"><div class="product_txt_child"><abbr title="Key">Add NSOS</abbr></div><div class="product_txt_to_top_child" style="float: left;"><abbr title="Select from the list of parts below to add them to {greenbook}">Select from the list of parts below to add them to {greenbook}</abbr></div></div><div class="product_txt_div_child secondary_highlight" style="display: block;"></div><button type="button" class="btnconfig" data-dismiss="modal" onclick="closepopup_scrl()">CANCEL</button><button type="button" id="add_nsos" class="btnconfig" onclick="add_nsos()" data-dismiss="modal">ADD</button></div></div>'.format(
                Product.GetGlobal("TreeParam"), Product.GetGlobal("TreeParam"),greenbook = Product.GetGlobal("TreeParentLevel0"),
            )
            ...
            stp_account_id = Product.GetGlobal("stp_account_id")
            Pagination_M = Sql.GetFirst(
                "SELECT COUNT(PRLPBE.CpqTableEntryId) as count FROM {} (NOLOCK) WHERE GREENBOOK = '{}' AND PRICEBOOK_TYPE = 'POSS' AND POSS_NSO_PART_ID NOT IN (SELECT POSS_NSO_PART_ID FROM SAQSCN (NOLOCK) WHERE QUOTE_RECORD_ID = '{}' AND QTEREV_RECORD_ID = '{}' )".format(
            ...
            if where_string:
                where_string += " AND"
            where_string += """ GREENBOOK = '{}' AND PRICEBOOK_TYPE = 'POSS' AND  POSS_NSO_PART_ID NOT IN (SELECT POSS_NSO_PART_ID FROM SAQSCN (NOLOCK) WHERE QUOTE_RECORD_ID = '{}' AND QTEREV_RECORD_ID = '{}' )""".format(
        ...
        ##involved party equipment ends
            ...
            for invsk in list(Header_details):
                ...
                filter_control_function += (
                    ...
                    + "\", 'RECORDFEILD':  \""
                    + str(RECORDFEILD)
                    + "\", 'NEWVALUE': '', 'LOOKUPOBJ': '', 'LOOKUPAPI': '','A_Keys':a_list,'A_Values':ATTRIBUTE_VALUEList}, function(data) {  date_field = data[3]; var assoc = data[1]; var api_name = data[2];data4 = data[4];data5 = data[5]; try { console.log('date-field-->'+date_field);if(date_field!='NORECORDS') { $(\""
                    + str(table_ids)
                    ...
                    + str(table_ids)
                    + '").bootstrapTable("load", date_field  ); $("button#country_save").attr("disabled",true); $("#Include_add_on_addnew > tbody").html("<div id=\'noRecDisp\' class=\'noRecord\'>No Records to Display</div>"); $(".noRecord:not(:first)").remove(); $("#Include_add_on_addnew_footer").html(data[6]);} } catch(err) { if(date_field!="NORECORDS") { $("'
                    + str(table_ids)
                    + '").bootstrapTable("load", date_field  ); $("button#country_save").attr("disabled",false); } else{ $("'
                    + str(table_ids)
                    + '").bootstrapTable("load", date_field  );$("#Include_add_on_addnew > tbody").html("<div id=\'noRecDisp\' class=\'noRecord\'>No Records to Display</div>");$("#Include_add_on_addnew_footer").html(data[6]); $("button#country_save").attr("disabled",true); } } ; });  });'
                )
            ...
            account_id=None
            if account_id_query is not None:
            ...
            for invsk in list(Header_details):
                ...
                filter_control_function += (
                    ...
                    + "\", 'RECORDFEILD':  \""
                    + str(RECORDFEILD)
                    + "\", 'NEWVALUE': '', 'LOOKUPOBJ': '', 'LOOKUPAPI': '','A_Keys':a_list,'A_Values':ATTRIBUTE_VALUEList}, function(data) {  date_field = data[3]; var assoc = data[1]; var api_name = data[2];data4 = data[4];data5 = data[5];data15 =data[15]; data16 = data[16]; try { if(date_field!='NORECORDS') { $(\""
                    + str(table_ids)
                    + '"); $("'+str(table_ids)+' > tbody > tr").each(function () {var html_text = $(this).find(\'td:nth-child(11)\').text(); $(this).find(\'td:nth-child(11)\').html(html_text); var html_text = $(this).find(\'td:nth-child(10)\').text(); $(this).find(\'td:nth-child(10)\').html(html_text); var html_text = $(this).find(\'td:nth-child(9)\').text(); $(this).find(\'td:nth-child(9)\').html(html_text); var html_text = $(this).find(\'td:nth-child(8)\').text(); $(this).find(\'td:nth-child(8)\').html(html_text); var html_text = $(this).find(\'td:nth-child(7)\').text(); $(this).find(\'td:nth-child(7)\').html(html_text); var html_text = $(this).find(\'td:nth-child(6)\').text(); $(this).find(\'td:nth-child(6)\').html(html_text); var html_text = $(this).find(\'td:nth-child(5)\').text(); $(this).find(\'td:nth-child(5)\').html(html_text); var html_text = $(this).find(\'td:nth-child(4)\').text(); $(this).find(\'td:nth-child(4)\').html(html_text); var html_text = $(this).find(\'td:nth-child(3)\').text(); $(this).find(\'td:nth-child(3)\').html(html_text);}); $("button#country_save").attr("disabled",false); $("#noRecDisp").remove() } else{ var date_field = [];$("'
                    + str(table_ids)
                    + '").bootstrapTable("load", date_field  ); $("button#country_save").attr("disabled",true);  $("#add_credits_add_new > tbody").after("<div id=\'noRecDisp\' class=\'noRecord\'>No Records to Display</div>"); $(".noRecord:not(:first)").remove(); if (document.getElementById("RecordsStartAndEnd")) { document.getElementById("RecordsStartAndEnd").innerHTML = data15; if (document.getElementById("TotalRecordsCount")) { document.getElementById("TotalRecordsCount").innerHTML = data16;};};} } catch(err) { if(date_field!="NORECORDS") { $("'
                    + str(table_ids)
                    + '").bootstrapTable("load", date_field  ); $("button#country_save").attr("disabled",false); } else{ $("'
                    + str(table_ids)
                    + '").bootstrapTable("load", date_field  ); $("button#country_save").attr("disabled",true); } } ; });  });'
        ...
        ##Add receiving equipment popup.....
            ...
            new_value_dict = {}
            ObjectName = "SAQASE"
            table_id = "add_receiving_equipment"
            Header_details = {
                "QUOTE_REV_SENDING_ACC_FAB_EQUIPMENT_RECORD_ID": "KEY",
                "SND_EQUIPMENT_ID":"EQUIPMENT ID",
                "SND_EQUIPMENT_DESCRIPTION":"EQUIPMENT_DESCRIPTION",
                "GREENBOOK": "GREENBOOK",
                "PLATFORM": "PLATFORM",
            }
            ordered_keys = [
            ...
            sec_str += '<div class="col-md-12 padlftrhtnone" id="btnhide"><div class="row pad-10 bg-lt-wt brdr"><img style="height: 40px; margin-top: -1px; margin-left: -1px; float: left;" src="/mt/APPLIEDMATERIALS_TST/Additionalfiles/Secondary Icon.svg"/><div class="product_txt_div_child secondary_highlight" style="display: block;"><div class="product_txt_child"><abbr title="Key">Customer Region</abbr></div><div class="product_txt_to_top_child" style="float: left;"><abbr title="{region}">{region}</abbr></div></div><div class="product_txt_div_child secondary_highlight" style="display: block;"><div class="product_txt_child"><abbr title="Key">Sales Org</abbr></div><div class="product_txt_to_top_child" style="float: left;"><abbr title="{sales_org}">{sales_org}</abbr></div></div><div class="product_txt_div_child secondary_highlight" style="display: block;"><div class="product_txt_child"><abbr title="Key">Fab Location ID</abbr></div><div class="product_txt_to_top_child" style="float: left;"><abbr title="{}">{}</abbr></div></div><button type="button" class="btnconfig" data-dismiss="modal" onclick="closepopup_scrl()">CANCEL</button><button type="button" id="add-equipment" class="btnconfig" onclick="add_receving_equipment()" data-dismiss="modal">ADD</button></div></div>'.format(
                Product.GetGlobal("TreeParam"), Product.GetGlobal("TreeParam"),sales_org = sales_org,region = region
            )
            ...
            for key, invs in enumerate(list(ordered_keys)):
                ...
                if key == 0:
                    sec_str += (
                        '<th data-field="'
                        + str(invs)
                        + '" data-formatter="addReceivingEquipFtsLink" data-sortable="true" data-title-tooltip="'
                        + str(qstring)
                        + '" data-filter-control="input">'
                        + str(qstring)
                        + "</th>"
            ...
            if where_string:
                where_string += " AND"
            Pagination_M = Sql.GetFirst(
                "SELECT COUNT(CpqTableEntryId) as count FROM {} (NOLOCK) WHERE  ISNULL(GREENBOOK, '') <> '' AND  QUOTE_RECORD_ID = '{}' AND  QTEREV_RECORD_ID = '{}' AND {} SND_EQUIPMENT_RECORD_ID NOT IN (SELECT EQUIPMENT_RECORD_ID FROM SAQFEQ (NOLOCK) WHERE QUOTE_RECORD_ID = '{}'  AND QTEREV_RECORD_ID = '{}' )".format(
                    ObjectName,
                    contract_quote_record_id,
                    quote_revision_record_id,
                    where_string,
            ...
            order_by = "order by SNDFBL_NAME ASC"
            pop_val = {}
            where_string += """ ISNULL(GREENBOOK, '') <> '' AND  QUOTE_RECORD_ID = '{}' AND QTEREV_RECORD_ID = '{}' AND {} SND_EQUIPMENT_RECORD_ID NOT IN (SELECT EQUIPMENT_RECORD_ID FROM SAQFEQ (NOLOCK) WHERE QUOTE_RECORD_ID = '{}' AND QTEREV_RECORD_ID = '{}' )""".format(
                contract_quote_record_id,
                quote_revision_record_id,
                where_string,
                contract_quote_record_id,
                ...
                for key, value in zip(A_Keys, A_Values):
                    if value.strip():
                        ...
                        if TreeParam == 'Add-On Products':
                            key_field = "QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID"
                        else:
                            key_field = "QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID"
                        if key== str(key_field):
                            key="CpqTableEntryId"
                        where_string += "{Key} LIKE '%{Value}%'".format(Key=key, Value=value)
            ...
            if TreeParam == 'Add-On Products':
                Header_details = {
                    "QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID": "KEY",
                    "EQUIPMENT_ID": "EQUIPMENT ID",
                    "EQUIPMENTCATEGORY_DESCRIPTION": "EQUIPMENT CATEGORY",
                    "SERIAL_NO": "SERIAL NUMBER",
                    "CUSTOMER_TOOL_ID": "CUSTOMER TOOL ID",
                    "GREENBOOK": "GREENBOOK",
                    "EQUIPMENT_STATUS": "EQUIPMENT STATUS",
                    "PLATFORM": "PLATFORM",
                    "FABLOCATION_ID": "FABLOCATION ID",
                    "WARRANTY_START_DATE": "WARRANTY START DATE",
                    "WARRANTY_END_DATE": "WARRANTY END DATE",
                    "WARRANTY_END_DATE_ALERT": "WARRANTY END DATE ALERT"
            ...
            if TreeParentParam == "Add-On Products" and TreeParam !="":
                ...
                if TreeParam == 'Sending Equipment' or TreeParam == "Receiving Equipment":
                    ...
                    Pagination_M = Sql.GetFirst(
                    "SELECT COUNT(CpqTableEntryId) as count FROM SAQSCO (NOLOCK) WHERE {where_string} QUOTE_RECORD_ID = '{quo_rec_id}' AND QTEREV_RECORD_ID = '{qurev_rec_id}' AND {restrict_tools} SERVICE_ID = '{TreeSuperParentParam}' AND GREENBOOK = '{TreeParentParam}' AND EQUIPMENT_RECORD_ID NOT IN(SELECT EQUIPMENT_RECORD_ID FROM SAQSCO WHERE QUOTE_RECORD_ID = '{quo_rec_id}' and SERVICE_ID = '{TreeParam}' AND QTEREV_RECORD_ID = '{qurev_rec_id}')".format(where_string=str(where_string)+" AND " if where_string else "",
                        quo_rec_id=Quote.GetGlobal("contract_quote_record_id"),TreeParam = Product.GetGlobal("addon_service_id"),qurev_rec_id = quote_revision_record_id , restrict_tools = " EQUIPMENTCATEGORY_ID = 'Y' AND " if TreeParam == "Z0004" else "",TreeSuperParentParam = TreeSuperParentParam,TreeParentParam = TreeParentParam
                    ))
                else:
                    Pagination_M = Sql.GetFirst(
                    "SELECT COUNT(CpqTableEntryId) as count FROM SAQFEQ (NOLOCK) WHERE {where_string} QUOTE_RECORD_ID = '{quo_rec_id}' AND QTEREV_RECORD_ID = '{qurev_rec_id}' AND {restrict_tools} QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID NOT IN(SELECT QTEREVFEQ_RECORD_ID FROM SAQSCO WHERE QUOTE_RECORD_ID = '{quo_rec_id}' and SERVICE_ID = '{TreeParam}' AND QTEREV_RECORD_ID = '{qurev_rec_id}')".format(where_string=str(where_string)+" AND " if where_string else "",
            ...
            if TreeParam == 'Sending Equipment':
                ...
                where_string += " QUOTE_RECORD_ID = '{quo_rec_id}' AND QTEREV_RECORD_ID = '{qurev_rec_id}' AND SERVICE_ID = '{TreeSuperParentParam}' AND GREENBOOK = '{TreeParentParam}'".format(
                    quo_rec_id=Quote.GetGlobal("contract_quote_record_id"),TreeParam = TreeParam,qurev_rec_id = quote_revision_record_id,TreeSuperParentParam = TreeSuperParentParam, TreeParentParam = TreeParentParam, restrict_tools = " EQUIPMENTCATEGORY_ID = 'Y' AND " if TreeParam == "Z0004" else ""
                )
            else:
                Trace.Write('2572--POPUPPPPPPPPPPP----')
                if TreeParam == "Add-On Products":
                    # Changed condition for defect #1608
                    where_string += " QUOTE_RECORD_ID = '{quo_rec_id}' AND QTEREV_RECORD_ID = '{qurev_rec_id}' AND GREENBOOK = '{greenbook}' AND SERVICE_ID = '{par_serviceid}' AND {restrict_tools} EQUIPMENT_ID NOT IN (SELECT EQUIPMENT_ID FROM SAQSCO WHERE QUOTE_RECORD_ID = '{quo_rec_id}' and SERVICE_ID = '{TreeParam}' AND QTEREV_RECORD_ID  = '{qurev_rec_id}' AND GREENBOOK = '{greenbook}' AND PAR_SERVICE_ID = '{par_serviceid}' )".format(
                        quo_rec_id=Quote.GetGlobal("contract_quote_record_id"),greenbook = TreeParentParam,par_serviceid=TreeSuperParentParam,TreeParam = ADDON_PRD_ID,qurev_rec_id = quote_revision_record_id, restrict_tools = " EQUIPMENTCATEGORY_ID = 'Y' AND " if TreeParam == "Z0004" else ""
                    )
                else:
                    ...
                    if TreeParam == "Z0007" and TreeParentParam == "Complementary Products":
                        Trace.Write('2573----->')
                        where_string += "AND EXISTS (SELECT * FROM SAQASE WHERE QUOTE_RECORD_ID = '{quo_rec_id}' AND QTEREV_RECORD_ID  = '{qurev_rec_id}')".format(quo_rec_id=Quote.GetGlobal("contract_quote_record_id"),qurev_rec_id = quote_revision_record_id)
            if TreeParentParam == "Add-On Products" and TreeParam !="":
                ...
                if offset_skip_count != 0:
                    ...
                    offset_val=offset_skip_count
                table_data = Sql.GetList("SELECT QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID,EQUIPMENT_ID,EQUIPMENTCATEGORY_DESCRIPTION,SERIAL_NO,CUSTOMER_TOOL_ID,GREENBOOK,EQUIPMENT_STATUS,PLATFORM,FABLOCATION_ID,WARRANTY_START_DATE,WARRANTY_END_DATE,WARRANTY_END_DATE_ALERT FROM SAQSCO WHERE {where_string} AND SAQSCO.EQUIPMENT_ID NOT IN (SELECT EQUIPMENT_ID FROM SAQSCO WHERE QUOTE_RECORD_ID = '{quo_rec_id}' and SERVICE_ID ='{addon_service_id}' AND QTEREV_RECORD_ID ='{qurev_rec_id}') order by EQUIPMENT_ID ASC offset {offset_skip_count} rows fetch next {per_page} rows only".format(per_page=PerPage,quo_rec_id=Quote.GetGlobal("contract_quote_record_id"),addon_service_id= Product.GetGlobal("addon_service_id"),TreeParam = TreeParam,qurev_rec_id = quote_revision_record_id,offset_skip_count=offset_val,TreeSuperParentParam = TreeSuperParentParam, TreeParentParam = TreeParentParam,where_string = where_string))
            else:    
                table_data = Sql.GetList(
                    "select {} from {} (NOLOCK) {} {} {} ".format(
                        ", ".join(ordered_keys),
                ...
                for row_data in table_data:
                    ...
                    for data in row_data:
                        if str(data.Key) == "QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID":
                            ...
                            cpqidval = CPQID.KeyCPQId.GetCPQId(ObjectName, str(data.Value))
                            new_value_dict[data.Key] = cpqidval
                        elif str(data.Key) == "QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID":
                            pop_val = str(data.Value) + "|Covered Objects"
                            cpqidval = CPQID.KeyCPQId.GetCPQId(ObjectName, str(data.Value))
                            new_value_dict[data.Key] = cpqidval
                        elif str(data.Key) == "TEMP_TOOL":
                            new_value_dict[data.Key] = '<input  type= "checkbox" class= "custom"  value="' + str(data.Value) + '" style="text-align: center;" {checked} disabled><span class="lbl"></span>'.format(checked = "checked" if str(data.Value).upper() == "TRUE" else "")
                        else:
                            new_value_dict[data.Key] = data.Value
                        new_value_dict["pop_val"] = pop_val
                    date_field.append(new_value_dict)
        ...
        # UNMAPPED EQUIPMENTS STARTED
            ...
            if (("Sending Account -" in TreeParam) or ("Receiving Account -" in TreeParam)) and TreeParentParam == 'Fab Locations':
                ...
                Pagination_M = Sql.GetFirst(
                    "SELECT COUNT(CpqTableEntryId) as count FROM {} (NOLOCK) WHERE ACCOUNT_RECORD_ID IN (SELECT ACCOUNT_RECORD_ID FROM SAACNT WHERE PAR_ACCOUNT_RECORD_ID = (SELECT PAR_ACCOUNT_RECORD_ID FROM SAOPQT WHERE QUOTE_RECORD_ID = '{}' )) AND FABLOCATION_ID = '{}' AND ISNULL(SERIAL_NO, '') <> '' AND ISNULL(GREENBOOK, '') <> '' AND {} EQUIPMENT_RECORD_ID NOT IN (SELECT EQUIPMENT_RECORD_ID FROM SAQFEQ (NOLOCK) WHERE QUOTE_RECORD_ID = '{}' AND QTEREV_RECORD_ID = '{quote_revision_record_id}' AND FABLOCATION_ID = '{}' AND ISNULL(SERIAL_NUMBER,'') <> '')".format(
                        ObjectName,
                        contract_quote_record_id,
        ...
        # UNMAPPED EQUIPMENTS ENDED
            ...
            if (("Sending Account -" in TreeParam) or ("Receiving Account -" in TreeParam)) and TreeParentParam == 'Fab Locations':
                ...
                Pagination_M = Sql.GetFirst(
                    "SELECT COUNT(CpqTableEntryId) as count FROM {} (NOLOCK) WHERE ISNULL(SERIAL_NO, '') <> '' AND ISNULL(GREENBOOK, '') <> '' AND SALESORG_ID = '{}' AND {} 1=1  ".format(
                        ObjectName,
                        sales_org,
                        where_string,
                        contract_quote_record_id,
            ...
            if str(popup_obj)=="SAQRSP":
                ...
                for m in re.finditer(quote_item_tag, entitlement_xml):
                    ...
                    if (non_consumable and get_inclusion) or (non_consumable and get_exclusion):
                        ...
                        if TreeSuperParentParam == "Product Offerings" and TreeParam =='Z0092' and consumable and get_inclusion: 
                            ...
                            iclusions_val_list.append('C')
                where_condition_string = where_string
                '''
                if new_parts_yes == "Yes":
                    ...
                    where_string_1 += where_condition_string
                ...
                # HPQC #1210 - where_string_1 variable changed to paginationQuery since the count query modified
                if new_parts_yes == "Yes" and active_subtab=='New Parts':
                    Log.Info('LN NP 6426')
                    where_string += """ MAMTRL.IS_SPARE_PART = 'True' AND  MAMSOP.SALESORG_ID = '{sales}' AND MAMTRL.PRODUCT_TYPE IS NULL AND NOT EXISTS (SELECT PART_NUMBER FROM SAQRSP (NOLOCK) WHERE QUOTE_RECORD_ID = '{qt_rec_id}' AND QTEREV_RECORD_ID ='{qt_rev_id}' AND MAMTRL.SAP_PART_NUMBER = SAQRSP.PART_NUMBER AND SAQRSP.NEW_PART='True' AND SAQRSP.PAR_SERVICE_ID = '{service_id}')""".format(sales = get_salesval.SALESORG_ID,qt_rec_id = contract_quote_record_id,qt_rev_id = quote_revision_record_id, service_id = Service_Id)
                    
                    paginationQuery = """SELECT COUNT(MAMTRL.CpqTableEntryId) as count FROM MAMTRL (NOLOCK) JOIN MAMSOP (NOLOCK) ON MAMSOP.SAP_PART_NUMBER = MAMTRL.SAP_PART_NUMBER WHERE {whrStr1} MAMSOP.SALESORG_ID = '{sales}' AND MAMTRL.IS_SPARE_PART = 'True' AND MAMTRL.PRODUCT_TYPE IS NULL AND MAMTRL.SAP_PART_NUMBER NOT IN (SELECT PART_NUMBER FROM SAQRSP (NOLOCK) WHERE QUOTE_RECORD_ID = '{qt_rec_id}' AND QTEREV_RECORD_ID = '{qt_rev_id}' AND SAQRSP.NEW_PART='True' AND SAQRSP.PAR_SERVICE_ID = '{service_id}')""".format(sales = get_salesval.SALESORG_ID, qt_rec_id = contract_quote_record_id, qt_rev_id = quote_revision_record_id, service_id = Service_Id, whrStr1 = str(where_string_1) if where_string_1 else "")
                elif active_subtab=='Inclusions':
                    Log.Info('LN Inc 6430')
                    where_string += """ MAMTRL.IS_SPARE_PART = 'True' AND  MAMSOP.SALESORG_ID = '{sales}' AND MAMTRL.PRODUCT_TYPE IS NULL AND NOT EXISTS (SELECT PART_NUMBER FROM SAQRSP (NOLOCK) WHERE QUOTE_RECORD_ID = '{qt_rec_id}' AND QTEREV_RECORD_ID ='{qt_rev_id}' AND MAMTRL.SAP_PART_NUMBER = SAQRSP.PART_NUMBER AND SAQRSP.PAR_SERVICE_ID = '{service_id}' AND (SAQRSP.INCLUDED = 'True' OR (SAQRSP.INCLUDED = 'False' AND SAQRSP.NEW_PART='False')))""".format(sales = get_salesval.SALESORG_ID,qt_rec_id = contract_quote_record_id,qt_rev_id = quote_revision_record_id, service_id = Service_Id)
                    
                    paginationQuery = """SELECT COUNT(MAMTRL.CpqTableEntryId) as count FROM MAMTRL (NOLOCK) JOIN MAMSOP (NOLOCK) ON MAMSOP.SAP_PART_NUMBER = MAMTRL.SAP_PART_NUMBER WHERE {whrStr1} MAMSOP.SALESORG_ID = '{sales}' AND MAMTRL.IS_SPARE_PART = 'True' AND MAMTRL.PRODUCT_TYPE IS NULL AND MAMTRL.SAP_PART_NUMBER NOT IN (SELECT PART_NUMBER FROM SAQRSP (NOLOCK) WHERE QUOTE_RECORD_ID = '{qt_rec_id}' AND QTEREV_RECORD_ID = '{qt_rev_id}' AND SAQRSP.PAR_SERVICE_ID = '{service_id}' AND (SAQRSP.INCLUDED = 'True' OR (SAQRSP.INCLUDED = 'False' AND SAQRSP.NEW_PART='False')))""".format(sales = get_salesval.SALESORG_ID, qt_rec_id = contract_quote_record_id, qt_rev_id = quote_revision_record_id, service_id = Service_Id, whrStr1 = str(where_string_1) if where_string_1 else "")
                elif active_subtab=='Exclusions':
                    Log.Info('LN Exc 6434')
                    where_string += """ MAMTRL.IS_SPARE_PART = 'True' AND  MAMSOP.SALESORG_ID = '{sales}' AND MAMTRL.PRODUCT_TYPE IS NULL AND NOT EXISTS (SELECT PART_NUMBER FROM SAQRSP (NOLOCK) WHERE QUOTE_RECORD_ID = '{qt_rec_id}' AND QTEREV_RECORD_ID ='{qt_rev_id}' AND MAMTRL.SAP_PART_NUMBER = SAQRSP.PART_NUMBER AND SAQRSP.PAR_SERVICE_ID = '{service_id}' AND SAQRSP.NEW_PART='False' AND (SAQRSP.INCLUDED = 'True' OR SAQRSP.INCLUDED = 'False'))""".format(sales = get_salesval.SALESORG_ID,qt_rec_id = contract_quote_record_id,qt_rev_id = quote_revision_record_id, service_id = Service_Id)
                    
                    paginationQuery = """SELECT COUNT(MAMTRL.CpqTableEntryId) as count FROM MAMTRL (NOLOCK) JOIN MAMSOP (NOLOCK) ON MAMSOP.SAP_PART_NUMBER = MAMTRL.SAP_PART_NUMBER WHERE {whrStr1} MAMSOP.SALESORG_ID = '{sales}' AND MAMTRL.IS_SPARE_PART = 'True' AND MAMTRL.PRODUCT_TYPE IS NULL AND MAMTRL.SAP_PART_NUMBER NOT IN (SELECT PART_NUMBER FROM SAQRSP (NOLOCK) WHERE QUOTE_RECORD_ID = '{qt_rec_id}' AND QTEREV_RECORD_ID = '{qt_rev_id}' AND SAQRSP.PAR_SERVICE_ID = '{service_id}' AND SAQRSP.NEW_PART='False' AND (SAQRSP.INCLUDED = 'True' OR SAQRSP.INCLUDED = 'False'))""".format(sales = get_salesval.SALESORG_ID, qt_rec_id = contract_quote_record_id, qt_rev_id = quote_revision_record_id, service_id = Service_Id, whrStr1 = str(where_string_1) if where_string_1 else "")
                else:
                    iclusions_val = str(tuple(iclusions_val_list)).replace(',)',')')
                    where_string += """ MAMTRL.IS_SPARE_PART = 'True' AND MAMSOP.MATPRIGRP_ID in {iclusions_val} and MAMSOP.SALESORG_ID = '{sales}' AND MAMTRL.PRODUCT_TYPE IS NULL AND NOT EXISTS (SELECT PART_NUMBER FROM SAQRSP (NOLOCK) WHERE QUOTE_RECORD_ID = '{qt_rec_id}' AND QTEREV_RECORD_ID ='{qt_rev_id}' AND MAMTRL.SAP_PART_NUMBER = SAQRSP.PART_NUMBER AND SAQRSP.NEW_PART='False' AND SAQRSP.INCLUDED='False')""".format(sales = get_salesval.SALESORG_ID,qt_rec_id = contract_quote_record_id,qt_rev_id = quote_revision_record_id,iclusions_val = iclusions_val)
                    
                    paginationQuery = """SELECT COUNT({ObjName}.CpqTableEntryId) as count FROM {ObjName} (NOLOCK) WHERE {whrStr1} {addWhr} MAMTRL.SAP_PART_NUMBER IN (SELECT SAP_PART_NUMBER FROM MAMSOP WHERE MAMSOP.MATPRIGRP_ID in {iclusions_val} and MAMSOP.SALESORG_ID = '{sales}' )AND MAMTRL.IS_SPARE_PART = 'True' AND MAMTRL.PRODUCT_TYPE IS NULL AND NOT EXISTS (SELECT PART_NUMBER FROM SAQRSP (NOLOCK) WHERE QUOTE_RECORD_ID = '{qt_rec_id}' AND QTEREV_RECORD_ID ='{qt_rev_id}' AND MAMTRL.SAP_PART_NUMBER = SAQRSP.PART_NUMBER AND SAQRSP.NEW_PART='False' AND SAQRSP.INCLUDED='False')""".format(sales = get_salesval.SALESORG_ID,qt_rec_id = contract_quote_record_id,qt_rev_id = quote_revision_record_id,iclusions_val = iclusions_val, ObjName = ObjectName, whrStr1 = str(where_string_1) if where_string_1 else "", addWhr = additional_where)
                
                Pagination_M = Sql.GetFirst("{}".format(str(paginationQuery)))
            ...
            if active_subtab in ('New Parts', 'Inclusions', 'Exclusions'):
                pass
            else:
                where_string_1 = "1=1" if where_string_1 == "" else where_string_1
                Pagination_M = Sql.GetFirst(
                    "SELECT COUNT({}.CpqTableEntryId) as count FROM {} (NOLOCK) WHERE {} {}".format(
                        ObjectName,ObjectName,str(where_string_1) if where_string_1 else "",additional_where
                    )
        ...
        elif str(ObjectName)=="SAQTIP" and str(CurrentTab)=="Quotes" and str(tool_type)=="REPLACE":
            where_string = ""
            if A_Keys != "" and A_Values != "":
                A_Keys = list(A_Keys)
                A_Values = list(A_Values)
                for key, value in zip(A_Keys, A_Values):
                    if value.strip():
                        if key=="ACCOUNT_PARTNER_FUNCTION_ID":
                            key="SAACPF.CpqTableEntryId"
                        where_string += "{Key} LIKE '%{Value}%' AND ".format(Key=key, Value=value)
            DIVNAME = "VIEW_DIV_ID"
            new_value_dict = {}
            ObjectName = "SAACPF"
            table_id = "replace-account"
            Header_details = {
                "ACCOUNT_PARTNER_FUNCTION_ID": "KEY",
                "PARTNER_ID": "ACCOUNT ID",
                "PARTNER_NAME": "ACCOUNT NAME",
                "PARTNERFUNCTION_DESCRIPTION": "ROLE",
                "ADDRESS_1": "ADDRESS"
                # "CITY": "CITY",
                # "REGION": "REGION",
            ...
            ordered_keys = [
                "ACCOUNT_PARTNER_FUNCTION_ID",
                "PARTNER_ID",
                "PARTNER_NAME",
                "PARTNERFUNCTION_DESCRIPTION",
                "ADDRESS_1"
            ]
            Objd_Obj = Sql.GetList(
                "select FIELD_LABEL,API_NAME,LOOKUP_OBJECT,LOOKUP_API_NAME,DATA_TYPE,FORMULA_DATA_TYPE from SYOBJD (NOLOCK)where OBJECT_NAME = '"
            ...
            for key, invs in enumerate(list(ordered_keys)):
                ...
                if key == 0:
                    sec_str += (
                        '<th data-field="'
                        + str(invs)
                        + '" data-formatter="replaceAccountKeyHyperLink" data-sortable="true" data-title-tooltip="'
                        + str(qstring)
                        + '" data-filter-control="input">'
                        + str(qstring)
                        + "</th>"
            ...
            for invsk in list(Header_details):
                ...
                filter_control_function += (
                    ...
                    + "\", 'RECORDFEILD':  \""
                    + str(RECORDFEILD)
                    + "\", 'NEWVALUE': '', 'LOOKUPOBJ': '', 'LOOKUPAPI': '','A_Keys':a_list,'A_Values':ATTRIBUTE_VALUEList,'TOOL_TYPE':'REPLACE'}, function(data) {  date_field = data[3]; var assoc = data[1]; var api_name = data[2];data4 = data[4];data5 = data[5]; data15 = data[15]; data16 = data[16]; try { if(date_field.length > 0) { $(\""
                    + str(table_ids)
                    + '").bootstrapTable("load", date_field  );$("'+str(table_ids)+' > tbody > tr").each(function(){var element = $(this).find("td:nth-child(5)");var text = $(element).text();$(element).attr("title",text);});$("#noRecDisp").remove(); if (document.getElementById("RecordsStartAndEnd")){document.getElementById("RecordsStartAndEnd").innerHTML = data15;}; if (document.getElementById("TotalRecordsCount")) {document.getElementById("TotalRecordsCount").innerHTML = data16;} } else{ $("'
                    + str(table_ids)
                    + '").bootstrapTable("load", date_field  );$("#replace-account").after("<div id=\'noRecDisp\' class=\'noRecord\'>No Records to Display</div>"); $(".noRecord:not(:first)").remove(); } } catch(err) { if(date_field.length > 0) { $("'
                    + str(table_ids)
                    + '").bootstrapTable("load", date_field  ); } else{ $("'
                    + str(table_ids)
                    + '").bootstrapTable("load", date_field  ); document.getElementById("replace-account-model-footer").style.border = "1px solid #ccc"; document.getElementById("replace-account-model-footer").style.padding = "5.5px"; document.getElementById("replace-account-model-footer").innerHTML = "No Records to Display"; } } ; });  });'
                )
            ...
            pagination_condition = "OFFSET {Offset_Skip_Count} ROWS FETCH NEXT {Fetch_Count} ROWS ONLY".format(
                Offset_Skip_Count=offset_skip_count, Fetch_Count= PerPage if str(PerPage)!='' else str(fetch_count)
            )
            TreeParam = Product.GetGlobal("TreeParam")
            Trace.Write("TreeParam"+str(TreeParam))
            inner_join = ""
            ...
            Trace.Write("sales"+str(get_partner_fun.SALESORG_ID))
            #if where_string and 'SAP_PART_NUMBER' in where_string:
            #	where_string = where_string.replace("SAP_PART_NUMBER", "MAMTRL.SAP_PART_NUMBER")
            
            #Pagination_M=Sql.GetFirst("SELECT COUNT (ACCOUNT_PARTNER_FUNCTION_ID)  FROM SAACPF WHERE SALESORG_ID='2034' AND DISTRIBUTIONCHANNEL_ID='01' AND DIVISION_ID='98' AND ACCOUNT_ID='10001191'")
            Pagination_M = Sql.GetFirst("SELECT COUNT(SAACPF.CpqTableEntryId) as count FROM SAACPF")
            ...
            if SortColumn != '' and SortColumnOrder !='':
                ...
                order_by = "order by PARTNER_ID ASC"

            pop_val = {}

            
            #Trace.Write("order_by"+str(order_by)+str(additional_where))
            function_id = Sql.GetFirst("SELECT PARTNERFUNCTION_ID FROM SAQTIP WHERE CpqTableEntryId = '{}' ".format(primary_value.split('-')[1].lstrip('0')))
            ...
            #QueryCountObj = Sql.GetFirst("SELECT COUNT (ACCOUNT_PARTNER_FUNCTION_ID)  FROM SAACPF WHERE SALESORG_ID='2034' AND DISTRIBUTIONCHANNEL_ID='01' AND DIVISION_ID='98' AND ACCOUNT_ID='10001191'")
            
            QueryCountObj = Sql.GetFirst("SELECT COUNT(A.CpqTableEntryId) as count FROM SAACPF A JOIN SAACNT B ON B.ACCOUNT_ID = A.PARTNER_ID WHERE {} SALESORG_ID='{}' AND DISTRIBUTIONCHANNEL_ID='{}' AND DIVISION_ID='{}' AND A.ACCOUNT_ID='{}' AND PARTNERFUNCTION_ID = '{}' AND PARTNER_ID NOT IN (SELECT PARTY_ID AS PARTNER_ID FROM SAQTIP WHERE QUOTE_RECORD_ID = '{}' AND QTEREV_RECORD_ID = '{}' AND PARTNERFUNCTION_ID = '{}' ) ".format(where_string,get_partner_fun.SALESORG_ID,get_partner_fun.DISTRIBUTIONCHANNEL_ID,get_partner_fun.DIVISION_ID,get_partner_fun.ACCOUNT_ID,function_id.PARTNERFUNCTION_ID,contract_quote_record_id,quote_revision_record_id,function_id.PARTNERFUNCTION_ID))

                    
            if QueryCountObj is not None:
                QryCount = QueryCountObj.count
