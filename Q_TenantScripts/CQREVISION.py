...
def set_active_revision(Opertion,cartrev):
	...
	if get_rev_quote_info_details:
		...
		get_rev_info_details = Sql.GetFirst("select QTEREV_ID,CART_ID,ADDUSR_RECORD_ID,REVISION_STATUS from SAQTRV where QUOTE_RECORD_ID = '"+str(quote_contract_recordId)+"' and QUOTE_REVISION_RECORD_ID = '"+str(recid)+"'")
		...
		NRev = QuoteHelper.Edit(get_quote_info_details.QUOTE_ID)
		revision_status = get_rev_info_details.REVISION_STATUS
		cartid = get_rev_info_details.CART_ID
		ownerid = get_rev_info_details.ADDUSR_RECORD_ID
		if str(revision_status.upper()) in ('APR-APPROVED','BOK-CONTRACT BOOKED','OPD-PREPARING QUOTE DOCUMENTS','OPD-CUSTOMER REJECTED','OPD-CUSTOMER ACCEPTED','LGL-PREPARING LEGAL SOW','LGL-LEGAL SOW REJECTED','LGL-LEGAL SOW ACCEPTED','CBC-PREPARING CBC','CBC-CBC COMPLETED','BOK-CONTRACT CREATED'):
			...
			GetApprovalPending = Sql.GetFirst("SELECT STATUS FROM QT__SAQAPP WHERE CartId = {} and OwnerId = {} AND STATUS = 'Approval Submission Pending'".format(cartid,ownerid))

			GetApproved = Sql.GetFirst("SELECT STATUS FROM QT__SAQAPP WHERE CartId = {} and OwnerId = {} AND STATUS = 'Approved'".format(cartid,ownerid))

			GetApprovalRequested = Sql.GetFirst("SELECT STATUS FROM QT__SAQAPP WHERE CartId = {} and OwnerId = {} AND STATUS = 'Approval Requested'".format(cartid,ownerid))

			if (GetApprovalPending and GetApproved and GetApprovalRequested) or (GetApprovalPending and GetApprovalRequested) or (GetApproved and GetApprovalRequested):
				Quote.ChangeQuoteStatus("Awaiting Internal Approval")
			if GetApprovalPending and not GetApprovalRequested and not GetApproved:
				Quote.ChangeQuoteStatus("Preparing")
