...
class Entitlements:
    ...
    def EntitlementRequest(self,cpsConfigID=None,cpsmatchID=None,AttributeID=None,NewValue=None,field_type=None,attr_datatype = None,product_id = None):
                ...
                if STANDARD_ATTRIBUTE_VALUES is not None:				
                    ...
                    for val in STANDARD_ATTRIBUTE_VALUES:						
                        ...
                        if (field_type == 'Check Box' and val.STANDARD_ATTRIBUTE_DISPLAY_VAL in NewValue) or (val.STANDARD_ATTRIBUTE_DISPLAY_VAL == NewValue):
                            ...
                            # try:
                            # 	previous_value = Quote.GetGlobal("previous_ent_val")
                            # except:
                            # 	previous_value = ""
        ...
        #Trace.Write("ELSE = "+str(Quote.GetGlobal("pre_ent_val")))
        ...
        if EntitlementType == 'Dropdown':
            ...
            if get_ent_type:
                if str(get_ent_type.ENTITLEMENT_TYPE).upper() not in ["VALUE DRIVER","VALUE DRIVER COEFFICIENT"] or AttributeID == "AGS_{}_VAL_SVCCMP".format(serviceId):
                                    ...
                                    for prdvalue in Productvalue:
                                        ...
                                        if prdvalue["possibleValues"]:
                                            for i in prdvalue["possibleValues"]:
                                                ...
                                                # and value.SYSTEM_ID == 'AGS_Z0091_KPI_PRPFGT_002' -- Commented for entitlement loading issue value not defined
                    ...
                    Trace.Write("characteristics_attr_values"+str(attributevalues_textbox))
            ...
            if "calc" in AttributeID:
                ...
                for key,dict_val in ENT_IP_DICT.items():
                    ...
                    if key != 'undefined' and dict_val.split("||")[3] != 'undefined' and str((dict_val).split("||")[0]).strip() != 'undefined':
                        ...
                        if str(serviceId) in ("Z0091","Z0004","Z0007","Z0006","Z0092","Z0035","Z0009","Z0090") and key in ( "AGS_{}_TSC_CONSUM".format(serviceId), "AGS_{}_TSC_NONCNS".format(serviceId), "AGS_{}_NON_CONSUMABLE".format(serviceId),"AGS_{}_TSC_RPPNNW".format(serviceId)) and str(tableName) in ('SAQSGE','SAQTSE'):
                            ...
                            if tableName == "SAQSGE":
                                Product.SetGlobal("Greenbook_Entitlement","Yes")
                        ...
                        ##A055S000P01-16862 code ends..
                            ...
                            Product.SetGlobal("IdlingAllowed","Yes")
                        elif "PQB_SPLQTE" in key and entitlement_value == "Yes":
                            Product.SetGlobal("SplitQuote","Yes")							
                        elif key == "AGS_Z0110_TSC_ONSTCP":
                            Trace.Write("@1274---"+str(ENT_IP_DICT["AGS_Z0110_TSC_ONSTCP"]))
                            if str(Product.GetGlobal("ConsignedQty")) != "":
                                ent_value = str(Product.GetGlobal("ConsignedQty"))
                            else:
                                ent_value = 10
                            if int(entitlement_value) >= int(ent_value):
                                try:
                                    ...
                                    Product.SetGlobal("ConsignedQty",str(entitlement_value))
                                except:
                ...
                if Product.GetGlobal("IdlingAllowed") == 'Yes':
                    Product.SetGlobal("IdlingAllowed","No")
                    ...
                    if  AttributeID and str((val).split("||")[1]).strip() == AttributeID:
                        ...
                        if Fullresponse and cpsmatc_incr:
                                        ...
                                        if Productattribute == "characteristics":
                                            for prdvalue in Productvalue:											
                                                for attribute in prdvalue["values"]:
                                                    ...
                                                    # else:
                                                    # 	characteristics_attr_values[str(prdvalue["id"])] = [attribute["value"]]
                            Trace.Write("characteristics_attr_values"+str(attributevalues_textbox)+str(AttributeID))
                            attributevalues_textbox = [text_attr for text_attr in attributevalues_textbox if text_attr.split('%#')[0] in get_text_attr_list ] 
                            temp_list = []
                            for text_val in attributevalues_textbox:
                                text_value = text_val
                            ...
                            if characteristics_attr_values and 'AGS_LAB_OPT' in AttributeID:
                                ...
                                attr_prices = self.get_product_attr_level_cps_pricing(characteristics_attr_values,serviceId)
                                #Quote.SetGlobal('attr_level_pricing',str(attr_prices))
    ...
    def EntitlementCancel(self,SectionRecordId, ENT_CANCEL, Getprevdict,subtabName,EquipmentId):		
        ...
        # gettotallist = Quote.GetGlobal('ent_data_List')	
        ## set entitlement_xml for cancel fn A055S000P01-3157
        # prev_dict = {}
        #try:
        #prev_dict = Quote.GetGlobal('previous_entitlement_xml') 
        ## set entitlement_xml for cancel fn A055S000P01-3157
    ...
    def Rolldown(self):
        ...
        # 	Getprevdict = {}
        ###tool relocation receiving entitilement starts
        # Fullresponse=Quote.GetGlobal("Fullresponse")
        # if Fullresponse:
        # 	Fullresponse = eval(Fullresponse)
        # 	if Fullresponse['complete'] == 'true':
        # 		configuration_status = 'COMPLETE'
