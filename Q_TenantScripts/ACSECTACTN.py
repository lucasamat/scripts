				...
				if statusupdate == True and len(Checkunanimous) <= 0:
					...
					if GetCurStatus:
						...
						if getQuote.QUOTE_STATUS == "APPROVED":
							Log.Info("===> QTPOSTACRM calling from ACSECTACTN 1st=====> ")
				...
				if GetCurrentTranStatus is None:
					...
					if result:
						...
						if GetCurStatus:
							...
							if getQuote.QUOTE_STATUS == "APPROVED":
								Log.Info("===> QTPOSTACRM calling from ACSECTACTN 2st=====> ")
								result = ScriptExecutor.ExecuteGlobal("QTPOSTACRM", {"QUOTE_ID": getQuote.QUOTE_ID, 'Fun_type':'cpq_to_crm'})
			try:
		...
		if GetStatus:        
			...
			if GetCurStatus:
				...
				if getQuote.REVISION_STATUS == "APR-APPROVED":
					Log.Info("===> QTPOSTACRM calling from ACSECTACTN 3st=====> ")
					result = ScriptExecutor.ExecuteGlobal("QTPOSTACRM", {"QUOTE_ID": getQuote.QUOTE_ID, 'Fun_type':'cpq_to_crm'})
		...
		for getnotify in GetNotificationData:
			...
			for objloop in objectdic.values():
				getobjquery = Sql.GetFirst(
					"SELECT OBJECT_NAME,RECORD_NAME FROM SYOBJH (NOLOCK) WHERE LABEL = 'Quote Revision'"
				)
