...
class EntitlementView():
    ...
    def entitlement_view(
        ...
        if EntitlementType != "NO_ENTITLEMENT":
            ...
            validation_dict = {}
            get_lastsection_val = disable_edit = get_requiredicon = ""
            # where = ""
        ...
        quote_status = Sql.GetFirst("SELECT REVISION_STATUS FROM SAQTRV WHERE QUOTE_RECORD_ID = '{}' AND QTEREV_RECORD_ID = '{}' ".format(self.contract_quote_record_id,self.quote_revision_record_id ))
        if TableObj is None and (EntitlementType == "EQUIPMENT"):
            getnameentallowed = []
            if product_tabs_obj:
                ...
                for product_tab_obj in product_tabs_obj:
                    ...
                    if tabwise_product_attributes.get(product_tab_obj.TAB_PROD_ID):
                        ...
                        for attribute in tabwise_product_attributes.get(product_tab_obj.TAB_PROD_ID):
                            ...
                            sec_str_cf =sec_str_imt = sec_str_faccur = sec_str_faccur = sec_str_primp = sec_str_ipp = ""
                            imgstr =""
                            ...
                            if GetXMLsecField and attrSysId in get_attr_leve_based_list:
                                ...
                                for val in GetXMLsecField:
                                    ...
                                    if DType == "Drop Down":
                                        ...
                                        if standard_attr_values and val.ENTITLEMENT_ID == str(attrSysId):
                                            ...
                                            for value in standard_attr_values:
                                                if value.SYSTEM_ID in dropdowndisallowlist or (attrSysId in ('AGS_Z0091_NET_PRMALB','AGS_Z0091W_NET_PRMALB','AGS_Z0035_NET_PRMALB','AGS_Z0035W_NET_PRMALB') and self.treeparam in ('PDC','MPS') and value.STANDARD_ATTRIBUTE_DISPLAY_VAL == 'Included - Monthly and Above') or (attrSysId in ('AGS_Z0091_KPI_TGTKPI','AGS_Z0091W_KPI_TGTKPI','AGS_Z0092W_KPI_TGTKPI','AGS_Z0092_KPI_TGTKPI','AGS_Z0090_KPI_TGTKPI' ) and self.treeparam != 'IMPLANT' and value.STANDARD_ATTRIBUTE_DISPLAY_VAL in ('Max wafer Output <= 4%','Max Wafer Output >4%')) or(attrSysId in ('AGS_Z0091_KPI_TGTKPI','AGS_Z0091W_KPI_TGTKPI','AGS_Z0092W_KPI_TGTKPI','AGS_Z0092_KPI_TGTKPI','AGS_Z0090_KPI_TGTKPI' ) and self.treeparam not in ('MDP','DDP') and value.STANDARD_ATTRIBUTE_DISPLAY_VAL in ('Excursion Detection') ) or (attrSysId in ('AGS_Z0010_PQB_USRFEE') and self.treeparam not in ('CMP') and value.STANDARD_ATTRIBUTE_DISPLAY_VAL == 'AGS_{}_PQB_USRFEE_002'.format(ProductPartnumber) ) or (attrSysId in ('AGS_Z0091_KPI_PRPFGT','AGS_Z0091W_KPI_PRPFGT') and self.treeparam in ('PDC') and value.SYSTEM_ID == 'AGS_{}_KPI_PRPFGT_002'.format(ProductPartnumber) ) :	
                                                    disallow_style = "style = 'display:none'"
                                                else:
                                                    disallow_style = ""
                                        ...
                                        if standard_attr_values and val.ENTITLEMENT_ID == str(attrSysId):
                                            ...
                                            for value in standard_attr_values:
                                                ...
                                                try:
                                                    if not (type(val.ENTITLEMENT_VALUE_CODE) is 'int' or type(val.ENTITLEMENT_VALUE_CODE) is 'float') and val.ENTITLEMENT_VALUE_CODE:
                                        ...
                                        if val.ENTITLEMENT_ID == str(attrSysId):
                                            ...
                                            # 		imgstr = ('<img title=Acquired src=/mt/APPLIEDMATERIALS_UAT/Additionalfiles/clock_exe.svg>')
                                            # 	else:
                                            # 		imgstr  = ""
                                            # else:
                                            # 	imgstr  = ""
                                    ...
                                    new_value_dicta["VALIDATION"]="<abbr id ='"+ str(attrSysId)+"'  class = 'wid90_per requ_validation' title='"+sec_validation+"'>"+sec_validation+"</abbr>"+str(edit_pencil_icon)
        ...
        quote_status = Sql.GetFirst("SELECT REVISION_STATUS FROM SAQTRV WHERE QUOTE_RECORD_ID = '{}' AND QTEREV_RECORD_ID = '{}' ".format(self.contract_quote_record_id,self.quote_revision_record_id ))
        if quote_status:
            if quote_status.REVISION_STATUS in ("PRI-PRICING","APR-APPROVAL PENDING","CFG-ON HOLD - COSTING","PRR-ON HOLD PRICING","CFG-ACQUIRING","APR-RECALLED","APR-REJECTED","APR-APPROVED","OPD-PREPARING QUOTE DOCUMENTS","OPD-CUSTOMER REJECTED","OPD-CUSTOMER ACCEPTED","APR-SUBMITTED TO CUSTOMER","LGL-PREPARING LEGAL SOW","LGL-LEGAL SOW REJECTED","LGL-LEGAL SOW ACCEPTED","CBC-PREPARING CBC","CBC-CBC COMPLETED","BOK-CONTRACT CREATED","BOK-CONTRACT BOOKED","CFG-ON HOLD TKM"):
                dbl_clk_function = ""
        date_field = ""
        new_value_dict = ""
        api_name = ""
