...
class ConfigUpdateScript:
    ...
    def build_query(self, column, obj_name, where_string):
        ...
        if obj_name == "SAQTMT":
            column = "SAQTMT.MASTER_TABLE_QUOTE_RECORD_ID,SAQTRV.QUOTE_ID,SAQTRV.QTEREV_ID,SAQTMT.ACCOUNT_ID,SAQTMT.ACCOUNT_NAME,SAQTRV.CONTRACT_VALID_FROM,SAQTRV.CONTRACT_VALID_TO,SAQTRV.REVISION_STATUS,SAQTRV.SALESORG_ID,SAQTMT.OWNER_NAME,SAQTRV.TRANSACTION_TYPE,SAQTMT.POES,SAQTMT.LOW,SAQTMT.EXPIRED"
            query_string = """
                    SELECT {Column_Name}
    ...
    def banner_content(self, record_id):
        ...
        if record_obj is not None:
            ...
            if objd_records_obj is not None:
                ...
                ##A055S000P01-9370 , A055S000P01-4191 code starts...
                if self.current_tab_name == "Quote":
                    field_lables = "Key,Quote ID,Revision ID,Account ID,Account Name,Contract Valid From,Contract Valid To,Revision status,Sales Org ID,Quote owner,Transaction Type,POES,LOW,Expired"
                else:
            ...
            if key_column and record_id:
                ...
                try:
                    if self.current_tab_name == "Quote":
                        if str(field_values.CONTRACT_VALID_FROM) is not None and str(field_values.CONTRACT_VALID_TO) is not None:
                            field_values.CONTRACT_VALID_FROM = str(field_values.CONTRACT_VALID_FROM).split()[0]
                            field_values.CONTRACT_VALID_TO = str(field_values.CONTRACT_VALID_TO).split()[0]
                        else:
                            pass
        ...
        if object_name is not None:
            ...
            if section_edit_access_list is not None:
                for section_edit_access in section_edit_access_list:
                    ...
                    objd_records_obj_query = """
                            SELECT
                                DISTINCT SYOBJD.OBJECT_NAME, SYOBJD.FIELD_LABEL,case when SYOBJD.EDITABLE_ONINSERT ='TRUE' then 'EDITABLE' Else 'READ ONLY' end AS PERMISSION
                                FROM SYOBJD (NOLOCK)
                                INNER JOIN SYSECT (NOLOCK) ON SYSECT.PRIMARY_OBJECT_NAME = SYOBJD.OBJECT_NAME
                                INNER JOIN SYSEFL (NOLOCK) ON SYSEFL.SECTION_RECORD_ID = SYSECT.RECORD_ID
        ...
        if mmtab_obj is not None:
            ...
            SYSECTs_obj = Sql.GetList(
                """
                    ...
                    WHERE
                        LTRIM(RTRIM(SYPAGE.TAB_RECORD_ID)) = '{Tab_Rec_Id}'  
                    """.format(
                    Tab_Rec_Id=mmtab_obj.RECORD_ID
                )
            )
