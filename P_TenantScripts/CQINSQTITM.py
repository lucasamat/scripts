...
class ContractQuoteItem:
    ...
    def _entitlement_valuedriver(self):
        ...
        Sql.RunQuery("""UPDATE A SET A.SPCCLC = B.ENTITLEMENT_COST_IMPACT,A.SPCCLP = B.ENTITLEMENT_PRICE_IMPACT FROM SAQICO A JOIN PREGBV B ON A.ITSPCL = B.ENTITLEMENT_DISPLAY_VALUE AND A.GREENBOOK = B.GREENBOOK WHERE B.ENTITLEMENT_NAME ='Specialized Cleaning' AND A.QUOTE_RECORD_ID = '{QuoteRecordId}' AND A.QTEREV_RECORD_ID = '{QuoteRevisionRecordId}'""".format(QuoteRecordId=self.contract_quote_record_id,QuoteRevisionRecordId=self.contract_quote_revision_record_id))

        Sql.RunQuery("""UPDATE A SET A.SPCCCI = B.ENTITLEMENT_COST_IMPACT,A.SPCCPI = B.ENTITLEMENT_PRICE_IMPACT FROM SAQICO A JOIN PREGBV B ON A.ITSPCT = B.ENTITLEMENT_DISPLAY_VALUE AND A.GREENBOOK = B.GREENBOOK WHERE B.ENTITLEMENT_NAME ='Specialized Coating' AND A.QUOTE_RECORD_ID = '{QuoteRecordId}' AND A.QTEREV_RECORD_ID ='{QuoteRevisionRecordId}'""".format(QuoteRecordId=self.contract_quote_record_id,QuoteRevisionRecordId=self.contract_quote_revision_record_id))
    ...
    def _quote_annualized_items_insert(self, update=False):
        ...
        if self.quote_service_entitlement_type in ('STR-OFFBGREQPODV OBJ-EQ','STR-OFFBGBEQ OBJ-EQ'):
            Sql.RunQuery("""INSERT SAQICO (EQUIPMENT_DESCRIPTION, STATUS, QUANTITY, OBJECT_ID, EQUPID, EQUIPMENT_ID, EQUIPMENT_RECORD_ID, LINE, QUOTE_ID, QTEITM_RECORD_ID, QUOTE_RECORD_ID, QTEREV_ID, QTEREV_RECORD_ID, KPU, SERNUM, SERIAL_NO, SERVICE_DESCRIPTION, SERVICE_ID, SERVICE_RECORD_ID, TECHNOLOGY, CUSTOMER_TOOL_ID, EQUCAT, EQUIPMENTCATEGORY_ID, EQUIPMENTCATEGORY_RECORD_ID, EQUIPMENT_STATUS, MNT_PLANT_ID, MNT_PLANT_NAME, MNT_PLANT_RECORD_ID, SALESORG_ID, SALESORG_NAME, SALESORG_RECORD_ID, FABLOC, FABLOCATION_ID, FABLOCATION_NAME, FABLOCATION_RECORD_ID, GRNBOK, GREENBOOK, GREENBOOK_RECORD_ID, GLOBAL_CURRENCY,GLOBAL_CURRENCY_RECORD_ID, OBJECT_TYPE, BLUBOK, WTYSTE, WTYEND, WTYDAY, PLTFRM, SUBSIZ, REGION, ISPOES, CSTSRC, PRCSRC, TAXVTP,TAXGRP,DCCRFX,DCCRXD,DOCCUR, INTCPV, INTCPC, OSSVDV, LTCOSS, POFVDV, POFVDC, GBKVDV, GBKVDC, UIMVDV, UIMVDC, CAVVDV, CAVVDC, WNDVDV, WNDVDC, CCRTMV, CCRTMC, SCMVDV, SCMVDC, CCDFFV, CCDFFC, NPIVDV, NPIVDC, DTPVDV, DTPVDC, CSTVDV, CSTVDC, CSGVDV, CSGVDC, QRQVDV, QRQVDC, SVCVDV, SVCVDC, RKFVDV, RKFVDC, PBPVDV, PBPVDC, CMLAB_ENT, CNSMBL_ENT, CNTCVG_ENT, NCNSMB_ENT, PMEVNT_ENT, PMLAB_ENT, PRMKPI_ENT, OFRING, QTETYP, BILTYP, BPTTKP, ATGKEY, TGKPNS, NWPTON, HEDBIN, WETCLN_ENT, SPQTEV, SVSPCT, SPSPCT,SWPKTA,ITTNBV,ITSDUB,ITSDUT,ITSPCL,ITSPCT,CNTYER, STADTE, CONTRACT_VALID_FROM, ENDDTE, CONTRACT_VALID_TO, CNTDAY, QUOTE_ITEM_COVERED_OBJECT_RECORD_ID, CPQTABLEENTRYADDEDBY, CPQTABLEENTRYDATEADDED,CpqTableEntryModifiedBy,CpqTableEntryDateModified)
                    ...
                    FROM 
                        SAQSCO (NOLOCK)					 
                        JOIN SAQSCE (NOLOCK) ON SAQSCE.QUOTE_RECORD_ID = SAQSCO.QUOTE_RECORD_ID AND SAQSCE.SERVICE_ID = SAQSCO.SERVICE_ID AND SAQSCE.QTEREV_RECORD_ID = SAQSCO.QTEREV_RECORD_ID AND ISNULL(SAQSCO.PAR_SERVICE_ID,'') = ISNULL(SAQSCE.PAR_SERVICE_ID,'')
            ...
            Sql.RunQuery("""INSERT SAQICO (EQUIPMENT_DESCRIPTION, STATUS, QUANTITY, OBJECT_ID, EQUPID, EQUIPMENT_ID, EQUIPMENT_RECORD_ID, LINE, QUOTE_ID, QTEITM_RECORD_ID, QUOTE_RECORD_ID, QTEREV_ID, QTEREV_RECORD_ID, KPU, SERNUM, SERIAL_NO, SERVICE_DESCRIPTION, SERVICE_ID, SERVICE_RECORD_ID, TECHNOLOGY, CUSTOMER_TOOL_ID, EQUCAT, EQUIPMENTCATEGORY_ID, EQUIPMENTCATEGORY_RECORD_ID, EQUIPMENT_STATUS, MNT_PLANT_ID, MNT_PLANT_NAME, MNT_PLANT_RECORD_ID, SALESORG_ID, SALESORG_NAME, SALESORG_RECORD_ID, FABLOC, FABLOCATION_ID, FABLOCATION_NAME, FABLOCATION_RECORD_ID, GRNBOK, GREENBOOK, GREENBOOK_RECORD_ID, GLOBAL_CURRENCY,GLOBAL_CURRENCY_RECORD_ID, OBJECT_TYPE, BLUBOK, WTYSTE, WTYEND, WTYDAY, PLTFRM, SUBSIZ, REGION, ISPOES, CSTSRC, PRCSRC, TAXVTP,TAXGRP,DCCRFX,DCCRXD,DOCCUR, PM_ID,KIT_ID, MNTEVT_LEVEL, PM_RECORD_ID, ASSEMBLY_ID, ASSEMBLY_RECORD_ID, GOT_CODE, PROCES, INTCPV, INTCPC, OSSVDV, LTCOSS, POFVDV, POFVDC, GBKVDV, GBKVDC, UIMVDV, UIMVDC, CAVVDV, CAVVDC, WNDVDV, WNDVDC, CCRTMV, CCRTMC, SCMVDV, SCMVDC, CCDFFV, CCDFFC, NPIVDV, NPIVDC, DTPVDV, DTPVDC, CSTVDV, CSTVDC, CSGVDV, CSGVDC, QRQVDV, QRQVDC, SVCVDV, SVCVDC, RKFVDV, RKFVDC, PBPVDV, PBPVDC, CMLAB_ENT, CNSMBL_ENT, CNTCVG_ENT, NCNSMB_ENT, PMEVNT_ENT, PMLAB_ENT, PRMKPI_ENT, OFRING, QTETYP, BILTYP, BPTTKP, ATGKEY, TGKPNS, NWPTON, HEDBIN, WETCLN_ENT, SPQTEV, SVSPCT, SPSPCT,SWPKTA,ITTNBV,ITCTAS,ITSDUB,ITSDUT,ITSPCL,ITSPCT,CNTYER, STADTE, CONTRACT_VALID_FROM, ENDDTE, CONTRACT_VALID_TO, CNTDAY, QUOTE_ITEM_COVERED_OBJECT_RECORD_ID, CPQTABLEENTRYADDEDBY, CPQTABLEENTRYDATEADDED,CpqTableEntryModifiedBy,CpqTableEntryDateModified)
                    SELECT DISTINCT OQ.*, CONVERT(VARCHAR(4000),NEWID()) as QUOTE_ITEM_COVERED_OBJECT_RECORD_ID, '{UserName}' as CPQTABLEENTRYADDEDBY, GETDATE() as CPQTABLEENTRYDATEADDED,{UserId} as CpqTableEntryModifiedBy, GETDATE() as CpqTableEntryDateModified FROM (
                    SELECT DISTINCT IQ.*, CONTRACT_TEMP.YEAR_WISE as CNTYER, CONTRACT_TEMP.VALID_FROM as STADTE, CONTRACT_TEMP.VALID_FROM as CONTRACT_VALID_FROM, CONTRACT_TEMP.VALID_TO as ENDDTE, CONTRACT_TEMP.VALID_TO as CONTRACT_VALID_TO, Abs(DATEDIFF(day,CONTRACT_TEMP.VALID_TO, CONTRACT_TEMP.VALID_FROM)) + 1 as CNTDAY FROM (
                    SELECT DISTINCT					
                    ...
                    FROM 
                        ...
                        ) SAQGPA ON SAQGPA.QUOTE_RECORD_ID = SAQRIT.QUOTE_RECORD_ID AND SAQGPA.QTEREV_RECORD_ID = SAQRIT.QTEREV_RECORD_ID AND SAQGPA.SERVICE_ID = SAQRIT.SERVICE_ID AND SAQGPA.GREENBOOK_RECORD_ID = SAQRIT.GREENBOOK_RECORD_ID AND SAQGPA.PM_ID = SAQRIT.OBJECT_ID	AND SAQGPA.GOT_CODE = SAQRIT.GOT_CODE AND SAQGPA.KIT_ID = SAQRIT.KIT_ID AND ISNULL(SAQGPA.PROCESS_TYPE,'') = ISNULL(SAQRIT.PROCESS_TYPE,'') AND ISNULL(SAQGPA.DEVICE_NODE,'') = ISNULL(SAQRIT.DEVICE_NODE,'')
                    ...
                    LEFT JOIN (
                        SELECT QUOTE_RECORD_ID, QTEREV_RECORD_ID, SERVICE_RECORD_ID, FABLOCATION_RECORD_ID, GREENBOOK_RECORD_ID,KIT_ID,GOT_CODE, EQUIPMENT_ID, CONTRACT_VALID_FROM, CONTRACT_VALID_TO, YEAR_WISE, LINE, YEAR_NUM, YEAR, CASE when YEAR_NUM = 1 THEN CONTRACT_VALID_FROM when (YEAR(VALID_TO) % 4) = 0 AND month(VALID_TO) = 2 AND day(VALID_TO) = 28 then DATEADD(yy,-1,DATEADD(dd,+2,VALID_TO)) ELSE DATEADD(yy,-1,DATEADD(dd,+1,VALID_TO)) END as VALID_FROM, CASE WHEN (YEAR(VALID_TO) % 4) = 0 AND month(VALID_TO) = 2 AND day(VALID_TO) = 28 THEN DATEADD(yy,0,DATEADD(dd,+1,VALID_TO)) WHEN YEAR_NUM = YEAR THEN CONTRACT_VALID_TO ELSE VALID_TO END as VALID_TO from ( SELECT QUOTE_RECORD_ID, QTEREV_RECORD_ID, SERVICE_RECORD_ID, FABLOCATION_RECORD_ID,KIT_ID,GOT_CODE, GREENBOOK_RECORD_ID, EQUIPMENT_ID, CONTRACT_VALID_FROM, CONTRACT_VALID_TO, CONCAT('YEAR ',YEAR_NUM) as YEAR_WISE, CASE WHEN DATEDIFF(dd, CONTRACT_VALID_FROM, DATEADD(yy,-(YEAR-YEAR_NUM),CONTRACT_VALID_TO)) < 0 THEN DATEADD(yy,-(YEAR-(YEAR_NUM+1)),CONTRACT_VALID_TO) WHEN CONTRACT_VALID_FROM = (select CONTRACT_VALID_FROM from SAQTRV (nolock) where QUOTE_RECORD_ID = '{QuoteRecordId}' AND QTEREV_RECORD_ID = '{QuoteRevisionRecordId}') then DATEADD(yy,+YEAR_NUM,DATEADD(dd,-1,CONTRACT_VALID_FROM)) ELSE DATEADD(yy,-(YEAR-YEAR_NUM),CONTRACT_VALID_TO) END as VALID_TO, LINE, YEAR_NUM, YEAR
                        FROM (
                            SELECT DISTINCT CASE WHEN CEILING(DATEDIFF(mm,CONTRACT_VALID_FROM,CONTRACT_VALID_TO)/12.0) = 0 THEN 1 ELSE CEILING(DATEDIFF(dd,CONTRACT_VALID_FROM,CONTRACT_VALID_TO)/365.0) END as YEAR, QUOTE_RECORD_ID,QTEREV_RECORD_ID,SERVICE_RECORD_ID, FABLOCATION_RECORD_ID,KIT_ID,GOT_CODE, GREENBOOK_RECORD_ID, SAQRIT.OBJECT_ID as EQUIPMENT_ID,CONTRACT_VALID_FROM,CONTRACT_VALID_TO,LINE 
                            FROM SAQRIT (NOLOCK) WHERE SAQRIT.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQRIT.QTEREV_RECORD_ID = '{QuoteRevisionRecordId}' AND SAQRIT.SERVICE_ID = '{ServiceId}'
                        ) IQ_SAQRIT CROSS JOIN (SELECT 1 as YEAR_NUM UNION SELECT 2 UNION SELECT 3 UNION SELECT 4 UNION SELECT 5 UNION SELECT 6 UNION SELECT 7 UNION SELECT 8 UNION SELECT 9 UNION SELECT 10) CJQ where YEAR>=YEAR_NUM) I
            ...
            Sql.RunQuery("""UPDATE SAQICO SET QUANTITY = CEILING(ROUND(SAQGPA.QUANTITY/(365/SAQICO.CNTDAY), 2)),
                            ...
                            FROM SAQICO (NOLOCK) 
                            JOIN (
                                SELECT SAQGPA.EQUIPMENT_ID,SAQGPA.SERVICE_ID,SAQGPA.GREENBOOK,SAQGPA.FABLOCATION_ID,SAQGPA.GOT_CODE,SAQGPA.PM_ID,KIT_ID,SAQGPA.QUOTE_RECORD_ID,SAQGPA.QTEREV_RECORD_ID,SAQGPA.PM_RECORD_ID, SAQGPA.GOTCODE_RECORD_ID, SAQGPA.MNTEVT_LEVEL,SAQGPA.SERVICE_DESCRIPTION, SAQGPA.SERVICE_RECORD_ID, SAQGPA.FABLOCATION_NAME, SAQGPA.FABLOCATION_RECORD_ID,SAQGPA.GREENBOOK_RECORD_ID, ISNULL(PROCESS_TYPE,'') AS PROCESS_TYPE , ISNULL(DEVICE_NODE,'') AS DEVICE_NODE, SUM(ISNULL(SAQGPA.PM_FREQUENCY, 0)) as QUANTITY, SUM(ISNULL(SAQGPA.SSCM_PM_FREQUENCY, 0)) as SSCM_PM_FREQUENCY
                                    FROM SAQGPA (NOLOCK) 
                                    WHERE SAQGPA.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQGPA.QTEREV_RECORD_ID = '{QuoteRevisionRecordId}' AND SAQGPA.SERVICE_ID = '{ServiceId}' AND SAQGPA.INCLUDED = 1
            ...
            Sql.RunQuery("""INSERT SAQICO (EQUIPMENT_DESCRIPTION, STATUS, QUANTITY, OBJECT_ID, EQUPID, EQUIPMENT_ID, EQUIPMENT_RECORD_ID, LINE, QUOTE_ID, QTEITM_RECORD_ID, QUOTE_RECORD_ID, QTEREV_ID, QTEREV_RECORD_ID, KPU, SERNUM, SERIAL_NO, SERVICE_DESCRIPTION, SERVICE_ID, SERVICE_RECORD_ID, TECHNOLOGY, CUSTOMER_TOOL_ID, EQUCAT, EQUIPMENTCATEGORY_ID, EQUIPMENTCATEGORY_RECORD_ID, EQUIPMENT_STATUS, MNT_PLANT_ID, MNT_PLANT_NAME, MNT_PLANT_RECORD_ID, SALESORG_ID, SALESORG_NAME, SALESORG_RECORD_ID, FABLOC, FABLOCATION_ID, FABLOCATION_NAME, FABLOCATION_RECORD_ID, GRNBOK, GREENBOOK, GREENBOOK_RECORD_ID, GLOBAL_CURRENCY,GLOBAL_CURRENCY_RECORD_ID, OBJECT_TYPE, BLUBOK, WTYSTE, WTYEND, WTYDAY, PLTFRM, SUBSIZ, REGION, ISPOES, CSTSRC, PRCSRC, TAXVTP,TAXGRP,DCCRFX,DCCRXD,DOCCUR, PM_ID,KIT_ID, MNTEVT_LEVEL, PM_RECORD_ID, ASSEMBLY_ID, ASSEMBLY_RECORD_ID, GOT_CODE, PROCES,ITCTAS, CNTYER, STADTE, CONTRACT_VALID_FROM, ENDDTE, CONTRACT_VALID_TO, CNTDAY, QUOTE_ITEM_COVERED_OBJECT_RECORD_ID, CPQTABLEENTRYADDEDBY, CPQTABLEENTRYDATEADDED,CpqTableEntryModifiedBy,CpqTableEntryDateModified)
                    SELECT DISTINCT OQ.*, CONVERT(VARCHAR(4000),NEWID()) as QUOTE_ITEM_COVERED_OBJECT_RECORD_ID, '{UserName}' as CPQTABLEENTRYADDEDBY, GETDATE() as CPQTABLEENTRYDATEADDED,{UserId} as CpqTableEntryModifiedBy, GETDATE() as CpqTableEntryDateModified FROM (
                    SELECT DISTINCT IQ.*, CONTRACT_TEMP.YEAR_WISE as CNTYER, CONTRACT_TEMP.VALID_FROM as STADTE, CONTRACT_TEMP.VALID_FROM as CONTRACT_VALID_FROM, CONTRACT_TEMP.VALID_TO as ENDDTE, CONTRACT_TEMP.VALID_TO as CONTRACT_VALID_TO, Abs(DATEDIFF(day,CONTRACT_TEMP.VALID_TO, CONTRACT_TEMP.VALID_FROM)) + 1 as CNTDAY FROM (
                    SELECT DISTINCT					
                        SAQGPA.EQUIPMENT_DESCRIPTION,
                    ...
                    LEFT JOIN SAQICO (NOLOCK) ON SAQICO.QUOTE_RECORD_ID = OQ.QUOTE_RECORD_ID AND SAQICO.QTEREV_RECORD_ID = OQ.QTEREV_RECORD_ID AND SAQICO.SERVICE_RECORD_ID = OQ.SERVICE_RECORD_ID AND SAQICO.GREENBOOK_RECORD_ID = OQ.GREENBOOK_RECORD_ID AND ISNULL(SAQICO.FABLOCATION_RECORD_ID,'') = ISNULL(OQ.FABLOCATION_RECORD_ID,'') AND SAQICO.PM_ID = OQ.PM_ID AND SAQICO.KIT_ID = OQ.KIT_ID AND SAQICO.GOT_CODE = OQ.GOT_CODE AND ISNULL(OQ.OBJECT_ID,'') = ISNULL(SAQICO.OBJECT_ID,'')
            ...
            Sql.RunQuery("""UPDATE SAQICO SET QUANTITY = CEILING(ROUND(SAQGPA.QUANTITY/(365/SAQICO.CNTDAY), 2)),
                            ...
                            FROM SAQICO (NOLOCK) 
                            JOIN (
                                SELECT SAQGPA.EQUIPMENT_ID,SAQGPA.SERVICE_ID,SAQGPA.GREENBOOK,SAQGPA.FABLOCATION_ID,SAQGPA.GOT_CODE,SAQGPA.PM_ID,SAQGPA.KIT_ID,SAQGPA.QUOTE_RECORD_ID,SAQGPA.QTEREV_RECORD_ID,SAQGPA.PM_RECORD_ID, SAQGPA.GOTCODE_RECORD_ID, SAQGPA.MNTEVT_LEVEL,SAQGPA.SERVICE_DESCRIPTION, SAQGPA.SERVICE_RECORD_ID, SAQGPA.FABLOCATION_NAME, SAQGPA.FABLOCATION_RECORD_ID,SAQGPA.GREENBOOK_RECORD_ID, ISNULL(PROCESS_TYPE,'') AS PROCESS_TYPE , ISNULL(DEVICE_NODE,'') AS DEVICE_NODE, SUM(ISNULL(SAQGPA.PM_FREQUENCY, 0)) as QUANTITY, SUM(ISNULL(SAQGPA.SSCM_PM_FREQUENCY, 0)) as SSCM_PM_FREQUENCY
                                    FROM SAQGPA (NOLOCK) 
                                    WHERE SAQGPA.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQGPA.QTEREV_RECORD_ID = '{QuoteRevisionRecordId}' AND SAQGPA.SERVICE_ID = '{ServiceId}' AND MNTEVT_LEVEL != 'Scheduled Maintenance' AND SAQGPA.INCLUDED = 1
                                    GROUP BY SAQGPA.SERVICE_ID,SAQGPA.GREENBOOK,SAQGPA.FABLOCATION_ID,SAQGPA.GOT_CODE,SAQGPA.PM_ID,SAQGPA.KIT_ID,SAQGPA.QUOTE_RECORD_ID,SAQGPA.QTEREV_RECORD_ID,SAQGPA.PM_RECORD_ID,SAQGPA.EQUIPMENT_ID,SAQGPA.GOTCODE_RECORD_ID, SAQGPA.MNTEVT_LEVEL,SAQGPA.SERVICE_DESCRIPTION, SAQGPA.SERVICE_RECORD_ID, SAQGPA.FABLOCATION_NAME, SAQGPA.FABLOCATION_RECORD_ID,SAQGPA.GREENBOOK_RECORD_ID, ISNULL(PROCESS_TYPE,'')  , ISNULL(DEVICE_NODE,'')
                                UNION
                                SELECT SAQGPA.EQUIPMENT_ID,SAQGPA.SERVICE_ID,SAQGPA.GREENBOOK,SAQGPA.FABLOCATION_ID,SAQGPA.GOT_CODE,SAQGPA.PM_ID,SAQGPA.KIT_ID,SAQGPA.QUOTE_RECORD_ID,SAQGPA.QTEREV_RECORD_ID,null as PM_RECORD_ID, SAQGPA.GOTCODE_RECORD_ID, SAQGPA.MNTEVT_LEVEL,SAQGPA.SERVICE_DESCRIPTION, SAQGPA.SERVICE_RECORD_ID, SAQGPA.FABLOCATION_NAME, SAQGPA.FABLOCATION_RECORD_ID,SAQGPA.GREENBOOK_RECORD_ID, ISNULL(PROCESS_TYPE,'') AS PROCESS_TYPE , ISNULL(DEVICE_NODE,'') AS DEVICE_NODE, SUM(ISNULL(SAQGPA.PM_FREQUENCY, 0)) as QUANTITY, SUM(ISNULL(SAQGPA.SSCM_PM_FREQUENCY, 0)) as SSCM_PM_FREQUENCY
                                    FROM SAQGPA (NOLOCK) 
                                    ...
                                    GROUP BY SAQGPA.SERVICE_ID,SAQGPA.GREENBOOK,SAQGPA.FABLOCATION_ID,SAQGPA.GOT_CODE,SAQGPA.PM_ID,SAQGPA.KIT_ID,SAQGPA.MNTEVT_LEVEL,SAQGPA.QUOTE_RECORD_ID,SAQGPA.QTEREV_RECORD_ID,SAQGPA.PM_RECORD_ID, SAQGPA.EQUIPMENT_ID,SAQGPA.GOTCODE_RECORD_ID, SAQGPA.MNTEVT_LEVEL,SAQGPA.SERVICE_DESCRIPTION, SAQGPA.SERVICE_RECORD_ID, SAQGPA.FABLOCATION_NAME, SAQGPA.FABLOCATION_RECORD_ID,SAQGPA.GREENBOOK_RECORD_ID, ISNULL(PROCESS_TYPE,'')  , ISNULL(DEVICE_NODE,'')
                            ) SAQGPA ON SAQGPA.QUOTE_RECORD_ID = SAQICO.QUOTE_RECORD_ID AND SAQGPA.QTEREV_RECORD_ID = SAQICO.QTEREV_RECORD_ID AND SAQGPA.SERVICE_ID = SAQICO.SERVICE_ID AND SAQGPA.FABLOCATION_ID = SAQICO.FABLOCATION_ID AND SAQGPA.GREENBOOK = SAQICO.GREENBOOK AND CASE WHEN SAQGPA.MNTEVT_LEVEL != 'Scheduled Maintenance' THEN  SAQGPA.PM_ID ELSE SAQGPA.MNTEVT_LEVEL END = SAQICO.OBJECT_ID AND SAQGPA.GOT_CODE = SAQICO.GOT_CODE AND SAQGPA.EQUIPMENT_ID = SAQICO.EQUIPMENT_ID AND SAQGPA.KIT_ID = SAQICO.KIT_ID AND ISNULL(SAQGPA.PROCESS_TYPE,'')= ISNULL(SAQICO.PROCES,'') AND ISNULL(SAQGPA.DEVICE_NODE,'') = ISNULL(SAQICO.DEVICE_NODE,'')
                            WHERE SAQICO.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQICO.QTEREV_RECORD_ID = '{QuoteRevisionRecordId}' AND SAQICO.SERVICE_ID = '{ServiceId}'""".format(QuoteRecordId=self.contract_quote_record_id,QuoteRevisionRecordId=self.contract_quote_revision_record_id,ServiceId=self.service_id)
            )
            ...
            Sql.RunQuery("""INSERT SAQICO (EQUIPMENT_DESCRIPTION, STATUS, QUANTITY, OBJECT_ID, EQUPID, EQUIPMENT_ID, EQUIPMENT_RECORD_ID, LINE, QUOTE_ID, QTEITM_RECORD_ID, QUOTE_RECORD_ID, QTEREV_ID, QTEREV_RECORD_ID, KPU, SERNUM, SERIAL_NO, SERVICE_DESCRIPTION, SERVICE_ID, SERVICE_RECORD_ID, TECHNOLOGY, CUSTOMER_TOOL_ID, EQUCAT, EQUIPMENTCATEGORY_ID, EQUIPMENTCATEGORY_RECORD_ID, EQUIPMENT_STATUS, MNT_PLANT_ID, MNT_PLANT_NAME, MNT_PLANT_RECORD_ID, SALESORG_ID, SALESORG_NAME, SALESORG_RECORD_ID, FABLOC, FABLOCATION_ID, FABLOCATION_NAME, FABLOCATION_RECORD_ID, GRNBOK, GREENBOOK, GREENBOOK_RECORD_ID, GLOBAL_CURRENCY,GLOBAL_CURRENCY_RECORD_ID, OBJECT_TYPE, BLUBOK, WTYSTE, WTYEND, WTYDAY, PLTFRM, SUBSIZ, REGION, ISPOES, CSTSRC, PRCSRC, TAXVTP,TAXGRP,DCCRFX,DCCRXD,DOCCUR, PM_ID, MNTEVT_LEVEL, PM_RECORD_ID, ASSEMBLY_ID, ASSEMBLY_RECORD_ID, GOT_CODE, PROCES, KIT_ID, INTCPV, INTCPC, OSSVDV, LTCOSS, POFVDV, POFVDC, GBKVDV, GBKVDC, UIMVDV, UIMVDC, CAVVDV, CAVVDC, WNDVDV, WNDVDC, CCRTMV, CCRTMC, SCMVDV, SCMVDC, CCDFFV, CCDFFC, NPIVDV, NPIVDC, DTPVDV, DTPVDC, CSTVDV, CSTVDC, CSGVDV, CSGVDC, QRQVDV, QRQVDC, SVCVDV, SVCVDC, RKFVDV, RKFVDC, PBPVDV, PBPVDC, CMLAB_ENT, CNSMBL_ENT, CNTCVG_ENT, NCNSMB_ENT, PMEVNT_ENT, PMLAB_ENT, PRMKPI_ENT, OFRING, QTETYP, BILTYP, BPTTKP, ATGKEY, TGKPNS, NWPTON, HEDBIN, WETCLN_ENT, SPQTEV, SVSPCT, SPSPCT, SWPKTA,ITTNBV,ITCTAS,ITSDUB,ITSDUT,ITSPCL,ITSPCT,CNTYER, STADTE, CONTRACT_VALID_FROM, ENDDTE, CONTRACT_VALID_TO, CNTDAY, QUOTE_ITEM_COVERED_OBJECT_RECORD_ID, CPQTABLEENTRYADDEDBY, CPQTABLEENTRYDATEADDED,CpqTableEntryModifiedBy,CpqTableEntryDateModified)
                    ...
                    LEFT JOIN (
                        SELECT QUOTE_RECORD_ID, QTEREV_RECORD_ID, SERVICE_RECORD_ID, FABLOCATION_RECORD_ID, GREENBOOK_RECORD_ID,KIT_ID,GOT_CODE, EQUIPMENT_ID, CONTRACT_VALID_FROM, CONTRACT_VALID_TO, YEAR_WISE, LINE, YEAR_NUM, YEAR, CASE when YEAR_NUM = 1 THEN CONTRACT_VALID_FROM when (YEAR(VALID_TO) % 4) = 0 AND month(VALID_TO) = 2 AND day(VALID_TO) = 28 then DATEADD(yy,-1,DATEADD(dd,+2,VALID_TO)) ELSE DATEADD(yy,-1,DATEADD(dd,+1,VALID_TO)) END as VALID_FROM, CASE WHEN (YEAR(VALID_TO) % 4) = 0 AND month(VALID_TO) = 2 AND day(VALID_TO) = 28 THEN DATEADD(yy,0,DATEADD(dd,+1,VALID_TO)) WHEN YEAR_NUM = YEAR THEN CONTRACT_VALID_TO ELSE VALID_TO END as VALID_TO from ( SELECT QUOTE_RECORD_ID, QTEREV_RECORD_ID, SERVICE_RECORD_ID, FABLOCATION_RECORD_ID,KIT_ID,GOT_CODE, GREENBOOK_RECORD_ID, EQUIPMENT_ID, CONTRACT_VALID_FROM, CONTRACT_VALID_TO, CONCAT('YEAR ',YEAR_NUM) as YEAR_WISE, CASE WHEN DATEDIFF(dd, CONTRACT_VALID_FROM, DATEADD(yy,-(YEAR-YEAR_NUM),CONTRACT_VALID_TO)) < 0 THEN DATEADD(yy,-(YEAR-(YEAR_NUM+1)),CONTRACT_VALID_TO) WHEN CONTRACT_VALID_FROM = (select CONTRACT_VALID_FROM from SAQTRV (nolock) where QUOTE_RECORD_ID = '{QuoteRecordId}' AND QTEREV_RECORD_ID = '{QuoteRevisionRecordId}') then DATEADD(yy,+YEAR_NUM,DATEADD(dd,-1,CONTRACT_VALID_FROM)) ELSE DATEADD(yy,-(YEAR-YEAR_NUM),CONTRACT_VALID_TO) END as VALID_TO, LINE, YEAR_NUM, YEAR
                        FROM (
                            SELECT DISTINCT CASE WHEN CEILING(DATEDIFF(mm,CONTRACT_VALID_FROM,CONTRACT_VALID_TO)/12.0) = 0 THEN 1 ELSE CEILING(DATEDIFF(dd,CONTRACT_VALID_FROM,CONTRACT_VALID_TO)/365.0) END as YEAR, QUOTE_RECORD_ID,QTEREV_RECORD_ID,SERVICE_RECORD_ID, FABLOCATION_RECORD_ID,KIT_ID,GOT_CODE, GREENBOOK_RECORD_ID, EQUIPMENT_ID,CONTRACT_VALID_FROM,CONTRACT_VALID_TO, LINE 
                            FROM SAQRIT (NOLOCK) WHERE SAQRIT.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQRIT.QTEREV_RECORD_ID = '{QuoteRevisionRecordId}' AND SAQRIT.SERVICE_ID = '{ServiceId}'
                        ) IQ_SAQRIT CROSS JOIN (SELECT 1 as YEAR_NUM UNION SELECT 2 UNION SELECT 3 UNION SELECT 4 UNION SELECT 5 UNION SELECT 6 UNION SELECT 7 UNION SELECT 8 UNION SELECT 9 UNION SELECT 10) CJQ where YEAR>=YEAR_NUM) I
            ...
            Sql.RunQuery("""INSERT SAQICO (EQUIPMENT_DESCRIPTION, STATUS, QUANTITY, OBJECT_ID, EQUPID, EQUIPMENT_ID, EQUIPMENT_RECORD_ID, LINE, QUOTE_ID, QTEITM_RECORD_ID, QUOTE_RECORD_ID, QTEREV_ID, QTEREV_RECORD_ID, KPU, SERNUM, SERIAL_NO, SERVICE_DESCRIPTION, SERVICE_ID, SERVICE_RECORD_ID, TECHNOLOGY, CUSTOMER_TOOL_ID, EQUCAT, EQUIPMENTCATEGORY_ID, EQUIPMENTCATEGORY_RECORD_ID, EQUIPMENT_STATUS, MNT_PLANT_ID, MNT_PLANT_NAME, MNT_PLANT_RECORD_ID, SALESORG_ID, SALESORG_NAME, SALESORG_RECORD_ID, FABLOC, FABLOCATION_ID, FABLOCATION_NAME, FABLOCATION_RECORD_ID, GRNBOK, GREENBOOK, GREENBOOK_RECORD_ID,ASSEMBLY_ID,ASSEMBLY_RECORD_ID, GLOBAL_CURRENCY,GLOBAL_CURRENCY_RECORD_ID, OBJECT_TYPE, BLUBOK, WTYSTE, WTYEND, WTYDAY, PLTFRM, SUBSIZ, REGION, ISPOES, CSTSRC, PRCSRC, TAXVTP,TAXGRP,DCCRFX,DCCRXD,DOCCUR, INTCPV, INTCPC, OSSVDV, LTCOSS, POFVDV, POFVDC, GBKVDV, GBKVDC, UIMVDV, UIMVDC, CAVVDV, CAVVDC, WNDVDV, WNDVDC, CCRTMV, CCRTMC, SCMVDV, SCMVDC, CCDFFV, CCDFFC, NPIVDV, NPIVDC, DTPVDV, DTPVDC, CSTVDV, CSTVDC, CSGVDV, CSGVDC, QRQVDV, QRQVDC, SVCVDV, SVCVDC, RKFVDV, RKFVDC, PBPVDV, PBPVDC, CMLAB_ENT, CNSMBL_ENT, CNTCVG_ENT, NCNSMB_ENT, PMEVNT_ENT, PMLAB_ENT, PRMKPI_ENT, OFRING, QTETYP, BILTYP, BPTTKP, ATGKEY, TGKPNS, NWPTON, HEDBIN, WETCLN_ENT, SPQTEV, SVSPCT, SPSPCT,SWPKTA,ITTNBV,ITSDUB,ITSDUT,ITSPCL,ITSPCT,CNTYER, STADTE, CONTRACT_VALID_FROM, ENDDTE, CONTRACT_VALID_TO, CNTDAY, QUOTE_ITEM_COVERED_OBJECT_RECORD_ID, CPQTABLEENTRYADDEDBY, CPQTABLEENTRYDATEADDED,CpqTableEntryModifiedBy,CpqTableEntryDateModified)
                    ...
                    FROM 
                        SAQSCA (NOLOCK)	
                        JOIN SAQSCO (NOLOCK) ON SAQSCO.QUOTE_RECORD_ID = SAQSCA.QUOTE_RECORD_ID AND SAQSCO.SERVICE_ID = SAQSCA.SERVICE_ID AND SAQSCO.QTEREV_RECORD_ID = SAQSCA.QTEREV_RECORD_ID AND SAQSCO.EQUIPMENT_ID = SAQSCA.EQUIPMENT_ID
                        JOIN SAQSCE (NOLOCK) ON SAQSCE.QUOTE_RECORD_ID = SAQSCA.QUOTE_RECORD_ID AND SAQSCE.SERVICE_ID = SAQSCA.SERVICE_ID AND SAQSCE.QTEREV_RECORD_ID = SAQSCA.QTEREV_RECORD_ID AND SAQSCO.EQUIPMENT_ID = SAQSCE.EQUIPMENT_ID
        ...
        if self.service_id not in ('Z0048','A6200','Z0101','Z0046'):
            ...
            Sql.RunQuery("""INSERT SAQRIS (CONTRACT_VALID_FROM, CONTRACT_VALID_TO, DIVISION_ID, DIVISION_RECORD_ID, DOC_CURRENCY, DOCCURR_RECORD_ID, GLOBAL_CURRENCY, GLOBAL_CURRENCY_RECORD_ID, PLANT_ID, PLANT_RECORD_ID, SERVICE_DESCRIPTION, SERVICE_ID, SERVICE_RECORD_ID,PAR_SERVICE_DESCRIPTION,PAR_SERVICE_ID,PAR_SERVICE_RECORD_ID, QUANTITY, QUOTE_ID, QUOTE_RECORD_ID, QTEREV_ID, QTEREV_RECORD_ID, BILLING_TYPE, LINE, QUOTE_REV_ITEM_SUMMARY_RECORD_ID, CPQTABLEENTRYADDEDBY, CPQTABLEENTRYDATEADDED, CpqTableEntryModifiedBy, CpqTableEntryDateModified)
                SELECT IQ.*, ROW_NUMBER()OVER(ORDER BY(IQ.SERVICE_ID)) + {ItemSummaryLastLineNo} as LINE, CONVERT(VARCHAR(4000),NEWID()) as QUOTE_REV_ITEM_SUMMARY_RECORD_ID, '{UserName}' as CPQTABLEENTRYADDEDBY, GETDATE() as CPQTABLEENTRYDATEADDED,{UserId} as CpqTableEntryModifiedBy, GETDATE() as CpqTableEntryDateModified FROM (
                    SELECT DISTINCT
                        ...
                        {ObjectName}.SERVICE_ID,
                        {ObjectName}.SERVICE_RECORD_ID,	
                        {ObjectName}.PAR_SERVICE_DESCRIPTION,
                        {ObjectName}.PAR_SERVICE_ID,
                        {ObjectName}.PAR_SERVICE_RECORD_ID,						
                        1 as QUANTITY,
                        SAQTRV.QUOTE_ID,
                        SAQTRV.QUOTE_RECORD_ID,
                        SAQTMT.QTEREV_ID,
        ...
        if self.source_object_name:
            ...
            if self.quote_service_entitlement_type in ('STR-OFFBGBEQ OBJ-EQ') and not self.triggered_from:
                ...
                Sql.RunQuery("""INSERT SAQRIT (QUOTE_REVISION_CONTRACT_ITEM_ID, CPQTABLEENTRYADDEDBY, CPQTABLEENTRYDATEADDED, CpqTableEntryModifiedBy, CpqTableEntryDateModified, CONTRACT_VALID_FROM, CONTRACT_VALID_TO, DOC_CURRENCY, DOCURR_RECORD_ID, EXCHANGE_RATE, EXCHANGE_RATE_DATE, EXCHANGE_RATE_RECORD_ID, GL_ACCOUNT_NO, GLOBAL_CURRENCY, GLOBAL_CURRENCY_RECORD_ID, LINE,EQUIPMENT_ID,EQUIPMENT_RECORD_ID, OBJECT_ID,TEMP_TOOL, OBJECT_TYPE, FABLOCATION_ID, FABLOCATION_NAME, FABLOCATION_RECORD_ID, SERVICE_DESCRIPTION, SERVICE_ID, SERVICE_RECORD_ID,PAR_PRDOFR_ID,PAR_PRDOFR_RECORD_ID, PROFIT_CENTER, QUANTITY, QUOTE_ID, QUOTE_RECORD_ID, QTEREV_ID, QTEREV_RECORD_ID, REF_SALESORDER, STATUS, TAXCLASSIFICATION_DESCRIPTION, TAXCLASSIFICATION_ID, TAXCLASSIFICATION_RECORD_ID,TAX_PERCENTAGE,{DynamicColumnNames} GREENBOOK,BILLING_TYPE,GREENBOOK_RECORD_ID, QTEITMSUM_RECORD_ID)
                    ...
                    FROM (SELECT QUOTE_RECORD_ID,QTEREV_RECORD_ID,SERVICE_ID,GREENBOOK,FABLOCATION_ID,EQUIPMENT_ID,TEMP_TOOL,CONTRACT_VALID_FROM,CONTRACT_VALID_TO,ISNULL(PAR_SERVICE_ID,'') AS PAR_SERVICE_ID, ISNULL(PAR_SERVICE_RECORD_ID,'') AS PAR_SERVICE_RECORD_ID FROM SAQSCO (NOLOCK) WHERE SAQSCO.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQSCO.QTEREV_RECORD_ID = '{QuoteRevisionRecordId}' AND SAQSCO.SERVICE_ID = '{ServiceId}' GROUP BY QUOTE_RECORD_ID,QTEREV_RECORD_ID,SERVICE_ID,GREENBOOK,FABLOCATION_ID,EQUIPMENT_ID,TEMP_TOOL,CONTRACT_VALID_FROM,CONTRACT_VALID_TO,ISNULL(PAR_SERVICE_ID,''),ISNULL(PAR_SERVICE_RECORD_ID,'')) SAQSCO
                    JOIN {ObjectName} (NOLOCK) ON SAQSCO.QUOTE_RECORD_ID = {ObjectName}.QUOTE_RECORD_ID AND SAQSCO.QTEREV_RECORD_ID = {ObjectName}.QTEREV_RECORD_ID AND SAQSCO.SERVICE_ID = {ObjectName}.SERVICE_ID AND SAQSCO.FABLOCATION_ID = SAQSCO.FABLOCATION_ID AND ISNULL(SAQSCO.GREENBOOK,'') = ISNULL({ObjectName}.GREENBOOK,'') AND SAQSCO.EQUIPMENT_ID = {ObjectName}.EQUIPMENT_ID AND ISNULL(SAQSCO.PAR_SERVICE_ID,'') = ISNULL({ObjectName}.PAR_SERVICE_ID,'') 
                    {AncillaryJoin}
                    JOIN SAQTMT (NOLOCK) ON SAQTMT.MASTER_TABLE_QUOTE_RECORD_ID = {ObjectName}.QUOTE_RECORD_ID AND SAQTMT.QTEREV_RECORD_ID = {ObjectName}.QTEREV_RECORD_ID     
                    JOIN SAQTRV (NOLOCK) ON SAQTRV.SALESORG_RECORD_ID = {ObjectName}.SALESORG_RECORD_ID AND SAQTRV.QTEREV_RECORD_ID = {ObjectName}.QTEREV_RECORD_ID AND SAQTRV.QUOTE_RECORD_ID = SAQTMT.MASTER_TABLE_QUOTE_RECORD_ID
            ...
            ##saqrit quantity update					
                ...
                Sql.RunQuery("""INSERT SAQRIT (QUOTE_REVISION_CONTRACT_ITEM_ID, CPQTABLEENTRYADDEDBY, CPQTABLEENTRYDATEADDED, CpqTableEntryModifiedBy, CpqTableEntryDateModified, CONTRACT_VALID_FROM, CONTRACT_VALID_TO, DOC_CURRENCY, DOCURR_RECORD_ID, EXCHANGE_RATE, EXCHANGE_RATE_DATE, EXCHANGE_RATE_RECORD_ID, GL_ACCOUNT_NO, GLOBAL_CURRENCY, GLOBAL_CURRENCY_RECORD_ID, LINE, EQUIPMENT_ID, EQUIPMENT_RECORD_ID, OBJECT_ID, POSS_NSO_PART_ID, OBJECT_TYPE, FABLOCATION_ID, FABLOCATION_NAME, FABLOCATION_RECORD_ID, SERVICE_DESCRIPTION, SERVICE_ID, SERVICE_RECORD_ID,PAR_PRDOFR_ID, PAR_PRDOFR_RECORD_ID,PROFIT_CENTER, QUANTITY, QUOTE_ID, QUOTE_RECORD_ID, QTEREV_ID, QTEREV_RECORD_ID, REF_SALESORDER, STATUS, TAXCLASSIFICATION_DESCRIPTION, TAXCLASSIFICATION_ID, TAXCLASSIFICATION_RECORD_ID,TAX_PERCENTAGE,NET_VALUE_INGL_CURR, NET_PRICE_INGL_CURR,NET_PRICE,YEAR_1_INGL_CURR,YEAR_1,COMVAL_INGL_CURR,ESTVAL_INGL_CURR,COMMITTED_VALUE,ESTIMATED_VALUE, GREENBOOK,BILLING_TYPE,GREENBOOK_RECORD_ID, QTEITMSUM_RECORD_ID, BUSINESS_UNIT)
                    ...
                    LEFT JOIN SAQRIT (NOLOCK) ON SAQRIT.QUOTE_RECORD_ID = OQ.QUOTE_RECORD_ID AND SAQRIT.QTEREV_RECORD_ID = OQ.QTEREV_RECORD_ID AND SAQRIT.SERVICE_RECORD_ID = OQ.SERVICE_RECORD_ID  AND SAQRIT.GREENBOOK_RECORD_ID = OQ.GREENBOOK_RECORD_ID AND SAQRIT.EQUIPMENT_RECORD_ID = OQ.EQUIPMENT_RECORD_ID AND SAQRIT.POSS_NSO_PART_ID = OQ.POSS_NSO_PART_ID AND ISNULL(SAQRIT.PAR_PRDOFR_ID,'') = ISNULL(OQ.PAR_PRDOFR_ID,'')
