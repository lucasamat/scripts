...
class SYLDRTLIST:
	def MDYNMICSQLOBJECT(self, RECORD_ID, PerPage, PageInform, SubTab, PR_CURR, TP, equipment_id,line_item):         
		...
		get_billing_types = get_ttl_amt = get_dt_amt = pricing_manual = ''  
		...
		if Columns != "" and Obj_Name != "":
			...
			if Qstn_REC_ID != "" and Wh_API_NAME != "":
				...
				if str(Qury_str) == "" and str(QuryCount_str) == "":
					...
					if str(RECORD_ID) == "SYOBJR-98795":                                           
						...
						if Product.GetGlobal("TreeParentLevel2") == "Quote Items":                            
							...
							on_hold_tkm = '<img title="ON-HOLD-TKM" src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/icons/on_hold_tkm.svg>'
							on_hold_costing = '<img title="CFG-ON HOLD - COSTING" src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/pricing_on_hold.svg>'
							on_hold_pricing = '<img title="PRR-ON HOLD PRICING" src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/pricing_on_hold.svg>'
							offline_pricing = '<img title="OFFLINE PRICING" src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/manual_pricing.svg>'
							...
							#A055S000P01-4578 starts
								...
								Qury_str = (
									"select top "
										+ str(PerPage)
										+ " CASE WHEN STATUS = 'ACQUIRED' THEN '"+ imgstr +"' WHEN STATUS = 'CFG-ON HOLD TKM' THEN '"+ on_hold_tkm +"' WHEN STATUS = 'APPROVAL REQUIRED' THEN '" +exclamation+ "' WHEN STATUS = 'CFG-ON HOLD - COSTING' THEN '"+on_hold_costing+"' WHEN STATUS = 'PRR-ON HOLD PRICING' THEN '"+on_hold_pricing+"' WHEN STATUS = 'OFFLINE PRICING' THEN '"+offline_pricing+"' WHEN STATUS = 'ERROR' THEN '"+ error +"' WHEN STATUS = 'PARTIALLY PRICED' THEN '"+ partially_priced +"' WHEN STATUS = 'ASSEMBLY IS MISSING' THEN '"+ assembly_missing +"'  ELSE '"+ acquiring_img_str +"' END AS STATUS, QUOTE_ITEM_COVERED_OBJECT_RECORD_ID,LINE,SERVICE_ID,OBJECT_ID,EQUIPMENT_ID,ASSEMBLY_ID,GOT_CODE,CNTYER,EQNODE,PROCES,PM_ID,KIT_NUMBER,KIT_ID,MNTEVT_LEVEL,SAPMMP,CNTDAY,FABLOCATION_ID,GREENBOOK,QUANTITY,TOLCFG,KPU,SPQTEV,SPSPCT,SVSPCT,WTYSTE,WTYEND,WTYDAY,INWRTY,ATGKEC,ATGKEP,NWPTOC,NWPTOP,CONSCP,CONSPI,NONCCI,NONCPI,AIUICI,AIUIPI,AMNCCI,AMNPPI,TNTVGC,TENVGC,ADDCOF,CAVVCI,CAVVPI,BPTKCI,BPTKPI,TRGPRC,SLSPRC,BDVPRC,CELPRC,TGADJP,YOYPCT,USRPRC,BCHPGC,BCHDPT,CNTPRC,ATKNCI,ATKNPI,CpqTableEntryId from ( select  ROW_NUMBER() OVER( ORDER BY LINE) AS ROW, * from SAQICO (NOLOCK) where QUOTE_ID = '"
										+ str(qt_rec_id.QUOTE_ID)
										+ "'  AND QTEREV_RECORD_ID = '"+str(quote_revision_record_id)+"') m where m.ROW BETWEEN "
										+ str(Page_start)
										+ " and "
					...
					## involved parties equipmemt starts
						...
						if billing_date_column:
							...
							if get_current_record:
								...
								if str(get_current_record.PAR_SERVICE_ID) != '' and TreeParam == 'Z0105':
							...
							if get_billing_type:
								...
								if str(get_billing_types).upper() =='FIXED':
									...
									get_ttl_amt = 'ESTVAL_INGL_CURR'
									column_before_pivot_changes = column_before_pivot_change
								elif str(get_billing_types).upper() =='MILESTONE' and TreeParam not in ("Z0006","Z0007","Z0009","Z0123"):
									get_ttl_amt = 'ESTVAL_INGL_CURR'
									column_before_pivot_changes = column_before_pivot_change
								elif str(get_billing_types).upper() =='MILESTONE' and TreeParam in ("Z0006","Z0007","Z0009","Z0123"):
									column_before_pivot_changes = column_before_pivot_change
									get_ttl_amt = 'BILLING_VALUE_INGL_CURR'
								else:
									get_ttl_amt = 'BILLING_VALUE_INGL_CURR'
									column_before_pivot_changes = column_before_pivot_change
								column_before_pivot_changes_updated = column_before_pivot_changes.replace('ESTVAL_INGL_CURR','ISNULL(ESTVAL_INGL_CURR, 0.00) AS ESTVAL_INGL_CURR')
								column_before_pivot_changes_updated_val = column_before_pivot_changes_updated.replace('BILLING_VALUE_INGL_CURR', 'ISNULL(BILLING_VALUE_INGL_CURR, 0.00) AS BILLING_VALUE_INGL_CURR')
								Trace.Write("=>> column_before_pivot_changes_updated_val "+str(column_before_pivot_changes_updated_val))
							if Qustr:
								...
								Qustr += " AND BILLING_YEAR = '{}' AND QTEREV_RECORD_ID = '{}' AND SERVICE_ID = '{}' AND ISNULL(PAR_SERVICE_ID,'') = '{}' AND BILLING_DATE BETWEEN '{}' AND '{}'".format(SubTab,quote_revision_record_id,TreeParam,par_ser_id,billing_date_column[0], billing_date_column[-1])
							Trace.Write(str(TreeParam)+'--Qustr---1925--'+str(get_ttl_amt)+'--Qustr--'+str(Qustr))
							Qustr += " AND QTEREV_RECORD_ID = '{}' AND SERVICE_ID = '{}' AND ISNULL(PAR_SERVICE_ID,'') = '{}' AND BILLING_DATE BETWEEN '{}' AND '{}'".format(quote_revision_record_id,TreeParam,par_ser_id,billing_date_column[0], billing_date_column[-1])
							if TreeParam in ("Z0123","Z0010","Z0006","Z0007","Z0009"):
								Trace.Write(str(TreeParam)+'--TreeParam---Qustr---1995--'+str(Qustr))
								pivot_query_str = """
										...
										""".format(OrderByColumn=Wh_API_NAMEs, Columns=column_before_pivot_changes_updated_val, ObjectName=ObjectName,
													WhereString=Qustr, PivotColumns=pivot_columns,get_ttl_amt=get_ttl_amt)
							else:
								#Trace.Write('2003--->'+str(column_before_pivot_changes_updated))
								pivot_query_str = """
											SELECT ROW_NUMBER() OVER(ORDER BY EQUIPMENT_ID)
											AS ROW, *
												FROM (
											...
											""".format(OrderByColumn=Wh_API_NAMEs, Columns=column_before_pivot_changes_updated_val, ObjectName=ObjectName,
														WhereString=Qustr, PivotColumns=pivot_columns,get_ttl_amt=get_ttl_amt)
								Trace.Write('2003--2019->'+str(pivot_query_str))
							if TreeParam == "Z0009":                        
								Qury_str = """
					...
					##involved parties source fab ends
						...
						Qury_str = (
							...
							+ " "
							+ str(select_obj_str)
							+ ",S.CpqTableEntryId from "
							+ str(ObjectName)
							+ " S (nolock) INNER JOIN MAADPR (NOLOCK) MA ON MA.COMP_PRDOFR_ID=S.SERVICE_ID"
							+ str(Qustr)
						)
						QuryCount_str = "select count(DISTINCT SERVICE_ID) as cnt from " + str(ObjectName) + " S (nolock) INNER JOIN MAADPR (NOLOCK) MA ON MA.COMP_PRDOFR_ID=S.SERVICE_ID " + str(Qustr)                        
						#QuryCount_str = "select count(*) as cnt from " + str(ObjectName) + " (nolock) " + str(Qustr)
				...
				if RECORD_ID == "SYOBJR-94443" and str(current_tab) == "Tab":                    
					...
					acquiring_img_str = '<img title="Acquiring" src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/Cloud_Icon.svg>'
					cps_pricing_img = ""
					#cps_pricing_img ="<a href=''#'' onclick=''cps_pricing_call(this)''><img src=''/mt/APPLIEDMATERIALS_PRD/Additionalfiles/info.png''  style=''height: 15px; width: 15px; background-size: 20px 20px;''> </a>" 
					error = '<img title="Error" src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/exclamation_icon.svg>'
					Qury_str = (
						"SELECT DISTINCT TOP "
						+ str(PerPage)
				...
				for ik in Query_Obj:				                  
					...
					if primary_view != "":                        
						...
						if str(current_tab).upper() == "PROFILE" and (ObjectName == "SYPROF"):
							...
							if qt_status.REVISION_STATUS in ('CFG-CONFIGURING'):
						...
						# elif str(current_tab).upper() == "ROLE":
							...
							#Action_str += '<li><a class="dropdown-item" href="#" onclick="Commonteree_view_RL(this)">VIEW</a></li>'
							Trace.Write("ObjectName_chkZ_1-- "+str(ObjectName))
								...
								if saqdlt_query:
									for role in saqdlt_query:
										if str(role.C4C_PARTNERFUNCTION_ID).upper().strip() in roles_list:
											Trace.Write("SAQICO_IF_CHK")
											Action_str += '<li><a class="dropdown-item" href="#" onclick="Commonteree_view_RL(this)">VIEW</a></li>'
											pricing_manual = 'True'
											break
										else:
											Trace.Write("SAQICO_ELSE_CHK")
											Action_str += ""
					...
					if str(Action_permission.get("Delete")).upper() == "TRUE":
						...
						if related_list_delete_permission:
							quote_contract_recordId = Quote.GetGlobal("contract_quote_record_id")
							quote_revision_record_id = Quote.GetGlobal("quote_revision_record_id")
							qt_status =Sql.GetFirst("""SELECT REVISION_STATUS FROM SAQTRV WHERE  QUOTE_RECORD_ID = '{ContractRecordId}' AND QTEREV_RECORD_ID = '{quote_revision_record_id}'""".format(ContractRecordId =quote_contract_recordId,quote_revision_record_id =quote_revision_record_id))
						...
						if RECORD_ID != 'SYOBJR-98869':
							if value1234 == "ACQUIRED" or value1234 == "PRICED":
								if RECORD_ID == 'SYOBJR-00005' or RECORD_ID == 'SYOBJR-00029':
									value1234 = value1234.replace(value1234,"<img title='"+str(value1234).title()+"' src='/mt/APPLIEDMATERIALS_PRD/Additionalfiles/Green_Tick.svg'> ")
								else:
									value1234 = value1234.replace(value1234,"<img title='"+str(value1234).title()+"' src='/mt/APPLIEDMATERIALS_PRD/Additionalfiles/Green_Tick.svg'> "+str(value1234))
								
							if value1234 == "APPROVAL REQUIRED":
								value1234 = value1234.replace("APPROVAL REQUIRED","<img title='Approval Required' src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/clock_exe.svg> APPROVAL REQUIRED")
							if value1234 == "ACQUIRING":
								if RECORD_ID == 'SYOBJR-00005' or RECORD_ID =='SYOBJR-00029':
									value1234 = value1234.replace("ACQUIRING","<img title='Acquiring' src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/Cloud_Icon.svg>")
								else:
									value1234 = value1234.replace("ACQUIRING","<img title='Acquiring' src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/Cloud_Icon.svg> ACQUIRING")
							if value1234 == "ERROR":
								if RECORD_ID == 'SYOBJR-00005' or RECORD_ID == 'SYOBJR-00029':
									value1234 = value1234.replace("ERROR","<img title='Error' src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/exclamation_icon.svg>")
								else:
									value1234 = value1234.replace("ERROR","<img title='Error' src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/exclamation_icon.svg> ERROR")
							if value1234 == "NOT PRICED" or RECORD_ID =='SYOBJR-00029':
									if RECORD_ID == 'SYOBJR-00005':
										value1234 = value1234.replace("NOT PRICED","<img title='NOT PRICED' class ='img-noprice' src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/no-price.svg>") 
							if value1234 == "ASSEMBLY IS MISSING":
								value1234 = value1234.replace("ASSEMBLY IS MISSING","<img title='Assembly Missing' src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/Orange1_Circle.svg> ASSEMBLY IS MISSING")
							if value1234 == "CFG-ON HOLD - COSTING":
								#Trace.Write("cmng_inside_on_hold_1 "+str(value1234))
								value1234 = value1234.replace("CFG-ON HOLD - COSTING","<img title='CFG-ON HOLD - COSTING' src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/pricing_on_hold.svg>")
							if value1234 ==  "PRR-ON HOLD PRICING":
								value1234 = value1234.replace("PRR-ON HOLD PRICING","<img title='PRR-ON HOLD PRICING' src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/pricing_on_hold.svg>")
							if value1234 ==  "CFG-ON HOLD TKM":
								value1234 = value1234.replace("CFG-ON HOLD TKM","<img title='ON-HOLD-TKM' src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/icons/on_hold_tkm.svg>")
							if value1234 ==  "OFFLINE PRICING":
								value1234 = value1234.replace("OFFLINE PRICING","<img title='OFFLINE PRICING' src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/manual_pricing.svg>")
							if value1234 == "PARTIALLY PRICED":
								value1234 = value1234.replace("PARTIALLY PRICED","<img title='Partially Priced' src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/Red1_Circle.svg> PARTIALLY PRICED")
						else:
							value1234 = value1234    
						...
						if str(cur_api_name) is not None and (
							...
							if value1234 is not None:
								if value1234 != "":
									...
									#else:
									my_format = "{:,." + str(decimal_place) + "f}"
									#Trace.Write(str(value123)+"value1234--"+str(value1234)+'-'+str(decimal_place))
									if value1234:
										value1234 = str(my_format.format(round(float(value1234), int(decimal_place))))
			...
			dict_form = {}			
			cv_list = []
			ignorecol = ['BILLING_DATE', 'BILLING_AMOUNT', 'BILLING_VALUE_INGL_CURR','ESTVAL_INGL_CURR','DELIVERY_SCHED_DATE']
			...
			filter_control_function += (
				...
				+ str(table_ids)
				+ '").bootstrapTable("load", data[0] );if (table_id == "SYOBJR_00009_E5504B40_36E7_4EA6_9774_EA686705A63F"){annualgrid_show();} if (table_id == "SYOBJR_98882_ACE2CB08_73E0_4295_AC8C_78845C297161"){ $("#SYOBJR_98882_ACE2CB08_73E0_4295_AC8C_78845C297161 #INCLUDED").attr("onclick", "OnclickNsoEdit(this)"); if ($(\'.segment_revision_sale_id_text\').text() != \'CFG-CONFIGURING\'){$("#SYOBJR_98882_ACE2CB08_73E0_4295_AC8C_78845C297161 #INCLUDED").attr("disabled","disabled");}else{$("#SYOBJR_98882_ACE2CB08_73E0_4295_AC8C_78845C297161 #INCLUDED").remove("disabled");} } if (document.getElementById(\''
				+ str(table_id)
				...
				if key == 0 and invs != "INCLUDED":
					if invs in primary_link_popup:
						...
						if str(qstring) == "Purchase Order Notes" or str(qstring) == "Equipment Status":
							...
							if (str(TreeParam) != 'Quote Preview' and str(TreeParam) != 'Contract Preview' and  str(TreeParam) != 'Billing Matrix' and str(current_tab).upper() != "APP") and RECORD_ID not in ("SYOBJR-98875","SYOBJR-98873","SYOBJR-00010","SYOBJR-98881","SYOBJR-00005","SYOBJR-98882","SYOBJR-98877"):
		...
		DropDownList = []
		filter_level_list = []
		filter_clas_name = get_billing_cycle = ""
		cv_list = []		
		footer_str, footer = "", ""
		gettotalamt = gettotaldateamt = get_billing_cylce = ""
		if ObjectName == "SAQIBP":
			...
			elif str(get_billing_types).upper() == "MILESTONE" and TreeParam in ("Z0006","Z0007","Z0009"):
				if TreeParam != 'Z0105':
					par_ser_id = ''
				
				gettotaldateamt = Sql.GetList("SELECT BILLING_VALUE_INGL_CURR=SUM(BILLING_VALUE_INGL_CURR),ESTVAL_INGL_CURR = SUM(ESTVAL_INGL_CURR),ANNBILAMT_INGL_CURR = SUM(ANNBILAMT_INGL_CURR),BILLING_DATE FROM SAQIBP WHERE BILLING_DATE in ('{billing_date_column}') and BILLING_YEAR='{subtab}' AND QUOTE_RECORD_ID ='{cq}' AND QTEREV_RECORD_ID='{revision_rec_id}' AND SERVICE_ID = '{service_id}' AND ISNULL(PAR_SERVICE_ID, '') = '{par_ser_id}'  GROUP BY BILLING_DATE".format(cq=str(ContractRecordId),revision_rec_id = quote_revision_record_id,billing_date_column="','".join(billing_date_column),service_id = TreeParam,subtab=subTab, par_ser_id=par_ser_id))
			elif TreeParam == "Z0105":
				 
				gettotaldateamt = Sql.GetList("SELECT BILLING_VALUE_INGL_CURR=SUM(BILLING_VALUE_INGL_CURR),ESTVAL_INGL_CURR = SUM(ESTVAL_INGL_CURR),ANNBILAMT_INGL_CURR = SUM(ANNBILAMT_INGL_CURR),BILLING_DATE FROM SAQIBP WHERE BILLING_DATE in ('{billing_date_column}') and QUOTE_RECORD_ID ='{cq}' AND QTEREV_RECORD_ID='{revision_rec_id}' AND SERVICE_ID = '{service_id}' AND BILLING_YEAR = '{BillingYear}' AND ISNULL(PAR_SERVICE_ID, '') = '{par_ser_id}'      group by BILLING_DATE, BILLING_YEAR".format(cq=str(ContractRecordId),revision_rec_id = quote_revision_record_id,billing_date_column="','".join(billing_date_column),service_id = TreeParam, BillingYear=SubTab, par_ser_id=par_ser_id))
			else:
				gettotaldateamt = Sql.GetList("SELECT BILLING_VALUE_INGL_CURR=SUM(BILLING_VALUE_INGL_CURR),ESTVAL_INGL_CURR = SUM(ESTVAL_INGL_CURR),ANNBILAMT_INGL_CURR = SUM(ANNBILAMT_INGL_CURR),BILLING_DATE FROM SAQIBP WHERE BILLING_DATE in ('{billing_date_column}') and QUOTE_RECORD_ID ='{cq}' AND QTEREV_RECORD_ID='{revision_rec_id}' AND SERVICE_ID = '{service_id}' AND BILLING_YEAR = '{BillingYear}' group by BILLING_DATE, BILLING_YEAR".format(cq=str(ContractRecordId),revision_rec_id = quote_revision_record_id,billing_date_column="','".join(billing_date_column),service_id = TreeParam, BillingYear=SubTab))
			if gettotaldateamt:
				my_format = "{:,." + str(decimal_place) + "f}"
			...
			if gettotaldateamt:
				...
				for val in gettotaldateamt:
					...
					elif str(get_billing_types).upper() == "MILESTONE" and TreeParam == "Z0123":
				...
				for string in StringValue_list:
					...
					if RECORD_ID != 'SYOBJR-98869':
						if string == "ACQUIRED" or string == "PRICED":
							...
							string_value = string.replace(string,"<img title='"+str(string).title()+"' src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/Green_Tick.svg> ")
						...
						if string == "ASSEMBLY IS MISSING":
							string_value = string.replace("ASSEMBLY IS MISSING","<img title='Assembly Missing' src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/Orange1_Circle.svg> ASSEMBLY IS MISSING")
						if string == "OFFLINE PRICING":
							string_value = string.replace("OFFLINE PRICING","<img title='OFFLINE PRICING' src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/manual_pricing.svg> OFFLINE PRICING")
						if string == "CFG-ON HOLD - COSTING":
							#Trace.Write("cmng_inside_on_hold_1 "+str(value1234))
							string_value = string.replace("CFG-ON HOLD - COSTING","<img title='CFG-ON HOLD - COSTING' src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/pricing_on_hold.svg> CFG-ON HOLD - COSTING")
						if string ==  "PRR-ON HOLD PRICING":
							string_value = string.replace("PRR-ON HOLD PRICING","<img title='PRR-ON HOLD PRICING' src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/pricing_on_hold.svg>")
						if string ==  "CFG-ON HOLD TKM":
							Trace.Write("Coming inside ON HOLD TKM")
							string_value = string.replace("CFG-ON HOLD TKM","<img title='ON-HOLD-TKM' src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/icons/on_hold_tkm.svg>")                     
						...
						string_value = string
					if (string != "ACQUIRED" and string != "APPROVAL REQUIRED" and string != "ERROR" and string != "ASSEMBLY IS MISSING" and string != "CFG-ON HOLD - COSTING" and string != "PARTIALLY PRICED" and string != "ACQUIRING" and string != "PRICED" and string != "PRR-ON HOLD PRICING") or string != "CFG-ON HOLD TKM":
						Trace.Write("Coming Or Not"+str(string))                    
						string_value = string
		...
		if ObjectName == 'SAQIBP' and TreeParam != 'Quote Items':
			...
			footer_str = '<tfoot><tr><th colspan="32" id= "getbill1year" class="text-left">{}</th>{}</tr><tr></tr></tfoot>'.format("GRAND TOTAL", footer_tot)
		RelatedDrop_str = (
		...
		if (str(getRevision.REVISION_STATUS).upper() in ("CBC-CBC COMPLETED","BOK-CONTRACT CREATED","BOK-CONTRACT BOOKED","LGL-LEGAL SOW ACCEPTED","LGL-LEGAL SOW REJECTED","LGL-PREPARING LEGAL SOW","CBC-PREPARING CBC")) or (str(getRevision.REVISION_STATUS).upper() in ("CFG-ACQUIRING","PRR-ON HOLD PRICING","CFG-ON HOLD - COSTING","PRI-PRICING") and str(TreeParam).upper() not in ("QUOTE ITEMS","BILLING","APPROVALS","QUOTE DOCUMENTS")) or (str(getRevision.REVISION_STATUS).upper() in ("APR-APPROVED","APR-REJECTED","APR-RECALLED","APR-APPROVAL PENDING","OPD-PREPARING QUOTE DOCUMENTS","OPD-CUSTOMER ACCEPTED","APR-SUBMITTED TO CUSTOMER","OPD-CUSTOMER REJECTED") and str(TreeParam).upper() not in ("APPROVALS","QUOTE DOCUMENTS")):
			...
			Nso_Included = "False"
		Trace.Write("pricing_manual_CHKZ_--- "+str(pricing_manual))
		return (
			table_header,
			table_list,
			table_id,
	...
	def MDYNMICSQLOBJECTFILTER(
		...
		if Columns != "" and Obj_Name != "":
			...
			if lookup_str != "":
				...
				select_obj_str = str(obj_str)
			Trace.Write(str(RECORD_ID)+"--RECORD_ID-obj_str---- "+str(obj_str))
			Trace.Write("select_obj_str---- "+str(select_obj_str))
			lookup_disply_list123 = ""
			lookup_str = ",".join(list(lookup_disply_list))
			if len(list(lookup_disply_list)) > 1:
			...
			if RECORD_ID == "SYOBJR-00029" and subTab.upper() =='INCLUSIONS':
				select_obj_str = select_obj_str.replace('NEW_PART','UNIT_PRICE_INGL_CURR,EXTENDED_PRICE_INGL_CURR')
			edit_field = []
			...
			if Qstn_REC_ID != "" and Wh_API_NAME != "":
				...
				if ATTRIBUTE_NAME:					
					...
					if Dict_formation.get(str(xa)) != "":					
						...
						Trace.Write('6005--J_obj-'+str(J_obj))
						...
						if str(quer_values).find(",") == -1:
							...
							if str(picklist_data).upper() == "TRUE":
								if str(quer_values).upper() == "TRUE":
									...
									Trace.Write('6005--6129-----'+str(RECORD_ID))
				...
				if ATTRIBUTE_VALUE_STR != "":
					...
					if RECORD_ID == "SYOBJR-95868":                        
						...
						if Product.GetGlobal("TreeParentLevel2") == "Quote Items":  
							...
							if TreeParam == "Quote Items":
								...
								Qury_str = (
									...
									"select top "
										+ str(PerPage)
										+ " CASE WHEN STATUS = 'ACQUIRED' THEN '"+ imgstr +"' WHEN STATUS = 'CFG-ON HOLD TKM' THEN '"+ on_hold_tkm +"' WHEN STATUS = 'APPROVAL REQUIRED' THEN '" +exclamation+ "'  WHEN STATUS = 'OFFLINE PRICING' THEN '" +offline_pricing+ "' WHEN STATUS = 'CFG-ON HOLD - COSTING' THEN '"+on_hold_costing+"' WHEN STATUS = 'PRR-ON HOLD PRICING' THEN '"+on_hold_pricing+"' WHEN STATUS = 'ERROR' THEN '"+ error +"' WHEN STATUS = 'PARTIALLY PRICED' THEN '"+ partially_priced +"' WHEN STATUS = 'ASSEMBLY IS MISSING' THEN '"+ assembly_missing +"'  ELSE '"+ acquiring_img_str +"' END AS STATUS,QUOTE_ITEM_COVERED_OBJECT_RECORD_ID,LINE,SERVICE_ID,OBJECT_ID,EQUIPMENT_ID,ASSEMBLY_ID,GOT_CODE,CNTYER,EQNODE,PROCES,PM_ID,KIT_ID,KIT_NUMBER,MNTEVT_LEVEL,SAPMMP,CNTDAY,FABLOCATION_ID,GREENBOOK,QUANTITY,TOLCFG,KPU,SPQTEV,SPSPCT,SVSPCT,WTYSTE,WTYEND,WTYDAY,INWRTY,ATGKEC,ATGKEP,NWPTOC,NWPTOP,CONSCP,CONSPI,NONCCI,NONCPI,AIUICI,AIUIPI,AMNCCI,AMNPPI,TNTVGC,TENVGC,ADDCOF,CAVVCI,CAVVPI,BPTKCI,BPTKPI,TRGPRC,SLSPRC,BDVPRC,CELPRC,TGADJP,YOYPCT,USRPRC,BCHPGC,BCHDPT,CNTPRC,ATKNCI,ATKNPI,CpqTableEntryId from ( select  ROW_NUMBER() OVER( ORDER BY LINE) AS ROW, * from SAQICO (NOLOCK) where "+ str(ATTRIBUTE_VALUE_STR)+" QUOTE_ID = '"
										+ str(qt_rec_id.QUOTE_ID)
										+ "' AND QTEREV_RECORD_ID = '"
										+ str(quote_revision_record_id)
										+ "') m where m.ROW BETWEEN "
						...
						if Product.GetGlobal("TreeParentLevel2") == "Quote Items":
							...
							if TreeParam == "Quote Items":
								...
								Qury_str = (
									"select top "
										+ str(PerPage)
										+ " CASE WHEN STATUS = 'ACQUIRED' THEN '"+ imgstr +"' WHEN STATUS = 'CFG-ON HOLD TKM' THEN '"+ on_hold_tkm +"' WHEN STATUS = 'APPROVAL REQUIRED' THEN '" +exclamation+ "' WHEN STATUS = 'CFG-ON HOLD - COSTING' THEN '"+on_hold_costing+"' WHEN STATUS = 'PRR-ON HOLD PRICING' THEN '"+on_hold_pricing+"' WHEN STATUS = 'OFFLINE PRICING' THEN '"+offline_pricing+"' WHEN STATUS = 'ERROR' THEN '"+ error +"'  WHEN STATUS = 'PARTIALLY PRICED' THEN '"+ partially_priced +"' WHEN STATUS = 'ASSEMBLY IS MISSING' THEN '"+ assembly_missing +"'  ELSE '"+ acquiring_img_str +"' END AS STATUS,QUOTE_ITEM_COVERED_OBJECT_RECORD_ID,LINE,SERVICE_ID,OBJECT_ID,EQUIPMENT_ID,ASSEMBLY_ID,GOT_CODE,CNTYER,EQNODE,PROCES,PM_ID,KIT_NUMBER,KIT_ID,MNTEVT_LEVEL,SAPMMP,CNTDAY,FABLOCATION_ID,GREENBOOK,QUANTITY,TOLCFG,KPU,SPQTEV,SPSPCT,SVSPCT,WTYSTE,WTYEND,WTYDAY,INWRTY,ATGKEC,ATGKEP,NWPTOC,NWPTOP,CONSCP,CONSPI,NONCCI,NONCPI,AIUICI,AIUIPI,AMNCCI,AMNPPI,TNTVGC,TENVGC,ADDCOF,CAVVCI,CAVVPI,BPTKCI,BPTKPI,TRGPRC,SLSPRC,BDVPRC,CELPRC,TGADJP,YOYPCT,USRPRC,BCHPGC,BCHDPT,CNTPRC,ATKNCI,ATKNPI,CpqTableEntryId from ( select  ROW_NUMBER() OVER( ORDER BY "+ str(Wh_API_NAMEs)
										+") AS ROW, * from SAQICO (NOLOCK) where "+ str(ATTRIBUTE_VALUE_STR)+" QUOTE_ID = '"
										+ str(qt_rec_id.QUOTE_ID)
										+ "' AND QTEREV_RECORD_ID = '"+str(quote_revision_record_id)+"') m where m.ROW BETWEEN "
										+ str(Page_start)
								...
								Qury_str = (
									"select top "
										+ str(PerPage)
										+ " CASE  WHEN STATUS = 'ACQUIRED' THEN '"+ imgstr +"' WHEN STATUS = 'CFG-ON HOLD TKM' THEN '"+ on_hold_tkm +"' WHEN STATUS = 'APPROVAL REQUIRED' THEN '" +exclamation+ "' WHEN STATUS = 'CFG-ON HOLD - COSTING' THEN '"+on_hold_costing+"' WHEN STATUS = 'PRR-ON HOLD PRICING' THEN '"+on_hold_pricing+"' WHEN STATUS = 'ERROR' THEN '"+ error +"' WHEN STATUS = 'PARTIALLY PRICED' THEN '"+ partially_priced +"' WHEN STATUS = 'OFFLINE PRICING' THEN '"+offline_pricing+"' WHEN STATUS = 'ASSEMBLY IS MISSING' THEN '"+ assembly_missing +"' ELSE '"+ acquiring_img_str +"' END AS STATUS, QUOTE_ITEM_COVERED_OBJECT_RECORD_ID, EQUIPMENT_LINE_ID, SERVICE_ID, EQUIPMENT_ID,LINE_ITEM_ID,YEAR_1, SERIAL_NO,GREENBOOK,FABLOCATION_ID,TECHNOLOGY,KPU, TARGET_PRICE, SALES_DISCOUNT_PRICE, CEILING_PRICE,NET_VALUE,PRICE_BENCHMARK_TYPE,CONTRACT_ID,CONVERT(VARCHAR(10),CONTRACT_VALID_FROM,101) AS [CONTRACT_VALID_FROM],CONVERT(VARCHAR(10),CONTRACT_VALID_TO,101) AS [CONTRACT_VALID_TO],CpqTableEntryId from ( select  ROW_NUMBER() OVER( ORDER BY "+ str(Wh_API_NAMEs)
										+") AS ROW, * from SAQICO (NOLOCK) where  QUOTE_ID = '"
										+ str(qt_rec_id.QUOTE_ID)
										+ "' AND SERVICE_ID = '"
										+ str(LineAndEquipIDList)
						...
						error = '<img title="Error" src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/exclamation_icon.svg>'
						cps_pricing_img = ""                     
						#cps_pricing_img ='<a href="#" onclick=""><img src="/mt/APPLIEDMATERIALS_PRD/Additionalfiles/info.png"></a>' 
						Qury_str = (
							"SELECT DISTINCT TOP "
							+ str(PerPage)
					...
					##involved parties source fab ends
						...
						if TreeSuperParentParam == "Product Offerings":
							...
							service_id = TreeParentParam.split('-')[0]
							Qustr = " where "+str(ATTRIBUTE_VALUE_STR)+" "+ str(Wh_API_NAME) + " = '" + str(RecAttValue) + "' "
							Qustr +=  " AND QTEREV_RECORD_ID = '{}'".format(Quote.GetGlobal("quote_revision_record_id"))
							if subTab == "New Parts":
								Qustr += " AND PAR_SERVICE_ID = '"+str(service_id)+"' AND GREENBOOK = '"+str(TreeParam)+"' AND NEW_PART = 'True'"	
							elif subTab == "Inclusions":
								Qustr += " AND PAR_SERVICE_ID = '"+str(service_id)+"' AND GREENBOOK = '"+str(TreeParam)+"' AND NEW_PART = 0 AND INCLUDED = 1"
					...
					elif str(RECORD_ID) == "SYOBJR-95557":
						Trace.Write("record id 11 95557----"+str(RECORD_ID))
						unique_id = str(equipment_id).split('-')[1].lstrip('0')
						get_bom = Sql.GetFirst("SELECT ASSEMBLY_ID,EQUIPMENT_ID,KIT_ID,KIT_NUMBER,PM_ID,SERVICE_ID FROM SAQSKP where CpqTableEntryId = '"+str(unique_id)+"' ")
						...
						Qustr = " where "+str(ATTRIBUTE_VALUE_STR)+" "+ str(Wh_API_NAME) + " = '" + str(RecAttValue) + "' AND QTEREV_RECORD_ID = '"+str(quote_revision_record_id)+"'"
					elif str(RECORD_ID) == "SYOBJR-98882":
						...
						Trace.Write('8005-----'+str(ATTRIBUTE_VALUE_STR)+ '-'+str(Wh_API_NAME))
						Qustr = " where "+str(ATTRIBUTE_VALUE_STR)+" "+ str(Wh_API_NAME) + " = '" + str(RecAttValue) + "' AND QTEREV_RECORD_ID = '"+str(quote_revision_record_id)+"'"
					...
					try:     
						...
						RecAttValue = ''
					Trace.Write("checking RECORD_ID value---"+str(RECORD_ID))                 
					TreeTopSuperParentParam = Product.GetGlobal("CommonTreeTopSuperParentParam")
					if RECORD_ID == "SYOBJR-95868":                        
						Qury_str = (
							"select top "
						...
						if RECORD_ID == "SYOBJR-93122":
							...
							if Product.GetGlobal("TreeParentLevel2") == "Quote Items":
								...
								if TreeParam == "Quote Items":
									...
									Qury_str = (
										"select top "
											+ str(PerPage)
											+ " CASE WHEN STATUS = 'ACQUIRED' THEN '"+ imgstr +"' WHEN STATUS = 'CFG-ON HOLD TKM' THEN '"+ on_hold_tkm +"' WHEN STATUS = 'APPROVAL REQUIRED' THEN '" +exclamation+ "' WHEN STATUS = 'CFG-ON HOLD - COSTING' THEN '"+on_hold_costing+"' WHEN STATUS = 'PRR-ON HOLD PRICING' THEN '"+on_hold_pricing+"' WHEN STATUS = 'OFFLINE PRICING' THEN '"+offline_pricing+"' WHEN STATUS = 'ERROR' THEN '"+ error +"'  WHEN STATUS = 'PARTIALLY PRICED' THEN '"+ partially_priced +"' WHEN STATUS = 'ASSEMBLY IS MISSING' THEN '"+ assembly_missing +"'  ELSE '"+ acquiring_img_str +"' END AS STATUS, QUOTE_ITEM_COVERED_OBJECT_RECORD_ID,LINE,SERVICE_ID,OBJECT_ID,EQUIPMENT_ID,ASSEMBLY_ID,GOT_CODE,CNTYER,EQNODE,PROCES,PM_ID,KIT_NUMBER,KIT_ID,MNTEVT_LEVEL,SAPMMP,CNTDAY,FABLOCATION_ID,GREENBOOK,QUANTITY,TOLCFG,KPU,SPQTEV,SPSPCT,SVSPCT,WTYSTE,WTYEND,WTYDAY,INWRTY,ATGKEC,ATGKEP,NWPTOC,NWPTOP,CONSCP,CONSPI,NONCCI,NONCPI,AIUICI,AIUIPI,AMNCCI,AMNPPI,TNTVGC,TENVGC,ADDCOF,CAVVCI,CAVVPI,BPTKCI,BPTKPI,TRGPRC,SLSPRC,BDVPRC,CELPRC,TGADJP,YOYPCT,USRPRC,BCHPGC,BCHDPT,CNTPRC,ATKNCI,ATKNPI,CpqTableEntryId from ( select  ROW_NUMBER() OVER( ORDER BY "+ str(wh)
											+") AS ROW, * from SAQICO (NOLOCK) where  QUOTE_ID = '"
											+ str(qt_rec_id.QUOTE_ID)
											+ "' AND QTEREV_RECORD_ID = '"+str(quote_revision_record_id)+"') m where m.ROW BETWEEN "
											+ str(Page_start)
							...
							if "ACQUIRED" in price_status and ('ACQUIRING' not in price_status and 'APPROVAL REQUIRED' not in price_status and 'ERROR' not in price_status):
								...
								icon = '<img title="Acquiring" src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/exclamation_icon.svg'
							elif "APPROVAL REQUIRED" in price_status and ('ACQUIRED' not in price_status and 'ERROR' not in price_status and 'ACQUIRING' not in price_status):
								icon = '<img title="Acquiring" src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/clock_exe.svg>'
							elif "ACQUIRING" in price_status and 'ACQUIRED' not in price_status and 'ERROR' not in price_status and 'APPROVAL REQUIRED' not in price_status:
								icon = '<img title="Acquiring" src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/Cloud_Icon.svg>'
						...
						##involved parties equipmemt ends
						...
						elif str(RECORD_ID) == "SYOBJR-98872" or str(RECORD_ID) == "SYOBJR-00643":
							Qustr = " where " + str(Wh_API_NAME) + " = '" + str(RecAttValue) + "' AND QTEREV_RECORD_ID = '"+str(quote_revision_record_id)+"' "
						elif str(RECORD_ID) == "SYOBJR-95557":
							Trace.Write("record id 22 95557----"+str(RECORD_ID))
							unique_id = str(equipment_id).split('-')[1].lstrip('0')
							get_bom = Sql.GetFirst("SELECT ASSEMBLY_ID,EQUIPMENT_ID,KIT_ID,KIT_NUMBER,PM_ID,SERVICE_ID FROM SAQSKP where CpqTableEntryId = '"+str(unique_id)+"' ")
							...
							Trace.Write("CMG inside NSO")
							Qustr = " where " + str(Wh_API_NAME) + " = '" + str(RecAttValue) + "' AND GREENBOOK = '"+str(TreeParentParam)+"' AND QTEREV_RECORD_ID = '"+str(quote_revision_record_id)+"'"
						else:
							Qustr = " where " + str(Wh_API_NAME) + " = '" + str(RecAttValue) + "' AND QTEREV_RECORD_ID = '"+str(quote_revision_record_id)+"'"
				...
				if str(Qury_str) == "" and str(QuryCount_str) == "":  
					...
					if str(RECORD_ID) == "SYOBJR-00007":
						...
						if Qustr:
							if str(TreeParentParam)== "Billing":
								Qustr += " AND BILLING_YEAR = '{}' AND SERVICE_ID = '{}' AND BILLING_DATE BETWEEN '{}' AND '{}'".format(SubTab,TreeParam,billing_date_column[0], billing_date_column[-1])
							else:
						...
						QuryCount_str = "select count(*) as cnt from " + str(ObjectName) + " (nolock) " + str(Qustr) + " AND  QTEREV_RECORD_ID = '" + str(quote_revision_record_id) + "'"
					else:
						Qury_str = (
							"select top "
							+ str(PerPage)
			...
			for ik in Query_Obj:				                  
				...
				if str(Action_permission.get("Edit")).upper() == "TRUE":
					if ObjectName == "SAQTRV":
						...
						# if  qt_status.REVISION_STATUS in ('CFG-CONFIGURING','CFG-ACQUIRING','PRR-ON HOLD PRICING','PRI-PRICING','APR-APPROVAL PENDING','APR-RECALLED','APR-REJECTED'):
						if  qt_status.REVISION_STATUS in ('CFG-CONFIGURING'):
							Action_str += '<li><a id = "" class="dropdown-item" href="#" " onclick="Repricing_CPS(this)">REPRICE_CPS</a></li>'
					elif ObjectName == "SAQSPT" :
						contract_quote_rec_id = Quote.GetGlobal("contract_quote_record_id")
						quote_revision_rec_id = Quote.GetGlobal("quote_revision_record_id")
						qt_status =Sql.GetFirst("""SELECT REVISION_STATUS FROM SAQTRV WHERE  QUOTE_RECORD_ID = '{ContractRecordId}' AND QTEREV_RECORD_ID = '{quote_revision_record_id}'""".format(ContractRecordId =contract_quote_rec_id,quote_revision_record_id =quote_revision_rec_id))
				...
				for inm in ik:
					...
					if RECORD_ID != 'SYOBJR-98869':
						...
						if value1234 == "PARTIALLY PRICED":
							value1234 = value1234.replace("PARTIALLY PRICED","<img title='Partially Priced' src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/Red1_Circle.svg> PARTIALLY PRICED")
						if value1234 == "OFFLINE PRICING":
							value1234 = value1234.replace("OFFLINE PRICING","<img title='OFFLINE PRICING' src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/manual_pricing.svg> OFFLINE PRICING")
					...
					if str(cur_api_name) is not None and (
						...
						if value1234 is not None:
							if value1234 != "":
								my_format = "{:,." + str(decimal_place) + "f}"
								Trace.Write("my_format--->"+str(my_format))                               
								value1234 = str(my_format.format(round(float(value1234), int(decimal_place))))
								if str(value123) == "ANNBILAMT_INGL_CURR" and str(ObjectName) == "SAQIBP":
									value1234 = value1234
								else:
									...
									if str(value123) != "CUSTOMER_ANNUAL_QUANTITY":                                                
											...
											if value123 in precentage_columns:                                                    
												...
												if value123 in img_list:
													...
													#imgvalue =str(imgValue)+">"
													Trace.Write("9067"+str(RECORD_ID))
				...
				if ObjectName == "SAQIBP" and TreeParam != 'Quote Items':
					...
					if TreeParam == "Z0117":
						...
						gettotaldateamt = Sql.GetList("SELECT BILLING_VALUE_INGL_CURR=SUM(BILLING_VALUE_INGL_CURR),ESTVAL_INGL_CURR = SUM(ESTVAL_INGL_CURR),ANNBILAMT_INGL_CURR = SUM(ANNBILAMT_INGL_CURR),BILLING_DATE FROM SAQIBP WHERE BILLING_DATE in {billing_date_column} and QUOTE_RECORD_ID ='{cq}' AND QTEREV_RECORD_ID='{revision_rec_id}' AND SERVICE_ID = '{service_id}' AND BILLING_YEAR = '{BillingYear}' AND {lineQry} ISNULL(PAR_SERVICE_ID,'')='{par_ser_id}' group by BILLING_DATE,BILLING_YEAR ".format(cq=str(ContractRecordId),revision_rec_id = quote_revision_record_id,billing_date_column=str(tuple(billing_date_column)),service_id = TreeParam,BillingYear=SubTab, par_ser_id=par_ser_id, lineQry = lineQuery))
					if gettotaldateamt:
						my_format = "{:,." + str(decimal_place) + "f}"
						for val in gettotaldateamt: 
							gettotalamt = str(my_format.format(round(float(val.ANNBILAMT_INGL_CURR), int(decimal_place))))  
		...
		if ObjectName == 'SAQICO':
			...
			SAQICO_dbl_clk_function += (    
				...
				+ '").text());  data1 = $element.closest("tr").find("td:'   
				+ str(cls)  
				+ '").text(); localStorage.setItem("multiedit_save_date", data1); reco_id = removeDuplicates(reco); }else{reco_id=$element.closest("tr").find("td:'+ str(cls)  
				+ '").text(); reco_id=reco_id.split(","); localStorage.setItem("multiedit_save_date", reco_id); } localStorage.setItem("multiedit_data_clicked", reco_id); localStorage.setItem("table_id_RL_edit", "'  
				+ str(table_id) 
				+ '");edit_index = $("'+str(table_ids)+'").find("[data-field="+ field +"]").index()+1;localStorage.setItem("edit_index",edit_index); cpq.server.executeScript("SYBLKETRLG", {"TITLE":field, "VALUE":value, "CLICKEDID":"'   
				+ str(table_id) 
	...
	def SPARE_PARTS_ORDERING(self,select_obj_str, Qustr,PerPage, Page_start, Page_End,Wh_API_NAMEs):
		...
		if "CpqTableEntryId" in str(Wh_API_NAMEs) or "QUOTE_RECORD_ID" in str(Wh_API_NAMEs):
			ordered_values = []
			parent_parts = []
			child_parts=Sql.GetList("SELECT "+str(select_obj_str)+",CpqTableEntryId from SAQSPT (nolock) "+str(Qustr)+" AND QTEREV_RECORD_ID = '" + str(quote_revision_record_id) + "'  AND PAR_PART_NUMBER IS NOT NULL AND PAR_PART_NUMBER!='' ")
			for child in child_parts:
				Trace.Write("child--->"+str(child.PAR_PART_NUMBER))
				parent_parts.append(child.PAR_PART_NUMBER)
				parent_part = Sql.GetFirst("SELECT "+str(select_obj_str)+",CpqTableEntryId from SAQSPT (nolock) "+str(Qustr)+" AND QTEREV_RECORD_ID = '" + str(quote_revision_record_id) + "'  AND PART_NUMBER = '"+str(child.PAR_PART_NUMBER)+"' ")
			...
			no_child_count = Sql.GetFirst("select COUNT(*) AS CNT from SAQSPT (nolock) "+str(Qustr)+" AND QTEREV_RECORD_ID = '" + str(quote_revision_record_id) + "' AND (PAR_PART_NUMBER IS NULL OR PAR_PART_NUMBER ='') "+str(notin_condition))
			...
			if no_child_count > 1000:
				...
				while fetch_count < no_child_count: #fetching only 1000 records since we are not able to get more than 1000 records at a time
					parts = Sql.GetList("SELECT "+str(select_obj_str)+",CpqTableEntryId from SAQSPT (nolock) "+str(Qustr)+" AND QTEREV_RECORD_ID = '" + str(quote_revision_record_id) + "'  AND (PAR_PART_NUMBER IS NULL OR PAR_PART_NUMBER ='') "+str(notin_condition)+" ORDER BY CpqTableEntryId ASC OFFSET "+str(fetch_count)+" ROWS FETCH NEXT 1000 ROWS ONLY ")
					fetch_count +=1000
				...
				parts = Sql.GetList("SELECT TOP 1000 "+str(select_obj_str)+",CpqTableEntryId from SAQSPT (nolock) "+str(Qustr)+" AND QTEREV_RECORD_ID = '" + str(quote_revision_record_id) + "' AND (PAR_PART_NUMBER IS NULL OR PAR_PART_NUMBER ='') "+str(notin_condition)+" ORDER BY CpqTableEntryId ASC ")
				ordered_values.extend(parts) #appending parts without having any child parts
			Query_Obj = []
			...
			Query_Obj = Sql.GetList("select top "+ str(PerPage)+ " "+ str(select_obj_str)+ ",CpqTableEntryId from ( select ROW_NUMBER() OVER(order by "+ str(Wh_API_NAMEs)+ ") AS ROW, * from SAQSPT (nolock) "+ str(Qustr)+" AND QTEREV_RECORD_ID = '" + str(quote_revision_record_id) + "') m where m.ROW BETWEEN "+ str(Page_start)+ " and "+ str(Page_End)+ " ")	
