...
try:
    ...
    Temp_Table_Name = 'ERROR_SESSION1'
    ...
    primaryQueryItems = SqlHelper.GetFirst(
        ""
        + str(Parameter1.QUERY_CRITERIA_1)
        + "  MAEQUP_INBOUND set AVAILABILITY_PERCENT = REPLACE(AVAILABILITY_PERCENT,'','',''.'') FROM MAEQUP_INBOUND (NOLOCK) ' "
    )
    ...
    primaryQueryItems = SqlHelper.GetFirst(
        ""
        + str(Parameter1.QUERY_CRITERIA_1)
        + "  MAEQUP_INBOUND set WARRANTYENDDATE = NULL FROM MAEQUP_INBOUND (NOLOCK) where WARRANTYENDDATE = ''00000000'' ' "
    )
    
    primaryQueryItems = SqlHelper.GetFirst(
        ""
        + str(Parameter1.QUERY_CRITERIA_1)
        + "  MAEQUP_INBOUND set WARRANTYSTARTDATE = NULL FROM MAEQUP_INBOUND (NOLOCK) where WARRANTYSTARTDATE = ''00000000'' ' "
    )
    ...
    S=SqlHelper.GetFirst("sp_executesql @T=N'update maequp_inbound set EQUIPMENT_DESC= replace(EQUIPMENT_DESC,''&#92;&#39;'',''''''''  )  ' ")
    S=SqlHelper.GetFirst("sp_executesql @T=N'update maequp_inbound set EQUIPMENT_DESC= replace(EQUIPMENT_DESC,''&#92;&#92;'',''\\''  )  ' ")
    S=SqlHelper.GetFirst("sp_executesql @T=N'update maequp_inbound set EQUIPMENT_DESC= replace(EQUIPMENT_DESC,''&#92;&#34;'',''\"''  )  ' ")
    S=SqlHelper.GetFirst("sp_executesql @T=N'update maequp_inbound set customer_tool_id= replace(customer_tool_id,''&#92;&#39;'',''''''''  )  ' ")
    S=SqlHelper.GetFirst("sp_executesql @T=N'update maequp_inbound set customer_tool_id= replace(customer_tool_id,''&#92;&#92;'',''\\''  )  ' ")
    S=SqlHelper.GetFirst("sp_executesql @T=N'update maequp_inbound set customer_tool_id= replace(customer_tool_id,''&#92;&#34;'',''\"''  )  ' ")
    ...
    S=SqlHelper.GetFirst("sp_executesql @T=N'update maequp_inbound set DEVICENODE= replace(DEVICENODE,''&#39;'',''''''''  )  ' ")
    S=SqlHelper.GetFirst("sp_executesql @T=N'update maequp_inbound set DEVICENODE= replace(DEVICENODE,''&#92;'',''\\''  )  ' ")
    S=SqlHelper.GetFirst("sp_executesql @T=N'update maequp_inbound set DEVICENODE= replace(DEVICENODE,''&#34;'',''\"''  )  ' ")
    
    S=SqlHelper.GetFirst("sp_executesql @T=N'update maequp_inbound set DEVICETYPE= replace(DEVICETYPE,''&#39;'',''''''''  )  ' ")
    S=SqlHelper.GetFirst("sp_executesql @T=N'update maequp_inbound set DEVICETYPE= replace(DEVICETYPE,''&#92;'',''\\''  )  ' ")
    S=SqlHelper.GetFirst("sp_executesql @T=N'update maequp_inbound set DEVICETYPE= replace(DEVICETYPE,''&#34;'',''\"''  )  ' ")
    
    S=SqlHelper.GetFirst("sp_executesql @T=N'update maequp_inbound set SUBSTRATE_SIZE= replace(SUBSTRATE_SIZE,''&#92;&#39;'',''''''''  )  ' ")
    S=SqlHelper.GetFirst("sp_executesql @T=N'update maequp_inbound set SUBSTRATE_SIZE= replace(SUBSTRATE_SIZE,''&#92;&#92;'',''\\''  )  ' ")
    S=SqlHelper.GetFirst("sp_executesql @T=N'update maequp_inbound set SUBSTRATE_SIZE= replace(SUBSTRATE_SIZE,''&#92;&#34;'',''\"''  )  ' ")
    
    S=SqlHelper.GetFirst("sp_executesql @T=N'update maequp_inbound set ADDRESS_1= replace(ADDRESS_1,''&#92;&#39;'',''''''''  )  ' ")
    S=SqlHelper.GetFirst("sp_executesql @T=N'update maequp_inbound set ADDRESS_1= replace(ADDRESS_1,''&#92;&#92;'',''\\''  )  ' ")
    S=SqlHelper.GetFirst("sp_executesql @T=N'update maequp_inbound set ADDRESS_1= replace(ADDRESS_1,''&#92;&#34;'',''\"''  )  ' ")
    ...
    S=SqlHelper.GetFirst("sp_executesql @T=N'update maequp_inbound set GOT_CODE= replace(GOT_CODE,''&#92;&#39;'',''''''''  )  ' ")
    S=SqlHelper.GetFirst("sp_executesql @T=N'update maequp_inbound set GOT_CODE= replace(GOT_CODE,''&#92;&#92;'',''\\''  )  ' ")
    S=SqlHelper.GetFirst("sp_executesql @T=N'update maequp_inbound set GOT_CODE= replace(GOT_CODE,''&#92;&#34;'',''\"''  )  ' ")
    ...
    S=SqlHelper.GetFirst("sp_executesql @T=N'update maequp set TECHNOLOGY= replace(TECHNOLOGY,''&#92;&#39;'',''''''''  )  ' ")
    S=SqlHelper.GetFirst("sp_executesql @T=N'update maequp set TECHNOLOGY= replace(TECHNOLOGY,''&#92;&#92;'',''\\''  )  ' ")
    S=SqlHelper.GetFirst("sp_executesql @T=N'update maequp set TECHNOLOGY= replace(TECHNOLOGY,''&#92;&#34;'',''\"''  )  ' ")
    ...
    # Updating Error sessions in "ERROR_SESSION1" table		

    primaryQueryItems = SqlHelper.GetFirst(
        ""
        + str(Parameter.QUERY_CRITERIA_1)
        + "   ERROR_SESSION1 (SESSION_ID,CpqTableEntryDateModified) SELECT SESSION_ID,getdate() FROM (SELECT DISTINCT MAEQUP_INBOUND.SESSION_ID as SESSION_ID FROM MAEQUP_INBOUND (NOLOCK) WHERE  ISNULL(MAEQUP_INBOUND.PROCESS_STATUS,'''')=''ERROR'' )SUB_ERRLOG  ' "
    )

    # Updating process status as "ERROR" in ERROR_SESSION1 table
    primaryQueryItems = SqlHelper.GetFirst(
        ""
        + str(Parameter1.QUERY_CRITERIA_1)
        + "  MAEQUP_INBOUND SET PROCESS_STATUS=''ERROR'' FROM MAEQUP_INBOUND JOIN ERROR_SESSION1 ON MAEQUP_INBOUND.SESSION_ID = ERROR_SESSION1.SESSION_ID WHERE MAEQUP_INBOUND.TIMESTAMP='"
        + str(timestamp_sessionid)
        + "' '"
    )
    ...
    while Check_flag1 == 1:
        ...
        if str(primaryQueryItems) != "None":
            ...
            primaryQueryItems = SqlHelper.GetFirst(
                                ""
                                + str(Parameter1.QUERY_CRITERIA_1)
                                + " MAEQUP SET VALDRV_WAFERNODE = ''Mature'' FROM MAEQUP_INBOUND(NOLOCK) JOIN TEMP_PROCESS_EQUP(NOLOCK) ON MAEQUP_INBOUND.EQUIPMENT_ID = TEMP_PROCESS_EQUP.SESSION_ID  JOIN MAEQUP (NOLOCK) ON MAEQUP_INBOUND.EQUIPMENT_ID = MAEQUP.EQUIPMENT_ID WHERE MAEQUP_INBOUND.TIMESTAMP='"+ str(timestamp_sessionid)+ "'  AND ISNULL(MAEQUP_INBOUND.INTEGRATION_STATUS,'''') ='''' AND ISNULL(MAEQUP_INBOUND.PROCESS_STATUS,'''')=''READY FOR UPLOAD'' AND ISNULL(TEMP_PROCESS_EQUP.PROCESS_STATUS,'''')='''' AND MAEQUP.GENERATION IN (''GEN 8.5'', ''GEN 8.5S'', ''GEN 8.5H'', ''GEN 8.6'', ''GEN 8.7'', ''GEN 10'', ''GEN 10.5'', ''GEN 11'') AND MAEQUP.TECHNOLOGY IN (''SOLAR'', ''ARRAY'', ''A-SI'', ''LTPS'', ''METAL OXIDE'', ''NEW ARISTO'', ''SPUTTERING'') AND END_PRODUCT_TECHNOLOGY IN (''LCD'') AND MAEQUP.GREENBOOK=''DISPLAY'' '" )
            ...
            primaryQueryItems = SqlHelper.GetFirst(
                                ""
                                + str(Parameter1.QUERY_CRITERIA_1)
                                + " MAEQUP SET VALDRV_WAFERNODE = ''Legacy'' FROM MAEQUP_INBOUND(NOLOCK) JOIN TEMP_PROCESS_EQUP(NOLOCK) ON MAEQUP_INBOUND.EQUIPMENT_ID = TEMP_PROCESS_EQUP.SESSION_ID  JOIN MAEQUP (NOLOCK) ON MAEQUP_INBOUND.EQUIPMENT_ID = MAEQUP.EQUIPMENT_ID WHERE MAEQUP_INBOUND.TIMESTAMP='"+ str(timestamp_sessionid)+ "'  AND ISNULL(MAEQUP_INBOUND.INTEGRATION_STATUS,'''') ='''' AND ISNULL(MAEQUP_INBOUND.PROCESS_STATUS,'''')=''READY FOR UPLOAD'' AND ISNULL(TEMP_PROCESS_EQUP.PROCESS_STATUS,'''')='''' AND ISNULL(MAEQUP.SUBSTRATE_SIZE_GROUP,'''') = ''<= 200 MM''  AND MAEQUP.GREENBOOK NOT IN (''DISPLAY'',''PPC'') '" )

            primaryQueryItems = SqlHelper.GetFirst(
                                ""
                                + str(Parameter1.QUERY_CRITERIA_1)
                                + " MAEQUP SET VALDRV_WAFERNODE = ''Legacy'' FROM MAEQUP_INBOUND(NOLOCK) JOIN TEMP_PROCESS_EQUP(NOLOCK) ON MAEQUP_INBOUND.EQUIPMENT_ID = TEMP_PROCESS_EQUP.SESSION_ID  JOIN MAEQUP (NOLOCK) ON MAEQUP_INBOUND.EQUIPMENT_ID = MAEQUP.EQUIPMENT_ID WHERE MAEQUP_INBOUND.TIMESTAMP='"+ str(timestamp_sessionid)+ "'  AND ISNULL(MAEQUP_INBOUND.INTEGRATION_STATUS,'''') ='''' AND ISNULL(MAEQUP_INBOUND.PROCESS_STATUS,'''')=''READY FOR UPLOAD'' AND ISNULL(TEMP_PROCESS_EQUP.PROCESS_STATUS,'''')='''' AND ISNULL(MAEQUP.SUBSTRATE_SIZE_GROUP,'''') = ''300 mm'' AND DEVICE_NODE IN( ''NOR'',''EQUIP/MAT''''L'',''MPS'',''PACKAGING'',''WFR MFG'',''G&W'',''SOLAR'',''PCM'',''Other'')  AND MAEQUP.GREENBOOK NOT IN (''DISPLAY'',''PPC'') '" )

            primaryQueryItems = SqlHelper.GetFirst(
                                ""
                                + str(Parameter1.QUERY_CRITERIA_1)
                                + " MAEQUP SET VALDRV_WAFERNODE = ''Mature'' FROM MAEQUP_INBOUND(NOLOCK) JOIN TEMP_PROCESS_EQUP(NOLOCK) ON MAEQUP_INBOUND.EQUIPMENT_ID = TEMP_PROCESS_EQUP.SESSION_ID  JOIN MAEQUP (NOLOCK) ON MAEQUP_INBOUND.EQUIPMENT_ID = MAEQUP.EQUIPMENT_ID WHERE MAEQUP_INBOUND.TIMESTAMP='"+ str(timestamp_sessionid)+ "'  AND ISNULL(MAEQUP_INBOUND.INTEGRATION_STATUS,'''') ='''' AND ISNULL(MAEQUP_INBOUND.PROCESS_STATUS,'''')=''READY FOR UPLOAD'' AND ISNULL(TEMP_PROCESS_EQUP.PROCESS_STATUS,'''')='''' AND ISNULL(MAEQUP.SUBSTRATE_SIZE_GROUP,'''') IN (''300 mm'') AND DEVICE_NODE IN( ''2X'',''2Y'',''2Z'',''3D 48P'',''3D 64P'',''3D 96P'')  AND MAEQUP.GREENBOOK NOT IN (''DISPLAY'',''PPC'') '" )

            primaryQueryItems = SqlHelper.GetFirst(
                                ""
                                + str(Parameter1.QUERY_CRITERIA_1)
                                + " MAEQUP SET VALDRV_WAFERNODE = ''Advanced'' FROM MAEQUP_INBOUND(NOLOCK) JOIN TEMP_PROCESS_EQUP(NOLOCK) ON MAEQUP_INBOUND.EQUIPMENT_ID = TEMP_PROCESS_EQUP.SESSION_ID  JOIN MAEQUP (NOLOCK) ON MAEQUP_INBOUND.EQUIPMENT_ID = MAEQUP.EQUIPMENT_ID WHERE MAEQUP_INBOUND.TIMESTAMP='"+ str(timestamp_sessionid)+ "'  AND ISNULL(MAEQUP_INBOUND.INTEGRATION_STATUS,'''') ='''' AND ISNULL(MAEQUP_INBOUND.PROCESS_STATUS,'''')=''READY FOR UPLOAD'' AND ISNULL(TEMP_PROCESS_EQUP.PROCESS_STATUS,'''')='''' AND ISNULL(MAEQUP.SUBSTRATE_SIZE_GROUP,'''') IN (''300 mm'') AND DEVICE_NODE IN( ''1A'',''1X'',''1Y'',''1Z'',''3D 96P'',''XPOINT'',''F1X '')  AND MAEQUP.GREENBOOK NOT IN (''DISPLAY'',''PPC'') '" )

            primaryQueryItems = SqlHelper.GetFirst(
                                ""
                                + str(Parameter1.QUERY_CRITERIA_1)
                                + " MAEQUP SET VALDRV_WAFERNODE = ''Legacy'' FROM MAEQUP_INBOUND(NOLOCK) JOIN TEMP_PROCESS_EQUP(NOLOCK) ON MAEQUP_INBOUND.EQUIPMENT_ID = TEMP_PROCESS_EQUP.SESSION_ID  JOIN MAEQUP (NOLOCK) ON MAEQUP_INBOUND.EQUIPMENT_ID = MAEQUP.EQUIPMENT_ID WHERE MAEQUP_INBOUND.TIMESTAMP='"+ str(timestamp_sessionid)+ "'  AND ISNULL(MAEQUP_INBOUND.INTEGRATION_STATUS,'''') ='''' AND ISNULL(MAEQUP_INBOUND.PROCESS_STATUS,'''')=''READY FOR UPLOAD'' AND ISNULL(TEMP_PROCESS_EQUP.PROCESS_STATUS,'''')='''' AND ISNULL(MAEQUP.SUBSTRATE_SIZE_GROUP,'''') = ''300 mm'' AND MAEQUP.GREENBOOK NOT IN (''DISPLAY'',''PPC'') AND DEVICE_NODE LIKE ''%NM'' AND DEVICE_NODE NOT LIKE ''%XNM'' AND DEVICE_NODE NOT LIKE ''%-%''  AND  REPLACE(REPLACE(REPLACE(device_node,SUBSTRING(device_node,PATINDEX(''%[^0-9]%'',device_node),1),''''),''NM'',''''),''M'','''')  >=28  '" )

            primaryQueryItems = SqlHelper.GetFirst(
                                            ""
                                            + str(Parameter1.QUERY_CRITERIA_1)
                                            + " MAEQUP SET VALDRV_WAFERNODE = ''Legacy'' FROM MAEQUP_INBOUND(NOLOCK) JOIN TEMP_PROCESS_EQUP(NOLOCK) ON MAEQUP_INBOUND.EQUIPMENT_ID = TEMP_PROCESS_EQUP.SESSION_ID  JOIN MAEQUP (NOLOCK) ON MAEQUP_INBOUND.EQUIPMENT_ID = MAEQUP.EQUIPMENT_ID WHERE MAEQUP_INBOUND.TIMESTAMP='"+ str(timestamp_sessionid)+ "'  AND ISNULL(MAEQUP_INBOUND.INTEGRATION_STATUS,'''') ='''' AND ISNULL(MAEQUP_INBOUND.PROCESS_STATUS,'''')=''READY FOR UPLOAD'' AND ISNULL(TEMP_PROCESS_EQUP.PROCESS_STATUS,'''')='''' AND ISNULL(MAEQUP.SUBSTRATE_SIZE_GROUP,'''') = ''300 mm'' AND MAEQUP.GREENBOOK NOT IN (''DISPLAY'',''PPC'') AND DEVICE_NODE LIKE ''%X'' AND  REPLACE(REPLACE(device_node,SUBSTRING(device_node,PATINDEX(''%[^0-9]%'',device_node),1),''''),''X'','''')  >=4  '" )

            primaryQueryItems = SqlHelper.GetFirst(
                                            ""
                                            + str(Parameter1.QUERY_CRITERIA_1)
                                            + " MAEQUP SET VALDRV_WAFERNODE = ''Mature'' FROM MAEQUP_INBOUND(NOLOCK) JOIN TEMP_PROCESS_EQUP(NOLOCK) ON MAEQUP_INBOUND.EQUIPMENT_ID = TEMP_PROCESS_EQUP.SESSION_ID  JOIN MAEQUP (NOLOCK) ON MAEQUP_INBOUND.EQUIPMENT_ID = MAEQUP.EQUIPMENT_ID WHERE MAEQUP_INBOUND.TIMESTAMP='"+ str(timestamp_sessionid)+ "'  AND ISNULL(MAEQUP_INBOUND.INTEGRATION_STATUS,'''') ='''' AND ISNULL(MAEQUP_INBOUND.PROCESS_STATUS,'''')=''READY FOR UPLOAD'' AND ISNULL(TEMP_PROCESS_EQUP.PROCESS_STATUS,'''')='''' AND ISNULL(MAEQUP.SUBSTRATE_SIZE_GROUP,'''') IN (''300 mm'') AND MAEQUP.GREENBOOK NOT IN (''DISPLAY'',''PPC'') AND DEVICE_NODE LIKE ''%NM'' AND DEVICE_NODE NOT LIKE ''%XNM'' AND DEVICE_NODE NOT LIKE ''%-%'' AND  REPLACE(REPLACE(REPLACE(device_node,SUBSTRING(device_node,PATINDEX(''%[^0-9]%'',device_node),1),''''),''NM'',''''),''M'','''')  BETWEEN 11 AND 27 '" )
                                            
            primaryQueryItems = SqlHelper.GetFirst(
                                            ""
                                            + str(Parameter1.QUERY_CRITERIA_1)
                                            + " MAEQUP SET VALDRV_WAFERNODE = ''Advanced'' FROM MAEQUP_INBOUND(NOLOCK) JOIN TEMP_PROCESS_EQUP(NOLOCK) ON MAEQUP_INBOUND.EQUIPMENT_ID = TEMP_PROCESS_EQUP.SESSION_ID  JOIN MAEQUP (NOLOCK) ON MAEQUP_INBOUND.EQUIPMENT_ID = MAEQUP.EQUIPMENT_ID WHERE MAEQUP_INBOUND.TIMESTAMP='"+ str(timestamp_sessionid)+ "'  AND ISNULL(MAEQUP_INBOUND.INTEGRATION_STATUS,'''') ='''' AND ISNULL(MAEQUP_INBOUND.PROCESS_STATUS,'''')=''READY FOR UPLOAD'' AND ISNULL(TEMP_PROCESS_EQUP.PROCESS_STATUS,'''')='''' AND ISNULL(MAEQUP.SUBSTRATE_SIZE_GROUP,'''') IN (''300 mm'') AND MAEQUP.GREENBOOK NOT IN (''DISPLAY'',''PPC'') AND DEVICE_NODE LIKE ''%3D %'' AND REPLACE(REPLACE(REPLACE(device_node,''3D '',''''),SUBSTRING(REPLACE(device_node,''3D '',''''),PATINDEX(''%[^0-9]%'',REPLACE(device_node,''3D '','''')),10),''''),''P'','''') >96 '" )
    ...
    primaryQueryItems = SqlHelper.GetFirst(
        ""
        + str(Parameter1.QUERY_CRITERIA_1)
        + "   B SET SALESREP_ID = A.SALES_REP FROM MAEQUP_INBOUND (NOLOCK) A JOIN MAEQUP (NOLOCK) B ON A.EQUIPMENT_ID = B.EQUIPMENT_ID WHERE  TIMESTAMP='"+ str(timestamp_sessionid)+ "'  AND ISNULL(INTEGRATION_STATUS,'''') ='''' AND ISNULL(PROCESS_STATUS,'''')=''READY FOR UPLOAD'' '"	)
    
    S = SqlHelper.GetFirst("sp_executesql @T=N'update maequp set got_code=''OTHER'' where isnull(equipmenttype_id,'''')<>'''' and isnull(got_code,'''')='''' '")
    
    primaryQueryItems = SqlHelper.GetFirst(
        ""
        + str(Parameter1.QUERY_CRITERIA_1)
        + "   B SET PAR_EQUIPMENT_ID = A.PAR_EQUIPMENT_ID,PAR_EQUIPMENT_DESCRIPTION=NULL,PAR_EQUIPMENT_RECORD_ID=NULL FROM MAEQUP_INBOUND (NOLOCK) A JOIN MAEQUP (NOLOCK) B ON A.EQUIPMENT_ID = B.EQUIPMENT_ID   '"	)
        
    primaryQueryItems = SqlHelper.GetFirst(
            ""
            + str(Parameter1.QUERY_CRITERIA_1)
            + "   A SET PAR_EQUIPMENT_DESCRIPTION = B.EQUIPMENT_DESCRIPTION,PAR_EQUIPMENT_RECORD_ID = B.EQUIPMENT_RECORD_ID FROM MAEQUP (NOLOCK) A JOIN MAEQUP (NOLOCK) B ON A.PAR_EQUIPMENT_ID = B.EQUIPMENT_ID JOIN MAEQUP_INBOUND (NOLOCK)C ON A.EQUIPMENT_ID = C.EQUIPMENT_ID  WHERE ISNULL(A.PAR_EQUIPMENT_ID,'''') <> '''' '"	)
    
    primaryQueryItems = SqlHelper.GetFirst(
        ""
        + str(Parameter.QUERY_CRITERIA_1)
        + " SAGTCD(GOT_CODE,GOT_CODE_DESCRIPTION,GOT_CODES_RECORD_ID)SELECT A.GOT_CODE,A.GOT_CODE,CONVERT(VARCHAR(100),NEWID()) FROM (SELECT DISTINCT GOT_CODE FROM MAEQUP(NOLOCK) WHERE ISNULL(GOT_CODE,'''')<>'''' )A LEFT JOIN SAGTCD(NOLOCK) ON A.GOT_CODE = SAGTCD.GOT_CODE WHERE SAGTCD.GOT_CODE IS NULL '"	)
    ...
    while Email_Check == 1:
        Error_primaryQuery = SqlHelper.GetList(
            "select top 10 SESSION_ID from ERROR_SESSION1(NOLOCK) where ISNULL(PROCESS_STATUS,'')= '' and isnull(SESSION_ID,'')<>''  "
        )
        ...
        if len(Session_lst_count) > 0:
            ...
            ErrorQueryItems = SqlHelper.GetFirst(""+ str(Parameter1.QUERY_CRITERIA_1)+ "  ERROR_SESSION1 SET PROCESS_STATUS = ''UPLOADED'',CpqTableEntryDateModified = GetDate() WHERE SESSION_ID  IN "+ str(Session_lst).replace("'","''")+ "  '")
    ...
    if len(Error_list) > 0:
        ...
        msg.Subject = "EQUIPMENT EXCEPTIONS - AMAT CPQ(P-Tenant)"
        msg.IsBodyHtml = True
        msg.Body = Error_Info
