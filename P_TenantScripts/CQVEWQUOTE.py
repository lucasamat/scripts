...
class RedirectQuote:
    ...
    def view_quote(self, ComposId):        
        if ComposId !='':
            ...
            #Trace.Write("SELECT USER_NAME AS Username,Password,Domain FROM SYCONF (NOLOCK) where Domain='AMAT_PRD'")
            LOGIN_CREDENTIALS = Sql.GetFirst(
                "SELECT USER_NAME AS Username,Password,Domain FROM SYCONF (NOLOCK) where Domain='AMAT_PRD'"
            )
        ...
        record_obj = Sql.GetList(
            """SELECT MOD.* FROM (
                SELECT TOP 100  MM.APP_LABEL, P.PRODUCT_ID, MM.APP_RECORD_ID, MM.DISPLAY_ORDER,
                 ROW_NUMBER() OVER(PARTITION BY MM.APP_LABEL ORDER BY MM.APP_LABEL) AS RANK
