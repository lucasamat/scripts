...
def quote_items_pricing(Qt_id, service_ids=[]):
        ...
        Sql.RunQuery("""UPDATE SAQRIT 
                        ...
                        FROM SAQRIT
                            INNER JOIN (SELECT SAQICO.QUOTE_RECORD_ID, SAQICO.QTEREV_RECORD_ID,SAQICO.SERVICE_ID,SAQICO.LINE,
                                    ...
                                    FROM SAQICO WHERE SAQICO.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQICO.QTEREV_RECORD_ID = '{rev}'  GROUP BY SAQICO.QUOTE_RECORD_ID, SAQICO.QTEREV_RECORD_ID,SAQICO.SERVICE_ID, SAQICO.LINE
    ...
    Sql.RunQuery("""UPDATE SAQTRV
                        ...
                        FROM SAQTRV (NOLOCK)
                        INNER JOIN (SELECT SAQICO.QUOTE_RECORD_ID, SAQICO.QTEREV_RECORD_ID,
                                    (CASE WHEN BILTYP = 'Variable' AND SUM(ISNULL(SAQICO.TENVGC, 1)) <> 0 THEN SUM(ISNULL(SAQICO.TENVGC, 1)) WHEN BILTYP = 'Variable' AND SUM(ISNULL(SAQICO.TENVGC, 1)) = 0 THEN 1 WHEN BILTYP <> 'Variable' AND SUM(ISNULL(SAQICO.TNTVGC, 1)) = 0 THEN 1 ELSE  SUM(ISNULL(SAQICO.TNTVGC, 1))   END ) - SUM(ISNULL(SAQICO.CNTCST, 0)) as CNTMRG_INGL_CURR,
                                    (SUM(ISNULL(SAQICO.CNTMGN, 0)) / (CASE WHEN BILTYP = 'Variable' AND SUM(ISNULL(SAQICO.TENVGC, 1)) <> 0 THEN SUM(ISNULL(SAQICO.TENVGC, 1)) WHEN BILTYP = 'Variable' AND SUM(ISNULL(SAQICO.TENVGC, 1)) = 0 THEN 1 WHEN BILTYP <> 'Variable' AND SUM(ISNULL(SAQICO.TNTVGC, 1)) = 0 THEN 1 ELSE  SUM(ISNULL(SAQICO.TNTVGC, 1))   END )) *100 as TOTAL_MARGIN_PERCENT
                                    FROM SAQICO (NOLOCK) WHERE SAQICO.QUOTE_RECORD_ID = '{quote_rec_id}' AND SAQICO.QTEREV_RECORD_ID = '{quote_revision_rec_id}' AND SERVICE_ID !='Z0117' GROUP BY SAQICO.QTEREV_RECORD_ID, SAQICO.QUOTE_RECORD_ID,BILTYP) IQ ON SAQTRV.QUOTE_RECORD_ID = IQ.QUOTE_RECORD_ID AND SAQTRV.QUOTE_REVISION_RECORD_ID = IQ.QTEREV_RECORD_ID
                        WHERE SAQTRV.QUOTE_RECORD_ID = '{quote_rec_id}' AND SAQTRV.QUOTE_REVISION_RECORD_ID = '{quote_revision_rec_id}' 	""".format(quote_rec_id = contract_quote_record_id ,quote_revision_rec_id = contract_quote_revision_record_id ) )
