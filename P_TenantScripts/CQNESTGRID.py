...
def GetEquipmentMaster(PerPage, PageInform, A_Keys, A_Values):
    ...
    if ibase_missing_flag:
        missing_text = "MISSING ATTRIBUTES FROM IBASE | You have tools in the quote with missing iBase attributes relevant for pricing. Please remove the impacted tools in the fab location node and work with the FSO to correct the iBase. Once FSO has completed the iBase, you may re-add the tools and proceed."
        filter_control_function += ('$(".cpq_notification_div .cpq_cust_warning").html("<img src=\'/mt/APPLIEDMATERIALS_PRD/Additionalfiles/warning1.svg\' class=\'wid20_px\'><p>'+str(missing_text)+'</p>").css("display","block").parent().css("display","block").parent().prev().children("div").removeClass("collapsed");$("#notify").addClass("in");$("#notify").css("height","auto");')
    else:
...
def GetEventsMaster(PerPage, PageInform, A_Keys, A_Values):
    ...
    Action_Str += str(Page_End)
    Action_Str += " of"
    if (str(isBookedQuote.REVISION_STATUS).upper() in ("CBC-CBC COMPLETED","BOK-CONTRACT CREATED","BOK-CONTRACT BOOKED","LGL-LEGAL SOW ACCEPTED","LGL-LEGAL SOW REJECTED","LGL-PREPARING LEGAL SOW","CBC-PREPARING CBC")) or (str(isBookedQuote.REVISION_STATUS).upper() in ("CFG-ACQUIRING","PRR-ON HOLD PRICING","CFG-ON HOLD - COSTING","PRI-PRICING") and str(TreeParam).upper() not in ("QUOTE ITEMS","BILLING","APPROVALS","QUOTE DOCUMENTS")) or (str(isBookedQuote.REVISION_STATUS).upper() in ("APR-APPROVED","APR-REJECTED","APR-RECALLED","APR-APPROVAL PENDING","OPD-PREPARING QUOTE DOCUMENTS","OPD-CUSTOMER ACCEPTED","APR-SUBMITTED TO CUSTOMER","OPD-CUSTOMER REJECTED") and str(TreeParam).upper() not in ("APPROVALS","QUOTE DOCUMENTS")):
        Nso_Included = "True"
    else:
        Nso_Included = "False"
    return (
    ...
    if ATTRIBUTE_VALUE is None or ATTRIBUTE_VALUE == "" or ATTRIBUTE_VALUE_STR is None or ATTRIBUTE_VALUE_STR == "":
        ...
        if TreeParam == 'Fab Locations' or TreeParam == 'Customer Information':
            ...
            if (TreeParam.startswith("Sending") or TreeParam.startswith("Receiving")):
                child_obj_recid = Sql.GetList(
                    "select top "+str(PerPage)+" QUOTE_FAB_LOC_COV_OBJ_ASSEMBLY_RECORD_ID,EQUIPMENT_ID,SERIAL_NUMBER,ASSEMBLY_ID,ASSEMBLY_DESCRIPTION,GOT_CODE,INCLUDED,MNT_PLANT_ID,FABLOCATION_ID,WARRANTY_START_DATE,EQUIPMENTCATEGORY_ID,WARRANTY_END_DATE,SALESORG_ID,EQUIPMENTTYPE_ID AS ASSEMBLYTYPE_ID from SAQFEA (NOLOCK) where EQUIPMENT_ID = '"+ str(recid)+ "' and QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' ORDER BY  {ORDER_BY}".format(ContractRecordId=Quote.GetGlobal("contract_quote_record_id"),RevisionRecordId = Quote.GetGlobal("quote_revision_record_id"), ORDER_BY = orderby))
                Count = Sql.GetFirst(
...
def QuoteAssemblyPreventiveMaintainenceParent(PerPage, PageInform, A_Keys, A_Values,ASSEMBLYID,EQUIPMENTID):
    ...
    if TopSuperParentParam in ('Comprehensive Services','Complementary Products'): 
        ...
        dbl_clk_function += (
            ...
            + '").on("check-all.bs.table", function (e) { var table = $("'
            + str(table_ids)
            + '").closest("table"); table.find("tbody tr").each(function() { checkedRows.push($(this).find("td:'+str(cls)+'").text()); }); localStorage.setItem("multiedit_checkbox_clicked", checkedRows); }); $("'
            + str(table_ids)
            + '").on("uncheck-all.bs.table", function (e) { localStorage.setItem("multiedit_checkbox_clicked", []); checkedRows=[]; }); $("'
            + str(table_ids)
            + '").on("uncheck.bs.table", function (e, row, $element) { var rec_ids=$element.closest("tr").find("td:'
...
def GetCovObjMaster(PerPage, PageInform, A_Keys, A_Values):    
    ...
    if TreeSuperParentParam == "Product Offerings" or (TreeTopSuperParentParam== "Comprehensive Services" and TreeParentParam=="Add-On Products"):
        Qstr = (
            "select top "
            + str(PerPage)
            + " * from ( select ROW_NUMBER() OVER( ORDER BY "+str(orderby)+") AS ROW, QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID,EQUIPMENT_ID,EQUIPMENT_DESCRIPTION,PLATFORM,SERIAL_NO,GREENBOOK,FABLOCATION_ID,WARRANTY_END_DATE,WARRANTY_END_DATE_ALERT,WARRANTY_START_DATE,CONTRACT_VALID_FROM,CONTRACT_VALID_TO,MNT_PLANT_ID,EQUIPMENT_STATUS,CUSTOMER_TOOL_ID,EQUIPMENTCATEGORY_DESCRIPTION AS EQUIPMENTCATEGORY_DESCRIPTION,SNDFBL_ID,TEMP_TOOL,IBASE_ATTSTS from SAQSCO (NOLOCK) where QUOTE_RECORD_ID = '"
            + str(ContractRecordId)
            + "' and QTEREV_RECORD_ID = '"
            + str(RevisionRecordId)
            + "' and SERVICE_ID = '"
...
def GetCovObjMasterFilter(ATTRIBUTE_NAME, ATTRIBUTE_VALUE,PerPage,PageInform):
    ...
    if ATTRIBUTE_VALUE is None or ATTRIBUTE_VALUE == "" or ATTRIBUTE_VALUE_STR is None or ATTRIBUTE_VALUE_STR == "":
        ...
        if TreeSuperParentParam == "Product Offerings" or TreeParentParam == "Add-On Products":
            ...
            if TreeTopSuperParentParam == "Product Offerings":
                if TreeParam == "Receiving Equipment":
                    parent_obj = Sql.GetList(
                        "SELECT top "+str(PerPage)+" QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID,EQUIPMENTCATEGORY_DESCRIPTION,EQUIPMENT_ID,EQUIPMENT_DESCRIPTION,SERIAL_NO,GREENBOOK,FABLOCATION_ID, WARRANTY_END_DATE,WARRANTY_START_DATE,MNT_PLANT_ID,EQUIPMENT_STATUS,CUSTOMER_TOOL_ID,EQUIPMENTCATEGORY_ID AS DESCRIPTION,WARRANTY_END_DATE_ALERT,TEMP_TOOL ,PLATFORM,CONTRACT_VALID_FROM,CONTRACT_VALID_TO,IBASE_ATTSTS from SAQSCO (NOLOCK) where  QUOTE_RECORD_ID = '"
                        + str(ContractRecordId)
                        + "' and QTEREV_RECORD_ID = '"
                        + str(RevisionRecordId)
                        + "' AND SERVICE_ID = '"
                    ...
                    parent_obj = Sql.GetList(
                        "SELECT TOP "+str(PerPage)+" QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID,EQUIPMENTCATEGORY_DESCRIPTION,EQUIPMENT_ID,EQUIPMENT_DESCRIPTION,SERIAL_NO,GREENBOOK,FABLOCATION_ID, WARRANTY_END_DATE,WARRANTY_START_DATE,MNT_PLANT_ID,EQUIPMENT_STATUS,CUSTOMER_TOOL_ID,"+str(equipment_column)+",WARRANTY_END_DATE_ALERT,TEMP_TOOL,PLATFORM,CONTRACT_VALID_FROM,CONTRACT_VALID_TO,IBASE_ATTSTS from SAQSCO (NOLOCK) where QUOTE_RECORD_ID = '"
                        + str(ContractRecordId)
                        + "' and QTEREV_RECORD_ID = '"
                        + str(RevisionRecordId)
                        + "'AND SERVICE_ID = '"
                ...
                if TreeParam == "Receiving Equipment":
                    parent_obj = Sql.GetList(
                        "SELECT top "+str(PerPage)+" QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID,EQUIPMENTCATEGORY_DESCRIPTION,EQUIPMENT_ID,EQUIPMENT_DESCRIPTION,SERIAL_NO,GREENBOOK,FABLOCATION_ID, WARRANTY_END_DATE,WARRANTY_START_DATE,MNT_PLANT_ID,EQUIPMENT_STATUS,CUSTOMER_TOOL_ID,"+str(equipment_column)+",WARRANTY_END_DATE_ALERT,TEMP_TOOL,PLATFORM,CONTRACT_VALID_FROM,CONTRACT_VALID_TO,IBASE_ATTSTS from SAQSCO (NOLOCK) where "+ str(ATTRIBUTE_VALUE_STR)+ " 1=1 and QUOTE_RECORD_ID = '"+ str(ContractRecordId)+ "' and QTEREV_RECORD_ID = '"+ str(RevisionRecordId)+ "' AND SERVICE_TYPE = '"+ str(TreeTopSuperParentParam)+ "' AND SERVICE_ID ='"+ str(TreeSuperParentParam)+ "' ORDER BY "  + str(orderby))
                    ...
                    parent_obj = Sql.GetList(
                        "SELECT top "+str(PerPage)+" QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID,EQUIPMENTCATEGORY_DESCRIPTION,EQUIPMENT_ID,EQUIPMENT_DESCRIPTION,SERIAL_NO,GREENBOOK,FABLOCATION_ID, WARRANTY_END_DATE,WARRANTY_START_DATE,MNT_PLANT_ID,EQUIPMENT_STATUS,CUSTOMER_TOOL_ID,"+str(equipment_column)+",WARRANTY_END_DATE_ALERT,TEMP_TOOL,PLATFORM,CONTRACT_VALID_FROM,CONTRACT_VALID_TO,IBASE_ATTSTS from SAQSCO (NOLOCK) where "
                        + str(ATTRIBUTE_VALUE_STR)
                        + " 1=1 and QUOTE_RECORD_ID = '"
                        + str(ContractRecordId)
        ...
        if TreeSuperParentParam == "Product Offerings" or TreeParentParam == "Add-On Products":
            parent_obj = Sql.GetList(
                "SELECT top "+str(PerPage)+" QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID,EQUIPMENTCATEGORY_DESCRIPTION,EQUIPMENT_ID,EQUIPMENT_DESCRIPTION,SERIAL_NO,GREENBOOK,FABLOCATION_ID, WARRANTY_END_DATE,WARRANTY_START_DATE,MNT_PLANT_ID,EQUIPMENT_STATUS,CUSTOMER_TOOL_ID,EQUIPMENTCATEGORY_ID,EQUIPMENTCATEGORY_ID AS DESCRIPTION,SNDFBL_ID,WARRANTY_END_DATE_ALERT,TEMP_TOOL,PLATFORM,CONTRACT_VALID_FROM,CONTRACT_VALID_TO,IBASE_ATTSTS from SAQSCO (NOLOCK) where "
                + str(ATTRIBUTE_VALUE_STR)
                + " 1=1 and QUOTE_RECORD_ID = '"
                + str(ContractRecordId)
                + "' and QTEREV_RECORD_ID = '"
            ...
            if TreeTopSuperParentParam == "Product Offerings":
                if TreeParam == "Receiving Equipment":
                    parent_obj = Sql.GetList(
                        "SELECT top "+str(PerPage)+" QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID,EQUIPMENTCATEGORY_DESCRIPTION,EQUIPMENT_ID,EQUIPMENT_DESCRIPTION,SERIAL_NO,GREENBOOK,FABLOCATION_ID, WARRANTY_END_DATE,WARRANTY_START_DATE,MNT_PLANT_ID,EQUIPMENT_STATUS,CUSTOMER_TOOL_ID,EQUIPMENTCATEGORY_ID AS DESCRIPTION,WARRANTY_END_DATE_ALERT,TEMP_TOOL,PLATFORM,CONTRACT_VALID_FROM,CONTRACT_VALID_TO,IBASE_ATTSTS from SAQSCO (NOLOCK) where "
                        + str(ATTRIBUTE_VALUE_STR)
                        + " 1=1 and QUOTE_RECORD_ID = '"
                        + str(ContractRecordId)
                        + "' and QTEREV_RECORD_ID = '"
                ...
                if TreeParam == "Receiving Equipment":
                    parent_obj = Sql.GetList(
                        "SELECT top "+str(PerPage)+" QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID,TEMP_TOOL,EQUIPMENTCATEGORY_DESCRIPTION,EQUIPMENT_ID,EQUIPMENT_DESCRIPTION,SERIAL_NO,GREENBOOK,FABLOCATION_ID, WARRANTY_END_DATE,WARRANTY_START_DATE,MNT_PLANT_ID,EQUIPMENT_STATUS,CUSTOMER_TOOL_ID,"+str(equipment_column)+",WARRANTY_END_DATE_ALERT,PLATFORM,CONTRACT_VALID_FROM,CONTRACT_VALID_TO,IBASE_ATTSTS from SAQSCO (NOLOCK) where "+ str(ATTRIBUTE_VALUE_STR)+ " 1=1 and QUOTE_RECORD_ID = '"+ str(ContractRecordId)+ "' and QTEREV_RECORD_ID = '"+ str(RevisionRecordId)+ "' AND SERVICE_TYPE = '"+ str(TreeTopSuperParentParam)+ "' AND SERVICE_ID ='"+ str(TreeSuperParentParam)+ "' ORDER BY "  + str(orderby))
                    ...
                    parent_obj = Sql.GetList(
                        "SELECT top "+str(PerPage)+" QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID,TEMP_TOOL,EQUIPMENTCATEGORY_DESCRIPTION,EQUIPMENT_ID,EQUIPMENT_DESCRIPTION,SERIAL_NO,GREENBOOK,FABLOCATION_ID, WARRANTY_END_DATE,WARRANTY_START_DATE,MNT_PLANT_ID,EQUIPMENT_STATUS,CUSTOMER_TOOL_ID,"+str(equipment_column)+",WARRANTY_END_DATE_ALERT,PLATFORM,CONTRACT_VALID_FROM,CONTRACT_VALID_TO,IBASE_ATTSTS from SAQSCO (NOLOCK) where "
                        + str(ATTRIBUTE_VALUE_STR)
                        + " 1=1 and QUOTE_RECORD_ID = '"
                        + str(ContractRecordId)
                        + "' and QTEREV_RECORD_ID = '"
                    ...
                    parent_obj = Sql.GetList(
                        "SELECT top "+str(PerPage)+" QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID,EQUIPMENTCATEGORY_DESCRIPTION,EQUIPMENT_ID,EQUIPMENT_DESCRIPTION,SERIAL_NO,GREENBOOK,FABLOCATION_ID, WARRANTY_END_DATE,WARRANTY_START_DATE,MNT_PLANT_ID,EQUIPMENT_STATUS,CUSTOMER_TOOL_ID,"+str(equipment_column)+",WARRANTY_END_DATE_ALERT,TEMP_TOOL,PLATFORM,CONTRACT_VALID_FROM,CONTRACT_VALID_TO,PLATFORM,CONTRACT_VALID_FROM,CONTRACT_VALID_TO,IBASE_ATTSTS from SAQSCO (NOLOCK) where "
                        + str(ATTRIBUTE_VALUE_STR)
                        + " 1=1 and QUOTE_RECORD_ID = '"
                        + str(ContractRecordId)
                        + "' and QTEREV_RECORD_ID = '"
...
def GetCovObjChildFilter(ATTRIBUTE_NAME, ATTRIBUTE_VALUE,RECID,PerPage,PageInform):
    ...
    if ATTRIBUTE_VALUE is None or ATTRIBUTE_VALUE == "" or ATTRIBUTE_VALUE_STR is None or ATTRIBUTE_VALUE_STR == "":
        ...
        if TreeSuperParentParam == "Product Offerings":
            parent_obj = Sql.GetList(
                "select top "+str(PerPage)+"  QUOTE_SERVICE_COVERED_OBJECT_ASSEMBLIES_RECORD_ID,EQUIPMENT_ID,ASSEMBLY_ID,ASSEMBLY_DESCRIPTION,GOT_CODE, EQUIPMENTTYPE_ID, EQUIPMENT_DESCRIPTION, MNT_PLANT_ID,FABLOCATION_ID,WARRANTY_START_DATE,WARRANTY_END_DATE,INCLUDED from SAQSCA (NOLOCK) where EQUIPMENT_ID = '{recid}' and QUOTE_RECORD_ID = '{ContractRecordId}'  and QTEREV_RECORD_ID = '{RevisionRecordId}' and SERVICE_ID = '{treeparam}' ORDER BY {ord_by} ".format(
                    ContractRecordId=Quote.GetGlobal("contract_quote_record_id"),RevisionRecordId = Quote.GetGlobal("quote_revision_record_id"), recid=RECID, treeparam=TreeParam,ord_by = orderby
                )
            )
            ...
            if TreeTopSuperParentParam == "Product Offerings":
                ...
                Trace.Write("5 level empty search --->")
                parent_obj = Sql.GetList(
                    "select top "+str(PerPage)+"  QUOTE_SERVICE_COVERED_OBJECT_ASSEMBLIES_RECORD_ID,EQUIPMENT_ID,ASSEMBLY_ID,ASSEMBLY_DESCRIPTION,GOT_CODE, EQUIPMENTTYPE_ID, EQUIPMENT_DESCRIPTION, MNT_PLANT_ID,FABLOCATION_ID,WARRANTY_START_DATE,WARRANTY_END_DATE,INCLUDED from SAQSCA (NOLOCK) where   QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and EQUIPMENT_ID = '{recid}' and SERVICE_ID = '{service_id}'and FABLOCATION_ID = '{fablocation_id}' ORDER BY {ord_by} ".format(
                        ContractRecordId=Quote.GetGlobal("contract_quote_record_id"),
                        RevisionRecordId = Quote.GetGlobal("quote_revision_record_id"),
                        recid=RECID,
                        service_id=TreeSuperParentParam,
        ...
        if TreeSuperParentParam == "Product Offerings":
            parent_obj = Sql.GetList(
                "select top "+str(PerPage)+"  QUOTE_SERVICE_COVERED_OBJECT_ASSEMBLIES_RECORD_ID,EQUIPMENT_ID,ASSEMBLY_ID,ASSEMBLY_DESCRIPTION,GOT_CODE, EQUIPMENTTYPE_ID, EQUIPMENT_DESCRIPTION, MNT_PLANT_ID,FABLOCATION_ID,WARRANTY_START_DATE,WARRANTY_END_DATE,INCLUDED from SAQSCA (NOLOCK) where  "
                + str(ATTRIBUTE_VALUE_STR)
