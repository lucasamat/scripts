        ...
        if RelTABEL_NAME != "QSTN_R_SYOBJR_80011":
            ...
            # elif str(tab_Name) =="Quote" and TABLEID == 'PRTXCL':
                ...
                if TESTEDOBJECT in("BILL TO","SHIP TO","SOLD TO","PAYER","SELLER"):
                    VAL_Str = (
                        "SELECT top 1000 ACCOUNT_RECORD_ID,ACCOUNT_ID,ACCOUNT_NAME FROM SAACNT WHERE ACCOUNT_ID NOT IN (SELECT PARTY_ID FROM SAQTIP WHERE CPQ_PARTNER_FUNCTION = '{}' AND QUOTE_RECORD_ID = '{}' AND QTEREV_RECORD_ID = '{}') ".format(SELECTED_ROLE,Quote.GetGlobal("contract_quote_record_id"),Quote.GetGlobal("quote_revision_record_id")) 
                    ) 
                ...
                if str(TABLEID) == "SASBCU":
                    saqtrv_query = Sql.GetFirst("SELECT REGION, SALESORG_ID FROM SAQTRV (NOLOCK) WHERE QUOTE_RECORD_ID = '{quote_rec_id}' AND QTEREV_RECORD_ID = '{quote_rev_rec_id}'".format(quote_rec_id = Quote.GetGlobal("contract_quote_record_id"), quote_rev_rec_id=quote_revision_record_id))
                    where = " where " + str(where)
                    where += "SALESORG_ID = '{salesorg_id}' AND REGION = '{region}'".format(salesorg_id= saqtrv_query.SALESORG_ID,region= saqtrv_query.REGION)
                    VAL_Str = "SELECT top 10 " + str(API_NAME_str) + " FROM " + str(TABLEID) + " " + str(where)
                    count_query=Sql.GetList("SELECT COUNT(*) as cnt FROM " + str(TABLEID) + " " + str(where))
                elif str(where).strip() != "":
                    where = " where " + str(where)
                    VAL_Str = "SELECT top 10 " + str(API_NAME_str) + " FROM " + str(TABLEID) + " " + str(where)
                ...
                #     count_query = SqlHelper.GetList("SELECT COUNT(*) as cnt FROM " + str(TABLEID))
                    ...
                    VAL_Obj = Sql.GetList(VAL_Str) 
                elif str(TABLEID) =="SAACNT":
                    where = "WHERE ACCOUNT_ID NOT IN (SELECT PARTY_ID FROM SAQTIP WHERE CPQ_PARTNER_FUNCTION = '{}' AND QUOTE_RECORD_ID = '{}' AND QTEREV_RECORD_ID = '{}') ".format(SELECTED_ROLE,Quote.GetGlobal("contract_quote_record_id"),Quote.GetGlobal("quote_revision_record_id")) 
                    VAL_Str = "SELECT top 10 " + str(API_NAME_str) + " FROM " + str(TABLEID)+" "+str(where)
                    count_query = SqlHelper.GetList("SELECT COUNT(*) as cnt FROM " + str(TABLEID)+" "+str(where))
                elif str(TABLEID) == "PRPTRM":
        ...
        for key, header in enumerate(FIELD_LABEL_list[1:]):
            ...
            if len(api_name_dict_list) > 0:
                ...
                filter_control_function += (
                    ...
                    + '" , "SELECTED_ROLE": $("#CPQ_PARTNER_FUNCTION").val() }, function(data) { $("#'
                    + str(TABLEIDS)
        ...
        if str(ATTRIBUTE_VALUE_STR) != "":
            ...
            if RelTABEL_NAME != "QSTN_R_SYOBJR_80011":
                ...
                #     VAL_Obj = Sql.GetList(VAL_Str)                     
                    ...
                    existing_condition = " AND ACCOUNT_ID NOT IN (SELECT PARTY_ID FROM SAQTIP WHERE CPQ_PARTNER_FUNCTION = '{}' AND QUOTE_RECORD_ID = '{}' AND QTEREV_RECORD_ID = '{}') ".format(SELECTED_ROLE,Quote.GetGlobal("contract_quote_record_id"),Quote.GetGlobal("quote_revision_record_id")) 
                    VAL_Str = "SELECT top 100 " + COLUMNS_NAME + " FROM " + TABLEID + " where " + ATTRIBUTE_VALUE_STR + where +' '+str(existing_condition)
                    VAL_Obj = Sql.GetList(VAL_Str)
                else:
                    VAL_Str = (
                        "SELECT top 1000 "
                ...
                if str(where).strip() != "" :
                    where = " and " + str(where)
                if str(TABLEID) != "SYOBJD" and str(TABLEID) != "PRTXCL" and str(TABLEID) != "MAFBLC" and str(TABLEID) != "SAQSCO" and str(TABLEID) != "SAEMPL" and str(TABLEID) != "SYPFTY" and str(TABLEID)!="SAACNT" and (str(TreeParentParam) != "Approval Chain Steps" or str(SegmentsClickParam) == "Approval Chain Steps") and TESTEDOBJECT !="SOURCE ACCOUNT":    
            ...
            if RelTABEL_NAME != "QSTN_R_SYOBJR_80011":
                if str(where).strip() != "" and str(ATTRIBUTE_VALUE_STR).strip() != "":
                    ...
                    VAL_Obj = Sql.GetList("SELECT " + str(COLUMNS_NAME) + " FROM PRPTRM order by 1 OFFSET {offset_skip_count} ROWS FETCH NEXT {per_page} ROWS ONLY".format(offset_skip_count=OFFSET_SKIP_COUNT,per_page=FETCH_COUNT))
                else:
        ...
        for val_api in VAL_Obj:
            ...
            for Colums_final in Colums_final_list:
                ...
                if str(Colums_final) != "EFFECTIVEDATE_BEG":
                    data_dict[str(Colums_final)] = str(eval(
