...
def attachments():
	secstr = ""
	secstr += '<div class="row modulebnr brdr ma_mar_btm">UPLOAD ATTACHMENT <button type="button" class="close flt_rt" onclick="closepopup_scrl()" data-dismiss="modal">X</button></div><div class="col-md-12 padlftrhtnone"><div class="row pad-10 bg-lt-wt brdr" id="seginnerbnr"><img style="height:40px;margin-top:-1px;margin-left:-1px;float:left" src="/mt/APPLIEDMATERIALS_PRD/Additionalfiles/Secondary Icon.svg"><div class="product_txt_div_child secondary_highlight" style="display:block;text-align:left"><div class="product_txt_child"><abbr title="Key">Upload Attachment</abbr></div><div class="product_txt_to_top_child"><abbr title="ALL">Please click on the file upload icon to add an attachment to your quote revision...</abbr></div></div></div></div><div style="padding-left:0!important;padding-right:5px!important" class="col-md-12 col-sm-12 col-xs-12 section_row_fld opport_info trans_hide enable_custom_fld transaction_summary_new quote_summary collapse in upload_attachment"><div class="col-md-5 col-sm-6 col-xs-12 property_label section_cust_fld_txt"><label style="display:inline-block">Attachment File Name</label></div><div class="col-md-7 col-sm-6 col-xs-12 property_value section_cust_fld_val"><div class="col-md-2"><span class="hint_icon_req"><i class="fa fa-info-circle autoClosePopover"></i></span> <span class="required" style="display:none;">*</span></div><div class="col-md-9"><div id="divDocAttachement"></div>   <label class="upload_attachment_label"><img class="upload_icon" src="/mt/APPLIEDMATERIALS_PRD/Additionalfiles/document_upload_icon.svg" width="20px"> <input class="upload_icon_file" type="file" id="my_file" style="display:none;" value="test.pdf" oninput="OnChangeAttachFile()"></label></div>'
	secstr +='<div class="col-md-1"><i class="fa fa-pencil" aria-hidden="true" style="font-size:13px;float:right;color:#cccaca!important"></i></div></div></div><div class="modal-footer col-md-12"><button type="button" class="btnconfig" data-dismiss="modal" onclick="closepopup_scrl()">CANCEL</button> <button type="button" id="add-offerings" class="btnconfig" onclick="Saveattachfile()" data-dismiss="modal">SAVE</button></div>'
	secstr += '<script>$(document).ready(function() { console.log("inside script tag");$(".upload_icon_file").change(function(){var i=document.getElementById("my_file").files[0].name;$("#input_file").val(i);}); });</script>'
...
def DownloadAttachments():
	...
	if get_last_attached_filename:
		fileContent =get_last_attached_filename.contentLength
