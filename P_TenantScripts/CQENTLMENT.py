                                        ...
                                        if prdvalue["possibleValues"]:
                                            for i in prdvalue["possibleValues"]:
                                                ...
                                                if i['selectable'] == 'false' and 'valueLow' in i.keys():
                                                    ...
                                                    if (prdvalue["id"] in ('AGS_Z0091_NET_PRMALB','AGS_Z0091W_NET_PRMALB','AGS_Z0035_NET_PRMALB','AGS_Z0035W_NET_PRMALB') and self.treeparam in ('PDC','MPS') and get_attribute_value.STANDARD_ATTRIBUTE_DISPLAY_VAL == 'Included - Monthly and Above') or (prdvalue["id"] in ('AGS_Z0091_KPI_TGTKPI','AGS_Z0091W_KPI_TGTKPI','AGS_Z0092W_KPI_TGTKPI','AGS_Z0092_KPI_TGTKPI','AGS_Z0090_KPI_TGTKPI' ) and self.treeparam != 'IMPLANT' and get_attribute_value.STANDARD_ATTRIBUTE_DISPLAY_VAL in ('Max wafer Output <= 4%','Max Wafer Output >4%')) or(prdvalue["id"] in ('AGS_Z0091_KPI_TGTKPI','AGS_Z0091W_KPI_TGTKPI','AGS_Z0092W_KPI_TGTKPI','AGS_Z0092_KPI_TGTKPI','AGS_Z0090_KPI_TGTKPI' ) and self.treeparam not in ('MDP','DDP') and get_attribute_value.STANDARD_ATTRIBUTE_DISPLAY_VAL in ('Excursion Detection') ) or (prdvalue["id"] in ('AGS_Z0010_PQB_USRFEE') and self.treeparam not in ('CMP') and get_attribute_value.STANDARD_ATTRIBUTE_DISPLAY_VAL == 'AGS_Z0010_PQB_USRFEE_002' ) or (prdvalue["id"] in ('AGS_Z0091_KPI_PRPFGT','AGS_Z0091W_KPI_PRPFGT') and self.treeparam in ('PDC') and value.SYSTEM_ID == 'AGS_Z0091_KPI_PRPFGT_002' ) :
                                                        dropdowndisallowlist.append(str(prdvalue["id"])+'_'+str(i['valueLow'])	)
                                                    else:
            ...
            if "calc" in AttributeID:
                ...
                for key,dict_val in ENT_IP_DICT.items():
                    ...
                    if key != 'undefined' and dict_val.split("||")[3] != 'undefined' and str((dict_val).split("||")[0]).strip() != 'undefined':
                            ...
                            if entitlement_value in ("Event Based","Flex Event Based"):
                                try:
                                    if str(serviceId) in ("Z0009","Z0010","Z0128"):
                                        check_assembly = True
                                        if check_assembly:
                                            Sql.RunQuery("UPDATE SAQSCO SET IBASE_ATTSTS = 'INCOMPLETE' FROM (SELECT DISTINCT A.EQUIPMENT_ID FROM SAQSCA A JOIN MAEQUP B ON  B.PAR_EQUIPMENT_ID = A.EQUIPMENT_ID AND B.EQUIPMENT_ID = A.ASSEMBLY_ID WHERE A.QUOTE_RECORD_ID = '"+str(self.ContractRecordId)+"' AND A.QTEREV_RECORD_ID = '"+str(self.revision_recordid)+"' AND A.SERVICE_ID = '"+str(serviceId)+"' AND ISNULL(B.PROCESS_TYPE,'') = '') equp WHERE QUOTE_RECORD_ID = '"+str(self.ContractRecordId)+"' AND QTEREV_RECORD_ID = '"+str(self.revision_recordid)+"' AND SERVICE_ID = '"+str(serviceId)+"' AND SAQSCO.EQUIPMENT_ID = equp.EQUIPMENT_ID  AND SAQSCO.IBASE_ATTSTS = 'COMPLETE'")
                                ...
                                Sql.RunQuery("UPDATE A SET A.IBASE_ATTSTS = B.IBASE_ATTSTS  FROM SAQSCO A JOIN SAQFEQ B ON A.QUOTE_RECORD_ID =B.QUOTE_RECORD_ID  AND A.QTEREV_RECORD_ID  =B.QTEREV_RECORD_ID AND A.FABLOCATION_ID  = B.FABLOCATION_ID AND A.GREENBOOK  = B.GREENBOOK WHERE  A.QUOTE_RECORD_ID ='"+str(self.ContractRecordId)+"' AND A.QTEREV_RECORD_ID='"+str(self.revision_recordid)+"' AND A.SERVICE_ID ='"+str(serviceId)+"'")
                        ...
                        if key in ( "AGS_{}_NET_PRMALB".format(serviceId)) and str(tableName) in ('SAQTSE') and Entitlement_edit_value in ( "AGS_{}_NET_PRMALB".format(serviceId),  "AGS_{}_NET_WECLLB".format(serviceId),  "AGS_{}_NET_CRMALB".format(serviceId)):##A055S000P01-16862 code starts....
