...
###A055S000P01-12558
updatesaqtmtexpire = (""" UPDATE SAQTMT SET EXPIRED = '1' FROM SAQTMT INNER JOIN SAQTRV ON SAQTMT.MASTER_TABLE_QUOTE_RECORD_ID = SAQTRV.QUOTE_RECORD_ID AND SAQTMT.QTEREV_RECORD_ID = SAQTRV.QTEREV_RECORD_ID WHERE  SAQTRV.REV_EXPIRE_DATE = '{today_current_date}' AND SAQTRV.REVISION_STATUS NOT IN ('CBC-SUBMITTED FOR BOOKING') """.format(today_current_date = today_current_date ))
