...
def fts_z0006_z0007_insert(milestone_amt=0,milestone_date=None,service_id='',par_service_id=''):	
    ...
    Sql.RunQuery(""" INSERT SAQIBP (
                QUOTE_ITEM_BILLING_PLAN_RECORD_ID, BILLING_END_DATE, BILLING_START_DATE,ANNUAL_BILLING_AMOUNT,ANNBILAMT_INGL_CURR,TTLMRG_INGL_CURR,BILLING_VALUE, BILLING_VALUE_INGL_CURR,BILLING_TYPE,LINE, QUOTE_ID,DOC_CURRENCY, QTEITM_RECORD_ID,COMMITTED_VALUE_INGL_CURR,ESTVAL_INGL_CURR,BILLING_DAY,
...
def insert_items_monthly_billing_plan_records(temp_table, service_id, par_service_id):
    Sql.RunQuery(""" INSERT SAQIBP (
                    ...
                    UserId=user_id, QuoteRecordId=contract_quote_rec_id,
                    RevisionRecordId=quote_revision_rec_id,
                    ServiceId = service_id, ParServiceId = par_service_id, DecimalPlaces=get_round_val,get_dc_cur_val=get_dc_cur_val,dc_DecimalPlaces=get_dc_cur_val))
...
def insert_items_billing_plan(total_months=1, billing_date='',billing_end_date ='', amount_column='YEAR_1', service_id=None,get_ent_val_type =None,get_ent_billing_type_value=None,get_billling_data_dict=None,billing_day =None,datediff=None):
    ...
    if str(get_billing_type).upper() == "FIXED" and get_billing_type != '' and service_id not in ('Z0100'):		
        ...
        Sql.RunQuery(""" INSERT SAQIBP (
                    QUOTE_ITEM_BILLING_PLAN_RECORD_ID, BILLING_END_DATE, BILLING_START_DATE,ANNUAL_BILLING_AMOUNT,ANNBILAMT_INGL_CURR,TTLMRG_INGL_CURR,BILLING_VALUE, BILLING_VALUE_INGL_CURR,BILLING_TYPE,LINE, QUOTE_ID, QTEITM_RECORD_ID,COMMITTED_VALUE_INGL_CURR,ESTVAL_INGL_CURR,
                    QUOTE_RECORD_ID,QTEREV_ID,QTEREV_RECORD_ID,GLOBAL_CURRENCY,GLOBALCURRENCY_RECORD_ID,
            ...
            for val in billing_plan_obj:
                if billing_plan_obj:					
                    ...
                    if str(get_ent_bill_cycle).upper() == "ONE ITEM PER QUOTE":
                        ...
                        years, months = int(years), int(round(remainder / avgmonth))            
                        if months > 0:
                            years += 1 
                        yearwise_dates_dict = {}
                        billing_dates_list = []
                        for year in range(1, years+1):
                            ...
                            one_year, months = int(one_year), int(round(remainder / avgmonth))   
                            total_months = one_year * 12 + months  
                            ...
                            for index, billing_date_res in enumerate(billing_dates_list):
                                ...
                                if billing_plan_year in toolwise_date_details.keys():
                                    ...
                                    if total_days_per_month:									
                                        ...
                                        one_day_tool_amt = quote_item.get(billing_plan_year)/toolwise_contract_year_total_days
                                        one_day_tool_amt_in_global_currency = quote_item.get(billing_plan_year+'_INGL_CURR')/toolwise_contract_year_total_days
                                        ...
                                        if float(per_month_amt) > 0 or (get_service_val == 'Z0116' and float(per_month_amt) < 0) or (get_service_val == 'Z0117' and float(per_month_amt) < 0):
                                            ...
                                            records.append([str(billing_plan_start_date), str(billing_plan_end_date), billing_plan_year, get_service_val, par_service_id, billing_day, str(per_month_amt), str(per_month_amt_in_global_currency), str(billing_date), quote_revision_contract_item_id, year_column_split])
