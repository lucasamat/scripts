...
def Related_Sub_Banner(
    ...
    #else:     
    #Trace.Write('status----->'+str(revision_status.REVISION_STATUS))
    dynamic_Button = None
    add_button = ""
    ...
    # SHP Contract Document Ends
        ...
        if (subTabName == "Equipment Details" or subTabName == "Equipment Assemblies" or subTabName == "Equipment Entitlements" or subTabName == "Equipment Fab Value Drivers" or subTabName == "Equipment Cost and Value Drivers" or subTabName == "Customer Value Drivers" or subTabName == "Product Value Drivers"):
            ...
            FifthLable = "Serial Number"
            FifthValue = str(SerialNumber)
        elif(str(ObjName)=="SAQRGG" and (subTabName == 'Details' or subTabName == 'Events' or subTabName == 'Kit Detials' or subTabName == 'BoM')):
            PrimaryLable = "Product Offering ID"
            PrimaryValue = str(TreeSuperParentParam)
            SecondLable = "Product Offering Description"
            SecondValue = desc
    ...
    if CurrentRecordId.startswith("SYOBJR", 0) == True:
        ...
        if get_quote_status and (str(quote_status.REVISION_STATUS) == 'CFG-CONFIGURATION' or str(quote_status.REVISION_STATUS) == 'CFG-CONFIGURING' or str(quote_status.REVISION_STATUS) == 'NEW REVISION' or str(quote_status.REVISION_STATUS) == 'APR-RECALLED'  or str(quote_status.REVISION_STATUS) == 'CFG-ACQUIRING' or (quote_status.REVISION_STATUS) == 'PRI-PRICING') and Submit_approval == "True":
        ...
        #     sec_rel_sub_bnr += ('<button id="delete_parts" onclick="bulk_del_yes()"  class="btnconfig" disabled = "">DELETE</button>') 
            ...
            if str(subTabName)=="Events" and revision_status.REVISION_STATUS != 'APR-APPROVED':
                ...
                if str(multi_buttons) != "":
                    ...
                    sec_rel_sub_bnr += str(add_button)
            elif str(subTabName) == "Spare Parts" and str(TreeParentParam)=="Complementary Products" and revision_status.WORKFLOW_STATUS in ('APPROVALS','CLEAN BOOKING CHECKLIST','LEGAL SOW','QUOTE DOCUMENTS'):
                if str(multi_buttons) != "":
