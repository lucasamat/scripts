...
def Related_Sub_Banner(
    ...
    #         SixthValue = ""
        ...
        getService = Sql.GetFirst("select SERVICE_DESCRIPTION from SAQTSV where SERVICE_ID = '"+str(TreeParentParam)+"'")
        covered_obj = Sql.GetFirst("select EQUIPMENT_ID from SAQSCO(nolock) where QUOTE_RECORD_ID = '{contract_quote_record_id}' AND QTEREV_RECORD_ID = '{quote_revision_record_id}' AND SERVICE_ID = '{TreeParentParam}' AND GREENBOOK = '{TreeParam}' AND QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID = '"+str(CurrentRecordId)+"'".format(contract_quote_record_id = Quote.GetGlobal("contract_quote_record_id"),quote_revision_record_id = quote_revision_record_id,TreeParam = TreeParam,TreeParentParam = TreeParentParam))
        if subTabName == "Equipment Details":
            PrimaryLable = "Product Offering ID"
            PrimaryValue = str(TreeParentParam)
            SecondLable = "Product Offering Description"
