...
def Related_Sub_Banner(
	...
	if CurrentRecordId.startswith("SYOBJR", 0) == True:
		...
		if get_quote_status and str(quote_status.QUOTE_STATUS) == 'IN-PROGRESS' and Submit_approval == "True" and Quote_item_obj is not None:
			...
			sec_rel_sub_bnr += (
					'<button class="btnconfig cust_def_btn submitbutton" data-target="#submit_for_approval" data-toggle="modal" id="submit_for_approval_btn_primary" onclick="submitfor_approval()">SUBMIT FOR APPROVAL</button>'
					)
		# else:
		# 	Trace.Write("elseeee")
