	...
	def MDYNMICSQLOBJECT(self, RECORD_ID, PerPage, PageInform, SubTab, PR_CURR, TP, equipment_id): 
		...
		if ObjectName == 'SAQICO':
			...
			SAQICO_dbl_clk_function += (    
				...
				+ str(table_id) 
				+ '", "RECORDID":reco_id, "ELEMENT":"RELATEDEDIT"}, function(data) { debugger; data1=data[0]; data2=data[1]; data3 = data[2];if(data1 != "NO"){ if(document.getElementById("RL_EDIT_DIV_ID") ) { localStorage.setItem("saqico_title", field); if(field == "DISCOUNT"){inp = "discount_" + data3; }else{inp = "#"+data3;} $("#SYOBJR_00009_E5504B40_36E7_4EA6_9774_EA686705A63F").find(inp).prop("disabled", false);localStorage.setItem("value_tag", "'+ str(table_id)+' "+inp); if (field == "DISCOUNT"){ $("'+str(table_ids)+' ").find("abbr[id="+inp+"]").attr("contenteditable", true);} else{ $("'+str(table_ids)+' "+inp).closest("tr").find("td:nth-child("+edit_index+")").attr("contenteditable", true);}   var buttonlen = $("#seginnerbnr").find("button#saveButton"); if (buttonlen.length == 0){  RecId = "SYOBJR-00009";RecName = "div_CTR_Assemblies";$("#seginnerbnr").append("<button class=\'btnconfig\' onclick=\'loadRelatedList(RecId,RecName);\' type=\'button\' value=\'Cancel\' id=\'cancelButton\'>CANCEL</button><button class=\'btnconfig\' type=\'button\' value=\'Save\' onclick=\'multiedit_save_RL()\' id=\'saveButton\'>SAVE</button>");} else{$("#cancelButton").css("display", "block");$("#saveButton").css("display", "block");} if (field == "DISCOUNT"){ $("'+str(table_ids)+' ").find("abbr[id="+inp+"]").addClass("light_yellow");}else{$("'+str(table_ids)+' " +inp).closest("tr").find("td:nth-child("+edit_index+")").addClass("light_yellow");} document.getElementById("cont_multiEditModalSection").style.display = "none";  var divHeight = $("#cont_multiEditModalSection").height(); $("#cont_multiEditModalSection .modal-backdrop").css("min-height", divHeight+"px"); $("#cont_multiEditModalSection .modal-dialog").css("width","550px"); $(".modal-dialog").css("margin-top","100px"); } if (data2.length !== 0){ $.each( data2, function( key, values ) { onclick_datepicker(values) }); } } }); }                   $("' 
				+ str(table_ids)    
				+ "\").on('sort.bs.table', function (e, name, order) {  currenttab = $(\"ul#carttabs_head .active\").text().trim(); localStorage.setItem('" 
				+ str(table_id) 
				+ "_SortColumn', name); localStorage.setItem('" 
						...
						if Product.GetGlobal("TreeParentLevel2") == "Quote Items":  
							...
							if TreeParam == "Quote Items":
								Qury_str = (
									"select top "
										+ str(PerPage)
										+ " CASE WHEN STATUS = 'ACQUIRED' THEN '"+ imgstr +"' WHEN STATUS = 'APPROVAL REQUIRED' THEN '" +exclamation+ "' WHEN STATUS = 'ERROR' THEN '"+ error +"' WHEN STATUS = 'PARTIALLY PRICED' THEN '"+ partially_priced +"' WHEN STATUS = 'ASSEMBLY MISSING' THEN '"+ assembly_missing +"'  ELSE '"+ acquiring_img_str +"' END AS STATUS, QUOTE_ITEM_COVERED_OBJECT_RECORD_ID, EQUIPMENT_LINE_ID, EQUIPMENT_ID,SERVICE_ID,LINE_ITEM_ID,BD_DISCOUNT,BD_PRICE_MARGIN,DISCOUNT,NET_PRICE,YEAR_OVER_YEAR,"+col_year+",SERIAL_NO, GREENBOOK,FABLOCATION_ID,TECHNOLOGY,KPU,MODEL_PRICE, TARGET_PRICE_MARGIN, TARGET_PRICE, SALES_DISCOUNT_PRICE, CEILING_PRICE, SALDIS_PERCENT,SRVTAXCLA_DESCRIPTION,TAX_PERCENTAGE, NET_VALUE,PRICE_BENCHMARK_TYPE,TOOL_CONFIGURATION,ANNUAL_BENCHMARK_BOOKING_PRICE,CONTRACT_ID,CONVERT(VARCHAR(10),CONTRACT_VALID_FROM,101) AS [CONTRACT_VALID_FROM],CONVERT(VARCHAR(10),CONTRACT_VALID_TO,101) AS [CONTRACT_VALID_TO],BENCHMARKING_THRESHOLD,CpqTableEntryId,ASSEMBLY_ID from ( select  ROW_NUMBER() OVER( ORDER BY "+ str(Wh_API_NAMEs)
