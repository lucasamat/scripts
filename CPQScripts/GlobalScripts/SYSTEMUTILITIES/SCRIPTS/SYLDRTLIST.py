		...
		if Columns != "" and Obj_Name != "":
			...
			for key, invs in enumerate(list(eval(Columns))):
				...
				# 	continue
					...
					rowspan_level1 = ""
					if not table_group_columns:
						table_header += '<th colspan="8" '+rowspan_level1+'  data-align="center"><div><button style="border:none;" class="glyphicon glyphicon-minus-sign" id="object_info_column_toggle" onclick="quote_items_column_toggle(this)"></button>OBJECT INFORMATION</div></th>'
					if str(invs) in right_align_list:
						align = 'right'
					elif str(invs) in center_align_list:
						align = 'center'
				...
				elif RECORD_ID == 'SYOBJR-00009' and invs in ('TARGET_PRICE_INGL_CURR','SLSDIS_PRICE_INGL_CURR','BD_PRICE_INGL_CURR','CEILING_PRICE_INGL_CURR','NET_PRICE_INGL_CURR','DISCOUNT','TOTAL_AMOUNT_INGL_CURR'):
					...
					if not table_group_columns2:
						table_header += '<th colspan="6" '+rowspan_level1+'  data-align="center"><div><button style="border:none;" class="glyphicon glyphicon-minus-sign" id="price_info_column_toggle" onclick="quote_items_column_toggle(this)"></button>PRICING INFORMATION</div></th>'
					if str(invs) in right_align_list:
						align = 'right'
					elif str(invs) in center_align_list:
						align = 'center'
						...
						if Product.GetGlobal("TreeParentLevel2") == "Quote Items":  
							...
							if TreeParam == "Quote Items":
								...
								Qury_str = (
									"select top "
										+ str(PerPage)
										+ " CASE WHEN STATUS = 'ACQUIRED' THEN '"+ imgstr +"' WHEN STATUS = 'APPROVAL REQUIRED' THEN '" +exclamation+ "' WHEN STATUS = 'ON HOLD - COSTING' THEN '"+ error +"' WHEN STATUS = 'ERROR' THEN '"+ error +"' WHEN STATUS = 'PARTIALLY PRICED' THEN '"+ partially_priced +"' WHEN STATUS = 'ASSEMBLY MISSING' THEN '"+ assembly_missing +"'  ELSE '"+ acquiring_img_str +"' END AS STATUS, QUOTE_ITEM_COVERED_OBJECT_RECORD_ID, EQUIPMENT_LINE_ID,SERVICE_ID,EQUIPMENT_ID,EQUIPMENT_DESCRIPTION,SERIAL_NO,CUSTOMER_TOOL_ID,ASSEMBLY_ID,GREENBOOK,FABLOCATION_ID,KPU,TECHNOLOGY,TOOL_CONFIGURATION,TARGET_PRICE_INGL_CURR,SLSDIS_PRICE_INGL_CURR,BD_PRICE_INGL_CURR,CEILING_PRICE_INGL_CURR,NET_PRICE_INGL_CURR,DISCOUNT,TOTAL_AMOUNT_INGL_CURR,YEAR_OVER_YEAR,CONVERT(VARCHAR(10),CONTRACT_VALID_FROM,101) AS [CONTRACT_VALID_FROM],CONVERT(VARCHAR(10),CONTRACT_VALID_TO,101) AS [CONTRACT_VALID_TO] from ( select  ROW_NUMBER() OVER( ORDER BY "+ str(Wh_API_NAMEs)
										+") AS ROW, * from SAQICO (NOLOCK) where "+ str(ATTRIBUTE_VALUE_STR)+" QUOTE_ID = '"
										+ str(qt_rec_id.QUOTE_ID)
										+ "') m where m.ROW BETWEEN "
										+ str(Page_start)
							...
							if Product.GetGlobal("TreeParentLevel2") == "Quote Items":
								...
								if TreeParam == "Quote Items":
									...
									Qury_str = (
										"select top "
											+ str(PerPage)
											+ " CASE WHEN STATUS = 'ACQUIRED' THEN '"+ imgstr +"' WHEN STATUS = 'APPROVAL REQUIRED' THEN '" +exclamation+ "' WHEN STATUS = 'ON HOLD - COSTING' THEN '"+ error +"' WHEN STATUS = 'ERROR' THEN '"+ error +"'  WHEN STATUS = 'PARTIALLY PRICED' THEN '"+ partially_priced +"' WHEN STATUS = 'ASSEMBLY MISSING' THEN '"+ assembly_missing +"'  ELSE '"+ acquiring_img_str +"' END AS STATUS, QUOTE_ITEM_COVERED_OBJECT_RECORD_ID, EQUIPMENT_LINE_ID,SERVICE_ID,EQUIPMENT_ID,EQUIPMENT_DESCRIPTION,SERIAL_NO,CUSTOMER_TOOL_ID,ASSEMBLY_ID,GREENBOOK,FABLOCATION_ID,KPU,TECHNOLOGY,TOOL_CONFIGURATION,TARGET_PRICE_INGL_CURR,SLSDIS_PRICE_INGL_CURR,BD_PRICE_INGL_CURR,CEILING_PRICE_INGL_CURR,NET_PRICE_INGL_CURR,DISCOUNT,TOTAL_AMOUNT_INGL_CURR,YEAR_OVER_YEAR,CONTRACT_VALID_FROM,CONTRACT_VALID_TO from ( select  ROW_NUMBER() OVER( ORDER BY "+ str(Wh_API_NAMEs)
											+") AS ROW, * from SAQICO (NOLOCK) where  QUOTE_ID = '"
											+ str(qt_rec_id.QUOTE_ID)
											+ "' AND QTEREV_RECORD_ID = '"+str(quote_revision_record_id)+"') m where m.ROW BETWEEN "
											+ str(Page_start)
