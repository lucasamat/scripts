		...
		if CHL_STS_OBJ is not None: # Not Needed
			...
			if RECORD_ID != "SYOBJR-00009":
				...
				dbl_clk_function += (
					...
					+ str(table_ids)
					+ '").closest("table"); table.find("tbody tr").each(function() { if (localStorage.getItem("CommonTreeParam") == "Add-On Products"){checkedRows.push($(this).find("td:nth-child(4)").text()}else{checkedRows.push($(this).find("td:nth-child(4)").text()}); }); localStorage.setItem("multiedit_checkbox_clicked", checkedRows); }); $("'
					+ str(table_ids)
