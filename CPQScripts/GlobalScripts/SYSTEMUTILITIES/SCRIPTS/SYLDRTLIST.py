                        ...
                        if Product.GetGlobal("TreeParentLevel2") == "Quote Items":                            
                            ...
                            if TreeParentParam == "Quote Items":
                                Qury_str = (
                                    "select top "
                                        + str(PerPage)
                                        + " CASE  WHEN PRICING_STATUS = 'ACQUIRED' THEN '"+ imgstr +"' WHEN PRICING_STATUS = 'APPROVAL REQUIRED' THEN '" +exclamation+ "' WHEN PRICING_STATUS = 'ERROR' THEN '"+ error +"' WHEN PRICING_STATUS = 'PARTIALLY PRICED' THEN '"+ partially_priced +"' WHEN PRICING_STATUS = 'ASSEMBLY MISSING' THEN '"+ assembly_missing +"' ELSE '"+ acquiring_img_str +"' END AS PRICING_STATUS, QUOTE_ITEM_COVERED_OBJECT_RECORD_ID, EQUIPMENT_LINE_ID, EQUIPMENT_ID,SERVICE_ID,LINE_ITEM_ID,LIST_PRICE,BD_DISCOUNT,BD_PRICE_MARGIN,DISCOUNT,NET_PRICE,YEAR_OVER_YEAR,"+col_year+",BASE_PRICE,SERIAL_NO, GREENBOOK,FABLOCATION_ID, TOTAL_COST, TARGET_PRICE_MARGIN, TARGET_PRICE, SALES_DISCOUNT_PRICE, CEILING_PRICE, SALES_DISCOUNT,SRVTAXCLA_DESCRIPTION,TAX_PERCENTAGE,TAX, NET_VALUE,PRICE_BENCHMARK_TYPE,TOOL_CONFIGURATION,ANNUAL_BENCHMARK_BOOKING_PRICE,CONTRACT_ID,CONVERT(VARCHAR(10),CONTRACT_VALID_FROM,101) AS [CONTRACT_VALID_FROM],CONVERT(VARCHAR(10),CONTRACT_VALID_TO,101) AS [CONTRACT_VALID_TO],BENCHMARKING_THRESHOLD,CpqTableEntryId from ( select  ROW_NUMBER() OVER( ORDER BY EQUIPMENT_LINE_ID) AS ROW, * from SAQICO (NOLOCK) where QUOTE_ID = '"
                                        + str(qt_rec_id.QUOTE_ID)
                                        + "' AND SERVICE_ID = '"
                                ...
                                Qury_str = (
                                    "SELECT DISTINCT TOP "
                                    + str(PerPage)
                                    + " CASE WHEN PRICING_STATUS = 'ACQUIRED' THEN '"+ imgstr +"' WHEN PRICING_STATUS = 'APPROVAL REQUIRED' THEN '"+ exclamation +"' WHEN PRICING_STATUS = 'ERROR' THEN '"+ error +"' WHEN PRICING_STATUS = 'PARTIALLY PRICED' THEN '"+ partially_priced +"' WHEN PRICING_STATUS = 'ASSEMBLY MISSING' THEN '"+ assembly_missing +"' ELSE '"+ acquiring_img_str +"' END AS PRICING_STATUS, QUOTE_ITEM_COVERED_OBJECT_RECORD_ID,EQUIPMENT_LINE_ID,EQUIPMENT_ID,SERVICE_ID,SERIAL_NO,GREENBOOK,FABLOCATION_ID, TOTAL_COST,TARGET_PRICE_MARGIN,TARGET_PRICE,CEILING_PRICE,LINE_ITEM_ID,LIST_PRICE,BD_DISCOUNT,BD_PRICE_MARGIN,DISCOUNT,SALES_PRICE,YEAR_OVER_YEAR,"+col_year+",SALES_DISCOUNT,SRVTAXCLA_DESCRIPTION,TAX_PERCENTAGE,TAX,NET_PRICE,EQUIPMENT_RECORD_ID,BASE_PRICE,SERVICE_RECORD_ID,FABLOCATION_RECORD_ID,BD_DISCOUNT_RECORD_ID,EQUIPMENTCATEGORY_RECORD_ID,QUOTE_RECORD_ID,MNT_PLANT_RECORD_ID,SALE_DISCOUNT_RECORD_ID,SALES_DISCOUNT_PRICE,SALESORG_RECORD_ID,GREENBOOK_RECORD_ID,QUOTE_CURRENCY_RECORD_ID,PRICE_BENCHMARK_TYPE,TOOL_CONFIGURATION,ANNUAL_BENCHMARK_BOOKING_PRICE,CONTRACT_ID,CONVERT(VARCHAR(10),CONTRACT_VALID_FROM,101) AS [CONTRACT_VALID_FROM],CONVERT(VARCHAR(10),CONTRACT_VALID_TO,101) AS [CONTRACT_VALID_TO],BENCHMARKING_THRESHOLD,CpqTableEntryId from ( select ROW_NUMBER() OVER(order by "+ str(Wh_API_NAMEs) +") AS ROW, * from SAQICO (nolock)  where QUOTE_RECORD_ID ='"+str(RecAttValue)
                        ...
                        if Product.GetGlobal("TreeParentLevel1") == "Cart Items":                            
                            ...
                            if TreeParentParam == "Cart Items":
                                Qury_str = (
                                    "select top "
                                        + str(PerPage)
                                        + " CASE WHEN DISCOUNT = 'ACQUIRED' THEN '"+ imgstr +"' ELSE '"+ imgstr +"' END AS EQUIPMENT_STATUS, CONTRACT_ITEM_COVERED_OBJECT_RECORD_ID, EQUIPMENT_LINE_ID, SERVICE_ID, EQUIPMENT_ID,LINE_ITEM_ID,DISCOUNT,SERIAL_NO, GREENBOOK, TOTAL_COST, TAX, NET_VALUE_PRICE, CONTRACT_CURRENCY, CONTRACT_CURRENCY_RECORD_ID,CpqTableEntryId from ( select  ROW_NUMBER() OVER( ORDER BY EQUIPMENT_LINE_ID) AS ROW, * from CTCICO (NOLOCK) where CONTRACT_ID = '"
                                        + str(qt_rec_id.CONTRACT_ID)
                                        + "' AND SERVICE_ID = '"
                                        + str(LineAndEquipIDList[1])
                                        + "') m where m.ROW BETWEEN "
                ...
                if RECORD_ID == "SYOBJR-94443" and str(current_tab) == "Tab":                    
                    ...
                    if TreeParam == "Quote Items":                        
                        ...
                        Qury_str = (
                            "select DISTINCT top "
                            + str(PerPage)
                            + " '"+ icon +"' AS PO_NOTES, QUOTE_ITEM_RECORD_ID, LINE_ITEM_ID, SERVICE_ID, SERVICE_DESCRIPTION, OBJECT_QUANTITY, QUANTITY,TOTAL_COST, ONSITE_PURCHASE_COMMIT, SALES_DISCOUNT_PRICE,SRVTAXCLA_DESCRIPTION,TAX_PERCENTAGE,TAX, NET_VALUE_PRICE, TARGET_PRICE, CEILING_PRICE, BD_PRICE, BD_PRICE_MARGIN, DISCOUNT, SALES_PRICE, YEAR_OVER_YEAR, "+col_year+" "
                            + ",CpqTableEntryId from ( select TOP 10 ROW_NUMBER() OVER(order by "
                            + str(Wh_API_NAMEs)
                            + ") AS ROW, * from "
                            + str(ObjectName)
                    ...
                    if RECORD_ID == "SYOBJR-95868":                        
                        ...
                        if Product.GetGlobal("TreeParentLevel2") == "Quote Items":  
                            ...
                            Qury_str = (
                                    "SELECT DISTINCT TOP "
                                    + str(PerPage)
                                    + " CASE WHEN PRICING_STATUS = 'ACQUIRED' THEN '"+ imgstr +"' WHEN PRICING_STATUS = 'APPROVAL REQUIRED' THEN '"+ exclamation +"' WHEN PRICING_STATUS = 'ERROR' THEN '"+ error +"' WHEN PRICING_STATUS = 'PARTIALLY PRICED' THEN '"+ partially_priced +"' WHEN PRICING_STATUS = 'ASSEMBLY MISSING' THEN '"+ assembly_missing +"' ELSE '"+ acquiring_img_str +"' END AS PRICING_STATUS, QUOTE_ITEM_COVERED_OBJECT_RECORD_ID,EQUIPMENT_LINE_ID,TARGET_PRICE_MARGIN,SERVICE_ID,EQUIPMENT_ID,SERIAL_NO,GREENBOOK,FABLOCATION_ID,TOTAL_COST,TARGET_PRICE,CEILING_PRICE,SALES_DISCOUNT,SRVTAXCLA_DESCRIPTION,TAX_PERCENTAGE,TAX,NET_VALUE,EQUIPMENT_RECORD_ID,LINE_ITEM_ID,LIST_PRICE,BD_DISCOUNT,BD_PRICE_MARGIN,DISCOUNT,NET_PRICE,YEAR_OVER_YEAR,"+col_year+",BASE_PRICE,SERVICE_RECORD_ID,FABLOCATION_RECORD_ID,BD_DISCOUNT_RECORD_ID,EQUIPMENTCATEGORY_RECORD_ID,QUOTE_RECORD_ID,MNT_PLANT_RECORD_ID,SALES_DISCOUNT_PRICE,SALE_DISCOUNT_RECORD_ID,SALESORG_RECORD_ID,GREENBOOK_RECORD_ID,QUOTE_CURRENCY_RECORD_ID,PRICE_BENCHMARK_TYPE,TOOL_CONFIGURATION,ANNUAL_BENCHMARK_BOOKING_PRICE,CONTRACT_ID,CONVERT(VARCHAR(10),CONTRACT_VALID_FROM,101) AS [CONTRACT_VALID_FROM],CONVERT(VARCHAR(10),CONTRACT_VALID_TO,101) AS [CONTRACT_VALID_TO],BENCHMARKING_THRESHOLD,CpqTableEntryId from ( select ROW_NUMBER() OVER(order by "+ str(Wh_API_NAMEs) +") AS ROW, * from SAQICO (nolock)  where "+ str(ATTRIBUTE_VALUE_STR)+" QUOTE_RECORD_ID ='"+str(RecAttValue)
                                    +"' and GREENBOOK = '"+str(TreeParam)+"' and FABLOCATION_ID = '"+str(fab_location_id)+"' and SERVICE_ID = '"+str(ServiceId)+"') m where m.ROW BETWEEN "
                                    + str(Page_start)
                                    + " AND "
                                    + str(Page_End)
