				...
				for ik in Query_Obj:					                  
					...
					for inm in ik:
						...
						if str(RECORD_ID) == "SYOBJR-98859" and value123 == 'SERVICE_ID':
							contract_quote_record_id = Quote.GetGlobal("contract_quote_record_id")
							get_key_value = Sql.GetFirst("SELECT {} AS VAL from {} (nolock) where QUOTE_RECORD_ID = '{}' AND QTEREV_RECORD_ID='{}' and SERVICE_ID = '{}' and PAR_SERVICE_ID = '{}'".format(str(objRecName),str(ObjectName),contract_quote_record_id,quote_revision_record_id,str(value1234),str(TreeSuperParentParam)))
			...
			for key, invs in enumerate(list(eval(Columns))):
				...
				# 	continue
					...
					if str(invs) in right_align_list:  
						...
						if (str(RECORD_ID) == "SYOBJR-00029" and str(invs)=="QUANTITY" and str(Product.GetGlobal("TreeParentLevel2"))=="Product Offerings") or (str(RECORD_ID)=="SYOBJR-00005" and str(invs) == "CUSTOMER_ANNUAL_QUANTITY" and str(TreeParentParam)=="Complementary Products") or (SubTab.upper() =='INCLUSIONS' and str(RECORD_ID) == "SYOBJR-00029" and str(invs)=="QUANTITY"  and str(Product.GetGlobal("TreeParentLevel2"))=="Product Offerings") and TreeParam == 'Z0092':
