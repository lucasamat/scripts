            ...
            if Query_Obj is not None:
                for ik in Query_Obj:				                  
                    ...
                    for inm in ik:
                        ...
                        if value1234 != "ACQUIRED" and value1234 != "APPROVAL REQUIRED" and value1234 != "ERROR" and value1234 != "ASSEMBLY IS MISSING" and value1234 != "ON HOLD COSTING" and value1234 != "PARTIALLY PRICED" and value1234 != "ACQUIRING" and value1234 != "PRICED" and value1234 != "PRR-ON HOLD PRICING":                        
                            value1234 = value1234
                        if value123 == objRecName:
        ...
        for key, col_name in enumerate(list(eval(Columns))):            
            ...
            if filter_level_data == "select" and col_name not in checkbox_list:                
                ...
                for string in StringValue_list:
                    ...
                    if string != "ACQUIRED" and string != "APPROVAL REQUIRED" and string != "ERROR" and string != "ASSEMBLY IS MISSING" and string != "ON HOLD COSTING" and string != "PARTIALLY PRICED" and string != "ACQUIRING" and string != "PRICED" and string != "PRR-ON HOLD PRICING":                        
                        string_value = string
                        ...
                        if Product.GetGlobal("TreeParentLevel2") == "Quote Items":
                            ...
                            if TreeParam == "Quote Items":
                                ...
                                Qury_str = (
                                    "select top "
                                        + str(PerPage)
                                        + " CASE WHEN STATUS = 'ACQUIRED' THEN '"+ imgstr +"' WHEN STATUS = 'APPROVAL REQUIRED' THEN '" +exclamation+ "' WHEN STATUS in ('PRR-ON HOLD PRICING','ON HOLD COSTING') THEN '"+ on_hold_costing +"' WHEN STATUS = 'ERROR' THEN '"+ error +"'  WHEN STATUS = 'PARTIALLY PRICED' THEN '"+ partially_priced +"' WHEN STATUS = 'ASSEMBLY IS MISSING' THEN '"+ assembly_missing +"'  ELSE '"+ acquiring_img_str +"' END AS STATUS, QUOTE_ITEM_COVERED_OBJECT_RECORD_ID,LINE,SERVICE_ID,OBJECT_ID,ASSEMBLY_ID,GOT_CODE,EQNODE,PROCES,PM_ID,KIT_ID,KIT_NUMBER,MNTEVT_LEVEL,SAPMMP,CNTDAY,FABLOCATION_ID,GREENBOOK,QUANTITY,TOLCFG,KPU,SPQTEV,SPSPCT,SVSPCT,WTYSTE,WTYEND,WTYDAY,INWRTY,ATGKEC,ATGKEP,ATKNCI,ATKNPI,NWPTOC,NWPTOP,CONSCP,CONSPI,NONCCI,NONCPI,AMNCCI,AMNPPI,TNTVGC,TENVGC,BPTKCI,BPTKPI,TRGPRC,SLSPRC,BDVPRC,CELPRC,TGADJP,YOYPCT,USRPRC,BCHPGC,BCHDPT,CNTPRC,CpqTableEntryId from ( select  ROW_NUMBER() OVER( ORDER BY "+ str(Wh_API_NAMEs)
                                        +") AS ROW, * from SAQICO (NOLOCK) where "+ str(ATTRIBUTE_VALUE_STR)+" QUOTE_ID = '"
                                        + str(qt_rec_id.QUOTE_ID)
                                        + "' AND QTEREV_RECORD_ID = '"+str(quote_revision_record_id)+"') m where m.ROW BETWEEN "
                                        + str(Page_start)
