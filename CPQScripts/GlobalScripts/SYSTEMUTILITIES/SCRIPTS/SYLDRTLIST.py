    ...
    def MDYNMICSQLOBJECT(self, RECORD_ID, PerPage, PageInform, SubTab, PR_CURR, TP): 
                                ...
                                if (value123 == "SET_NAME" or value123 == "SETMAT_NAME") and (
                                    ...
                                    if value123 in [
                                        ...
                                        if (str(TreeParentParam).upper() == "BRIDGE PRODUCTS" and  str(RECORD_ID) == "SYOBJR-00005" and str(value123) in ['SCHEDULE_MODE','CUSTOMER_ANNUAL_QUANTITY']) or (str(RECORD_ID) == "SYOBJR-00009" and str(value123) == 'SALES_PRICE'):
                                            ...
                                            new_dict[value123] = (
                                                '<input id ="' + key_value + '" value="' + value1234 + '" style="border: 0px solid;" class="fltltlightyello"  disabled> </input>'
                                            )
        ...
        if ObjectName == 'SAQICO':
            ...
            SAQICO_dbl_clk_function += (	
                ...
                + '"); cpq.server.executeScript("SYBLKETRLG", {"TITLE":field, "VALUE":value, "CLICKEDID":"'	
                + str(table_id)	
                + '", "RECORDID":reco_id, "ELEMENT":"RELATEDEDIT"}, function(data) { debugger; data1=data[0]; data2=data[1]; data3 = data[2];if(data1 != "NO"){ if(document.getElementById("RL_EDIT_DIV_ID") ) { localStorage.setItem("saqico_title", field); inp = "input#"+data3;localStorage.setItem("value_tag", inp); $("#SYOBJR_00009_E5504B40_36E7_4EA6_9774_EA686705A63F").find(inp).prop("disabled", false); var buttonlen = $("#seginnerbnr").find("button#saveButton"); if (buttonlen.length == 0){	$("#seginnerbnr").append("<button class=\'btnconfig\' onclick=\'multiedit_RL_cancel();\' type=\'button\' value=\'Cancel\' id=\'cancelButton\'>CANCEL</button><button class=\'btnconfig\' type=\'button\' value=\'Save\' onclick=\'multiedit_save_RL()\' id=\'saveButton\'>SAVE</button>");} document.getElementById("cont_multiEditModalSection").style.display = "none";  var divHeight = $("#cont_multiEditModalSection").height(); $("#cont_multiEditModalSection .modal-backdrop").css("min-height", divHeight+"px"); $("#cont_multiEditModalSection .modal-dialog").css("width","550px"); $(".modal-dialog").css("margin-top","100px"); } if (data2.length !== 0){ $.each( data2, function( key, values ) { onclick_datepicker(values) }); } } }); }                   $("'	
                + str(table_ids)	
                + "\").on('sort.bs.table', function (e, name, order) {  currenttab = $(\"ul#carttabs_head .active\").text().trim(); localStorage.setItem('"	
                + str(table_id)	
                + "_SortColumn', name); localStorage.setItem('"	
                        ...
                        if value123 in checkbox_list:
                            ...
                            if (value123 == "SET_NAME" or value123 == "SETMAT_NAME") and (
                                ...
                                if value123 in [
                                    ...
                                    if (str(TreeParentParam).upper() == "BRIDGE PRODUCTS" and  str(RECORD_ID) == "SYOBJR-00005" and str(value123) in ['SCHEDULE_MODE','CUSTOMER_ANNUAL_QUANTITY']) or (str(RECORD_ID) == "SYOBJR-00009" and str(value123) == 'SALES_PRICE'):
                                        new_dict[value123] = (
                                            '<input id ="' + key_value + '"   value="' + value1234 + '" style="border: 0px solid;" class="fltltlightyello" disabled>' + value1234 + "</input>"
                                        )
