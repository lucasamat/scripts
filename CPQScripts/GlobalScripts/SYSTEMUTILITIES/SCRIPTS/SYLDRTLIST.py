...
class SYLDRTLIST:
	def MDYNMICSQLOBJECT(self, RECORD_ID, PerPage, PageInform, SubTab, PR_CURR, TP, equipment_id):         
		...
		if Columns != "" and Obj_Name != "":
			...
			for key, invs in enumerate(list(eval(Columns))):
				...
				# 	continue
					...
					rowspan_level1 = ""
					if not table_group_columns:
						table_header += '<th colspan="8" '+rowspan_level1+'  data-align="center"><div><button style="border:none;" class="glyphicon glyphicon-minus-sign" id="price-benchmark-column-toggle" onclick="price_benchmark_column_toggle(this)"></button>OBJECT INFORMATION</div></th>'
					if str(invs) in right_align_list:
						align = 'right'
					elif str(invs) in center_align_list:
						align = 'center'
		...
		if  RECORD_ID == 'SYOBJR-00009' and str(TreeParam) == "Quote Items":
				columns_temp = ['STATUS','EQUIPMENT_LINE_ID','SERVICE_ID','EQUIPMENT_ID','EQUIPMENT_DESCRIPTION','CEILING_PRICE_INGL_CURR','TARGET_PRICE_INGL_CURR','SLSDIS_PRICE_INGL_CURR','TOTAL_AMOUNT_INGL_CURR','NET_PRICE','NET_VALUE','YEAR_OVER_YEAR','DISCOUNT','WARRANTY_START_DATE','WARRANTY_END_DATE','SERIAL_NO','CUSTOMER_TOOL_ID','ASSEMBLY_ID','GREENBOOK','FABLOCATION_ID','KPU','TECHNOLOGY','TOOL_CONFIGURATION']
