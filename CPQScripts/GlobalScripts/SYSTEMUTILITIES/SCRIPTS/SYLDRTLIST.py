	...
	def MDYNMICSQLOBJECT(self, RECORD_ID, PerPage, PageInform, SubTab, PR_CURR, TP, equipment_id):         
		...
		if Columns != "" and Obj_Name != "":
			...
			for key, invs in enumerate(list(eval(Columns))):
				...
				elif RECORD_ID == 'SYOBJR-00009' and invs in ('CEILING_PRICE_INGL_CURR','TARGET_PRICE_INGL_CURR','SLSDIS_PRICE_INGL_CURR','TOTAL_AMOUNT_INGL_CURR','NET_PRICE_INGL_CURR','NET_VALUE_INGL_CURR','DISCOUNT'):
						...
						if Product.GetGlobal("TreeParentLevel2") == "Quote Items":  
							...
							if TreeParam == "Quote Items":
								...
								Qury_str = (
									"select top "
										+ str(PerPage)
										+ " CASE WHEN STATUS = 'ACQUIRED' THEN '"+ imgstr +"' WHEN STATUS = 'APPROVAL REQUIRED' THEN '" +exclamation+ "' WHEN STATUS = 'ON HOLD - COSTING' THEN '"+ error +"' WHEN STATUS = 'ERROR' THEN '"+ error +"' WHEN STATUS = 'PARTIALLY PRICED' THEN '"+ partially_priced +"' WHEN STATUS = 'ASSEMBLY MISSING' THEN '"+ assembly_missing +"'  ELSE '"+ acquiring_img_str +"' END AS STATUS, QUOTE_ITEM_COVERED_OBJECT_RECORD_ID, EQUIPMENT_LINE_ID,SERVICE_ID,EQUIPMENT_ID,EQUIPMENT_DESCRIPTION,SERIAL_NO,CUSTOMER_TOOL_ID,ASSEMBLY_ID,GREENBOOK,FABLOCATION_ID,KPU,TECHNOLOGY,TOOL_CONFIGURATION,CEILING_PRICE_INGL_CURR,TARGET_PRICE_INGL_CURR,SLSDIS_PRICE_INGL_CURR,TOTAL_AMOUNT_INGL_CURR,NET_PRICE,NET_VALUE,YEAR_OVER_YEAR,DISCOUNT,CONVERT(VARCHAR(10),WARRANTY_START_DATE,101) AS [WARRANTY_START_DATE],CONVERT(VARCHAR(10),WARRANTY_END_DATE,101) AS [WARRANTY_END_DATE] from ( select  ROW_NUMBER() OVER( ORDER BY "+ str(Wh_API_NAMEs)
										+") AS ROW, * from SAQICO (NOLOCK) where "+ str(ATTRIBUTE_VALUE_STR)+" QUOTE_ID = '"
										+ str(qt_rec_id.QUOTE_ID)
										+ "') m where m.ROW BETWEEN "
										+ str(Page_start)
