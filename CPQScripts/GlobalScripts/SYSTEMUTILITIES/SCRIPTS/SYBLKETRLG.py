				...
				if SELECTALL != "noselection":
					...
					if TITLE not in ('NET_PRICE','DISCOUNT','PM_FREQUENCY','QUANTITY','CUSTOMER_ANNUAL_QUANTITY','NEW_PART') :
						...
						if obj_obj == 'SAQSAP':
							...
							field = "Enter Updated Target Quantity to add to your Parts List..." if TITLE=="QUANTITY" else "Select New Part to Update Parts List..."
							label = "UPDATED TARGET QUANTITY" if TITLE=="QUANTITY" else "UPDATED NEW PART"
							input_id = "updatedQuantity" if TITLE=="QUANTITY" else "updatedNewPart"
							input_type = "type = \'checkbox\' " if TITLE=="NEW_PART" else ""
			...
			##multi select bulk edit..	
				...
				if obj_name == "SAQTRV":
					...
					if len(TITLE.split(','))==1:
						Sql.RunQuery("""UPDATE SAQRSP SET {column} = {value} WHERE QUOTE_RECORD_ID = '{QuoteRecordId}' AND QTEREV_RECORD_ID = '{rev_rec_id}' AND {rec_name} = '{rec_id}' """.format(column=TITLE.split(',')[0],value = ALLVALUES[index] if str(type(ALLVALUES))=="<type 'ArrayList'>" else str(ALLVALUES),QuoteRecordId = Qt_rec_id,rev_rec_id = Quote.GetGlobal("quote_revision_record_id"),rec_name = objh_head,rec_id = sql_obj.QUOTE_REV_PO_PRODUCT_LIST_ID))
					else:
						Sql.RunQuery("""UPDATE SAQRSP SET {column} = {value} , {column1} = '{value1}' WHERE QUOTE_RECORD_ID = '{QuoteRecordId}' AND QTEREV_RECORD_ID = '{rev_rec_id}' AND {rec_name} = '{rec_id}' """.format(column=TITLE.split(',')[0],value = ALLVALUES[index] if str(type(ALLVALUES))=="<type 'ArrayList'>" else ALLVALUES,column1=TITLE.split(',')[1],value1 = ALLVALUES1[index] if str(type(ALLVALUES1))=="<type 'ArrayList'>" else str(ALLVALUES1),QuoteRecordId = Qt_rec_id,rev_rec_id = Quote.GetGlobal("quote_revision_record_id"),rec_name = objh_head,rec_id = sql_obj.QUOTE_REV_PO_PRODUCT_LIST_ID))
				else:
					Table.TableActions.Update(obj_name, objh_head, row)
