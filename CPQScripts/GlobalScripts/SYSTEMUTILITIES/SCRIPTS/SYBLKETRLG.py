	...
	if objh_obj is not None:
		...
		if obj_name == 'SAQICO':
			if TITLE != 'SALES_PRICE':
				...
				fmt = '%-m%d%Y'
				d1 = dt.datetime.strptime(str(getdates.CONTRACT_VALID_FROM).replace("/",""), fmt)
				d2 = dt.datetime.strptime(str(getdates.CONTRACT_VALID_TO).replace("/",""), fmt)
				days = (d2 - d1).days
				...
				c = Sql.GetFirst("SELECT SUM(SALES_PRICE) AS SUM_PRICE FROM SAQICO (NOLOCK) WHERE QUOTE_RECORD_ID = '{}' AND SERVICE_ID = '{}' AND GREENBOOK = '{}'".format(a.QUOTE_RECORD_ID,a.SERVICE_ID,a.GREENBOOK))
