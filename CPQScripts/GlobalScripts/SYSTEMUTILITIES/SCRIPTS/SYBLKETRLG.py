	...
	if objh_obj is not None:
		...
		for rec in selected_rows:
			...
			##multi select bulk edit..	
				...
				Sql.RunQuery("""UPDATE SAQSAP SET {column} = {value} WHERE QUOTE_RECORD_ID = '{QuoteRecordId}' AND QTEREV_RECORD_ID = '{rev_rec_id}' AND QUOTE_SERVICE_COV_OBJ_ASS_PM_KIT_RECORD_ID = '{rec_id}' """.format(column=TITLE,value=ALLVALUES[index_val],QuoteRecordId = Qt_rec_id,rev_rec_id = Quote.GetGlobal("quote_revision_record_id"),rec_id = sql_obj.QUOTE_SERVICE_COV_OBJ_ASS_PM_KIT_RECORD_ID))
			else:
