...
def RELATEDMULTISELECTONSAVE(TITLE, VALUE, CLICKEDID, RECORDID,selectPN,ALLVALUES,ALLVALUES1,ALLVALUES2,ALLVALUES3,DEL_PN,ADD_PN,SELECTALL,offering_description,delivery_schedules):
	...
	if quote_status.REVISION_STATUS !="BOK-CONTRACT BOOKED" and objh_obj is not None:
		...
		if obj_name in ('SAQSAP','SAQRSP','SAQSPT','SAQGPA','SAQGPM','SAQRIS','SAQRDS','SAQSCN') and (not (TITLE == 'QUANTITY' and obj_name == 'SAQRSP')):
			...
			if(SELECTALL=="PM_BULKEDIT_ALL" and obj_name == "SAQSAP" and TITLE == "PM_FREQUENCY"):
				...
				if str(TreeParam) == "Z0009":
					Sql.RunQuery("""UPDATE SAQGPA SET {column} = {value} WHERE {qury_str} QUOTE_RECORD_ID = '{QuoteRecordId}' AND QTEREV_RECORD_ID = '{rev_rec_id}' AND SERVICE_ID = 'Z0009' AND PM_FREQUENCY_EDITABLE = 'True' """.format(column=TITLE,value=ALLVALUES,QuoteRecordId = Qt_rec_id,rev_rec_id = rev_rec_id,qury_str=qury_str))
				...
				if str(TreeSuperParentParam) == "Z0009":
					Sql.RunQuery("""UPDATE SAQGPM SET {column} = {value} WHERE {qury_str} QUOTE_RECORD_ID = '{QuoteRecordId}' AND QTEREV_RECORD_ID = '{rev_rec_id}' AND SERVICE_ID = 'Z0009' AND GREENBOOK = '{greenbook}' AND GOT_CODE = '{got_code}' AND PM_FREQUENCY_EDITABLE = 'True' """.format(column=TITLE,value=ALLVALUES,QuoteRecordId = Qt_rec_id,rev_rec_id = rev_rec_id,qury_str=qury_str,got_code = TreeParam,greenbook = TreeParentParam))
				return ""
			...
			##multi select bulk edit..	
				...
				if TITLE == "CUSTOMER_ANNUAL_QUANTITY":
					...
					if value==0 or value=='' or value=='0':
						...
						Sql.RunQuery("""UPDATE SAQSPT SET {column} =  (CASE WHEN PAR_PART_NUMBER= '' THEN NULL WHEN PAR_PART_NUMBER != ''   THEN '1' ELSE '1' END) WHERE QUOTE_RECORD_ID = '{QuoteRecordId}' AND QTEREV_RECORD_ID = '{rev_rec_id}' AND {rec_name} = '{rec_id}' """.format(column=TITLE,QuoteRecordId = Qt_rec_id,rev_rec_id = rev_rec_id,rec_name = objh_head,rec_id = sql_obj.QUOTE_SERVICE_PART_RECORD_ID))
						...
						Sql.RunQuery("""UPDATE SAQSPT SET {column} = (CASE WHEN PAR_PART_NUMBER= '' THEN '{value}' WHEN PAR_PART_NUMBER != '' AND  {value} >= '1' THEN {value} ELSE '1' END)  WHERE QUOTE_RECORD_ID = '{QuoteRecordId}' AND QTEREV_RECORD_ID = '{rev_rec_id}' AND {rec_name} = '{rec_id}' """.format(column=TITLE,value = ALLVALUES[index] if str(type(ALLVALUES))=="<type 'ArrayList'>" else ALLVALUES,QuoteRecordId = Qt_rec_id,rev_rec_id = rev_rec_id,rec_name = objh_head,rec_id = sql_obj.QUOTE_SERVICE_PART_RECORD_ID))
		...
		if ADD_PN and len(ADD_PN)>0:
			...
			get_party_role = Sql.GetList("SELECT CPQ_PARTNER_FUNCTION, PARTY_ID FROM SAQTIP(NOLOCK) WHERE QUOTE_RECORD_ID = '"+str(Qt_rec_id)+"' AND QTEREV_RECORD_ID = '"+str(rev_rec_id)+"' and CPQ_PARTNER_FUNCTION in ('SOLD TO')")
			account_info = {}
			for keyobj in get_party_role:
				account_info[keyobj.CPQ_PARTNER_FUNCTION] = keyobj.PARTY_ID
			
			get_party_role = Sql.GetList("SELECT CPQ_PARTNER_FUNCTION, PARTY_ID FROM SAQTIP(NOLOCK) WHERE QUOTE_RECORD_ID = '"+str(Qt_rec_id)+"' AND QTEREV_RECORD_ID = '"+str(rev_rec_id)+"' and CPQ_PARTNER_FUNCTION in ('SHIP TO')")
			shipto_list=[]
			for keyobj in get_party_role:
