				...
				if TreeParam == 'Z0101':
					...
					for m in re.finditer(quote_item_tag, entitlement_xml):
						...
						if consigned_parts_id and consigned_parts_value:
							...
							break
					Sql.RunQuery("UPDATE SAQSPT SET SCHEDULE_MODE = ' ', DELIVERY_MODE = ' '  WHERE QUOTE_RECORD_ID = '{QuoteRecordId}' AND QTEREV_RECORD_ID= '{RevisionRecordId}' AND SERVICE_ID = '{service_id}' AND ISNULL(CUSTOMER_ANNUAL_QUANTITY,'') = '' ".format(QuoteRecordId = ContractRecordId,RevisionRecordId = quote_revision_record_id,service_id=TreeParam,consigned_parts_value = consigned_parts_value))
		...
		for index,rec in enumerate(selected_rows):
			...
			##multi select bulk edit..	
				...
				if TITLE.split(',') == ["RETURN_NOTIFY_EMAIL","SHIP_NOTIFY_EMAIL"]:
					if ALLVALUES1:
						Trace.Write('555 '+str(ALLVALUES1)+'555 '+str(objh_head))
						Sql.RunQuery("""UPDATE SAQICT SET RETURN_NOTIFY_EMAIL = '{value}',SHIP_NOTIFY_EMAIL = '{value1}' WHERE QUOTE_RECORD_ID = '{QuoteRecordId}' AND QTEREV_RECORD_ID = '{rev_rec_id}' """.format(value = ALLVALUES1[0],value1 = ALLVALUES1[1],QuoteRecordId = Qt_rec_id,rev_rec_id = Quote.GetGlobal("quote_revision_record_id"),rec_name = objh_head))
