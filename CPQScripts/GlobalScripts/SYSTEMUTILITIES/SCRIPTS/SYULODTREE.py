						...
						for childdata in childQuery:							
							...
							if getParentObjRightView is not None and len(getParentObjRightView) > 0:
								for getRightView in getParentObjRightView:
									...
									# 	subTabName = ""
									# 	service_id = 'Z0101'
									# 	spare_parts_object = Sql.GetFirst("SELECT count(CpqTableEntryId) as cnt FROM SAQIFP (NOLOCK) WHERE QUOTE_RECORD_ID = '{}' AND SERVICE_ID = '{}' AND QTEREV_RECORD_ID = '{}'".format(Product.GetGlobal("contract_quote_record_id"),str(service_id),quote_revision_record_id))
