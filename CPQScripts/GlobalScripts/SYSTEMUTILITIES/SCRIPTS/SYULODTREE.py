...
try:
	...
	ent_temp_drop = SqlHelper.GetFirst("sp_executesql @T=N'IF EXISTS (SELECT ''X'' FROM SYS.OBJECTS WHERE NAME= ''"+str(ent_temp)+"'' ) BEGIN DROP TABLE "+str(ent_temp)+" END  ' ")
							...
							if getParentObjRightView is not None and len(getParentObjRightView) > 0:
								for getRightView in getParentObjRightView:
									...
									if (str(ObjRecId) == '354C16C4-BDCA-4045-BC4A-40F1A6600AFD' and  str(getRightView.SUBTAB_TYPE) == 'OBJECT SECTION LAYOUT'):
										...
										table_name = "SAQTSE"
										entitlement_obj =Sql.GetFirst("SELECT * FROM {temp_tab} where SERVICE_ID = {get_service_id} and ENTITLEMENT_DISPLAY_VALUE IN ('Some Exclusions','Some Inclusions') AND ( ENTITLEMENT_ID like 'AGS_%TSC_NONCNS' OR ENTITLEMENT_ID like 'AGS_%TSC_CONSUM')".format(temp_tab = ent_temp,get_service_id = service_id))
