							...
							if getParentObjRightView is not None and len(getParentObjRightView) > 0:
								for getRightView in getParentObjRightView:
									...
									if (str(ObjRecId) == '354C16C4-BDCA-4045-BC4A-40F1A6600AFD' and  str(getRightView.SUBTAB_TYPE) == 'OBJECT SECTION LAYOUT'):
										...
										table_name = "SAQTSE"
										entitlement_obj_temp =Sql.GetFirst("SELECT * FROM "+str(ent_temp)+" where SERVICE_ID = '"+str(service_id)+"' and ENTITLEMENT_DISPLAY_VALUE IN ('Some Exclusions','Some Inclusions') AND ( ENTITLEMENT_ID like 'AGS_%TSC_NONCNS' OR ENTITLEMENT_ID like 'AGS_%TSC_CONSUM')")
