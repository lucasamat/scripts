...
def POPUPLISTVALUEADDNEW(
	...
	if Question_obj is not None:
		...
		##Added the below code for the receiving fab popup...
			...
			sec_str += '<div class="col-md-12 padlftrhtnone" id="btnhide"><div class="row pad-10 bg-lt-wt brdr"><img style="height: 40px; margin-top: -1px; margin-left: -1px; float: left;" src="/mt/APPLIEDMATERIALS_TST/Additionalfiles/Secondary Icon.svg"/><div class="product_txt_div_child secondary_highlight" style="display: block;"><div class="product_txt_child"><abbr title="Key">Add NSOS</abbr></div><div class="product_txt_to_top_child" style="float: left;"><abbr title="{sales_org}">{sales_org}</abbr></div></div><div class="product_txt_div_child secondary_highlight" style="display: block;"></div><button type="button" class="btnconfig" data-dismiss="modal" onclick="closepopup_scrl()">CANCEL</button><button type="button" id="add-unmapped_equipment" class="btnconfig" onclick="addUnmappedEquipments()" data-dismiss="modal">ADD</button></div></div>'.format(
				Product.GetGlobal("TreeParam"), Product.GetGlobal("TreeParam"),sales_org = "TEST",
			)
