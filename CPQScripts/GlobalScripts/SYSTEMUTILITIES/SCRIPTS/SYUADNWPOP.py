...
def POPUPLISTVALUEADDNEW(
	...
	if Question_obj is not None:
		...
		##involved party equipment ends
			...
			new_value_dict = {}
			ObjectName = "SAACNT"
			table_id = "account_replace_addnew"
			Header_details = {
				"ACCOUNT_RECORD_ID": "KEY",
			...
			sec_str += '</tr></thead><tbody class ="equipments_id" ></tbody></table>'
			sec_str += '<div id="account_replace_addnew_footer"></div>'
			values_list = ""
			values_lists = ""
			a_test = []
			for invsk in list(Header_details):
				...
				filter_control_function += (
					...
					+ "\", 'RECORDFEILD':  \""
					+ str(RECORDFEILD)
					+ "\", 'NEWVALUE': '', 'LOOKUPOBJ': '', 'LOOKUPAPI': '','A_Keys':a_list,'A_Values':ATTRIBUTE_VALUEList,'ROLE':ROLE}, function(data) {  date_field = data[3]; var assoc = data[1]; var api_name = data[2];data4 = data[4];data5 = data[5]; try { if(date_field.length > 0) { $(\""
					+ str(table_ids)
					+ '").bootstrapTable("load", date_field  ); $("button#country_save").attr("disabled",false); $("#noRecDisp").remove() } else{ var date_field = [];$("'
					+ str(table_ids)
					+ '").bootstrapTable("load", date_field  ); $("button#country_save").attr("disabled",true); $("#account_replace_addnew").after("<div id=\'noRecDisp\' class=\'noRecord\'>No Records to Display</div>"); $(".noRecord:not(:first)").remove(); } } catch(err) { if(date_field.length > 0) { $("'
					+ str(table_ids)
					+ '").bootstrapTable("load", date_field  ); $("button#country_save").attr("disabled",false); } else{ $("'
					+ str(table_ids)
					+ '").bootstrapTable("load", date_field  ); $("button#country_save").attr("disabled",true); } } ; });  });'
			...
			if TreeParam == "Customer Information":
				where_string += """ SAACNT.ACCOUNT_ID  NOT IN (SELECT PARTY_ID FROM SAQTIP (NOLOCK) where QUOTE_RECORD_ID = '{}' AND QTEREV_RECORD_ID = '{}' and PARTY_ROLE ='{}')""".format(contract_quote_record_id,quote_revision_record_id,ROLE)
