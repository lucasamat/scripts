	...
	if Question_obj is not None:
		...
		##Added the below code for the receiving fab popup...
			...
			for invsk in list(Header_details):
				...
				filter_control_function += (
					...
					+ '").bootstrapTable("load", date_field  ); $("button#country_save").attr("disabled",false); $("#noRecDisp").remove();if (document.getElementById("RecordsStartAndEnd")){document.getElementById("RecordsStartAndEnd").innerHTML = data15;}; if (document.getElementById("TotalRecordsCount")) {document.getElementById("TotalRecordsCount").innerHTML = data16;} } else{ $("'
					+ str(table_ids)
					+ '").bootstrapTable("load", date_field  ); $("button#country_save").attr("disabled",true); $("#'+str(table_id)+' > tbody").html("<tr class='"'noRecDisp'"'><td colspan='"'7'"' class='"'txt_al_lt_imp'"'>No Records to Display</td></tr>");console.log("binded correctly") $(".noRecord:not(:first)").remove(); } } catch(err) { if(date_field != "NORECORDS") { $("'
					+ str(table_ids)
					+ '").bootstrapTable("load", date_field  ); $("button#country_save").attr("disabled",false); } else{ $("'
					+ str(table_ids)
					+ '").bootstrapTable("load", date_field  ); $("button#country_save").attr("disabled",true); } } ; });  });'
			...
			if QryCount==0:
				...
				for invsk in list(Header_details):
					...
					filter_control_function += (
						...
						+ "\", 'RECORDFEILD':  \""
						+ str(RECORDFEILD)
						+ "\", 'NEWVALUE': '', 'LOOKUPOBJ': '', 'LOOKUPAPI': '','A_Keys':a_list,'A_Values':ATTRIBUTE_VALUEList}, function(data) {  date_field = data[3]; var assoc = data[1]; var api_name = data[2];data4 = data[4];data5 = data[5]; try { if(date_field!='NORECORDS') { $(\""
						+ str(table_ids)
						+ '").bootstrapTable("load", date_field  ); $("button#country_save").attr("disabled",false); $("#noRecDisp").remove() } else{ var date_field = [];$("'
						+ str(table_ids)
						+ '").bootstrapTable("load", date_field  ); $("button#country_save").attr("disabled",true);  $("#add_credits_add_new > tbody").html("<div id=\'noRecDisp\' class=\'noRecord\'>No Records to Display</div>"); $(".noRecord:not(:first)").remove(); } } catch(err) { if(date_field!="NORECORDS") { $("'
						+ str(table_ids)
						+ '").bootstrapTable("load", date_field  ); $("button#country_save").attr("disabled",false); } else{ $("'
						+ str(table_ids)
						+ '").bootstrapTable("load", date_field  ); $("button#country_save").attr("disabled",true); } } ; });  });'
		...
		# UNMAPPED EQUIPMENTS ENDED
			...
			if (("Sending Account -" in TreeParam) or ("Receiving Account -" in TreeParam)) and TreeParentParam == 'Fab Locations':
				...
				Pagination_M = Sql.GetFirst(
					"SELECT COUNT(CpqTableEntryId) as count FROM {} (NOLOCK) WHERE ACCOUNT_RECORD_ID = '{}' AND FABLOCATION_ID = '{}' AND SALESORG_ID = '{}' AND ISNULL(SERIAL_NO, '') <> '' AND ISNULL(GREENBOOK, '') <> '' AND {} EQUIPMENT_RECORD_ID NOT IN (SELECT EQUIPMENT_RECORD_ID FROM SAQFEQ (NOLOCK) WHERE QUOTE_RECORD_ID = '{}' AND FABLOCATION_ID = '{}' AND QTEREV_RECORD_ID = '{}' AND ISNULL(SERIAL_NUMBER,'') <> '')".format(
						ObjectName,
						account_record_id,
						Product.GetGlobal("TreeParam"),
						sales_org,
