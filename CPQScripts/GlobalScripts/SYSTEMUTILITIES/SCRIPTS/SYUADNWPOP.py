	...
	if Question_obj is not None:
		...
		##involved party equipment ends
			...
			if Objd_Obj is not None:
				...
				Product.GetGlobal("TreeParentLevel0"),Product.GetGlobal("TreeParentLevel0"),
			...
			#    )
			#)
			QueryCountObj = Sql.GetFirst("select count(*) as cnt from MAADPR(NOLOCK) WHERE PRDOFR_ID = '"+str(TreeParentParam)+"' ")
		...
		# UNMAPPED EQUIPMENTS ENDED
		...
		elif str(ObjectName) == "SAQSPT" and str(CurrentTab) == "Quotes":
			Trace.Write('In SAQSPT')
			where_string = ""
			if A_Keys != "" and A_Values != "":
				A_Keys = list(A_Keys)
				A_Values = list(A_Values)
			...
			sec_str += '<div class="col-md-12 padlftrhtnone"><div class="row pad-10 bg-lt-wt brdr"> <img style="height: 40px; margin-top: -1px; margin-left: -1px; float: left;" src="/mt/APPLIEDMATERIALS_TST/Additionalfiles/Secondary Icon.svg"/><div class="product_txt_div_child secondary_highlight" style="display: block;text-align: left;"><div class="product_txt_child"><abbr title="Key">PRODUCT OFFERING</abbr></div><div class="product_txt_to_top_child"><abbr title="ALL">ALL</abbr></div></div><button type="button" class="btnconfig" data-dismiss="modal" onclick="closepopup_scrl()">CANCEL</button><button type="button" id="add-parts" class="btnconfig" onclick="addPartsList()" data-dismiss="modal">ADD</button></div></div>'
			...
			iclusions_val_list = []
			entitlement_obj = SqlHelper.GetFirst("select replace(ENTITLEMENT_XML,'&',';#38') as ENTITLEMENT_XML from SAQTSE (nolock) where QUOTE_RECORD_ID = '860B6B5E-B2C6-45E9-BB8C-2C342B9A0E44' AND QTEREV_RECORD_ID = 'DF7D71DE-ACE0-4CFF-81DD-F53F7E8DFD6F' and SERVICE_ID = '{}' ".format(TreeParam))
			get_xml_val = SqlHelper.GetList("select ENTITLEMENT_ID,ENTITLEMENT_DISPLAY_VALUE from (SELECT distinct e.QUOTE_RECORD_ID,e.QTEREV_RECORD_ID,replace(X.Y.value('(ENTITLEMENT_ID)[1]', 'VARCHAR(128)'),';#38','&') as ENTITLEMENT_ID,replace(X.Y.value('(ENTITLEMENT_DISPLAY_VALUE)[1]', 'VARCHAR(128)'),';#38','&') as ENTITLEMENT_DISPLAY_VALUE FROM (select QUOTE_RECORD_ID,QTEREV_RECORD_ID,convert(xml,'"+str(entitlement_obj.ENTITLEMENT_XML)+"') as ENTITLEMENT_XML from SAQTSE (nolock) where QUOTE_RECORD_ID = '860B6B5E-B2C6-45E9-BB8C-2C342B9A0E44' AND QTEREV_RECORD_ID = 'DF7D71DE-ACE0-4CFF-81DD-F53F7E8DFD6F' and SERVICE_ID = '{}' ) e OUTER APPLY e.ENTITLEMENT_XML.nodes('QUOTE_ITEM_ENTITLEMENT') as X(Y) ) as m where ENTITLEMENT_ID in ('AGS_Z0091_TSC_NONCNS','AGS_Z0091_TSC_CONSUM') ".format(TreeParam))
			...
			Pagination_M = Sql.GetFirst(
				"SELECT COUNT({}.CpqTableEntryId) as count FROM {} (NOLOCK) {} WHERE {} {}.IS_SPARE_PART = 'True' AND PRODUCT_TYPE IS NULL AND  NOT EXISTS (SELECT PART_NUMBER FROM SAQSPT (NOLOCK) WHERE QUOTE_RECORD_ID = '{}' AND QTEREV_RECORD_ID ='{}' and MAMTRL.SAP_PART_NUMBER = SAQSPT.PART_NUMBER) {} ".format(
					ObjectName,ObjectName,inner_join if inner_join else "",str(where_string)+" AND " if where_string else "",ObjectName,contract_quote_record_id,quote_revision_record_id,additional_where
				)
			)
			if QueryCountObj is not None:
