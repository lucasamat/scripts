	...
	def PreviewApprovers(self, AllParams, FromSeg):        
		...
		if 1:
			...
			if CurrentTabName == 'Quotes' or CurrentTabName == 'Quote':
				...
				my_approval_queue_obj = Sql.GetFirst("""select OWNER_NAME from SAQTMT where MASTER_TABLE_QUOTE_RECORD_ID = '{contract_quote_record_id}'  AND QTEREV_RECORD_ID='{quote_rec_id}'""".format(quote_rec_id = my_queue_obj.APRTRXOBJ_RECORD_ID,revision_rec_id=  self.quote_revision_record_id,contract_quote_record_id = Quote.GetGlobal("contract_quote_record_id"))
			if CurrentTabName == 'Quotes' or CurrentTabName == 'Quote':
