...
class ViolationConditions:
    ...
    def DeleteforApprovalHeaderTable(self, RecordId, chainId, ChainStep, ObjectName):
        ...
        if getApprovalId:
            for transdelete in getApprovalId:
                DeleteQueryStatementApprovalTrans = "DELETE ACAPTX WHERE APPROVAL_ID = '" + str(transdelete.APPROVAL_ID) + "' "
                DeleteApprovalTrans = Sql.RunQuery(DeleteQueryStatementApprovalTrans)
                DeleteQueryStatementTrackedvalue = "DELETE ACAPFV WHERE APPROVAL_ID = '" + str(transdelete.APPROVAL_ID) + "' "
        ...
        Log.Info(
            "User Id:" + str(self.Get_UserID) + "Script Name:ACVIORULES.PY Query Statement:" + str(DeleteQueryStatementApprovalHeader)
        )
        return True
    ...
    def ViolationRuleForApprovals(self, CurrentId, ObjectName, chainid):
        ...
        GetObjHPromaryKey = Sql.GetFirst(
            "SELECT RECORD_NAME,RECORD_ID FROM SYOBJH WHERE OBJECT_NAME ='{ObjectName}' ".format(ObjectName=ObjectName)
        )

        GetQuoteId = Sql.GetFirst(
            "SELECT QUOTE_ID,QTEREV_ID FROM {ObjectName} WHERE {primarykey} = '{CurrentId}'".format(
                ObjectName=ObjectName, primarykey=str(GetObjHPromaryKey.RECORD_NAME), CurrentId=CurrentId
            )
        )
        ...
        Getlatestauto = Sql.GetFirst(
            "SELECT APPROVAL_ID FROM ACAPMA (NOLOCK) WHERE APPROVAL_ID LIKE '%" + str(ApprovalCombo) + "%' ORDER BY APPROVAL_ID DESC "
        )
        if Getlatestauto:
        ...
        INSERT
            ACAPMA (
                ...
                APPROVE_TEMPLATE_RECORD_ID,
                TOTALDAYS_IN_APPROVAL,
                TOTALDAYS_IN_APRCHNSTP,
                CUR_APRCHNSTP,
                FIN_APPROVE_USER_ID,
                FIN_APPROVE_USER_RECORD_ID,
                FIN_REJECT_USER_ID,
                FIN_REJECT_USER_RECORD_ID,
                REJECT_TEMPLATE_RECORD_ID,
                REQUEST_DATE,
                REQUEST_USER_ID,
                REQUEST_USER_RECORD_ID,
                REQUEST_TEMPLATE_RECORD_ID,
                CUR_APRCHNSTP_LASTACTIONDATE,
                CUR_APPCHNSTP_APPROVER_ID,
                CUR_APRCHNSTP_APPROVER_RECORD_ID,
                CUR_APRCHNSTP_ENTRYDATE,
                FIN_APPROVE_DATE,
                REJECT_DATE,
                APPROVE_TEMPLATE_ID,
                APROBJ_STATUSFIELD_VALUE,
                REJECT_TEMPLATE_ID,
                REQUEST_TEMPLATE_ID,
                ADDUSR_RECORD_ID,
        ...
        SELECT
            TOP 1 CONVERT(VARCHAR(4000), NEWID()) AS APPROVAL_RECORD_ID,
            '{recid}' AS APROBJ_ID,
            ...
            CONVERT(
                VARCHAR(4000),
                SYOBJH.OBJECT_NAME + '-' + '{QuoteId}' + '-' + '{RevisionId}' + '-' + ACAPCH.APRCHN_ID + '-' + '{approval_id_auto}'
            ) AS APPROVAL_ID,
            ...
            convert(
                VARCHAR(10),
                ACACST.CpqTableEntryDateModified,
                101
            ) AS CUR_APRCHNSTP_LASTACTIONDATE,
        ...
        FROM
            ...
            INNER JOIN SYOBJH(NOLOCK) ON ACAPCH.APROBJ_RECORD_ID = SYOBJH.RECORD_ID
        """.format(
    ...
    def ApprovalTranscationDataInsert(self, ApprovalChainRecordId=None, QuoteId=None, RoundKey=None, Round=None):
        ...
        INSERT
            ACAPTX (
                ...
                APRTRXOBJ_ID,
                APRCHN_ID,
                APPROVAL_TRANSACTION_RECORD_ID,
                APRCHN_RECORD_ID,
                APRCHNSTP_APPROVER_ID,
                APRCHNSTP_APPROVER_RECORD_ID,
                APRCHNSTP_ID,
                APRCHNSTP_NAME,
                APRCHNSTP_RECORD_ID,
                APRCHNSTP_STATUS_RECORD_ID,
                APRCHNSTPTRX_ID,
                APPROVAL_ID,
                APPROVAL_RECIPIENT,
                APPROVAL_RECIPIENT_RECORD_ID,
                APPROVAL_RECORD_ID,
                APPROVALSTATUS,
                APPROVE_TEMPLATE_ID,
                APPROVE_TEMPLATE_RECORD_ID,
                APPROVED_BY,
                APPROVEDBY_RECORD_ID,
                ARCHIVED,
                ASSIGNED_GROUP_ID,
                ASSIGNED_RECIPIENT,
                ASSIGNED_TO,
        ...
        SELECT
            DISTINCT '{roundkey}' AS APRCHNRND_RECORD_ID,
            {round} AS APPROVAL_ROUND,
            '{QuoteId}' AS APRTRXOBJ_ID,
            ACAPCH.APRCHN_ID AS APRCHN_ID,
            ...
            CONVERT(
                VARCHAR(4000),
                ACAPMA.APPROVAL_ID + '-' + CONVERT(VARCHAR(4000), ACACST.APRCHNSTP_NUMBER) + '-' + APPRO.APRCHNSTP_APPROVER_ID
            ) AS APRCHNSTPTRX_ID,
            ...
            INNER JOIN (
                ...
                FROM
                    ...
                    LEFT JOIN (
                        ...
                        WHERE
                            ACACSA.APRCHN_RECORD_ID = '{ApprovalChainRecordId}'
                        UNION
                        SELECT
                            NAME AS USER_NAME,
                        ...
                        WHERE
                            USERS_PERMISSIONS.permission_id = ACACSA.PROFILE_RECORD_ID
                            AND ACACSA.APRCHN_RECORD_ID = '{ApprovalChainRecordId}'
                        UNION
                        SELECT
                            NAME AS USER_NAME,
                            ID AS USER_RECORD_ID,
                    ...
                    AND usr.APPROVAL_CHAIN_STEP_APPROVER_RECORD_ID = ACACSA.APPROVAL_CHAIN_STEP_APPROVER_RECORD_ID
            ) AS APPRO ON APPRO.APRCHNSTP_RECORD_ID = ACACST.APPROVAL_CHAIN_STEP_RECORD_ID
            INNER JOIN ACAPMA (NOLOCK) ON ACAPCH.APPROVAL_CHAIN_RECORD_ID = ACAPMA.APRCHN_RECORD_ID
        """.format(
            Get_UserID=self.Get_UserID,
            datetime_value=self.datetime_value,
            UserName=self.Get_UserNAME,
            ApprovalChainRecordId=ApprovalChainRecordId,
            QuoteId=QuoteId,
            roundkey=RoundKey,
            round=Round,
        )
    ...
    def CustomApprovalTranscationDataInsert(self, ApprovalChainRecordId=None, QuoteId=None, RoundKey=None, Round=None, CustomQuery=None):
        ...
        SELECT
            ...
            CONVERT(
                VARCHAR(4000),
                ACAPMA.APPROVAL_ID + '-' + CONVERT(VARCHAR(4000), ACACST.APRCHNSTP_NUMBER) + '-' + SAQDLT.MEMBER_ID
            ) AS APRCHNSTPTRX_ID,
            ACAPMA.APPROVAL_ID AS APPROVAL_ID,
        ...
        FROM
            ...
            INNER JOIN ACACST(NOLOCK) ON ACAPCH.APPROVAL_CHAIN_RECORD_ID = ACACST.APRCHN_RECORD_ID
            INNER JOIN ACAPMA (NOLOCK) ON ACAPCH.APPROVAL_CHAIN_RECORD_ID = ACAPMA.APRCHN_RECORD_ID
            INNER JOIN ACACSS ON ACACSS.APRCHN_RECORD_ID = ACAPMA.APRCHN_RECORD_ID
            INNER JOIN SAQDLT(NOLOCK) ON SAQDLT.QTEREV_RECORD_ID = ACAPMA.APRTRXOBJ_RECORD_ID
            AND {CustomQuery}
        """.format(
            Get_UserID=self.Get_UserID,
            datetime_value=self.datetime_value,
    ...
    def TrackedValueDataInsert(self, objName, trackedfield, TrackedobjectApiName):
        GetKey = SqlHelper.GetList(
            """SELECT API_NAME FROM SYOBJD (NOLOCK) WHERE OBJECT_NAME ='{objName}' AND IS_KEY = 'True' """.format(objName=objName)
        )
    ...
    def TrackedValueDataUpdate(self, objName, ApprovelObjectId):
        ...
        GetAllApprovalMater = SqlHelper.GetList(
            ...
            WHERE
                APRTRXOBJ_RECORD_ID = '{ApprovelObjectId}'
                AND SYOBJD.OBJECT_NAME = '{objName}'
            """.format(
        ...
        for eachAllApprovalMater in GetAllApprovalMater:
            ...
            UPDATE
                ACAPFV
            SET
                TRKOBJ_TRACKEDFIELD_OLDVALUE = ACAPFV.TRKOBJ_TRACKEDFIELD_NEWVALUE,
            ...
            FROM
                (
                    ...
                    FROM
                        ...
                        INNER JOIN ACAPTF ON ACAPFV.TRKOBJ_TRACKEDFIELD_RECORD_ID = ACAPTF.TRKOBJ_TRACKEDFIELD_RECORD_ID
                        INNER JOIN SYOBJD(NOLOCK) ON SYOBJD.FIELD_LABEL = ACAPTF.TRKOBJ_TRACKEDFIELD_LABEL
                    WHERE
                        AGMREV_ID = '{ApprovelObjectId}'
                        AND OBJECT_NAME = '{objName}'
                ) TST
            WHERE
                ACAPFV.TRKOBJ_TRACKEDFIELD_RECORD_ID = '{TrackedFieldRecId}'
            """.format(
    ...
    def InsertAction(self, Objh_Id, RecordId=None, ObjectName=None, method=None):
        ...
        CHSqlObjs = Sql.GetList(
            "SELECT APPROVAL_CHAIN_RECORD_ID,APRCHN_ID FROM ACAPCH (NOLOCK) WHERE APROBJ_RECORD_ID = '" + str(Objh_Id) + "'"
        )
        for index, val in enumerate(CHSqlObjs):
            ...
            for result in CSSqlObjs:
                ...
                if FirstReturn is not None:
                    ...
                    roundd = 1
                    if QuoteId != "":
                        round_obj = Sql.GetFirst(
                            """
                        SELECT
                            TOP 1 APPROVAL_ROUND
                        FROM
                            ACACHR
                        WHERE
                            APPROVAL_ID LIKE '%{}%'
                        ...
                        ORDER BY
                            CpqTableEntryId DESC
                        """.format(
                                QuoteId, val.APPROVAL_CHAIN_RECORD_ID
                            )
                        )
                        if round_obj:
                    ...
                    INSERT INTO
                        ACACHR (
                            APPROVAL_CHAIN_ROUND_RECORD_ID,
                            TOTAL_CHNSTP,
                            TOTAL_APRTRX,
                            COMPLETED_DATE,
                            COMPLETEDBY_RECORD_ID,
                            COMPLETED_BY,
                            APPROVAL_ROUND,
                            APPROVAL_RECORD_ID,
                            APPROVAL_ID,
                    ...
                    if CheckViolaionRule2:
                        for result in CheckViolaionRule2:
                            ...
                            if SecondReturn:
                                ...
                                where_conditon += " AND ACAPMA.APPROVAL_RECORD_ID = '" + str(GetLatestApproval.APPROVAL_RECORD_ID) + "' "
                                if method is None:
                                    where_conditon += " AND ACACSS.APPROVALSTATUS = 'APPROVAL REQUIRED' "
                                    flag = 0
                                else:
                                ...
                                FROM
                                    ACACSA (NOLOCK)
                                WHERE
                                    APPROVER_SELECTION_METHOD = ' CUSTOM QUERY'
                                    AND APRCHN_RECORD_ID = '{}'
                                    AND APRCHNSTP_RECORD_ID = '{}'
                                """.format(
                                        str(val.APPROVAL_CHAIN_RECORD_ID), result.APPROVAL_CHAIN_STEP_RECORD_ID
                                    )
                                )
                                if getCustomQuery is not None:
                                    ...
                                    Transcationrulebody = self.CustomApprovalTranscationDataInsert(
                                        ApprovalChainRecordId=result.APPROVAL_CHAIN_RECORD_ID,
                                        QuoteId=QuoteId,
                                        RoundKey=primarykey,
                                        Round=roundd,
                                        CustomQuery=CustomQuery,
                                    ...
                                    if getCustomQuery.APRCHN_ID == "SELFAPPR":
                                        Sql.RunQuery(
                                            "UPDATE ACAPTX SET APPROVALSTATUS = 'REQUESTED' WHERE APPROVAL_RECORD_ID = '{}'".format(
                                                GetLatestApproval.APPROVAL_RECORD_ID
                                            )
                                        )

                                        Sql.RunQuery(
                                            "UPDATE SAQTRV SET REVISION_STATUS = 'APR-APPROVAL PENDING' WHERE QUOTE_REVISION_RECORD_ID ='{}'".format(
                                                RecordId
                                            )
                                        )
                                    ...
                                    where_conditon += """
                                    GROUP BY
                                        APPRO.USER_RECORD_ID,
                                        ACAPCH.APRCHN_ID,
                                        ACAPCH.APPROVAL_CHAIN_RECORD_ID,
                                        APPRO.APRCHNSTP_APPROVER_ID,
                                        APPRO.APPROVAL_CHAIN_STEP_APPROVER_RECORD_ID,
                                        ACACST.APRCHNSTP_NUMBER,
                                        ACACST.APPROVAL_CHAIN_STEP_RECORD_ID,
                                        ACACSS.APPROVAL_CHAIN_STATUS_MAPPING_RECORD_ID,
                                        ACAPMA.APPROVAL_ID,
                                        APPRO.USER_NAME,
                                        ACAPMA.APPROVAL_RECORD_ID,
                                        ACACSS.APPROVALSTATUS,
                                        ACACST.APPROVE_TEMPLATE_ID,
                                        ACACST.APPROVE_TEMPLATE_RECORD_ID,
                                        APPRO.DELEGATED_APPROVER_ID,
                                        ACACST.REJECT_TEMPLATE_ID,
                                        ACACST.REJECT_TEMPLATE_RECORD_ID,
                                        ACACST.REQUEST_TEMPLATE_ID,
                                        ACACST.REQUEST_TEMPLATE_RECORD_ID,
                                        ACACST.REQUIRE_EXPLICIT_APPROVAL,
                                        APPRO.UNANIMOUS_CONSENT,
                                ...
                                GetTrackedFields = Sql.GetList(
                                    """
                                    SELECT
                                        APPROVAL_TRACKED_FIELD_RECORD_ID,
                                        API_NAME,
                                        OBJECT_NAME
                                    FROM
                                        ACAPTF (NOLOCK)
                                        INNER JOIN SYOBJD (NOLOCK) ON ACAPTF.TRKOBJ_TRACKEDFIELD_RECORD_ID = SYOBJD.RECORD_ID
                                    WHERE
                                        ACAPTF.APRCHN_RECORD_ID = '{chainrecordId}'
                                ...
                                for trackedfield in GetTrackedFields:
                                    ...
                                    TackedRuleBody = self.TrackedValueDataInsert(Trackedobject, TrackedFieldName, TrackedobjectApiName)
                                    Tracked_where_conditon = """
                                    WHERE
                                        ACAPTF.APRCHN_RECORD_ID = '{chainrecordId}'
                                        AND ACAPTF.APRCHNSTP = '{chainstep}'
                    ...
                    if QuoteId != "":
                        transaction_count_obj = Sql.GetFirst(
                            """
                            SELECT
                                count(CpqTableEntryId) as cnt
                            from
                                ACAPTX
                            where
                                APRTRXOBJ_ID = '{}'
                                and APRCHNRND_RECORD_ID = '{}'
                        """.format(
                                QuoteId, primarykey
                            )
                        )

                        chnstp_count_obj = Sql.GetFirst(
                            """
                        SELECT
                            count(distinct APRCHNSTP_ID) as cnt
                        from
                            ACAPTX
                        where
                            APRTRXOBJ_ID = '{}'
    ...
    """.format(
                        ...
                        UPDATE
                            ACACHR
                        SET
                            ACACHR.TOTAL_APRTRX = {total},
                            ACACHR.TOTAL_CHNSTP = {totalchnstp},
                            ACACHR.APRCHN_NAME = ACAPCH.APRCHN_NAME,
                            ACACHR.APPROVAL_RECORD_ID = ACAPTX.APPROVAL_RECORD_ID,
                            ACACHR.APPROVAL_ID = ACAPTX.APPROVAL_ID,
                            ACACHR.APRCHN_RECORD_ID = ACAPTX.APRCHN_RECORD_ID,
                            ACACHR.APRCHN_ID = ACAPTX.APRCHN_ID
                        FROM
                            ACAPTX
                            INNER JOIN ACAPCH (NOLOCK) ON ACAPCH.APPROVAL_CHAIN_RECORD_ID = ACAPTX.APRCHN_RECORD_ID
                            INNER JOIN ACACHR ON ACAPTX.APRCHNRND_RECORD_ID = ACACHR.APPROVAL_CHAIN_ROUND_RECORD_ID
                        WHERE
                            ACAPTX.APRTRXOBJ_ID = '{quoteId}'
                            AND ACACHR.APPROVAL_CHAIN_ROUND_RECORD_ID = '{primarykey}'
                        """.format(
                                quoteId=QuoteId, primarykey=primarykey, total=transaction_count_obj.cnt, totalchnstp=chnstp_count_obj.cnt
                            )
                        )
                else:
                    Log.Info("else @758")
    ...
    def AutoApproval(self, revisionId, segmentId):
        ...
        sqlObjs = Sql.GetFirst(
            ...
            from
                PAPBEN (nolock)
                inner join PASPCC (nolock) on PAPBEN.AGMREV_ID = PASPCC.AGMREV_ID
                AND PAPBEN.PRICEAGREEMENT_ID = PASPCC.PRICEAGREEMENT_ID
            where
                PAPBEN.PRICEAGREEMENT_ID = '{segmentId}'
                AND PAPBEN.AGMREV_ID = '{revisionId}'
               """.format(
        ...
        if sqlObjs and int(sqlObjs.cnt) > 0:
            ...
            FROM
                PASGRV (nolock) QH
            WHERE
                QH.AGMREV_ID = '{revisionId}'
                AND QH.PRICEAGREEMENT_ID = '{segmentId}'
                            """.format(
                revisionId=revisionId, segmentId=segmentId
            )
        ...
        if result.APRCHN_ID != "SELFAPPR":
            for x in GetACACSF:
                ...
                if config_entry:
                    ...
                    selectQuery = self.snap_query_config["default"].format(
                        x.TSTOBJ_TESTEDFIELD_LABEL,
                        x.TSTOBJ_LABEL,
                        x.TSTOBJ_TESTEDFIELD_LABEL,
                        self.operators_config[x.CMP_OPERATOR],
                        x.CMP_VALUE,
                    )
                # ----------------------------------------
            ...
            if result.CONDITIONS_MET in ["ANY", "ALL"]:
                ...
                getMax = Sql.GetFirst(
                    """
                    SELECT
                        MAX(APRCHNSTP_NUMBER_CONDITION) AS MAX_COUNT
                    FROM
                        ACACSF (NOLOCK)
                    WHERE
                        APRCHNSTP_RECORD_ID = '{}'
    ...
    def SnapshotConditions(self, result, RecordId, QuoteId, ApprovalRecordId):
        ...
        for x in GetACACSF:
            ...
            SELECT
                DISTINCT CONVERT(VARCHAR(4000), NEWID()) AS APPROVAL_TRANSACTION_SNAPSHOT_RECORD_ID,
                ACAPTX.APRCHN_ID,
            ...
            FROM
                ACAPTX (NOLOCK)
                JOIN ACACSF (NOLOCK) ON ACAPTX.APRCHNSTP_RECORD_ID = ACACSF.APRCHNSTP_RECORD_ID
                AND ACAPTX.APPROVAL_RECORD_ID = '{}'
            """.format(
