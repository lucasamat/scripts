...
from SYDATABASE import sql_get_first, sql_get_list, sql_run_query
import CQPARTIFLW
import CQCPQC4CWB
import time
...
def Dynamic_Status_Bar(quote_item_insert,Text):
    ...
    if Text == 'COMPLETE STAGE' and current_prod == "Sales" and (TabName in ["Quotes", "Quote"]):
        ...
        get_quality_required = Sql.GetList("SELECT QUALITY_REQUIRED FROM SAQFBL where QUOTE_RECORD_ID = '{}' AND QTEREV_RECORD_ID = '{}'".format(contract_quote_rec_id,quote_revision_record_id))
            ...
            if service_id_query:
                for service_id in service_id_query:
                    ...
                    if get_ent_config_status.COUNT > 0 or service_id.MATERIALCONFIG_TYPE =='SIMPLE MATERIAL' or service_id.SERVICE_ID == 'Z0117':
                        ScriptExecutor.ExecuteGlobal("CQINSQTITM",{"ContractQuoteRecordId":contract_quote_rec_id, "ContractQuoteRevisionRecordId":quote_revision_record_id, "ServiceId":service_id.SERVICE_ID, "ActionType":'INSERT_LINE_ITEMS'})
