...
try:
    def getting_cps_tax(service_id):
		...
		x= str(x)
		y = x.split(" ")
		contract_quote_obj = Sql.GetFirst("Select * from SAQTMT WHERE MASTER_TABLE_QUOTE_RECORD_ID ='{QuoteRecordId}' AND QTEREV_RECORD_ID = '{QuoteRevisionRecordId}'".format(QuoteRecordId = quote_revision_record_id,QuoteRevisionRecordId=quote_revision_record_id))
