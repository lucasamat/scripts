					...
					for service_obj in services_obj:
						...
						if get_billing_type:
							...
							if str(get_billing_types).upper() =='FIXED':
								...
								get_ttl_amt = 'BILLING_VALUE'
						gett = Sql.GetList("""SELECT DISTINCT TOP 1000 * FROM ( SELECT * FROM (   SELECT ROW_NUMBER() OVER(ORDER BY EQUIPMENT_ID) AS ROW, SERVICE_ID,EQUIPMENT_ID,SERIAL_NUMBER,GREENBOOK,{get_ttl_amt},{BillingYear} as BILLING_YEAR,ANNUAL_BILLING_AMOUNT,QUOTE_RECORD_ID,GREENBOOK_RECORD_ID,QTEREV_RECORD_ID,EQUIPMENT_RECORD_ID,SERVICE_RECORD_ID,[WARRANTY_END_DATE],
						{SelectDateColoumn}    FROM ( SELECT       SERVICE_ID,EQUIPMENT_ID,SERIAL_NUMBER,GREENBOOK,BILLING_VALUE,{get_ttl_amt},BILLING_YEAR,ANNUAL_BILLING_AMOUNT,QUOTE_RECORD_ID,GREENBOOK_RECORD_ID,QTEREV_RECORD_ID,EQUIPMENT_RECORD_ID,SERVICE_RECORD_ID,CONVERT(VARCHAR(10),WARRANTY_END_DATE,101) AS [WARRANTY_END_DATE],CONVERT(VARCHAR(10),FORMAT(BILLING_DATE,'MM-dd-yyyy'),101) AS [BILLING_DATE]    FROM SAQIBP     {WhereString}) AS IQ
						PIVOT (    SUM(BILLING_VALUE)    FOR BILLING_DATE IN ({PivotColumns})    )AS PVT   ) OQ WHERE ROW BETWEEN 1 AND 1000 ) AS FQ ORDER BY EQUIPMENT_ID""".format(WhereString=Qustr,PivotColumns=pivot_columns,SelectDateColoumn=select_date_columns,get_ttl_amt=get_ttl_amt,BillingYear=no_of_year))
