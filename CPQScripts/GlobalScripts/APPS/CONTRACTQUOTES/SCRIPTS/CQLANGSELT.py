...
def _insert_subtotal_by_offerring_quote_table():
	...
	insrt_dummy_offering = ("""INSERT QT__SAQRIS (LINE,SERVICE_DESCRIPTION,SERVICE_ID,QUOTE_ID,QUOTE_RECORD_ID,ownerId, cartId) select SAQRIS.LINE,SAQRIS.SERVICE_DESCRIPTION,SAQRIS.SERVICE_ID,SAQRIS.QUOTE_ID,SAQRIS.QUOTE_RECORD_ID,{UserId} as ownerId,{CartId} as cartId from SAQRIS (NOLOCK)  where SAQRIS.QUOTE_RECORD_ID ='{c4c_quote_id}' """.format(CartId = cartobj.CART_ID,UserId= cartobj.USERID,c4c_quote_id = contract_quote_record_id))
	Sql.RunQuery(insrt_dummy_offering)
