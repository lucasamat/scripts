					...
					for service_obj in services_obj:
						...
						if get_billing_type:
							...
							if str(get_billing_types).upper() =='FIXED':
								...
								get_ttl_amt = 'BILLING_VALUE'
						S = SqlHelper.GetList("""SELECT DISTINCT TOP 1000 * FROM ( SELECT * FROM (
																				SELECT ROW_NUMBER() OVER(ORDER BY EQUIPMENT_ID)
																				AS ROW, SERVICE_ID,EQUIPMENT_ID,SERIAL_NUMBER,GREENBOOK,ESTVAL_INGL_CURR,1 as BILLING_YEAR,ANNUAL_BILLING_AMOUNT,QUOTE_RECORD_ID,GREENBOOK_RECORD_ID,QTEREV_RECORD_ID,EQUIPMENT_RECORD_ID,SERVICE_RECORD_ID,[WARRANTY_END_DATE], {SelectDateColoumn}
																						FROM (
																								SELECT
																				...
																				) OQ WHERE ROW BETWEEN 1 AND 1000 ) AS FQ ORDER BY EQUIPMENT_ID""".format(PivotColumns=pivot_columns,WhereString=Qustr,SelectDateColoumn=select_date_columns))
