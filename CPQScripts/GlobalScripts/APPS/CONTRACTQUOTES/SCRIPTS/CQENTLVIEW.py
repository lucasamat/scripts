...
class EntitlementView():
    ...
    def contract_entitlement_view(
        ...
        if EntitlementType == "EQUIPMENT":
            ...
            TableObj = Sql.GetFirst("select * from CTCSGE (NOLOCK) where CONTRACT_RECORD_ID = '" + str(quoteid) + "' AND SERVICE_ID = '" + str(self.treeparentparam) + "'")
            if TableObj is not None:
                RECORD_ID = str(TableObj.SERVICE_RECORD_ID)
        ...
        if self.treesuperparentparam == "Product Offerings":
            # requestdata= '{"productKey":"'+self.treeparam+'","date":"2020-10-14","context":[{"name":"VBAP-MATNR","value":"'+self.treeparam+'"}]}'
            ProductPartnumber = self.treeparam
        elif self.treetopsuperparentparam == "Product Offerings":		
            # requestdata= '{"productKey":"'+self.treeparentparam+'","date":"2020-09-01","context":[{"name":"VBAP-MATNR","value":"'+self.treeparentparam+'"}]}'		
            ProductPartnumber = self.treeparentparam
        elif self.treeparentparam == "Cart Items":
            GetItem = Sql.GetFirst("select * from CTCICO (NOLOCK) where CONTRACT_ITEM_COVERED_OBJECT_RECORD_ID = '" + str(RECORD_ID) + "'")
            if GetItem is not None:
                ProductPartnumber = GetItem.SERVICE_ID
            if "-" in self.treeparam:
                self.treeparam = self.treeparam.split("-")[1].strip()
        elif self.treesuperparentparam == "Cart Items":
            self.treeparentparam = self.treeparentparam.split("-")[1].strip()
            ProductPartnumber = self.treeparentparam
        ...
        if TableObj is None and EntitlementType == "EQUIPMENT":  # or len(TableObj)>0:
            ...
            for row in ServiceContainer.Rows:
                if self.treeparam.upper() == str(row.Product.PartNumber).upper():
