	...
	def entitlement_view(
		...
		if self.treeparam == "Quote Items":
			...
			if quote_item_revision_rec_id:
				...
				TableObj = Sql.GetFirst("select * from SAQITE (NOLOCK) where QUOTE_RECORD_ID = '" + str(quoteid) + "' AND QTEREV_RECORD_ID = '"+str(self.quote_revision_record_id)+"' AND SERVICE_ID = '" + str(ProductPartnumber) + "' AND QTEITM_RECORD_ID = '" + str(self.quote_item_revision_rec_id) + "' ")
