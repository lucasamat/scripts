...
class EntitlementView():
	...
	def entitlement_view(
		...
		if TableObj is None and (EntitlementType == "EQUIPMENT"): 
			...
			Trace.Write('767-----where-----'+str(where))
			#ScriptExecutor.ExecuteGlobal("CQENTLNVAL", {'where_cond':where,'partnumber':ProductPartnumber,'ent_level_table':ObjectName,'inserted_value_list':inserted_value_list})
