											...
											for value in STDVALUES:
												...
												try:
													...
													if str(val.ENTITLEMENT_DISPLAY_VALUE).strip() == str(value.STANDARD_ATTRIBUTE_DISPLAY_VAL).strip():
														...
														Trace.Write(str(disallow_style)+'--disallow_style----'+str(disallow_style)+'---disallow_style--1025--'+str(value.STANDARD_ATTRIBUTE_DISPLAY_VAL)+'drpppppp---3031---durga---3342-----'+str(attrName))
														VAR1 += (
															'<option '
															+ str(disallow_style)
