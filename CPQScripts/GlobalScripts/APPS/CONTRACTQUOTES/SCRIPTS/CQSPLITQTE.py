...
def servicelevel_split_equip(seid):
	...
	Sql.RunQuery("""UPDATE SAQICO SET 
					...
					TAMTDC = P_SAQICO.TAMTDC			
					FROM SAQICO (NOLOCK) 
					JOIN (SELECT QUOTE_RECORD_ID,QTEREV_RECORD_ID,FABLOCATION_ID,GREENBOOK,EQUIPMENT_ID,SPSPCT,SPCTPR,SPCTCS,SPCTMG,SVSPCT,SVCTPR,SVCTCS,SVCTMG,STATUS,TAMTDC FROM SAQICO WHERE SAQICO.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQICO.QTEREV_RECORD_ID = '{QuoteRevisionRecordId}' AND SAQICO.SERVICE_ID = '{ParentServiceId}' AND ISNULL(SAQICO.SPQTEV,'No') = 'Yes') P_SAQICO ON P_SAQICO.QUOTE_RECORD_ID = SAQICO.QUOTE_RECORD_ID AND P_SAQICO.QTEREV_RECORD_ID = SAQICO.QTEREV_RECORD_ID AND P_SAQICO.FABLOCATION_ID = SAQICO.FABLOCATION_ID AND P_SAQICO.GREENBOOK = SAQICO.GREENBOOK AND P_SAQICO.EQUIPMENT_ID = SAQICO.EQUIPMENT_ID
...
def servicelevel_split_green(seid):
	...
	Sql.RunQuery("""UPDATE SAQICO SET 
					...
					TAMTDC = P_SAQICO.TAMTDC			
					FROM SAQICO (NOLOCK) 
					JOIN (SELECT QUOTE_RECORD_ID,QTEREV_RECORD_ID,FABLOCATION_ID,GREENBOOK,SPSPCT,SPCTPR,SPCTCS,SPCTMG,SVSPCT,SVCTPR,SVCTCS,SVCTMG,STATUS,TAMTDC FROM SAQICO WHERE SAQICO.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQICO.QTEREV_RECORD_ID = '{QuoteRevisionRecordId}' AND SAQICO.SERVICE_ID = '{ParentServiceId}' AND ISNULL(SAQICO.SPQTEV,'No') = 'Yes') P_SAQICO ON P_SAQICO.QUOTE_RECORD_ID = SAQICO.QUOTE_RECORD_ID AND P_SAQICO.QTEREV_RECORD_ID = SAQICO.QTEREV_RECORD_ID AND P_SAQICO.FABLOCATION_ID = SAQICO.FABLOCATION_ID AND P_SAQICO.GREENBOOK = SAQICO.GREENBOOK
					WHERE SAQICO.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQICO.QTEREV_RECORD_ID = '{QuoteRevisionRecordId}' AND SAQICO.SERVICE_ID = '{ServiceId}' AND ISNULL(SAQICO.SPQTEV,'No') = 'No'
				""".format(QuoteRecordId=contract_quote_rec_id,QuoteRevisionRecordId=quote_revision_rec_id,ServiceId=splitservice_id,ParentServiceId=parent_service_id)
	)
