...
def servicelevel_split_equip(seid):
	...
	Sql.RunQuery("""UPDATE SAQICO SET 
					SPSPCT = SAQICO.SPSPCT,
					SPCTPR = SAQICO.SPCTPR,
					SPCTCS = SAQICO.SPCTCS,
					SPCTMG = SAQICO.SPCTMG,	
					SVSPCT = SAQICO.SVSPCT,
					SVCTPR = SAQICO.SVCTPR,
					SVCTCS = SAQICO.SVCTCS,
					SVCTMG = SAQICO.SVCTMG			
					FROM SAQICO (NOLOCK) 
					JOIN (SELECT QUOTE_RECORD_ID,QTEREV_RECORD_ID,FABLOCATION_ID,GREENBOOK,EQUIPMENT_ID FROM SAQICO WHERE SAQICO.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQICO.QTEREV_RECORD_ID = '{QuoteRevisionRecordId}' AND SAQICO.SERVICE_ID = '{ParentServiceId}' AND ISNULL(SAQICO.SPQTEV,'No') = 'Yes') P_SAQICO ON P_SAQICO.QUOTE_RECORD_ID = SAQICO.QUOTE_RECORD_ID AND P_SAQICO.QTEREV_RECORD_ID = SAQICO.QTEREV_RECORD_ID AND P_SAQICO.FABLOCATION_ID = SAQICO.FABLOCATION_ID AND P_SAQICO.GREENBOOK = SAQICO.GREENBOOK AND P_SAQICO.EQUIPMENT_ID = SAQICO.EQUIPMENT_ID
