	...
	def CalculatePlusDiscount(self):
		...
		Sql.RunQuery("""UPDATE SAQIRT
							...
							INNER JOIN (SELECT SAQIRT.CpqTableEntryId,
										...
										FROM SAQIRT (NOLOCK) 
										JOIN SAQICO (NOLOCK) ON SAQICO.QUOTE_RECORD_ID = SAQIRT.QUOTE_RECORD_ID  AND SAQICO.QTEREV_RECORD_ID = SAQIRT.QTEREV_RECORD_ID AND SAQICO.LINE_ITEM_ID = SAQIRT.LINE_ITEM_ID
