...
def QuoteAssemblyPreventiveMaintainenceParent(PerPage, PageInform, A_Keys, A_Values,ASSEMBLYID,EQUIPMENTID):
    ...
    if TopSuperParentParam in ('Comprehensive Services','Complementary Products'): 
        ...
        dbl_clk_function += (	
            ...
            + str(table_id)	 
            + '"); cpq.server.executeScript("SYBLKETRLG", {"TITLE":field, "VALUE":value, "CLICKEDID":"SYOBJR_00011_271F55CA_C844_43C5_99AB_806A72152F25", "RECORDID":reco_id, "ELEMENT":"RELATEDEDIT"}, function(data) { debugger; data1=data[0]; data2=data[1]; data3 = data[2];if(data1 != "NO"){ if(document.getElementById("RL_EDIT_DIV_ID") ) { localStorage.setItem("saqico_title", field); inp = "input#"+data3;localStorage.setItem("value_tag", inp); $("#SYOBJR_00011_271F55CA_C844_43C5_99AB_806A72152F25").find(inp).prop("disabled", false); var buttonlen = $("#seginnerbnr").find("button#saveButton"); if (buttonlen.length == 0){	RecId = "SYOBJR-00011";RecName = "";$("#seginnerbnr").append("<button class=\'btnconfig\' onclick=\'PreventiveMaintainenceTreeTable();\' type=\'button\' value=\'Cancel\' id=\'cancelButton\'>CANCEL</button><button class=\'btnconfig\' type=\'button\' value=\'Save\' onclick=\'multiedit_save_RL()\' id=\'saveButton\'>SAVE</button>");$("#SYOBJR_00011_271F55CA_C844_43C5_99AB_806A72152F25").find(inp).addClass("light_yellow");} document.getElementById("cont_multiEditModalSection").style.display = "none";  var divHeight = $("#cont_multiEditModalSection").height(); $("#cont_multiEditModalSection .modal-backdrop").css("min-height", divHeight+"px"); $("#cont_multiEditModalSection .modal-dialog").css("width","550px"); $(".modal-dialog").css("margin-top","100px"); } if (data2.length !== 0){ $.each( data2, function( key, values ) { onclick_datepicker(values) }); } } }); }                   $("'	
            + str(table_ids)	
