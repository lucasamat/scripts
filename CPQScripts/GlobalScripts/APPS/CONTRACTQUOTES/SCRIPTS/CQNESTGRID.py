    ...
    if Parent_Equipmentid:
        ...
        if sale_type != "TOOL RELOCATION":
            ...
            if TreeParentParam == 'Complementary Products':
                ...
                edit_button = '<button class = "m-5px" id="assembly_edit" onclick="SendEqupAssemblyEdit()" class="btnconfig">EDIT</button><button class = "m-5px" id="assembly_save" style="display:none" onclick="SendEqupAssemblySave()" class="btnconfig">SAVE</button><button class = "mr-0" id="assembly_cancel" style="display:none" onclick="SendEqupAssemblyCancel()" class="btnconfig">CANCEL</button>'
...
def GetCovObjMaster(PerPage, PageInform, A_Keys, A_Values):    
    ...
    if TreeSuperParentParam == "Product Offerings" or (TreeTopSuperParentParam== "Comprehensive Services" and TreeParentParam=="Add-On Products"):
        Qstr = (
            "select top "
            + str(PerPage)
            + " * from ( select ROW_NUMBER() OVER( ORDER BY "+str(orderby)+") AS ROW, QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID,EQUIPMENT_ID,EQUIPMENT_DESCRIPTION,SERIAL_NO,GREENBOOK,FABLOCATION_ID,WARRANTY_END_DATE,WARRANTY_END_DATE_ALERT,WARRANTY_START_DATE,MNT_PLANT_ID,EQUIPMENT_STATUS,CUSTOMER_TOOL_ID,EQUIPMENTCATEGORY_DESCRIPTION AS EQUIPMENT_CATEGORY_DESCRIPTION,SNDFBL_ID from SAQSCO (NOLOCK) where QUOTE_RECORD_ID = '"
            + str(ContractRecordId)
            + "' and SERVICE_ID = '"
            + str(TreeParam)
            + "' and SERVICE_TYPE = '"
    ...
    for par in parent_obj:
        ...
        data_dict["WARRANTY_START_DATE"] = ('<abbr id ="" title="' + str(par.WARRANTY_START_DATE) + '">' + str(par.WARRANTY_START_DATE) + "</abbr>")
        data_dict["WARRANTY_END_DATE"] = ('<abbr id ="" title="' + str(par.WARRANTY_END_DATE) + '">' + str(par.WARRANTY_END_DATE) + "</abbr>") 
        data_dict["WARRANTY_END_DATE_ALERT"] = ('<abbr id ="" title="' + str(par.WARRANTY_END_DATE_ALERT) + '">' + str(par.WARRANTY_END_DATE_ALERT) + "</abbr>") 
        data_list.append(data_dict)
