...
def GetEquipmentMaster(PerPage, PageInform, A_Keys, A_Values):
	...
	for par in parent_obj:
		...
		if TreeParam != 'Fab Locations':       
			...
			+ str(data_id)
			+ '" onclick="Commonteree_view_RL(this)">VIEW</a></li>'
			'<li style="display:none"><a class="dropdown-item" id="deletebtn" data-target="#cont_CommonModalDelete" data-toggle="modal" onclick="CommonDelete(this, \'SAQFEA#'+ data_id +'\', \'WARNING\')" href="#">DELETE</a></li>')
...
def GetEquipmentMasterFilter(ATTRIBUTE_NAME, ATTRIBUTE_VALUE,PerPage,PageInform):
	...
	if ATTRIBUTE_VALUE is None or ATTRIBUTE_VALUE == "" or ATTRIBUTE_VALUE_STR is None or ATTRIBUTE_VALUE_STR == "":
		...
		if TreeParam == 'Fab Locations':
			...
			"select top "+str(PerPage)+" QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID,SALESORG_ID,EQUIPMENTCATEGORY_ID,EQUIPMENT_ID,MNT_PLANT_ID,CUSTOMER_TOOL_ID,SERIAL_NUMBER,GREENBOOK,QUOTE_NAME,SALESORG_NAME,EQUIPMENT_DESCRIPTION,EQUIPMENT_STATUS,PLATFORM,FABLOCATION_ID,WARRANTY_START_DATE,QUOTE_ID,FABLOCATION_NAME,WARRANTY_END_DATE,TEMP_TOOL from SAQFEQ (NOLOCK) where QUOTE_RECORD_ID = '"
			+ str(ContractRecordId)
			+ "' AND QTEREV_RECORD_ID = '{}' ".format(str(Quote.GetGlobal("quote_revision_record_id")))
			+ " ORDER BY "
			+ str(orderby)
			...
			if (("Sending Account -" in TreeParam) or ("Receiving Account -" in TreeParam)) and TreeParentParam == 'Fab Locations':
				...
				if get_fab_query:
					...
					get_fab = "= ''"
				parent_obj = Sql.GetList(
					"select top "+str(PerPage)+" QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID,SALESORG_ID,EQUIPMENTCATEGORY_ID,EQUIPMENT_ID,MNT_PLANT_ID,CUSTOMER_TOOL_ID,SERIAL_NUMBER,GREENBOOK,QUOTE_NAME,SALESORG_NAME,EQUIPMENT_DESCRIPTION,EQUIPMENT_STATUS,PLATFORM,FABLOCATION_ID,WARRANTY_START_DATE,QUOTE_ID,FABLOCATION_NAME,WARRANTY_END_DATE,TEMP_TOOL from SAQFEQ (NOLOCK) where QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and FABLOCATION_ID {get_fab} and RELOCATION_EQUIPMENT_TYPE = '{equp_type}' ORDER BY {orderby}".format(orderby = orderby, ContractRecordId = ContractRecordId,RevisionRecordId = Quote.GetGlobal("quote_revision_record_id"), get_fab = get_fab,equp_type = 'SENDING EQUIPMENT' if "Sending Account -" in TreeParam else "RECEIVING EQUIPMENT" if "Receiving Account -" in TreeParam else "")
				)
				...
				parent_obj = Sql.GetList(
					"select top {PerPage} QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID,SALESORG_ID,EQUIPMENTCATEGORY_ID,EQUIPMENT_ID,MNT_PLANT_ID,CUSTOMER_TOOL_ID,SERIAL_NUMBER,GREENBOOK,QUOTE_NAME,SALESORG_NAME,EQUIPMENT_DESCRIPTION,EQUIPMENT_STATUS,PLATFORM,FABLOCATION_ID,WARRANTY_START_DATE,QUOTE_ID,FABLOCATION_NAME,WARRANTY_END_DATE,TEMP_TOOL from SAQFEQ (NOLOCK) where QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and FABLOCATION_ID = '{TreeParam}' and RELOCATION_EQUIPMENT_TYPE = '{equp_type}' ORDER BY {orderby}".format(PerPage=str(PerPage),ContractRecordId=str(ContractRecordId),RevisionRecordId = Quote.GetGlobal("quote_revision_record_id"),TreeParam=str(TreeParam),equp_type = 'SENDING EQUIPMENT' if "Sending Account -" in TreeParentParam else "RECEIVING EQUIPMENT" if "Receiving Account -" in TreeParentParam else "",orderby=str(orderby))
				)
		...
		if TreeSuperParentParam == "Fab Locations":
			if "Sending Account -" in TreeParentParam or "Receiving Account -" in TreeParentParam:
				if "Sending Account -" in TreeParentParam:
					...
					equp_type = "RECEIVING EQUIPMENT"
				parent_obj = Sql.GetList(
					"select top "+str(PerPage)+" QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID,SALESORG_ID,EQUIPMENTCATEGORY_ID,EQUIPMENT_ID,MNT_PLANT_ID,CUSTOMER_TOOL_ID,SERIAL_NUMBER,GREENBOOK,QUOTE_NAME,SALESORG_NAME,EQUIPMENT_DESCRIPTION,EQUIPMENT_STATUS,PLATFORM,FABLOCATION_ID,WARRANTY_START_DATE,QUOTE_ID,FABLOCATION_NAME,WARRANTY_END_DATE,TEMP_TOOL from SAQFEQ (NOLOCK) where "
				...
				parent_obj = Sql.GetList(
					"select top "+str(PerPage)+"  QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID,EQUIPMENTCATEGORY_ID,SERIAL_NUMBER,CUSTOMER_TOOL_ID,GREENBOOK,EQUIPMENT_STATUS,PLATFORM,MNT_PLANT_ID,EQUIPMENT_ID,EQUIPMENT_DESCRIPTION,FABLOCATION_ID,FABLOCATION_NAME,QUOTE_ID,QUOTE_NAME,SALESORG_ID,SALESORG_NAME,WARRANTY_START_DATE,WARRANTY_END_DATE,WAFER_SIZE,TEMP_TOOL from SAQFEQ (NOLOCK) where "
					+ str(ATTRIBUTE_VALUE_STR)
					+ " 1=1 and QUOTE_RECORD_ID = '"
					+ str(ContractRecordId)
					+ "' and QTEREV_RECORD_ID = '"
...
def GetFtsAssembliesChildFilter(ATTRIBUTE_NAME, ATTRIBUTE_VALUE, RECID):
	...
	if ATTRIBUTE_VALUE is None or ATTRIBUTE_VALUE == "" or ATTRIBUTE_VALUE_STR is None or ATTRIBUTE_VALUE_STR == "":
		parent_obj = Sql.GetList(
			"select QUOTE_REV_PO_GRNBK_PM_EVEN_ASSEMBLIES_RECORD_ID,EQUIPMENT_ID,ASSEMBLY_ID,ASSEMBLY_DESCRIPTION,GOT_CODE, EQUIPMENT_DESCRIPTION from SAQGPA (NOLOCK) where EQUIPMENT_ID = '{recid}' and QUOTE_RECORD_ID = '{ContractRecordId}'  and QTEREV_RECORD_ID = '{RevisionRecordId}' and SERVICE_ID = '{TreeParam}'".format(
...
def GetCovObjMaster(PerPage, PageInform, A_Keys, A_Values):    
	...
	if TreeSuperParentParam == "Product Offerings" or (TreeTopSuperParentParam== "Comprehensive Services" and TreeParentParam=="Add-On Products"):
		Qstr = (
			"select top "
			+ str(PerPage)
			+ " * from ( select ROW_NUMBER() OVER( ORDER BY "+str(orderby)+") AS ROW, QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID,EQUIPMENT_ID,EQUIPMENT_DESCRIPTION,PLATFORM,SERIAL_NO,GREENBOOK,FABLOCATION_ID,WARRANTY_END_DATE,WARRANTY_END_DATE_ALERT,WARRANTY_START_DATE,CONTRACT_VALID_FROM,CONTRACT_VALID_TO,MNT_PLANT_ID,EQUIPMENT_STATUS,CUSTOMER_TOOL_ID,EQUIPMENTCATEGORY_DESCRIPTION AS EQUIPMENTCATEGORY_DESCRIPTION,SNDFBL_ID,TEMP_TOOL from SAQSCO (NOLOCK) where QUOTE_RECORD_ID = '"
			+ str(ContractRecordId)
			+ "' and QTEREV_RECORD_ID = '"
			+ str(RevisionRecordId)
			+ "' and SERVICE_ID = '"
...
def GetCovObjMasterFilter(ATTRIBUTE_NAME, ATTRIBUTE_VALUE,PerPage,PageInform):
	...
	if ATTRIBUTE_VALUE is None or ATTRIBUTE_VALUE == "" or ATTRIBUTE_VALUE_STR is None or ATTRIBUTE_VALUE_STR == "":
		...
		if TreeSuperParentParam == "Product Offerings" or TreeParentParam == "Add-On Products":
			...
			if TreeTopSuperParentParam == "Product Offerings":
				if TreeParam == "Receiving Equipment":
					parent_obj = Sql.GetList(
						"SELECT top "+str(PerPage)+" QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID,EQUIPMENTCATEGORY_DESCRIPTION,EQUIPMENT_ID,EQUIPMENT_DESCRIPTION,SERIAL_NO,GREENBOOK,FABLOCATION_ID, WARRANTY_END_DATE,WARRANTY_START_DATE,MNT_PLANT_ID,EQUIPMENT_STATUS,CUSTOMER_TOOL_ID,EQUIPMENTCATEGORY_ID AS DESCRIPTION,WARRANTY_END_DATE_ALERT,TEMP_TOOL from SAQSCO (NOLOCK) where  QUOTE_RECORD_ID = '"
						+ str(ContractRecordId)
						+ "' and QTEREV_RECORD_ID = '"
						+ str(RevisionRecordId)
						+ "' AND SERVICE_ID = '"
				...
				if TreeParam == "Receiving Equipment":
					parent_obj = Sql.GetList(
						"SELECT top "+str(PerPage)+" QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID,EQUIPMENTCATEGORY_DESCRIPTION,EQUIPMENT_ID,EQUIPMENT_DESCRIPTION,SERIAL_NO,GREENBOOK,FABLOCATION_ID, WARRANTY_END_DATE,WARRANTY_START_DATE,MNT_PLANT_ID,EQUIPMENT_STATUS,CUSTOMER_TOOL_ID,"+str(equipment_column)+",WARRANTY_END_DATE_ALERT,TEMP_TOOL from SAQSCO (NOLOCK) where "+ str(ATTRIBUTE_VALUE_STR)+ " 1=1 and QUOTE_RECORD_ID = '"+ str(ContractRecordId)+ "' and QTEREV_RECORD_ID = '"+ str(RevisionRecordId)+ "' AND SERVICE_TYPE = '"+ str(TreeTopSuperParentParam)+ "' AND SERVICE_ID ='"+ str(TreeSuperParentParam)+ "' ORDER BY "  + str(orderby))
					...
					parent_obj = Sql.GetList(
						"SELECT top "+str(PerPage)+" QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID,EQUIPMENTCATEGORY_DESCRIPTION,EQUIPMENT_ID,EQUIPMENT_DESCRIPTION,SERIAL_NO,GREENBOOK,FABLOCATION_ID, WARRANTY_END_DATE,WARRANTY_START_DATE,MNT_PLANT_ID,EQUIPMENT_STATUS,CUSTOMER_TOOL_ID,"+str(equipment_column)+",WARRANTY_END_DATE_ALERT,TEMP_TOOL from SAQSCO (NOLOCK) where "
						+ str(ATTRIBUTE_VALUE_STR)
						+ " 1=1 and QUOTE_RECORD_ID = '"
						+ str(ContractRecordId)
		...
		if TreeSuperParentParam == "Product Offerings" or TreeParentParam == "Add-On Products":
			parent_obj = Sql.GetList(
				"SELECT top "+str(PerPage)+" QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID,EQUIPMENTCATEGORY_DESCRIPTION,EQUIPMENT_ID,EQUIPMENT_DESCRIPTION,SERIAL_NO,GREENBOOK,FABLOCATION_ID, WARRANTY_END_DATE,WARRANTY_START_DATE,MNT_PLANT_ID,EQUIPMENT_STATUS,CUSTOMER_TOOL_ID,EQUIPMENTCATEGORY_ID,EQUIPMENTCATEGORY_ID AS DESCRIPTION,SNDFBL_ID,WARRANTY_END_DATE_ALERT,TEMP_TOOL from SAQSCO (NOLOCK) where "
				+ str(ATTRIBUTE_VALUE_STR)
				+ " 1=1 and QUOTE_RECORD_ID = '"
				+ str(ContractRecordId)
				+ "' and QTEREV_RECORD_ID = '"
			...
			if TreeTopSuperParentParam == "Product Offerings":
				if TreeParam == "Receiving Equipment":
					parent_obj = Sql.GetList(
						"SELECT top "+str(PerPage)+" QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID,EQUIPMENTCATEGORY_DESCRIPTION,EQUIPMENT_ID,EQUIPMENT_DESCRIPTION,SERIAL_NO,GREENBOOK,FABLOCATION_ID, WARRANTY_END_DATE,WARRANTY_START_DATE,MNT_PLANT_ID,EQUIPMENT_STATUS,CUSTOMER_TOOL_ID,EQUIPMENTCATEGORY_ID AS DESCRIPTION,WARRANTY_END_DATE_ALERT,TEMP_TOOL from SAQSCO (NOLOCK) where "
						+ str(ATTRIBUTE_VALUE_STR)
						+ " 1=1 and QUOTE_RECORD_ID = '"
						+ str(ContractRecordId)
						+ "' and QTEREV_RECORD_ID = '"
				...
				if TreeParam == "Receiving Equipment":
					...
					parent_obj = Sql.GetList(
						"SELECT top "+str(PerPage)+" QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID,TEMP_TOOL,EQUIPMENTCATEGORY_DESCRIPTION,EQUIPMENT_ID,EQUIPMENT_DESCRIPTION,SERIAL_NO,GREENBOOK,FABLOCATION_ID, WARRANTY_END_DATE,WARRANTY_START_DATE,MNT_PLANT_ID,EQUIPMENT_STATUS,CUSTOMER_TOOL_ID,"+str(equipment_column)+",WARRANTY_END_DATE_ALERT from SAQSCO (NOLOCK) where "
						+ str(ATTRIBUTE_VALUE_STR)
						+ " 1=1 and QUOTE_RECORD_ID = '"
						+ str(ContractRecordId)
						+ "' and QTEREV_RECORD_ID = '"
