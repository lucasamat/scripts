...
def QuoteAssemblyPreventiveMaintainenceParent(PerPage, PageInform, A_Keys, A_Values,ASSEMBLYID,EQUIPMENTID):
    ...
    if TopSuperParentParam in ('Comprehensive Services','Complementary Products'): 
        ...
        dbl_clk_function += (	
            ...
            + str(cls)	
            + '").text();console.log("reco_id2--",reco_id); reco_id=reco_id.split(",").trim(); localStorage.setItem("multiedit_save_date", reco_id); } localStorage.setItem("multiedit_data_clicked", reco_id); localStorage.setItem("table_id_RL_edit", "'	
            + str(table_id)	 
