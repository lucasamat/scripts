...
def QuoteAssemblyPreventiveMaintainenceParent(PerPage, PageInform, A_Keys, A_Values,ASSEMBLYID,EQUIPMENTID):
    ...
    if TopSuperParentParam in ('Comprehensive Services','Complementary Products'): 
        ...
        dbl_clk_function += (	 
            ...
            + str(cls)	
            + '").text();console.log("reco_id2--",reco_id);reco_id=reco_id; reco_id=reco_id.split(","); localStorage.setItem("multiedit_save_date", reco_id); } localStorage.setItem("multiedit_data_clicked", reco_id); localStorage.setItem("table_id_RL_edit", "'	
            + str(table_id)	 
...
def GetCovObjMaster(PerPage, PageInform, A_Keys, A_Values):    
    ...
    dbl_clk_function = (
                ...
                + '").text());  data1 = $element.closest("tr").find("td:'
                + str(cls)
                + '").text(); localStorage.setItem("multiedit_save_date", data1.trim()); reco_id = removeDuplicates(reco); }else{reco_id=$element.closest("tr").find("td:'
                + str(cls)
