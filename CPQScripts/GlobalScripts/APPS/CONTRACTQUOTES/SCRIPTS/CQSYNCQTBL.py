				...
				if not quote_obj:					
					...
					if payload_json_obj:
						...
						if contract_quote_obj:
							...
							Log.Info("product_offering_CHK_J"+str(contract_quote_obj))
							if service_ids:
							...
							if equipment_data:
								...
								Sql.RunQuery("""
											...
											SELECT A.*, CONVERT(VARCHAR(4000),NEWID()) as QUOTE_FAB_LOC_COV_OBJ_ASSEMBLY_RECORD_ID, {UserId} as CPQTABLEENTRYADDEDBY, GETDATE() as CPQTABLEENTRYDATEADDED FROM (
												...
												WHERE MAEQUP.ACCOUNT_RECORD_ID = '{AccountRecordId}' AND  AND MAEQTY.COSTING_RELEVANT = 'True' AND ISNULL(MAEQUP.SERIAL_NO, '') = '' AND SAQFEQ.QUOTE_RECORD_ID = '{QuoteRecordId}'
												) A
