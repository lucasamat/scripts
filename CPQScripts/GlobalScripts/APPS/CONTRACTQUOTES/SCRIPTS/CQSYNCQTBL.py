		...
		try:
			if self.quote:
				...
				if not quote_obj:					
					...
					checklist_desc = ['Signed Terms & Conditions','Contract is current or Temporary Extension is Authorized','If a Volume Purchased Agreement(VPA) or Commercial Twins Agreement(CTA) applies to the 01(101,then:','- The current unexpired VPA is referenced on the PO and on the Quote(Contract Law Department for expired Master Agreement).','- Terms,Conditions, and Pricing on the P01 Sales Agreement are per Master Agreement.','- If the order is not under a Master Agreement and not per the Applied Materials Standard Ts & Cs. Legal has reviewed and approved.','- The customer PO and any other document or communication relating to the order received does not contain unacceptable commercial terms such as:a) the right to receive Applieds best pricing for purshased products or services: b) the right to receive better or more favourable pricing compared than other Applied Customers; c) Process or method patent infringement Indemnification by Applied; d) the customer right to view or audit any Applied Sales records','MEA is required for an order or group of orders received on or about the same time frame from the same customer(always review special or unusual circumstances with Corp Finance-(ORCA).Situation that requires an MEA determination Include:','- Multiple EPG products','- An EPG product combined with any software and/or  Services deliverable','- An order or group of orders that have multiple deliverable other than being exclusively from APG(Sotware and Maintenance) or SPG(Service Agreements, TPM,T SP or Paid Service Agreement)','Approved SAF(Per AGS Order Policy)','Bill Plan and Entitlements Template','Revised Billing Matrix for Contract Changes','Email Notification for Customer,Acknowledging Contract Changes','Delivery Date and INCO Terms Can be Met','Customer Purchase Order','- Addressed to correct AMAT Regional office','- Tax Exemption Certificate on File or VAT Tax # on File','- PO Funding','- PO References Applied Valid Quote I Agreement','- Payment Terms','- Change Order(if required)','SDA Assessment Worksheet is attached.(Display Only) and Specify if this is a SDA Level I,II or III','The Signed Customer Spec has been received','Customer required Shipment or delivery date is within 12 months of the booking date, or PO change order date,whichever is later.']
					checklist_id=0
					finalchecklist_id=0
						...
						if contract_quote_obj:
							...
							if service_ids:
								...
								service_insert = Sql.RunQuery("""
																INSERT
																SAQTSV (QTEREV_RECORD_ID,QTEREV_ID,QUOTE_ID, QUOTE_NAME,UOM_ID, QUOTE_RECORD_ID, SERVICE_DESCRIPTION, SERVICE_ID, SERVICE_RECORD_ID, SERVICE_TYPE, CONTRACT_VALID_FROM, CONTRACT_VALID_TO, SALESORG_ID, SALESORG_NAME, SALESORG_RECORD_ID, QUOTE_SERVICE_RECORD_ID, CPQTABLEENTRYADDEDBY, CPQTABLEENTRYDATEADDED, CpqTableEntryModifiedBy, CpqTableEntryDateModified)
