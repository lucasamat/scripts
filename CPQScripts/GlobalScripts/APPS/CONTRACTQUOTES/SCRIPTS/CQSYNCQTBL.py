					...
					if payload_json_obj:
						...
						if contract_quote_obj:
							...
							try:
								...
								saqfeq_update ="""UPDATE A SET A.TEMP_TOOL = B.TEMP_TOOL FROM SAQFEQ A INNER JOIN {} B on A.EQUIPMENT_ID = B.EQUIPMENT_ID and A.QUOTE_RECORD_ID = B.QUOTE_RECORD_ID and A.FABLOCATION_ID =B.FABLOCATION_ID and A.QTEREV_RECORD_ID = B.QTEREV_RECORD_ID where A.QUOTE_RECORD_ID = '{Quote_id}' AND A.QTEREV_RECORD_ID = '{qtrv_id}'""".format(fab_object_temp_table_name,Quote_id =Quote.GetGlobal("contract_quote_record_id"),qtrv_id =Quote.GetGlobal("quote_revision_record_id"))
