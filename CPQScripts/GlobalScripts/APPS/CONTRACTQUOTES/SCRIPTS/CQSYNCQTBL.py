                ...
                for attrs in overallattributeslist:
                    ...
                    if PRODUCT_ATTRIBUTES:
                        ...
                        if str(attrs) in ('AGS_POA_PROD_TYPE','AGS_{}_GEN_POAPDT'.format(OfferingRow_detail.SERVICE_ID) ) and ent_disp_val != '':
                            ...
                            #Log.Info("ENTERED POA-----275-------->"+str(val))
                        ...
                        <ENTITLEMENT_NAME>{ent_desc}</ENTITLEMENT_NAME>
                            ...
                            ent_disp_val = ent_disp_val.replace("&",";#38").replace(">","&gt;").replace("<","&lt;") if  HasDefaultvalue else '' ,
                            ct = '',pi = '',
                    ...
                    if payload_json_obj:
                        ...
                        '''if custom_fields_detail.get("PrimaryContactName"):	
                            if payload_json.get('SAQICT'):	
                                #MULTIPLE SAQICT INSERT
                                partner_function_obj = Sql.GetFirst("Select * from SYPFTY(nolock) where PARTNERFUNCTION_ID = 'CP'")
                                for employee in payload_json.get('SAQICT'):
                                    master_primary_name= employee["PRIMARY_CONTACT_NAME"]
                                    master_primary_id = employee["PRIMARY_CONTACT_ID"]
                                    employee_obj = Sql.GetFirst("select PHONE from SAEMPL(nolock) where EMPLOYEE_NAME = '{employee_name}'".format(employee_name = master_check))
                                    contact_master_table = Sql.GetFirst("SELECT CONTACT_RECORD_ID FROM SACONT (NOLOCK) WHERE CONTACT_ID = '{master_primary_id}'".format(master_primary_id = master_primary_id))
                                    if employee_obj is None:
                                        country_obj = Sql.GetFirst("select COUNTRY_RECORD_ID from SACTRY(nolock) where COUNTRY = '{country}'".format(country = employee.get("COUNTRY")))
                                        salesorg_obj = Sql.GetFirst("select STATE_RECORD_ID from SASORG(nolock) where STATE = '{state}'".format(state = employee.get("STATE")))
                                        employee_dict = {}
                                    ...
                                    if contact_master_table is None:
                                        employee_obj = Sql.GetFirst("select * from SAEMPL(nolock) where EMPLOYEE_NAME = '{employee_name}'".format(employee_name = master_primary_name))
                                        contact_master_table_update = {
                                            "CONTACT_RECORD_ID": str(Guid.NewGuid()).upper(),
                                    ...
                                    if employee_obj:								
                                        ...
                                        contact_info_update = {
                                            ...
                                            "QUOTE_ID": contract_quote_data.get("QUOTE_ID"),
                                            "QUOTE_RECORD_ID": contract_quote_data.get("MASTER_TABLE_QUOTE_RECORD_ID"),
                                            "CONTACT_ID": custom_fields_detail.CONTACT_ID,
                        ...
                        if contract_quote_obj:
                                    ...
                                    for fab_location_id, value in equipment_fab_data.items():
                                        ...
                                        equipment_temp_insert = Sql.RunQuery("""
                                                                ...
                                                                SELECT A.*, CONVERT(VARCHAR(4000),NEWID()) as QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID, '{UserName}' as CPQTABLEENTRYADDEDBY, GETDATE() as CPQTABLEENTRYDATEADDED, {UserId} as CpqTableEntryModifiedBy, GETDATE() as CpqTableEntryDateModified FROM (
                                                                    SELECT '{quote_revision_id}' AS QTEREV_RECORD_ID,'{quote_rev_id}' AS QTEREV_ID,EQUIPMENT_DESCRIPTION, EQUIPMENT_ID, EQUIPMENT_RECORD_ID, '{FabLocationId}' as FABLOCATION_ID, FABLOCATION_NAME, FABLOCATION_RECORD_ID, MNT_PLANT_ID,'' as MNT_PLANT_NAME, MNT_PLANT_RECORD_ID, PLATFORM, '{QuoteId}' as QUOTE_ID, '{QuoteName}' as QUOTE_NAME, '{QuoteRecordId}' as QUOTE_RECORD_ID, SALESORG_ID, SALESORG_NAME, SALESORG_RECORD_ID, SERIAL_NO, SUBSTRATE_SIZE, TECHNOLOGY, EQUIPMENTCATEGORY_ID, EQUIPMENTCATEGORY_RECORD_ID, EQUIPMENTCATEGORY_DESCRIPTION, EQUIPMENT_STATUS, PBG,KPU, WARRANTY_END_DATE, WARRANTY_START_DATE, CUSTOMER_TOOL_ID,  GREENBOOK, GREENBOOK_RECORD_ID,'True' as TEMP_TOOL FROM MAEQUP (NOLOCK)
                                                                    JOIN (SELECT NAME FROM SPLITSTRING('{EquipmentIds}'))B ON MAEQUP.EQUIPMENT_ID = NAME WHERE ISNULL(SERIAL_NO, '') <> '' 
                                                                    ) A
                            ...
                            if service_ids:								
                                ...
                                for val in fpm_service_ids_list:									
                                    if val != '' and val in ('Z0108','Z0110'):
                                        ...
                                        get_party_role = Sql.GetList("SELECT CPQ_PARTNER_FUNCTION, PARTY_ID FROM SAQTIP(NOLOCK) WHERE QUOTE_RECORD_ID = '"+str(contract_quote_record_id)+"' AND QTEREV_RECORD_ID = '"+str(quote_revision_record_id)+"' and CPQ_PARTNER_FUNCTION in ('SOLD TO')")
                                        account_info = {}
                                        for keyobj in get_party_role:
                                            account_info[keyobj.CPQ_PARTNER_FUNCTION] = keyobj.PARTY_ID
                            ...
                            if equipment_data:
                                ...
                                equipment_assembly_insert=Sql.RunQuery("""
                                                ...
                                                JOIN MAEQTY (NOLOCK)
                                                ON MAEQTY.EQUIPMENT_TYPE_ID = MAEQUP.EQUIPMENTTYPE_ID
                                                WHERE MAEQTY.COSTING_RELEVANT = 'True' AND ISNULL(MAEQUP.SERIAL_NO, '') = '' AND SAQFEQ.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQFEQ.QTEREV_RECORD_ID = '{quote_revision_id}' AND ISNULL(SAQFEQ.TEMP_TOOL,'')=''
                                                ) A
                                            """.format(UserId=User.Id,QuoteId=quote_id, QuoteName=contract_quote_obj.QUOTE_NAME, QuoteRecordId=quote_record_id, AccountRecordId=contract_quote_obj.ACCOUNT_RECORD_ID,quote_revision_id=quote_revision_id,quote_rev_id=quote_rev_id))
                                ...
                                Log.Info("""
                                                ...
                                                JOIN MAEQTY (NOLOCK)
                                                ON MAEQTY.EQUIPMENT_TYPE_ID = MAEQUP.EQUIPMENTTYPE_ID
                                                WHERE MAEQUP.ACCOUNT_RECORD_ID = '{AccountRecordId}' AND MAEQTY.COSTING_RELEVANT = 'True' AND ISNULL(MAEQUP.SERIAL_NO, '') = '' AND SAQFEQ.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQFEQ.QTEREV_RECORD_ID = '{quote_revision_id}' AND ISNULL(SAQFEQ.TEMP_TOOL,'')=''
                                                ) A
                                            """.format(UserId=User.Id,QuoteId=quote_id, QuoteName=contract_quote_obj.QUOTE_NAME, QuoteRecordId=quote_record_id, AccountRecordId=contract_quote_obj.ACCOUNT_RECORD_ID,quote_revision_id=quote_revision_id,quote_rev_id=quote_rev_id))
