		...
		try:
			if self.quote:
				...
				if not quote_obj:					
					...
					update_customer_pn = """UPDATE SAQSPT SET SAQSPT.CUSTOMER_PART_NUMBER = (SELECT M.CUSTOMER_PART_NUMBER FROM MAMSAC M (NOLOCK) INNER JOIN SAFPLT ON S.PART_NUMBER = M.SAP_PART_NUMBER WHERE M.SALESORG_ID = '{sales_id}' and M.ACCOUNT_ID='{stp_acc_id}')
					WHERE SAQSPT.QUOTE_RECORD_ID = '{quote_rec_id}' AND SAQSPT.QUOTE_REVISION_RECORD_ID = '{quote_revision_rec_id}' """.format(quote_rec_id = contract_quote_record_id ,sales_id = sales_id,stp_acc_id=str(account_info.get('SOLD TO')),quote_revision_rec_id =quote_revision_record_id ) )
