...
def replace_contact(repalce_values,cont_rec_id):
    Trace.Write("repalce_values===="+str(repalce_values))
    Trace.Write("acct_rec_id===="+str(cont_rec_id))
    
    con_data_chk = Sql.GetFirst("Select * from SAQICT(NOLOCK) WHERE QUOTE_RECORD_ID ='{}' AND QTEREV_RECORD_ID = '{}' AND QUOTE_INVOLVED_PARTY_RECORD_ID ='{}'".format(contract_quote_record_id,quote_revision_record_id,cont_rec_id))
    rpl_con_data_chk =Sql.GetFirst("Select * FROM SACONT(NOLOCK) WHERE ACCOUNT_RECORD_ID = '{}'".format(repalce_values))
    if con_data_chk:
        Sql.RunQuery("UPDATE SAQICT SET CONTACT_ID = '{party_id}',CONTACT_NAME = '{party_name}',ADDRESS ='{address}',EMAIL ='{email}',PHONE = '{phone}',PARTY_RECORD_ID = '{part_rec_id}' WHERE QUOTE_RECORD_ID = '{QuoteRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and QUOTE_INVOLVED_PARTY_RECORD_ID ='{acct_rec_id}' ".format(party_id = rpl_con_data_chk.ACCOUNT_ID,party_name = rpl_con_data_chk.ACCOUNT_NAME,address = rpl_con_data_chk.ADDRESS_1,email=rpl_con_data_chk.EMAIL,phone= rpl_con_data_chk.PHONE,part_rec_id =rpl_con_data_chk.ACCOUNT_RECORD_ID,QuoteRecordId = contract_quote_record_id,RevisionRecordId = quote_revision_record_id,acct_rec_id = acct_rec_id))
...
replace_contact(repalce_values,cont_rec_id)
