...
class ContractQuoteItem:
	...
	def _quote_annualized_items_insert(self, update=False):
		...
		if self.quote_service_entitlement_type in ('STR-OFFBGBEQ OBJ-EQ','OFFERING + EQUIPMENT + NSO'):
	...
	def _quote_items_object_insert(self, update=False):
		...
		if self.quote_service_entitlement_type not in ('STR-OFFBGBEQ OBJ-EQ','OFFERING + SCH. MAIN. EVENT','OFFERING + PM EVENT','OFFERING + EQUIPMENT + NSO','OFFERING + FAB + SCH. MAIN EVT'):
			self._quote_item_qty_update()
			...
			for quote_item_tag in re.finditer(quote_item_tag_pattern, service_entitlement_obj.ENTITLEMENT_XML):
				...
				if entitlement_id_tag_match:
					...
					if entitlement_display_value_tag_match:
						...
						elif self.quote_service_entitlement_type in ('STR-OFFBGB OBJ-GREQ','OFFERING+FAB+GREENBOOK+CREDIT','OFFERING + CHILD GROUP OF EQP'):
	...
	def _quote_items_insert(self, update=False):		
		...
		if self.source_object_name:		
			...
			elif self.quote_service_entitlement_type in ('STR-OFFBGB OBJ-GREQ','OFFERING + CHILD GROUP OF EQP'):				
