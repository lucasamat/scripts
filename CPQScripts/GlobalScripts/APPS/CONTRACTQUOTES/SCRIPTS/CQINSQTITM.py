...
class ContractQuoteItem:
	...
	def _quote_items_insert(self):
		...
		if service_obj:
			...
			if service_obj.MATERIALCONFIG_TYPE == 'SIMPLE MATERIAL':
				...
				self._simple_quote_item_insert_process(where_string=item_where_string, join_string=item_join_string, outer_where_string=item_outer_where_string)
			else:
	...
	def _simple_quote_item_insert_process(self, where_string='', join_string='', outer_where_string='', max_quote_item_count=0):
		...
		Sql.RunQuery("""
					...
					JOIN SAQTMT (NOLOCK) ON SAQTMT.MASTER_TABLE_QUOTE_RECORD_ID = SAQSCO.QUOTE_RECORD_ID  AND SAQTMT.QTEREV_RECORD_ID = SAQSCO.QTEREV_RECORD_ID 
					JOIN SAQSCE (NOLOCK) ON SAQSCE.QUOTE_RECORD_ID = SAQSCO.QUOTE_RECORD_ID AND SAQSCE.PAR_SERVICE_ID = SAQSCO.PAR_SERVICE_ID AND SAQSCE.QTEREV_RECORD_ID = SAQSCO.QTEREV_RECORD_ID         
					...
					{JoinString}			
					WHERE SAQSCO.QUOTE_RECORD_ID = '{QuoteRecordId}' AND ISNULL(SAQSCE.CONFIGURATION_STATUS,'') = 'COMPLETE' AND SAQSCO.QTEREV_RECORD_ID = '{RevisionRecordId}' {WhereString} {OuterWhereString} 
			""".format(
				QuoteRecordId=self.contract_quote_record_id,
				RevisionRecordId=self.contract_quote_revision_record_id,
				UserId=self.user_id,
