		...
		if self.quote_service_entitlement_type in ("OFFERING + PM EVENT","OFFERING + SCH. MAIN. EVENT"):
			Sql.RunQuery(""" UPDATE SAQRIT SET QUANTITY = IQ.QUANTITY 
							...
							INNER JOIN (SELECT SAQICO.QUOTE_RECORD_ID, SAQICO.QTEREV_RECORD_ID,SAQICO.SERVICE_ID,SAQICO.GRNBOK,
								SAQICO.OBJECT_ID, SAQICO.GOT_CODE,SAQICO.MNTEVT_LEVEL,
								SUM(ISNULL(SAQICO.QUANTITY, 0)) as QUANTITY
								FROM SAQICO WHERE SAQICO.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQICO.QTEREV_RECORD_ID = '{QuoteRevisionRecordId}' AND SAQICO.SERVICE_ID = '{ServiceId}') IQ ON SAQICO.QUOTE_RECORD_ID = SAQRIT.QUOTE_RECORD_ID AND SAQICO.QTEREV_RECORD_ID = SAQRIT.QTEREV_RECORD_ID AND SAQICO.SERVICE_ID = SAQRIT.SERVICE_ID AND SAQICO.GRNBOK = SAQRIT.GREENBOOK AND SAQICO.PM_ID = SAQRIT.OBJECT_ID AND SAQICO.GOT_CODE = SAQRIT.GOT_CODE AND SAQICO.MNTEVT_LEVEL = SAQRIT.MNTEVT_LEVEL
