			...
			if self.service_id == 'Z0091':
				Sql.RunQuery("""UPDATE SAQICO
						...
						INNER JOIN(SELECT PRLPBE.BLUEBOOK,
							...
							INNER JOIN SAQITE (NOLOCK) ON SAQITE.QUOTE_RECORD_ID = SAQTRV.QUOTE_RECORD_ID AND SAQITE.QTEREV_RECORD_ID = SAQTRV.QTEREV_RECORD_ID AND SAQITE.GREENBOOK = PRLPBE.GREENBOOK AND PRLPBE.CONTRACT_COVERAGE = SAQITE.CONCOV
							WHERE SAQTRV.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQTRV.QTEREV_RECORD_ID = '{QuoteRevisionRecordId}' AND SAQICO.SERVICE_ID = '{ServiceId}' AND PRLPBE.GREENBOOK = 'PDC' ) PRLPBE ON SAQICO.LINE = PRLPBE.LINE AND PRLPBE.QUOTE_RECORD_ID = SAQICO.QUOTE_RECORD_ID AND PRLPBE.QTEREV_RECORD_ID = SAQICO.QTEREV_RECORD_ID AND 
