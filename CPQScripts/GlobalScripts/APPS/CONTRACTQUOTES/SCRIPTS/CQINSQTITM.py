...
class ContractQuoteItem:
    ...
    def _quote_item_delete_process(self):
        for delete_object in ['SAQIAE','SAQICA', 'SAQICO']:
        	delete_statement = "DELETE FROM " +str(delete_object)+" A JOIN SAQSCE B ON A.EQUIPMENT_RECORD_ID = B.EQUIPMENT_RECORD_ID AND A.SERVICE_ID=B.SERVICE_ID AND A.QUOTE_RECORD_ID=B.QUOTE_RECORD_ID AND A.QTEREV_RECORD_ID=B.QTEREV_RECORD_ID WHERE A.QUOTE_RECORD_ID='{}' AND A.QTEREV_RECORD_ID='{}' AND B.CONFIGURATION_STATUS ='INCOMPLETE' AND A.SERVICE_ID='{}' ".format(str(self.contract_quote_record_id), str(self.contract_quote_revision_record_id), str(self.service_id))
    ...
    def _insert_quote_item_greenbook(self):		
        ...
        Sql.RunQuery("""UPDATE SAQIGB
            ...
            INNER JOIN (
                ...
                WHERE SAQICO.QUOTE_RECORD_ID ='{QuoteRecordId}' AND SAQICO.QTEREV_RECORD_ID = '{RevisionRecordId}' AND SAQICO.SERVICE_ID = '{ServiceId}' GROUP BY SAQICO.FABLOCATION_RECORD_ID,SAQICO.SERVICE_RECORD_ID,SAQICO.GREENBOOK_RECORD_ID,SAQIGB.CpqTableEntryId
            )AS IQ
