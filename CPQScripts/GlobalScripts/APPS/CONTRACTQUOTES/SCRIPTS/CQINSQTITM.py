		...
		if self.quote_service_entitlement_type == "OFFERING + PM EVENT":
			Sql.RunQuery(""" UPDATE SAQRIT SET QUANTITY = IQ.QUANTITY 
							...
							INNER JOIN (SELECT SERVICE_ID,SAQGPA.GREENBOOK,SAQGPA.FABLOCATION_ID,SAQGPA.GOT_CODE,SAQGPA.PM_ID,SAQGPA.QUOTE_RECORD_ID,SAQGPA.QTEREV_RECORD_ID,SUM(ISNULL(SAQGPA.PM_FREQUENCY, 0)) AS QUANTITY, ISNULL(PROCESS_TYPE,'') AS PROCESS_TYPE , ISNULL(DEVICE_NODE,'') AS DEVICE_NODE
								FROM SAQGPA (NOLOCK)  
								WHERE SAQGPA.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQGPA.QTEREV_RECORD_ID = '{QuoteRevisionRecordId}' AND SAQGPA.SERVICE_ID = '{ServiceId}' 
								GROUP BY SERVICE_ID,SAQGPA.GREENBOOK,SAQGPA.FABLOCATION_ID,SAQGPA.GOT_CODE,SAQGPA.PM_ID,SAQGPA.QUOTE_RECORD_ID,SAQGPA.QTEREV_RECORD_ID,ISNULL(PROCESS_TYPE,'')  , ISNULL(DEVICE_NODE,'') ) IQ 
							ON IQ.QUOTE_RECORD_ID = SAQRIT.QUOTE_RECORD_ID AND IQ.QTEREV_RECORD_ID = SAQRIT.QTEREV_RECORD_ID AND IQ.SERVICE_ID = SAQRIT.SERVICE_ID AND SAQRIT.OBJECT_ID = IQ.PM_ID AND SAQRIT.FABLOCATION_ID = IQ.FABLOCATION_ID AND SAQRIT.GREENBOOK = IQ.GREENBOOK AND SAQRIT.GOT_CODE = IQ.GOT_CODE WHERE 
