...
if 'Param' in globals():
    ...
    if hasattr(Param, 'CPQ_Columns'):
        ...
        get_approver_id = Sql.GetFirst("SELECT EMPLOYEE_ID FROM SAEMPL(NOLOCK) WHERE C4C_EMPLOYEE_ID = '{approver_id}'".format(approver_id = approver_id))
        if get_approver_id:
            update_obj_id="""UPDATE ACAPTX SET OWNER_ID = '{obj_id}' WHERE APRTRXOBJ_ID = '{quote_id}' AND APRCHNSTP_APPROVER_ID ='{emp_approver_id}' AND APRCHNSTP_ID ='{approver_step_id}'""".format(obj_id=obj_id,quote_id =quote_id,emp_approver_id = get_approver_id.EMPLOYEE_ID,approver_step_id =approver_step_id)
