	...
	while L == 1:
		...
		if get_part_query:      
			...
			if len(partids) == 1:
				...
				if salesUOM !='':
					curr_attr += ','+'"quantity":{"value":'+str(quantity[0])+',"unit":"'+str(ISOCode[salesUOM])+'"},"exchRateType":"'+str(exch)+'","exchRateDate":"'+str(y[0])+'","productDetails":{"productId":"'+str(partids[0])+'","baseUnit":"EA","alternateProductUnits": [{"alternateUnitName": "'+str(ISOCode[salesUOM])+'","numerator": "'+str(salesUOMConv)+'","denominator": 1}]}'
				if str_odcc_flag !='':
