...
def BILLEDIT_SAVE(GET_DICT,totalyear,getedited_amt,):
	...
	for val in GET_DICT:
		...
		gettotalamt_beforeupdate = Sql.GetFirst("SELECT BILLING_VALUE=SUM(BILLING_VALUE) FROM SAQIBP WHERE  QUOTE_RECORD_ID ='{cq}' AND QTEREV_RECORD_ID ='{revision_rec_id}' and EQUIPMENT_ID = '{EID}'".format(cq=str(ContractRecordId),EID=value[0], revision_rec_id = quote_revision_record_id ))
