...
def DELIVERYEDIT_SAVE(deliverydict,totalyear,getedited_amt,deliveryEdit):
	...
	for qty,deldate,sp_rec in join_all_list:
		...
		get_spare_qty = Sql.GetFirst("SELECT CUSTOMER_ANNUAL_QUANTITY from SAQSPT where QUOTE_RECORD_ID ='{qt_rec_id}' AND QTEREV_RECORD_ID ='{revision_rec_id}' and QUOTE_SERVICE_PART_RECORD_ID='{rev_spare_rec_id}'".format(revision_rec_id = quote_revision_record_id,rev_spare_rec_id=partrec[sp_rec],qt_rec_id = str(ContractRecordId)))
		if get_spare_qty:
			saqspt_total_qty = get_spare_qty.CUSTOMER_ANNUAL_QUANTITY
		get_current_details = Sql.GetFirst("SELECT SUM(QUANTITY) as total FROM SAQSPD where QUOTE_RECORD_ID ='{ContractRecordId}' AND QTEREV_RECORD_ID ='{quote_revision_record_id}' and  QTEREVSPT_RECORD_ID ='{rev_spare_rec_id}' and DELIVERY_SCHED_DATE not in {deliverydates}".format(ContractRecordId=ContractRecordId,quote_revision_record_id=quote_revision_record_id,rev_spare_rec_id=partrec[sp_rec],deliverydates=str(tuple(get_delivery_date_list)).replace(',)',')')))
