...
def save_annualiziedgrid_inline(Quote,line,CAT1,CAT2,CAT3,CAT4,CAT5,CAT6,CAT7,CAT8,CAT9,CAT10,CAT11,CAT12,CAT13,CAT14,MODE):
	...
	for index,val in enumerate(line):
		...
		Trace.Write("UPDATE SAQICO SET UIMVCI = {cval1}, UIMVPI = '{cval2}' WHERE LINE = '{line}' AND QUOTE_RECORD_ID ='{quote_rec_id}' AND QTEREV_RECORD_ID ='{quo_rev_rec_id}'".format(quote_rec_id = get_quote_details.MASTER_TABLE_QUOTE_RECORD_ID,quo_rev_rec_id = get_quote_details.QTEREV_RECORD_ID,cval1 = CAT1[index] if CAT1[index] != '' else 'NULL' ,cval2 = CAT2[index],line =line[index]))
		update_saqico = "UPDATE SAQICO SET UIMVCI = {cval1}, UIMVPI = '{cval2}' WHERE LINE = '{line}' AND QUOTE_RECORD_ID ='{quote_rec_id}' AND QTEREV_RECORD_ID ='{quo_rev_rec_id}'".format(quote_rec_id = get_quote_details.MASTER_TABLE_QUOTE_RECORD_ID,quo_rev_rec_id = get_quote_details.QTEREV_RECORD_ID,cval1 = CAT1[index] if CAT1[index] != '' else 'NULL' ,cval2 = CAT2[index],line =line[index])
