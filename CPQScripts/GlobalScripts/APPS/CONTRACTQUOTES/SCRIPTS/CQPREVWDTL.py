...
def EditCBC(Qt_rec_id, Quote, MODE):		
	...
	get_quote_details = Sql.GetFirst("Select QUOTE_ID,REVISION_ID FROM SAQTRV WHERE QUOTE_RECORD_ID = '{quote_rec_id}' AND QTEREV_RECORD_ID = '{quote_rev_recid}' AND ACTIVE = '1' ".format(quote_rec_id = Quote,quote_rev_recid = quote_revision_record_id))
	crm_result = ScriptExecutor.ExecuteGlobal('QTPOSTACRM',{'QUOTE_ID':str(get_quote_details.QUOTE_ID),'REVISION_ID':str(get_quote_details.REVISION_ID),'Fun_type':'cpq_to_crm'})		
	return True
