...
def writeback_to_c4c(writeback,contract_quote_record_id,quote_revision_record_id):
    if writeback == "quote_header":
        ...
        c4c_employee_obj = Sql.GetFirst("SELECT C4C_EMPLOYEE_ID FROM SAEMPL INNER JOIN SAQTMT ON SAEMPL.EMPLOYEE_ID = SAQTMT.OWNER_ID WHERE QUOTE_RECORD_ID = '{}' AND QTEREV_RECORD_ID = '{}'".format(contract_quote_record_id,quote_revision_record_id)))
