...
class ContractQuoteDownloadTableData(ContractQuoteSpareOpertion):	
	...
	def get_results(self, table_total_rows=0, colums='*'):		
		...
		replace_col ={'CONSUMABLE/NON CONSUMABLE':'MATPRIGRP_ID','CUSTOMER WILL ACCPET W/6K PART':"CASE WHEN CUSTOMER_ACCEPT_PART IN ('True','TRUE') THEN 'Yes' ELSE 'No' END AS CUSTOMER_ACCEPT_PART",'CUSTOMER ANNUAL COMMIT':'CUSTOMER_ANNUAL_QUANTITY','EXCHANGE_ELIGIBLE':"CASE WHEN EXCHANGE_ELIGIBLE IN ('True','TRUE') THEN 'Yes' ELSE 'No' END AS EXCHANGE_ELIGIBLE",'CUSTOMER_PARTICIPATE':"CASE WHEN CUSTOMER_PARTICIPATE ='True' THEN 'Yes' ELSE 'No' END AS CUSTOMER_PARTICIPATE",'CUSTOMER_ELIGIBLE':"CASE WHEN CUSTOMER_ELIGIBLE IN ('True','TRUE') THEN 'Yes' ELSE 'No' END AS CUSTOMER_ELIGIBLE"}
