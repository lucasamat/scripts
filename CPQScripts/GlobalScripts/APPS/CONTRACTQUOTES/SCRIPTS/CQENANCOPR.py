		...
		for addon in get_service_details:
			...
			if ProductVersionObj and get_existing_record.cnt == 0:
				...
				for attrs in overallattributeslist:
					...
					service_entitlement_obj =Sql.GetFirst("select ENTITLEMENT_XML from SAQTSE (nolock) where QUOTE_RECORD_ID = '{QuoteRecordId}' AND QTEREV_RECORD_ID = '{RevisionRecordId}' and SERVICE_ID = '{par_service}' ".format(QuoteRecordId = self.contract_quote_record_id,RevisionRecordId=self.contract_quote_revision_record_id,par_service= self.service_id))
