	...
	def _entitlement_rolldown(self):
		try:
			...
			if get_ancillaryservice :
				...
				if get_ancillary_grn:
					if get_ancillary_grn.cnt == 0: 
						qtqsge_query_anc="""
							...
							FROM
							SAQTSE (NOLOCK)
							JOIN SAQSCO  (NOLOCK) ON SAQSCO.PAR_SERVICE_ID = SAQTSE.PAR_SERVICE_ID AND SAQSCO.QUOTE_RECORD_ID = SAQTSE.QUOTE_RECORD_ID  AND SAQSCO.QTEREV_RECORD_ID = SAQTSE.QTEREV_RECORD_ID 
							...
							JOIN SAQSGE ON SAQSGE.SERVICE_ID = SAQSCO.PAR_SERVICE_ID AND SAQSCO.QUOTE_RECORD_ID = SAQSGE.QUOTE_RECORD_ID AND SAQSCO.QTEREV_RECORD_ID = SAQSGE.QTEREV_RECORD_ID AND SAQSCO.GREENBOOK_RECORD_ID = SAQSGE.GREENBOOK_RECORD_ID
							
							WHERE SAQTSE.QUOTE_RECORD_ID = '{QuoteRecordId}'  AND SAQTSE.QTEREV_RECORD_ID = '{revision_rec_id}' AND SAQTSE.PAR_SERVICE_ID = '{par_service_id}'  AND ISNULL(SAQSGE.CONFIGURATION_STATUS,'') = 'INCOMPLETE' AND SAQSCO.GREENBOOK not in (SELECT GREENBOOK FROM SAQSGE M WHERE M.QUOTE_RECORD_ID = '{QuoteRecordId}' AND M.QTEREV_RECORD_ID = '{revision_rec_id}' AND M.SERVICE_ID = SAQTSE.SERVICE_ID AND PAR_SERVICE_ID = '{par_service_id}') ) IQ JOIN SAQSFE (NOLOCK) M ON IQ.QTSFBLENT_RECORD_ID = QUOTE_SERVICE_FAB_LOC_ENT_RECORD_ID )OQ""".format(UserId=self.user_id, QuoteRecordId=self.contract_quote_record_id, revision_rec_id = self.contract_quote_revision_record_id,par_service_id = self.service_id)
						Log.Info("qtqsge_query_anc---163------"+str(qtqsge_query_anc))
				...
				SAQSAE_ent_anc_renewal = """INSERT SAQSAE (KB_VERSION,EQUIPMENT_ID,EQUIPMENT_RECORD_ID,QUOTE_ID,QUOTE_RECORD_ID,QTEREV_RECORD_ID,QTEREV_ID,SERVICE_DESCRIPTION,SERVICE_ID,SERVICE_RECORD_ID,CPS_CONFIGURATION_ID,CPS_MATCH_ID,GREENBOOK,GREENBOOK_RECORD_ID,FABLOCATION_ID,FABLOCATION_NAME,FABLOCATION_RECORD_ID,ASSEMBLY_DESCRIPTION,ASSEMBLY_ID,ASSEMBLY_RECORD_ID,QTESRVCOA_RECORD_ID,SALESORG_ID,SALESORG_NAME,SALESORG_RECORD_ID,PAR_SERVICE_ID,PAR_SERVICE_RECORD_ID,PAR_SERVICE_DESCRIPTION,ENTITLEMENT_XML,CONFIGURATION_STATUS,QTESRVCOE_RECORD_ID,QUOTE_SERVICE_COV_OBJ_ASS_ENT_RECORD_ID,CPQTABLEENTRYADDEDBY,CPQTABLEENTRYDATEADDED) SELECT IQ.*, CONVERT(VARCHAR(4000),NEWID()) as QUOTE_SERVICE_COV_OBJ_ASS_ENT_RECORD_ID, {UserId} as CPQTABLEENTRYADDEDBY, GETDATE() as CPQTABLEENTRYDATEADDED FROM(SELECT IQ.*,M.ENTITLEMENT_XML,M.CONFIGURATION_STATUS,M.QUOTE_SERVICE_COVERED_OBJ_ENTITLEMENTS_RECORD_ID as QTESRVCOE_RECORD_ID FROM ( SELECT DISTINCT SAQTSE.KB_VERSION,SAQSCA.EQUIPMENT_ID,SAQSCA.EQUIPMENT_RECORD_ID,SAQTSE.QUOTE_ID,SAQTSE.QUOTE_RECORD_ID,SAQTSE.QTEREV_RECORD_ID,SAQTSE.QTEREV_ID,SAQTSE.SERVICE_DESCRIPTION,SAQTSE.SERVICE_ID,SAQTSE.SERVICE_RECORD_ID,SAQTSE.CPS_CONFIGURATION_ID,SAQTSE.CPS_MATCH_ID,SAQSCA.GREENBOOK,SAQSCA.GREENBOOK_RECORD_ID,SAQSCA.FABLOCATION_ID,SAQSCA.FABLOCATION_NAME,SAQSCA.FABLOCATION_RECORD_ID,SAQSCA.ASSEMBLY_DESCRIPTION,SAQSCA.ASSEMBLY_ID,SAQSCA.ASSEMBLY_RECORD_ID,SAQSCA.QUOTE_SERVICE_COVERED_OBJECT_ASSEMBLIES_RECORD_ID,SAQSCA.PAR_SERVICE_ID,SAQSCA.PAR_SERVICE_RECORD_ID,SAQSCA.PAR_SERVICE_DESCRIPTION as QTESRVCOA_RECORD_ID,SAQTSE.SALESORG_ID,SAQTSE.SALESORG_NAME,SAQTSE.SALESORG_RECORD_ID FROM SAQTSE (NOLOCK) JOIN (SELECT * FROM SAQSCA (NOLOCK) WHERE SAQSCA.QUOTE_RECORD_ID = 'AB1342C6-9AB9-4A49-BEAB-0F6BBEE15041' AND SAQSCA.QTEREV_RECORD_ID = 'EB70CEA2-CDB4-455A-823A-69A3CC42351A' AND SAQSCA.PAR_SERVICE_ID = 'Z0091' ) SAQSCA ON SAQTSE.QUOTE_RECORD_ID = SAQSCA.QUOTE_RECORD_ID AND SAQTSE.QTEREV_RECORD_ID = SAQSCA.QTEREV_RECORD_ID  AND SAQTSE.PAR_SERVICE_ID = SAQSCA.PAR_SERVICE_ID AND SAQTSE.SERVICE_ID = SAQSCA.SERVICE_ID JOIN SAQSAE ON SAQSAE.SERVICE_ID = SAQSCA.PAR_SERVICE_ID AND SAQSCA.QUOTE_RECORD_ID = SAQSAE.QUOTE_RECORD_ID AND SAQSCA.QTEREV_RECORD_ID = SAQSAE.QTEREV_RECORD_ID AND SAQSAE.EQUIPMENT_ID = SAQSCA.EQUIPMENT_ID  WHERE  SAQTSE.QUOTE_RECORD_ID = 'AB1342C6-9AB9-4A49-BEAB-0F6BBEE15041' AND SAQTSE.QTEREV_RECORD_ID = 'EB70CEA2-CDB4-455A-823A-69A3CC42351A' AND SAQSCA.PAR_SERVICE_ID = 'Z0091' AND ISNULL(SAQSAE.CONFIGURATION_STATUS,'') = 'INCOMPLETE' ) IQ JOIN SAQSCE (NOLOCK) M ON M.SERVICE_RECORD_ID = IQ.SERVICE_RECORD_ID AND M.QUOTE_RECORD_ID = IQ.QUOTE_RECORD_ID AND M.QTEREV_RECORD_ID = IQ.QTEREV_RECORD_ID AND M.EQUIPMENT_ID = IQ.EQUIPMENT_ID AND M.PAR_SERVICE_ID = IQ.PAR_SERVICE_ID )IQ""".format(UserId=self.user_id, QuoteRecordId=self.contract_quote_record_id, serviceId=self.ancillary_obj, revision_rec_id = self.contract_quote_revision_record_id)
