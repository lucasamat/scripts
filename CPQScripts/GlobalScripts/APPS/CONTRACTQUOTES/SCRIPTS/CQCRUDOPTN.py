	...
	def _create(self):		
		...
		if self.action_type == "ADD_OFFERING":
			...
			if self.values:
				...
				self._process_query("""DELETE FROM SYSPBT JOIN SAQSPT ON SYSPBT.SAP_PART_NUMBER = SAQSPT.PART_NUMBER AND SYSPBT.QUOTE_RECORD_ID = SAQSPT.QUOTE_RECORD_ID AND  SYSPBT.QTEREV_RECORD_ID = SAQSPT.QTEREV_RECORD_ID """)
				self._process_query("""
