		...
		if kwargs.get('applied_preventive_maintainence_quote_type_changed') == "Yes" and kwargs.get('tools_from_ui') != "Yes":
			...
			self._process_query(
				"""INSERT SAQGPA (
					...
					QTEREV_RECORD_ID = '{RevisionRecordId}' AND saqgpa_backup_table.SERVICE_ID = '{TreeParam}' AND MAEQTY.COSTING_RELEVANT = 'True' ) PM_EVENT_ASSEMBLY  
				...
				if self.all_values:        
					...
					if self.tree_param != '' and self.tree_parent_level_0 == 'Add-On Products' : #ADDED FOR ADD ON PRODUCT SCENARIO
						...
						if self.tree_param == "Sending Equipment" or self.tree_param == "Receiving Equipment":
							...
							query_string = "SELECT QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID FROM SAQFEQ (NOLOCK) WHERE QUOTE_RECORD_ID = '{QuoteRecordId}' AND QTEREV_RECORD_ID = '{RevisionRecordId}' {is_temptool} AND {Qury_Str} EQUIPMENT_ID NOT IN(SELECT EQUIPMENT_ID FROM SAQSCO WHERE QUOTE_RECORD_ID ='{QuoteRecordId}' AND QTEREV_RECORD_ID = '{RevisionRecordId}' AND SERVICE_ID ='{TreeParam}')".format(QuoteRecordId=self.contract_quote_record_id,RevisionRecordId=self.quote_revision_record_id,TreeParam = self.tree_param,Qury_Str=qury_str,is_temptool= " AND TEMP_TOOL ='TRUE' " if self.tree_param == "Z0099" else " AND (TEMP_TOOL IS NULL OR TEMP_TOOL = 'False' OR TEMP_TOOL = '') ")
					query_string_for_count = "SELECT COUNT(*) as count FROM ({Query_String})OQ".format(
						Query_String=query_string
					)
					table_count_data = Sql.GetFirst(query_string_for_count)
