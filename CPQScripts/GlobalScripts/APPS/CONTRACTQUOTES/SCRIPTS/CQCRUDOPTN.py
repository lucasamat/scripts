...
class ContractQuoteCoveredObjModel(ContractQuoteCrudOpertion):
	...
	def applied_preventive_maintainence(self, **kwargs):
		...
		if kwargs.get('applied_preventive_maintainence_quote_type_changed') == "Yes" and kwargs.get('tools_from_ui') != "Yes":
			...
			Sql.RunQuery("""UPDATE SAQGPE SET ENTITLEMENT_XML = SAQSGE.ENTITLEMENT_XML,CONFIGURATION_STATUS = SAQSGE.CONFIGURATION_STATUS FROM SAQGPE JOIN SAQSGE ON SAQSGE.QUOTE_RECORD_ID = SAQGPE.QUOTE_RECORD_ID AND SAQSGE.QTEREV_RECORD_ID = SAQGPE.QTEREV_RECORD_ID AND SAQSGE.SERVICE_RECORD_ID = SAQGPE.SERVICE_RECORD_ID AND SAQSGE.GREENBOOK_RECORD_ID = SAQGPE.GREENBOOK_RECORD_ID WHERE SAQGPE.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQGPE.QTEREV_RECORD_ID = '{RevisionRecordId}' AND SAQGPE.SERVICE_ID = '{TreeParam}'""".format(QuoteRecordId=self.contract_quote_record_id,RevisionRecordId=self.quote_revision_record_id,TreeParam=self.tree_param))
		else:
			self._process_query("""INSERT SAQRGG (
					GOT_CODE,
					GOTCODE_RECORD_ID,
			...
			Sql.RunQuery("""UPDATE SAQGPA
				SET FABLOCATION_ID = IQ.FABLOCATION_ID,
					...
					INNER JOIN (SELECT MAEQUP.FABLOCATION_ID,MAEQUP.FABLOCATION_NAME,MAEQUP.FABLOCATION_RECORD_ID,SAQGPA.QUOTE_RECORD_ID,SAQGPA.QTEREV_RECORD_ID
								...
								WHERE SAQGPA.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQGPA.QTEREV_RECORD_ID = '{RevisionRecordId}' AND SAQGPA.SERVICE_ID = '{TreeParam}' AND ISNULL(SAQGPA.FABLOCATION_ID,'') = ''
								)IQ ON SAQGPA.QUOTE_RECORD_ID = IQ.QUOTE_RECORD_ID AND SAQGPA.QTEREV_RECORD_ID = IQ.QTEREV_RECORD_ID AND SAQGPA.SERVICE_ID = '{TreeParam}' """.format(QuoteRecordId=self.contract_quote_record_id,RevisionRecordId=self.quote_revision_record_id,TreeParam=self.tree_param))
			Sql.RunQuery("""UPDATE SAQGPA
				SET PROCESS_TYPE = IQ.PROCESS_TYPE,
					DEVICE_NODE = IQ.DEVICE_NODE		
			...
			self._process_query(
				"""INSERT SAQSKP (
					...
					SELECT KP.*,CONVERT(VARCHAR(4000),NEWID()) as QUOTE_SERVICE_COV_OBJ_ASS_PM_KIT_PARTS_RECORD_ID,'{UserName}' as CPQTABLEENTRYADDEDBY, GETDATE() as CPQTABLEENTRYDATEADDED FROM (
						...
						JOIN SAQGPM (NOLOCK) ON SAQSCO.QUOTE_RECORD_ID = SAQGPM.QUOTE_RECORD_ID AND SAQSCO.QTEREV_RECORD_ID = SAQGPM.QTEREV_RECORD_ID AND SAQSCO.SERVICE_RECORD_ID =  SAQGPM.SERVICE_RECORD_ID AND SAQSCO.GREENBOOK_RECORD_ID =  SAQGPM.GREENBOOK_RECORD_ID  AND
						SAQGPA.QUOTE_RECORD_ID = SAQGPM.QUOTE_RECORD_ID AND SAQGPA.QTEREV_RECORD_ID = SAQGPM.QTEREV_RECORD_ID AND
						SAQGPA.SERVICE_RECORD_ID =  SAQGPM.SERVICE_RECORD_ID AND SAQGPA.GREENBOOK_RECORD_ID =  SAQGPM.GREENBOOK_RECORD_ID AND SAQGPA.GOTCODE_RECORD_ID =  SAQGPM.GOTCODE_RECORD_ID AND SAQGPA.PM_RECORD_ID =  SAQGPM.PM_RECORD_ID WHERE SAQSCO.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQSCO.QTEREV_RECORD_ID = '{RevisionRecordId}' AND SAQSCO.SERVICE_ID = '{TreeParam}' AND SYSPBT.BATCH_GROUP_RECORD_ID = '{BatchGroupRecordId}') assembly_and_event_object
					JOIN MAPMEV(NOLOCK) ON MAPMEV.PM_NAME = assembly_and_event_object.PM_NAME
					WHERE assembly_and_event_object.QUOTE_RECORD_ID = '{QuoteRecordId}' AND assembly_and_event_object.QTEREV_RECORD_ID = '{RevisionRecordId}' AND assembly_and_event_object.SERVICE_ID = '{TreeParam}') KP LEFT JOIN SAQSKP (NOLOCK) AS KIT ON 
					KP.QUOTE_RECORD_ID =KIT.QUOTE_RECORD_ID AND KP.QTEREV_RECORD_ID =KIT.QTEREV_RECORD_ID AND KP.SERVICE_RECORD_ID =KIT.SERVICE_RECORD_ID AND KP.EQUIPMENT_RECORD_ID =KIT.EQUIPMENT_RECORD_ID  AND KP.ASSEMBLY_RECORD_ID =KIT.ASSEMBLY_RECORD_ID AND KP.PM_RECORD_ID =KIT.PM_RECORD_ID AND KP.KIT_RECORD_ID =KIT.KIT_RECORD_ID  AND KP.KITNUMBER_RECORD_ID =KIT.KIT_NUMBER_RECORD_ID AND ISNULL(KIT.KIT_RECORD_ID,'') = '' """.format(
					UserName=self.user_name,
			...
			Sql.RunQuery("""UPDATE SAQGPE SET ENTITLEMENT_XML = SAQSGE.ENTITLEMENT_XML,CONFIGURATION_STATUS = SAQSGE.CONFIGURATION_STATUS FROM SAQGPE JOIN SAQSGE ON SAQSGE.QUOTE_RECORD_ID = SAQGPE.QUOTE_RECORD_ID AND SAQSGE.QTEREV_RECORD_ID = SAQGPE.QTEREV_RECORD_ID AND SAQSGE.SERVICE_RECORD_ID = SAQGPE.SERVICE_RECORD_ID AND SAQSGE.GREENBOOK_RECORD_ID = SAQGPE.GREENBOOK_RECORD_ID WHERE SAQGPE.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQGPE.QTEREV_RECORD_ID = '{RevisionRecordId}' AND SAQGPE.SERVICE_ID = '{TreeParam}' AND ISNULL(SAQGPE.ENTITLEMENT_XML,'') = '' """.format(QuoteRecordId=self.contract_quote_record_id,RevisionRecordId=self.quote_revision_record_id,TreeParam=self.tree_param))
