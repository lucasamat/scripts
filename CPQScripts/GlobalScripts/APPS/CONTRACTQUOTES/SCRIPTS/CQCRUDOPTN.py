...
class ContractQuoteCoveredObjModel(ContractQuoteCrudOpertion):
	...
	def applied_preventive_maintainence(self, **kwargs):
		...
		if kwargs.get('applied_preventive_maintainence_quote_type_changed') == "Yes" and kwargs.get('tools_from_ui') != "Yes":
			...
			Sql.RunQuery("""UPDATE SAQGPE SET ENTITLEMENT_XML = SAQSGE.ENTITLEMENT_XML,CONFIGURATION_STATUS = SAQSGE.CONFIGURATION_STATUS FROM SAQGPE JOIN SAQSGE ON SAQSGE.QUOTE_RECORD_ID = SAQGPE.QUOTE_RECORD_ID AND SAQSGE.QTEREV_RECORD_ID = SAQGPE.QTEREV_RECORD_ID AND SAQSGE.SERVICE_RECORD_ID = SAQGPE.SERVICE_RECORD_ID AND SAQSGE.GREENBOOK_RECORD_ID = SAQGPE.GREENBOOK_RECORD_ID WHERE SAQGPE.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQGPE.QTEREV_RECORD_ID = '{RevisionRecordId}' AND SAQSGE.SERVICE_ID = '{TreeParam}' AND ISNULL(SAQGPE.ENTITLEMENT_XML,'') = '' """.format(QuoteRecordId=self.contract_quote_record_id,RevisionRecordId=self.quote_revision_record_id,TreeParam=self.tree_param))
