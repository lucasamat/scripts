		...
		if self.action_type == "ADD_OFFERING":
			...
			if self.values:
				...
				if self.action_type == "ADD_PARTS":
					...
					if 'Z0101' in parent_based_condition :
		...
		if ProductVersionObj:
			...
			if OfferingRow_detail.get("SERVICE_ID") in ['Z0108','Z0110']:
				try:
					...
					CONVERT(VARCHAR(4000),NEWID()) as QUOTE_SERVICE_PART_RECORD_ID ,'' as BASEUOM_ID,'' as BASEUOM_RECORD_ID,M.CUSTOMER_PART_NUMBER,S.CUSTOMER_PART_NUMBER_RECORD_ID,S.DELIVERY_MODE,S.EXTENDED_UNIT_PRICE,S.PART_NUMBER,S.PART_DESCRIPTION,S.PART_RECORD_ID,S.PRDQTYCON_RECORD_ID,0 as CUSTOMER_ANNUAL_QUANTITY,'{qt_id}' as QUOTE_ID,'' as QUOTE_NAME,'{qtt}' as QUOTE_RECORD_ID,'{sales_id}' as SALESORG_ID,'{sales_rec}' as SALESORG_RECORD_ID,S.SALESUOM_CONVERSION_FACTOR,S.SALESUOM_ID,S.SALESUOM_RECORD_ID,S.SCHEDULE_MODE,'' as SERVICE_DESCRIPTION,'' as SERVICE_ID,'' as SERVICE_RECORD_ID,S.UNIT_PRICE,'{ctf}' as VALID_FROM_DATE,'{ctt}' as VALID_TO_DATE,'' as DELIVERY_INTERVAL,S.MATPRIGRP_ID,S.MATPRIGRP_NAME,MATPRIGRP_RECORD_ID,'' as PAR_SERVICE_DESCRIPTION,'' as PAR_SERVICE_ID,'' as PAR_SERVICE_RECORD_ID,'{qt_rev_id}' as QTEREV_ID,'{rid}' as QTEREV_RECORD_ID,'' as PRICE_REQUEST_ID,'' as PRICE_REQUEST_STATUS,'' as PRICE_REQUEST_TYPE,S.CORE_CREDIT_PRICE,s.CUSTOMER_PARTICIPATE,S.CUSTOMER_ACCEPT_PART,s.EXCHANGE_ELIGIBLE,'' as INCLUDED,'' as MATERIALSTATUS_ID,'' as MATERIALSTATUS_RECORD_ID,'' as NEW_PART,'' as ODCC_FLAG,PROD_INSP_MEMO,S.RETURN_TYPE,S.SHELF_LIFE,S.SHPACCOUNT_ID,S.SHPACCOUNT_RECORD_ID,S.STPACCOUNT_ID,S.STPACCOUNT_RECORD_ID,S.YEAR_1_DEMAND,S.YEAR_2_DEMAND,S.YEAR_3_DEMAND FROM SAFPLT S JOIN MAMSAC M ON S.PART_NUMBER= M.SAP_PART_NUMBER AND S.STPACCOUNT_ID = M.ACCOUNT_ID where S.SHPACCOUNT_ID = '{ship_record_id}' AND S.STPACCOUNT_ID = '{stp_acc_id}' AND M.SALESORG_ID='{sales_rec}'""".format(ctf =get_rev_sales_ifo.CONTRACT_VALID_FROM ,ctt= get_rev_sales_ifo.CONTRACT_VALID_TO,rid=quote_revision_record_id,qtt=contract_quote_record_id,ship_record_id=str(account_info.get('SHIP TO')),sales_id = sales_id,sales_rec =sales_rec,qt_rev_id=qt_rev_id,qt_id=qt_id,stp_acc_id=str(account_info.get('SOLD TO')))
...
class ContractQuoteFabModel(ContractQuoteCrudOpertion):
	...
	def _create(self):
		...
		if self.action_type == "ADD_FAB":
			...
			SAQTSVObj=Sql.GetList("Select SAQSAO.*,SAQTSV.QUOTE_SERVICE_RECORD_ID from SAQSAO (nolock) inner join SAQTSV on SAQTSV.QUOTE_RECORD_ID=SAQSAO.QUOTE_RECORD_ID and SAQTSV.QTEREV_RECORD_ID=SAQSAO.QTEREV_RECORD_ID and SAQTSV.SERVICE_ID = SAQSAO.SERVICE_ID where SAQSAO.QUOTE_RECORD_ID= '{QuoteRecordId}' AND SAQSAO.QTEREV_RECORD_ID = '{RevisionRecordId}' AND SAQSAO.ACTIVE ='TRUE' AND NOT EXISTS (SELECT SERVICE_ID FROM SAQTSV WHERE QUOTE_RECORD_ID ='{QuoteRecordId}' AND QTEREV_RECORD_ID = '{RevisionRecordId}' AND SAQTSV.SERVICE_ID=SAQSAO.ADNPRD_ID)".format(QuoteRecordId=self.contract_quote_record_id,RevisionRecordId=self.quote_revision_record_id))
			...
			for OfferingRow_detail in SAQTSVObj:
				...
				if ProductVersionObj is not None:
					...
					for attrs in overallattributeslist:
						...
						if PRODUCT_ATTRIBUTES:
							get_tooltip_info = PRODUCT_ATTRIBUTES.ATTRDESC
						insertservice += """<QUOTE_ITEM_ENTITLEMENT>
						<ENTITLEMENT_ID>{ent_name}</ENTITLEMENT_ID>
						<ENTITLEMENT_VALUE_CODE>{ent_val_code}</ENTITLEMENT_VALUE_CODE>
						<ENTITLEMENT_DESCRIPTION>{tool_desc}</ENTITLEMENT_DESCRIPTION>
						...
						<CALCULATION_FACTOR>{cf}</CALCULATION_FACTOR>
						<ENTITLEMENT_NAME>{ent_desc}</ENTITLEMENT_NAME>
						</QUOTE_ITEM_ENTITLEMENT>""".format(ent_name = str(attrs),ent_val_code = attributevalues[attrs] if HasDefaultvalue==True else '',ent_type = DTypeset[PRODUCT_ATTRIBUTES.ATT_DISPLAY_DESC] if PRODUCT_ATTRIBUTES else  '',ent_desc = ATTRIBUTE_DEFN.STANDARD_ATTRIBUTE_NAME,ent_disp_val = ent_disp_val if HasDefaultvalue==True else '',ct = '',pi = '',is_default =  1 if str(attrs) in attributedefaultvalue else '0',pm = '',cf = '',tool_desc= get_tooltip_info.replace("'","''") if "'" in get_tooltip_info else get_tooltip_info)
					...
					qtqsge_query= """INSERT SAQSGE (KB_VERSION,QUOTE_ID,QUOTE_NAME,QUOTE_RECORD_ID,QTEREV_RECORD_ID,QTEREV_ID,SERVICE_DESCRIPTION,SERVICE_ID,SERVICE_RECORD_ID,SALESORG_ID,SALESORG_NAME,SALESORG_RECORD_ID,	
					...
					# 	CPS_CONFIGURATION_ID, CPS_MATCH_ID,GREENBOOK,GREENBOOK_RECORD_ID,QTESRVENT_RECORD_ID,PAR_SERVICE_DESCRIPTION,PAR_SERVICE_ID,PAR_SERVICE_RECORD_ID,ENTITLEMENT_XML, QUOTE_SERVICE_GREENBOOK_ENTITLEMENT_RECORD_ID,CPQTABLEENTRYDATEADDED, CPQTABLEENTRYADDEDBY)
		...
		self._process_query(
			"""
				INSERT SAQSCA (
					...
					JOIN MAEQTY (NOLOCK) ON MAEQTY.EQUIPMENT_TYPE_ID  = SAQFEA.EQUIPMENTTYPE_ID
					JOIN SAQTSV (NOLOCK) ON SAQFEA.QUOTE_RECORD_ID = SAQTSV.QUOTE_RECORD_ID  AND SAQFEA.QTEREV_RECORD_ID = SAQTSV.QTEREV_RECORD_ID
					WHERE SYSPBT.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SYSPBT.BATCH_GROUP_RECORD_ID = '{BatchGroupRecordId}' AND MAEQTY.COSTING_RELEVANT = 'False' AND  SYSPBT.QTEREV_RECORD_ID = '{RevisionRecordId}' AND SAQTSV.SERVICE_ID = '{TreeParam}' AND SAQTSV.SERVICE_TYPE = '{TreeParentParam}'
				""".format( 
				UserId=self.user_id,
				UserName=self.user_name,
