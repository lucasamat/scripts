			...
			if self.values:
				...
				if self.action_type == "ADD_PARTS":
					...
					if self.tree_param in ('Z0108','Z0110'):
						...
						CQIFLSPARE.iflow_pullspareparts_call(str(User.UserName),str(account_info.get('SOLD TO')),'["00'+str(account_info.get('SHIP TO'))+'"]',salesorg, pricelist,pricegroup,'Yes','Yes',part_numbers,validfrom,validto,self.contract_quote_id,self.quote_revision_record_id,auth)
					else:	
						self._process_query("""
				...
				if self.all_values:        
					...
					#ADDED FOR ADD ON PRODUCT SCENARIO
						...
						if self.tree_param == "Sending Equipment" or self.tree_param == "Receiving Equipment":
							...
							query_string = "SELECT DISTINCT QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID FROM SAQFEQ (NOLOCK) LEFT JOIN SAQSCO(NOLOCK) ON SAQSCO.EQUIPMENT_ID = SAQFEQ.EQUIPMENT_ID AND SAQSCO.TEMP_TOOL = SAQFEQ.TEMP_TOOL WHERE QUOTE_RECORD_ID = '{QuoteRecordId}' AND QTEREV_RECORD_ID = '{RevisionRecordId}' {is_temptool} AND {Qury_Str} AND SAQSCO.SERVICE_ID ='{TreeParam}' AND ISNULL(SAQSCO.EQUIPMENT_RECORD_ID,'')='' ".format(QuoteRecordId=self.contract_quote_record_id,RevisionRecordId=self.quote_revision_record_id,TreeParam = self.tree_param,Qury_Str=qury_str,is_temptool= " AND TEMP_TOOL ='TRUE' " if self.tree_param == "Z0099" else " AND (TEMP_TOOL IS NULL OR TEMP_TOOL = 'False' OR TEMP_TOOL = '') ")
					query_string_for_count = "SELECT COUNT(*) as count FROM ({Query_String})OQ".format(
						Query_String=query_string
					)
					table_count_data = Sql.GetFirst(query_string_for_count)
