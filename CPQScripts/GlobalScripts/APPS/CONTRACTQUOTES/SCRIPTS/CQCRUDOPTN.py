	...
	def _insert_quote_service_preventive_maintenance_kit_parts(self, **kwargs):
		...
		if self.pmevents_changes_insert == "Yes":
			self._process_query("""INSERT SAQSAP (
					...
					FROM SAQSCO (NOLOCK) 
					JOIN SAQSCA(NOLOCK) ON SAQSCA.QUOTE_RECORD_ID = SAQSCO.QUOTE_RECORD_ID AND SAQSCA.QTEREV_RECORD_ID = SAQSCO.QTEREV_RECORD_ID
					...
					WHERE SAQSCO.QUOTE_RECORD_ID = '{QuoteRecordId}' AND {additional_where} SAQSCO.QTEREV_RECORD_ID = '{RevisionRecordId}' AND SAQSCA.SERVICE_ID = '{TreeParam}' ) PM """.format(
					UserName=self.user_name,
					TreeParam=self.tree_param,
					QuoteId = self.contract_quote_id,
					QuoteRecordId=self.contract_quote_record_id,
	...
	def _create(self):
		...
		if self.action_type == "ADD_COVERED_OBJ":
			...
			if self.values:
				...
				Trace.Write("primaryQueryItems_chk "+str(primaryQueryItems))
