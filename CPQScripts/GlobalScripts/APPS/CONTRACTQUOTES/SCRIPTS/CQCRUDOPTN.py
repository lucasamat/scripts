		...
		if self.values:
			...
			self._process_query(
							"""
								...
								warranty_start = " '' AS WARRANTY_START_DATE" if self.tool_type=="TEMP_TOOL" else " MAEQUP.WARRANTY_START_DATE ",
								warranty_end = " '' AS WARRANTY_END_DATE" if self.tool_type=="TEMP_TOOL" else " MAEQUP.WARRANTY_END_DATE ",
...
class ContractQuoteCoveredObjModel(ContractQuoteCrudOpertion):
	...
	def applied_preventive_maintainence(self, **kwargs):
		...
		if kwargs.get('applied_preventive_maintainence_quote_type_changed') == "Yes":
			...
			saqgpa_temp_table_insert = SqlHelper.GetFirst("sp_executesql @T=N'SELECT SAQSCO.EQUIPMENT_ID,SAQSCO.EQUIPMENT_DESCRIPTION,SAQSCO.EQUIPMENT_RECORD_ID,SAQGPM.GOT_CODE,SAQGPM.GOTCODE_RECORD_ID,SAQGPM.GREENBOOK,SAQGPM.GREENBOOK_RECORD_ID,SAQGPM.PM_ID,SAQGPM.PM_NAME,SAQGPM.PM_RECORD_ID,SAQGPM.PM_LEVEL,SAQGPM.SERVICE_ID,SAQGPM.SERVICE_DESCRIPTION,SAQGPM.SERVICE_RECORD_ID,SAQGPM.PM_FREQUENCY,SAQGPM.SSCM_PM_FREQUENCY,SAQGPM.PM_FREQUENCY_EDITABLE,SAQGPM.PROCESS_TYPE,SAQGPM.DEVICE_NODE,SAQGPM.QUOTE_ID,SAQGPM.QUOTE_RECORD_ID,SAQGPM.QTEREV_ID,SAQGPM.QTEREV_RECORD_ID,SAQGPM.QTESRV_RECORD_ID,SAQGPM.QTESRVGBK_RECORD_ID,SAQGPM.QUOTE_REV_PO_GBK_GOT_CODE_PM_EVENTS_RECORD_ID as QTEREVPME_RECORD_ID INTO "+str(saqgpa_backup_table)+" FROM SAQGPM (NOLOCK) JOIN SAQSCO(NOLOCK) ON SAQSCO.QUOTE_RECORD_ID = SAQGPM.QUOTE_RECORD_ID AND SAQSCO.QTEREV_RECORD_ID = SAQGPM.QTEREV_RECORD_ID AND SAQSCO.SERVICE_RECORD_ID = SAQGPM.SERVICE_RECORD_ID AND SAQSCO.GREENBOOK_RECORD_ID = SAQGPM.GREENBOOK_RECORD_ID  WHERE SAQGPM.QUOTE_RECORD_ID = ''{}'' AND SAQGPM.QTEREV_RECORD_ID = ''{}'' AND  SAQGPM.SERVICE_ID = ''{}'' '".format(QuoteRecordId,RevisionRecordId,TreeParam))
			...
			self._process_query(
				"""INSERT SAQGPA (
						...
						FROM saqgpa_backup_table (NOLOCK) 
			...
			self._process_query(
					"""INSERT SAQSKP (
						...
						--SAQSCO.QUOTE_SERVICE_COVERED_OBJECT_ASSEMBLIES_RECORD_ID AS QTESRVMASY_RECORD_ID,
						SAQSCO.PAR_SERVICE_DESCRIPTION,
						SAQSCO.PAR_SERVICE_ID,
						SAQSCO.PAR_SERVICE_RECORD_ID,
						SAQGPM.QUOTE_REV_PO_GBK_GOT_CODE_PM_EVENTS_RECORD_ID
						FROM SAQGPA (NOLOCK)
						JOIN MAEQUP (NOLOCK) ON MAEQUP.EQUIPMENT_ID = SAQGPA.ASSEMBLY_ID  AND MAEQUP.PAR_EQUIPMENT_RECORD_ID = SAQGPA.EQUIPMENT_RECORD_ID
						JOIN SAQSCO (NOLOCK) ON SAQSCO.QUOTE_RECORD_ID = SAQGPA.QUOTE_RECORD_ID AND SAQSCO.QTEREV_RECORD_ID = SAQGPA.QTEREV_RECORD_ID
						...
						SAQGPA.SERVICE_RECORD_ID =  SAQGPM.SERVICE_RECORD_ID AND SAQGPA.GREENBOOK_RECORD_ID =  SAQGPM.GREENBOOK_RECORD_ID AND SAQGPA.GOTCODE_RECORD_ID =  SAQGPM.GOTCODE_RECORD_ID AND SAQGPA.PM_RECORD_ID =  SAQGPM.PM_RECORD_ID
			...
			self._process_query(
				"""INSERT SAQGPA (
						...
						SAQGPM.SERVICE_RECORD_ID,
						SAQGPM.PM_FREQUENCY,
						SAQGPM.SSCM_PM_FREQUENCY,
						SAQGPM.PM_FREQUENCY_EDITABLE,
						SAQGPM.PROCESS_TYPE,
						SAQGPM.DEVICE_NODE,
						SAQGPM.QUOTE_ID,
						SAQGPM.QUOTE_RECORD_ID,
					...
					JOIN SAQSCO(NOLOCK) ON SAQGPM.QUOTE_RECORD_ID = SAQSCO.QUOTE_RECORD_ID AND SAQGPM.QTEREV_RECORD_ID = SAQSCO.QTEREV_RECORD_ID AND SAQSCO.SERVICE_RECORD_ID = SAQGPM.SERVICE_RECORD_ID AND SAQSCO.GREENBOOK_RECORD_ID = SAQGPM.GREENBOOK_RECORD_ID
					JOIN MAEQUP(NOLOCK) ON MAEQUP.PAR_EQUIPMENT_RECORD_ID = SAQSCO.EQUIPMENT_RECORD_ID
					WHERE SYSPBT.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SYSPBT.BATCH_GROUP_RECORD_ID = '{BatchGroupRecordId}' AND SYSPBT.
			...
			self._process_query(
				"""INSERT SAQSKP (
					...
					JOIN SAQGPM (NOLOCK) ON SAQSCO.QUOTE_RECORD_ID = SAQGPM.QUOTE_RECORD_ID AND SAQSCO.QTEREV_RECORD_ID = SAQGPM.QTEREV_RECORD_ID AND SAQSCO.SERVICE_RECORD_ID =  SAQGPM.SERVICE_RECORD_ID AND SAQSCO.GREENBOOK_RECORD_ID =  SAQGPM.GREENBOOK_RECORD_ID  AND
					SAQGPA.QUOTE_RECORD_ID = SAQGPM.QUOTE_RECORD_ID AND SAQGPA.QTEREV_RECORD_ID = SAQGPM.QTEREV_RECORD_ID AND
					SAQGPA.SERVICE_RECORD_ID =  SAQGPM.SERVICE_RECORD_ID AND SAQGPA.GREENBOOK_RECORD_ID =  SAQGPM.GREENBOOK_RECORD_ID AND SAQGPA.GOTCODE_RECORD_ID =  SAQGPM.GOTCODE_RECORD_ID AND SAQGPA.GOTCODE_RECORD_ID =  SAQGPM.GOTCODE_RECORD_ID AND SAQGPA.PM_RECORD_ID =  SAQGPM.PM_RECORD_ID
					JOIN MAPMEV(NOLOCK) ON MAPMEV.PM_NAME = SAQGPA.PM_NAME
					JOIN MAKTPT(NOLOCK) ON MAKTPT.KIT_ID = SAQGPM.KIT_ID
					JOIN MAMTRL(NOLOCK) ON MAMTRL.SAP_PART_NUMBER = MAKTPT.PART_NUMBER
