							...
							"""
								INSERT SAQFEQ (
									...
									TEMP_TOOL
										...
										JOIN SYSPBT (NOLOCK) ON SYSPBT.BATCH_RECORD_ID = MAEQUP.EQUIPMENT_RECORD_ID JOIN MAEQCT(NOLOCK)
										ON MAEQUP.EQUIPMENTCATEGORY_ID = MAEQCT.EQUIPMENTCATEGORY_ID
										JOIN SAQTMT ON SAQTMT.MASTER_TABLE_QUOTE_RECORD_ID = SYSPBT.QUOTE_RECORD_ID AND SAQTMT.QTEREV_RECORD_ID = SYSPBT.QTEREV_RECORD_ID AND SAQTMT.ACCOUNT_ID = MAEQUP.ACCOUNT_ID
										JOIN SAQTRV ON SAQTRV.QUOTE_RECORD_ID = SAQTMT.MASTER_TABLE_QUOTE_RECORD_ID AND SAQTRV.QTEREV_RECORD_ID = SAQTMT.QTEREV_RECORD_ID  AND MAEQUP.SALESORG_ID = SAQTRV.SALESORG_ID
										WHERE 
			...
			"""
				INSERT SAQSCO (
					...
					CpqTableEntryDateModified
						...
						SAQTSV.SERVICE_ID = '{TreeParam}' AND
						SAQTSV.SERVICE_TYPE = '{TreeParentParam}'
						LEFT JOIN SAQSCO(NOLOCK) ON SAQSCO.EQUIPMENT_ID = SAQFEQ.EQUIPMENT_ID AND SAQSCO.TEMP_TOOL = SAQFEQ.TEMP_TOOL AND SAQSCO.QTEREV_RECORD_ID = SAQFEQ.QTEREV_RECORD_ID AND SAQSCO.QUOTE_RECORD_ID=SAQFEQ.QUOTE_RECORD_ID
