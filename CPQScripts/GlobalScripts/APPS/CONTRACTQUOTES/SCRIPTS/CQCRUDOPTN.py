	...
	def _create(self):		
		...
		if self.action_type == "ADD_OFFERING":
			...
			if self.values:
				...
				Trace.Write("self.values[0].splitlines()----->"+str(self.values[0].splitlines()))
		...
		if 'Z0016' in where_string:
			self._process_query("""UPDATE SAQICO
									...
									LEFT JOIN (
										...
										) SAQSCE_TEMP ON SAQSCE_TEMP.QUOTE_ID = SAQICO.QUOTE_ID AND SAQSCE_TEMP.EQUIPMENT_ID = SAQICO.EQUIPMENT_ID AND SAQSCE_TEMP.SERVICE_ID = SAQICO.SERVICE_ID				
									WHERE 
