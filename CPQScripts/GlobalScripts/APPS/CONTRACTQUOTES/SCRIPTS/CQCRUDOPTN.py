		...
		if self.action_type == "ADD_COVERED_OBJ":
			...
			if self.values:
				...
				if self.trigger_from == 'PythonScript':
					...
					ancillary_dict = ancillary_dict_val= ancillary_dict_val_service =""
					try: 
						ancillary_dict = Quote.GetCustomField('ANCILLARY_DICT').Content
						if ancillary_dict:
							ancillary_dict_val_service = (re.sub(r'^{|}$','',ancillary_dict)).split(':')[0]
