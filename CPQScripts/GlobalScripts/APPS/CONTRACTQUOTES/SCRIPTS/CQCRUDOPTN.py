		...
		if self.values:
			...
			self._process_query(
					"""INSERT SAQFGB(
						...
						JOIN SAQFGB (NOLOCK) ON SAQTMT.MASTER_TABLE_QUOTE_RECORD_ID = SAQFGB.QUOTE_RECORD_ID AND SAQTMT.QTEREV_RECORD_ID = SAQFGB.QTEREV_RECORD_ID AND 
    					SAQFEQ.GREENBOOK != SAQFGB.GREENBOOK AND SAQFGB.QTEREV_RECORD_ID = SAQFGB.QTEREV_RECORD_ID AND SAQFGB.FABLOCATION_ID=SAQFEQ.FABLOCATION_ID
						WHERE SAQFEQ.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQFEQ.QTEREV_RECORD_ID = '{RevisionRecordId}' AND SYSPBT.BATCH_GROUP_RECORD_ID = '{BatchGroupRecordId}' AND SAQFGB.FABLOCATION_ID IN {fab} AND SAQFEQ.FABLOCATION_ID IN {fab} 
						) FB""".format(
										treeparam=self.tree_param,
										QuoteRecordId=self.contract_quote_record_id,
										RevisionRecordId=self.quote_revision_record_id,
	...
	def _create(self):
		if self.action_type == "ADD_COVERED_OBJ":
			...
			if self.values:
				...
				try:						
					CQTVLDRIFW.iflow_valuedriver_rolldown(self.contract_quote_record_id,"CSA TOOLS",self.tree_param, self.tree_parent_level_0, self.tree_parent_level_1, self.tree_parent_level_2,self.user_id,self.user_name,self.quote_revision_record_id)
				except:
					Trace.Write("EXCEPT----CSA TOOLS VALUE DRIVER LEVEL IFLOW")
