	...
	def applied_preventive_maintainence(self, **kwargs):
		...
		if kwargs.get('applied_preventive_maintainence_quote_type_changed') == "Yes":
			...
			self._process_query("""INSERT SAQGPM(CHAMBER_QUANTITY,
					...
					SAQRGG.SERVICE_RECORD_ID = SAQSCO.SERVICE_RECORD_ID AND SAQRGG.GREENBOOK_RECORD_ID = SAQSCO.GREENBOOK_RECORD_ID
					JOIN MAEQUP(NOLOCK) ON MAEQUP.PAR_EQUIPMENT_ID = SAQSCO.EQUIPMENT_ID
					JOIN MAEAPK(NOLOCK) ON MAEAPK.EQUIPMENT_RECORD_ID = SAQSCO.EQUIPMENT_RECORD_ID AND MAEAPK.ASSEMBLY_ID = MAEQUP.EQUIPMENT_ID 
			...
			self._process_query(
				"""INSERT SAQSKP (
					...
					SELECT DISTINCT
					SAQGPA.ASSEMBLY_ID,
					SAQSCO.ASSEMBLY_DESCRIPTION,
					SAQSCO.ASSEMBLY_RECORD_ID,
					SAQGPA.EQUIPMENT_ID,
					SAQSCO.EQUIPMENT_DESCRIPTION,
					SAQSCO.EQUIPMENT_RECORD_ID,
					MAMTRL.SAP_PART_NUMBER,
