		...
		if self.action_type == "ADD_OFFERING":
			...
			if self.values:
				...
				self._process_query("""
									...
									WHERE SYSPBT.BATCH_STATUS = 'IN PROGRESS' AND SYSPBT.BATCH_GROUP_RECORD_ID = '{BatchGroupRecordId}' AND SYSPBT.QUOTE_RECORD_ID = '{QuoteRecordId}' AND MAMTRL.PRODUCT_TYPE IS NULL AND (MAMTRL.IS_SPARE_PART = 1 or MAMTRL.IS_SPARE_PART IS NULL)) IQ
									""".format(
						ServiceId=self.tree_param,
						BatchGroupRecordId=batch_group_record_id,
						QuoteRecordId=self.contract_quote_record_id,
