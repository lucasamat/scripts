			...
			self._process_query(
						"""
							INSERT SAQSCA (
								...
								{included} as INCLUDED 
								FROM SAQSCO (NOLOCK)
								JOIN (select SAQFEA.* from SAQFEA(nolock) join SAQFEQ (nolock) on SAQFEA.QUOTE_RECORD_ID = SAQFEQ.QUOTE_RECORD_ID and SAQFEQ.QTEREV_RECORD_ID = SAQFEQ.QTEREV_RECORD_ID where  SAQFEA.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQFEA.QTEREV_RECORD_ID = '{RevisionRecordId}' ) SAQFEA ON SAQFEA.QUOTE_RECORD_ID = SAQFEQ.QUOTE_RECORD_ID AND SAQFEA.QTEREV_RECORD_ID = SAQFEQ.QTEREV_RECORD_ID
								...
								WHERE SAQFEQ.QUOTE_RECORD_ID = '{QuoteRecordId}' AND  SAQFEQ.QTEREV_RECORD_ID = '{RevisionRecordId}' AND SAQTSV.SERVICE_ID = '{TreeParam}' AND SAQTSV.SERVICE_TYPE = '{TreeParentParam}'
							""".format( 
							UserId=self.user_id,
							UserName=self.user_name,
