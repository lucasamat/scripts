	...
	def _create(self):
		...
		if self.action_type == "ADD_FAB":
			...
			for OfferingRow_detail in service_addon_object:
				...
				CQADDONPRD.addon_greenbook_level_entitlement(OfferingRow_detail,self.tree_parent_level_0)
				'''Request_URL="https://cpservices-product-configuration.cfapps.us10.hana.ondemand.com/api/v2/configurations?autoCleanup=False"
			...
			for key,val in enumerate(list(self.values)):
				...
				if APPLIED_CREDITS!='' and CREDIT_AMOUNTS!='':
					try:
						unapplied = abs(int(CREDIT_AMOUNTS[key]))-int(APPLIED_CREDITS[key]) if APPLIED_CREDITS[key]!='' and CREDIT_AMOUNTS[key] else int(CREDIT_AMOUNTS[key])
