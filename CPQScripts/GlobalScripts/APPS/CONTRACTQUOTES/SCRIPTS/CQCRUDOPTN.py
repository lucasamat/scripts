		...
		if kwargs.get('applied_preventive_maintainence_quote_type_changed') == "Yes":
			...
			self._process_query(
				"""INSERT SAQGPA (
						...
						FROM SAQGPM (NOLOCK) 
					JOIN SAQSCO(NOLOCK) ON SAQSCO.QUOTE_RECORD_ID = SAQGPM.QUOTE_RECORD_ID AND SAQSCO.QTEREV_RECORD_ID = SAQGPM.QTEREV_RECORD_ID AND SAQSCO.SERVICE_RECORD_ID = SAQGPM.SERVICE_RECORD_ID AND SAQSCO.GREENBOOK_RECORD_ID = SAQGPM.GREENBOOK_RECORD_ID
					JOIN MAEQUP(NOLOCK) ON MAEQUP.PAR_EQUIPMENT_RECORD_ID = SAQSCO.EQUIPMENT_RECORD_ID 
					WHERE SAQGPM.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQGPM.
					QTEREV_RECORD_ID = '{RevisionRecordId}' AND SAQSCO.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQSCO.
	...
	def _create(self):
		...
		if self.action_type == "ADD_COVERED_OBJ":
			...
			if self.pmevents_changes_insert == "Yes":
				...
				Trace.Write("record_ids--->"+str(record_ids))
				primaryQueryItems = Sql.RunQuery("INSERT SYSPBT(BATCH_RECORD_ID, BATCH_STATUS, QUOTE_ID, QUOTE_RECORD_ID, BATCH_GROUP_RECORD_ID,QTEREV_RECORD_ID) SELECT SAQFEQ.EQUIPMENT_RECORD_ID as BATCH_RECORD_ID, 'IN PROGRESS' as BATCH_STATUS, SAQFEQ.QUOTE_ID, SAQFEQ.QUOTE_RECORD_ID, '"+str(batch_group_record_id)+"' as BATCH_GROUP_RECORD_ID,'"+str(self.quote_revision_record_id)+"' as QTEREV_RECORD_ID FROM SAQFEQ (NOLOCK) JOIN splitstring('"+record_ids+"') ON ltrim(rtrim(NAME)) = SAQFEQ.QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID")
