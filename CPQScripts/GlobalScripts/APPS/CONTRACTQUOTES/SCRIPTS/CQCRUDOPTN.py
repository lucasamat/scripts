		...
		if kwargs.get('applied_preventive_maintainence_quote_type_changed') == "Yes" and kwargs.get('tools_from_ui') != "Yes":
			...
			self._process_query(
					"""INSERT SAQSKP (
						...
						WHERE SAQSCO.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQSCO.QTEREV_RECORD_ID = '{RevisionRecordId}' AND SAQSCO.SERVICE_ID = '{TreeParam}') KP LEFT JOIN SAQSKP (NOLOCK) AS KIT ON 
			...
			self._process_query(
				"""INSERT SAQSKP (
					...
					SELECT KP.*,CONVERT(VARCHAR(4000),NEWID()) as QUOTE_SERVICE_COV_OBJ_ASS_PM_KIT_PARTS_RECORD_ID,'{UserName}' as CPQTABLEENTRYADDEDBY, GETDATE() as CPQTABLEENTRYDATEADDED FROM (
						...
						SAQGPA.SERVICE_RECORD_ID =  SAQGPM.SERVICE_RECORD_ID AND SAQGPA.GREENBOOK_RECORD_ID =  SAQGPM.GREENBOOK_RECORD_ID AND SAQGPA.GOTCODE_RECORD_ID =  SAQGPM.GOTCODE_RECORD_ID AND SAQGPA.PM_RECORD_ID =  SAQGPM.PM_RECORD_ID WHERE SAQSCO.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQSCO.QTEREV_RECORD_ID = '{RevisionRecordId}' AND SAQSCO.SERVICE_ID = '{TreeParam}' ) assembly_and_event_object
					JOIN MAPMEV(NOLOCK) ON MAPMEV.PM_NAME = assembly_and_event_object.PM_NAME
					WHERE SYSPBT.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SYSPBT.BATCH_GROUP_RECORD_ID = '{BatchGroupRecordId}' AND SYSPBT.QTEREV_RECORD_ID = '{RevisionRecordId}' AND SAQSCO.SERVICE_ID = '{TreeParam}') KP LEFT JOIN SAQSKP (NOLOCK) AS KIT ON 
					KP.QUOTE_RECORD_ID =KIT.QUOTE_RECORD_ID AND KP.QTEREV_RECORD_ID =KIT.QTEREV_RECORD_ID AND KP.SERVICE_RECORD_ID =KIT.SERVICE_RECORD_ID AND KP.EQUIPMENT_RECORD_ID =KIT.EQUIPMENT_RECORD_ID  AND KP.ASSEMBLY_RECORD_ID =KIT.ASSEMBLY_RECORD_ID AND KP.PM_RECORD_ID =KIT.PM_RECORD_ID AND KP.KIT_RECORD_ID =KIT.KIT_RECORD_ID  AND KP.KITNUMBER_RECORD_ID =KIT.KIT_NUMBER_RECORD_ID AND ISNULL(KIT.KIT_RECORD_ID,'') = '' """.format(
					UserName=self.user_name,
					TreeParam=self.tree_param,
					QuoteId = self.contract_quote_id,
			...
			Sql.RunQuery("""UPDATE SAQSKP
						SET PART_NUMBER = IQ.SAP_PART_NUMBER,
							...
							PART_RECORD_ID = IQ.MATERIAL_RECORD_ID		
							FROM SAQSKP (NOLOCK)
							INNER JOIN (SELECT MAKTPT.KIT_ID,MAMTRL.SAP_PART_NUMBER,MAMTRL.SAP_DESCRIPTION,MAMTRL.MATERIAL_RECORD_ID FROM SAQSKP(NOLOCK) 
			...
			Sql.RunQuery("""UPDATE SAQSKP
									SET QUANTITY = IQ.QUANTITY		
										FROM SAQSKP (NOLOCK)
										INNER JOIN (SELECT MAKTPT.KIT_ID,MAKTPT.QUANTITY FROM SAQSKP(NOLOCK) 
										JOIN  MAKTPT(NOLOCK) ON MAKTPT.KIT_ID = SAQSKP.KIT_ID
										JOIN SYSPBT (NOLOCK) ON SYSPBT.QUOTE_RECORD_ID = SAQSKP.QUOTE_RECORD_ID AND SYSPBT.QTEREV_RECORD_ID = SAQSKP.QTEREV_RECORD_ID 
											WHERE SAQSKP.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQSKP.QTEREV_RECORD_ID = '{RevisionRecordId}' AND SAQSKP.SERVICE_ID = '{TreeParam}' AND SYSPBT.BATCH_GROUP_RECORD_ID = '{BatchGroupRecordId}'
											)IQ ON SAQSKP.QUOTE_RECORD_ID = IQ.QUOTE_RECORD_ID AND SAQSKP.QTEREV_RECORD_ID = IQ.QTEREV_RECORD_ID AND SAQSKP.SERVICE_ID = '{TreeParam}' """.format(QuoteRecordId=self.contract_quote_record_id,RevisionRecordId=self.quote_revision_record_id,TreeParam=self.tree_param,BatchGroupRecordId =kwargs.get('batch_group_record_id')  ))
	...
	def _create(self):
		...
		if self.action_type == "ADD_COVERED_OBJ":
			...
			if self.values:
				...
				primaryQueryItems = SqlHelper.GetFirst(""+str(parameter.QUERY_CRITERIA_1)+" SYSPBT(BATCH_RECORD_ID, BATCH_STATUS, QUOTE_ID, QUOTE_RECORD_ID, BATCH_GROUP_RECORD_ID,QTEREV_RECORD_ID,EQUIPMENT_RECORD_ID) SELECT SAQFEQ.EQUIPMENT_RECORD_ID as BATCH_RECORD_ID, ''IN PROGRESS'' as BATCH_STATUS, SAQFEQ.QUOTE_ID, SAQFEQ.QUOTE_RECORD_ID, ''"+str(batch_group_record_id)+"'' as BATCH_GROUP_RECORD_ID,''"+str(self.quote_revision_record_id)+"'' as QTEREV_RECORD_ID,SAQFEQ.QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID AS EQUIPMENT_RECORD_ID FROM SAQFEQ (NOLOCK) JOIN splitstring(''"+record_ids+"'') ON ltrim(rtrim(NAME)) = SAQFEQ.QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID'")
