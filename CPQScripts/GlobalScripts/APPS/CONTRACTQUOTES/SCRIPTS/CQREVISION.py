...
def create_new_revision(Opertion,cartrev):
	...
	if Quote is not None:
		...
		Quote = QuoteHelper.Edit(get_quote_info_details.QUOTE_ID)
...
def set_active_revision(Opertion,cartrev):
	...
	if get_rev_quote_info_details:
		...
		get_rev_info_details = Sql.GetFirst("select QTEREV_ID,CART_ID from SAQTRV where QUOTE_RECORD_ID = '"+str(quote_contract_recordId)+"' and QUOTE_REVISION_RECORD_ID = '"+str(recid)+"'")
