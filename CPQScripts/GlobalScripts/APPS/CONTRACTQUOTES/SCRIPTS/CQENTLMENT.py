		...
		if EntitlementType == 'Dropdown':
			...
			if "calc" in AttributeID:
				...
				for key,dict_val in ENT_IP_DICT.items():
					...
					if str(self.treeparam) in ("Z0091", "Z0004","Z0007","Z0006","Z0092","Z0004") and key in ( "AGS_{}_TSC_CONSUM".format(serviceId), "AGS_{}_TSC_NONCNS".format(serviceId), "AGS_{}_NON_CONSUMABLE".format(serviceId) ):
	...
	def Rolldown(self):
		...
		if AttributeList:
			...
			try:						
				...
				ent_params_list = str(whereReq)+"||"+str(add_where)+"||"+str(AttributeID)+"||"+str(NewValue)+"||"+str(ServiceId) + "||" + 'SAQTSE'
