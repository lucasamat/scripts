...
def update_document_type(QuoteRecordId,RevisionRecordId,ServicerecordId):
    ...
    getRevision = Sql.GetFirst("SELECT CpqTableEntryId FROM SAQTRV (NOLOCK) WHERE QUOTE_RECORD_ID = '{}' AND QUOTE_REVISION_RECORD_ID = '{}' AND DOCTYP_ID IS NOT NULL AND DOCTYP_ID != '' ".format(QuoteRecordId,RevisionRecordId))
