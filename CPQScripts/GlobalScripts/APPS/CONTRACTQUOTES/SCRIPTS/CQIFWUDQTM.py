	...
	Sql.RunQuery("""UPDATE SAQRIT 
					...
					FROM SAQRIT
						INNER JOIN (SELECT SAQICO.QUOTE_RECORD_ID, SAQICO.QTEREV_RECORD_ID,SAQICO.SERVICE_ID,SAQICO.GREENBOOK,SAQICO.EQUIPMENT_ID,
							...
							WHERE SAQRIS.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQRIS.QTEREV_RECORD_ID='{rev}' AND SAQRIT.SERVICE_ID IN ('Z0046') AND STATUS IS NULL OR STATUS = 'ACQUIRING' """.format( QuoteRecordId= get_rev_rec_id.MASTER_TABLE_QUOTE_RECORD_ID ,rev =get_rev_rec_id.QTEREV_RECORD_ID))
