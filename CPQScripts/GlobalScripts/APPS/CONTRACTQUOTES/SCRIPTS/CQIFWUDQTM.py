...
def quote_items_pricing(Qt_id):
	...
	Sql.RunQuery("""UPDATE SAQICO
			...
			JOIN (
				...
				GROUP BY PRPRBM.SERVICE_PRODUCT_NAME, PRPRBM.EQUIPMENT_NUMBER, PRPRBM.SERNUM, SAQICO.CpqTableEntryId
			)AS IQ ON SAQICO.CpqTableEntryId = IQ.CpqTableEntryId
			JOIN PRPRBM (NOLOCK) ON PRPRBM.EQUIPMENT_NUMBER = IQ.EQUIPMENT_NUMBER AND PRPRBM.SERVICE_PRODUCT_NAME = IQ.SERVICE_PRODUCT_NAME AND PRPRBM.SERNUM = IQ.SERNUM AND PRPRBM.CONTRACT_START_DATE = IQ.CONTRACT_START_DATE
			WHERE SAQICO.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQICO.QTEREV_RECORD_ID = '{QuoteRevisionRecordId}'					
