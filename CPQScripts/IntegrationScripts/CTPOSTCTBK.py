...
try:
	...
	""
	+ str(Parameter.QUERY_CRITERIA_1)
	+ "  CTCSCO(CONTRACT_ID,CONTRACT_NAME,SERVICE_ID,SERVICE_DESCRIPTION,CUSTOMER_TOOL_ID,WAFER_SIZE,PBG,PLATFORM,MATERIAL_RECORD_ID,MNT_PLANT_RECORD_ID,MNT_PLANT_NAME,MNT_PLANT_ID,EQUIPMENTCATEGORY_ID,EQUIPMENT_DESCRIPTION,EQUIPMENT_ID,EQUIPMENT_STATUS,EQUIPMENTTYPE_ID,FABLOCATION_ID,FABLOCATION_NAME,EQUIPMENTCATEGORY_RECORD_ID,EQUIPMENT_RECORD_ID,EQUIPMENTTYPE_RECORD_ID,FABLOCATION_RECORD_ID,SERIAL_NUMBER,WARRANTY_START_DATE,SAP_PART_NUMBER,GREENBOOK,GREENBOOK_RECORD_ID,CONTRACT_RECORD_ID,CNTSRV_RECORD_ID,WARRANTY_END_DATE,TECHNOLOGY,SERVICE_RECORD_ID,CONTRACT_SERVICE_EQUIPMENT_RECORD_ID,CPQTABLEENTRYADDEDBY,ADDUSR_RECORD_ID,CPQTABLEENTRYDATEADDED,CpqTableEntryModifiedBy,CpqTableEntryDateModified) SELECT SUB_CTCSCO.*,CONVERT(VARCHAR(4000),NEWID()),''"+ str(User.UserName)+ "'',''"+ str(User.Id)+ "'',GETDATE(), ''"+ str(User.Id)+ "'',GETDATE() FROM (SELECT DISTINCT CTCICO.CONTRACT_ID,CTCICO.CONTRACT_NAME,CTCICO.SERVICE_ID,CTCICO.SERVICE_DESCRIPTION,CTCICO.CUSTOMER_TOOL_ID,CTCICO.WAFER_SIZE,CTCICO.PBG,CTCICO.PLATFORM,CTCICO.MATERIAL_RECORD_ID,CTCICO.MNT_PLANT_RECORD_ID,CTCICO.MNT_PLANT_NAME,CTCICO.MNT_PLANT_ID,CTCICO.EQUIPMENTCATEGORY_ID,CTCICO.EQUIPMENT_DESCRIPTION,CTCICO.EQUIPMENT_ID,CTCICO.EQUIPMENT_STATUS,CTCICO.EQUIPMENTTYPE_ID,CTCICO.FABLOCATION_ID,CTCICO.FABLOCATION_NAME,CTCICO.EQUIPMENTCATEGORY_RECORD_ID,CTCICO.EQUIPMENT_RECORD_ID,CTCICO.EQUIPMENTTYPE_RECORD_ID,CTCICO.FABLOCATION_RECORD_ID,CTCICO.SERIAL_NO,CTCICO.WARRANTY_START_DATE,CTCICO.SAP_PART_NUMBER,CTCICO.GREENBOOK,CTCICO.GREENBOOK_RECORD_ID,CTCICO.CONTRACT_RECORD_ID,CTCTSV.CONTRACT_SERVICES_RECORD_ID,CTCICO.WARRANTY_END_DATE,CTCICO.TECHNOLOGY,CTCICO.SERVICE_RECORD_ID FROM CTCICO_INBOUND(NOLOCK) JOIN CTCICO (NOLOCK) ON CTCICO_INBOUND.CONTRACT_ID = CTCICO.CONTRACT_ID AND CTCICO_INBOUND.LINE_ITEM_ID = CTCICO.EQUIPMENT_LINE_ID JOIN CTCTSV (NOLOCK) ON CTCICO.CONTRACT_ID = CTCTSV.CONTRACT_ID AND CTCICO.SERVICE_ID = CTCTSV.SERVICE_ID WHERE CTCICO_INBOUND.PROCESS_STATUS = ''READY FOR UPLOAD'' AND ISNULL(INTEGRATION_STATUS,'''')='''' AND CTCICO_INBOUND.TIMESTAMP='"+ str(timestamp_sessionid)+ "' )SUB_CTCSCO LEFT JOIN CTCSCO (NOLOCK) ON SUB_CTCSCO.CONTRACT_ID = CTCSCO.CONTRACT_ID AND SUB_CTCSCO.SERVICE_ID = CTCSCO.SERVICE_ID AND SUB_CTCSCO.EQUIPMENT_ID = CTCSCO.EQUIPMENT_ID WHERE CTCSCO.CONTRACT_ID IS NULL '")
