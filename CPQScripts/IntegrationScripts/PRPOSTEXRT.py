...
try:
	...
	if Check_flag == 1:
		...
		primaryQueryItems = SqlHelper.GetList("select distinct EXCHANGE_RATE_DATE AS sap_part_number,INTEGRATION_STATUS from "+str(Table_Name)+" (nolock) where isnull(INTEGRATION_STATUS,'')<>'' AND INTEGRATION_NAME = 'EXCHANGERATE_DATA' AND  and timestamp = "+str(timestamp_sessionid)+" ")
