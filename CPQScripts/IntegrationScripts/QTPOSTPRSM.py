...
try:
	...
	for crmifno in input_data:	
		...
		CRMTMP = "CRMTMP_BKP_1"+str(CRMQT.c4c_quote_id)
		...
		while Check_flag == 1:
			...
			table3 = SqlHelper.GetFirst(
				"sp_executesql @T=N'INSERT "+str(CRMTMP)+" SELECT DISTINCT equipment_id FROM (SELECT DISTINCT equipment_id, ROW_NUMBER()OVER(ORDER BY equipment_id) AS SNO FROM "+str(SAQSCO)+"  (NOLOCK) where quote_id=''"+str(Qt_id)+"'' ) A WHERE SNO>= "+str(start)+" AND SNO<="+str(end)+" '")

			if str(ent_query) != "None":
		
				S = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET COVERAGE=ENTITLEMENT_DISPLAY_VALUE FROM  "+str(SAQSCA)+" A(NOLOCK) JOIN (SELECT distinct quote_ID,equipment_id,assembly_id,service_id, replace(X.Y.value(''(ENTITLEMENT_DISPLAY_VALUE)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DISPLAY_VALUE,replace(X.Y.value(''(ENTITLEMENT_NAME)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_NAME FROM (SELECT quote_ID,equipment_id,assembly_id,service_id,CONVERT(XML,''<QUOTE_ENTITLEMENT>''+substring(entitlement_xml,charindex (''<ENTITLEMENT_ID>AGS_CRT_CON_COV<'',entitlement_xml),charindex (''Contract Coverage</ENTITLEMENT_NAME>'',entitlement_xml)-charindex (''<ENTITLEMENT_ID>AGS_CRT_CON_COV<'',entitlement_xml)+len(''Contract Coverage</ENTITLEMENT_NAME>''))+''</QUOTE_ENTITLEMENT>'') as entitlement_xml FROM "+str(SAQSAE)+" (nolock)a JOIN "+str(CRMTMP)+" ON A.EQUIPMENT_ID = EQUIPMENT_IDD WHERE QUOTE_ID = ''"+str(Qt_id)+"'' AND SERVICE_ID NOT IN (''Z0092'',''Z0099'',''Z0091'',''Z0004'') ) e OUTER APPLY e.ENTITLEMENT_XML.nodes(''QUOTE_ENTITLEMENT'') as X(Y))B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND A.ASSEMBLY_ID = B.ASSEMBLY_ID WHERE B.ENTITLEMENT_NAME=''Contract Coverage'' '")
				
				S = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET COVERAGE=ENTITLEMENT_DISPLAY_VALUE FROM  "+str(SAQSCA)+" A(NOLOCK) JOIN (SELECT distinct quote_ID,equipment_id,assembly_id,service_id, replace(X.Y.value(''(ENTITLEMENT_DISPLAY_VALUE)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DISPLAY_VALUE,replace(X.Y.value(''(ENTITLEMENT_NAME)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_NAME FROM (SELECT quote_ID,equipment_id,assembly_id,service_id,CONVERT(XML,''<QUOTE_ENTITLEMENT>''+substring(entitlement_xml,charindex (''<ENTITLEMENT_ID>AGS_Z0091_CVR_CNTCOV<'',entitlement_xml),charindex (''Contract Coverage</ENTITLEMENT_NAME>'',entitlement_xml)-charindex (''<ENTITLEMENT_ID>AGS_Z0091_CVR_CNTCOV<'',entitlement_xml)+len(''Contract Coverage</ENTITLEMENT_NAME>''))+''</QUOTE_ENTITLEMENT>'') as entitlement_xml FROM "+str(SAQSAE)+" (nolock)a JOIN "+str(CRMTMP)+" ON A.EQUIPMENT_ID = EQUIPMENT_IDD WHERE QUOTE_ID = ''"+str(Qt_id)+"'' AND SERVICE_ID = ''Z0091'' ) e OUTER APPLY e.ENTITLEMENT_XML.nodes(''QUOTE_ENTITLEMENT'') as X(Y))B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND A.ASSEMBLY_ID = B.ASSEMBLY_ID WHERE B.ENTITLEMENT_NAME=''Contract Coverage'' '")
				
				S = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET COVERAGE=ENTITLEMENT_DISPLAY_VALUE FROM  "+str(SAQSCA)+" A(NOLOCK) JOIN (SELECT distinct quote_ID,equipment_id,assembly_id,service_id, replace(X.Y.value(''(ENTITLEMENT_DISPLAY_VALUE)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DISPLAY_VALUE,replace(X.Y.value(''(ENTITLEMENT_NAME)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_NAME FROM (SELECT quote_ID,equipment_id,assembly_id,service_id,CONVERT(XML,''<QUOTE_ENTITLEMENT>''+substring(entitlement_xml,charindex (''<ENTITLEMENT_ID>AGS_Z0004_CVR_CNTCOV<'',entitlement_xml),charindex (''Contract Coverage</ENTITLEMENT_NAME>'',entitlement_xml)-charindex (''<ENTITLEMENT_ID>AGS_Z0004_CVR_CNTCOV<'',entitlement_xml)+len(''Contract Coverage</ENTITLEMENT_NAME>''))+''</QUOTE_ENTITLEMENT>'') as entitlement_xml FROM "+str(SAQSAE)+" (nolock)a JOIN "+str(CRMTMP)+" ON A.EQUIPMENT_ID = EQUIPMENT_IDD WHERE QUOTE_ID = ''"+str(Qt_id)+"''  AND SERVICE_ID = ''Z0004'' ) e OUTER APPLY e.ENTITLEMENT_XML.nodes(''QUOTE_ENTITLEMENT'') as X(Y))B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND A.ASSEMBLY_ID = B.ASSEMBLY_ID WHERE B.ENTITLEMENT_NAME=''Contract Coverage'' '")
				
				S = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET COVERAGE=ENTITLEMENT_DISPLAY_VALUE FROM  "+str(SAQSCA)+" A(NOLOCK) JOIN (SELECT distinct quote_ID,equipment_id,assembly_id,service_id, replace(X.Y.value(''(ENTITLEMENT_DISPLAY_VALUE)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DISPLAY_VALUE,replace(X.Y.value(''(ENTITLEMENT_NAME)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_NAME FROM (SELECT quote_ID,equipment_id,assembly_id,service_id,CONVERT(XML,''<QUOTE_ENTITLEMENT>''+substring(entitlement_xml,charindex (''<ENTITLEMENT_ID>AGS_Z0099_CVR_CNTCOV<'',entitlement_xml),charindex (''Contract Coverage</ENTITLEMENT_NAME>'',entitlement_xml)-charindex (''<ENTITLEMENT_ID>AGS_Z0099_CVR_CNTCOV<'',entitlement_xml)+len(''Contract Coverage</ENTITLEMENT_NAME>''))+''</QUOTE_ENTITLEMENT>'') as entitlement_xml FROM "+str(SAQSAE)+" (nolock)a JOIN "+str(CRMTMP)+" ON A.EQUIPMENT_ID = EQUIPMENT_IDD WHERE QUOTE_ID = ''"+str(Qt_id)+"''  AND SERVICE_ID = ''Z0099'' ) e OUTER APPLY e.ENTITLEMENT_XML.nodes(''QUOTE_ENTITLEMENT'') as X(Y))B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND A.ASSEMBLY_ID = B.ASSEMBLY_ID WHERE B.ENTITLEMENT_NAME=''Contract Coverage'' '")
				
				S = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET COVERAGE=ENTITLEMENT_DISPLAY_VALUE FROM  "+str(SAQSCA)+" A(NOLOCK) JOIN (SELECT distinct quote_ID,equipment_id,assembly_id,service_id, replace(X.Y.value(''(ENTITLEMENT_DISPLAY_VALUE)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DISPLAY_VALUE,replace(X.Y.value(''(ENTITLEMENT_NAME)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_NAME FROM (SELECT quote_ID,equipment_id,assembly_id,service_id,CONVERT(XML,''<QUOTE_ENTITLEMENT>''+substring(entitlement_xml,charindex (''<ENTITLEMENT_ID>AGS_CONTRACT_COVERAGE<'',entitlement_xml),charindex (''Contract Coverage</ENTITLEMENT_NAME>'',entitlement_xml)-charindex (''<ENTITLEMENT_ID>AGS_CONTRACT_COVERAGE<'',entitlement_xml)+len(''Contract Coverage</ENTITLEMENT_NAME>''))+''</QUOTE_ENTITLEMENT>'') as entitlement_xml FROM "+str(SAQSAE)+" (nolock)a JOIN "+str(CRMTMP)+" ON A.EQUIPMENT_ID = EQUIPMENT_IDD WHERE QUOTE_ID = ''"+str(Qt_id)+"''  AND SERVICE_ID = ''Z0092'' ) e OUTER APPLY e.ENTITLEMENT_XML.nodes(''QUOTE_ENTITLEMENT'') as X(Y))B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND A.ASSEMBLY_ID = B.ASSEMBLY_ID WHERE B.ENTITLEMENT_NAME=''Contract Coverage'' '")

				S1 = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET WETCLEAN=ENTITLEMENT_DISPLAY_VALUE FROM  "+str(SAQSCA)+" A(NOLOCK) JOIN (SELECT distinct quote_ID,equipment_id,assembly_id,service_id, replace(X.Y.value(''(ENTITLEMENT_DISPLAY_VALUE)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DISPLAY_VALUE,replace(X.Y.value(''(ENTITLEMENT_NAME)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_NAME FROM (SELECT quote_ID,equipment_id,assembly_id,service_id,CONVERT(XML,''<QUOTE_ENTITLEMENT>''+substring(entitlement_xml,charindex (''<ENTITLEMENT_ID>AGS_NET_WETL<'',entitlement_xml),charindex (''Wet Cleans Labor</ENTITLEMENT_NAME>'',entitlement_xml)-charindex (''<ENTITLEMENT_ID>AGS_NET_WETL<'',entitlement_xml)+len(''Wet Cleans Labor</ENTITLEMENT_NAME>'')) +''</QUOTE_ENTITLEMENT>'') as entitlement_xml FROM "+str(SAQSAE)+" (nolock)a JOIN "+str(CRMTMP)+" ON A.EQUIPMENT_ID = EQUIPMENT_IDD WHERE QUOTE_ID = ''"+str(Qt_id)+"''  AND SERVICE_ID NOT IN (''Z0092'',''Z0099'',''Z0091'',''Z0004'') ) e OUTER APPLY e.ENTITLEMENT_XML.nodes(''QUOTE_ENTITLEMENT'') as X(Y) )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND A.ASSEMBLY_ID = B.ASSEMBLY_ID WHERE B.ENTITLEMENT_NAME=''Wet Cleans Labor'' '")
				
				S1 = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET WETCLEAN=ENTITLEMENT_DISPLAY_VALUE FROM  "+str(SAQSCA)+" A(NOLOCK) JOIN (SELECT distinct quote_ID,equipment_id,assembly_id,service_id, replace(X.Y.value(''(ENTITLEMENT_DISPLAY_VALUE)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DISPLAY_VALUE,replace(X.Y.value(''(ENTITLEMENT_NAME)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_NAME FROM (SELECT quote_ID,equipment_id,assembly_id,service_id,CONVERT(XML,''<QUOTE_ENTITLEMENT>''+substring(entitlement_xml,charindex (''<ENTITLEMENT_ID>AGS_Z0091_NET_WECLLB<'',entitlement_xml),charindex (''Wet Cleans Labor</ENTITLEMENT_NAME>'',entitlement_xml)-charindex (''<ENTITLEMENT_ID>AGS_Z0091_NET_WECLLB<'',entitlement_xml)+len(''Wet Cleans Labor</ENTITLEMENT_NAME>'')) +''</QUOTE_ENTITLEMENT>'') as entitlement_xml FROM "+str(SAQSAE)+" (nolock)a JOIN "+str(CRMTMP)+" ON A.EQUIPMENT_ID = EQUIPMENT_IDD WHERE QUOTE_ID = ''"+str(Qt_id)+"''  AND SERVICE_ID = ''Z0091'' ) e OUTER APPLY e.ENTITLEMENT_XML.nodes(''QUOTE_ENTITLEMENT'') as X(Y) )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND A.ASSEMBLY_ID = B.ASSEMBLY_ID WHERE B.ENTITLEMENT_NAME=''Wet Cleans Labor'' '")
				
				S1 = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET WETCLEAN=ENTITLEMENT_DISPLAY_VALUE FROM  "+str(SAQSCA)+" A(NOLOCK) JOIN (SELECT distinct quote_ID,equipment_id,assembly_id,service_id, replace(X.Y.value(''(ENTITLEMENT_DISPLAY_VALUE)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DISPLAY_VALUE,replace(X.Y.value(''(ENTITLEMENT_NAME)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_NAME FROM (SELECT quote_ID,equipment_id,assembly_id,service_id,CONVERT(XML,''<QUOTE_ENTITLEMENT>''+substring(entitlement_xml,charindex (''<ENTITLEMENT_ID>AGS_Z0004_NET_WECLLB<'',entitlement_xml),charindex (''Wet Cleans Labor</ENTITLEMENT_NAME>'',entitlement_xml)-charindex (''<ENTITLEMENT_ID>AGS_Z0004_NET_WECLLB<'',entitlement_xml)+len(''Wet Cleans Labor</ENTITLEMENT_NAME>'')) +''</QUOTE_ENTITLEMENT>'') as entitlement_xml FROM "+str(SAQSAE)+" (nolock)a JOIN "+str(CRMTMP)+" ON A.EQUIPMENT_ID = EQUIPMENT_IDD WHERE QUOTE_ID = ''"+str(Qt_id)+"'' AND ENTITLEMENT_XML LIKE ''%Wet Cleans Labor%''  AND SERVICE_ID = ''Z0004'' ) e OUTER APPLY e.ENTITLEMENT_XML.nodes(''QUOTE_ENTITLEMENT'') as X(Y) )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND A.ASSEMBLY_ID = B.ASSEMBLY_ID WHERE B.ENTITLEMENT_NAME=''Wet Cleans Labor'' '")
				
				S1 = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET WETCLEAN=ENTITLEMENT_DISPLAY_VALUE FROM  "+str(SAQSCA)+" A(NOLOCK) JOIN (SELECT distinct quote_ID,equipment_id,assembly_id,service_id, replace(X.Y.value(''(ENTITLEMENT_DISPLAY_VALUE)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DISPLAY_VALUE,replace(X.Y.value(''(ENTITLEMENT_NAME)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_NAME FROM (SELECT quote_ID,equipment_id,assembly_id,service_id,CONVERT(XML,''<QUOTE_ENTITLEMENT>''+substring(entitlement_xml,charindex (''<ENTITLEMENT_ID>AGS_Z0099_NET_WECLLB<'',entitlement_xml),charindex (''Wet Cleans Labor</ENTITLEMENT_NAME>'',entitlement_xml)-charindex (''<ENTITLEMENT_ID>AGS_Z0099_NET_WECLLB<'',entitlement_xml)+len(''Wet Cleans Labor</ENTITLEMENT_NAME>'')) +''</QUOTE_ENTITLEMENT>'') as entitlement_xml FROM "+str(SAQSAE)+" (nolock)a JOIN "+str(CRMTMP)+" ON A.EQUIPMENT_ID = EQUIPMENT_IDD WHERE QUOTE_ID = ''"+str(Qt_id)+"''  AND SERVICE_ID = ''Z0099'' ) e OUTER APPLY e.ENTITLEMENT_XML.nodes(''QUOTE_ENTITLEMENT'') as X(Y) )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND A.ASSEMBLY_ID = B.ASSEMBLY_ID WHERE B.ENTITLEMENT_NAME=''Wet Cleans Labor'' '")
				
				S1 = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET WETCLEAN=ENTITLEMENT_DISPLAY_VALUE FROM  "+str(SAQSCA)+" A(NOLOCK) JOIN (SELECT distinct quote_ID,equipment_id,assembly_id,service_id, replace(X.Y.value(''(ENTITLEMENT_DISPLAY_VALUE)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DISPLAY_VALUE,replace(X.Y.value(''(ENTITLEMENT_NAME)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_NAME FROM (SELECT quote_ID,equipment_id,assembly_id,service_id,CONVERT(XML,''<QUOTE_ENTITLEMENT>''+substring(entitlement_xml,charindex (''<ENTITLEMENT_ID>AGS_WET_CLEANS_LABOR<'',entitlement_xml),charindex (''Wet Cleans Labor</ENTITLEMENT_NAME>'',entitlement_xml)-charindex (''<ENTITLEMENT_ID>AGS_WET_CLEANS_LABOR<'',entitlement_xml)+len(''Wet Cleans Labor</ENTITLEMENT_NAME>'')) +''</QUOTE_ENTITLEMENT>'') as entitlement_xml FROM "+str(SAQSAE)+" (nolock)a JOIN "+str(CRMTMP)+" ON A.EQUIPMENT_ID = EQUIPMENT_IDD WHERE QUOTE_ID = ''"+str(Qt_id)+"''  AND SERVICE_ID = ''Z0092'' ) e OUTER APPLY e.ENTITLEMENT_XML.nodes(''QUOTE_ENTITLEMENT'') as X(Y) )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND A.ASSEMBLY_ID = B.ASSEMBLY_ID WHERE B.ENTITLEMENT_NAME=''Wet Cleans Labor'' '")

				S2 = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET PMLABOR=ENTITLEMENT_DISPLAY_VALUE FROM  "+str(SAQSCA)+" A(NOLOCK) JOIN (SELECT distinct quote_ID,equipment_id,assembly_id,service_id, replace(X.Y.value(''(ENTITLEMENT_DISPLAY_VALUE)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DISPLAY_VALUE,replace(X.Y.value(''(ENTITLEMENT_NAME)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_NAME FROM (SELECT 	quote_ID,equipment_id,assembly_id,service_id,CONVERT(XML,''<QUOTE_ENTITLEMENT>''+ substring(entitlement_xml,charindex (''<ENTITLEMENT_ID>AGS_LAB_PRE_MAI<'',entitlement_xml),charindex (''Preventive Maintenance Labor</ENTITLEMENT_NAME>'',entitlement_xml)-charindex (''<ENTITLEMENT_ID>AGS_LAB_PRE_MAI<'',entitlement_xml)+len(''Preventive Maintenance Labor</ENTITLEMENT_NAME>'')) +''</QUOTE_ENTITLEMENT>'') as entitlement_xml FROM "+str(SAQSAE)+" (nolock)a JOIN "+str(CRMTMP)+" ON A.EQUIPMENT_ID = EQUIPMENT_IDD WHERE QUOTE_ID = ''"+str(Qt_id)+"'' AND SERVICE_ID NOT IN (''Z0092'',''Z0099'',''Z0091'',''z0004'') ) e OUTER APPLY e.ENTITLEMENT_XML.nodes(''QUOTE_ENTITLEMENT'') as X(Y) )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND A.ASSEMBLY_ID = B.ASSEMBLY_ID WHERE B.ENTITLEMENT_NAME=''Preventive Maintenance Labor''  '")
				
				S2 = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET PMLABOR=ENTITLEMENT_DISPLAY_VALUE FROM  "+str(SAQSCA)+" A(NOLOCK) JOIN (SELECT distinct quote_ID,equipment_id,assembly_id,service_id, replace(X.Y.value(''(ENTITLEMENT_DISPLAY_VALUE)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DISPLAY_VALUE,replace(X.Y.value(''(ENTITLEMENT_NAME)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_NAME FROM (SELECT 	quote_ID,equipment_id,assembly_id,service_id,CONVERT(XML,''<QUOTE_ENTITLEMENT>''+ substring(entitlement_xml,charindex (''<ENTITLEMENT_ID>AGS_Z0091_NET_PRMALB<'',entitlement_xml),charindex (''Preventative Maintenance Labor</ENTITLEMENT_NAME>'',entitlement_xml)-charindex (''<ENTITLEMENT_ID>AGS_Z0091_NET_PRMALB<'',entitlement_xml)+len(''Preventative Maintenance Labor</ENTITLEMENT_NAME>'')) +''</QUOTE_ENTITLEMENT>'') as entitlement_xml FROM "+str(SAQSAE)+" (nolock)a JOIN "+str(CRMTMP)+" ON A.EQUIPMENT_ID = EQUIPMENT_IDD WHERE QUOTE_ID = ''"+str(Qt_id)+"'' AND SERVICE_ID = ''Z0091'' ) e OUTER APPLY e.ENTITLEMENT_XML.nodes(''QUOTE_ENTITLEMENT'') as X(Y) )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND A.ASSEMBLY_ID = B.ASSEMBLY_ID WHERE B.ENTITLEMENT_NAME=''Preventative Maintenance Labor''  '")
				...
				S3 = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET CMLABOR=ENTITLEMENT_DISPLAY_VALUE FROM  "+str(SAQSCA)+" A(NOLOCK) JOIN (SELECT distinct quote_ID,equipment_id,assembly_id,service_id, replace(X.Y.value(''(ENTITLEMENT_DISPLAY_VALUE)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DISPLAY_VALUE,replace(X.Y.value(''(ENTITLEMENT_NAME)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_NAME FROM (SELECT quote_ID,equipment_id,assembly_id,service_id,CONVERT(XML,''<QUOTE_ENTITLEMENT>''+substring(entitlement_xml,charindex (''<ENTITLEMENT_ID>AGS_CORRECTIVE_MAINTEN_LABOR<'',entitlement_xml),charindex (''Corrective  Maintenance Labor</ENTITLEMENT_NAME>'',entitlement_xml)-charindex (''<ENTITLEMENT_ID>AGS_CORRECTIVE_MAINTEN_LABOR<'',entitlement_xml)+len(''Corrective  Maintenance Labor</ENTITLEMENT_NAME>''))+''</QUOTE_ENTITLEMENT>'') as entitlement_xml FROM "+str(SAQSAE)+" (nolock)a JOIN "+str(CRMTMP)+" ON A.EQUIPMENT_ID = EQUIPMENT_IDD WHERE QUOTE_ID = ''"+str(Qt_id)+"''  AND SERVICE_ID=''Z0092'') e OUTER APPLY e.ENTITLEMENT_XML.nodes(''QUOTE_ENTITLEMENT'') as X(Y) )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND A.ASSEMBLY_ID = B.ASSEMBLY_ID WHERE B.ENTITLEMENT_NAME=''Corrective  Maintenance Labor''  '")

				S3 = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET CMLABOR=ENTITLEMENT_DISPLAY_VALUE FROM  "+str(SAQSCA)+" A(NOLOCK) JOIN (SELECT distinct quote_ID,equipment_id,assembly_id,service_id, replace(X.Y.value(''(ENTITLEMENT_DISPLAY_VALUE)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DISPLAY_VALUE,replace(X.Y.value(''(ENTITLEMENT_NAME)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_NAME FROM (SELECT quote_ID,equipment_id,assembly_id,service_id,CONVERT(XML,''<QUOTE_ENTITLEMENT>''+substring(entitlement_xml,charindex (''<ENTITLEMENT_ID>AGS_Z0091_NET_CRMALB<'',entitlement_xml),charindex (''Corrective Maintenance Labor</ENTITLEMENT_NAME>'',entitlement_xml)-charindex (''<ENTITLEMENT_ID>AGS_Z0091_NET_CRMALB<'',entitlement_xml)+len(''Corrective Maintenance Labor</ENTITLEMENT_NAME>''))+''</QUOTE_ENTITLEMENT>'') as entitlement_xml FROM "+str(SAQSAE)+" (nolock)a JOIN "+str(CRMTMP)+" ON A.EQUIPMENT_ID = EQUIPMENT_IDD WHERE QUOTE_ID = ''"+str(Qt_id)+"''  AND SERVICE_ID=''Z0091'') e OUTER APPLY e.ENTITLEMENT_XML.nodes(''QUOTE_ENTITLEMENT'') as X(Y) )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND A.ASSEMBLY_ID = B.ASSEMBLY_ID WHERE B.ENTITLEMENT_NAME=''Corrective Maintenance Labor''  '")

				S4 = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET PERFGUARANTEE=ENTITLEMENT_DISPLAY_VALUE FROM  "+str(SAQSCA)+" A(NOLOCK) JOIN (SELECT distinct quote_ID,equipment_id,assembly_id,service_id, replace(X.Y.value(''(ENTITLEMENT_DISPLAY_VALUE)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DISPLAY_VALUE,replace(X.Y.value(''(ENTITLEMENT_NAME)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_NAME FROM (SELECT quote_ID,equipment_id,assembly_id,service_id,CONVERT(XML,''<QUOTE_ENTITLEMENT>''+substring(entitlement_xml,charindex (''<ENTITLEMENT_ID>AGS_KPI_PRI_PER<'',entitlement_xml),charindex (''Primary KPI. Perf Guarantee</ENTITLEMENT_NAME>'',entitlement_xml)-charindex (''<ENTITLEMENT_ID>AGS_KPI_PRI_PER<'',entitlement_xml)+len(''Primary KPI. Perf Guarantee</ENTITLEMENT_NAME>'')) +''</QUOTE_ENTITLEMENT>'') as entitlement_xml FROM "+str(SAQSAE)+" (nolock)a JOIN "+str(CRMTMP)+" ON A.EQUIPMENT_ID = EQUIPMENT_IDD WHERE QUOTE_ID = ''"+str(Qt_id)+"''  AND SERVICE_ID NOT IN (''Z0092'',''Z0099'',''Z0091'',''Z0004'') ) e OUTER APPLY e.ENTITLEMENT_XML.nodes(''QUOTE_ENTITLEMENT'') as X(Y) )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND A.ASSEMBLY_ID = B.ASSEMBLY_ID WHERE B.ENTITLEMENT_NAME=''Primary KPI. Perf Guarantee''  '")	
				...
				S4 = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET PERFGUARANTEE=ENTITLEMENT_DISPLAY_VALUE FROM  "+str(SAQSCA)+" A(NOLOCK) JOIN (SELECT distinct quote_ID,equipment_id,assembly_id,service_id, replace(X.Y.value(''(ENTITLEMENT_DISPLAY_VALUE)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DISPLAY_VALUE,replace(X.Y.value(''(ENTITLEMENT_NAME)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_NAME FROM (SELECT quote_ID,equipment_id,assembly_id,service_id,CONVERT(XML,''<QUOTE_ENTITLEMENT>''+substring(entitlement_xml,charindex (''<ENTITLEMENT_ID>AGS_PRIMARY_KPI_PERF_GUARANTEE<'',entitlement_xml),charindex (''Primary KPI. Perf Guarantee</ENTITLEMENT_NAME>'',entitlement_xml)-charindex (''<ENTITLEMENT_ID>AGS_PRIMARY_KPI_PERF_GUARANTEE<'',entitlement_xml)+len(''Primary KPI. Perf Guarantee</ENTITLEMENT_NAME>'')) +''</QUOTE_ENTITLEMENT>'') as entitlement_xml FROM "+str(SAQSAE)+" (nolock)a JOIN "+str(CRMTMP)+" ON A.EQUIPMENT_ID = EQUIPMENT_IDD WHERE QUOTE_ID = ''"+str(Qt_id)+"'' AND SERVICE_ID = ''Z0092'') e OUTER APPLY e.ENTITLEMENT_XML.nodes(''QUOTE_ENTITLEMENT'') as X(Y) )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND A.ASSEMBLY_ID = B.ASSEMBLY_ID WHERE B.ENTITLEMENT_NAME=''Primary KPI. Perf Guarantee''  '")
