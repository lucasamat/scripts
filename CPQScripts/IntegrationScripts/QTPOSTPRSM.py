...
try:
	...
	for crmifno in input_data:	
		...
		if "Successfully" in crm_response1:
			...
			StatusUpdate = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE SAQICO SET STATUS=''ASSEMBLY IS MISSING'' FROM SAQICO (NOLOCK) LEFT JOIN "+str(SAQSCA)+"  SAQSCA(NOLOCK) ON SAQSCA.QUOTE_ID = SAQICO.QUOTE_ID AND SAQSCA.EQUIPMENT_ID = SAQICO.EQUIPMENT_ID AND SAQSCA.SERVICE_ID = SAQICO.SERVICE_ID WHERE SAQICO.QUOTE_ID = ''"+str(Qt_id)+"'' AND SAQICO.QTEREV_ID= ''"+str(REVISION_ID) +"'' AND SAQSCA.EQUIPMENT_ID IS NULL AND SAQICO.SERVICE_ID IN (SELECT DISTINCT SERVICE_ID FROM PRSPRV(NOLOCK) WHERE ISNULL(SSCM_COST,''FALSE'')=''TRUE'' ) AND QTETYP=''TOOL BASED'' '")
			...
			StatusUpdate = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE SAQICO SET STATUS=''ASSEMBLY IS MISSING'' FROM SAQICO (NOLOCK) LEFT JOIN "+str(SAQGPA)+"  SAQSCA(NOLOCK) ON SAQSCA.QUOTE_ID = SAQICO.QUOTE_ID AND SAQSCA.SERVICE_ID = SAQICO.SERVICE_ID WHERE SAQICO.QUOTE_ID = ''"+str(Qt_id)+"'' AND SAQICO.QTEREV_ID= ''"+str(REVISION_ID) +"'' AND SAQSCA.ASSEMBLY_ID IS NULL AND SAQICO.SERVICE_ID IN (SELECT DISTINCT SERVICE_ID FROM PRSPRV(NOLOCK) WHERE ISNULL(SSCM_COST,''FALSE'')=''TRUE'' ) AND QTETYP IN (''EVENT BASED'' ,''FLEX EVENT BASED'') '")
