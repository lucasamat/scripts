...
try:
	...
	for crmifno in input_data:	
		...
		SAQSCO_SEL = SqlHelper.GetFirst("sp_executesql @T=N'select DISTINCT A.QUOTE_ID,EQUIPMENT_ID,SERVICE_ID,B.SALESORG_ID,C.REGION,A.QTEREV_ID,B.CONTRACT_VALID_FROM,B.CONTRACT_VALID_TO,C.ACCOUNT_NAME,A.FABLOCATION_NAME AS FAB_NAME,(SELECT DISTINCT VALDRV_WAFERNODE FROM MAEQUP(NOLOCK) WHERE MAEQUP.EQUIPMENT_ID = A.EQUIPMENT_ID) AS TECH_NODE_RANGE INTO "+str(SAQSCO)+" from SAQICO(NOLOCK) A JOIN SAQTRV B(NOLOCK) ON A.QUOTE_ID = B.QUOTE_ID AND A.QTEREV_ID = B.QTEREV_ID JOIN SAQTMT C(NOLOCK) ON B.QUOTE_ID = C.QUOTE_ID AND B.QTEREV_ID = C.QTEREV_ID WHERE A.QUOTE_ID = ''"+str(Qt_id)+"'' AND A.QTEREV_ID=''"+str(REVISION_ID) +"'' AND SERVICE_ID IN (SELECT DISTINCT SERVICE_ID FROM PRSPRV(NOLOCK) WHERE ISNULL(SSCM_COST,''FALSE'')=''TRUE'' ) AND ISNULL(A.STATUS,'''')=''''  ' ")
		
		#Sql = SqlHelper.GetFirst("sp_executesql @T=N'select distinct quote_ID,equipment_id,service_id,ENTITLEMENT_XML into "+str(SAQSAE)+" from  SAQIEN(nolock)a WHERE quote_id = ''"+str(Qt_id)+"'' AND QTEREV_ID=''"+str(REVISION_ID) +"'' '")
		...
		Check_flag = 1
		while Check_flag == 1:
			ent_query = SqlHelper.GetFirst("SELECT DISTINCT QUOTE_ID,LINE FROM (SELECT DISTINCT quote_id,LINE, ROW_NUMBER()OVER(ORDER BY LINE) AS SNO FROM SAQIEN (NOLOCK) where quote_id='"+str(Qt_id)+"' AND QTEREV_ID='"+str(REVISION_ID) +"' AND SERVICE_ID IN (SELECT DISTINCT SERVICE_ID FROM PRSPRV(NOLOCK) WHERE ISNULL(SSCM_COST,'FALSE')='TRUE') ) A WHERE SNO>= "+str(start)+" AND SNO<="+str(end)+" ")
			if str(ent_query) != "None":
				...
				if start == 2:
					SAQIEN_SEL = SqlHelper.GetFirst("sp_executesql @T=N'declare @H int; Declare @val Varchar(MAX);DECLARE @XML XML; SELECT @val = FINAL from(select  REPLACE(entitlement_xml,''<QUOTE_ITEM_ENTITLEMENT>'',sml) AS FINAL FROM (select ''<QUOTE_ITEM_ENTITLEMENT><QUOTE_ID>''+quote_id+''</QUOTE_ID><QTEREV_ID>''+CONVERT(VARCHAR,QTEREV_ID)+''</QTEREV_ID><SERVICE_ID>''+service_id+''</SERVICE_ID><EQUIPMENT_ID>''+equipment_id+''</EQUIPMENT_ID>'' AS sml,replace(replace(replace(replace(replace(replace(replace(ENTITLEMENT_XML,''&'','';#38''),'''','';#39''),'' < '','' &lt; '' ),'' > '','' &gt; '' ),''_>'',''_&gt;''),''_<'',''_&lt;''),''&'','';#38'')  as entitlement_xml from SAQIEN(nolock) where quote_id=''"+str(Qt_id)+"'' AND QTEREV_ID=''"+str(REVISION_ID) +"'' AND LINE = "+str(ent_query.LINE)+" )A )a SELECT @XML = CONVERT(XML,''<ROOT>''+@VAL+''</ROOT>'') exec sys.sp_xml_preparedocument @H output,@XML; select QUOTE_ID,QTEREV_ID,EQUIPMENT_ID,SERVICE_ID,ENTITLEMENT_ID,ENTITLEMENT_NAME,ENTITLEMENT_VALUE_CODE,ENTITLEMENT_DISPLAY_VALUE INTO "+str(SAQIEN)+"  from openxml(@H, ''ROOT/QUOTE_ITEM_ENTITLEMENT'', 0) with (QUOTE_ID VARCHAR(100) ''QUOTE_ID'',QTEREV_ID VARCHAR(100) ''QTEREV_ID'',EQUIPMENT_ID VARCHAR(100) ''EQUIPMENT_ID'',ENTITLEMENT_ID VARCHAR(100) ''ENTITLEMENT_ID'',SERVICE_ID VARCHAR(100) ''SERVICE_ID'',ENTITLEMENT_VALUE_CODE VARCHAR(100) ''ENTITLEMENT_VALUE_CODE'',ENTITLEMENT_NAME VARCHAR(100) ''ENTITLEMENT_NAME'',ENTITLEMENT_DISPLAY_VALUE VARCHAR(100) ''ENTITLEMENT_DISPLAY_VALUE'' ) ; exec sys.sp_xml_removedocument @H; '")
				
				else:
					SAQIEN_SEL = SqlHelper.GetFirst("sp_executesql @T=N'declare @H int; Declare @val Varchar(MAX);DECLARE @XML XML; SELECT @val = FINAL from(select  REPLACE(entitlement_xml,''<QUOTE_ITEM_ENTITLEMENT>'',sml) AS FINAL FROM (select ''<QUOTE_ITEM_ENTITLEMENT><QUOTE_ID>''+quote_id+''</QUOTE_ID><QTEREV_ID>''+CONVERT(VARCHAR,QTEREV_ID)+''</QTEREV_ID><SERVICE_ID>''+service_id+''</SERVICE_ID><EQUIPMENT_ID>''+equipment_id+''</EQUIPMENT_ID>'' AS sml,replace(replace(replace(replace(replace(replace(replace(ENTITLEMENT_XML,''&'','';#38''),'''','';#39''),'' < '','' &lt; '' ),'' > '','' &gt; '' ),''_>'',''_&gt;''),''_<'',''_&lt;''),''&'','';#38'')  as entitlement_xml from SAQIEN(nolock) where quote_id=''"+str(Qt_id)+"''   AND QTEREV_ID=''"+str(REVISION_ID) +"'' AND LINE = "+str(ent_query.LINE)+" )A )a SELECT @XML = CONVERT(XML,''<ROOT>''+@VAL+''</ROOT>'') exec sys.sp_xml_preparedocument @H output,@XML; insert "+str(SAQIEN)+" (QUOTE_ID,QTEREV_ID,EQUIPMENT_ID,SERVICE_ID,ENTITLEMENT_ID,ENTITLEMENT_NAME,ENTITLEMENT_VALUE_CODE,ENTITLEMENT_DISPLAY_VALUE) select QUOTE_ID,QTEREV_ID,EQUIPMENT_ID,SERVICE_ID,ENTITLEMENT_ID,ENTITLEMENT_NAME,ENTITLEMENT_VALUE_CODE,ENTITLEMENT_DISPLAY_VALUE  from openxml(@H, ''ROOT/QUOTE_ITEM_ENTITLEMENT'', 0) with (QUOTE_ID VARCHAR(100) ''QUOTE_ID'',QTEREV_ID VARCHAR(100) ''QTEREV_ID'',EQUIPMENT_ID VARCHAR(100) ''EQUIPMENT_ID'',ENTITLEMENT_ID VARCHAR(100) ''ENTITLEMENT_ID'',SERVICE_ID VARCHAR(100) ''SERVICE_ID'',ENTITLEMENT_VALUE_CODE VARCHAR(100) ''ENTITLEMENT_VALUE_CODE'',ENTITLEMENT_NAME VARCHAR(100) ''ENTITLEMENT_NAME'',ENTITLEMENT_DISPLAY_VALUE VARCHAR(100) ''ENTITLEMENT_DISPLAY_VALUE''); exec sys.sp_xml_removedocument @H; '")
			
			else:
				Check_flag=0 
		
		SAQSCA_SEL = SqlHelper.GetFirst("sp_executesql @T=N'select DISTINCT QUOTE_ID,EQUIPMENT_ID,SERVICE_ID,ASSEMBLY_ID,CONVERT(VARCHAR(100),NULL) AS COVERAGE,CONVERT(VARCHAR(100),NULL) AS WETCLEAN,CONVERT(VARCHAR(100),NULL) AS PERFGUARANTEE,CONVERT(VARCHAR(100),NULL) AS PMLABOR,CONVERT(VARCHAR(100),NULL) AS CMLABOR,CONVERT(VARCHAR(100),NULL) AS PM_EVENTS,CONVERT(VARCHAR(100),NULL) AS CONSUMABLE,CONVERT(VARCHAR(100),NULL) AS NON_CONSUMABLE,QTEREV_ID INTO "+str(SAQSCA)+" from SAQICA(NOLOCK) WHERE QUOTE_ID = ''"+str(Qt_id)+"'' AND QTEREV_ID=''"+str(REVISION_ID) +"'' AND SERVICE_ID IN (SELECT DISTINCT SERVICE_ID FROM PRSPRV(NOLOCK) WHERE ISNULL(SSCM_COST,''FALSE'')=''TRUE'' ) ' ")
		...
		#Contract Coverage
		S = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET COVERAGE=ENTITLEMENT_DISPLAY_VALUE FROM  "+str(SAQSCA)+" A(NOLOCK) JOIN "+str(SAQIEN)+" B(NOLOCK) ON A.QUOTE_ID = B.QUOTE_ID AND A.QTEREV_ID = B.QTEREV_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID  WHERE B.ENTITLEMENT_ID IN (''AGS_Z0091_CVR_CNTCOV'' ,''AGS_Z0092_CVR_CNTCOV'',''AGS_Z0004_CVR_CNTCOV'',''AGS_Z0099_CVR_CNTCOV'',''AGS_Z0035_CVR_CNTCOV'',''AGS_Z0009_CVR_CNTCOV'',''AGS_Z0100_CVR_CNTCOV'' ) '")
		
		#Wet Cleans Labor
		S = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET WETCLEAN=ENTITLEMENT_DISPLAY_VALUE FROM  "+str(SAQSCA)+" A(NOLOCK) JOIN "+str(SAQIEN)+" B(NOLOCK) ON A.QUOTE_ID = B.QUOTE_ID AND A.QTEREV_ID = B.QTEREV_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID  WHERE B.ENTITLEMENT_ID IN (''AGS_Z0091_NET_WECLLB'' ,''AGS_Z0004_NET_WECLLB'',''AGS_Z0099_NET_WECLLB'',''AGS_Z0092_NET_WECLLB'',''AGS_Z0035_NET_WECLLB'',''AGS_Z0009_NET_WECLLB'',''AGS_Z0100_NET_WECLLB'' ) '")
		
		#Preventive Maintenance Labor
		S = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET PMLABOR=ENTITLEMENT_DISPLAY_VALUE FROM  "+str(SAQSCA)+" A(NOLOCK) JOIN "+str(SAQIEN)+" B(NOLOCK) ON A.QUOTE_ID = B.QUOTE_ID AND A.QTEREV_ID = B.QTEREV_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID  WHERE B.ENTITLEMENT_ID IN (''AGS_Z0091_NET_PRMALB'', ''AGS_Z0004_NET_PRMALB'',''AGS_Z0099_NET_PRMALB'',''AGS_Z0092_NET_PRMALB'',''AGS_Z0035_NET_PRMALB'',''AGS_Z0009_NET_PRMALB'',''AGS_Z0100_NET_PRMALB''  ) '")
		
		#Corrective Maintenance Labor
		S = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET CMLABOR=ENTITLEMENT_DISPLAY_VALUE FROM  "+str(SAQSCA)+" A(NOLOCK) JOIN "+str(SAQIEN)+" B(NOLOCK) ON A.QUOTE_ID = B.QUOTE_ID AND A.QTEREV_ID = B.QTEREV_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID  WHERE B.ENTITLEMENT_ID IN (''AGS_Z0004_NET_CRMALB'', ''AGS_Z0099_NET_CRMALB'',''AGS_Z0092_NET_CRMALB'',''AGS_Z0091_NET_CRMALB'',''AGS_Z0035_NET_CRMALB'',''AGS_Z0009_NET_PRMALB'',''AGS_Z0100_NET_PRMALB''  ) '")
		
		#Primary KPI. Perf Guarantee
		S = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET PERFGUARANTEE=ENTITLEMENT_DISPLAY_VALUE FROM  "+str(SAQSCA)+" A(NOLOCK) JOIN "+str(SAQIEN)+" B(NOLOCK) ON A.QUOTE_ID = B.QUOTE_ID AND A.QTEREV_ID = B.QTEREV_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID  WHERE B.ENTITLEMENT_ID IN (''AGS_Z0091_KPI_PRPFGT'', ''AGS_Z0004_KPI_PRPFGT'',''AGS_Z0099_KPI_PRPFGT'',''AGS_Z0092_KPI_PRPFGT'',''AGS_Z0035_KPI_PRPFGT'',''AGS_Z0009_KPI_PRPFGT'',''AGS_Z0100_KPI_PRPFGT''  ) '")
		
		#PM Event
		S = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET PM_EVENTS=ENTITLEMENT_DISPLAY_VALUE FROM  "+str(SAQSCA)+" A(NOLOCK) JOIN "+str(SAQIEN)+" B(NOLOCK) ON A.QUOTE_ID = B.QUOTE_ID AND A.QTEREV_ID = B.QTEREV_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID  WHERE B.ENTITLEMENT_ID=''AGS_Z0009_STT_PMEVNT'' '")
		
		#Consumable
		S = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET CONSUMABLE=ENTITLEMENT_DISPLAY_VALUE FROM  "+str(SAQSCA)+" A(NOLOCK) JOIN "+str(SAQIEN)+" B(NOLOCK) ON A.QUOTE_ID = B.QUOTE_ID AND A.QTEREV_ID = B.QTEREV_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID  WHERE B.ENTITLEMENT_ID IN (''AGS_Z0091_TSC_CONSUM'', ''AGS_Z0004_TSC_CONSUM'',''AGS_Z0099_TSC_CONSUM'',''AGS_Z0092_TSC_CONSUM'',''AGS_Z0035_TSC_CONSUM'',''AGS_Z0009_TSC_CONSUM'',''AGS_Z0100_TSC_CONSUM''  ) '")
		
		#Non Consumable
		S = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET NON_CONSUMABLE=ENTITLEMENT_DISPLAY_VALUE FROM  "+str(SAQSCA)+" A(NOLOCK) JOIN "+str(SAQIEN)+" B(NOLOCK) ON A.QUOTE_ID = B.QUOTE_ID AND A.QTEREV_ID = B.QTEREV_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID  WHERE B.ENTITLEMENT_ID IN (''AGS_Z0091_TSC_NONCNS'', ''AGS_Z0004_TSC_NONCNS'',''AGS_Z0099_TSC_NONCNS'',''AGS_Z0092_TSC_NONCNS'',''AGS_Z0035_TSC_NONCNS'',''AGS_Z0009_TSC_NONCNS'',''AGS_Z0100_TSC_NONCNS''  ) '")
		...
		SAQSAP_SEL = SqlHelper.GetFirst("sp_executesql @T=N'DELETE A FROM "+str(SAQSCO)+" A JOIN "+str(SAQSCA)+" B ON A.QUOTE_ID = B.QUOTE_ID AND A.QTEREV_ID = B.QTEREV_ID AND A.SERVICE_ID = B.SERVICE_ID WHERE A.SERVICE_ID = ''Z0100'' AND ISNULL(B.CONSUMABLE,'''') <> ''INCLUDED''  ' ")
