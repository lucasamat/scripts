...
try:
	...
	for crmifno in input_data:	
		...
		QuoteInvolvedPartiesquery = SqlHelper.GetFirst("select replace(replace(STUFF((SELECT  ' '+final_xml FROM (SELECT DISTINCT  '<QUOTE_INVOLVED_PARTY>'+'<QUOTE_ID>"+str(CRMQT.c4c_quote_id)+"</QUOTE_ID><SOLD_TO>'+ ISNULL((SELECT REPLACE(ACCOUNT_ID,'STP-','') FROM SAQTMT(NOLOCK) WHERE QUOTE_ID = '"+str(QUOTE_ID)+"' ),'') + '</SOLD_TO><BILL_TO>'+ISNULL((SELECT TOP 1 PARTY_ID FROM SAQTIP(NOLOCK) WHERE QUOTE_ID = '"+str(QUOTE_ID)+"' AND PARTY_ROLE = 'BILL TO' AND QTEREV_ID = '"+str(REVISION_ID)+"' ),'')+'</BILL_TO><SHIP_TO>'+ISNULL((SELECT TOP 1 PARTY_ID FROM SAQTIP(NOLOCK) WHERE QUOTE_ID = '"+str(QUOTE_ID)+"' AND PARTY_ROLE = 'SHIP TO' AND QTEREV_ID = '"+str(REVISION_ID)+"' AND  ISNULL([PRIMARY],'FALSE')='TRUE' ),'')+'</SHIP_TO><PAYER>'+ISNULL((SELECT TOP 1 PARTY_ID FROM SAQTIP(NOLOCK) WHERE QUOTE_ID = '"+str(QUOTE_ID)+"' AND QTEREV_ID = '"+str(REVISION_ID)+"' AND PARTY_ROLE = 'PAYER' ),'')+'</PAYER><GPMRPM>'+ISNULL((SELECT TOP 1 CONVERT(VARCHAR(10),CRM_EMPLOYEE_ID) FROM SAQDLT(NOLOCK)A JOIN SAEMPL (NOLOCK) ON MEMBER_ID = EMPLOYEE_ID WHERE QUOTE_ID = '"+str(QUOTE_ID)+"' AND C4C_PARTNERFUNCTION_ID = 'BD MANAGER' AND QTEREV_ID = '"+str(REVISION_ID)+"' ),'')+'</GPMRPM><CONT_MNGR>'+ISNULL((SELECT TOP 1 CONVERT(VARCHAR(10),CRM_EMPLOYEE_ID) FROM SAQDLT(NOLOCK)A JOIN SAEMPL (NOLOCK) ON MEMBER_ID = EMPLOYEE_ID WHERE QUOTE_ID = '"+str(QUOTE_ID)+"' AND C4C_PARTNERFUNCTION_ID = 'CONTRACT MANAGER' AND QTEREV_ID = '"+str(REVISION_ID)+"' ),'')+'</CONT_MNGR><SALES_PERSON>'+ISNULL((SELECT TOP 1 CONVERT(VARCHAR(10),CRM_EMPLOYEE_ID) FROM SAQDLT(NOLOCK)A JOIN SAEMPL (NOLOCK) ON MEMBER_ID = EMPLOYEE_ID WHERE QUOTE_ID = '"+str(QUOTE_ID)+"' AND PARTNERFUNCTION_ID = 'ER' AND QTEREV_ID = '"+str(REVISION_ID)+"' ),'')+'</SALES_PERSON>' AS final_xml  FROM SAQTIP(NOLOCK) WHERE QUOTE_ID = '"+str(QUOTE_ID)+"' AND QTEREV_ID = '"+str(REVISION_ID)+"')A FOR XML PATH ('')  ), 1, 1, ''),'&lt;','<'),'&gt;','>')AS RESULT")
