		...
		for json_data in Jsonquery:
			...
			if len(rebuilt_data) != 0:      
				...
				if Check_flag == 1 and Saqico_Flag == 1:  
					...
					""
					+ str(Parameter1.QUERY_CRITERIA_1)
					+ "  A SET NPI_COEFFICIENT = CONVERT(VARCHAR,ENTITLEMENT_COEFFICIENT) FROM SAQICO_INBOUND (NOLOCK) A JOIN PRENVL B(NOLOCK) ON A.TOOL_NPI = B.ENTITLEMENT_DISPLAY_VALUE WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' AND ISNULL(TOOL_NPI,'''') <> '''' AND ENTITLEMENT_ID = ''AGS_Z0091_VAL_NPIREC'' ' ")
					...
					while Check_flag2 == 1:
						...
						table11 = SqlHelper.GetFirst(
									"SELECT DISTINCT equipment_id FROM (SELECT DISTINCT equipment_id, ROW_NUMBER()OVER(ORDER BY equipment_id) AS SNO FROM SAQICO_INBOUND (NOLOCK) WHERE ISNULL(PROCESS_STATUS,'')='INPROGRESS' AND TIMESTAMP = "+str(timestamp_sessionid)+" ) A WHERE SNO>= "+str(start2)+" AND SNO<="+str(end2)+""
								)
						...
						table3 = SqlHelper.GetFirst(
							"sp_executesql @T=N'INSERT "+str(CRMTMP)+" SELECT DISTINCT equipment_id FROM (SELECT DISTINCT equipment_id, ROW_NUMBER()OVER(ORDER BY equipment_id) AS SNO FROM SAQICO_INBOUND (NOLOCK) WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' ) A WHERE SNO>= "+str(start2)+" AND SNO<="+str(end2)+"  '")
