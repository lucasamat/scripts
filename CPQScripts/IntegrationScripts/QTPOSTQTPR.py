		...
		for json_data in Jsonquery:
			...
			if len(rebuilt_data) != 0:      
				...
				primaryQueryItems = SqlHelper.GetFirst(
									""
									+ str(Parameter1.QUERY_CRITERIA_1)
									+ "  A SET SERVICE_ID = B.SERVICE_ID,TOTWO = TOTAL_COST_WOSEEDSTOCK ,TOTWI = TOTAL_COST_WISEEDSTOCK  FROM SAQICO_INBOUND (NOLOCK) A JOIN SAQICO B(NOLOCK) ON A.QUOTE_ID = B.QUOTE_ID AND A.REVISION_ID = B.QTEREV_ID AND A.LINE = B.LINE WHERE ISNULL(PROCESS_STATUS,'''')='''' AND ISNULL(SESSION_ID,'''')=''"+str(sessiondetail.A)+ "'' '")					
				...
				if Check_flag == 1 and Saqico_Flag == 1:
					...
					primaryQueryItems = SqlHelper.GetFirst(
						""
						+ str(Parameter1.QUERY_CRITERIA_1)
						+ "  A SET CMLBRC = B.LABOUR_COST, CMPREC = REPLACE_COST, CMPRFC =  REFURB_COST, GQPLBC = GREATER_THAN_QTLY_COST, LQPLBC = LESS_THAN_QTLY_COST, PMPCLC = CLEAN_COST, PMPMEC	= METROLOGY_COST, PMPRCC = RECOATING_COST, SDSTKC =  SEEDSTOCK_COST, KPINDC = KPI_COST, FAILRC = FAILURE_COST, OUTSRC = OUTSOURCE_COST, LGSTCC = LOGISTICS_COST , TNHRPT = HEADREBUILD_QTY, TCWOSS = B.PER_EVENT_COST_WOSEEDSTOCK,TCWISS = B.PER_EVENT_COST_WISEEDSTOCK,PER_EVENT_COST_WOSEEDSTOCK=B.PER_EVENT_COST_WOSEEDSTOCK,PER_EVENT_COST_WISEEDSTOCK = B.PER_EVENT_COST_WISEEDSTOCK FROM SAQICO A(NOLOCK) JOIN (SELECT QUOTE_ID,SERVICE_ID,SUM(CONVERT(FLOAT,CM_PART_COST)) AS CM_PART_COST,SUM(CONVERT(FLOAT,PM_PART_COST)) AS PM_PART_COST,SUM(CONVERT(FLOAT,CLEAN_COST)) AS CLEAN_COST, SUM(CONVERT(FLOAT,GREATER_THAN_QTLY_COST)) AS GREATER_THAN_QTLY_COST,SUM(CONVERT(FLOAT,KPI_COST)) AS KPI_COST,SUM(CONVERT(FLOAT,LABOUR_COST)) AS LABOUR_COST,SUM(CONVERT(FLOAT,LESS_THAN_QTLY_COST)) AS LESS_THAN_QTLY_COST,SUM(CONVERT(FLOAT,METROLOGY_COST)) AS METROLOGY_COST,SUM(CONVERT(FLOAT,RECOATING_COST)) AS RECOATING_COST,SUM(CONVERT(FLOAT,REFURB_COST)) AS REFURB_COST, SUM(CONVERT(FLOAT,SEEDSTOCK_COST)) AS SEEDSTOCK_COST,SUM(CONVERT(FLOAT,TOTAL_COST_WOSEEDSTOCK)) AS TOTAL_COST_WOSEEDSTOCK,SUM(CONVERT(FLOAT,TOTAL_COST_WISEEDSTOCK)) AS TOTAL_COST_WISEEDSTOCK,REVISION_ID,MIN(HEADREBUILD_QTY) AS HEADREBUILD_QTY,SUM(REPLACE_COST) AS REPLACE_COST,SUM(FAILURE_COST) AS FAILURE_COST, SUM(OUTSOURCE_COST) AS OUTSOURCE_COST, SUM(LOGISTICS_COST) AS LOGISTICS_COST,LINE,SUM(CONVERT(FLOAT,PER_EVENT_COST_WISEEDSTOCK)) AS PER_EVENT_COST_WISEEDSTOCK,SUM(CONVERT(FLOAT,PER_EVENT_COST_WOSEEDSTOCK)) AS PER_EVENT_COST_WOSEEDSTOCK,EQUIPMENT_ID,CONTRACT_YEAR FROM SAQICO_INBOUND B(NOLOCK) WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' GROUP BY QUOTE_ID,SERVICE_ID,REVISION_ID,LINE,EQUIPMENT_ID,CONTRACT_YEAR )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID  AND A.QTEREV_ID = B.REVISION_ID AND A.LINE = B.LINE AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND A.CNTYER = B.CONTRACT_YEAR WHERE QTETYP <> ''TOOL BASED'' ' ")
						
					primaryQueryItems = SqlHelper.GetFirst(
						""
						+ str(Parameter1.QUERY_CRITERIA_1)
						+ "  A SET MODULE_ID=B.MODULE_ID,MODULE_NAME = B.MODULE_NAME,MODULE_VERSION_ID = B.MODULE_VERSION_ID FROM SAQICA A(NOLOCK) JOIN (SELECT  DISTINCT QUOTE_ID,SERVICE_ID,REVISION_ID,MODULE_ID,MODULE_NAME,LINE,EQUIPMENT_ID,ASSEMBLY_ID,MODULE_VERSION_ID  FROM SAQICO_INBOUND B(NOLOCK) WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"'  )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID  AND A.QTEREV_ID = B.REVISION_ID AND A.LINE = B.LINE AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND A.ASSEMBLY_ID = B.ASSEMBLY_ID  ' ")
					...
					primaryQueryItems = SqlHelper.GetFirst(
												""
												+ str(Parameter1.QUERY_CRITERIA_1)
												+ "  A SET NPI_COEFFICIENT = CONVERT(VARCHAR,ENTITLEMENT_COEFFICIENT),NPI_CODE = ENTITLEMENT_VALUE_CODE FROM SAQICO_INBOUND (NOLOCK) A JOIN PRENVL B(NOLOCK) ON A.TOOL_NPI = B.ENTITLEMENT_DISPLAY_VALUE WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' AND ISNULL(TOOL_NPI,'''') <> '''' AND ENTITLEMENT_ID = ''AGS_Z0091_VAL_NPIREC'' AND A.SERVICE_ID = ''Z0091'' AND A.GREENBOOK <> ''PDC''  ' ")
					
					#Z0092 NPI
					primaryQueryItems = SqlHelper.GetFirst(
					""
					+ str(Parameter1.QUERY_CRITERIA_1)
					+ "  A SET NPI_COEFFICIENT = CONVERT(VARCHAR,ENTITLEMENT_COEFFICIENT),NPI_CODE = ENTITLEMENT_VALUE_CODE FROM SAQICO_INBOUND (NOLOCK) A JOIN PRENVL B(NOLOCK) ON A.TOOL_NPI = B.ENTITLEMENT_DISPLAY_VALUE WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' AND ISNULL(TOOL_NPI,'''') <> '''' AND ENTITLEMENT_ID = ''AGS_Z0092_VAL_NPIREC'' AND A.SERVICE_ID = ''Z0092'' AND A.GREENBOOK <> ''PDC'' ' ")
					
					#Z0004 NPI
					primaryQueryItems = SqlHelper.GetFirst(
					""
					+ str(Parameter1.QUERY_CRITERIA_1)
					+ "  A SET NPI_COEFFICIENT = CONVERT(VARCHAR,ENTITLEMENT_COEFFICIENT),NPI_CODE = ENTITLEMENT_VALUE_CODE FROM SAQICO_INBOUND (NOLOCK) A JOIN PRENVL B(NOLOCK) ON A.TOOL_NPI = B.ENTITLEMENT_DISPLAY_VALUE WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' AND ISNULL(TOOL_NPI,'''') <> '''' AND ENTITLEMENT_ID = ''AGS_Z0004_VAL_NPIREC'' AND A.SERVICE_ID = ''Z0004'' AND A.GREENBOOK <> ''PDC'' ' ")
					
					#Z0099 NPI
					primaryQueryItems = SqlHelper.GetFirst(
					""
					+ str(Parameter1.QUERY_CRITERIA_1)
					+ "  A SET NPI_COEFFICIENT = CONVERT(VARCHAR,ENTITLEMENT_COEFFICIENT),NPI_CODE = ENTITLEMENT_VALUE_CODE FROM SAQICO_INBOUND (NOLOCK) A JOIN PRENVL B(NOLOCK) ON A.TOOL_NPI = B.ENTITLEMENT_DISPLAY_VALUE WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' AND ISNULL(TOOL_NPI,'''') <> '''' AND ENTITLEMENT_ID = ''AGS_Z0099_VAL_NPIREC'' AND A.SERVICE_ID = ''Z0099'' AND A.GREENBOOK <> ''PDC'' ' ")
					
					#Z0035 NPI
					primaryQueryItems = SqlHelper.GetFirst(
					""
					+ str(Parameter1.QUERY_CRITERIA_1)
					+ "  A SET NPI_COEFFICIENT = CONVERT(VARCHAR,ENTITLEMENT_COEFFICIENT),NPI_CODE = ENTITLEMENT_VALUE_CODE FROM SAQICO_INBOUND (NOLOCK) A JOIN PRENVL B(NOLOCK) ON A.TOOL_NPI = B.ENTITLEMENT_DISPLAY_VALUE WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' AND ISNULL(TOOL_NPI,'''') <> '''' AND ENTITLEMENT_ID = ''AGS_Z0035_VAL_NPIREC'' AND A.SERVICE_ID = ''Z0035'' AND A.GREENBOOK <> ''PDC'' ' ")
					
					#Z0009 NPI
					primaryQueryItems = SqlHelper.GetFirst(
					""
					+ str(Parameter1.QUERY_CRITERIA_1)
					+ "  A SET NPI_COEFFICIENT = CONVERT(VARCHAR,ENTITLEMENT_COEFFICIENT),NPI_CODE = ENTITLEMENT_VALUE_CODE FROM SAQICO_INBOUND (NOLOCK) A JOIN PRENVL B(NOLOCK) ON A.TOOL_NPI = B.ENTITLEMENT_DISPLAY_VALUE WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' AND ISNULL(TOOL_NPI,'''') <> '''' AND ENTITLEMENT_ID = ''AGS_Z0009_VAL_NPIREC'' AND A.SERVICE_ID = ''Z0009'' AND A.GREENBOOK <> ''PDC'' ' ")
					
					#Z0010 NPI
					primaryQueryItems = SqlHelper.GetFirst(
					""
					+ str(Parameter1.QUERY_CRITERIA_1)
					+ "  A SET NPI_COEFFICIENT = CONVERT(VARCHAR,ENTITLEMENT_COEFFICIENT),NPI_CODE = ENTITLEMENT_VALUE_CODE FROM SAQICO_INBOUND (NOLOCK) A JOIN PRENVL B(NOLOCK) ON A.TOOL_NPI = B.ENTITLEMENT_DISPLAY_VALUE WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' AND ISNULL(TOOL_NPI,'''') <> '''' AND ENTITLEMENT_ID = ''AGS_Z0010_VAL_NPIREC'' AND A.SERVICE_ID = ''Z0010'' AND A.GREENBOOK <> ''PDC'' ' ")
					...
					primaryQueryItems = SqlHelper.GetFirst(
						""
						+ str(Parameter1.QUERY_CRITERIA_1)
						+ "  A SET NPIVDV = TOOL_NPI FROM SAQICO A(NOLOCK) JOIN (SELECT  DISTINCT QUOTE_ID,SERVICE_ID,REVISION_ID,LINE,TOOL_NPI,EQUIPMENT_ID,CONTRACT_YEAR  FROM SAQICO_INBOUND B(NOLOCK) WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"'  )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID  AND A.QTEREV_ID = B.REVISION_ID AND A.LINE = B.LINE AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND A.CNTYER = B.CONTRACT_YEAR ' ")
					...
					primaryQueryItems = SqlHelper.GetFirst(
						""
						+ str(Parameter1.QUERY_CRITERIA_1)
						+ "  A SET PBPVDC = LOG(PDCBPR) * PBPVDV FROM SAQICO A(NOLOCK) JOIN (SELECT  DISTINCT QUOTE_ID,SERVICE_ID,REVISION_ID,LINE,NPI_COEFFICIENT,EQUIPMENT_ID,CONTRACT_YEAR  FROM SAQICO_INBOUND B(NOLOCK) WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"'  )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.QTEREV_ID = B.REVISION_ID AND A.LINE = B.LINE AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND A.CNTYER = B.CONTRACT_YEAR WHERE GRNBOK = ''PDC'' AND EQUIPMENTCATEGORY_ID IN (''S'',''C'') ' ")
					
					#RKFVDV/RKFVDC - Risk Factor Value Driver / Coefficent
					primaryQueryItems = SqlHelper.GetFirst(
												""
												+ str(Parameter1.QUERY_CRITERIA_1)
												+ "  A SET RKFVDV = CASE WHEN CNTYER =''YEAR 1'' THEN ''FIRST - 1 YEAR CONTRACT'' WHEN CNTYER =''YEAR 2'' THEN ''FIRST - 2 YEAR CONTRACT'' ELSE ''NO RISK'' END, RKFVDC = CASE WHEN CNTYER =''YEAR 1'' THEN CAVVDC WHEN CNTYER =''YEAR 2'' THEN CAVVDC/2 ELSE 0 END FROM SAQICO (NOLOCK) A JOIN (SELECT DISTINCT QUOTE_ID,REVISION_ID,SERVICE_ID,LINE,EQUIPMENT_ID,CONTRACT_YEAR FROM SAQICO_INBOUND(NOLOCK)B  WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' )B ON A.QUOTE_ID = B.QUOTE_ID AND A.QTEREV_ID = B.REVISION_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.LINE = B.LINE AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND A.CNTYER = B.CONTRACT_YEAR WHERE A.SERVICE_ID IN (''Z0009'',''Z0010'') AND A.QTETYP NOT IN (''TOOL BASED'')  ' ")
					...
					primaryQueryItems = SqlHelper.GetFirst(
						""
						+ str(Parameter1.QUERY_CRITERIA_1)
						+ "  A SET TAMTGC = ROUND(TAMTGC ,CONVERT(INT,"+str(roundcurr.DECIMAL_PLACES)+"),CONVERT(INT,"+str(roundcurr.ROUNDING_METHOD)+")), TAXVGC = ROUND(TAXVGC ,CONVERT(INT,"+str(roundcurr.DECIMAL_PLACES)+"),CONVERT(INT,"+str(roundcurr.ROUNDING_METHOD)+")), TNTVGC = ROUND(TNTVGC ,CONVERT(INT,"+str(roundcurr.DECIMAL_PLACES)+"),CONVERT(INT,"+str(roundcurr.ROUNDING_METHOD)+")),MTGPRC = ROUND(MTGPRC ,CONVERT(INT,"+str(roundcurr.DECIMAL_PLACES)+"),CONVERT(INT,"+str(roundcurr.ROUNDING_METHOD)+")),MSLPRC = ROUND(MSLPRC ,CONVERT(INT,"+str(roundcurr.DECIMAL_PLACES)+"),CONVERT(INT,"+str(roundcurr.ROUNDING_METHOD)+")),MBDPRC = ROUND(MBDPRC ,CONVERT(INT,"+str(roundcurr.DECIMAL_PLACES)+"),CONVERT(INT,"+str(roundcurr.ROUNDING_METHOD)+")),MCLPRC = ROUND(MCLPRC ,CONVERT(INT,"+str(roundcurr.DECIMAL_PLACES)+"),CONVERT(INT,"+str(roundcurr.ROUNDING_METHOD)+")),INMP01 = ROUND(INMP01 ,CONVERT(INT,"+str(roundcurr.DECIMAL_PLACES)+"),CONVERT(INT,"+str(roundcurr.ROUNDING_METHOD)+")),INMP02 = ROUND(INMP02 ,CONVERT(INT,"+str(roundcurr.DECIMAL_PLACES)+"),CONVERT(INT,"+str(roundcurr.ROUNDING_METHOD)+")),FNMDPR = ROUND(FNMDPR ,CONVERT(INT,"+str(roundcurr.DECIMAL_PLACES)+"),CONVERT(INT,"+str(roundcurr.ROUNDING_METHOD)+")),CELPRC = ROUND(CELPRC ,CONVERT(INT,"+str(roundcurr.DECIMAL_PLACES)+"),CONVERT(INT,"+str(roundcurr.ROUNDING_METHOD)+"))   FROM SAQICO A(NOLOCK) JOIN (SELECT  DISTINCT QUOTE_ID,SERVICE_ID,REVISION_ID,LINE,EQUIPMENT_ID,CONTRACT_YEAR  FROM SAQICO_INBOUND B(NOLOCK) WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"'  )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.QTEREV_ID = B.REVISION_ID AND A.LINE = B.LINE AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND A.CNTYER = B.CONTRACT_YEAR  ' ")
					
					#TNTVDC / TAXVDC / TAMTDC /TENVDC - Total Net Value / Tax / Total Amount/ Estimated (Document Currency) 
					primaryQueryItems = SqlHelper.GetFirst(
						""
						+ str(Parameter1.QUERY_CRITERIA_1)
						+ "  A SET TNTVDC = ROUND( (TNTVGC * ISNULL(DCCRFX,1)) ,CONVERT(INT,"+str(roundcurr.DECIMAL_PLACES)+"),CONVERT(INT,"+str(roundcurr.ROUNDING_METHOD)+")), TAXVDC = ROUND( (TAXVGC * ISNULL(DCCRFX,1)) ,CONVERT(INT,"+str(roundcurr.DECIMAL_PLACES)+"),CONVERT(INT,"+str(roundcurr.ROUNDING_METHOD)+")), TAMTDC = ROUND( (TAMTGC * ISNULL(DCCRFX,1)) ,CONVERT(INT,"+str(roundcurr.DECIMAL_PLACES)+"),CONVERT(INT,"+str(roundcurr.ROUNDING_METHOD)+")), TENVDC = ROUND( (TENVGC * ISNULL(DCCRFX,1)) ,CONVERT(INT,"+str(roundcurr.DECIMAL_PLACES)+"),CONVERT(INT,"+str(roundcurr.ROUNDING_METHOD)+")),TRGPRC = ROUND(TRGPRC ,CONVERT(INT,"+str(roundcurr.DECIMAL_PLACES)+"),CONVERT(INT,"+str(roundcurr.ROUNDING_METHOD)+")),USRPRC = ROUND(USRPRC ,CONVERT(INT,"+str(roundcurr.DECIMAL_PLACES)+"),CONVERT(INT,"+str(roundcurr.ROUNDING_METHOD)+")),SLSPRC = ROUND(SLSPRC ,CONVERT(INT,"+str(roundcurr.DECIMAL_PLACES)+"),CONVERT(INT,"+str(roundcurr.ROUNDING_METHOD)+")),BDVPRC = ROUND(BDVPRC ,CONVERT(INT,"+str(roundcurr.DECIMAL_PLACES)+"),CONVERT(INT,"+str(roundcurr.ROUNDING_METHOD)+")),CNTPRC = ROUND(CNTPRC ,CONVERT(INT,"+str(roundcurr.DECIMAL_PLACES)+"),CONVERT(INT,"+str(roundcurr.ROUNDING_METHOD)+")),CNTCST = ROUND(CNTCST ,CONVERT(INT,"+str(roundcurr.DECIMAL_PLACES)+"),CONVERT(INT,"+str(roundcurr.ROUNDING_METHOD)+"))  FROM SAQICO A(NOLOCK) JOIN (SELECT  DISTINCT QUOTE_ID,SERVICE_ID,REVISION_ID,LINE,EQUIPMENT_ID,CONTRACT_YEAR FROM SAQICO_INBOUND B(NOLOCK) WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"'  )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.QTEREV_ID = B.REVISION_ID AND A.LINE = B.LINE AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND A.CNTYER = B.CONTRACT_YEAR  ' ")
						
					primaryQueryItems = SqlHelper.GetFirst(
						""
						+ str(Parameter1.QUERY_CRITERIA_1)
						+ "  A SET  TCWOSS = CONVERT(FLOAT,TOTWO),TCWISS = CONVERT(FLOAT,TOTWI) FROM SAQICO A(NOLOCK) JOIN (SELECT QUOTE_ID,SERVICE_ID,SUM(CONVERT(FLOAT,TOTWO)) AS TOTWO,SUM(CONVERT(FLOAT,TOTWI)) AS TOTWI,REVISION_ID,LINE,EQUIPMENT_ID,CONTRACT_YEAR FROM SAQICO_INBOUND B(NOLOCK) WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' GROUP BY QUOTE_ID,SERVICE_ID,REVISION_ID,LINE,EQUIPMENT_ID ,CONTRACT_YEAR )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.QTEREV_ID = B.REVISION_ID AND A.LINE = B.LINE AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND A.CNTYER = B.CONTRACT_YEAR  ' ")
					...
					""
					+ str(Parameter1.QUERY_CRITERIA_1)
					+ "  SAQITM SET PEREVTCST_INGL_CURR= ROUND(ISNULL(CNTCST,0),CONVERT(INT,"+str(roundcurr.DECIMAL_PLACES)+"),CONVERT(INT,"+str(roundcurr.ROUNDING_METHOD)+")) ,PEREVTPRC_INDT_CURR = ROUND(ISNULL(CASE WHEN BILTYP=''VARIABLE'' THEN TENVDC ELSE TNTVDC END,0),CONVERT(INT,"+str(roundcurr.DECIMAL_PLACES)+"),CONVERT(INT,"+str(roundcurr.ROUNDING_METHOD)+"))/QUANTITY,PEREVTPRC_INGL_CURR = ROUND(ISNULL(CASE WHEN BILTYP=''VARIABLE'' THEN TENVGC ELSE TNTVGC END,0),CONVERT(INT,"+str(roundcurr.DECIMAL_PLACES)+"),CONVERT(INT,"+str(roundcurr.ROUNDING_METHOD)+"))/QUANTITY ,PEREVTCST_INDT_CURR= ROUND(ISNULL(CNTCST,0) * ISNULL((SELECT EXCHANGE_RATE FROM SAQTRV(NOLOCK) WHERE QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"''),1),CONVERT(INT,"+str(roundcurr.DECIMAL_PLACES)+"),CONVERT(INT,"+str(roundcurr.ROUNDING_METHOD)+")) FROM SAQRIT SAQITM(NOLOCK) JOIN(SELECT SAQICO.QTEREV_ID,LINE,SAQICO.QUOTE_ID,SAQICO.SERVICE_ID,SUM(TAXVGC) AS TAXVGC, SUM(TNTVGC) AS TNTVGC,SUM(USRPRC) AS USRPRC,SUM(TAXVDC) AS TAXVDC, SUM(TAMTDC) AS TAMTDC,SUM(TAMTGC) AS TAMTGC, SUM(TNTVDC) AS TNTVDC,SUM(TENVDC) AS TENVDC, SUM(TENVGC) AS TENVGC,SUM(CNTCST) AS CNTCST,BILTYP FROM SAQICO (NOLOCK) WHERE QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"''  AND LINE IN(SELECT DISTINCT LINE FROM SAQICO_INBOUND (NOLOCK) WHERE QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND REVISION_ID = ''"+str(Qt_Id.REVISION_ID)+"'') AND SERVICE_ID IN( ''Z0009'',''Z0010'') AND QTETYP NOT IN (''TOOL BASED'')  GROUP BY SAQICO.QUOTE_ID,SAQICO.SERVICE_ID,SAQICO.QTEREV_ID,SAQICO.LINE,BILTYP) SUB_SAQITM  ON SAQITM.QUOTE_ID = SUB_SAQITM.QUOTE_ID AND SAQITM.SERVICE_ID = SUB_SAQITM.SERVICE_ID  AND SUB_SAQITM.QTEREV_ID = SAQITM.QTEREV_ID  AND SAQITM.LINE = SUB_SAQITM.LINE ' ")

					#Item Roll Up Year 1
					primaryQueryItems = SqlHelper.GetFirst(
					""
					+ str(Parameter1.QUERY_CRITERIA_1)
					+ "  SAQITM SET YEAR_1_INGL_CURR = CASE WHEN BILTYP=''VARIABLE'' THEN TENVGC ELSE TNTVGC END, YEAR_1 =  CASE WHEN BILTYP=''VARIABLE'' THEN TENVDC ELSE TNTVDC END FROM SAQRIT SAQITM(NOLOCK) JOIN(SELECT SUM(SAQICO.TNTVGC) AS TNTVGC,SAQICO.QTEREV_ID,LINE,SAQICO.QUOTE_ID,SAQICO.SERVICE_ID,SUM(TNTVDC) AS TNTVDC,SUM(TENVGC) AS TENVGC,SUM(TENVDC) AS TENVDC,BILTYP FROM SAQICO (NOLOCK) WHERE QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"'' AND CNTYER=''YEAR 1''  AND LINE IN(SELECT DISTINCT LINE FROM SAQICO_INBOUND (NOLOCK) WHERE QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND REVISION_ID = ''"+str(Qt_Id.REVISION_ID)+"'') GROUP BY SAQICO.QUOTE_ID,SAQICO.SERVICE_ID,SAQICO.QTEREV_ID,SAQICO.LINE,BILTYP) SUB_SAQITM  ON SAQITM.QUOTE_ID = SUB_SAQITM.QUOTE_ID AND SAQITM.SERVICE_ID = SUB_SAQITM.SERVICE_ID AND SUB_SAQITM.QTEREV_ID = SAQITM.QTEREV_ID  AND SAQITM.LINE = SUB_SAQITM.LINE ' ")

					#Item Roll Up Year 2
					primaryQueryItems = SqlHelper.GetFirst(
					""
					+ str(Parameter1.QUERY_CRITERIA_1)
					+ "  SAQITM SET YEAR_2_INGL_CURR = CASE WHEN BILTYP=''VARIABLE'' THEN TENVGC ELSE TNTVGC END, YEAR_2 =  CASE WHEN BILTYP=''VARIABLE'' THEN TENVDC ELSE TNTVDC END FROM SAQRIT SAQITM(NOLOCK) JOIN(SELECT SUM(SAQICO.TNTVGC) AS TNTVGC,SAQICO.QTEREV_ID,LINE,SAQICO.QUOTE_ID,SAQICO.SERVICE_ID,SUM(TNTVDC) AS TNTVDC,SUM(TENVGC) AS TENVGC,SUM(TENVDC) AS TENVDC,BILTYP FROM SAQICO (NOLOCK) WHERE QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"'' AND CNTYER=''YEAR 2''   AND LINE IN(SELECT DISTINCT LINE FROM SAQICO_INBOUND (NOLOCK) WHERE QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND REVISION_ID = ''"+str(Qt_Id.REVISION_ID)+"'') GROUP BY SAQICO.QUOTE_ID,SAQICO.SERVICE_ID,SAQICO.QTEREV_ID,SAQICO.LINE,BILTYP) SUB_SAQITM  ON SAQITM.QUOTE_ID = SUB_SAQITM.QUOTE_ID AND SAQITM.SERVICE_ID = SUB_SAQITM.SERVICE_ID AND SUB_SAQITM.QTEREV_ID = SAQITM.QTEREV_ID  AND SAQITM.LINE = SUB_SAQITM.LINE ' ")

					#Item Roll Up Year 3
					primaryQueryItems = SqlHelper.GetFirst(
					""
					+ str(Parameter1.QUERY_CRITERIA_1)
					+ "  SAQITM SET YEAR_3_INGL_CURR = CASE WHEN BILTYP=''VARIABLE'' THEN TENVGC ELSE TNTVGC END, YEAR_3 =  CASE WHEN BILTYP=''VARIABLE'' THEN TENVDC ELSE TNTVDC END FROM SAQRIT SAQITM(NOLOCK) JOIN(SELECT SUM(SAQICO.TNTVGC) AS TNTVGC,SAQICO.QTEREV_ID,LINE,SAQICO.QUOTE_ID,SAQICO.SERVICE_ID,SUM(TNTVDC) AS TNTVDC,SUM(TENVGC) AS TENVGC,SUM(TENVDC) AS TENVDC,BILTYP FROM SAQICO (NOLOCK) WHERE QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"'' AND CNTYER=''YEAR 3''  AND LINE IN(SELECT DISTINCT LINE FROM SAQICO_INBOUND (NOLOCK) WHERE QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND REVISION_ID = ''"+str(Qt_Id.REVISION_ID)+"'') GROUP BY SAQICO.QUOTE_ID,SAQICO.SERVICE_ID,SAQICO.QTEREV_ID,SAQICO.LINE,BILTYP) SUB_SAQITM  ON SAQITM.QUOTE_ID = SUB_SAQITM.QUOTE_ID AND SAQITM.SERVICE_ID = SUB_SAQITM.SERVICE_ID AND SUB_SAQITM.QTEREV_ID = SAQITM.QTEREV_ID  AND SAQITM.LINE = SUB_SAQITM.LINE ' ")

					#Item Roll Up Year 4
					primaryQueryItems = SqlHelper.GetFirst(
					""
					+ str(Parameter1.QUERY_CRITERIA_1)
					+ "  SAQITM SET YEAR_4_INGL_CURR = CASE WHEN BILTYP=''VARIABLE'' THEN TENVGC ELSE TNTVGC END, YEAR_4 =  CASE WHEN BILTYP=''VARIABLE'' THEN TENVDC ELSE TNTVDC END FROM SAQRIT SAQITM(NOLOCK) JOIN(SELECT SUM(SAQICO.TNTVGC) AS TNTVGC,SAQICO.QTEREV_ID,LINE,SAQICO.QUOTE_ID,SAQICO.SERVICE_ID,SUM(TNTVDC) AS TNTVDC,SUM(TENVGC) AS TENVGC,SUM(TENVDC) AS TENVDC,BILTYP FROM SAQICO (NOLOCK) WHERE QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"'' AND CNTYER=''YEAR 4''   AND LINE IN(SELECT DISTINCT LINE FROM SAQICO_INBOUND (NOLOCK) WHERE QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND REVISION_ID = ''"+str(Qt_Id.REVISION_ID)+"'') GROUP BY SAQICO.QUOTE_ID,SAQICO.SERVICE_ID,SAQICO.QTEREV_ID,SAQICO.LINE,BILTYP) SUB_SAQITM  ON SAQITM.QUOTE_ID = SUB_SAQITM.QUOTE_ID AND SAQITM.SERVICE_ID = SUB_SAQITM.SERVICE_ID AND SUB_SAQITM.QTEREV_ID = SAQITM.QTEREV_ID  AND SAQITM.LINE = SUB_SAQITM.LINE ' ")

					#Item Roll Up Year 5
					primaryQueryItems = SqlHelper.GetFirst(
					""
					+ str(Parameter1.QUERY_CRITERIA_1)
					+ "  SAQITM SET YEAR_5_INGL_CURR = CASE WHEN BILTYP=''VARIABLE'' THEN TENVGC ELSE TNTVGC END, YEAR_5 =  CASE WHEN BILTYP=''VARIABLE'' THEN TENVDC ELSE TNTVDC END FROM SAQRIT SAQITM(NOLOCK) JOIN(SELECT SUM(SAQICO.TNTVGC) AS TNTVGC,SAQICO.QTEREV_ID,LINE,SAQICO.QUOTE_ID,SAQICO.SERVICE_ID,SUM(TNTVDC) AS TNTVDC,SUM(TENVGC) AS TENVGC,SUM(TENVDC) AS TENVDC,BILTYP FROM SAQICO (NOLOCK) WHERE QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"'' AND CNTYER=''YEAR 5''   AND LINE IN(SELECT DISTINCT LINE FROM SAQICO_INBOUND (NOLOCK) WHERE QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND REVISION_ID = ''"+str(Qt_Id.REVISION_ID)+"'') GROUP BY SAQICO.QUOTE_ID,SAQICO.SERVICE_ID,SAQICO.QTEREV_ID,SAQICO.LINE,BILTYP) SUB_SAQITM  ON SAQITM.QUOTE_ID = SUB_SAQITM.QUOTE_ID AND SAQITM.SERVICE_ID = SUB_SAQITM.SERVICE_ID AND SUB_SAQITM.QTEREV_ID = SAQITM.QTEREV_ID  AND SAQITM.LINE = SUB_SAQITM.LINE ' ")
					
					#Status - On Hold Costing
					primaryQueryItems = SqlHelper.GetFirst(
						""
					+ str(Parameter1.QUERY_CRITERIA_1)
					+ "  SAQICO SET STATUS=''ON HOLD COSTING'' FROM SAQICO (NOLOCK) JOIN (SELECT DISTINCT QUOTE_ID,SERVICE_ID,LINE,REVISION_ID FROM SAQICO_INBOUND(NOLOCK)  WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' AND ISNULL(COST_MODULE_AVAILABLE,'''')=''UNAVAILABLE'' AND ISNULL(COST_CALCULATION_STATUS,'''') <> ''Chamber map not required'' )SAQICO_INBOUND ON SAQICO.QUOTE_ID = SAQICO_INBOUND.QUOTE_ID AND SAQICO.SERVICE_ID = SAQICO_INBOUND.SERVICE_ID AND SAQICO.LINE = SAQICO_INBOUND.LINE AND SAQICO.QTEREV_ID = SAQICO_INBOUND.REVISION_ID  '")
					
					#Status - On Hold Costing
					primaryQueryItems = SqlHelper.GetFirst(
						""
					+ str(Parameter1.QUERY_CRITERIA_1)
					+ "  SAQICO SET STATUS=''CFG-ON HOLD - COSTING'' FROM SAQICO (NOLOCK) JOIN (SELECT DISTINCT QUOTE_ID,SERVICE_ID,LINE,REVISION_ID FROM SAQICO_INBOUND(NOLOCK)  WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' )SAQICO_INBOUND ON SAQICO.QUOTE_ID = SAQICO_INBOUND.QUOTE_ID AND SAQICO.SERVICE_ID = SAQICO_INBOUND.SERVICE_ID AND SAQICO.LINE = SAQICO_INBOUND.LINE AND SAQICO.QTEREV_ID = SAQICO_INBOUND.REVISION_ID WHERE ISNULL(TCWISS,0)<=0  '")
					...
					primaryQueryItems = SqlHelper.GetFirst(
						""
					+ str(Parameter1.QUERY_CRITERIA_1)
					+ "  SAQICO SET STATUS=''PRR-ON HOLD PRICING'' FROM SAQICO (NOLOCK) WHERE QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"'' AND STATUS NOT IN (''ERROR'',''ASSEMBLY IS MISSING'',''CFG-ON HOLD - COSTING'',''PRR-ON HOLD PRICING'') AND ISNULL(CNTPRC,0)>0 AND ISNULL(ATGKEY,'''') = ''INCLUDED'' AND ISNULL(ATGKEC,0)=0 AND ISNULL(ATGKEP,0)=0 AND LINE IN (SELECT DISTINCT LINE FROM SAQICO_INBOUND WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' AND QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' ) '")
					
					#Status - On Hold Pricing (New Parts)
					primaryQueryItems = SqlHelper.GetFirst(
						""
					+ str(Parameter1.QUERY_CRITERIA_1)
					+ "  SAQICO SET STATUS=''PRR-ON HOLD PRICING'' FROM SAQICO (NOLOCK) WHERE QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"'' AND STATUS NOT IN (''ERROR'',''ASSEMBLY IS MISSING'',''CFG-ON HOLD - COSTING'',''PRR-ON HOLD PRICING'') AND ISNULL(CNTPRC,0)>0 AND ISNULL(NWPTON,'''') = ''YES'' AND ISNULL(NWPTOC,0)=0 AND ISNULL(NWPTOP,0)=0 AND LINE IN (SELECT DISTINCT LINE FROM SAQICO_INBOUND WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' AND QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' ) '")
									
					#Status - Acquired
					primaryQueryItems = SqlHelper.GetFirst(
						""
					+ str(Parameter1.QUERY_CRITERIA_1)
					+ "  SAQICO SET STATUS=''ACQUIRED'' FROM SAQICO (NOLOCK) JOIN SAQICO_INBOUND C(NOLOCK)ON SAQICO.QUOTE_ID = C.QUOTE_ID AND SAQICO.QTEREV_ID = C.REVISION_ID  AND SAQICO.SERVICE_ID = C.SERVICE_ID AND SAQICO.LINE = C.LINE WHERE TIMESTAMP = '"+str(timestamp_sessionid)+"' AND STATUS NOT IN (''ERROR'',''ASSEMBLY IS MISSING'',''CFG-ON HOLD - COSTING'',''CFG-ON HOLD - COSTING'') '")
					
					#Status Item - On Hold Costing
					primaryQueryItems = SqlHelper.GetFirst(
						""
					+ str(Parameter1.QUERY_CRITERIA_1)
					+ "  SAQITM SET STATUS=''CFG-ON HOLD - COSTING'' FROM SAQRIT SAQITM (NOLOCK) WHERE QUOTE_REVISION_CONTRACT_ITEM_ID IN (SELECT DISTINCT QTEITM_RECORD_ID FROM SAQICO_INBOUND(NOLOCK)A JOIN SAQICO B(NOLOCK) ON A.QUOTE_ID= B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.LINE = B.LINE AND A.REVISION_ID = B.QTEREV_ID WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' AND B.STATUS=''CFG-ON HOLD - COSTING'') AND SERVICE_ID IN (SELECT DISTINCT SERVICE_ID FROM PRSPRV(NOLOCK) WHERE ISNULL(SSCM_COST,''FALSE'')=''TRUE'' )  '")
					
					#Status Item - On Hold Pricing
					primaryQueryItems = SqlHelper.GetFirst(
						""
					+ str(Parameter1.QUERY_CRITERIA_1)
					+ "  SAQITM SET STATUS=''PRR-ON HOLD PRICING'' FROM SAQRIT SAQITM (NOLOCK) WHERE QUOTE_REVISION_CONTRACT_ITEM_ID IN (SELECT DISTINCT QTEITM_RECORD_ID FROM SAQICO_INBOUND(NOLOCK)A JOIN SAQICO B(NOLOCK) ON A.QUOTE_ID= B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.LINE = B.LINE AND A.REVISION_ID = B.QTEREV_ID WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' AND B.STATUS=''PRR-ON HOLD PRICING'') AND SERVICE_ID IN (SELECT DISTINCT SERVICE_ID FROM PRSPRV(NOLOCK) WHERE ISNULL(SSCM_COST,''FALSE'')=''TRUE'' ) AND STATUS NOT IN (''ON HOLD COSTING'') '")
					
					#Status Item - Acquired
					primaryQueryItems = SqlHelper.GetFirst(
						""
					+ str(Parameter1.QUERY_CRITERIA_1)
					+ "  SAQITM SET STATUS=''ACQUIRED'' FROM SAQRIT SAQITM (NOLOCK) WHERE QUOTE_REVISION_CONTRACT_ITEM_ID IN (SELECT DISTINCT QTEITM_RECORD_ID FROM SAQICO_INBOUND(NOLOCK)A JOIN SAQICO B(NOLOCK) ON A.QUOTE_ID= B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.LINE = B.LINE AND A.REVISION_ID = B.QTEREV_ID WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' AND B.STATUS=''ACQUIRED'') AND SERVICE_ID IN (SELECT DISTINCT SERVICE_ID FROM PRSPRV(NOLOCK) WHERE ISNULL(SSCM_COST,''FALSE'')=''TRUE'' ) AND ISNULL(STATUS,'''') NOT IN (''CFG-ON HOLD - COSTING'',''PRR-ON HOLD PRICING'') '")
										
					#Revision Status - On Hold Costing
					primaryQueryItems = SqlHelper.GetFirst(
						""
					+ str(Parameter1.QUERY_CRITERIA_1)
					+ "  SAQTRV SET REVISION_STATUS=''CFG-ON HOLD - COSTING'',WORKFLOW_STATUS = ''CONFIGURE'' FROM SAQTRV A(NOLOCK) JOIN (SELECT DISTINCT QUOTE_ID,QTEREV_ID FROM SAQRIT B(NOLOCK)  WHERE  ISNULL(STATUS,'''')  IN (''CFG-ON HOLD - COSTING'',''ERROR'',''ASSEMBLY IS MISSING'',''ACQURING'','''') AND QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"'')B ON A.QUOTE_ID = B.QUOTE_ID AND A.QTEREV_ID = B.QTEREV_ID '")
					
					#Revision Status - On Hold Pricing
					primaryQueryItems = SqlHelper.GetFirst(
						""
					+ str(Parameter1.QUERY_CRITERIA_1)
					+ "  SAQTRV SET REVISION_STATUS=''PRR-ON HOLD PRICING'',WORKFLOW_STATUS = ''PRICING REVIEW'' FROM SAQTRV A(NOLOCK) JOIN (SELECT DISTINCT QUOTE_ID,QTEREV_ID FROM SAQRIT B(NOLOCK)  WHERE  ISNULL(STATUS,'''')  IN (''PRR-ON HOLD PRICING'') AND QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"'')B ON A.QUOTE_ID = B.QUOTE_ID AND A.QTEREV_ID = B.QTEREV_ID AND REVISION_STATUS NOT IN (''CFG-ON HOLD - COSTING'') '")
