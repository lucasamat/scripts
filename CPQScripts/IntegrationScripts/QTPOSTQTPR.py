			...
			if len(rebuilt_data) != 0:      
				...
				if Check_flag == 1 and Saqico_Flag == 1:  
					...
					primaryQueryItems = SqlHelper.GetFirst(
						""
						+ str(Parameter1.QUERY_CRITERIA_1)
						+ "  SAQSCV SET VALUEDRIVER_COEFFICIENT = C.VALUEDRIVER_COEFFICIENT FROM SAQICO_INBOUND (NOLOCK) JOIN SAQSCV (NOLOCK) ON SAQICO_INBOUND.QUOTE_ID = SAQSCV.QUOTE_ID AND SAQICO_INBOUND.REVISION_ID = SAQSCV.QTEREV_ID AND SAQICO_INBOUND.EQUIPMENT_ID = SAQSCV.EQUIPMENT_ID AND SAQICO_INBOUND.SERVICE_ID = SAQSCV.SERVICE_ID JOIN PRSDVL C(NOLOCK) ON SAQSCV.SERVICE_ID = C.SERVICE_ID AND SAQSCV.TOOL_VALUEDRIVER_ID = C.VALUEDRIVER_ID AND SAQSCV.TOOL_VALUEDRIVER_VALUE_CODE = C.VALUEDRIVER_VALUE_CODE  WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' AND C.VALUEDRIVER_ID = ''NPI''  ' ")
					...
					primaryQueryItems = SqlHelper.GetFirst(
						""
						+ str(Parameter1.QUERY_CRITERIA_1)
						+ "  A SET MODEL_PRICE = MODEL_PRICE + ISNULL(ENTITLEMENT_PRICE_IMPACT,0) FROM SAQICO A(NOLOCK) JOIN (SELECT DISTINCT QUOTE_ID,SERVICE_ID,EQUIPMENT_ID,REVISION_ID FROM SAQICO_INBOUND(NOLOCK)  WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND QTEREV_ID = REVISION_ID ' ")
					...
					primaryQueryItems = SqlHelper.GetFirst(
						""
					+ str(Parameter1.QUERY_CRITERIA_1)
					+ "  SAQICO SET STATUS=''APPROVAL PENDING'' FROM SAQICO (NOLOCK) JOIN SAQICA(NOLOCK) ON SAQICO.QUOTE_ID = SAQICA.QUOTE_ID AND SAQICO.SERVICE_ID = SAQICA.SERVICE_ID AND SAQICO.QTEREV_ID = SAQICA.QTEREV_ID AND SAQICO.EQUIPMENT_LINE_ID = SAQICA.EQUIPMENT_LINE_ID AND SAQICO.EQUIPMENT_ID = SAQICA.EQUIPMENT_ID JOIN SAQICO_INBOUND C(NOLOCK)ON SAQICO.QUOTE_ID = C.QUOTE_ID AND SAQICO.QTEREV_ID = C.REVISION_ID  AND SAQICO.SERVICE_ID = C.SERVICE_ID AND SAQICO.EQUIPMENT_ID = C.EQUIPMENT_ID  LEFT JOIN (SELECT DISTINCT QUOTE_ID,SERVICE_ID,EQUIPMENT_ID,REVISION_ID FROM SAQICO_INBOUND(NOLOCK)  WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' AND ISNULL(COST_MODULE_AVAILABLE,'''')=''UNAVAILABLE'')SAQICO_INBOUND ON SAQICO.QUOTE_ID = SAQICO_INBOUND.QUOTE_ID AND SAQICO.QTEREV_ID = SAQICO_INBOUND.REVISION_ID AND SAQICO.SERVICE_ID = SAQICO_INBOUND.SERVICE_ID AND SAQICO.EQUIPMENT_ID = SAQICO_INBOUND.EQUIPMENT_ID WHERE SAQICO_INBOUND.EQUIPMENT_ID IS NULL AND ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' '")
					
					primaryQueryItems = SqlHelper.GetFirst(
						""
					+ str(Parameter1.QUERY_CRITERIA_1)
					+ "  SAQITM SET PRICING_STATUS=''ON HOLD - PRICING'' FROM SAQITM (NOLOCK) WHERE QUOTE_ITEM_RECORD_ID IN (SELECT DISTINCT QTEITM_RECORD_ID FROM SAQICO_INBOUND(NOLOCK)A JOIN SAQICO B(NOLOCK) ON A.QUOTE_ID= B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID  WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' AND ISNULL(COST_MODULE_AVAILABLE,'''')=''UNAVAILABLE'' AND PRICING_STATUS=''APPROVAL PENDING'') AND PRICING_STATUS NOT IN (''ON HOLD - COSTING'',''ON HOLD - PRICING'',''ACQUIRING'') '")
