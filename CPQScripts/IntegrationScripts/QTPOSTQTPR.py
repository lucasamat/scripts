		...
		for json_data in Jsonquery:
			...
			if len(rebuilt_data) != 0:      
				...
				if Check_flag == 1 and Saqico_Flag == 1:  
					...
					primaryQueryItems = SqlHelper.GetFirst("sp_executesql @t=N'UPDATE A SET ENTITLEMENT_XML = REPLACE(replace(replace(entitlement_xml,''    '',''''),''\n'','''') ,  substring(replace(replace(entitlement_xml,''    '',''''),''\n'',''''),charindex(''<ENTITLEMENT_NAME>AGS_Z0091_VAL_NPIREC</ENTITLEMENT_NAME><ENTITLEMENT_VALUE_CODE>'' ,replace(replace(entitlement_xml,''    '',''''),''\n'','''')),charindex(''</ENTITLEMENT_VALUE_CODE>''  ,substring(replace(replace(entitlement_xml,''    '',''''),''\n'',''''),charindex(''<ENTITLEMENT_NAME>AGS_Z0091_VAL_NPIREC</ENTITLEMENT_NAME><ENTITLEMENT_VALUE_CODE>'' ,replace(replace(entitlement_xml,''    '',''''),''\n'','''')) , 1000 ))-1  ), ''<ENTITLEMENT_NAME>AGS_Z0091_VAL_NPIREC</ENTITLEMENT_NAME><ENTITLEMENT_VALUE_CODE>''+TOOL_NPI+'' ) FROM SAQSCE(NOLOCK) A JOIN (SELECT DISTINCT QUOTE_ID ,SERVICE_ID,EQUIPMENT_ID,REVISION_ID,TOOL_NPI SAQICO_INBOUND B(NOLOCK) WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' ) B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID  = B.EQUIPMENT_ID AND A.QTEREV_ID = B.REVISION_ID  WHERE A.QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND A.QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"'' AND ISNULL(TOOL_NPI,'''')<>'''' '")

					primaryQueryItems = SqlHelper.GetFirst(
					""
					+ str(Parameter1.QUERY_CRITERIA_1)
					+ "  SAQICO_INBOUND SET NPI_COEFFICIENT = CONVERT(VARCHAR,ENTITLEMENT_COEFFICIENT) FROM SAQICO_INBOUND (NOLOCK) A JOIN PRENVL B(NOLOCK) ON A.TOOL_NPI = B.ENTITLEMENT_DISPLAY_VALUE WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' AND ISNULL(TOOL_NPI,'''') <> '''' AND ENTITLEMENT_NAME = ''AGS_Z0091_VAL_NPIREC'' ' ")

					primaryQueryItems = SqlHelper.GetFirst("sp_executesql @t=N'UPDATE A SET ENTITLEMENT_XML = REPLACE(replace(replace(entitlement_xml,''    '',''''),''\n'','''') ,  substring(replace(replace(entitlement_xml,''    '',''''),''\n'',''''),charindex(''<ENTITLEMENT_NAME>AGS_Z0091_VAL_NPICOF</ENTITLEMENT_NAME><ENTITLEMENT_VALUE_CODE>'' ,replace(replace(entitlement_xml,''    '',''''),''\n'','''')),charindex(''</ENTITLEMENT_VALUE_CODE>''  ,substring(replace(replace(entitlement_xml,''    '',''''),''\n'',''''),charindex(''<ENTITLEMENT_NAME>AGS_Z0091_VAL_NPICOF</ENTITLEMENT_NAME><ENTITLEMENT_VALUE_CODE>'' ,replace(replace(entitlement_xml,''    '',''''),''\n'','''')) , 1000 ))-1  ), ''<ENTITLEMENT_NAME>AGS_Z0091_VAL_NPICOF</ENTITLEMENT_NAME><ENTITLEMENT_VALUE_CODE>''+TOOL_NPI+'' ) FROM SAQSCE(NOLOCK) A JOIN (SELECT DISTINCT QUOTE_ID ,SERVICE_ID,EQUIPMENT_ID,REVISION_ID,TOOL_NPI SAQICO_INBOUND B(NOLOCK) WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID  = B.EQUIPMENT_ID AND A.QTEREV_ID = B.REVISION_ID  WHERE A.QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND A.QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"''  AND ISNULL(TOOL_NPI,'''')<>'''' '")
					...
					primaryQueryItems = SqlHelper.GetFirst(
						""
						+ str(Parameter1.QUERY_CRITERIA_1)
						+ " A GREENBOOK = B.GREENBOOK FROM SAQICO_INBOUND (NOLOCK) A JOIN SAQICO (NOLOCK) B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND A.REVISION_ID = B.QTEREV_ID WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' '")
						
					primaryQueryItems = SqlHelper.GetFirst(
											""
											+ str(Parameter1.QUERY_CRITERIA_1)
											+ " A SET TOOL_SERVICE_COMPLEXITY = SERVICE_COMPLEXITY FROM SAQICO_INBOUND A(NOLOCK) JOIN (SELECT A.QUOTE_ID,A.SERVICE_ID,A.EQUIPMENT_ID,A.REVISION_ID AS QTEREV_ID,CASE WHEN A.SERVICE_COMPLEXITY = ''DIFFICULT'' THEN ''Difficult'' ELSE A.SERVICE_COMPLEXITY END AS SERVICE_COMPLEXITY FROM SAQICO_INBOUND (NOLOCK) A JOIN SAQICA (NOLOCK) B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND A.REVISION_ID = B.QTEREV_ID AND A.ASSEMBLY_ID = B.ASSEMBLY_ID WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' AND A.SERVICE_COMPLEXITY = ''Difficult'' AND B.EQUIPMENTTYPE_ID=''MAINFRAME'' AND A.GREENBOOK IN (''CMP'',''PDC'',''MPS'',''IMPLANT'') )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND A.REVISION_ID = B.QTEREV_ID   '")

					primaryQueryItems = SqlHelper.GetFirst(
																""
																+ str(Parameter1.QUERY_CRITERIA_1)
																+ " A SET TOOL_SERVICE_COMPLEXITY = ''Difficult'' FROM SAQICO_INBOUND A(NOLOCK) JOIN (SELECT A.QUOTE_ID,A.SERVICE_ID,A.EQUIPMENT_ID,A.REVISION_ID AS QTEREV_ID,COUNT(*) AS SERVICE_COMPLEXITY FROM SAQICO_INBOUND (NOLOCK) A WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' AND A.SERVICE_COMPLEXITY = ''Difficult'' AND A.GREENBOOK NOT IN (''CMP'',''PDC'',''MPS'',''IMPLANT'') GROUP BY A.QUOTE_ID,A.SERVICE_ID,A.EQUIPMENT_ID,A.REVISION_ID )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND A.REVISION_ID = B.QTEREV_ID WHERE SERVICE_COMPLEXITY >= 2  '")

					primaryQueryItems = SqlHelper.GetFirst(
																""
																+ str(Parameter1.QUERY_CRITERIA_1)
																+ " A SET TOOL_SERVICE_COMPLEXITY = CASE WHEN WA>=75 THEN ''HIGH'' WHEN WA>=25 AND WA<75 THEN ''MEDIUM'' WHEN WA<25 THEN ''EASY'' ELSE NULL END  FROM SAQICO_INBOUND A(NOLOCK) JOIN (SELECT QUOTE_ID,SERVICE_ID,EQUIPMENT_ID,QTEREV_ID,SCORE/SERVICE_COMPLEXITY AS WA FROM (SELECT A.QUOTE_ID,A.SERVICE_ID,A.EQUIPMENT_ID,A.REVISION_ID AS QTEREV_ID,COUNT(*) AS SERVICE_COMPLEXITY,SUM(CASE WHEN SERVICE_COMPLEXITY=''DIFFICULT'' THEN 100 WHEN SERVICE_COMPLEXITY=''MEDIUM'' THEN 50 ELSE 0 END) AS SCORE FROM SAQICO_INBOUND (NOLOCK) A JOIN SAQICO (NOLOCK) B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND A.REVISION_ID = B.QTEREV_ID  WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' AND B.GREENBOOK NOT IN (''CMP'',''PDC'',''MPS'',''IMPLANT'') GROUP BY A.QUOTE_ID,A.SERVICE_ID,A.EQUIPMENT_ID,A.REVISION_ID)B WHERE EQUIPMENT_ID IN (SELECT DISTINCT EQUIPMENT_ID FROM (SELECT A.QUOTE_ID,A.SERVICE_ID,A.EQUIPMENT_ID,A.REVISION_ID AS QTEREV_ID,COUNT(*) AS SERVICE_COMPLEXITY FROM SAQICO_INBOUND (NOLOCK) A JOIN SAQICO (NOLOCK)B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND A.REVISION_ID = B.QTEREV_ID  WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' AND A.SERVICE_COMPLEXITY = ''Difficult'' AND B.GREENBOOK NOT IN (''CMP'',''PDC'',''MPS'',''IMPLANT'') GROUP BY A.QUOTE_ID,A.SERVICE_ID,A.EQUIPMENT_ID,A.REVISION_ID ) A WHERE SERVICE_COMPLEXITY<2) )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND A.REVISION_ID = B.QTEREV_ID    '")

					primaryQueryItems = SqlHelper.GetFirst("sp_executesql @t=N'UPDATE A SET ENTITLEMENT_XML = REPLACE(replace(replace(entitlement_xml,''    '',''''),''\n'','''') ,  substring(replace(replace(entitlement_xml,''    '',''''),''\n'',''''),charindex(''<ENTITLEMENT_NAME>AGS_Z0091_VAL_SCCCDF</ENTITLEMENT_NAME><ENTITLEMENT_VALUE_CODE>'' ,replace(replace(entitlement_xml,''    '',''''),''\n'','''')),charindex(''</ENTITLEMENT_VALUE_CODE>''  ,substring(replace(replace(entitlement_xml,''    '',''''),''\n'',''''),charindex(''<ENTITLEMENT_NAME>AGS_Z0091_VAL_SCCCDF</ENTITLEMENT_NAME><ENTITLEMENT_VALUE_CODE>'' ,replace(replace(entitlement_xml,''    '',''''),''\n'','''')) , 1000 ))-1  ), ''<ENTITLEMENT_NAME>AGS_Z0091_VAL_SCCCDF</ENTITLEMENT_NAME><ENTITLEMENT_VALUE_CODE>''+TOOL_SERVICE_COMPLEXITY+'' ) FROM SAQSCE(NOLOCK) A JOIN (SELECT DISTINCT QUOTE_ID ,SERVICE_ID,EQUIPMENT_ID,REVISION_ID,TOOL_SERVICE_COMPLEXITY SAQICO_INBOUND B(NOLOCK) WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"'  )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID  = B.EQUIPMENT_ID AND A.QTEREV_ID = B.REVISION_ID  WHERE A.QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND A.QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"'' AND ISNULL(TOOL_SERVICE_COMPLEXITY,'''')<>'''' '")

					primaryQueryItems = SqlHelper.GetFirst(
										""
										+ str(Parameter1.QUERY_CRITERIA_1)
										+ "  SAQICO_INBOUND SET SERVICECOMPLEXITY_COEFFICIENT = CONVERT(VARCHAR,ENTITLEMENT_COEFFICIENT) FROM SAQICO_INBOUND (NOLOCK) A JOIN PRENVL B(NOLOCK) ON A.TOOL_SERVICE_COMPLEXITY = B.ENTITLEMENT_DISPLAY_VALUE WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' AND ISNULL(TOOL_SERVICE_COMPLEXITY,'''') <> '''' AND ENTITLEMENT_NAME = ''AGS_Z0091_VAL_SCCCDF'' ' ")
										
					primaryQueryItems = SqlHelper.GetFirst("sp_executesql @t=N'UPDATE A SET ENTITLEMENT_XML = REPLACE(replace(replace(entitlement_xml,''    '',''''),''\n'','''') ,  substring(replace(replace(entitlement_xml,''    '',''''),''\n'',''''),charindex(''<ENTITLEMENT_NAME>AGS_Z0091_VAL_SCCCCO</ENTITLEMENT_NAME><ENTITLEMENT_VALUE_CODE>'' ,replace(replace(entitlement_xml,''    '',''''),''\n'','''')),charindex(''</ENTITLEMENT_VALUE_CODE>''  ,substring(replace(replace(entitlement_xml,''    '',''''),''\n'',''''),charindex(''<ENTITLEMENT_NAME>AGS_Z0091_VAL_SCCCCO</ENTITLEMENT_NAME><ENTITLEMENT_VALUE_CODE>'' ,replace(replace(entitlement_xml,''    '',''''),''\n'','''')) , 1000 ))-1  ), ''<ENTITLEMENT_NAME>AGS_Z0091_VAL_SCCCCO</ENTITLEMENT_NAME><ENTITLEMENT_VALUE_CODE>''+SERVICECOMPLEXITY_COEFFICIENT+'' ) FROM SAQSCE(NOLOCK) A JOIN (SELECT DISTINCT QUOTE_ID ,SERVICE_ID,EQUIPMENT_ID,REVISION_ID,SERVICECOMPLEXITY_COEFFICIENT SAQICO_INBOUND B(NOLOCK) WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"'  )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID  = B.EQUIPMENT_ID AND A.QTEREV_ID = B.REVISION_ID  WHERE A.QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND A.QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"'' AND ISNULL(SERVICECOMPLEXITY_COEFFICIENT,'''')<>'''' '")

					#Swap Kit AMAT Provided
					S3 = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET SWAP_KIT=ENTITLEMENT_DISPLAY_VALUE FROM SAQICO_INBOUND A(NOLOCK) JOIN (SELECT distinct quote_ID,equipment_id,service_id, replace(X.Y.value(''(ENTITLEMENT_DISPLAY_VALUE)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DISPLAY_VALUE,replace(X.Y.value(''(ENTITLEMENT_DESCRIPTION)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DESCRIPTION FROM (SELECT quote_ID,equipment_id,service_id,CONVERT(XML,''<QUOTE_ENTITLEMENT>''+substring(entitlement_xml,charindex (''<ENTITLEMENT_NAME>AGS_Z0004_STT_SWKTAP'',entitlement_xml),charindex (''Swap Kits (Applied provided)</ENTITLEMENT_DESCRIPTION>'',entitlement_xml)-charindex (''<ENTITLEMENT_NAME>AGS_Z0004_STT_SWKTAP'',entitlement_xml)+len(''Swap Kits (Applied provided)</ENTITLEMENT_DESCRIPTION>''))+''</QUOTE_ENTITLEMENT>'') as entitlement_xml FROM SAQSCE (nolock)a WHERE QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"'' AND SERVICE_ID = ''Z0004'') e OUTER APPLY e.ENTITLEMENT_XML.nodes(''QUOTE_ENTITLEMENT'') as X(Y) )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID  WHERE B.ENTITLEMENT_DESCRIPTION=''Swap Kits (Applied provided)''  '")
					
					S3 = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET SWAP_KIT=ENTITLEMENT_DISPLAY_VALUE FROM SAQICO_INBOUND A(NOLOCK) JOIN (SELECT distinct quote_ID,equipment_id,service_id, replace(X.Y.value(''(ENTITLEMENT_DISPLAY_VALUE)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DISPLAY_VALUE,replace(X.Y.value(''(ENTITLEMENT_DESCRIPTION)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DESCRIPTION FROM (SELECT quote_ID,equipment_id,service_id,CONVERT(XML,''<QUOTE_ENTITLEMENT>''+substring(entitlement_xml,charindex (''<ENTITLEMENT_NAME>AGS_Z0091_STT_SWKTAP'',entitlement_xml),charindex (''Swap Kits (Applied provided)</ENTITLEMENT_DESCRIPTION>'',entitlement_xml)-charindex (''<ENTITLEMENT_NAME>AGS_Z0091_STT_SWKTAP'',entitlement_xml)+len(''Swap Kits (Applied provided)</ENTITLEMENT_DESCRIPTION>''))+''</QUOTE_ENTITLEMENT>'') as entitlement_xml FROM SAQSCE (nolock)a WHERE QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"'' AND SERVICE_ID =''Z0091'') e OUTER APPLY e.ENTITLEMENT_XML.nodes(''QUOTE_ENTITLEMENT'') as X(Y) )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID  WHERE B.ENTITLEMENT_DESCRIPTION=''Swap Kits (Applied provided)''  '")
					
					S3 = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET SWAP_KIT=ENTITLEMENT_DISPLAY_VALUE FROM SAQICO_INBOUND A(NOLOCK) JOIN (SELECT distinct quote_ID,equipment_id,service_id, replace(X.Y.value(''(ENTITLEMENT_DISPLAY_VALUE)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DISPLAY_VALUE,replace(X.Y.value(''(ENTITLEMENT_DESCRIPTION)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DESCRIPTION FROM (SELECT quote_ID,equipment_id,service_id,CONVERT(XML,''<QUOTE_ENTITLEMENT>''+substring(entitlement_xml,charindex (''<ENTITLEMENT_NAME>AGS_Z0035_STT_SWKTAP'',entitlement_xml),charindex (''Swap Kits (Applied provided)</ENTITLEMENT_DESCRIPTION>'',entitlement_xml)-charindex (''<ENTITLEMENT_NAME>AGS_Z0035_STT_SWKTAP'',entitlement_xml)+len(''Swap Kits (Applied provided)</ENTITLEMENT_DESCRIPTION>''))+''</QUOTE_ENTITLEMENT>'') as entitlement_xml FROM SAQSCE (nolock)a WHERE QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"'' AND SERVICE_ID =''Z0035'') e OUTER APPLY e.ENTITLEMENT_XML.nodes(''QUOTE_ENTITLEMENT'') as X(Y) )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID  WHERE B.ENTITLEMENT_DESCRIPTION=''Swap Kits (Applied provided)''  '")
					
					#Subfab like SS
					S3 = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET SUBFAB_SS=ENTITLEMENT_DISPLAY_VALUE FROM SAQICO_INBOUND A(NOLOCK) JOIN (SELECT distinct quote_ID,equipment_id,service_id, replace(X.Y.value(''(ENTITLEMENT_DISPLAY_VALUE)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DISPLAY_VALUE,replace(X.Y.value(''(ENTITLEMENT_DESCRIPTION)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DESCRIPTION FROM (SELECT quote_ID,equipment_id,service_id,CONVERT(XML,''<QUOTE_ENTITLEMENT>''+substring(entitlement_xml,charindex (''<ENTITLEMENT_NAME>AGS_Z0004_KPI_PRPFGT'',entitlement_xml),charindex (''Primary KPI. Perf Guarantee</ENTITLEMENT_DESCRIPTION>'',entitlement_xml)-charindex (''<ENTITLEMENT_NAME>AGS_Z0004_KPI_PRPFGT'',entitlement_xml)+len(''Primary KPI. Perf Guarantee</ENTITLEMENT_DESCRIPTION>''))+''</QUOTE_ENTITLEMENT>'') as entitlement_xml FROM SAQSCE (nolock)a WHERE QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"'' AND SERVICE_ID =''Z0004'') e OUTER APPLY e.ENTITLEMENT_XML.nodes(''QUOTE_ENTITLEMENT'') as X(Y) )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID  WHERE B.ENTITLEMENT_DESCRIPTION=''Primary KPI. Perf Guarantee''  '")
					...
					primaryQueryItems = SqlHelper.GetFirst("sp_executesql @t=N'UPDATE A SET ENTITLEMENT_XML = REPLACE(replace(replace(entitlement_xml,''    '',''''),''\n'','''') ,  substring(replace(replace(entitlement_xml,''    '',''''),''\n'',''''),charindex(''<ENTITLEMENT_NAME>AGS_Z0091_VAL_CAPAVD</ENTITLEMENT_NAME><ENTITLEMENT_VALUE_CODE>'' ,replace(replace(entitlement_xml,''    '',''''),''\n'','''')),charindex(''</ENTITLEMENT_VALUE_CODE>''  ,substring(replace(replace(entitlement_xml,''    '',''''),''\n'',''''),charindex(''<ENTITLEMENT_NAME>AGS_Z0091_VAL_CAPAVD</ENTITLEMENT_NAME><ENTITLEMENT_VALUE_CODE>'' ,replace(replace(entitlement_xml,''    '',''''),''\n'','''')) , 1000 ))-1  ), ''<ENTITLEMENT_NAME>AGS_Z0091_VAL_CAPAVD</ENTITLEMENT_NAME><ENTITLEMENT_VALUE_CODE>''+TOOL_CAPITAL_AVOIDANCE+'' ) FROM SAQSCE(NOLOCK) A JOIN (SELECT DISTINCT QUOTE_ID ,SERVICE_ID,EQUIPMENT_ID,REVISION_ID,TOOL_CAPITAL_AVOIDANCE SAQICO_INBOUND B(NOLOCK) WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' ) B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID  = B.EQUIPMENT_ID AND A.QTEREV_ID = B.REVISION_ID  WHERE A.QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND A.QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"'' AND ISNULL(TOOL_CAPITAL_AVOIDANCE,'''')<>'''' '")

					primaryQueryItems = SqlHelper.GetFirst(
										""
										+ str(Parameter1.QUERY_CRITERIA_1)
										+ "  SAQICO_INBOUND SET CAPITALAVOIDANCE_COEFFICIENT = CONVERT(VARCHAR,ENTITLEMENT_COEFFICIENT) FROM SAQICO_INBOUND (NOLOCK) A JOIN PRENVL B(NOLOCK) ON A.TOOL_CAPITAL_AVOIDANCE = B.ENTITLEMENT_DISPLAY_VALUE WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' AND ISNULL(TOOL_CAPITAL_AVOIDANCE,'''') <> '''' AND ENTITLEMENT_NAME = ''AGS_Z0091_VAL_CAPAVD'' ' ")
										
					primaryQueryItems = SqlHelper.GetFirst("sp_executesql @t=N'UPDATE A SET ENTITLEMENT_XML = REPLACE(replace(replace(entitlement_xml,''    '',''''),''\n'','''') ,  substring(replace(replace(entitlement_xml,''    '',''''),''\n'',''''),charindex(''<ENTITLEMENT_NAME>AGS_Z0091_VAL_CAPACO</ENTITLEMENT_NAME><ENTITLEMENT_VALUE_CODE>'' ,replace(replace(entitlement_xml,''    '',''''),''\n'','''')),charindex(''</ENTITLEMENT_VALUE_CODE>''  ,substring(replace(replace(entitlement_xml,''    '',''''),''\n'',''''),charindex(''<ENTITLEMENT_NAME>AGS_Z0091_VAL_CAPACO</ENTITLEMENT_NAME><ENTITLEMENT_VALUE_CODE>'' ,replace(replace(entitlement_xml,''    '',''''),''\n'','''')) , 1000 ))-1  ), ''<ENTITLEMENT_NAME>AGS_Z0091_VAL_CAPACO</ENTITLEMENT_NAME><ENTITLEMENT_VALUE_CODE>''+CAPITALAVOIDANCE_COEFFICIENT+'' ) FROM SAQSCE(NOLOCK) A JOIN (SELECT DISTINCT QUOTE_ID ,SERVICE_ID,EQUIPMENT_ID,REVISION_ID,CAPITALAVOIDANCE_COEFFICIENT SAQICO_INBOUND B(NOLOCK) WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID  = B.EQUIPMENT_ID AND A.QTEREV_ID = B.REVISION_ID  WHERE A.QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND A.QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"''  AND ISNULL(CAPITALAVOIDANCE_COEFFICIENT,'''')<>'''' '")
					
					#Customer Segment
					S3 = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET TOTAL_COEFFICIENT= ISNULL(TOTAL_COEFFICIENT,0) + ISNULL(CONVERT(FLOAT,ENTITLEMENT_VALUE_CODE),0) FROM SAQICO_INBOUND A(NOLOCK) JOIN (SELECT distinct quote_ID,equipment_id,service_id, replace(X.Y.value(''(ENTITLEMENT_VALUE_CODE)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_VALUE_CODE,replace(X.Y.value(''(ENTITLEMENT_DESCRIPTION)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DESCRIPTION FROM (SELECT quote_ID,equipment_id,service_id,CONVERT(XML,''<QUOTE_ENTITLEMENT>''+substring(entitlement_xml,charindex (''<ENTITLEMENT_NAME>AGS_Z0091_VAL_CSSGCO'',entitlement_xml),charindex (''Customer Segment Coefficent</ENTITLEMENT_DESCRIPTION>'',entitlement_xml)-charindex (''<ENTITLEMENT_NAME>AGS_Z0091_VAL_CSSGCO'',entitlement_xml)+len(''Customer Segment Coefficent</ENTITLEMENT_DESCRIPTION>''))+''</QUOTE_ENTITLEMENT>'') as entitlement_xml FROM SAQSCE (nolock)a WHERE QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"'' AND SERVICE_ID = ''Z0091'') e OUTER APPLY e.ENTITLEMENT_XML.nodes(''QUOTE_ENTITLEMENT'') as X(Y) )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID  WHERE B.ENTITLEMENT_DESCRIPTION=''Customer Segment Coefficent'' AND ISNULL(ENTITLEMENT_VALUE_CODE,'''') <>''''  '")

					#Service Competition
					S3 = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET TOTAL_COEFFICIENT= ISNULL(TOTAL_COEFFICIENT,0) + ISNULL(CONVERT(FLOAT,ENTITLEMENT_VALUE_CODE),0) FROM SAQICO_INBOUND A(NOLOCK) JOIN (SELECT distinct quote_ID,equipment_id,service_id, replace(X.Y.value(''(ENTITLEMENT_VALUE_CODE)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_VALUE_CODE,replace(X.Y.value(''(ENTITLEMENT_DESCRIPTION)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DESCRIPTION FROM (SELECT quote_ID,equipment_id,service_id,CONVERT(XML,''<QUOTE_ENTITLEMENT>''+substring(entitlement_xml,charindex (''<ENTITLEMENT_NAME>AGS_Z0091_VAL_SVCCCO'',entitlement_xml),charindex (''Service Competition Coefficient</ENTITLEMENT_DESCRIPTION>'',entitlement_xml)-charindex (''<ENTITLEMENT_NAME>AGS_Z0091_VAL_SVCCCO'',entitlement_xml)+len(''Service Competition Coefficient</ENTITLEMENT_DESCRIPTION>''))+''</QUOTE_ENTITLEMENT>'') as entitlement_xml FROM SAQSCE (nolock)a WHERE QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"'' AND SERVICE_ID = ''Z0091'') e OUTER APPLY e.ENTITLEMENT_XML.nodes(''QUOTE_ENTITLEMENT'') as X(Y) )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID  WHERE B.ENTITLEMENT_DESCRIPTION=''Service Competition Coefficient'' AND ISNULL(ENTITLEMENT_VALUE_CODE,'''') <>''''  '")

					#Quality Required 
					S3 = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET TOTAL_COEFFICIENT= ISNULL(TOTAL_COEFFICIENT,0) + ISNULL(CONVERT(FLOAT,ENTITLEMENT_VALUE_CODE),0) FROM SAQICO_INBOUND A(NOLOCK) JOIN (SELECT distinct quote_ID,equipment_id,service_id, replace(X.Y.value(''(ENTITLEMENT_VALUE_CODE)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_VALUE_CODE,replace(X.Y.value(''(ENTITLEMENT_DESCRIPTION)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DESCRIPTION FROM (SELECT quote_ID,equipment_id,service_id,CONVERT(XML,''<QUOTE_ENTITLEMENT>''+substring(entitlement_xml,charindex (''<ENTITLEMENT_NAME>AGS_Z0091_VAL_QLYRCO'',entitlement_xml),charindex (''Quality Required Coefficient</ENTITLEMENT_DESCRIPTION>'',entitlement_xml)-charindex (''<ENTITLEMENT_NAME>AGS_Z0091_VAL_QLYRCO'',entitlement_xml)+len(''Quality Required Coefficient</ENTITLEMENT_DESCRIPTION>''))+''</QUOTE_ENTITLEMENT>'') as entitlement_xml FROM SAQSCE (nolock)a WHERE QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"'' AND SERVICE_ID = ''Z0091'') e OUTER APPLY e.ENTITLEMENT_XML.nodes(''QUOTE_ENTITLEMENT'') as X(Y) )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID  WHERE B.ENTITLEMENT_DESCRIPTION=''Quality Required Coefficient'' AND ISNULL(ENTITLEMENT_VALUE_CODE,'''') <>''''  '")

					#Intercept
					S3 = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET INTERCEPT=  ISNULL(CONVERT(FLOAT,ENTITLEMENT_VALUE_CODE),0) FROM SAQICO_INBOUND A(NOLOCK) JOIN (SELECT distinct quote_ID,equipment_id,service_id, replace(X.Y.value(''(ENTITLEMENT_VALUE_CODE)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_VALUE_CODE,replace(X.Y.value(''(ENTITLEMENT_DESCRIPTION)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DESCRIPTION FROM (SELECT quote_ID,equipment_id,service_id,CONVERT(XML,''<QUOTE_ENTITLEMENT>''+substring(entitlement_xml,charindex (''<ENTITLEMENT_NAME>AGS_Z0091_VAL_INTCCO'',entitlement_xml),charindex (''Intercept Coefficient</ENTITLEMENT_DESCRIPTION>'',entitlement_xml)-charindex (''<ENTITLEMENT_NAME>AGS_Z0091_VAL_INTCCO'',entitlement_xml)+len(''Intercept Coefficient</ENTITLEMENT_DESCRIPTION>''))+''</QUOTE_ENTITLEMENT>'') as entitlement_xml FROM SAQSCE (nolock)a WHERE QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"'' AND SERVICE_ID = ''Z0091'') e OUTER APPLY e.ENTITLEMENT_XML.nodes(''QUOTE_ENTITLEMENT'') as X(Y) )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID  WHERE B.ENTITLEMENT_DESCRIPTION=''Intercept Coefficient'' AND ISNULL(ENTITLEMENT_VALUE_CODE,'''') <>'''' '")
					...
					S3 = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET TOTAL_COEFFICIENT= ISNULL(TOTAL_COEFFICIENT,0) + ISNULL(CONVERT(FLOAT,ENTITLEMENT_VALUE_CODE),0) FROM SAQICO_INBOUND A(NOLOCK) JOIN (SELECT distinct quote_ID,equipment_id,service_id, replace(X.Y.value(''(ENTITLEMENT_VALUE_CODE)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_VALUE_CODE,replace(X.Y.value(''(ENTITLEMENT_DESCRIPTION)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DESCRIPTION FROM (SELECT quote_ID,equipment_id,service_id,CONVERT(XML,''<QUOTE_ENTITLEMENT>''+substring(entitlement_xml,charindex (''<ENTITLEMENT_NAME>AGS_Z0091_VAL_POFFCO'',entitlement_xml),charindex (''Product Offering Coefficient</ENTITLEMENT_DESCRIPTION>'',entitlement_xml)-charindex (''<ENTITLEMENT_NAME>AGS_Z0091_VAL_POFFCO'',entitlement_xml)+len(''Product Offering Coefficient</ENTITLEMENT_DESCRIPTION>''))+''</QUOTE_ENTITLEMENT>'') as entitlement_xml FROM SAQSCE (nolock)a WHERE QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"'' AND SERVICE_ID = ''Z0091'') e OUTER APPLY e.ENTITLEMENT_XML.nodes(''QUOTE_ENTITLEMENT'') as X(Y) )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID  WHERE B.ENTITLEMENT_DESCRIPTION=''Product Offering Coefficient'' AND ISNULL(ENTITLEMENT_VALUE_CODE,'''') <>''''  '")

					#Greenbook Coefficient
					S3 = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET TOTAL_COEFFICIENT= ISNULL(TOTAL_COEFFICIENT,0) + ISNULL(CONVERT(FLOAT,ENTITLEMENT_VALUE_CODE),0) FROM SAQICO_INBOUND A(NOLOCK) JOIN (SELECT distinct quote_ID,equipment_id,service_id, replace(X.Y.value(''(ENTITLEMENT_VALUE_CODE)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_VALUE_CODE,replace(X.Y.value(''(ENTITLEMENT_DESCRIPTION)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DESCRIPTION FROM (SELECT quote_ID,equipment_id,service_id,CONVERT(XML,''<QUOTE_ENTITLEMENT>''+substring(entitlement_xml,charindex (''<ENTITLEMENT_NAME>AGS_Z0091_VAL_GRNBCO'',entitlement_xml),charindex (''Greenbook Coefficient</ENTITLEMENT_DESCRIPTION>'',entitlement_xml)-charindex (''<ENTITLEMENT_NAME>AGS_Z0091_VAL_GRNBCO'',entitlement_xml)+len(''Greenbook Coefficient</ENTITLEMENT_DESCRIPTION>''))+''</QUOTE_ENTITLEMENT>'') as entitlement_xml FROM SAQSCE (nolock)a WHERE QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"'' AND SERVICE_ID = ''Z0091'') e OUTER APPLY e.ENTITLEMENT_XML.nodes(''QUOTE_ENTITLEMENT'') as X(Y) )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID  WHERE B.ENTITLEMENT_DESCRIPTION=''Greenbook Coefficient'' AND ISNULL(ENTITLEMENT_VALUE_CODE,'''') <>''''  '")

					#Uptime Improvement Coefficient
					S3 = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET TOTAL_COEFFICIENT= ISNULL(TOTAL_COEFFICIENT,0) + ISNULL(CONVERT(FLOAT,ENTITLEMENT_VALUE_CODE),0) FROM SAQICO_INBOUND A(NOLOCK) JOIN (SELECT distinct quote_ID,equipment_id,service_id, replace(X.Y.value(''(ENTITLEMENT_VALUE_CODE)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_VALUE_CODE,replace(X.Y.value(''(ENTITLEMENT_DESCRIPTION)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DESCRIPTION FROM (SELECT quote_ID,equipment_id,service_id,CONVERT(XML,''<QUOTE_ENTITLEMENT>''+substring(entitlement_xml,charindex (''<ENTITLEMENT_NAME>AGS_Z0091_VAL_UPIMCO'',entitlement_xml),charindex (''Uptime Improvement Coefficient</ENTITLEMENT_DESCRIPTION>'',entitlement_xml)-charindex (''<ENTITLEMENT_NAME>AGS_Z0091_VAL_UPIMCO'',entitlement_xml)+len(''Uptime Improvement Coefficient</ENTITLEMENT_DESCRIPTION>''))+''</QUOTE_ENTITLEMENT>'') as entitlement_xml FROM SAQSCE (nolock)a WHERE QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"'' AND SERVICE_ID = ''Z0091'') e OUTER APPLY e.ENTITLEMENT_XML.nodes(''QUOTE_ENTITLEMENT'') as X(Y) )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID  WHERE B.ENTITLEMENT_DESCRIPTION=''Uptime Improvement Coefficient'' AND ISNULL(ENTITLEMENT_VALUE_CODE,'''') <>''''  '")

					#Wafer Node Coefficient
					S3 = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET TOTAL_COEFFICIENT= ISNULL(TOTAL_COEFFICIENT,0) + ISNULL(CONVERT(FLOAT,ENTITLEMENT_VALUE_CODE),0) FROM SAQICO_INBOUND A(NOLOCK) JOIN (SELECT distinct quote_ID,equipment_id,service_id, replace(X.Y.value(''(ENTITLEMENT_VALUE_CODE)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_VALUE_CODE,replace(X.Y.value(''(ENTITLEMENT_DESCRIPTION)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DESCRIPTION FROM (SELECT quote_ID,equipment_id,service_id,CONVERT(XML,''<QUOTE_ENTITLEMENT>''+substring(entitlement_xml,charindex (''<ENTITLEMENT_NAME>AGS_Z0091_VAL_WAFNCO'',entitlement_xml),charindex (''Wafer Node Coefficient</ENTITLEMENT_DESCRIPTION>'',entitlement_xml)-charindex (''<ENTITLEMENT_NAME>AGS_Z0091_VAL_WAFNCO'',entitlement_xml)+len(''Wafer Node Coefficient</ENTITLEMENT_DESCRIPTION>''))+''</QUOTE_ENTITLEMENT>'') as entitlement_xml FROM SAQSCE (nolock)a WHERE QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"'' AND SERVICE_ID = ''Z0091'') e OUTER APPLY e.ENTITLEMENT_XML.nodes(''QUOTE_ENTITLEMENT'') as X(Y) )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID  WHERE B.ENTITLEMENT_DESCRIPTION=''Wafer Node Coefficient'' AND ISNULL(ENTITLEMENT_VALUE_CODE,'''') <>''''  '")

					#Device Type Coefficient
					S3 = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET TOTAL_COEFFICIENT= ISNULL(TOTAL_COEFFICIENT,0) + ISNULL(CONVERT(FLOAT,ENTITLEMENT_VALUE_CODE),0) FROM SAQICO_INBOUND A(NOLOCK) JOIN (SELECT distinct quote_ID,equipment_id,service_id, replace(X.Y.value(''(ENTITLEMENT_VALUE_CODE)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_VALUE_CODE,replace(X.Y.value(''(ENTITLEMENT_DESCRIPTION)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DESCRIPTION FROM (SELECT quote_ID,equipment_id,service_id,CONVERT(XML,''<QUOTE_ENTITLEMENT>''+substring(entitlement_xml,charindex (''<ENTITLEMENT_NAME>AGS_Z0091_VAL_DEVTCO'',entitlement_xml),charindex (''Device Type Coefficient</ENTITLEMENT_DESCRIPTION>'',entitlement_xml)-charindex (''<ENTITLEMENT_NAME>AGS_Z0091_VAL_DEVTCO'',entitlement_xml)+len(''Device Type Coefficient</ENTITLEMENT_DESCRIPTION>''))+''</QUOTE_ENTITLEMENT>'') as entitlement_xml FROM SAQSCE (nolock)a WHERE QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"'' AND SERVICE_ID = ''Z0091'') e OUTER APPLY e.ENTITLEMENT_XML.nodes(''QUOTE_ENTITLEMENT'') as X(Y) )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID  WHERE B.ENTITLEMENT_DESCRIPTION=''Device Type Coefficient'' AND ISNULL(ENTITLEMENT_VALUE_CODE,'''') <>''''  '")

					#CSA Tools per Fab Coefficient
					S3 = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET TOTAL_COEFFICIENT= ISNULL(TOTAL_COEFFICIENT,0) + ISNULL(CONVERT(FLOAT,ENTITLEMENT_VALUE_CODE),0) FROM SAQICO_INBOUND A(NOLOCK) JOIN (SELECT distinct quote_ID,equipment_id,service_id, replace(X.Y.value(''(ENTITLEMENT_VALUE_CODE)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_VALUE_CODE,replace(X.Y.value(''(ENTITLEMENT_DESCRIPTION)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DESCRIPTION FROM (SELECT quote_ID,equipment_id,service_id,CONVERT(XML,''<QUOTE_ENTITLEMENT>''+substring(entitlement_xml,charindex (''<ENTITLEMENT_NAME>AGS_Z0091_VAL_TLSFCO'',entitlement_xml),charindex (''# CSA Tools per Fab Coefficient</ENTITLEMENT_DESCRIPTION>'',entitlement_xml)-charindex (''<ENTITLEMENT_NAME>AGS_Z0091_VAL_TLSFCO'',entitlement_xml)+len(''# CSA Tools per Fab Coefficient</ENTITLEMENT_DESCRIPTION>''))+''</QUOTE_ENTITLEMENT>'') as entitlement_xml FROM SAQSCE (nolock)a WHERE QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"'' AND SERVICE_ID = ''Z0091'') e OUTER APPLY e.ENTITLEMENT_XML.nodes(''QUOTE_ENTITLEMENT'') as X(Y) )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID  WHERE B.ENTITLEMENT_DESCRIPTION=''# CSA Tools per Fab Coefficient'' AND ISNULL(ENTITLEMENT_VALUE_CODE,'''') <>''''  '")

					#SSCM Coefficient
					S3 = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET TOTAL_COEFFICIENT= ISNULL(TOTAL_COEFFICIENT,0) + ISNULL(CONVERT(FLOAT,NPI_COEFFICIENT),0)+ ISNULL(CONVERT(FLOAT,SERVICECOMPLEXITY_COEFFICIENT),0) + ISNULL(CONVERT(FLOAT,CAPITALAVOIDANCE_COEFFICIENT),0) FROM SAQICO_INBOUND A(NOLOCK) WHERE QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND REVISION_ID = ''"+str(Qt_Id.REVISION_ID)+"'' AND SERVICE_ID = ''Z0091''  '")
					...
					S3 = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET MODEL_PRICE = MODEL_PRICE * (1 + ISNULL(CONVERT(FLOAT,ENTITLEMENT_VALUE_CODE),0) ) FROM SAQICO A(NOLOCK) JOIN (SELECT distinct quote_ID,equipment_id,service_id, replace(X.Y.value(''(ENTITLEMENT_VALUE_CODE)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_VALUE_CODE,replace(X.Y.value(''(ENTITLEMENT_DESCRIPTION)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DESCRIPTION FROM (SELECT quote_ID,equipment_id,service_id,QTEREV_ID,CONVERT(XML,''<QUOTE_ENTITLEMENT>''+substring(entitlement_xml,charindex (''<ENTITLEMENT_NAME>AGS_Z0091_VAL_CCRTCO'',entitlement_xml),charindex (''Contract Coverage & Response Time Coefficient</ENTITLEMENT_DESCRIPTION>'',entitlement_xml)-charindex (''<ENTITLEMENT_NAME>AGS_Z0091_VAL_CCRTCO'',entitlement_xml)+len(''Contract Coverage & Response Time Coefficient</ENTITLEMENT_DESCRIPTION>''))+''</QUOTE_ENTITLEMENT>'') as entitlement_xml FROM SAQSCE (nolock)a WHERE QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"'' AND SERVICE_ID = ''Z0091'') e OUTER APPLY e.ENTITLEMENT_XML.nodes(''QUOTE_ENTITLEMENT'') as X(Y) )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND A.QTEREV_ID = B.QTEREV_ID  WHERE B.ENTITLEMENT_DESCRIPTION=''Contract Coverage & Response Time Coefficient'' AND ISNULL(ENTITLEMENT_VALUE_CODE,'''') <>''''  '")
					...
					S3 = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET MODEL_PRICE = MODEL_PRICE + ISNULL(CONVERT(FLOAT,C.ENTITLEMENT_PRICE_IMPACT),0) , TOTAL_COST_WSEEDSTOCK = TOTAL_COST_WSEEDSTOCK + ISNULL(CONVERT(FLOAT,C.ENTITLEMENT_COST_IMPACT),0)  FROM SAQICO A(NOLOCK) JOIN (SELECT distinct quote_ID,equipment_id,service_id, replace(X.Y.value(''(ENTITLEMENT_VALUE_CODE)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_VALUE_CODE,replace(X.Y.value(''(ENTITLEMENT_DESCRIPTION)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DESCRIPTION FROM (SELECT quote_ID,equipment_id,service_id,QTEREV_ID,CONVERT(XML,''<QUOTE_ENTITLEMENT>''+substring(entitlement_xml,charindex (''<ENTITLEMENT_NAME>AGS_Z0091_NET_NUMLAY'',entitlement_xml),charindex (''Number of Layers</ENTITLEMENT_DESCRIPTION>'',entitlement_xml)-charindex (''<ENTITLEMENT_NAME>AGS_Z0091_NET_NUMLAY'',entitlement_xml)+len(''Number of Layers</ENTITLEMENT_DESCRIPTION>''))+''</QUOTE_ENTITLEMENT>'') as entitlement_xml FROM SAQIEN (nolock)A JOIN SAQICO B (NOLOCK) ON A.QUOTE_ID = B.QUOTE_ID AND A.QTEREV_ID = B.QTEREV_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID WHERE A.QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND A.QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"'' AND A.SERVICE_ID = ''Z0091'' AND A.GREENBOOK=''PDC'') e OUTER APPLY e.ENTITLEMENT_XML.nodes(''QUOTE_ENTITLEMENT'') as X(Y) )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND A.QTEREV_ID = B.QTEREV_ID JOIN PREGBV C(NOLOCK) ON B.ENTITLEMENT_DESCRIPTION = C.ENTITLEMENT_DESCRIPTION AND B.ENTITLEMENT_VALUE_CODE = C.ENTITLEMENT_VALUE_CODE WHERE B.ENTITLEMENT_DESCRIPTION=''Number of Layers'' AND ISNULL(B.ENTITLEMENT_VALUE_CODE,'''') <>''''  '")
					
					#Specialized Coating
					S3 = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET TOTAL_COST_WSEEDSTOCK = TOTAL_COST_WSEEDSTOCK + ISNULL(CONVERT(FLOAT,C.ENTITLEMENT_COST_IMPACT),0), MODEL_PRICE = MODEL_PRICE + ISNULL(CONVERT(FLOAT,C.ENTITLEMENT_PRICE_IMPACT),0)   FROM SAQICO A(NOLOCK) JOIN (SELECT distinct quote_ID,equipment_id,service_id, replace(X.Y.value(''(ENTITLEMENT_VALUE_CODE)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_VALUE_CODE,replace(X.Y.value(''(ENTITLEMENT_DESCRIPTION)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DESCRIPTION FROM (SELECT quote_ID,equipment_id,service_id,QTEREV_ID,CONVERT(XML,''<QUOTE_ENTITLEMENT>''+substring(entitlement_xml,charindex (''<ENTITLEMENT_NAME>AGS_Z0091_STT_SPCCOT'',entitlement_xml),charindex (''Specialized Coating</ENTITLEMENT_DESCRIPTION>'',entitlement_xml)-charindex (''<ENTITLEMENT_NAME>AGS_Z0091_STT_SPCCOT'',entitlement_xml)+len(''Specialized Coating</ENTITLEMENT_DESCRIPTION>''))+''</QUOTE_ENTITLEMENT>'') as entitlement_xml FROM SAQIEN (nolock)A JOIN SAQICO B (NOLOCK) ON A.QUOTE_ID = B.QUOTE_ID AND A.QTEREV_ID = B.QTEREV_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID WHERE A.QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND A.QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"'' AND A.SERVICE_ID = ''Z0091'' AND A.GREENBOOK=''PDC'') e OUTER APPLY e.ENTITLEMENT_XML.nodes(''QUOTE_ENTITLEMENT'') as X(Y) )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND A.QTEREV_ID = B.QTEREV_ID JOIN PREGBV C(NOLOCK) ON B.ENTITLEMENT_DESCRIPTION = C.ENTITLEMENT_DESCRIPTION AND B.ENTITLEMENT_VALUE_CODE = C.ENTITLEMENT_VALUE_CODE WHERE B.ENTITLEMENT_DESCRIPTION=''Specialized Coating'' AND ISNULL(B.ENTITLEMENT_DISPLAY_VALUE,'''') = ''Included''  '")
					
					#Specialized Cleaning					
					S3 = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET TOTAL_COST_WSEEDSTOCK = TOTAL_COST_WSEEDSTOCK + ISNULL(CONVERT(FLOAT,C.ENTITLEMENT_COST_IMPACT),0), MODEL_PRICE = MODEL_PRICE + ISNULL(CONVERT(FLOAT,C.ENTITLEMENT_PRICE_IMPACT),0)   FROM SAQICO A(NOLOCK) JOIN (SELECT distinct quote_ID,equipment_id,service_id, replace(X.Y.value(''(ENTITLEMENT_VALUE_CODE)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_VALUE_CODE,replace(X.Y.value(''(ENTITLEMENT_DESCRIPTION)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DESCRIPTION FROM (SELECT quote_ID,equipment_id,service_id,QTEREV_ID,CONVERT(XML,''<QUOTE_ENTITLEMENT>''+substring(entitlement_xml,charindex (''<ENTITLEMENT_NAME>AGS_Z0091_STT_SPCCLN'',entitlement_xml),charindex (''Specialized Cleaning</ENTITLEMENT_DESCRIPTION>'',entitlement_xml)-charindex (''<ENTITLEMENT_NAME>AGS_Z0091_STT_SPCCLN'',entitlement_xml)+len(''Specialized Cleaning</ENTITLEMENT_DESCRIPTION>''))+''</QUOTE_ENTITLEMENT>'') as entitlement_xml FROM SAQIEN (nolock)A JOIN SAQICO B (NOLOCK) ON A.QUOTE_ID = B.QUOTE_ID AND A.QTEREV_ID = B.QTEREV_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID WHERE A.QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND A.QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"'' AND A.SERVICE_ID = ''Z0091'' AND A.GREENBOOK=''PDC'') e OUTER APPLY e.ENTITLEMENT_XML.nodes(''QUOTE_ENTITLEMENT'') as X(Y) )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND A.QTEREV_ID = B.QTEREV_ID JOIN PREGBV C(NOLOCK) ON B.ENTITLEMENT_DESCRIPTION = C.ENTITLEMENT_DESCRIPTION AND B.ENTITLEMENT_VALUE_CODE = C.ENTITLEMENT_VALUE_CODE WHERE B.ENTITLEMENT_DESCRIPTION=''Specialized Cleaning'' AND ISNULL(B.ENTITLEMENT_DISPLAY_VALUE,'''') = ''Included''  '")
					
					#Head Break In
					S3 = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET TOTAL_COST_WSEEDSTOCK = TOTAL_COST_WSEEDSTOCK + ISNULL(CONVERT(FLOAT,C.FACTOR_TXTVAR),0)   FROM SAQICO A(NOLOCK) JOIN (SELECT distinct quote_ID,equipment_id,service_id, replace(X.Y.value(''(ENTITLEMENT_VALUE_CODE)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_VALUE_CODE,replace(X.Y.value(''(ENTITLEMENT_DESCRIPTION)[1]'', ''VARCHAR(128)''),'';#38'',''&'') as ENTITLEMENT_DESCRIPTION FROM (SELECT quote_ID,equipment_id,service_id,QTEREV_ID,CONVERT(XML,''<QUOTE_ENTITLEMENT>''+substring(entitlement_xml,charindex (''<ENTITLEMENT_NAME>AGS_Z0091_STT_HDBRIN'',entitlement_xml),charindex (''Head break-in</ENTITLEMENT_DESCRIPTION>'',entitlement_xml)-charindex (''<ENTITLEMENT_NAME>AGS_Z0091_STT_HDBRIN'',entitlement_xml)+len(''Head break-in</ENTITLEMENT_DESCRIPTION>''))+''</QUOTE_ENTITLEMENT>'') as entitlement_xml FROM SAQIEN (nolock)A JOIN SAQICO B (NOLOCK) ON A.QUOTE_ID = B.QUOTE_ID AND A.QTEREV_ID = B.QTEREV_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID WHERE A.QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND A.QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"'' AND A.SERVICE_ID = ''Z0091'' AND A.GREENBOOK=''PDC'') e OUTER APPLY e.ENTITLEMENT_XML.nodes(''QUOTE_ENTITLEMENT'') as X(Y) )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND A.QTEREV_ID = B.QTEREV_ID JOIN PRCFVA C(NOLOCK) ON A.WAFER_SIZE = C.FACTOR_VARIABLE_ID WHERE B.ENTITLEMENT_DESCRIPTION=''Head break-in'' AND ISNULL(B.ENTITLEMENT_DISPLAY_VALUE,'''') = ''Included'' AND C.FACTOR_ID  = ''HBWFCT''  '")
					...
					Greenbkquery=SqlHelper.GetList("SELECT DISTINCT SAQICO.GREENBOOK,ISNULL(SABUUN.DISTRIBUTION_EMAIL,'') AS DISTRIBUTION_EMAIL  FROM SAQICO(NOLOCK) JOIN SAQICA (NOLOCK) ON SAQICO.QUOTE_ID = SAQICA.QUOTE_ID AND SAQICO.QTEREV_ID = SAQICA.QTEREV_ID AND SAQICO.SERVICE_ID = SAQICA.SERVICE_ID AND SAQICO.EQUIPMENT_ID = SAQICA.EQUIPMENT_ID JOIN SABUUN (NOLOCK) ON SAQICO.GREENBOOK = SABUUN.BUSINESSUNIT_ID WHERE SAQICO.STATUS IN ('ERROR','PARTIALLY PRICED') AND SAQICO.QUOTE_ID = '"+str(Qt_Id.QUOTE_ID)+"' AND SAQICO.QTEREV_ID = '"+str(Qt_Id.REVISION_ID)+"'  ")

					for Gbk in Greenbkquery:


						Grnbkdataquery=SqlHelper.GetList("SELECT SAQICO.GREENBOOK,SAQICO.QUOTE_ID,SAQICO.SERVICE_ID,SAQICO.EQUIPMENT_ID,SAQICA.ASSEMBLY_ID,SAQICA.COST_MODULE_AVAILABLE,SAQICA.COST_MODULE_STATUS FROM SAQICO (NOLOCK) JOIN SAQICA (NOLOCK) ON SAQICO.QUOTE_ID = SAQICA.QUOTE_ID AND SAQICO.QTEREV_ID = SAQICA.QTEREV_ID AND SAQICO.SERVICE_ID = SAQICA.SERVICE_ID AND SAQICO.EQUIPMENT_ID = SAQICA.EQUIPMENT_ID WHERE SAQICO.STATUS  IN ('ERROR','PARTIALLY PRICED') AND SAQICO.GREENBOOK= '"+str(Gbk.GREENBOOK)+"' AND SAQICO.QUOTE_ID = '"+str(Qt_Id.QUOTE_ID)+"' AND SAQICO.QTEREV_ID = '"+str(Qt_Id.REVISION_ID)+"' AND  SAQICA.COST_MODULE_AVAILABLE= 'UNAVAILABLE' AND SAQICA.COST_MODULE_STATUS <> 'Chamber map not required' ")
