		...
		for json_data in Jsonquery:
			...
			if len(rebuilt_data) != 0:      
				...
				"""primaryQueryItems = SqlHelper.GetFirst(
									""
									+ str(Parameter1.QUERY_CRITERIA_1)
									+ "  SAQICO_INBOUND SET ANNITM_RECORD_ID = B.QUOTE_ITEM_COVERED_OBJECT_RECORD_ID,LINE = B.LINE FROM SAQICO_INBOUND (NOLOCK) A JOIN SAQICO B(NOLOCK) ON A.QUOTE_ID = B.QUOTE_ID AND A.REVISION_ID = B.QTEREV_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.LINE = CONVERT(VARCHAR,B.LINE)+''-''+QUOTE_ITEM_COVERED_OBJECT_RECORD_ID  WHERE ISNULL(PROCESS_STATUS,'''')='''' AND ISNULL(SESSION_ID,'''')=''"+str(sessiondetail.A)+ "'' '")"""						
				...
				if Check_flag == 1 and Saqico_Flag == 1:
					...
					primaryQueryItems = SqlHelper.GetFirst(
						""
						+ str(Parameter1.QUERY_CRITERIA_1)
						+ "  A SET  TCWOSS = B.TOTAL_COST_WOSEEDSTOCK,TCWISS = TOTAL_COST_WISEEDSTOCK FROM SAQICO A(NOLOCK) JOIN (SELECT QUOTE_ID,SERVICE_ID,SUM(CONVERT(FLOAT,TOTAL_COST_WOSEEDSTOCK)) AS TOTAL_COST_WOSEEDSTOCK,SUM(CONVERT(FLOAT,TOTAL_COST_WISEEDSTOCK)) AS TOTAL_COST_WISEEDSTOCK,REVISION_ID,LINE,EQUIPMENT_ID,CONTRACT_YEAR FROM SAQICO_INBOUND B(NOLOCK) WHERE ISNULL(PROCESS_STATUS,'''')=''INPROGRESS'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' GROUP BY QUOTE_ID,SERVICE_ID,REVISION_ID,LINE,EQUIPMENT_ID ,CONTRACT_YEAR )B ON A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.QTEREV_ID = B.REVISION_ID AND A.LINE = B.LINE AND A.EQUIPMENT_ID = B.EQUIPMENT_ID AND A.CNTYER = B.CONTRACT_YEAR  ' ")
					...
					""
					+ str(Parameter1.QUERY_CRITERIA_1)
					+ "  SAQITM SET PEREVTCST_INGL_CURR= ROUND(ISNULL(CNTCST,0),CONVERT(INT,"+str(roundcurr.DECIMAL_PLACES)+"),CONVERT(INT,"+str(roundcurr.ROUNDING_METHOD)+")) ,PEREVTPRC_INDT_CURR = ROUND(ISNULL(CASE WHEN BILTYP=''VARIABLE'' THEN TENVDC ELSE TNTVDC END,0),CONVERT(INT,"+str(roundcurr.DECIMAL_PLACES)+"),CONVERT(INT,"+str(roundcurr.ROUNDING_METHOD)+")),PEREVTPRC_INGL_CURR = ROUND(ISNULL(CASE WHEN BILTYP=''VARIABLE'' THEN TENVGC ELSE TNTVGC END,0),CONVERT(INT,"+str(roundcurr.DECIMAL_PLACES)+"),CONVERT(INT,"+str(roundcurr.ROUNDING_METHOD)+")) ,PEREVTCST_INDT_CURR= ROUND(ISNULL(CNTCST,0) * ISNULL((SELECT EXCHANGE_RATE FROM SAQTRV(NOLOCK) WHERE QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"''),1),CONVERT(INT,"+str(roundcurr.DECIMAL_PLACES)+"),CONVERT(INT,"+str(roundcurr.ROUNDING_METHOD)+")) FROM SAQRIT SAQITM(NOLOCK) JOIN(SELECT SAQICO.QTEREV_ID,LINE,SAQICO.QUOTE_ID,SAQICO.SERVICE_ID,SUM(TAXVGC) AS TAXVGC, SUM(TNTVGC) AS TNTVGC,SUM(USRPRC) AS USRPRC,SUM(TAXVDC) AS TAXVDC, SUM(TAMTDC) AS TAMTDC,SUM(TAMTGC) AS TAMTGC, SUM(TNTVDC) AS TNTVDC,SUM(TENVDC) AS TENVDC, SUM(TENVGC) AS TENVGC,SUM(CNTCST) AS CNTCST,BILTYP FROM SAQICO (NOLOCK) WHERE QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND QTEREV_ID = ''"+str(Qt_Id.REVISION_ID)+"''  AND LINE IN(SELECT DISTINCT LINE FROM SAQICO_INBOUND (NOLOCK) WHERE QUOTE_ID = ''"+str(Qt_Id.QUOTE_ID)+"'' AND REVISION_ID = ''"+str(Qt_Id.REVISION_ID)+"'') AND SERVICE_ID IN( ''Z0009'',''Z0010'') AND QTETYP NOT IN (''TOOL BASED'')  GROUP BY SAQICO.QUOTE_ID,SAQICO.SERVICE_ID,SAQICO.QTEREV_ID,SAQICO.LINE,BILTYP) SUB_SAQITM  ON SAQITM.QUOTE_ID = SUB_SAQITM.QUOTE_ID AND SAQITM.SERVICE_ID = SUB_SAQITM.SERVICE_ID  AND  AND SUB_SAQITM.QTEREV_ID = SAQITM.QTEREV_ID  AND SAQITM.LINE = SUB_SAQITM.LINE ' ")
