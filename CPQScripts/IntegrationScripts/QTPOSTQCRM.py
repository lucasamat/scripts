...
try:
	...
	for crmifno in input_data:	
		...
		SAQSAP_BKP = SqlHelper.GetFirst("sp_executesql @T=N'select QUOTE_ID,SERVICE_ID,EQUIPMENT_ID,KIT_NUMBER,KIT_NAME INTO "+str(SAQSAP)+" from SAQSAP(NOLOCK) WHERE QUOTE_ID = ''"+str(QUOTE_ID)+"'' AND QTEREV_ID = ''"+str(REVISION_ID)+"''  ' ")

		SAQTSE_BKP = SqlHelper.GetFirst("sp_executesql @T=N'select LINE_ITEM_ID,QUOTE_ID,MAMTRL.SAP_PART_NUMBER AS SERVICE_ID,CONVERT(VARCHAR(MAX),'''') AS ET_XML INTO "+str(SAQTSE)+" from SAQITM(NOLOCK) JOIN MAMTRL (NOLOCK) ON SAQITM.SERVICE_RECORD_ID = MAMTRL.MATERIAL_RECORD_ID WHERE QUOTE_ID = ''"+str(QUOTE_ID)+"'' AND SAQITM.QTEREV_ID = ''"+str(REVISION_ID)+"'' ' ")
		...
		while Check_flag2 == 1:
			...
			table11 = SqlHelper.GetFirst(
						"SELECT DISTINCT equipment_id FROM (SELECT DISTINCT equipment_id, ROW_NUMBER()OVER(ORDER BY equipment_id) AS SNO FROM "+str(SAQICO)+"  (NOLOCK) where quote_id='"+str(QUOTE_ID)+"' ) A WHERE SNO>= "+str(start2)+" AND SNO<="+str(end2)+""
					)
			...
			table3 = SqlHelper.GetFirst(
				"sp_executesql @T=N'INSERT "+str(CRMTMP)+" SELECT DISTINCT EQUIPMENT_LINE_ID FROM (SELECT DISTINCT EQUIPMENT_LINE_ID, ROW_NUMBER()OVER(ORDER BY EQUIPMENT_LINE_ID) AS SNO FROM "+str(SAQICO)+"  (NOLOCK) where quote_id=''"+str(QUOTE_ID)+"'' ) A WHERE SNO>= "+str(start2)+" AND SNO<="+str(end2)+"  '")
			...
			if str(table11) != "None":
				...
				SAQIEN_SEL = SqlHelper.GetFirst("sp_executesql @T=N'declare @H int; Declare @val Varchar(MAX);DECLARE @XML XML; SELECT @val = FINAL from(select  REPLACE(entitlement_xml,''<QUOTE_ITEM_ENTITLEMENT>'',sml) AS FINAL FROM (select ''<QUOTE_ITEM_ENTITLEMENT><QUOTE_ID>''+quote_id+''</QUOTE_ID><SERVICE_ID>''+service_id+''</SERVICE_ID><EQUIPMENT_ID>''+equipment_id+''</EQUIPMENT_ID><EQUIPMENT_LINE_ID>''+CONVERT(VARCHAR,EQUIPMENT_LINE_ID)+''</EQUIPMENT_LINE_ID>'' AS sml,replace(replace(replace(replace(replace(replace(replace(ENTITLEMENT_XML,''&'','';#38''),'''','';#39''),'' < '','' &lt; '' ),'' > '','' &gt; '' ),''_>'',''_&gt;''),''_<'',''_&lt;''),''&'','';#38'')  as entitlement_xml from SAQIEN(nolock) where quote_id=''"+str(QUOTE_ID)+"'' AND EQUIPMENT_LINE_ID IN (SELECT EQUIPMENT_IDD FROM "+str(CRMTMP)+")  )A )a SELECT @XML = CONVERT(XML,''<ROOT>''+@VAL+''</ROOT>'') exec sys.sp_xml_preparedocument @H output,@XML; INSERT "+str(SAQIEN)+" (QUOTE_ID,EQUIPMENT_ID,SERVICE_ID,EQUIPMENT_LINE_ID,ENTITLEMENT_NAME,ENTITLEMENT_VALUE_CODE)select QUOTE_ID,EQUIPMENT_ID,SERVICE_ID,EQUIPMENT_LINE_ID,ENTITLEMENT_ID,ENTITLEMENT_VALUE_CODE from openxml(@H, ''ROOT/QUOTE_ITEM_ENTITLEMENT'', 0) with (QUOTE_ID VARCHAR(100) ''QUOTE_ID'',EQUIPMENT_ID VARCHAR(100) ''EQUIPMENT_ID'',EQUIPMENT_LINE_ID VARCHAR(100) ''EQUIPMENT_LINE_ID'',SERVICE_ID VARCHAR(100) ''SERVICE_ID'',ENTITLEMENT_VALUE_CODE VARCHAR(100) ''ENTITLEMENT_VALUE_CODE'',ENTITLEMENT_ID VARCHAR(100) ''ENTITLEMENT_ID'') ; exec sys.sp_xml_removedocument @H; '")
				
				Quoteitemquer = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE B SET ET_XML= final_xml FROM (SELECT A.EQUIPMENT_LINE_ID, ISNULL(REPLACE(REPLACE(STUFF((SELECT '' ''+ JSON FROM (SELECT ''<QUOTE_ITEM_ENTITLEMENT>''+''<ITEM_LINE_ID>''+ISNULL(CONVERT(VARCHAR(100),A.EQUIPMENT_LINE_ID),'''')+''</ITEM_LINE_ID>''+''<SERVICE_ID>''+ISNULL(A.SERVICE_ID,'''')+''</SERVICE_ID>''+''<ENTITLEMENT_NAME>''+ISNULL(A.ENTITLEMENT_NAME,'''')+''</ENTITLEMENT_NAME>''+''<ENTITLEMENT_VALUE>''+ISNULL(CONVERT(VARCHAR(100),A.ENTITLEMENT_VALUE_CODE),'''')+''</ENTITLEMENT_VALUE>''+''</QUOTE_ITEM_ENTITLEMENT>'' AS JSON FROM ( 	SELECT B.EQUIPMENT_LINE_ID,B.SERVICE_ID,B.QUOTE_ID, B.ENTITLEMENT_NAME,B.ENTITLEMENT_VALUE_CODE FROM "+str(SAQIEN)+" B(NOLOCK) WHERE A.QUOTE_ID = B.QUOTE_ID AND A.SERVICE_ID = B.SERVICE_ID AND A.EQUIPMENT_LINE_ID = B.EQUIPMENT_LINE_ID AND B.QUOTE_ID = ''"+str(QUOTE_ID)+"'' AND EQUIPMENT_LINE_ID IN (SELECT EQUIPMENT_IDD FROM "+str(CRMTMP)+") )A )A FOR XML PATH ('''')  ), 1, 1,'''' ),''&lt;'',''<''),''&gt;'',''>''),''<QUOTE_ITEM_ENTITLEMENT></QUOTE_ITEM_ENTITLEMENT>'') AS final_xml,A.EQUIPMENT_ID  FROM "+str(SAQICO)+" (NOLOCK) A WHERE A.QUOTE_ID = ''"+str(QUOTE_ID)+"''  AND EQUIPMENT_LINE_ID IN (SELECT EQUIPMENT_IDD FROM "+str(CRMTMP)+") )A JOIN "+str(SAQICO)+" B(NOLOCK) ON A.EQUIPMENT_LINE_ID = B.EQUIPMENT_LINE_ID '")
		...
		while Check_flag1 == 1:
			...
			table1 = SqlHelper.GetFirst(
				"SELECT DISTINCT equipment_id FROM (SELECT DISTINCT equipment_id, ROW_NUMBER()OVER(ORDER BY equipment_id) AS SNO FROM "+str(SAQICO)+"  (NOLOCK) where quote_id='"+str(QUOTE_ID)+"' ) A WHERE SNO>= "+str(start1)+" AND SNO<="+str(end1)+""
			)
			...
			table3 = SqlHelper.GetFirst(
				"sp_executesql @T=N'INSERT "+str(CRMTMP)+" SELECT DISTINCT equipment_id FROM (SELECT DISTINCT equipment_id, ROW_NUMBER()OVER(ORDER BY equipment_id) AS SNO FROM "+str(SAQICO)+"  (NOLOCK) where quote_id=''"+str(QUOTE_ID)+"'' ) A WHERE SNO>= "+str(start1)+" AND SNO<="+str(end1)+"  '")
