...
try:
	...
	for crmifno in input_data:	
		...
		Quoteitemquer = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE B SET ET_XML= FINAL FROM (select  replace(replace(replace(REPLACE(entitlement_xml,''<QUOTE_ITEM_ENTITLEMENT>'',sml),''ENTITLEMENT_VALUE_CODE'',''ENTITLEMENT_VALUE''),''ENTITLEMENT_NAME'',''ENTITLEMENT_NAME1''),''ENTITLEMENT_ID'',''ENTITLEMENT_NAME'')  AS FINAL,EQUIPMENT_LINE_ID from(select distinct ''<QUOTE_ITEM_ENTITLEMENT><ITEM_LINE_ID>''+convert(varchar(100),EQUIPMENT_LINE_ID)+''</ITEM_LINE_ID><SERVICE_ID>''+service_id+''</SERVICE_ID>'' AS sml,replace(entitlement_xml,''&'','';#38'')  as entitlement_xml,EQUIPMENT_LINE_ID from SAQIEN(nolock) where quote_id=''"+str(QUOTE_ID)+"'' AND QTEREV_ID = ''"+str(REVISION_ID)+"'' )A)A JOIN "+str(SAQICO)+" B(NOLOCK) ON A.EQUIPMENT_LINE_ID = B.EQUIPMENT_LINE_ID '")
		...
		while Check_flag1 == 1:
			...
			CRMTMP = SqlHelper.GetFirst("sp_executesql @T=N'IF NOT EXISTS (SELECT ''X'' FROM SYS.OBJECTS WHERE NAME= ''"+str(CRMTMP)+"'' ) BEGIN CREATE TABLE "+str(CRMTMP)+" (EQUIPMENT_IDD VARCHAR(100)) END  ' ")
