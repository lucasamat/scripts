...
try:
	...
	for crmifno in input_data:	
		...
		#Target Qty
		SAQITM_DRP = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET TARGET_QTY= A.QUANTITY,QUANTITY=0  FROM "+str(SAQITM)+" A JOIN SAQICO(NOLOCK) B ON A.QUOTE_ID = B.QUOTE_ID AND A.LINE = B.LINE WHERE B.QUOTE_ID = ''"+str(QUOTE_ID)+"'' AND B.QTEREV_ID = ''"+str(REVISION_ID)+"'' AND A.SERVICE_ID=''Z0009'' AND B.QTETYP=''Event Based'' AND B.BILTYP=''Variable'' ' ")
		...
		SAQICO_BKP = SqlHelper.GetFirst("sp_executesql @T=N'select QUOTE_ID,SERVICE_ID,LINE,EQUIPMENT_ID,SERIAL_NUMBER,NULL AS EXTENDED_PRICE,NULL AS SRVTAXCLA_ID,CONVERT(VARCHAR(100),NULL) AS PAR_SERVICE_ID,QTEREV_ID INTO "+str(SAQICO)+" from SAQRIO(NOLOCK) WHERE QUOTE_ID = ''"+str(QUOTE_ID)+"'' AND QTEREV_ID = ''"+str(REVISION_ID)+"'' UNION select QUOTE_ID,SERVICE_ID,LINE,ASSEMBLY_ID,SERIAL_NUMBER,NULL AS EXTENDED_PRICE,NULL AS SRVTAXCLA_ID,CONVERT(VARCHAR(100),NULL) AS PAR_SERVICE_ID,QTEREV_ID from SAQRIO(NOLOCK) WHERE QUOTE_ID = ''"+str(QUOTE_ID)+"'' AND QTEREV_ID = ''"+str(REVISION_ID)+"'' AND ISNULL(ASSEMBLY_ID,'''')<>'''' ' ")
