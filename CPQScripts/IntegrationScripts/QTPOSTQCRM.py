...
try:
	...
	for crmifno in input_data:	
		...
		SAQITM_DRP = SqlHelper.GetFirst("sp_executesql @T=N'select distinct SAQRIT.QUOTE_ID,SAQRIT.SERVICE_ID,SAQRIT.LINE,SAQRIT.QUANTITY,MAMTRL.UNIT_OF_MEASURE AS UOM_ID,SAQRIT.TOTAL_AMOUNT_INGL_CURR AS EXTENDED_PRICE,'''' AS ITEM_TYPE,'''' AS PO_NUMBER,'''' AS PO_ITEM,'''' AS PO_NOTES,SAQRIT.PLANT_ID,SAQRIT.CONTRACT_VALID_FROM AS LINE_ITEM_FROM_DATE,SAQRIT.CONTRACT_VALID_TO AS LINE_ITEM_TO_DATE,'''' AS INTERNAL_NOTES,SAQRIT.STATUS,SAQRIT.SERVICE_RECORD_ID,SAQRIT.TAXCLASSIFICATION_ID AS SRVTAXCLA_ID,CONVERT(NVARCHAR(MAX),'''') AS BP_XML,CONVERT(NVARCHAR(MAX),'''') AS ET_XML INTO "+str(SAQITM)+" FROM SAQRIT (NOLOCK) JOIN MAMTRL (NOLOCK) ON SAQRIT.SERVICE_ID = MAMTRL.SAP_PART_NUMBER WHERE SAQRIT.QUOTE_ID = ''"+str(QUOTE_ID)+"'' AND SAQRIT.QTEREV_ID = ''"+str(REVISION_ID)+"''  ' ")

		"""SAQITM_DRP = SqlHelper.GetFirst("sp_executesql @T=N'INSERT "+str(SAQITM)+" select distinct SAQITM.QUOTE_ID,SAQIFP.SERVICE_ID,SAQIFP.PART_LINE_ID,SAQITM.QUANTITY,SAQITM.UOM_ID,SAQITM.NET_VALUE AS EXTENDED_PRICE,SAQITM.ITEM_TYPE,SAQITM.PO_NUMBER,SAQITM.PO_ITEM,SAQITM.PO_NOTES,SAQITM.PLANT_ID,SAQITM.LINE_ITEM_FROM_DATE,SAQITM.LINE_ITEM_TO_DATE,SAQITM.INTERNAL_NOTES,SAQITM.ITEM_STATUS,SAQITM.SERVICE_RECORD_ID,SAQITM.SRVTAXCLA_ID  from SAQIFP(NOLOCK) JOIN SAQITM (NOLOCK) ON SAQIFP.QUOTE_ID = SAQITM.QUOTE_ID AND SAQIFP.QTEREV_ID = SAQITM.QTEREV_ID AND SAQIFP.LINE_ITEM_ID = SAQITM.LINE_ITEM_ID WHERE SAQITM.QUOTE_ID = ''"+str(QUOTE_ID)+"'' AND SAQITM.QTEREV_ID = ''"+str(REVISION_ID)+"''  ' ")"""
		...
		"""SAQICO_BKP = SqlHelper.GetFirst("sp_executesql @T=N'INSERT "+str(SAQICO)+" select DISTINCT QUOTE_ID,MAMTRL.SAP_PART_NUMBER AS SERVICE_ID,LINE_ITEM_ID AS ITEM_LINE_ID,PART_LINE_ID,CONVERT(NVARCHAR(MAX),'''') AS BP_XML,CONVERT(NVARCHAR(MAX),'''') AS ET_XML,NULL AS EQUIPMENT_ID,NULL AS  SERIAL_NO,NULL AS EXTENDED_PRICE,NULL AS SRVTAXCLA_ID,CONVERT(VARCHAR(100),NULL) AS PAR_SERVICE_ID  from SAQIFP(NOLOCK) JOIN MAMTRL (NOLOCK) ON SAQIFP.SERVICE_RECORD_ID = MAMTRL.MATERIAL_RECORD_ID WHERE QUOTE_ID = ''"+str(QUOTE_ID)+"'' AND QTEREV_ID = ''"+str(REVISION_ID)+"'' ' ")"""

		SAQIBP_BKP = SqlHelper.GetFirst("sp_executesql @T=N'select QUOTE_ID,MAMTRL.SAP_PART_NUMBER AS SERVICE_ID,LINE ,REPLACE(CONVERT(VARCHAR(11),BILLING_DATE,121),''-'','''') AS BILLING_DATE,BILLING_AMOUNT,EQUIPMENT_ID INTO "+str(SAQIBP)+" from SAQIBP(NOLOCK)  JOIN MAMTRL (NOLOCK) ON SAQIBP.SERVICE_RECORD_ID = MAMTRL.MATERIAL_RECORD_ID WHERE QUOTE_ID = ''"+str(QUOTE_ID)+"''  AND QTEREV_ID = ''"+str(REVISION_ID)+"'' ' ")

		SAQSAP_BKP = SqlHelper.GetFirst("sp_executesql @T=N'select QUOTE_ID,SERVICE_ID,LINE,PART_NUMBER,PART_DESCRIPTION INTO "+str(SAQSAP)+" from SAQRIP(NOLOCK) WHERE QUOTE_ID = ''"+str(QUOTE_ID)+"'' AND QTEREV_ID = ''"+str(REVISION_ID)+"''  ' ")
		...
		QuoteInvolvedPartiesquery = SqlHelper.GetFirst("select replace(replace(STUFF((SELECT  ' '+final_xml FROM (SELECT DISTINCT  '<QUOTE_INVOLVED_PARTY>'+'<QUOTE_ID>"+str(CRMQT.c4c_quote_id)+"</QUOTE_ID><SOLD_TO>'+ ISNULL((SELECT REPLACE(ACCOUNT_ID,'STP-','') FROM SAQTMT(NOLOCK) WHERE QUOTE_ID = '"+str(QUOTE_ID)+"' ),'') + '</SOLD_TO><BILL_TO>'+ISNULL((SELECT TOP 1 PARTY_ID FROM SAQTIP(NOLOCK) WHERE QUOTE_ID = '"+str(QUOTE_ID)+"' AND PARTY_ROLE = 'BILL TO' AND QTEREV_ID = '"+str(REVISION_ID)+"' ),'')+'</BILL_TO><SHIP_TO>'+ISNULL((SELECT TOP 1 PARTY_ID FROM SAQTIP(NOLOCK) WHERE QUOTE_ID = '"+str(QUOTE_ID)+"' AND PARTY_ROLE = 'SHIP TO' AND QTEREV_ID = '"+str(REVISION_ID)+"' ),'')+'</SHIP_TO><PAYER>'+ISNULL((SELECT TOP 1 PARTY_ID FROM SAQTIP(NOLOCK) WHERE QUOTE_ID = '"+str(QUOTE_ID)+"' AND QTEREV_ID = '"+str(REVISION_ID)+"' AND PARTY_ROLE = 'PAYER' ),'')+'</PAYER><GPMRPM>'+ISNULL((SELECT TOP 1 PARTY_ID FROM SAQTIP(NOLOCK) WHERE QUOTE_ID = '"+str(QUOTE_ID)+"' AND PARTY_ROLE = 'GPMRPM' AND QTEREV_ID = '"+str(REVISION_ID)+"' ),'')+'</GPMRPM><CONT_MNGR>'+ISNULL((SELECT TOP 1 CONVERT(VARCHAR(10),CRM_EMPLOYEE_ID) FROM SAQDLT(NOLOCK)A JOIN SAEMPL (NOLOCK) ON MEMBER_ID = EMPLOYEE_ID WHERE QUOTE_ID = '"+str(QUOTE_ID)+"' AND C4C_PARTNERFUNCTION_ID = 'CONTRACT MANAGER' AND QTEREV_ID = '"+str(REVISION_ID)+"' ),'')+'</CONT_MNGR><SALES_PERSON>'+ISNULL((SELECT TOP 1 CONVERT(VARCHAR(10),CRM_EMPLOYEE_ID) FROM SAQDLT(NOLOCK)A JOIN SAEMPL (NOLOCK) ON MEMBER_ID = EMPLOYEE_ID WHERE QUOTE_ID = '"+str(QUOTE_ID)+"' AND PARTNERFUNCTION_ID = 'ER' AND QTEREV_ID = '"+str(REVISION_ID)+"' ),'')+'</SALES_PERSON>' AS final_xml  FROM SAQTIP(NOLOCK) WHERE QUOTE_ID = '"+str(QUOTE_ID)+"' AND QTEREV_ID = '"+str(REVISION_ID)+"')A FOR XML PATH ('')  ), 1, 1, ''),'&lt;','<'),'&gt;','>')AS RESULT")
		
		if str(QuoteInvolvedPartiesquery.RESULT) != "" :
			QuoteInvolvedPartiesquery = str(QuoteInvolvedPartiesquery.RESULT)
			
			QuoteInvolvedPartiesfabquery = SqlHelper.GetFirst("select replace(replace(STUFF((SELECT  ' '+final_xml FROM (SELECT  DISTINCT '<FAB>'+CASE WHEN ISNULl(CRM_FABLOCATION_ID,'')<>'' THEN CRM_FABLOCATION_ID ELSE ISNULL(SAQFBL.FABLOCATION_ID,'') END+'</FAB>' AS final_xml  FROM SAQSFB SAQFBL(NOLOCK) JOIN MAFBLC (NOLOCK) ON SAQFBL.FABLOCATION_ID = MAFBLC.FAB_LOCATION_ID WHERE QUOTE_ID  = '"+str(QUOTE_ID)+"' AND QTEREV_ID = '"+str(REVISION_ID)+"')SUB_SAQFBL FOR XML PATH ('')  ), 1, 1, ''),'&lt;','<'),'&gt;','>')AS RESULT")
