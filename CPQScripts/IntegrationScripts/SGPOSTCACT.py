...
try:
    ...
    UpdateQueryItems = SqlHelper.GetFirst(
    "sp_executesql @T=N'INSERT SABLBK(BLUEBOOK,DESCRIPTION,BLUEBOOK_RECORD_ID)  SELECT AGS_BLUEBOOK,AGS_BLUEBOOK AS DESC,CONVERT(VARCHAR(100),NEWID()) FROM (SELECT AGS_BLUEBOOK FROM SAACNT_INBOUND(NOLOCK) WHERE ISNULL(AGS_BLUEBOOK,'')<>'')A LEFT JOIN SABLBK B ON A.AGS_BLUEBOOK = B.BLUEBOOK WHERE BLUEBOOK IS NULL  ' ")   
