...
try:
    ...
    + "  SAACNT_INBOUND SET INTEGRATION_STATUS = ''DUPLICATE'' WHERE CpqTableEntryId NOT IN (SELECT MAX(CpqTableEntryId) FROM SAACNT_INBOUND WHERE ISNULL(INTEGRATION_STATUS,'''')='''' AND ISNULL(INTEGRATION_OBJECT,'''')='''' GROUP BY Account_id HAVING COUNT(ACCOUNT_ID)>1) AND ISNULL(INTEGRATION_STATUS,'''')='''' AND ISNULL(INTEGRATION_OBJECT,'''')=''''  ' ")
    ...
    ""
    + str(Parameter1.QUERY_CRITERIA_1)
    + "  SAACNT_INBOUND SET INTEGRATION_STATUS = ''READY FOR UPLOAD'' WHERE ISNULL(INTEGRATION_STATUS,'''')=''''  ' ")
    ...
    InsertQueryItems = SqlHelper.GetFirst(
        ...
        + " SAACNT (ACCOUNT_ID,ACCOUNT_NAME,ACTIVE,ADDRESS_1,CITY,COUNTRY,COUNTRY_RECORD_ID,STATE,STATE_RECORD_ID,POSTAL_CODE,ACCOUNTGROUP_ID,ACCOUNTGROUP_NAME,ACCOUNTGROUP_RECORD_ID,TAXJUR_CODE,ACCOUNT_RECORD_ID,CPQTABLEENTRYADDEDBY,CPQTABLEENTRYDATEADDED)SELECT SUB_SAACNT.*,CONVERT(VARCHAR(4000),NEWID()),''"
        + str(User.UserName)
        + "'',GETDATE() FROM (SELECT DISTINCT SAACNT_INBOUND.ACCOUNT_ID,SAACNT_INBOUND.ACCOUNT_NAME,''True'' AS ACTIVE,SAACNT_INBOUND.ADDRESS_1,SAACNT_INBOUND.CITY,SAACNT_INBOUND.COUNTRY,SACTRY.COUNTRY_RECORD_ID,SAACNT_INBOUND.STATE,SACYST.STATE_RECORD_ID,SAACNT_INBOUND.POSTAL_CODE,SAACNT_INBOUND.ACCOUNTGROUP_ID,SAAGRP.ACCOUNT_GROUP_NAME,SAAGRP.ACCOUNT_GROUP_RECORD_ID,SAACNT_INBOUND.TAX_CODE FROM SAACNT_INBOUND(NOLOCK)  JOIN SACTRY(NOLOCK) on SAACNT_INBOUND.COUNTRY=SACTRY.COUNTRY LEFT JOIN SACYST(NOLOCK) on SAACNT_INBOUND.STATE=SACYST.STATE AND SAACNT_INBOUND.COUNTRY=SACYST.COUNTRY LEFT JOIN  SAAGRP on SAACNT_INBOUND.ACCOUNTGROUP_ID = SAAGRP.ACCOUNT_GROUP_ID WHERE SAACNT_INBOUND.INTEGRATION_STATUS = ''READY FOR UPLOAD'' AND ISNULL(SAACNT_INBOUND.INTEGRATION_OBJECT,'''')=''''  )SUB_SAACNT LEFT JOIN SAACNT(NOLOCK) ON SUB_SAACNT.ACCOUNT_ID = SAACNT.ACCOUNT_ID WHERE SAACNT.ACCOUNT_ID IS NULL '")
