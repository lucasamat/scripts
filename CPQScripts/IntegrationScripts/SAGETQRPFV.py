...
try:
    if 'Param' in globals():    
        if hasattr(Param, 'CPQ_Columns'): 
            ...
            Tools = SqlHelper.GetFirst("SELECT CASE WHEN ISNULL(RESULT,'') = '' THEN '' ELSE RESULT END AS RESULT FROM (SELECT replace ('\"Tools\": ['+STUFF((SELECT ','+ JSON FROM (SELECT DISTINCT '{\"ToolId\" : \"'+ToolId+'\",\"ToolDescription\" : \"'+ToolDescription+'\",\"SerialID\" : \"'+SerialID+'\",\"CustomerToolID\" : \"'+CustomerToolID+'\",\"FabName\" : \"'+FabName+'\",\"GreenBook\" : \"'+GreenBook+'\",\"PLATFORM\" : \"'+PLATFORM+'\"}' AS JSON from (SELECT DISTINCT ISNULL(SAQICO.EQUIPMENT_ID,'') AS ToolId,ISNULL(EQUIPMENT_DESCRIPTION,'') AS ToolDescription, ISNULL(SERIAL_NO,'') AS SerialID, ISNULL(CUSTOMER_TOOL_ID,'') AS CustomerToolID,  ISNULL(FABLOCATION_NAME,'') AS FabName, ISNULL(GREENBOOK,'') AS GreenBook,ISNULL(PLATFORM,'') AS Platform FROM SAQICO (NOLOCK)  WHERE SAQICO.QUOTE_ID ='"+ str(Quote_ID) +"' AND SAQICO.QTEREV_ID ='"+ str(Revision_ID) + "'  ) t    ) A FOR XML PATH ('')  ), 1, 1, '')+']','amp;#','#') AS RESULT)A " )


            ToolWorksheet = SqlHelper.GetFirst("SELECT CASE WHEN ISNULL(RESULT,'') = '' THEN '' ELSE RESULT END AS RESULT FROM (SELECT replace ('\"ToolWorksheet\": ['+STUFF((SELECT ','+ JSON FROM (SELECT DISTINCT '{\"ToolId\" : \"'+ToolId+'\",\"ServiceID\" : \"'+ServiceID+'\",\"ToolDescription\" : \"'+ToolDescription+'\",\"REgisteredProdctCatogory\" : \"'+REgisteredProdctCatogory+'\",\"Platform\" : \"'+Platform+'\",\"Greenbook\" : \"'+Greenbook+'\",\"Technology\" : \"'+Technology+'\",\"ProductOfferingName\" : \"'+ProductOfferingName+'\"}' AS JSON from (SELECT DISTINCT ISNULL(SAQICO.EQUIPMENT_ID,'') AS ToolId,ISNULL(SAQICO.SERVICE_ID,'') AS ServiceID,ISNULL(EQUIPMENT_DESCRIPTION,'') AS  ToolDescription ,ISNULL(EQUIPMENTCATEGORY_ID,'') AS REgisteredProdctCatogory, ISNULL(PLATFORM,'') AS Platform,ISNULL(GREENBOOK,'') AS Greenbook,ISNULL(TECHNOLOGY,'') AS Technology,ISNULL(SERVICE_DESCRIPTION,'') AS ProductOfferingName FROM SAQICO (NOLOCK)  WHERE SAQICO.QUOTE_ID ='"+ str(Quote_ID) +"' AND SAQICO.QTEREV_ID ='"+ str(Revision_ID) + "'  ) t    ) A FOR XML PATH ('')  ), 1, 1, '')+']','amp;#','#') AS RESULT)A " )
