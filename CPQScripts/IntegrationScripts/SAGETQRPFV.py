...
try:
    if 'Param' in globals():    
        if hasattr(Param, 'CPQ_Columns'): 
            ...
            BookingForcast = SqlHelper.GetFirst("SELECT CASE WHEN ISNULL(RESULT,'') = '' THEN '' ELSE RESULT END AS RESULT FROM (SELECT replace ('\"BookingForecast\": ['+STUFF((SELECT ','+ JSON FROM (SELECT DISTINCT '{\"ServiceID\" : \"'+ServiceID+'\",\"ForcastValue\" : \"'+ForcastValue+'\",\"ServiceDescription\" : \"'+ServiceDescription+'\"}' AS JSON from (SELECT DISTINCT ISNULL(SAQRIS.SERVICE_ID,'') AS ServiceID,CONVERT(VARCHAR,ISNULL(SAQRIS.NET_VALUE,0)) AS ForcastValue,ISNULL(SERVICE_DESCRIPTION,'') AS ServiceDescription FROM SAQRIS (NOLOCK)  WHERE SAQRIS.QUOTE_ID ='"+ str(Quote_ID) +"' AND SAQRIS.QTEREV_ID ='"+ str(Revision_ID) + "' ) t    ) A FOR XML PATH ('')  ), 1, 1, '')+']','amp;#','#') AS RESULT)A " )
