...
try:
    if 'Param' in globals():    
        if hasattr(Param, 'CPQ_Columns'): 
            ...
            ToolWorksheet = SqlHelper.GetFirst("SELECT CASE WHEN ISNULL(RESULT,'') = '' THEN '' ELSE RESULT END AS RESULT FROM (SELECT replace ('\"ToolWorksheet\": ['+STUFF((SELECT ','+ JSON FROM (SELECT DISTINCT '{\"ToolId\" : \"'+ToolId+'\",\"ServiceID\" : \"'+ServiceID+'\",\"ToolDescription\" : \"'+ToolDescription+'\",\"REgisteredProdctCatogory\" : \"'+REgisteredProdctCatogory+'\",\"DateFrom\" : \"'+DateFrom+'\",\"DateTo\" : \"'+DateTo+'\",\"Platform\" : \"'+Platform+'\",\"Greenbook\" : \"'+Greenbook+'\",\"Technology\" : \"'+Technology+'\",\"Select\" : \"'+Flag+'\",\"ProductOfferingName\" : \"'+ProductOfferingName+'\"}' AS JSON from (SELECT DISTINCT ISNULL(SAQICO.EQUIPMENT_ID,'') AS ToolId,ISNULL(SAQICO.SERVICE_ID,'') AS ServiceID,ISNULL(EQUIPMENT_DESCRIPTION,'') AS  ToolDescription ,ISNULL(EQUIPMENTCATEGORY_ID,'') AS REgisteredProdctCatogory, ISNULL(PLTFRM,'') AS Platform,ISNULL(GREENBOOK,'') AS Greenbook,ISNULL(TECHNOLOGY,'') AS Technology,ISNULL(SERVICE_DESCRIPTION,'') AS ProductOfferingName,ISNULL(CONVERT(VARCHAR(11),SAQICO.CONTRACT_VALID_FROM,101),'') AS DateFrom,ISNULL(CONVERT(VARCHAR(11),SAQICO.CONTRACT_VALID_TO,101),'') AS DateTo ,'True' as Flag FROM SAQICO (NOLOCK)  WHERE SAQICO.QUOTE_ID ='"+ str(Quote_ID) +"' AND SAQICO.QTEREV_ID ='"+ str(Revision_ID) + "' AND ISNULL(EQUIPMENT_ID,'')<>'' ) t    ) A FOR XML PATH ('')  ), 1, 1, '')+']','amp;#','#') AS RESULT)A " )
            
            ForcastWorksheet = SqlHelper.GetFirst("SELECT CASE WHEN ISNULL(RESULT,'') = '' THEN '' ELSE RESULT END AS RESULT FROM (SELECT replace ('\"ForcastWorksheet\": ['+STUFF((SELECT ','+ JSON FROM (SELECT DISTINCT '{\"GreenBook\" : \"'+GreenBook+'\",\"ServiceID\" : \"'+ServiceID+'\",\"ForcastValue\" : \"'+ForcastValue+'\",\"ProductOfferingName\" : \"'+ProductOfferingName+'\"}' AS JSON from (SELECT DISTINCT ISNULL(SAQIGS.GREENBOOK,'') AS GreenBook,ISNULL(SAQIGS.SERVICE_ID,'') AS ServiceID, ISNULL(CONVERT(VARCHAR,SAQIGS.NET_VALUE_INGL_CURR),'') AS ForcastValue,ISNULL(SERVICE_DESCRIPTION,'') AS ProductOfferingName FROM SAQIGS (NOLOCK)  WHERE SAQIGS.QUOTE_ID ='"+ str(Quote_ID) +"' AND SAQIGS.QTEREV_ID ='"+ str(Revision_ID) + "'  ) t    ) A FOR XML PATH ('')  ), 1, 1, '')+']','amp;#','#') AS RESULT)A " )
            
            BookingForcast = SqlHelper.GetFirst("SELECT CASE WHEN ISNULL(RESULT,'') = '' THEN '' ELSE RESULT END AS RESULT FROM (SELECT replace ('\"BookingForecast\": ['+STUFF((SELECT ','+ JSON FROM (SELECT DISTINCT '{\"ServiceID\" : \"'+ServiceID+'\",\"ForcastValue\" : \"'+ForcastValue+'\",\"ServiceDescription\" : \"'+ServiceDescription+'\"}' AS JSON from (SELECT DISTINCT ISNULL(SAQRIS.SERVICE_ID,'') AS ServiceID, ISNULL(CONVERT(VARCHAR,SAQRIS.NET_VALUE_INGL_CURR),'')  AS ForcastValue,ISNULL(SERVICE_DESCRIPTION,'') AS ServiceDescription FROM SAQRIS (NOLOCK)  WHERE SAQRIS.QUOTE_ID ='"+ str(Quote_ID) +"' AND SAQRIS.QTEREV_ID ='"+ str(Revision_ID) + "' ) t    ) A FOR XML PATH ('')  ), 1, 1, '')+']','amp;#','#') AS RESULT)A " )
