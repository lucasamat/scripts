...
try :
	...
	if status_flag== 1 :  
        ...
        + " SACONT (CITY,CONTACT_ID,CONTACT_NAME,COUNTRY,COUNTRY_RECORD_ID,ADDRESS,PHONE,POSTAL_CODE,EMAIL,FAX,STATE,STATE_RECORD_ID,CPQTABLEENTRYDATEADDED,CONTACT_RECORD_ID)SELECT SUB_SACONT.*,GetDate() ,CONVERT(VARCHAR(4000),NEWID()) FROM (SELECT DISTINCT SAACNT_INBOUND.CITY,SAACNT_INBOUND.CONTACT_ID,(FIRSTNAME+'' ''+LASTNAME) AS CONTACT_NAME,SAACNT.COUNTRY,SAACNT.COUNTRY_RECORD_ID,SAACNT.ADDRESS_1,SAACNT_INBOUND.PHONE,SAACNT.POSTAL_CODE,SAACNT_INBOUND.EMAIL,SAACNT_INBOUND.FAX,SAACNT.STATE,SAACNT.STATE_RECORD_ID FROM SAACNT_INBOUND(NOLOCK) LEFT JOIN SAACNT(NOLOCK) ON SAACNT_INBOUND.ACCOUNT_ID=SAACNT.ACCOUNT_ID WHERE  SAACNT_INBOUND.INTEGRATION_STATUS = ''READY FOR UPLOAD'' AND ISNULL(SAACNT_INBOUND.INTEGRATION_OBJECT,'''')=''SAACCT''  )SUB_SACONT LEFT JOIN SACONT(NOLOCK)  ON SUB_SACONT.CONTACT_ID=SACONT.CONTACT_ID WHERE  SACONT.CONTACT_ID IS NULL'"
        )
