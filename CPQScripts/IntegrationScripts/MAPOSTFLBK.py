...
try:
	...
	+ str(User.Id)
	+ "'',MAFBLC.CpqTableEntryDateModified = GetDate(),MAFBLC.FABCATEGORY_ID = MAEQCT.EQUIPMENTCATEGORY_ID,MAFBLC.FABCATEGORY_NAME = MAEQCT.EQUIPMENTCATEGORY_DESCRIPTION,MAFBLC.FABCATEGORY_RECORD_ID = MAEQCT.EQUIPMENT_CATEGORY_RECORD_ID,MAFBLC.MNT_PLANT_ID = MAPLNT.PLANT_ID,MAFBLC.MNT_PLANT_NAME = MAPLNT.PLANT_NAME,MAFBLC.MNT_PLANT_RECORD_ID = MAPLNT.PLANT_RECORD_ID,FAB_BLUEBOOK= MAFBLC_INBOUND.BLUEBOOK,FAB_LOCATION_NAME = MAFBLC_INBOUND.FAB_LOCATION_NAME,ACCOUNT_ID = SAACNT.ACCOUNT_ID,ACCOUNT_NAME = SAACNT.ACCOUNT_NAME,ACCOUNT_RECORD_ID = SAACNT.ACCOUNT_RECORD_ID,CITY = MAFBLC_INBOUND.CITY,ADDRESS_1 = MAFBLC_INBOUND.ADDRESS_1,COUNTRY = MAFBLC_INBOUND.COUNTRY,COUNTRY_RECORD_ID = SACTRY.COUNTRY_RECORD_ID,POSTAL_CODE= MAFBLC_INBOUND.POSTAL_CODE,AGS_FAB_NAME = AGS_SEMI_FAB FROM MAFBLC_INBOUND(NOLOCK)  JOIN MAFBLC (NOLOCK) ON MAFBLC_INBOUND.FAB_LOCATION_ID = MAFBLC.FAB_LOCATION_ID JOIN MAPLNT (NOLOCK) ON MAFBLC_INBOUND.MNT_PLANT_ID = MAPLNT.PLANT_ID JOIN MAEQCT (NOLOCK) ON MAFBLC_INBOUND.FABCATEGORY_ID = MAEQCT.EQUIPMENTCATEGORY_ID LEFT JOIN SAACNT SAACNT(NOLOCK) ON MAFBLC_INBOUND.ACCOUNT_ID = SAACNT.ACCOUNT_ID LEFT JOIN SACTRY (NOLOCK) ON MAFBLC_INBOUND.COUNTRY = SACTRY.COUNTRY WHERE  MAFBLC_INBOUND.TIMESTAMP='"+ str(timestamp_sessionid)+ "'  AND ISNULL(MAFBLC_INBOUND.INTEGRATION_STATUS,'''') ='''' AND ISNULL(PROCESS_STATUS,'''')=''READY FOR UPLOAD'' '"	)
