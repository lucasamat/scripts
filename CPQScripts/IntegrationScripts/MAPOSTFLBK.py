...
try:
	...
	primaryQueryItems = SqlHelper.GetFirst(
		...
		+ "   ERROR_SESSION (SESSION_ID,CpqTableEntryDateModified) SELECT SUB_ERRLOG,getdate() FROM (SELECT DISTINCT MAFBLC_INBOUND.SESSION_ID as SESSION_ID FROM MAFBLC_INBOUND (NOLOCK) WHERE  ISNULL(MAFBLC_INBOUND.PROCESS_STATUS,'''')=''ERROR'' )SUB_ERRLOG  ' "
	)
	...
	primaryQueryItems = SqlHelper.GetFirst(
		""
		+ str(Parameter1.QUERY_CRITERIA_1)
		+ "  MAFBLC_INBOUND SET PROCESS_STATUS=''ERROR'' FROM MAFBLC_INBOUND JOIN ERROR_SESSION  ON MAFBLC_INBOUND.SESSION_ID = ERROR_SESSION.SESSION_ID WHERE TIMESTAMP='"
		+ str(timestamp_sessionid)
		+ "' '"
	)
	...
	+ str(User.Id)
	+ "'',MAFBLC.CpqTableEntryDateModified = GetDate(),MAFBLC.FABCATEGORY_ID = MAEQCT.EQUIPMENTCATEGORY_ID,MAFBLC.FABCATEGORY_NAME = MAEQCT.EQUIPMENTCATEGORY_DESCRIPTION,MAFBLC.FABCATEGORY_RECORD_ID = MAEQCT.EQUIPMENT_CATEGORY_RECORD_ID,MAFBLC.MNT_PLANT_ID = MAPLNT.PLANT_ID,MAFBLC.MNT_PLANT_NAME = MAPLNT.PLANT_NAME,MAFBLC.MNT_PLANT_RECORD_ID = MAPLNT.PLANT_RECORD_ID,BLUEBOOK= MAFBLC_INBOUND.BLUEBOOK,BLUEBOOK_RECORD_ID=SAAGRP.ACCOUNT_GROUP_RECORD_ID,FAB_LOCATION_NAME = MAFBLC_INBOUND.FAB_LOCATION_NAME,ACCOUNT_ID = SAACNT.ACCOUNT_ID,ACCOUNT_NAME = SAACNT.ACCOUNT_NAME,ACCOUNT_RECORD_ID = SAACNT.ACCOUNT_RECORD_ID FROM MAFBLC_INBOUND(NOLOCK)  JOIN MAFBLC (NOLOCK) ON MAFBLC_INBOUND.FAB_LOCATION_ID = MAFBLC.FAB_LOCATION_ID JOIN MAPLNT (NOLOCK) ON MAFBLC_INBOUND.MNT_PLANT_ID = MAPLNT.PLANT_ID JOIN MAEQCT (NOLOCK) ON MAFBLC_INBOUND.FABCATEGORY_ID = MAEQCT.EQUIPMENTCATEGORY_ID LEFT JOIN SAAGRP SAAGRP(NOLOCK) ON MAFBLC_INBOUND.BLUEBOOK = SAAGRP.ACCOUNT_GROUP_ID JOIN SAACNT SAACNT(NOLOCK) ON MAFBLC_INBOUND.ACCOUNT_ID = SAACNT.ACCOUNT_ID  WHERE  MAFBLC_INBOUND.TIMESTAMP='"+ str(timestamp_sessionid)+ "'  AND ISNULL(MAFBLC_INBOUND.INTEGRATION_STATUS,'''') ='''' AND ISNULL(PROCESS_STATUS,'''')=''READY FOR UPLOAD'' '"	)
