...
try:
	if 'Param' in globals(): 	
		if hasattr(Param, 'CPQ_Columns'): 
			...
			primaryQueryItems = SqlHelper.GetFirst( ""+ str(Parameter.QUERY_CRITERIA_1)+ " SYINPL (INTEGRATION_PAYLOAD,SESSION_ID,INTEGRATION_NAME)  select ''"+str(Final_data)+ "'','"+ str(timestamp_sessionid)+ "',''SSCM_TO_CPQ_TOOL_PMKIT_DATA'' ' ")
			...
			if LOGIN_CREDENTIALS is not None:
				...
				#LOGIN_CRE = SqlHelper.GetFirst("SELECT URL FROM SYCONF where EXTERNAL_TABLE_NAME ='SSCM_TO_CPQ_TOOL_PMKIT_ASYNC'")
