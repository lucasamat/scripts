...
"""
	...
	+ " MAEAPK (APPLICATION,ASSEMBLY_DESCRIPTION,ASSEMBLY_ID,ASSEMBLY_RECORD_ID,REGION,REGION_RECORD_ID,CUSTOMER_MARKETING_NAME,DEVICE_TYPE,EQUIPMENT_DESCRIPTION,EQUIPMENT_ID,EQUIPMENT_RECORD_ID	,KIT_ID,KIT_NAME,KIT_NUMBER,KIT_NUMBER_RECORD_ID,KIT_RECORD_ID,LOGISTICS_COST,PM_FREQUENCY,PM_ID,PM_NAME,PM_RECORD_ID,PROCESS_TYPE,SEEDSTOCK_COST,SERVICE_COMPLEXITY,TECHNOLOGY_NODE,HARDWARE_TYPE,TKM_COST_PER_EVENT,CPQTABLEENTRYADDEDBY,ADDUSR_RECORD_ID,CPQTABLEENTRYDATEADDED,EQUIPMENT_ASSEMBLY_PM_KIT_RECORD_ID) SELECT SUB_MAEAPK.*,''"+ str(User.UserName)
	+ "'',''"+ str(User.Id)
	+ "'',GETDATE(),CONVERT(VARCHAR(1000),NEWID())  FROM (SELECT DISTINCT MAEAPM_INBOUND.APPLICATION,MAEQUP.EQUIPMENT_DESCRIPTION AS ASSEMBLY_DESCRIPTION,MAEQUP.EQUIPMENT_ID AS ASSEMBLY_ID,MAEQUP.EQUIPMENT_RECORD_ID AS ASSEMBLY_RECORD_ID,SAREGN.REGION,SAREGN.REGION_RECORD_ID,MAEAPM_INBOUND.CUSTOMER_MARKETING_NAME,MAEAPM_INBOUND.DEVICE,MAEQUP.PAR_EQUIPMENT_DESCRIPTION AS EQUIPMENT_DESCRIPTION,MAEQUP.PAR_EQUIPMENT_ID AS EQUIPMENT_ID,MAEQUP.PAR_EQUIPMENT_RECORD_ID AS EQUIPMENT_RECORD_ID,MAMKIT.KIT_ID,MAMKIT.KIT_NAME,NULL AS KIT_NUMBER,NULL AS KIT_NUMBER_RECORD_ID,MAMKIT.KIT_RECORD_ID,CONVERT(FLOAT,MAEAPM_INBOUND.LOGISTICS_COST) AS LOGISTICS_COST,CONVERT(FLOAT,MAEAPM_INBOUND.PM_FREQUENCY) AS PM_FREQUENCY,SGPMNT.PM_ID,SGPMNT.PM_NAME,SGPMNT.PM_RECORD_ID,MAEAPM_INBOUND.PROCESS_TYPE,CONVERT(FLOAT,MAEAPM_INBOUND.SEEDSTOCK_COST) AS SEEDSTOCK_COST,MAEAPM_INBOUND.SERVICE_COMPLEXITY,MAEAPM_INBOUND.TECHNOLOGY_NODE,HW_TYPE,CONVERT(FLOAT,MAEAPM_INBOUND.TKM_COST_PER_EVENT) AS TKM_COST_PER_EVENT FROM MAEAPM_INBOUND (NOLOCK) JOIN MAEQUP (NOLOCK) ON MAEAPM_INBOUND.ASSEMBLY_ID = MAEQUP.EQUIPMENT_ID JOIN MAMKIT (NOLOCK) ON MAEAPM_INBOUND.KIT_master_ID = MAMKIT.KIT_ID JOIN MAMKIT E(NOLOCK) ON MAEAPM_INBOUND.KIT_master_ID = MAMKIT.KIT_ID JOIN SGPMNT (NOLOCK) ON MAEAPM_INBOUND.PM_NAME = SGPMNT.PM_NAME LEFT JOIN SAREGN (NOLOCK) ON MAEAPM_INBOUND.REGION = SAREGN.REGION WHERE ISNULL(PROCESS_STATUS,'''')= ''READY FOR UPLOAD'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' AND ISNULL(INTEGRATION_STATUS,'''')='''' AND ISNULL(MAEAPM_INBOUND.KIT_NUMBER,''0'')=''0'' )SUB_MAEAPK LEFT JOIN MAEAPK (NOLOCK) ON SUB_MAEAPK.EQUIPMENT_ID = MAEAPK.EQUIPMENT_ID AND SUB_MAEAPK.ASSEMBLY_ID = MAEAPK.ASSEMBLY_ID AND SUB_MAEAPK.KIT_ID = MAEAPK.KIT_ID AND SUB_MAEAPK.PM_ID = MAEAPK.PM_ID WHERE MAEAPK.EQUIPMENT_ID IS NULL   '")
