...
try :
	...
	+ "'',''"+ str(User.Id)
	+ "'',GETDATE(),CONVERT(VARCHAR(1000),NEWID()),KIT_ID FROM (SELECT DISTINCT MAKTPT_INBOUND.KIT_ID AS KIT_NAME,MAKTPT_INBOUND.KIT_ID FROM MAKTPT_INBOUND (NOLOCK)  LEFT JOIN MAMKIT (NOLOCK) ON MAKTPT_INBOUND.KIT_ID = MAMKIT.KIT_ID WHERE ISNULL(PROCESS_STATUS,'''')= (''READY FOR UPLOAD'') AND TIMESTAMP = '"+str(timestamp_sessionid)+"' AND MAMKIT.KIT_ID IS NULL)SUB_MAMKIT '")
...
"""
	...
	+ " MAMKIT(KIT_NAME,CPQTABLEENTRYADDEDBY,ADDUSR_RECORD_ID,CPQTABLEENTRYDATEADDED,KIT_RECORD_ID,KIT_ID)SELECT DISTINCT KIT_NAME,''"+ str(User.UserName)
	+ "'',''"+ str(User.Id)
	+ "'',GETDATE(),CONVERT(VARCHAR(1000),NEWID()),KIT_ID FROM (SELECT DISTINCT MAEAPM_INBOUND.KIT_ID AS KIT_NAME,KIT_ID FROM MAEAPM_INBOUND (NOLOCK) LEFT JOIN MAMKIT (NOLOCK) ON MAEAPM_INBOUND.KIT_ID = MAMKIT.KIT_ID WHERE ISNULL(PROCESS_STATUS,'''')= ''READY FOR UPLOAD'' AND TIMESTAMP = '"+str(timestamp_sessionid)+"' AND MAMKIT.KIT_NAME IS NULL AND ISNULL(KIT_ID,''0'')<>''0'')SUB_MAMKIT '")
