	...
	for crmifno in input_data:	
		...
		while Check_flag1 == 1:
			...
			if str(table1) != "None":
				...
				primaryQueryItems = SqlHelper.GetFirst( ""+ str(Parameter.QUERY_CRITERIA_1)+ " SYINPL (INTEGRATION_PAYLOAD,INTEGRATION_KEY,CpqTableEntryDateModified,INTEGRATION_NAME)  select ''"+str(Final_xml)+ "'',''"+ str(QUOTE_ID)+ "'',getdate(),''CPQ to ECC Quote Replication Schedule'' ' ")
				...
				SAQIBP_UPD = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE SAQRDS SET ECC_FLAG = ''TRUE'' WHERE QUOTE_ID = ''"+str(QUOTE_ID)+"'' AND QTEREV_ID = ''"+str(REVISION_ID)+"'' AND CONVERT(VARCHAR,DELIVERY_PERIOD)  = ''"+str(table1.SCHEDULE_NO)+"''  ' ")
