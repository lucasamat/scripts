			...
			if 'ErrorCode' in rebuilt_data['CPQ_Columns']:
				...
				if 'CorrelationID' in rebuilt_data['CPQ_Columns']:
					...
					primaryQueryItems = SqlHelper.GetFirst(
						""
						+ str(Parameter1.QUERY_CRITERIA_1)
						+ "  SAQTRV SET REVISION=''OPD-LEGAL SOW CREATED'' FROM SAQTRV (NOLOCK) WHERE SAQTRV.QUOTE_ID +''-''+CONVERT(VARCHAR,SAQTRV.QTEREV_ID) = ''"+ str(Qt_Id)+"'' AND ISNULL(CLM_AGREEMENT_STATUS,'''')=''DRAFT'' ' "
					)

					primaryQueryItems = SqlHelper.GetFirst(
						""
						+ str(Parameter1.QUERY_CRITERIA_1)
						+ "  SAQTRV SET REVISION=''OPD-LEGAL SOW ACCEPTED'' FROM SAQTRV (NOLOCK) WHERE SAQTRV.QUOTE_ID +''-''+CONVERT(VARCHAR,SAQTRV.QTEREV_ID) = ''"+ str(Qt_Id)+"'' AND ISNULL(CLM_AGREEMENT_STATUS,'''')=''ACCEPTED'' ' "
					)
					...
					# Set message subject and body
					msg.Subject = "CLM Agrement Created - AMAT CPQ(X-Tenant)"
					msg.IsBodyHtml = True
					msg.Body = Error_Info
