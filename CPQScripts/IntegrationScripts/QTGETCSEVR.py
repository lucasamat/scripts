...
try:
	...
	S = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET STATUS = ''CFG-ACQUIRING'' FROM SAQTRV (NOLOCK)A JOIN TOOL_VERQTE_INBOUND (NOLOCK)B ON A.QUOTE_ID = B.QUOTE_ID AND A.QTEREV_ID = B.QTEREV_ID WHERE REVISION_STATUS IN (''CFG-CONFIGURING'',''CFG-ACQUIRING'',''CFG-ON HOLD - COSTING'',''PRR-ON HOLD PRICING'',''PRI-PRICING''  )  '")
