...
class SYLDRTLIST:
	def MDYNMICSQLOBJECT(self, RECORD_ID, PerPage, PageInform, SubTab, PR_CURR, TP, equipment_id,line_item):         
		...
		if obj_obj is not None:
			...
			try:
				Trace.Write("test352")
				#START  -A055S000P01- 20873-M
				if (str(getRevision.REVISION_STATUS).upper() in ("CBC-CBC COMPLETED","BOK-CONTRACT CREATED","BOK-CONTRACT BOOKED","LGL-LEGAL SOW ACCEPTED","LGL-LEGAL SOW REJECTED","LGL-PREPARING LEGAL SOW","CBC-PREPARING CBC")) or (str(getRevision.REVISION_STATUS).upper() in ("CFG-ACQUIRING","PRR-ON HOLD PRICING","CFG-ON HOLD - COSTING","PRI-PRICING","PRR-PRICING REVIEWED","PRR-RECALCULATING") and str(TreeParam).upper() not in ("QUOTE ITEMS","BILLING","APPROVALS","QUOTE DOCUMENTS")) or (str(getRevision.REVISION_STATUS).upper() in ("APR-APPROVED","APR-REJECTED","APR-RECALLED","APR-APPROVAL PENDING","OPD-PREPARING QUOTE DOCUMENTS","OPD-CUSTOMER ACCEPTED","APR-SUBMITTED TO CUSTOMER","OPD-CUSTOMER REJECTED") and str(TreeParam).upper() not in ("APPROVALS","QUOTE DOCUMENTS") or (RECORD_ID =="SYOBJR-00009")):
				#ENDS  -A055S000P01- 20873-M
					Trace.Write("test354")
		...
		if Columns != "" and Obj_Name != "":
			...
			if Qstn_REC_ID != "" and Wh_API_NAME != "":
				...
				if str(Qury_str) == "" and str(QuryCount_str) == "":
						...
						if qt_rec_id is not None:
							...
							for document_type in doc_type:
								if document_type.DOCTYP_ID == "ZWK1":
									...
									#A055S000P01-20655 - start
									Qustr+= " AND ISNULL(PAR_SERVICE_ID,'')='' "
									# A055S000P01-20655 - end
									Qury_str = (
										"select DISTINCT top "
										+ str(PerPage)
										+ " "
					...
					#START-P-INC09010656-M
							Qury_str = """
										SELECT DISTINCT TOP {PerPage} * FROM ( SELECT * FROM ({InnerQuery}) OQ WHERE ROW BETWEEN {Start} AND {End} ) AS FQ ORDER BY LINE 
						...
						if billing_date_column:
							...
							if get_current_record:
								...
								if str(get_current_record.PAR_SERVICE_ID) != '' and TreeParam == 'Z0105':
									...
									parent_level_where_condition = ""
								elif str(get_current_record.PAR_SERVICE_ID) != '':#INC08696998	
									par_ser_id =str(get_current_record.PAR_SERVICE_ID)#INC08696998
									parent_level_where_condition = "and isnull(PAR_PRDOFR_ID, '')='{}'".format(par_ser_id)
								else:
									par_ser_id =''
									parent_level_where_condition = ""
							# INC08696998 - Starts - M	
							get_billing_type = Sql.GetFirst("SELECT BILLING_TYPE from SAQRIT where SERVICE_ID = '{}' and QUOTE_RECORD_ID = '{}' AND QTEREV_RECORD_ID = '{}' {ParentLevelWhereCondition}".format(TreeParam,RecAttValue,quote_revision_record_id,ParentLevelWhereCondition=parent_level_where_condition))	
							# INC08696998 - Ends - M
							...
							if get_billing_type:
								...
								column_before_pivot_changes += ", BILADJ_DTYFLG AS dirtyFlag"
								...
								if str(TreeParentParam)== "Billing" and TreeParam == "Z0117":
									...
									if get_billing_cycle:
										...
										if get_billing_cylce == "ANNUALLY":
											#A055S000P01-20887 - Start
											Qustr += " AND BILLING_YEAR = '{}' AND QTEREV_RECORD_ID = '{}' AND SERVICE_ID = '{}' AND ISNULL(PAR_SERVICE_ID,'') = '{}' AND BILLING_DATE = '{}'".format(SubTab,quote_revision_record_id,TreeParam,par_ser_id,billing_date_column[0])
										else:
			...
			if Query_Obj is not None:
				for ik in Query_Obj:				                  
					...
					#A055S000P01-20779 Start - M
					dirtyFlag = ''
					if RECORD_ID == 'SYOBJR-00007':
						dirtyFlag = str(ik.dirtyFlag)
					#A055S000P01-20779 End - M
						...
						# elif str(current_tab).upper() == "ROLE":
							...
							if ObjectName== "SAQICT" :
							 ...
							 #START -A055S000P01- 20873-M
								...
								#ENDS -A055S000P01- 20873-M
							#INCO8642678 Start - M
							elif ObjectName != "SAQIBP" and ObjectName != "SAQSCN" and ObjectName != "SAQRDS":
							#INCO8642678 End - M
								Action_str += '<li><a class="dropdown-item" href="#" onclick="Commonteree_view_RL(this)">VIEW</a></li>'
									...
									if value123 in [
										...
										if str(value123) == "WARRANTY_END_DATE" and RECORD_ID != "SYOBJR-00009":
											...
											if getdate_indication:												
												...
												#A055S000P01-20779 Start - M
												bill_class = "billclassedit"
												if dirtyFlag == 'True':
													bill_class += " bill_mismatch_line "
												new_dict[value123] = (
													'<input  type= "text" id ="' + key_value + '" class= "' + bill_class + '"  value="' + value1234 + '" style="border: 0px solid;"  disabled>'
												)
												#A055S000P01-20779 End - M
													...
													# for redirecting the left tree node while viewing record from listgrid - end       
														...
														try:
															if RECORD_ID == 'SYOBJR-00009' and value123 == 'DISCOUNT':
																...
																#A055S000P01-20779 Start - M
																bill_class_style = ''
																if str(value123) == 'ANNBILAMT_INGL_CURR' and RECORD_ID == 'SYOBJR-00007' and dirtyFlag == 'True':
																	bill_class_style = 'style="text-align: right; background: #f8d7da" class="bill_mismatch_line"'
																new_dict[value123] = ('<abbr  title="' + str(value1234).upper() + '"' + str(bill_class_style) + '>' +str(value1234).upper() + "</abbr>")
																#A055S000P01-20779 End - M
														except:
		...
		if CHL_STS_OBJ is not None: # Not Needed
			...
			if RECORD_ID != "SYOBJR-00009":
				if RECORD_ID == "SYOBJR-95556":
					...
					if (RECORD_ID == "SYOBJR-00005" or RECORD_ID == "SYOBJR-00033"):#INC08632845
				...
				dbl_clk_function += (
					...
					+ str(cls)
					+ '").text()); localStorage.setItem("multiedit_checkbox_clicked", checkedRows); if(checkedRows.length > 1){ $("button#delete_send_equipment").css("display","block");} else { $("button#delete_send_equipment").css("display","none");} }); $("'
					+ str(table_ids)
					+ '").on("check-all.bs.table", function (e) { var table = $("'
					+ str(table_ids)
					+ '").closest("table"); table.find("tbody tr").each(function() { checkedRows.push($(this).find("td:'
					+ str(cls)
					+ '").text()); }); localStorage.setItem("multiedit_checkbox_clicked", checkedRows); $("button#delete_send_equipment").css("display","block"); }); $("'
					+ str(table_ids)
					+ '").on("uncheck-all.bs.table", function (e) { localStorage.setItem("multiedit_checkbox_clicked", []); checkedRows=[]; $("button#delete_send_equipment").css("display","none"); }); $("'
					+ str(table_ids)
					+ '").on("uncheck.bs.table", function (e, row, $element) { var rec_ids=$element.closest("tr").find("td:'
					+ str(cls)
					+ '").text(); $.each(checkedRows, function(index, value) { if (value === rec_ids) { checkedRows.splice(index,1); }}); localStorage.setItem("multiedit_checkbox_clicked", checkedRows); if(checkedRows.length > 1){ $("button#delete_send_equipment").css("display","block");} else { $("button#delete_send_equipment").css("display","none");} });'
						)
				#INC08632845 End - M
				dbl_clk_function += (
		...
		if ObjectName == "SAQIBP":
			...
			if TreeParam == "Z0117" and get_billing_cylce == "ANNUALLY":
				...
				gettotaldateamt_ann = Sql.GetList("SELECT BILLING_VALUE_INGL_CURR=SUM(BILLING_VALUE_INGL_CURR),ESTVAL_INGL_CURR = SUM(ESTVAL_INGL_CURR),ANNBILAMT_INGL_CURR,EQUIPMENT_ID FROM SAQIBP WHERE   QUOTE_RECORD_ID ='{cq}' AND QTEREV_RECORD_ID='{revision_rec_id}' AND SERVICE_ID = '{service_id}' AND BILLING_YEAR = '{BillingYear}' AND ISNULL(PAR_SERVICE_ID, '') = '{par_ser_id}' group by  BILLING_YEAR,ANNBILAMT_INGL_CURR,EQUIPMENT_ID".format(cq=str(ContractRecordId),revision_rec_id = quote_revision_record_id,service_id = TreeParam, BillingYear=SubTab, par_ser_id=par_ser_id))
			elif str(get_billing_types).upper() in ("MILESTONE","FIXED") and TreeParam not in ("Z0006","Z0007","Z0009"):
				# INC08696998 - Starts - M
				# if TreeParam != 'Z0105':
				# 	par_ser_id = ''
				# INC08696998 - Ends - M
				Trace.Write("CHKNG_TRACEZ")
				...
				#A055S000P01-20771-Start-A
				gettotaldateamt_ann = Sql.GetList("SELECT BILLING_VALUE_INGL_CURR=SUM(BILLING_VALUE_INGL_CURR),ESTVAL_INGL_CURR = SUM(ESTVAL_INGL_CURR),ANNBILAMT_INGL_CURR,EQUIPMENT_ID FROM SAQIBP WHERE   QUOTE_RECORD_ID ='{cq}' AND QTEREV_RECORD_ID='{revision_rec_id}' AND SERVICE_ID = '{service_id}' AND BILLING_YEAR = '{BillingYear}' AND ISNULL(PAR_SERVICE_ID, '') = '{par_ser_id}' group by  BILLING_YEAR,ANNBILAMT_INGL_CURR,EQUIPMENT_ID,LINE".format(cq=str(ContractRecordId),revision_rec_id = quote_revision_record_id,service_id = TreeParam, BillingYear=SubTab, par_ser_id=par_ser_id))
				#A055S000P01-20771-End-A
			elif str(get_billing_types).upper() in ("MILESTONE","FIXED") and TreeParam in ("Z0006","Z0007","Z0009"):
				Trace.Write("val_newtrace1")
				# INC08696998 - Starts - M
				# if TreeParam != 'Z0105':
				# 	par_ser_id = ''
				# INC08696998 - Ends - M
				...
				gettotaldateamt_ann = Sql.GetList("SELECT BILLING_VALUE_INGL_CURR=SUM(BILLING_VALUE_INGL_CURR),ESTVAL_INGL_CURR = SUM(ESTVAL_INGL_CURR),ANNBILAMT_INGL_CURR,EQUIPMENT_ID FROM SAQIBP WHERE   QUOTE_RECORD_ID ='{cq}' AND QTEREV_RECORD_ID='{revision_rec_id}' AND SERVICE_ID = '{service_id}' AND BILLING_YEAR = '{BillingYear}' AND ISNULL(PAR_SERVICE_ID, '') = '{par_ser_id}' group by  BILLING_YEAR,ANNBILAMT_INGL_CURR,EQUIPMENT_ID".format(cq=str(ContractRecordId),revision_rec_id = quote_revision_record_id,service_id = TreeParam, BillingYear=SubTab, par_ser_id=par_ser_id))
			# INC08840688 - Starts - A
			elif TreeParam == "Z0116":
				gettotaldateamt = Sql.GetList("SELECT BILLING_VALUE_INGL_CURR=SUM(BILLING_VALUE_INGL_CURR),ESTVAL_INGL_CURR = SUM(ESTVAL_INGL_CURR),ANNBILAMT_INGL_CURR = SUM(ANNBILAMT_INGL_CURR),BILLING_DATE FROM SAQIBP WHERE BILLING_DATE in ('{billing_date_column}') and QUOTE_RECORD_ID ='{cq}' AND QTEREV_RECORD_ID='{revision_rec_id}' AND SERVICE_ID = '{service_id}' AND BILLING_YEAR = '{BillingYear}' AND ISNULL(PAR_SERVICE_ID, '') = '{par_ser_id}' group by BILLING_DATE ".format(cq=str(ContractRecordId),revision_rec_id = quote_revision_record_id,billing_date_column="','".join(billing_date_column),service_id = TreeParam, BillingYear=SubTab, par_ser_id=par_ser_id))

				gettotaldateamt_ann = Sql.GetList("SELECT BILLING_VALUE_INGL_CURR=SUM(BILLING_VALUE_INGL_CURR),ESTVAL_INGL_CURR = SUM(ESTVAL_INGL_CURR),ANNBILAMT_INGL_CURR,EQUIPMENT_ID FROM SAQIBP WHERE   QUOTE_RECORD_ID ='{cq}' AND QTEREV_RECORD_ID='{revision_rec_id}' AND SERVICE_ID = '{service_id}' AND BILLING_YEAR = '{BillingYear}' AND ISNULL(PAR_SERVICE_ID, '') = '{par_ser_id}' group by  BILLING_YEAR,ANNBILAMT_INGL_CURR,EQUIPMENT_ID".format(cq=str(ContractRecordId),revision_rec_id = quote_revision_record_id,service_id = TreeParam, BillingYear=SubTab, par_ser_id=par_ser_id))
			# INC08840688 - Ends - A
			else:
				Trace.Write("val_newtrace")
				Trace.Write("get_billing_types---"+str(get_billing_types).upper())
				# INC08696998, INC08767131 - Starts - M
				gettotaldateamt = Sql.GetList("SELECT BILLING_VALUE_INGL_CURR=SUM(BILLING_VALUE_INGL_CURR),ESTVAL_INGL_CURR = SUM(ESTVAL_INGL_CURR),ANNBILAMT_INGL_CURR ,EQUIPMENT_ID FROM SAQIBP WHERE   QUOTE_RECORD_ID ='{cq}' AND QTEREV_RECORD_ID='{revision_rec_id}' AND SERVICE_ID = '{service_id}' AND BILLING_YEAR = '{BillingYear}' AND ISNULL(PAR_SERVICE_ID, '') = '{par_ser_id}' group by  BILLING_YEAR,ANNBILAMT_INGL_CURR,EQUIPMENT_ID".format(cq=str(ContractRecordId),revision_rec_id = quote_revision_record_id,service_id = TreeParam, BillingYear=SubTab, par_ser_id=par_ser_id))# INC08767131 -End M

				gettotaldateamt_ann = Sql.GetList("SELECT BILLING_VALUE_INGL_CURR=SUM(BILLING_VALUE_INGL_CURR),ESTVAL_INGL_CURR = SUM(ESTVAL_INGL_CURR),ANNBILAMT_INGL_CURR,EQUIPMENT_ID FROM SAQIBP WHERE   QUOTE_RECORD_ID ='{cq}' AND QTEREV_RECORD_ID='{revision_rec_id}' AND SERVICE_ID = '{service_id}' AND BILLING_YEAR = '{BillingYear}' AND ISNULL(PAR_SERVICE_ID, '') = '{par_ser_id}' group by  BILLING_YEAR,ANNBILAMT_INGL_CURR,EQUIPMENT_ID".format(cq=str(ContractRecordId),revision_rec_id = quote_revision_record_id,service_id = TreeParam, BillingYear=SubTab, par_ser_id=par_ser_id))
			# INC08836544 - Ends - M
			#INC09142771
            if gettotaldateamt_ann:
                my_format = "{:,." + str(decimal_place) + "f}"
                for val in gettotaldateamt_ann:                    
                    if val.ANNBILAMT_INGL_CURR:
                        gettotalamt_annual += val.ANNBILAMT_INGL_CURR                        
                        gettotalamt = str(my_format.format(round(float(gettotalamt_annual), int(decimal_place))))
            #INC09142771
			if gettotaldateamt:
				my_format = "{:,." + str(decimal_place) + "f}"
				...
				for val in gettotaldateamt:
					...
					# INC08836544 - Starts - M
					elif str(get_billing_types).upper() in ("MILESTONE","FIXED"):
		...
		RelatedDrop_str = (
			...
			+ str(table_id)
			+ "').getElementsByClassName('filter-control')[i].innerHTML = data6[i];  } for (j = 0; j < listws.length; j++) { if (data7[j] == 'select') { var dataAdapter = new $.jqx.dataAdapter(data8[j]); if(data8[j] == ''){ $('#"
            + str(table_id)
            + "_RelatedMutipleCheckBoxDrop_' + j.toString() ).jqxDropDownList( { source: dataAdapter, autoDropDownHeight: true}); }else if(data8[j].length>5){ $('#"
			+ str(table_id)
	...
	def MDYNMICSQLOBJECTFILTER(
		...
		if Columns != "" and Obj_Name != "":
			...
			if Qstn_REC_ID != "" and Wh_API_NAME != "":
						...
						if str(quer_values).find(",") == -1:
							...
							if str(quer_value) == "EXCLUSIVE_MATERIAL":
								...
								if api_data_type == "AUTO NUMBER":
            						...
            						#A055S000P01-19947 Start
									quer_values = ''.join(str(x) for x in quer_values)
									ATTRIBUTE_VALUE_STR += " " + str('BILLING_VALUE_INGL_CURR') + " in " + str(quer_values) + " and "
									#A055S000P01-19947 End
								else:
									trueFalseCondition = True
				...
				if ATTRIBUTE_VALUE_STR != "":
					...
					#END-P-INC09010656-M
						...
						if Qustr:
							if str(TreeParentParam)== "Billing":
								...
								#A055S000P01-20857 - Start
								Qustr += " AND BILLING_YEAR = '{}' AND QTEREV_RECORD_ID = '{}' AND ISNULL(PAR_SERVICE_ID,'') = '{}' AND BILLING_DATE BETWEEN '{}' AND '{}'".format(SubTab,quote_revision_record_id,par_ser_id,billing_date_column[0], billing_date_column[-1])
								#A055S000P01-20857 - End
					#START-P-INC09010656-M
						pivot_query_str = """
									SELECT ROW_NUMBER() OVER(ORDER BY LINE)
									AS ROW, *
					...
					#CR666-INC08864593 - START - M
						...
						Service_ID = Product.GetGlobal("TreeParentLevel1")
						Qustr = " where "+str(ATTRIBUTE_VALUE_STR)+" "+ str(Wh_API_NAME) + " = '" + str(RecAttValue) + "' AND GREENBOOK = '"+str(TreeParentParam)+"' AND PAR_SERVICE_ID = '"+str(Service_ID)+"' AND QTEREV_RECORD_ID = '"+str(quote_revision_record_id)+"'"
					#CR666-INC08864593 - END - M
					else:
					...
					if RECORD_ID == "SYOBJR-95868":                        
						...
						#CR666-INC08864593 - START - M
							...
							Trace.Write("CMG inside NSO")
							Service_ID = Product.GetGlobal("TreeParentLevel1")
							Qustr = " where " + str(Wh_API_NAME) + " = '" + str(RecAttValue) + "' AND GREENBOOK = '"+str(TreeParentParam)+"' AND  PAR_SERVICE_ID = '"+str(Service_ID)+"' AND QTEREV_RECORD_ID = '"+str(quote_revision_record_id)+"'"
						#CR666-INC08864593 - END - M
						else:
							Qustr = " where " + str(Wh_API_NAME) + " = '" + str(RecAttValue) + "' AND QTEREV_RECORD_ID = '"+str(quote_revision_record_id)+"'"
				...
				if str(Qury_str) == "" and str(QuryCount_str) == "":  
					...
					#END-P-INC09010656-M
					#START-P-INC09010656-M
						Qury_str = """SELECT DISTINCT TOP {PerPage} * FROM ( SELECT * FROM ({InnerQuery}) OQ WHERE ROW BETWEEN {Start} AND {End} ) AS FQ ORDER BY LINE
										""".format(PerPage=PerPage, OrderByColumn=Wh_API_NAMEs, InnerQuery=pivot_query_str, Start=Page_start, End=Page_End)
					#END-P-INC09010656-M
						QuryCount_str = "SELECT COUNT(*) AS cnt FROM ({InnerQuery}) OQ ".format(InnerQuery=pivot_query_str)
					elif  str(RECORD_ID) == "SYOBJR-34575":
					...
					#END-P-INC09010656-M
						...
						if str(RECORD_ID)=="SYOBJR-98872": #P-INC09039186
							Wh_API_NAMEs = "LINE"
						Qury_str = (
							"select top "
							+ str(PerPage)
			...
			for ik in Query_Obj:				                  
				...
				if str(current_tab).upper() == "PROFILE" and (ObjectName == "SYPROF"):
					...
					#INCO8642678 Start - M
					if ObjectName != "SAQIBP" and ObjectName != "SAQTRV" and ObjectName != "SAQSCN" and ObjectName != "SAQDOC" and ObjectName != "SAQSPT" and ObjectName != "SAQRDS":
					#INCO8642678 End - M
						#START -A055S000P01- 20873-M
						UserName=User.UserName
						saqdlt_query = Sql.GetList("SELECT C4C_PARTNERFUNCTION_ID FROM SAQDLT (NOLOCK) WHERE QUOTE_RECORD_ID= '{quote_rec_id}' AND QTEREV_RECORD_ID = '{qte_rev_rec_id}' AND MEMBER_ID = '{UserName}'".format(quote_rec_id=Quote.GetGlobal("contract_quote_record_id"),qte_rev_rec_id=Quote.GetGlobal("quote_revision_record_id"),UserName=UserName))
				...
				#INC08731795 Start - A
				if str(Action_permission.get("Edit")).upper() == "FALSE" and ObjectName == "SAQSPT":
					contract_quote_rec_id = Quote.GetGlobal("contract_quote_record_id")
					quote_revision_rec_id = Quote.GetGlobal("quote_revision_record_id")
					qt_status =Sql.GetFirst("""SELECT REVISION_STATUS FROM SAQTRV WHERE  QUOTE_RECORD_ID = '{ContractRecordId}' AND QTEREV_RECORD_ID = '{quote_revision_record_id}'""".format(ContractRecordId =contract_quote_rec_id,quote_revision_record_id =quote_revision_rec_id))
					if qt_status.REVISION_STATUS in ('CFG-CONFIGURING','CFG-ACQUIRING','PRR-ON HOLD PRICING','PRI-PRICING','APR-APPROVAL PENDING','APR-RECALLED','APR-REJECTED'):
				...
				for inm in ik:
					...
					value1234 = valu.replace("]", "").lstrip()
					try:
						value1234 =str(value1234.encode("ASCII", "ignore"))
					except:
						value1234 = value1234.encode("ASCII", "ignore")
					if RECORD_ID != 'SYOBJR-98869':
						if value1234 == "ACQUIRED" or value1234 == "PRICED":
							if RECORD_ID == 'SYOBJR-00005' or RECORD_ID == 'SYOBJR-00029':
								value1234 = value1234.replace(value1234,"<img title='"+str(value1234).title()+"' src='/mt/APPLIEDMATERIALS_PRD/Additionalfiles/Green_Tick.svg'> ")
							...
							if value123 in [
								...
								if str(value123) == "WARRANTY_END_DATE":
									...
									if curr_symbol_obj:
										...
										#INC08756550 start M 
										try:
											if value1234 =="" or value1234 is None:
												value1234 = 0          
											if  decimal_place == '0'or decimal_place is None or decimal_place == "":
												decimal_place = 2
												my_format = "{:,." + str(decimal_place) + "f}"
												value1234 = str(my_format.format(round(float(value1234), int(decimal_place))))
											else:
				...
				for key, col_name in enumerate(list(eval(Columns))):                    
					if ObjectName == 'SAQIBP' and TreeParam != 'Quote Items' and (col_name in billing_date_column or col_name == 'QUOTE_CURRENCY'):                        
						...
						try:                            
							...
							footer += '<th>0.00</th>'
		Trace.Write("footer_tot---> "+str(footer_tot))
		if ObjectName == 'SAQIBP' and TreeParam != 'Quote Items':
		...
		#Trace.Write("QueryCount---> "+str(QueryCount)+" Page_End---> "+str(Page_End))
		if QueryCount==0:
			Page_start=0
		if QueryCount < int(Page_End):
			PageInformS = str(Page_start) + " - " + str(QueryCount) + " of"
