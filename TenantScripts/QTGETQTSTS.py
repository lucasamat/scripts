...
# ================================================================================================================
# Modified Date : 09-Sept-2022
# Modified By   : Suresh Muniyandi / Baji /Arjun / Joe
# Modified Desc : Incident INC08675924, importing write back script (CQCPQC4CWB) script and Calling C4C write back script .
# Affected Line : 26,151-157
# Affected Table: None
# ================================================================================================================
import sys
import datetime 
			...
			if len(Dirct_record) != 0:
				...
				if Dirct_record['STATUS_DESCRIPTION'].upper() == "SUCCESS":
					...
					if 'QUOTE_ID'  in Dirct_record:
						...
						if 'CONTRACT_ID'  in Dirct_record:
							primaryQueryItems = SqlHelper.GetFirst(""+ str(Parameter1.QUERY_CRITERIA_1)	+ "  SAQTMT SET QUOTE_STATUS = ''BOK-CONTRACT BOOKED'',ECC_CONTRACT_ID = ''"+str(Dirct_record['CONTRACT_ID'])+"'' FROM SAQTMT(NOLOCK) JOIN SAQTRV (NOLOCK) ON SAQTMT.QUOTE_ID = SAQTRV.QUOTE_ID AND SAQTMT.QTEREV_ID = SAQTRV.QTEREV_ID WHERE C4C_QUOTE_ID = ''"+str(Dirct_record['QUOTE_ID'])+"'' AND DOCTYP_ID = ''ZWK1'' '")
							
							primaryQueryItems = SqlHelper.GetFirst(""+ str(Parameter1.QUERY_CRITERIA_1)	+ "  SAQTRV SET REVISION_STATUS = ''BOK-CONTRACT BOOKED'' FROM SAQTMT(NOLOCK) JOIN SAQTRV (NOLOCK) ON SAQTMT.QUOTE_ID = SAQTRV.QUOTE_ID AND SAQTMT.QTEREV_ID = SAQTRV.QTEREV_ID WHERE C4C_QUOTE_ID = ''"+str(Dirct_record['QUOTE_ID'])+"'' AND DOCTYP_ID = ''ZWK1'' '")
				...
				# INC08675924 - Starts - A
				Query2 =SqlHelper.GetFirst("SELECT REVISION_STATUS,QUOTE_RECORD_ID,QUOTE_REVISION_RECORD_ID FROM SAQTRV (NOLOCK) WHERE QUOTE_ID ='"+str(Dirct_record['QUOTE_ID'])+"' and ACTIVE = 1 ")
				...
				if Dirct_record['STATUS_DESCRIPTION'].upper() == "ERROR":
					...
					msg.Subject = "CPQ Quote to CRM/ECC Contract Error Status - Notification(P-Tenant)"
					msg.IsBodyHtml = True
					msg.Body = Error_Info
