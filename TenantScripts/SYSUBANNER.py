...
def Related_Sub_Banner(
    ...
    if page_details:
        ...
        if len(dynamic_Button) > 0:
            ...
            if subTabName == "NSO Catalog" and ObjName=='SAQSAO':
                add_button=''
    ...
    if ObjNameQuery is not None:
        ...
        #INC08632845 M
        if (CurrentRecordId.startswith("SYOBJR", 0) == True or subTabName == "Receiving Fab Details" or subTabName =="Sending Fab Details"):
            ...
            if TreeParam == "Customer Information":
                ...
                if(subTabName == "Details" or subTabName == "Sending Fab Locations" or subTabName == "Sending Account Details" or subTabName == "Sending Fab Location Details" or subTabName == "Sending Equipment" or subTabName =="Sending Fab Details") and TreeParam == "Customer Information":
                    ...
                    if subTabName == "Sending Fab Location Details" or subTabName == "Sending Equipment":
                        ...
                        FourthValue = "ALL"
                elif(subTabName == "Details" or subTabName == "Receiving Fab Locations" or subTabName == "Receiving Account Details" or subTabName == "Receiving Fab Location Details" or subTabName == "Receiving Equipment" or subTabName == "Receiving Fab Details") and TreeParam == "Customer Information":
                    PrimaryLable = "Party ID"
            ...
            if TabName == "Profile" and TreeParam != "" and str(ObjName) == "SYPRTB":
                ...
                TreeParentParam = Product.GetGlobal("TreeParentLevel0")
                getService = Sql.GetFirst("select SERVICE_DESCRIPTION from SAQTSV(nolock) where SERVICE_ID = '"+str(TreeParentParam)+"' and QUOTE_RECORD_ID = '"+str(contract_quote_record_id)+"' AND QTEREV_RECORD_ID = '"+str(quote_revision_record_id)+"'")
                PrimaryLable = "Product Offering ID"
                PrimaryValue = str(TreeParentParam)
            ...
            if Tab_Obj_Name is not None:
                ...
                if (TreeTopSuperParentParam == "Complementary Products" ) and (subTabName == "Equipment" or subTabName == "Entitlements" or subTabName == "Greenbook Fab Value Drivers" or subTabName == "Greenbook Cost and Value Drivers" or subTabName == "Equipment Fab Value Drivers" or subTabName =="Details" or subTabName =="Customer Value Drivers" or subTabName =="Product Value Drivers"):
                    ...
                    if subTabName == "Equipment Fab Value Drivers":
                        ...
                        try:
                            getService = Sql.GetFirst("select SERVICE_DESCRIPTION from SAQTSV (nolock) where SERVICE_ID = '"+str(TreeSuperParentParam)+"' and QUOTE_RECORD_ID = '"+str(contract_quote_record_id)+"' AND QTEREV_RECORD_ID = '"+str(quote_revision_record_id)+"'")
                            desc = getService.SERVICE_DESCRIPTION
                        except:
                            desc = ""
                        PrimaryLable = "Product Offering ID"
                ...
                if (str(ObjName) == 'SAQTSV'or str(ObjName) == 'SAQSCO' or str(ObjName) == 'SAQSPT') and TreeSuperParentParam == 'Product Offerings'and TabName == "Quotes":
                    ...
                    try:
                        ...
                        TreeParentParam = ''
                    getService = Sql.GetFirst("select SERVICE_DESCRIPTION from SAQTSV(nolock) where SERVICE_ID = '"+str(TreeParam)+"' and QUOTE_RECORD_ID = '"+str(contract_quote_record_id)+"' AND QTEREV_RECORD_ID = '"+str(quote_revision_record_id)+"'")
                    PrimaryLable = "Product Offering ID"
                    ...
                    if status_image:
                        if ObjName == "SAQSPT":
                            SixthValue = '<img class="treeinsideicon" src="/mt/APPLIEDMATERIALS_UAT/Additionalfiles/AMAT/Quoteimages/{image}"/>'.format(image = status_image)
                        else:
                            FourthValue = '<img class="treeinsideicon" src="/mt/APPLIEDMATERIALS_UAT/Additionalfiles/AMAT/Quoteimages/{image}"/>'.format(image = status_image)
                    # FifthLable = ""
                    # FifthValue = ""
                    if getService is not None:
                        SecondLable = "Product Offering Description"
                    ...
                    if covered_obj is not None and (subTabName == "Equipment" or subTabName == 'Entitlements' or subTabName == 'Service Fab Value Drivers' or subTabName == 'Service Cost and Value Drivers' or subTabName == 'Customer Value Drivers' or subTabName == 'Product Value Drivers'):
                        ...
                        if status_image:
                            FifthValue = '<img class="treeinsideicon" src="/mt/APPLIEDMATERIALS_UAT/Additionalfiles/AMAT/Quoteimages/{image}"/>'.format(image = status_image)                        
    ...
    if TreeParam == 'Quote Items' and (subTabName == "Summary" or subTabName == "Offerings" or subTabName == "Items" or subTabName == "Annualized Items" or subTabName == "Entitlement Cost/price"):        
        ...
        if subTabName == "Summary":
            ...
            if subTabName == "Items":
                ...
                #INC08626568  A
            elif subTabName == "Annualized Items":
                PrimaryLable = "Total Excluding Tax/VAT"
                PrimaryValue = decimal_format.format(float(get_quote_details.NET_VALUE_INGL_CURR))+" "+ curr if str(get_quote_details.NET_VALUE_INGL_CURR) != '' else decimal_format.format(float("0.00"))+" "+curr
                SecondLable = "Tax/VAT"
                #SecondValue = str("%.2f" % round(float(get_quote_details.TAX_AMOUNT_INGL_CURR),2))+" "+curr if str(get_quote_details.TAX_AMOUNT_INGL_CURR) != '' else '0.00'+" "+curr
        ...
        getService = Sql.GetFirst("select SERVICE_DESCRIPTION from SAQTSV (nolock) where SERVICE_ID = '{service_id}' and QUOTE_RECORD_ID = '{contract_quote_record_id}' and QTEREV_RECORD_ID = '{quote_revision_record_id}'".format(service_id=TreeSuperParentParam if "Add" in TreeParam else TreeParentParam,contract_quote_record_id=contract_quote_record_id,quote_revision_record_id=quote_revision_record_id))
        try:
            PrimaryLable = "Product Offering ID"
        ...
        ThirdLable = "Greenbook"
        ThirdValue = str(TreeParam)
        # FourthLable = "Assembly ID"  #INC08696637
        # FourthValue = str(AssemblyId) #INC08696637
    ...
    if CurrentRecordId.startswith("SYOBJR", 0) == True:
        ...
        if currecId:
            ...
            if (str(TabName) == "Quotes" or str(TabName == "Quote")):
                ...
                if getsaletypeloc:
                    if TreeParam == "Add-On Products" and "INCLUDE ADD-ON PRODUCTS" not in str(add_button) and subTabName == 'Equipment':
                        Trace.Write("multi_buttons--2754-J"+str(add_button))
                        sec_rel_sub_bnr += ""
                        Trace.Write('sec_rel_sub_bnr-2754-'+str(sec_rel_sub_bnr))
                    if len(multi_buttons)>0:                        
    ...
    if subTabName == 'Add On Products' and TreeParentParam == "Comprehensive Services":
        ...
        if str(TreeParam).upper() == "QUOTE INFORMATION" and subTabName == "Involved Parties" and TabName == "Quotes":
            ...
            if revision_status.REVISION_STATUS == 'CFG-CONFIGURING':
                sec_rel_sub_bnr += ('<button id="delete_receive_equipment" onclick="bulk_del_yes(\'po_greenbook_equipment_deletion\')" class="btnconfig" style="display:none;">DELETE</button>')
                ...
                if "ADD FROM LIST" in add_button:
                    ...
                    if str(multi_buttons) != "" and (subTabName == "NSO Catalog" or subTabName == "Credits"):
                        for btn in multi_buttons:
                            sec_rel_sub_bnr+= str(btn)
                    #A055S000P01-20453 Start - A
                    elif (str(ObjName) == 'SAQSCO' and subTabName == "Equipment" and TreeParam == "Add-On Products"):
                        all_buttons = list(multi_buttons)
                        all_buttons.remove(str(add_button)) #To remove the button duplications
                        for btn in all_buttons:
                            sec_rel_sub_bnr+= str(btn)
                        Trace.Write("sec_rel_sub_bnr==2 "+str(sec_rel_sub_bnr))
                    #A055S000P01-20453 End - A
                    else:
                        if subTabName =='Entitlements' and str(ObjName) == "SAQSAO" and TreeParam == "Add-On Products" :
                            sec_rel_sub_bnr+=''
            ...
            if len(multi_buttons)>0: ##adding dynamic buttons from SYPGAC if we have more than one button
                for btn in multi_buttons:
                    if ('SPLIT' in btn or 'EDIT' in btn) and subTabName =='Items':
                        ...
                        #         sec_rel_sub_bnr += (btn)
                    #INC08927523 - Start - M
                    if quote_status.QUOTE_STATUS != 'APPROVED' and 'SPLIT' not in btn and 'EDIT' not in btn and 'CREDIT' not in btn and 'ADD SENDING EQUIPMENT' not in btn:
                        #INC08927523 - End - M                       
                        sec_rel_sub_bnr += (btn)
    ...
    # INC08832348 - Start - M
    mismatched_billings_obj = Sql.GetList("SELECT MAX(SAQIBP.LINE) as LINE FROM SAQIBP JOIN SAQRIT ON SAQRIT.QUOTE_RECORD_ID = SAQIBP.QUOTE_RECORD_ID AND SAQRIT.QTEREV_RECORD_ID = SAQIBP.QTEREV_RECORD_ID AND SAQRIT.SERVICE_ID = SAQIBP.SERVICE_ID AND SAQRIT.LINE = SAQIBP.LINE WHERE SAQIBP.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQIBP.QTEREV_RECORD_ID = '{QuoteRevisionRecordId}' GROUP BY SAQIBP.QUOTE_RECORD_ID, SAQIBP.QTEREV_RECORD_ID, SAQIBP.SERVICE_ID, SAQIBP.LINE, SAQIBP.BILLING_YEAR HAVING SUM(CASE WHEN SAQIBP.BILLING_TYPE IN ('MILESTONE','FIXED') THEN SAQIBP.BILLING_VALUE_INGL_CURR ELSE SAQIBP.ESTVAL_INGL_CURR END) <> MAX(ANNBILAMT_INGL_CURR)".format(QuoteRecordId=contract_quote_record_id, QuoteRevisionRecordId=quote_revision_record_id))
    if mismatched_billings_obj:
        #INC08818816 - Start - M
        get_error_msg_obj = Sql.GetFirst("SELECT MESSAGE_TEXT FROM SYMSGS WHERE MESSAGE_CODE = 'BILL_MISMATCH' ")
        msg_text = get_error_msg_obj.MESSAGE_TEXT
        get_bill_message_warning += 'This Quote has the following notifications:'
        get_bill_message_warning += ('<div class="col-md-12" id="dirty-flag-warning"><div class="col-md-12 alert-warning"><label> <img src="/mt/APPLIEDMATERIALS_UAT/Additionalfiles/warning1.svg" alt="Warning">'+str(msg_text)+'</label></div></div>')
        #INC08818816 - End - M
        # get_bill_message_warning += 'This Quote has the following notifications:'
        # get_bill_message_warning += ('<div class="col-md-12" id="dirty-flag-warning"><div class="col-md-12 alert-warning"><label> <img src="/mt/APPLIEDMATERIALS_UAT/Additionalfiles/warning1.svg" alt="Warning"> 99999 | INCORRECT BILLING ITEMS | Billing Item price does not equal Annual Billing Amount.</label></div></div>')
    else:
        get_bill_message_warning = ''
    # INC08818816 - Start - M
    # bill_plan_years = Sql.GetList("SELECT DISTINCT BILLING_YEAR,SERVICE_ID,BILLING_TYPE from SAQIBP WHERE QUOTE_RECORD_ID = '"+str(contract_quote_record_id)+"' AND QTEREV_RECORD_ID = '"+str(quote_revision_record_id)+"' ")
    # # INC08818816 - End - M
    # billing_message= []
    # get_bill_message_warning = ''
    # if bill_plan_years:
    ...
    #         item_bill_type = Sql.GetFirst("SELECT BILLING_TYPE,"+bill_years_gl_curr+" as year_gc from SAQRIT where QUOTE_RECORD_ID= '"+str(contract_quote_record_id)+"' and QTEREV_RECORD_ID ='"+str(quote_revision_record_id)+"' and SERVICE_ID='"+str(service_id)+"' group by BILLING_TYPE")
    #         # INC08818816 - Stop - M
    #         if item_bill_type:
