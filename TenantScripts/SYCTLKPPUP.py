        ...
        for key, header in enumerate(FIELD_LABEL_list[1:]):
            ...
            if len(api_name_dict_list) > 0:
                ...
                filter_control_function += (
                    ...
                    + str(values_list)
                    + ' var attribute_value = $(this).val();Fetch_Count = $("#ShowResultCountsApp option:selected").val(); debugger;cpq.server.executeScript("SYCTLKPPUP", {"TABLEID":"'
                    + str(TABLEID)
                    + '","OPER":"'
                    + str(OPER)
                    + '","Fetch_Count": Fetch_Count, "ATTRIBUTE_NAME": '
                    ...
                    + '" , "SELECTED_ROLE": $("#CPQ_PARTNER_FUNCTION").val() }, function(data) { var len = (data[1] == "") ? data[0].length : data[1]; $("#'
                    + str(TABLEIDS)
                    + '").bootstrapTable("load", data[0]);Pagination_GS_ADD_NEW_POPUP(attribute_value,len);if(data[0]==""){ $("#'
                    + str(TABLEIDS)
                    + ' tbody").html("<tr class=\'noRecDisp\'><td colspan=2 class=\'txtltimp\'>No Records to Display</td></tr>");$("#popup_footer").hide();} }); });'
                )
                
                #A055S000P01-20968 - m ends
                if key == 0:
                    sec_str += (
                        '<th data-field="'
                        + str(api_name_header)
        ...
        if str(ATTRIBUTE_VALUE_STR) != "":
            ...
            if RelTABEL_NAME != "QSTN_R_SYOBJR_80011":
                ...
                #     VAL_Obj = Sql.GetList(VAL_Str)                     
                    ...
                    #A055S000P01-20996 - m starts,INC09104771
                    VAL_Str = "SELECT "+ COLUMNS_NAME + " FROM " + TABLEID + "(NOLOCK) where " + ATTRIBUTE_VALUE_STR + where +' '+str(existing_condition)+" order by CpqTableEntryId OFFSET {offset_skip_count} ROWS FETCH NEXT {per_page} ROWS ONLY".format(offset_skip_count=OFFSET_SKIP_COUNT,per_page=FETCH_COUNT)
                    VAL_Obj = Sql.GetList(VAL_Str)
                    count_query=SqlHelper.GetFirst("SELECT COUNT(*) as cnt FROM " + str(TABLEID) + "(NOLOCK) where " + ATTRIBUTE_VALUE_STR + where +' '+str(existing_condition))
                    count_query=str(count_query.cnt)
                    #A055S000P01-20996 - m ends,INC09104771
                else:
                    VAL_Str = (
                        "SELECT top 1000 "
                        + str(COLUMNS_NAME)
