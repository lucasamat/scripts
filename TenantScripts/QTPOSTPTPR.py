...
class FpmPriceUpdate:
        ...
        for i in self.price:    
            ...
            for condition in i['conditions']:
                ...
                if condition['conditionTypeDescription'] == "Net Value 2":
                    ...
                    salesUOMflag=0 #INC08733295 - Start - M
                    if str(i["netPriceUnit"]) != str(isocode_salesuom) and int(float((i["netPriceUnitValue"]))) == 1:
                        self.ancliary_netPrice= float(self.ancliary_netPrice) * float(numerator)
                        salesUOMflag=1
                    elif str(i["netPriceUnit"]) != str(isocode_salesuom) and int(float(i["netPriceUnitValue"])) > 1:
                        self.ancliary_netValue = (float(self.ancliary_netPrice / int(float(self.ancliary_netValue)))) * float(numerator)  
                        salesUOMflag=1        
                    if salesUOMflag==1:
                        self.ancliary_netPrice=str(self.ancliary_netPrice) #INC08733295 - END - M
    ...
    def fpm_batchupdate(self):
        ...
        if GetEquipment_count.CNT > 0:
            ...
            if getstatus.CNT>0:
                ...
                CQCPQC4CWB.writeback_to_c4c("quote_header",self.quote_record_id,self.quote_revision_id)
                CQCPQC4CWB.writeback_to_c4c("opportunity_header",self.quote_record_id,self.quote_revision_id)
    ...
    def ancillary_batchupdate(self):
        ...
        if quote_items_list:
            #INC08858696 - Start - M
            calling_waterfall = ScriptExecutor.ExecuteGlobal("CQUPPRWLFD",{"Records":str(quote_items_list),"auto_update_flag":"True","quote_record_id":self.quote_record_id, "qterev_record_id":self.quote_revision_id})
            #INC08858696 - End - M
        #HPQC-312 ends
