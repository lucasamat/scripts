...
try :
	...
	while While_flag == 1:
		...
		replacedquery = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE SYINPL  SET integration_payload =  replace(replace(replace(integration_payload,''\\\\'',''\\''),''VALID_TP'',''VALID_TO''),''AGS_FAB_NAME'',''AGS_SEMI_FAB'')  from SYINPL(NOLOCK) WHERE INTEGRATION_NAME = ''INSTALLED BASE'' AND ISNULL(STATUS,'''')='''' '")
