...
def CommonTreeViewHTMLDetail(
    ...
    if TreeParam == "Add-On Products":
        ...
        service_id_query = Sql.GetFirst("SELECT SERVICE_ID FROM SAQSGB (NOLOCK) WHERE QUOTE_SERVICE_GREENBOOK_RECORD_ID = '{primary_value}'".format(primary_value = primary_value))
        # service_id_query is return none so added the if statment
        if service_id_query:
            Product.SetGlobal('addon_service_id',service_id_query.SERVICE_ID)
    try:
    ...
    #INC08642678-M
    elif SubtabName == 'Periods':
        ObjectName = "SAQRDS"
        # A055S000P01-20393 Start - M
        if MODE == 'EDIT':
            getrec_id = Sql.GetFirst("SELECT CpqTableEntryId FROM SAQRDS (NOLOCK) WHERE QUOTE_RECORD_ID = '"+str(Quote.GetGlobal("contract_quote_record_id"))+"' AND QTEREV_RECORD_ID = '"+str(quote_revision_record_id)+"' AND DELIVERY_PERIOD = '"+str(RECORD_ID)+"' ")
            RECORD_ID = "SAQRDS-"+str(getrec_id.CpqTableEntryId)
    ...
    if ObjectName != 'SAQRIB':
        sec_str += ' <div class="col-md-12"   id="alert_msg" style="display: none;"><div class="row modulesecbnr brdr" data-toggle="collapse" data-target="#Alertmsg8" aria-expanded="true">NOTIFICATIONS<i class="pull-right fa fa-chevron-down "></i><i class="pull-right fa fa-chevron-up"></i></div><div  id="Alertmsg8" class="col-md-12  alert-notification  brdr collapse in" ><div  class="col-md-12 alert-danger"  ><label ><img src="/mt/APPLIEDMATERIALS_PRD/Additionalfiles/stopicon1.svg" alt="Error">  </label></div></div></div>'
    ...
    # 	QStr1 = ("SELECT TOP 1000 SYSECT.* FROM SYSECT (NOLOCK) WHERE  SYSECT.PRIMARY_OBJECT_NAME = '" + str(ObjectName) + "' ORDER BY ABS(SYSECT.DISPLAY_ORDER)")
        ...
        QStr1 = ("SELECT TOP 1000 SYSECT.* FROM SYSECT (NOLOCK) WHERE  SYSECT.PRIMARY_OBJECT_NAME = '" + str(ObjectName) + "' ORDER BY ABS(SYSECT.DISPLAY_ORDER)")	
    # A055S000P01-20393 Start - M
    elif (ObjectName == "SAQRDS") and SubtabName == "Periods":
        Trace.Write('elee====334')
        QStr1 = ("SELECT TOP 1000 SYSECT.* FROM SYSECT (NOLOCK) WHERE SECTION_NAME != 'AUDIT INFORMATION' AND SYSECT.PRIMARY_OBJECT_NAME = 'SAQRDS' AND RECORD_ID !='AED0A92A-8644-46AE-ACF0-90D6E331E506' ORDER BY ABS(SYSECT.DISPLAY_ORDER)")
    # A055S000P01-20393 End - M
    else:
    ...
    for sec in section_obj:
        ...
        if action_visible_obj:
            try:
                if action_visible_obj.PRIMARY_OBJECT_RECORD_ID :
                    action_visible_str = action_visible_obj.PRIMARY_OBJECT_RECORD_ID 
            except:
                Trace.Write("Exception: Expando object has no attribute OBJECT_RECORD_ID")
        if ObjectName == "SYOBFD":
            editable_permission = "TRUE"
        revision_query =  Sql.GetFirst("SELECT REVISION_STATUS,WORKFLOW_STATUS FROM SAQTRV (NOLOCK) WHERE QUOTE_RECORD_ID = '"+str(contract_quote_record_id)+"' AND QTEREV_RECORD_ID = '"+str(quote_revision_record_id)+"'  ")

        editable_permission = "TRUE"
        if (str(revision_query.REVISION_STATUS).upper() in ("CBC-CBC COMPLETED","BOK-CONTRACT CREATED","BOK-CONTRACT BOOKED","LGL-LEGAL SOW ACCEPTED","LGL-LEGAL SOW REJECTED","LGL-PREPARING LEGAL SOW","CBC-PREPARING CBC")):
            editable_permission = 'False'
        elif (str(revision_query.REVISION_STATUS).upper() in ("CFG-ACQUIRING","PRR-ON HOLD PRICING","CFG-ON HOLD - COSTING","PRI-PRICING")) and str(TreeParam).upper() not in ("QUOTE ITEMS","BILLING","APPROVALS","QUOTE DOCUMENTS") and str(TreeParentParam).upper() not in ("QUOTE ITEMS","BILLING","APPROVALS","QUOTE DOCUMENTS"):
            editable_permission = 'False'
        elif (str(revision_query.REVISION_STATUS).upper() in ("APR-APPROVED","APR-REJECTED","APR-RECALLED","APR-APPROVAL PENDING","OPD-PREPARING QUOTE DOCUMENTS","OPD-CUSTOMER ACCEPTED","APR-SUBMITTED TO CUSTOMER","OPD-CUSTOMER REJECTED")) and str(TreeParam).upper() not in ("APPROVALS","QUOTE DOCUMENTS") and str(TreeParentParam).upper() not in ("APPROVALS","QUOTE DOCUMENTS"):
            editable_permission = 'False'
        if str(revision_query.REVISION_STATUS).upper() not in ("BOK-CONTRACT BOOKED","BOK-CONTRACT CREATED","CBC-CBC COMPLETED"):
            if str(TreeParam).upper() == "REVISIONS":
                editable_permission = "True"
            else:
                pass
        else:
        ...
        # query_value return none so we add the if statment
        if query_value: 
        ...
        # Billing Matrix Details Load - Start
        if ObjectName == 'SAQRIB':
            Quote.SetGlobal("contract_quote_current_record_id", str(RECORD_ID))
...
and GREENBOOK = '{}' AND FABLOCATION_ID = '{}' AND SERVICE_ID = '{}'""".format(quote_record_id,quote_revision_record_id,TreeParam,TreeParentParam,Product.GetGlobal("TreeParentLevel1").split(" ")[0].strip())
        ...
        try:
            ...
            Trace.Write("==========>>>> 1111"+str(script))
            Custom_obj = Sql.GetFirst(script)
            if ObjectName == "SAQSKP":
                Product.SetGlobal("Kit id", str(Custom_obj.KIT_NAME))
                Kit_id_value = Product.SetGlobal("Kit id")
                Trace.Write()
        ...
        if data_obj is not None:
            ...
            for val in data_obj:				
                ...
                if Custom_obj is not None:
                    try:
                        ...
                        if ObjectName == 'SAQICO':
                            current_obj_value = current_obj_value.replace(';#38','&')
                ...
                sec_str += (
                    ...
                    + str(current_obj_field_lable).replace('<','&lt;')
                ...
                if data_type == "AUTO NUMBER":					
                        ...
                        try:
                            if RECORD_ID.startswith(ObjectName):							
                                if str(ObjectName) != "USERS":
                                    ...
                                    Trace.Write(bbbbbbbbbbbbbbbbbbbbbbbbb)
                                    Trace.Write("current_obj_value"+str(current_obj_value))
                                    current_obj_value = RECORD_ID
                            else:
                                Trace.Write(cccccccccccccccccccc)
                                Trace.Write("current_obj_valuecccccccc"+str(current_obj_value))
                                current_obj_value = RECORD_ID
                        except:
                            Trace.Write('check error')
                    ...
                    sec_str += (
                        ...
                        + '" type="text"  onclick="'
                        + str(datepicker)
                        + '"  class="form-control datePickerField wth157fltltbrdbt datepickers light_yellow"   '
                        + disable
                        + " ></td>"
                    )
                    #INC08642678 End - M
                    ...
                    if current_obj_api_name=='CANCELLATION_PERIOD_NOTPER':
                        len_restrict= 'oninput="this.value=this.value.slice(0,this.maxLength)" maxlength="4"' #INC08790169
                    else :
                        len_restrict=""
                    if str(MODE) == "EDIT":				
                        sec_str += (
    ...
    # 					WHERE QUOTE_BILLING_PLAN_RECORD_ID = '{}'""".format(RECORD_ID))
        ...
        # 		notification = ' <div class="col-md-12" id="alert_msg" style="display: block;"><div class="row modulesecbnr brdr" data-toggle="collapse" data-target="#Alertmsg8" aria-expanded="true" >NOTIFICATIONS<i class="pull-right fa fa-chevron-down "></i><i class="pull-right fa fa-chevron-up"></i></div><div  id="Alertmsg8" class="col-md-12  alert-notification  brdr collapse in" ><div  class="col-md-12 alert-warning"  ><label ><img src="/mt/APPLIEDMATERIALS_PRD/Additionalfiles/warning1.svg" alt="Warning">  Changes have been made to the Quote Configuration. Please refresh the Billing Matrix.</label></div></div></div>'
...
# 									imgstr = ('<img title=Acquired src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/clock_exe.svg>')
# 								else:
# 									imgstr  = ""
...
# 	if GetCPSVersion:
# 		if GetCPSVersion.KB_VERSION is not None and GetCPSVersion.KB_VERSION != Fullresponse["kbKey"]["version"]:
# 			sec_str += '<div id="Headerbnr" class="mart_col_back disp_blk"><div class="col-md-12" id="PageAlert"><div class="row modulesecbnr brdr" data-toggle="collapse" data-target="#Alert_notifcatio6" aria-expanded="true">NOTIFICATIONS<i class="pull-right fa fa-chevron-down"></i><i class="pull-right fa fa-chevron-up"></i></div><div id="Alert_notifcatio6" class="col-md-12 alert-notification brdr collapse in"><div class="col-md-12 alert-info"><label title=" Information : The Knowledgebase of the VC Characteristics has been updated in CPS."><img src="/mt/APPLIEDMATERIALS_PRD/Additionalfiles/infocircle1.svg" alt="Info"> Information : The Knowledgebase of the VC Characteristics has been updated in CPS.</label></div></div></div></div>'
# 		else:
# 			sec_str = ''
# 	else:
# 		Trace.Write("GETCPS VERSION EMPTY!")'''	
