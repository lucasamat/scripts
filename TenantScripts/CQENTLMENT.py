...
class Entitlements:
                ...
                if Parentgetdata:					
                    ...
                    for row in Parentgetdata:
                        ...
                        if row.ENTITLEMENT_VALUE_CODE and row.ENTITLEMENT_VALUE_CODE not in ('undefined','None') and   row.ENTITLEMENT_ID !='undefined' and row.ENTITLEMENT_DISPLAY_VALUE !='select' and row.IS_DEFAULT =='0' and (ent_type.upper() not in ('VALUE DRIVER','VALUE DRIVER COEFFICIENT') or row.ENTITLEMENT_ID == "AGS_{}_VAL_SVCCMP".format(serviceId)):
                            if product_obj:
                                get_datatype = Sql.GetFirst("""SELECT STANDARD_ATTRIBUTE_DATA_TYPE 
                                                FROM TAB_PRODUCTS
                                                ...
                                                WHERE TAB_PRODUCTS.PRODUCT_ID = {ProductId} AND SYSTEM_ID = '{service_id}'""".format(ProductId = product_obj.PRD_ID,service_id = row.ENTITLEMENT_ID ))
                            ...
                            try:
                                ...
                                if row.ENTITLEMENT_TYPE in ('Check Box','CheckBox'):
                                    ...
                                    attr_value = str(row.ENTITLEMENT_VALUE_CODE)
                                    if get_datatype:
                                        if get_datatype.STANDARD_ATTRIBUTE_DATA_TYPE:
                                            if get_datatype.STANDARD_ATTRIBUTE_DATA_TYPE.upper() == 'DATE':
                                                attr_value = datetime.datetime.strptime(attr_value, "%m/%d/%Y").strftime("%Y-%m-%d")
                                    requestdata+= '{"value":"' +str(attr_value) + '","selected":true}]},'
                                requestdata += ']}'
    ...
    def EntitlementRequest(self,cpsConfigID=None,cpsmatchID=None,AttributeID=None,NewValue=None,field_type=None,attr_datatype = None,product_id = None):
            ...
            if field_type != 'input':
                ...
                if (not NewValue) and previous_val:	
                    if attr_datatype.upper() == 'DATE':
                        previous_val = datetime.datetime.strptime(previous_val, "%m/%d/%Y").strftime("%Y-%m-%d")				
        ...
        if (not cps_error) and response2:
            ...
            if response_temp['conflicts']:
                cps_error = str(response_temp['conflicts'])
    ...
    def EntitlementSave(self, subtabName, NewValue, AttributeID, AttributeValCode,SectionRecordId,EquipmentId,calc_factor,costimpact,priceimapct,getmaualipval,ENT_IP_DICT,scheduled_parts):
        ...
        previous_configuration_status = Product.GetGlobal('previous_config_status')
        Fullresponse =''
        cps_error = ''
        cps_conflict = ''
        get_curr = ''
        ...
        if EntitlementType == 'Dropdown':
                                        ...
                                        if prdvalue["possibleValues"]:
                                            for i in prdvalue["possibleValues"]:
                                                ...
                                                if i['selectable'] == 'false' and 'valueLow' in i.keys():
                                                    ...
                                                    get_attribute_value = Sql.GetFirst("SELECT V.STANDARD_ATTRIBUTE_DISPLAY_VAL, V.STANDARD_ATTRIBUTE_VALUE FROM PRODUCT_ATTRIBUTES PA INNER JOIN ATTRIBUTES A ON PA.PA_ID=A.PA_ID INNER JOIN STANDARD_ATTRIBUTE_VALUES V ON A.STANDARD_ATTRIBUTE_VALUE_CD = V.STANDARD_ATTRIBUTE_VALUE_CD INNER JOIN ATTRIBUTE_DEFN (NOLOCK) AD ON AD.STANDARD_ATTRIBUTE_CODE=V.STANDARD_ATTRIBUTE_CODE WHERE AD.SYSTEM_ID = '{}' AND PA.PRODUCT_ID ={} AND STANDARD_ATTRIBUTE_VALUE  ='{}'".format(prdvalue["id"], product_obj.PRD_ID, str(i['valueLow']) ))
                                                    
                                                    #INC08621345 - M
                                                    if (prdvalue["id"] in ('AGS_Z0091_NET_PRMALB','AGS_Z0091W_NET_PRMALB','AGS_Z0035_NET_PRMALB','AGS_Z0035W_NET_PRMALB') and self.treeparam in ('PDC','MPS') and get_attribute_value.STANDARD_ATTRIBUTE_DISPLAY_VAL == 'Included - Monthly and Above') or (prdvalue["id"] in ('AGS_Z0091_KPI_TGTKPI','AGS_Z0091W_KPI_TGTKPI','AGS_Z0092W_KPI_TGTKPI','AGS_Z0092_KPI_TGTKPI','AGS_Z0090_KPI_TGTKPI','AGS_Z0035W_KPI_TGTKPI' ) and self.treeparam != 'IMPLANT' and get_attribute_value.STANDARD_ATTRIBUTE_DISPLAY_VAL in ('Max wafer Output <= 4%','Max Wafer Output >4%')) or(prdvalue["id"] in ('AGS_Z0091_KPI_TGTKPI','AGS_Z0091W_KPI_TGTKPI','AGS_Z0092W_KPI_TGTKPI','AGS_Z0092_KPI_TGTKPI','AGS_Z0090_KPI_TGTKPI','AGS_Z0035W_KPI_TGTKPI' ) and self.treeparam not in ('MDP','DDP') and get_attribute_value.STANDARD_ATTRIBUTE_DISPLAY_VAL in ('Excursion Detection') ) or (prdvalue["id"] in ('AGS_Z0010_PQB_USRFEE') and self.treeparam not in ('CMP') and get_attribute_value.STANDARD_ATTRIBUTE_DISPLAY_VAL == 'AGS_Z0010_PQB_USRFEE_002' ) or (prdvalue["id"] in ('AGS_Z0091_KPI_PRPFGT','AGS_Z0091W_KPI_PRPFGT') and self.treeparam in ('PDC') and get_attribute_value.STANDARD_ATTRIBUTE_DISPLAY_VAL == "Std Srvc + All PM's" ) or (prdvalue["id"] == 'AGS_Z0009_TSC_CUOWPN' and self.treeparam  != 'DISPLAY' and get_attribute_value.STANDARD_ATTRIBUTE_DISPLAY_VAL == 'Yes' ) or (prdvalue["id"] == 'AGS_Z0099_TSC_CONSUM' and self.treeparam  not in ('PDC','MPS') and get_attribute_value.STANDARD_ATTRIBUTE_DISPLAY_VAL == 'Included' ) or (prdvalue["id"] == 'AGS_Z0099_NET_PRMALB' and self.treeparam  in ('PDC','MPS') and get_attribute_value.STANDARD_ATTRIBUTE_DISPLAY_VAL in ('Included - Quarterly and Above','Included - Monthly and Above','Excluded') ):
                                                    #INC08621345 - M
                                                        dropdowndisallowlist.append(str(prdvalue["id"])+'_'+str(i['valueLow'])	)
                                                    else:
                    ...
                    for tab in product_tabs_obj:
                        if tabwise_product_attributes.get(tab.TAB_PROD_ID):
                            for attribute in tabwise_product_attributes.get(tab.TAB_PROD_ID):								
                                ...
                                GetDefault = Sql.GetFirst("SELECT PRICE_METHOD FROM PRENVL (NOLOCK) WHERE ENTITLEMENT_ID = '{}' AND ENTITLEMENT_DISPLAY_VALUE = '{}'".format(str(attrSysId),attrValue))
                                    ...
                                    if prdvalue["possibleValues"]:
                                        for i in prdvalue["possibleValues"]:
                                            ...
                                            if i['selectable'] == 'false' and 'valueLow' in i.keys():
                                                ...
                                                get_attribute_value = Sql.GetFirst("SELECT V.STANDARD_ATTRIBUTE_DISPLAY_VAL, V.STANDARD_ATTRIBUTE_VALUE FROM PRODUCT_ATTRIBUTES PA INNER JOIN ATTRIBUTES A ON PA.PA_ID=A.PA_ID INNER JOIN STANDARD_ATTRIBUTE_VALUES V ON A.STANDARD_ATTRIBUTE_VALUE_CD = V.STANDARD_ATTRIBUTE_VALUE_CD INNER JOIN ATTRIBUTE_DEFN (NOLOCK) AD ON AD.STANDARD_ATTRIBUTE_CODE=V.STANDARD_ATTRIBUTE_CODE WHERE AD.SYSTEM_ID = '{}' AND PA.PRODUCT_ID ={} AND STANDARD_ATTRIBUTE_VALUE  ='{}'".format(prdvalue["id"], product_obj.PRD_ID, str(i['valueLow']) ))
                                                #INC08638247,INC08621345 - Start - M
                                                if (prdvalue["id"] in ('AGS_Z0091_NET_PRMALB','AGS_Z0091W_NET_PRMALB','AGS_Z0035_NET_PRMALB','AGS_Z0035W_NET_PRMALB') and self.treeparam in ('PDC','MPS') and get_attribute_value.STANDARD_ATTRIBUTE_DISPLAY_VAL == 'Included - Monthly and Above') or (prdvalue["id"] in ('AGS_Z0091_KPI_TGTKPI','AGS_Z0091W_KPI_TGTKPI','AGS_Z0092W_KPI_TGTKPI','AGS_Z0092_KPI_TGTKPI','AGS_Z0090_KPI_TGTKPI','AGS_Z0035W_KPI_TGTKPI' ) and self.treeparam != 'IMPLANT' and get_attribute_value.STANDARD_ATTRIBUTE_DISPLAY_VAL in ('Max wafer Output <= 4%','Max Wafer Output >4%')) or(prdvalue["id"] in ('AGS_Z0091_KPI_TGTKPI','AGS_Z0091W_KPI_TGTKPI','AGS_Z0092W_KPI_TGTKPI','AGS_Z0092_KPI_TGTKPI','AGS_Z0090_KPI_TGTKPI','AGS_Z0035W_KPI_TGTKPI' ) and self.treeparam not in ('MDP','DDP') and get_attribute_value.STANDARD_ATTRIBUTE_DISPLAY_VAL in ('Excursion Detection') ) or (prdvalue["id"] in ('AGS_Z0010_PQB_USRFEE') and self.treeparam not in ('CMP') and get_attribute_value.STANDARD_ATTRIBUTE_DISPLAY_VAL == 'AGS_{}_PQB_USRFEE_002'.format(ProductPartnumber) ) or (prdvalue["id"] in ('AGS_Z0091_KPI_PRPFGT','AGS_Z0091W_KPI_PRPFGT') and self.treeparam in ('PDC') and get_attribute_value.STANDARD_ATTRIBUTE_DISPLAY_VAL == "Std Srvc + All PM's" ) or (prdvalue["id"] == 'AGS_Z0009_TSC_CUOWPN' and self.treeparam  != 'DISPLAY' and get_attribute_value.STANDARD_ATTRIBUTE_DISPLAY_VAL == 'Yes' ) or (prdvalue["id"] == 'AGS_Z0099_TSC_CONSUM' and self.treeparam  not in ('PDC','MPS') and get_attribute_value.STANDARD_ATTRIBUTE_DISPLAY_VAL == 'Included' ) or (prdvalue["id"] == 'AGS_Z0099_NET_PRMALB' and self.treeparam  in ('PDC','MPS') and get_attribute_value.STANDARD_ATTRIBUTE_DISPLAY_VAL in ('Included - Quarterly and Above','Included - Monthly and Above','Excluded') ):
                                                #INC08638247,INC08621345 - End - M
                                                    dropdowndisallowlist.append(str(prdvalue["id"])+'_'+str(i['valueLow'])	)
                                                else:
                                                    dropdownallowlist.append(str(prdvalue["id"])+'_'+str(i['valueLow'])	)#dropdownallowlist_selected.append(str(prdvalue["id"])+'}}'+str(i['valueLow'])	)
                                                #dropdownallow[prdvalue["id"]] = dropdownallowlist
                ...
                for tab in product_tabs_obj:
                    if tabwise_product_attributes.get(tab.TAB_PROD_ID):
                        for attribute in tabwise_product_attributes.get(tab.TAB_PROD_ID):							
                            ...
                            if str(DType) == "Check Box":
                                STANDARD_ATTRIBUTE_VALUES=SqlHelper.GetList("SELECT V.STANDARD_ATTRIBUTE_DISPLAY_VAL, V.STANDARD_ATTRIBUTE_VALUE,PA.ATTRDESC FROM PRODUCT_ATTRIBUTES PA INNER JOIN ATTRIBUTES A ON PA.PA_ID=A.PA_ID INNER JOIN STANDARD_ATTRIBUTE_VALUES V ON A.STANDARD_ATTRIBUTE_VALUE_CD = V.STANDARD_ATTRIBUTE_VALUE_CD INNER JOIN ATTRIBUTE_DEFN (NOLOCK) AD ON AD.STANDARD_ATTRIBUTE_CODE=V.STANDARD_ATTRIBUTE_CODE WHERE AD.SYSTEM_ID = '{sys_id}' AND  PA.PRODUCT_ID ='{pid}'".format(sys_id = attrSysId,pid =product_obj.PRD_ID))
                                display_value_arr =[]
                                # if ent_val_code:
                                # 	display_value_arr =[]
                                # 	ent_chkbox_code = str(tuple(eval(ent_val_code))).replace(',)',')')
            ...
            if "calc" in AttributeID:
                ...
                for key,dict_val in ENT_IP_DICT.items():
                    display_value_arr = ''
                    ent_val_code = ''
                    if key != 'undefined' and dict_val.split("||")[3] != 'undefined' and str((dict_val).split("||")[0]).strip() != 'undefined':
                        ...
                        if str(serviceId) in ("Z0091","Z0004","Z0007","Z0006","Z0092","Z0035","Z0009","Z0090") and key in ( "AGS_{}_TSC_CONSUM".format(serviceId), "AGS_{}_TSC_NONCNS".format(serviceId), "AGS_{}_NON_CONSUMABLE".format(serviceId),"AGS_{}_TSC_RPPNNW".format(serviceId)) and str(tableName) in ('SAQSGE','SAQTSE'):
                        ...
                        if str(serviceId) == 'Z0092' and key == 'AGS_{}_TSC_CONADD'.format(serviceId):
                            ...
                            if entitlement_value in ("Some Inclusions","Included"):
                                ...
                                if entitlement_value != "Some Inclusions":
                                    ancillary_object_dict['Z0101'] = "DELETE"
                        #1866,1771
                        elif key == "AGS_{}_TSC_CUOWPN".format(serviceId) and serviceId in ("Z0091",'Z0092','Z0004','Z0009','Z0006','Z0090') :
                        ...
                        # #HPQC-539
                        elif key == 'AGS_{}_PQB_USRFEE'.format(serviceId) and self.treeparam == 'CMP' and tableName == 'SAQSGE':
                            if entitlement_value == "Included":
                                ancillary_object_dict['Z0103'] = "INSERT"							
                            else:
                                ancillary_object_dict['Z0103'] = "DELETE"
                        #(key == "AGS_{}_KPI_BPTKPI".format(serviceId) and serviceId in ("Z0035")) or
                        #INC08621345 - Start - M
                        if key in ( "AGS_{}_PQB_QTETYP".format(serviceId)):
                            Product.SetGlobal("Quotetype",str(entitlement_value))
                        
                        if str(serviceId) in  ("Z0009","Z0010","Z0128") and key in ( "AGS_{}_PQB_QTETYP".format(serviceId)) and str(tableName) in ('SAQTSE'):
                        #INC08621345 - End - M
                            if entitlement_value in ("Event Based","Flex Event Based"):
                                try:
                                    if str(serviceId) in ("Z0009","Z0010","Z0128"):
                                        check_assembly = True
                                        if check_assembly:
                                            #CR357B
                                            Sql.RunQuery("UPDATE SAQSCO SET IBASE_ATTSTS = 'INCOMPLETE' FROM (SELECT DISTINCT A.EQUIPMENT_ID FROM SAQSCA A JOIN MAEQUP B ON  B.PAR_EQUIPMENT_ID = A.EQUIPMENT_ID AND B.EQUIPMENT_ID = A.ASSEMBLY_ID WHERE A.QUOTE_RECORD_ID = '"+str(self.ContractRecordId)+"' AND A.QTEREV_RECORD_ID = '"+str(self.revision_recordid)+"' AND A.SERVICE_ID = '"+str(serviceId)+"' AND B.EQUIPMENTTYPE_DESCRIPTION = 'CHAMBER' AND A.GREENBOOK IN ('EPI','ETCH','ALD','DSM','FEP','MDP','PPC','SRP','MPKG','DISPLAY') AND ISNULL(B.PROCESS_TYPE,'') = '') equp WHERE QUOTE_RECORD_ID = '"+str(self.ContractRecordId)+"' AND QTEREV_RECORD_ID = '"+str(self.revision_recordid)+"' AND SERVICE_ID = '"+str(serviceId)+"' AND SAQSCO.EQUIPMENT_ID = equp.EQUIPMENT_ID  AND SAQSCO.IBASE_ATTSTS = 'COMPLETE'")

                                            Sql.RunQuery("UPDATE SAQSCO SET IBASE_ATTSTS = 'INCOMPLETE' FROM (SELECT DISTINCT A.EQUIPMENT_ID FROM SAQSCA A JOIN MAEQUP B ON  B.PAR_EQUIPMENT_ID = A.EQUIPMENT_ID AND B.EQUIPMENT_ID = A.ASSEMBLY_ID WHERE A.QUOTE_RECORD_ID = '"+str(self.ContractRecordId)+"' AND A.QTEREV_RECORD_ID = '"+str(self.revision_recordid)+"' AND A.SERVICE_ID = '"+str(serviceId)+"' AND B.EQUIPMENTTYPE_DESCRIPTION = 'POLISHER' AND A.GREENBOOK IN ('CMP') AND ISNULL(B.PROCESS_TYPE,'') = '') equp WHERE QUOTE_RECORD_ID = '"+str(self.ContractRecordId)+"' AND QTEREV_RECORD_ID = '"+str(self.revision_recordid)+"' AND SERVICE_ID = '"+str(serviceId)+"' AND SAQSCO.EQUIPMENT_ID = equp.EQUIPMENT_ID  AND SAQSCO.IBASE_ATTSTS = 'COMPLETE'")

                                            Sql.RunQuery("UPDATE SAQSCO SET IBASE_ATTSTS = 'INCOMPLETE' FROM (SELECT DISTINCT A.EQUIPMENT_ID FROM SAQSCO A JOIN MAEQUP B ON A.EQUIPMENT_ID = B.EQUIPMENT_ID WHERE A.QUOTE_RECORD_ID = '"+str(self.ContractRecordId)+"' AND A.QTEREV_RECORD_ID = '"+str(self.revision_recordid)+"' AND A.SERVICE_ID = '"+str(serviceId)+"' AND ISNULL(B.DEVICE_NODE,'') = '') equp WHERE QUOTE_RECORD_ID = '"+str(self.ContractRecordId)+"' AND QTEREV_RECORD_ID = '"+str(self.revision_recordid)+"' AND SERVICE_ID = '"+str(serviceId)+"' AND SAQSCO.EQUIPMENT_ID = equp.EQUIPMENT_ID  AND SAQSCO.IBASE_ATTSTS = 'COMPLETE'")
                                            #CR357B
                                ...
                                #INC08700058
                                #CR681 - A055S000P01-20700 - Start - M
                                if str(serviceId) not in ("Z0010","Z0128"):
                                    Sql.RunQuery("UPDATE A SET A.IBASE_ATTSTS = B.IBASE_ATTSTS  FROM SAQSCO A JOIN SAQFEQ B ON A.QUOTE_RECORD_ID =B.QUOTE_RECORD_ID  AND A.QTEREV_RECORD_ID  =B.QTEREV_RECORD_ID AND A.FABLOCATION_ID  = B.FABLOCATION_ID AND A.GREENBOOK  = B.GREENBOOK WHERE A.IBASE_ATTSTS != 'REQUIRED EVENT IS MISSING' AND A.QUOTE_RECORD_ID ='"+str(self.ContractRecordId)+"' AND A.QTEREV_RECORD_ID='"+str(self.revision_recordid)+"' AND A.SERVICE_ID ='"+str(serviceId)+"'")
                                #CR681 - A055S000P01-20700 - End - M
                                #INC08700058
                        Trace.Write("PMevents changes started "+str(key)+" - "+str(tableName))
                        if str(serviceId) in ("Z0090") and key in ( "AGS_{}_PQB_QTETYP".format(serviceId)) and str(tableName) in ('SAQTSE'):
                            Trace.Write("### Quote Type -->"+str(entitlement_value))
                            if str(entitlement_value).upper() == "CHAMBER BASED":
                                Sql.RunQuery("UPDATE SAQSCA SET INCLUDED = 'True' WHERE QUOTE_RECORD_ID = '{QuoteRecordId}' AND QTEREV_RECORD_ID = '{RevisionRecordId}' and SERVICE_ID = '{service_id}' ".format(QuoteRecordId = self.ContractRecordId,RevisionRecordId=self.revision_recordid,service_id = serviceId))
                        elif "GEN_IDLALW" in key and entitlement_value == "Yes":
                            ...
                            if str(Quote.GetGlobal("ConsignedQty")) != "":
                                ent_value = str(Quote.GetGlobal("ConsignedQty"))
                            else:
                                ent_value = 10
                            if int(entitlement_value) >= int(ent_value):
                                try:
                        ...
                        if ent_disp_val in ('select'):
                            ent_disp_val =''
                        if ent_val_code in ('0') and str((dict_val).split("||")[2]) != "DropDown":
                            ent_val_code =''
                        updateentXML  += """<QUOTE_ITEM_ENTITLEMENT>
                            <ENTITLEMENT_ID>{ent_name}</ENTITLEMENT_ID>
                            <ENTITLEMENT_VALUE_CODE>{ent_val_code}</ENTITLEMENT_VALUE_CODE>
                ...
                if Quote.GetGlobal("IdlingAllowed") == 'Yes':
                    Quote.SetGlobal("IdlingAllowed","No")
                    #Log.Info("TOOL IDLING VALUE"+str(entitlement_value))
                    #Trace.Write("tool idling flag--"+str(idling_flag))
                    #if idling_flag == 'FALSE':
                    #if entitlement_value == "Yes" :
                    #A055S000P01-20807
                    #INC08942741 M
                    GetSAQTDA = Sql.GetFirst("SELECT CpqTableEntryId FROM SAQTDA (NOLOCK) WHERE QTEREV_RECORD_ID= '{}' AND QUOTE_RECORD_ID ='{}'".format(self.revision_recordid,self.ContractRecordId))
                    #INC08942741 M
                    getQuoteDetails = Sql.GetFirst("SELECT QUOTE_ID, QTEREV_ID FROM SAQTRV (NOLOCK) WHERE QUOTE_REVISION_RECORD_ID = '{}' AND QUOTE_RECORD_ID ='{}'".format(self.revision_recordid,self.ContractRecordId))

                    if getQuoteDetails:
                        QuoteId = getQuoteDetails.QUOTE_ID
                        #QuoteRecordId = getQuoteDetails.QUOTE_RECORD_ID
                        QuoteRevisionId = getQuoteDetails.QTEREV_ID
                    if GetSAQTDA is None:
                        ...
                        for x,y in VALUES.items():
                            if "28 Days" in y or "30 Days" in y:
                                ...
                                #Log.Info("sp_executesql @T=N'INSERT SAQTDA( QUOTE_REV_TOOL_IDLING_ATTR_VAL_RECORD_ID, QUOTE_ID, QUOTE_RECORD_ID, QTEREV_ID, QTEREV_RECORD_ID, TOLIDLVAL_RECORD_ID, TOOLIDLING_DISPLAY_VALUE, TOOLIDLING_ID, TOOLIDLING_NAME, TOOLIDLING_RECORD_ID, TOOLIDLING_VALUE_CODE, CPQTABLEENTRYADDEDBY, CPQTABLEENTRYDATEADDED ) SELECT CONVERT(VARCHAR(4000),NEWID()), ''{}'' AS QUOTE_ID, ''{}'' AS QUOTE_RECORD_ID, ''{}'' AS QTEREV_ID, ''{}'' AS QTEREV_RECORD_ID, PRTIAV.TOLIDLATTVAL_RECORD_ID, PRTIAV.TOOLIDLING_DISPLAY_VALUE, PRTIAV.TOOLIDLING_ID, PRTIAV.TOOLIDLING_NAME, PRTIAV.TOOLIDLING_RECORD_ID, PRTIAV.TOOLIDLING_VALUE_CODE, ''{}'' AS CPQTABLEENTRYADDEDBY, GETDATE() AS CPQTABLEENTRYDATEADDED FROM PRTIAV (NOLOCK) WHERE TOOLIDLING_VALUE_CODE = N''{}'' AND TOOLIDLING_ID = ''{}'' '".format(QuoteId,self.ContractRecordId,QuoteRevisionId,self.revision_recordid,User.UserName,y.encode('utf-8').decode('utf-8'),x))
                            else:    
                        ...
                        #INC09011829 - M
                        getCount = Sql.GetFirst("SELECT COUNT(SERVICE_ID) as cnt FROM SAQTSV(NOLOCK) WHERE QTEREV_RECORD_ID = '{}' AND QUOTE_RECORD_ID ='{}' AND SERVICE_ID != '{}'  AND SERVICE_ID != 'A6200'".format(self.revision_recordid,self.ContractRecordId,serviceId))
                        getServices = Sql.GetList("SELECT SERVICE_ID FROM SAQTSV(NOLOCK) WHERE QTEREV_RECORD_ID = '{}' AND QUOTE_RECORD_ID ='{}' AND SERVICE_ID != '{}' AND SERVICE_ID != 'A6200'".format(self.revision_recordid,self.ContractRecordId,serviceId))
                        #INC09011829 - M
                        idling_flag = 0
                        if getServices:
                            for service in getServices:
                                #INC08942741 M
                                getIdlingVal = Sql.GetFirst("select CpqTableEntryId from saqsce where QTEREV_RECORD_ID = '{}' AND QUOTE_RECORD_ID ='{}' AND SERVICE_ID = '{}' AND ISNULL(IDLING_ALLOWED,'No') = 'No'".format(self.revision_recordid,self.ContractRecordId,service.SERVICE_ID))
                                #INC08942741 M
                                if getIdlingVal:
                                    idling_flag += 1
                        if idling_flag == getCount.cnt:
                            Sql.RunQuery("DELETE FROM SAQTDA WHERE QTEREV_RECORD_ID = '{}' AND QUOTE_RECORD_ID ='{}'".format(Quote.GetGlobal("quote_revision_record_id"),self.ContractRecordId))
                ...
                if self.treeparam == 'Add-On Products':
                    Sql.RunQuery("UPDATE SAQTSE SET CONFIGURATION_STATUS = '{}' WHERE QUOTE_RECORD_ID = '{}' AND QTEREV_RECORD_ID = '{}' AND PAR_SERVICE_ID = '{}'  AND SERVICE_ID = '{}'  ".format(configuration_status,self.ContractRecordId,self.revision_recordid,par_serviceId, serviceId) )				
                ...
                if serviceId in ('Z0009','Z0010','Z0128'):
                ...
                remodified_value = re.compile(r'<ENTITLEMENT_ID>AGS_'+str(Service_Id)+'[^>]*?_TSC_SCPT</ENTITLEMENT_ID>')
                ...
                for m in re.finditer(quote_item_tag, entitlement_xml):
                    ...
                    scheduled_parts =re.findall(remodified_value,sub_string)
        ...
        dataent = ""
        #INC08621345 - Start - M
        where_cond = " WHERE QUOTE_RECORD_ID = '{}' AND QTEREV_RECORD_ID = '{}' AND SERVICE_ID = '{}'".format(self.ContractRecordId,self.revision_recordid,serviceId)
        ScriptExecutor.ExecuteGlobal("CQENTLNVAL", {'action':'ENTITLEMENT_COLUMN_UPDATE',
                                                                            'partnumber':serviceId,
                                                                            'where_cond' :where_cond, 
                                                                            'ent_level_table': tableName
                                                                            })
        get_updated_ent = Sql.GetFirst("SELECT * FROM {} WHERE {}".format(tableName,whereReq ))
        pm_labor = ''
        cm_labor = ''
        quote_type = '' 
        if get_updated_ent.PMLAB_ENT:
            pm_labor = get_updated_ent.PMLAB_ENT
        if get_updated_ent.CMLAB_ENT:
            cm_labor = get_updated_ent.CMLAB_ENT
        if (get_updated_ent.QTETYP if tableName != 'SAQTSE' else get_updated_ent.QUOTE_TYPE ) :
            quote_type = get_updated_ent.QTETYP if tableName != 'SAQTSE' else get_updated_ent.QUOTE_TYPE
        prev_quote_type = Product.GetGlobal("Quotetype")
        Product.SetGlobal("Quotetype","")
        if ((Entitlement_edit_value in ( "AGS_{}_NET_PRMALB".format(serviceId),  "AGS_{}_NET_WECLLB".format(serviceId),  "AGS_{}_NET_CRMALB".format(serviceId)) ) or (Entitlement_edit_value in "AGS_{}_PQB_QTETYP".format(serviceId) and serviceId in ('Z0009','Z0010','Z0128') and prev_quote_type.upper() == 'TOOL BASED'  )) and ( (pm_labor or cm_labor) and quote_type ):
            ##To get the quote type of the attribute to delete the events table and their child tables based on the quote type...
            if str(quote_type).upper() == "TOOL BASED":
                delete_obj_list = ["SAQSAP","SAQSKP"]
            else:
                delete_obj_list = ["SAQSKP","SAQGPA","SAQGPE","SAQGPM"]
            for object in delete_obj_list:
                if object in ('SAQSAP','SAQSKP') and tableName == 'SAQSGE' and str(quote_type).upper() == "TOOL BASED":
                    Sql.RunQuery("DELETE SAQSAP FROM SAQSAP(NOLOCK) WHERE SAQSAP.QUOTE_RECORD_ID = '{}' AND SAQSAP.QTEREV_RECORD_ID = '{}' AND SAQSAP.SERVICE_ID = '{}' AND EQUIPMENT_ID IN  (SELECT EQUIPMENT_ID FROM SAQSCO WHERE {}) ".format(self.ContractRecordId,self.revision_recordid,serviceId,whereReq))
                elif object in ('SAQSAP','SAQSKP') and tableName == 'SAQSCE' and str(quote_type).upper() == "TOOL BASED":
                    Sql.RunQuery("DELETE SAQSAP FROM SAQSAP(NOLOCK) WHERE SAQSAP.QUOTE_RECORD_ID = '{}' AND SAQSAP.QTEREV_RECORD_ID = '{}' AND SAQSAP.SERVICE_ID = '{}' AND EQUIPMENT_ID = '{}'".format(self.ContractRecordId,self.revision_recordid,serviceId,VALUE))
                else:
                    if object =='SAQSKP' and  str(quote_type).upper() != "TOOL BASED" and tableName == "SAQSGE":
                    ...
                    elif object =='SAQSKP' and  str(quote_type).upper() != "TOOL BASED" and tableName == "SAQSCE":
                        Sql.RunQuery("DELETE SAQSKP FROM SAQSKP(NOLOCK) JOIN SAQGPM ON SAQSKP.QTEGBKPME_RECORD_ID = SAQGPM.QUOTE_REV_PO_GBK_GOT_CODE_PM_EVENTS_RECORD_ID WHERE SAQSKP.QUOTE_RECORD_ID = '{}' AND SAQSKP.QTEREV_RECORD_ID = '{}' AND SAQSKP.SERVICE_ID = '{}' AND SAQSKP.EQUIPMENT_ID = '{}' ".format(self.ContractRecordId,self.revision_recordid,serviceId,VALUE))
                    elif object =='SAQGPM' and  str(quote_type).upper() != "TOOL BASED" and tableName == "SAQSCE":
                        ...
                        Sql.RunQuery("DELETE FROM {} WHERE {}".format(object,whereReq))
                       
            try:
                pm_entlmnt_val = str(pm_labor)+"||"+str(cm_labor)
                ScriptExecutor.ExecuteGlobal(
                        "CQCRUDOPTN",
                    {
                        "NodeType"   : "COVERED OBJ MODEL",
                        ...
                        "pm_entlmnt_val" : pm_entlmnt_val,
                        "entitlement_level":LEVEL,
                        "entitlement_level_value":VALUE,
                        "ent_quote_type" : str(quote_type).upper()
                    },
                )
            except Exception as e:
                Log.Info("Exception in triggering real time call "+str(e))
        #INC08621345 - End - M
        ...
        try:
            ...
            if customer_wants_participate == 'No':
                ...
                ScriptExecutor.ExecuteGlobal('CQPARTSINS',{"CPQ_Columns":{"Action": "LoadParts","QuoteID":saqtse_obj.QUOTE_ID}})
        ...
        if cps_conflict:
            ...
            # 	# msg_text = (
            # 	# 	'<div class="col-md-12" id="entitlement-info"><div class="col-md-12 alert-info"><label> <img src="/mt/APPLIEDMATERIALS_PRD/Additionalfiles/infocircle1.svg" alt="Info"> '
            # 	# 	+ str(Fullresponse['conflicts'][0]['explanation'])
            # 	# 	+ "</label></div></div>"
            # 	# )
            try:
                msg_text = '<div class="emp_notifiy" style="display: none;"><div class="col-md-12 page_alert_notifi" id="PageAlert"><div class="row modulesecbnr brdr" onclick="call_vertical_scrl()" data-toggle="collapse" data-target=".alertnotify" aria-expanded="true">NOTIFICATIONS<i class="pull-right fa fa-chevron-down "></i><i class="pull-right fa fa-chevron-up"></i></div><div id="alertnotify" class="col-md-12 alertnotify alert-notification  brdr collapse in"><div class="col-md-12" id="entitlement-info"><div class="col-md-12 alert-info"><label> <img src="/mt/APPLIEDMATERIALS_PRD/Additionalfiles/infocircle1.svg" alt="Info"> '+str(cps_conflict)+' </label></div></div> </div></div></div>'
            except Exception as e:
    ...
    def Rolldown(self):
        ...
        if ENT_IP_DICT != '':
            ...
            try:
                Log.Info("triggered roll down iflw--"+str(tableName))				
    ...
    def popup(self):
        ...
        sec_str += """<div id="container_service" class="drop-boxes" style="display: none;">
                    ...
                    <div class="col-md-2 p-0">
                        <img src="/mt/APPLIEDMATERIALS_PRD/Additionalfiles/info_icon.svg" class="img-responsive center-block">
                    </div>
...
try:
    ...
    scheduled_parts = ""
try:
    Entitlement_edit_value = Param.Entitlement_edit_value
except:
    Entitlement_edit_value =''
