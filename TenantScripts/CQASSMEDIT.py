...
def save_assembly_level(included_value,fab_id,equipment_id,assembly_id,selected_value):
    ...
    if prod_type == 'FTS':
        ...
        elif TreeTopSuperTopParentParam == 'Product Offerings':
            service_id = TreeTopSuperParentParam
        else:
            service_id = TreeParam
        #if selected_value:
        if subtab_name.upper() == 'EVENTS':
            join_condition = " AND QUOTE_REV_PO_GRNBK_PM_EVEN_ASSEMBLIES_RECORD_ID = '{}'".format( CPQID.KeyCPQId.GetKEYId(str(table_name), str(selected_value)) )	
        Sql.RunQuery("UPDATE {table_name} SET INCLUDED = {included_value} WHERE QUOTE_RECORD_ID= '{ContractRecordId}' AND QTEREV_RECORD_ID ='{revision_record_id}' AND EQUIPMENT_ID = '{equipment_id}' AND ASSEMBLY_ID = '{assembly_id}' AND SERVICE_ID ='{service_id}' {join_condition}".format(included_value =1 if included_value == True else 0 ,ContractRecordId=ContractRecordId,revision_record_id=revision_record_id,equipment_id=equipment_id,assembly_id=assembly_id, table_name = table_name, join_condition= join_condition, service_id =service_id))
        ##To update PM Frequency value based on the Included value updated in the SAQGPA table...
        Sql.RunQuery("""UPDATE SAQGPM
                        SET
                        PM_FREQUENCY = assembly.PM_FREQUENCY,
                        SSCM_PM_FREQUENCY = assembly.SSCM_PM_FREQUENCY
                        FROM SAQGPM (NOLOCK)
                        INNER JOIN (select SAQGPA.INCLUDED,SAQGPA.QTEREVPME_RECORD_ID,SAQGPA.QUOTE_RECORD_ID,SAQGPA.GREENBOOK_RECORD_ID,SAQGPA.QTEREV_RECORD_ID,SAQGPA.SERVICE_RECORD_ID,SAQGPA.GOTCODE_RECORD_ID,SAQGPA.PM_RECORD_ID,SAQGPA.DEVICE_NODE,SAQGPA.PROCESS_TYPE,SUM(ISNULL(SAQGPA.PM_FREQUENCY, 0)) as PM_FREQUENCY,SUM(ISNULL(SAQGPA.SSCM_PM_FREQUENCY, 0)) as SSCM_PM_FREQUENCY FROM SAQGPA where SAQGPA.QUOTE_RECORD_ID = '{QuoteRecordId}' and SAQGPA.QTEREV_RECORD_ID = '{RevisionRecordId}' AND SAQGPA.SERVICE_ID = '{TreeParam}' GROUP BY QUOTE_RECORD_ID,QTEREV_RECORD_ID,SERVICE_RECORD_ID,GOTCODE_RECORD_ID,PM_RECORD_ID,DEVICE_NODE,PROCESS_TYPE,GREENBOOK_RECORD_ID,QTEREVPME_RECORD_ID,INCLUDED) assembly ON SAQGPM.QUOTE_RECORD_ID = assembly.QUOTE_RECORD_ID AND SAQGPM.QTEREV_RECORD_ID = assembly.QTEREV_RECORD_ID  AND SAQGPM.QUOTE_REV_PO_GBK_GOT_CODE_PM_EVENTS_RECORD_ID = assembly.QTEREVPME_RECORD_ID WHERE SAQGPM.QUOTE_RECORD_ID = '{QuoteRecordId}' and SAQGPM.QTEREV_RECORD_ID = '{RevisionRecordId}' AND SAQGPM.SERVICE_ID = '{TreeParam}' AND assembly.INCLUDED = 'True' """.format(QuoteRecordId=ContractRecordId,RevisionRecordId=revision_record_id,TreeParam=service_id))

    
    else:
        Sql.RunQuery("UPDATE SAQSCA SET INCLUDED = {included_value} WHERE QUOTE_RECORD_ID= '{ContractRecordId}' AND QTEREV_RECORD_ID ='{revision_record_id}' AND EQUIPMENT_ID = '{equipment_id}' AND ASSEMBLY_ID = '{assembly_id}' AND SERVICE_ID ='{service_id}' ".format(included_value =1 if included_value == True else 0 ,ContractRecordId=ContractRecordId,revision_record_id=revision_record_id,equipment_id=equipment_id,assembly_id=assembly_id,service_id =TreeParam))	

    return True

def save_nso(selected_nso,included_value):
    Trace.Write(str(included_value)+'-'+str(selected_nso)+'-')
    cpq_id = str(selected_nso).split("-")[1]
    Sql.RunQuery("UPDATE SAQSCN SET INCLUDED = {included_value} WHERE QUOTE_RECORD_ID= '{ContractRecordId}' AND QTEREV_RECORD_ID ='{revision_record_id}' AND CpqTableEntryId = '{cpq_id}'".format(included_value =1 if included_value == True else 0 ,ContractRecordId=ContractRecordId,revision_record_id=revision_record_id,cpq_id=cpq_id))
...
#INC08700581
def save_assembly_level_all(included_value,prod_type,subtab_name):
    Trace.Write('533-included_value--'+str(included_value))
    if prod_type == 'TKMALL':
        table_name = 'SAQGPA'
        if TreeTopSuperParentParam == 'Product Offerings':
            service_id = TreeParentParam
            ...
            service_id = TreeParam
        if subtab_name.upper() == 'EVENTS':
            Trace.Write('533--service_id----'+str(service_id))
            Trace.Write("UPDATE {table_name} SET INCLUDED = {included_value} WHERE QUOTE_RECORD_ID= '{ContractRecordId}' AND QTEREV_RECORD_ID ='{revision_record_id}'AND SERVICE_ID ='{service_id}'".format(included_value =1 if included_value == True else 0 ,ContractRecordId=ContractRecordId,revision_record_id=revision_record_id,table_name = table_name, service_id =service_id))
            if where_events == "" or where_events is None:
                Sql.RunQuery("UPDATE {table_name} SET INCLUDED = {included_value} WHERE QUOTE_RECORD_ID= '{ContractRecordId}' AND QTEREV_RECORD_ID ='{revision_record_id}'AND SERVICE_ID ='{service_id}' and ( PM_FREQUENCY_EDITABLE = 'True' OR PM_FREQUENCY_EDITABLE = '1' ) ".format(included_value =1 if included_value == True else 0 ,ContractRecordId=ContractRecordId,revision_record_id=revision_record_id,table_name = table_name, service_id =service_id))
            else:
                Sql.RunQuery("UPDATE {table_name} SET INCLUDED = {included_value} WHERE {where}".format(included_value =1 if included_value == True else 0 ,table_name = table_name,where=where_events ))
            Sql.RunQuery("""UPDATE SAQGPM
                        SET
                        PM_FREQUENCY = assembly.PM_FREQUENCY,
                        SSCM_PM_FREQUENCY = assembly.SSCM_PM_FREQUENCY
                        FROM SAQGPM (NOLOCK)
                        INNER JOIN (select SAQGPA.INCLUDED,SAQGPA.QTEREVPME_RECORD_ID,SAQGPA.QUOTE_RECORD_ID,SAQGPA.GREENBOOK_RECORD_ID,SAQGPA.QTEREV_RECORD_ID,SAQGPA.SERVICE_RECORD_ID,SAQGPA.GOTCODE_RECORD_ID,SAQGPA.PM_RECORD_ID,SAQGPA.DEVICE_NODE,SAQGPA.PROCESS_TYPE,SUM(ISNULL(SAQGPA.PM_FREQUENCY, 0)) as PM_FREQUENCY,SUM(ISNULL(SAQGPA.SSCM_PM_FREQUENCY, 0)) as SSCM_PM_FREQUENCY FROM SAQGPA (NOLOCK) where SAQGPA.QUOTE_RECORD_ID = '{QuoteRecordId}' and SAQGPA.QTEREV_RECORD_ID = '{RevisionRecordId}' AND SAQGPA.SERVICE_ID = '{TreeParam}' GROUP BY QUOTE_RECORD_ID,QTEREV_RECORD_ID,SERVICE_RECORD_ID,GOTCODE_RECORD_ID,PM_RECORD_ID,DEVICE_NODE,PROCESS_TYPE,GREENBOOK_RECORD_ID,QTEREVPME_RECORD_ID,INCLUDED) assembly ON SAQGPM.QUOTE_RECORD_ID = assembly.QUOTE_RECORD_ID AND SAQGPM.QTEREV_RECORD_ID = assembly.QTEREV_RECORD_ID  AND SAQGPM.QUOTE_REV_PO_GBK_GOT_CODE_PM_EVENTS_RECORD_ID = assembly.QTEREVPME_RECORD_ID WHERE SAQGPM.QUOTE_RECORD_ID = '{QuoteRecordId}' and SAQGPM.QTEREV_RECORD_ID = '{RevisionRecordId}' AND SAQGPM.SERVICE_ID = '{TreeParam}' AND assembly.INCLUDED = 'True' """.format(QuoteRecordId=ContractRecordId,RevisionRecordId=revision_record_id,TreeParam=service_id))
    return True
#INC08700581
...
try:
    ...
    included_value = ''
try:
    selected_nso = Param.selected_nso
except:
    selected_nso = ''
try:
    equipment_id = Param.equipment_id
...
try:
    ...
    prod_type = ''
try:
    where_events = Param.WHERE_CONDITION
except:
    where_events = ''
...
elif ACTION == "SAVE_NSO":
    ApiResponse = ApiResponseFactory.JsonResponse(save_nso(selected_nso,included_value))
#INC08700581
elif ACTION == 'SAVE_ASSEMBLY_ALL':
    ApiResponse = ApiResponseFactory.JsonResponse(save_assembly_level_all(included_value,prod_type,subtab_name))
#INC08700581
# try:
