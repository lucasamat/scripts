...
# Affected Line : 143 API response code modified
# Affected Table: None
# ==========================================================================================
						...
						if tn in rebuilt_data:
							if str(tn).upper() == "SAACNT":
								...
								for record_dict in Tbl_data:
									...
									for col in ['ACCOUNT_ID','PARTNER_ID','DISTRIBUTIONCHANNEL_ID','DIVISION_ID','SALESORG_ID','RANK','PARTNERFUNCTION_ID','IS_DEFAULT']:
										
										if col not in record_dict:
											record_dict[col] = ''					
																		
									primaryQueryItems = SqlHelper.GetFirst( ""+ str(Parameter.QUERY_CRITERIA_1)+ " SAACNT_INBOUND (DISTRIBUTIONCHANNEL_ID,PARTNERFUNCTION_ID,SALESORG_ID,ACCOUNT_ID,DIVISION_ID,RANK,PARTNER_ID,INTEGRATION_OBJECT,cpqtableentrydatemodified,IS_DEFAULT,SESSION_ID)  select  ''"+str(record_dict['DISTRIBUTIONCHANNEL_ID'])+"'',N''"+str(record_dict['PARTNERFUNCTION_ID'])+ "'',''"+str(record_dict['SALESORG_ID'])+ "'',''"+str(record_dict['ACCOUNT_ID'])+ "'',''"+str(record_dict['DIVISION_ID'])+ "'',''"+str(record_dict['RANK'])+ "'',''"+str(record_dict['PARTNER_ID'])+ "'',''SAACPF'',''"+ str(Modi_date)+ "'',''"+str(record_dict['IS_DEFAULT'])+ "'',''"+ str(primaryQuerysession.A)+ "'' ' ")
