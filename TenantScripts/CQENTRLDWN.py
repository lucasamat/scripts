...
def sendEmail(level):
    ...
    #A055S000P01-20800 START
    getQuoteId = Sql.GetFirst("SELECT SAQTMT.QUOTE_ID,SAQTMT.QTEREV_ID,SAQTMT.OWNER_ID,users.EMAIL FROM SAQTMT (nolock) inner join users (nolock) on SAQTMT.OWNER_ID = users.USERNAME WHERE MASTER_TABLE_QUOTE_RECORD_ID = '{}'".format(quote))
    quotecreatedby = Sql.GetFirst("select EMAIL FROM SAQDLT (nolock) where QUOTE_RECORD_ID = '"+str(quote)+"' and C4C_PARTNERFUNCTION_ID  = 'CREATED BY'")
    ...
    ownerEmail = str(getQuoteId.EMAIL)
    quoteCreatorEmail = str(quotecreatedby.EMAIL)
    ...
    #Log.Info()
    from_email = ''
    from_email += 'noreply@calliduscloud.com'
    ...
    msg.From = MailAddress(from_email)
    if ownerEmail != quoteCreatorEmail:
        msg.To.Add(ownerEmail)
        msg.To.Add(quoteCreatorEmail)
    else:
        msg.To.Add(ownerEmail)
    # Set message subject and body
    msg.Subject = str(level)+" Rolldown P-Tenant"
    msg.IsBodyHtml = True
    msg.Body = Error_Info
    ...
    copyEmail3 = MailAddress("namrata_sivakumar@contractor.amat.com")
    ...
    #Send the message
    mailClient.Send(msg)
    Log.Info("CQENT MAIL ERROR ---->:" + str(sys.exc_info()[1]))
    #A055S000P01-20800 END
    return True
...
def _equp_predefined_value_driver_update(previous_xml):	
    ...
    if previous_xml and val_list:
        ...
        for val in previous_xml:
            ...
            get_updated_xml = Sql.GetFirst("SELECT ENTITLEMENT_XML FROM SAQSCE WHERE QUOTE_RECORD_ID = '{}' AND QTEREV_RECORD_ID = '{}' AND SERVICE_ID = '{}' AND EQUIPMENT_ID = '{}' AND ISNULL(GREENBOOK,'') ='{}'".format(quote,revision,get_serviceid,val.EQUIPMENT_ID,val.GREENBOOK))
...
def entitlement_rolldown(objectName,get_serviceid,where,ent_temp):
    ...
    # 	is_changed = True
        ...
        is_changed = True
    if objectName in ('SAQTSE','SAQSGE') and get_serviceid in ('Z0009','Z0010','Z0128'):
    ...
    try:
        for obj in obj_list:
            ...
            # 			Sql.RunQuery(UpdateEntitlement)
                ...
                Log.Info('GREENBOOK ROLLDOWN STARTED---'+str(get_c4c_quote_id.C4C_QUOTE_ID)+'-rev-'+str(get_c4c_quote_id.QTEREV_ID))	
                if 'Z00068' in get_serviceid and objectName == 'SAQSCE':
                    where_condition = SAQITMWhere.replace('A.','')
                    #fab_val = where_cond.split('AND ')
                    #where_condition += ' AND {}'.format( fab_val[len(fab_val)-1] )
                    ...
                    for grnbk in get_value_query:
                        ...
                        Sql.RunQuery(UpdateEntitlement)
                Log.Info('GREENBOOK ROLLDOWN COMPLETED---'+str(get_c4c_quote_id.C4C_QUOTE_ID)+'-rev-'+str(get_c4c_quote_id.QTEREV_ID))
            elif obj == 'SAQSCE' and GetXMLsecField:
                Log.Info('EQUIPMENT ROLLDOWN STARTED---'+str(get_c4c_quote_id.C4C_QUOTE_ID)+'-rev-'+str(get_c4c_quote_id.QTEREV_ID))
                ...
                Log.Info('EQUIPEMNT ROLLDOWN COMPLETED---'+str(get_c4c_quote_id.C4C_QUOTE_ID)+'-rev-'+str(get_c4c_quote_id.QTEREV_ID))
            else:
                where = wherecon.split(",")[0]
        ...
        #ent columns update
        #INC08621345 - Start - M
        rec_list = ['SAQSCE','SAQGPE','SAQSGE','SAQTSE','SAQSAE']
        rec_list.remove(objectName)
        for rec_table in rec_list:
...
Log.Info('ROLLDOWN STARTED---'+str(get_c4c_quote_id.C4C_QUOTE_ID)+'-rev-'+str(get_c4c_quote_id.QTEREV_ID)+'--'+str(objs))
#calling rolldown
entitlement_rolldown(objectName,get_serviceid,where,ent_temp)
Log.Info('ROLLDOWN COMPLETED---'+str(get_c4c_quote_id.C4C_QUOTE_ID)+'-rev-'+str(get_c4c_quote_id.QTEREV_ID))
if ent_temp:
    ent_temp_drop = Sql.GetFirst("sp_executesql @T=N'IF EXISTS (SELECT ''X'' FROM SYS.OBJECTS WHERE NAME= ''"+str(ent_temp)+"'' ) BEGIN DROP TABLE "+str(ent_temp)+" END  ' ")
if ent_roll_temp:
    ent_temp_drop1 = Sql.GetFirst("sp_executesql @T=N'IF EXISTS (SELECT ''X'' FROM SYS.OBJECTS WHERE NAME= ''"+str(ent_roll_temp)+"'' ) BEGIN DROP TABLE "+str(ent_roll_temp)+" END  ' ")