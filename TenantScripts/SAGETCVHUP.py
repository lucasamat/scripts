		...
		while check_flag2 == 1:
			...
			if len(response) > 0:			
				...
				for record_dict in response:
					...
					Stagingquery = SqlHelper.GetFirst( ""+ str(Parameter.QUERY_CRITERIA_1)+ " "+str(Table_Name)+" (HKONT, ZAF_UDATE, MANDT, ZAF_UTIME, ZAFNOTE, ZUONR, GJAHR, BUZEI, ZAFNOTE_ID, BELNR, BUKRS)  select  N''"+str(record_dict['HKONT'])+ "'',''"+str(record_dict['ZAF_UDATE'])+ "'',''"+str(record_dict['MANDT'])+ "'',''"+str(record_dict['ZAF_UTIME'])+ "'',''"+record_dict['ZAFNOTE'].replace("'","")+ "'',''"+str(record_dict['ZUONR'])+ "'',''"+str(record_dict['GJAHR'])+ "'',''"+str(record_dict['BUZEI'])+ "'',''"+str(record_dict['ZAFNOTE_ID'])+ "'',''"+str(record_dict['BELNR'])+ "'',''"+str(record_dict['BUKRS'])+ "'' ' ")
