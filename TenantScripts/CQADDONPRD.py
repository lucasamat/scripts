...
def _addon_service_level_entitlement(OfferingRow_detail):
    ...
    if ProductVersionObj and addon_entitlement_object.cnt == 0:
        ...
        tbrow["QUOTE_NAME"]=str(OfferingRow_detail.QUOTE_NAME).replace("'","''")#INC08958263
...
def _addon_equipment_insert(OfferingRow_detail,greenbook,record_ids,serviceid):#A055S000P01-20739 - START/END - M
    ...
    if record_ids != '' and record_ids  != 'undefined':
        #A055S000P01-21015,INC09026574 - Start - M
        record_id = str(tuple(record_id)).replace(",)",")").replace("' ","'")
        ...
        Sql.RunQuery(
                    ...
                    INSERT SAQSCO (
                        ...
                        TEMP_TOOL
                            ...
                            TEMP_TOOL
                            FROM SAQSCO (NOLOCK)
                            WHERE QUOTE_RECORD_ID = '{QuoteRecordId}' AND QTEREV_RECORD_ID = '{RevisionRecordId}' AND SERVICE_ID ='{par_service_id}' AND GREENBOOK = '{greenbook}' AND QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID in {record} AND SAQSCO.QTEREVFEQ_RECORD_ID not in (SELECT QTEREVFEQ_RECORD_ID FROM SAQSCO (NOLOCK) WHERE QUOTE_RECORD_ID = '{QuoteRecordId}'  AND QTEREV_RECORD_ID = '{RevisionRecordId}'  AND PAR_SERVICE_ID ='{par_service_id}' AND SERVICE_ID ='{serviceid}' AND GREENBOOK = '{greenbook}')
        ...
        #A055S000P01-21015 - End - M
        #INC09026574,A055S000P01-21015 - End - M 
            # else:
            #     Sql.RunQuery(
            #                 """
            #                 INSERT SAQSCO (
        ...
        Sql.RunQuery(
                    ...
                    INSERT SAQSCO (
                        ...
                        TEMP_TOOL
                            ...
                            TEMP_TOOL
                            FROM SAQSCO (NOLOCK)
                            WHERE QUOTE_RECORD_ID = '{QuoteRecordId}' AND QTEREV_RECORD_ID = '{RevisionRecordId}' AND SERVICE_ID ='{par_service_id}' AND GREENBOOK = '{greenbook}' AND SAQSCO.QTEREVFEQ_RECORD_ID not in (SELECT QTEREVFEQ_RECORD_ID FROM SAQSCO (NOLOCK) WHERE QUOTE_RECORD_ID = '{QuoteRecordId}'  AND QTEREV_RECORD_ID = '{RevisionRecordId}'  AND PAR_SERVICE_ID ='{par_service_id}' AND SERVICE_ID ='{serviceid}' AND GREENBOOK = '{greenbook}')
    ...
    Sql.RunQuery(""" INSERT SAQSCE
                    (KB_VERSION,ENTITLEMENT_XML,CONFIGURATION_STATUS,PAR_SERVICE_ID,PAR_SERVICE_RECORD_ID,PAR_SERVICE_DESCRIPTION,EQUIPMENT_ID,EQUIPMENT_RECORD_ID,QUOTE_ID,QUOTE_RECORD_ID,QTEREV_RECORD_ID,QTEREV_ID,QTESRVCOB_RECORD_ID,QTESRVENT_RECORD_ID,SERIAL_NO,SERVICE_DESCRIPTION,SERVICE_ID,SERVICE_RECORD_ID,CPS_CONFIGURATION_ID,CPS_MATCH_ID,GREENBOOK,GREENBOOK_RECORD_ID,FABLOCATION_ID,FABLOCATION_NAME,FABLOCATION_RECORD_ID,SALESORG_ID,SALESORG_NAME,SALESORG_RECORD_ID,QTEREVFEQ_RECORD_ID,QUOTE_SERVICE_COVERED_OBJ_ENTITLEMENTS_RECORD_ID,CPQTABLEENTRYADDEDBY,CPQTABLEENTRYDATEADDED) 
                    ...
                    SELECT IQ.*, CONVERT(VARCHAR(4000),NEWID()) as QUOTE_SERVICE_COVERED_OBJ_ENTITLEMENTS_RECORD_ID, {UserId} as CPQTABLEENTRYADDEDBY, GETDATE() as CPQTABLEENTRYDATEADDED FROM (
                        SELECT 
                        SAQSGE.KB_VERSION,SAQSGE.ENTITLEMENT_XML,SAQSGE.CONFIGURATION_STATUS,SAQTSE.PAR_SERVICE_ID,SAQTSE.PAR_SERVICE_RECORD_ID,SAQTSE.PAR_SERVICE_DESCRIPTION,SAQSCO.EQUIPMENT_ID,SAQSCO.EQUIPMENT_RECORD_ID,SAQTSE.QUOTE_ID,SAQTSE.QUOTE_RECORD_ID,SAQTSE.QTEREV_RECORD_ID,SAQTSE.QTEREV_ID,SAQSCO.QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID as QTESRVCOB_RECORD_ID,SAQTSE.QUOTE_SERVICE_ENTITLEMENT_RECORD_ID as QTESRVENT_RECORD_ID,SAQSCO.SERIAL_NO,SAQTSE.SERVICE_DESCRIPTION,SAQTSE.SERVICE_ID,SAQTSE.SERVICE_RECORD_ID,SAQSGE.CPS_CONFIGURATION_ID,SAQSGE.CPS_MATCH_ID,SAQSCO.GREENBOOK,SAQSCO.GREENBOOK_RECORD_ID,SAQSCO.FABLOCATION_ID,SAQSCO.FABLOCATION_NAME,SAQSCO.FABLOCATION_RECORD_ID,SAQTSE.SALESORG_ID,SAQTSE.SALESORG_NAME,SAQTSE.SALESORG_RECORD_ID,SAQSCO.QTEREVFEQ_RECORD_ID FROM
                        SAQTSE (NOLOCK)
                        JOIN SAQSGE (NOLOCK) ON SAQSGE.QUOTE_RECORD_ID = SAQTSE.QUOTE_RECORD_ID AND SAQSGE.QTEREV_RECORD_ID = SAQTSE.QTEREV_RECORD_ID AND SAQSGE.SERVICE_ID = SAQTSE.SERVICE_ID and SAQSGE.par_service_id = SAQTSE.par_service_id
                        JOIN SAQSCO (NOLOCK) ON SAQSCO.PAR_SERVICE_ID = SAQTSE.PAR_SERVICE_ID AND SAQSCO.SERVICE_ID = SAQTSE.SERVICE_ID AND SAQSCO.QUOTE_RECORD_ID = SAQTSE.QUOTE_RECORD_ID  AND SAQSCO.QTEREV_RECORD_ID = SAQTSE.QTEREV_RECORD_ID AND SAQSGE.GREENBOOK = SAQSCO.GREENBOOK and SAQSCO.par_service_id = SAQTSE.par_service_id
                        
                        LEFT JOIN SAQSCE (NOLOCK) ON SAQSGE.QUOTE_RECORD_ID = SAQSCE.QUOTE_RECORD_ID AND SAQSGE.QTEREV_RECORD_ID = SAQSCE.QTEREV_RECORD_ID AND SAQSGE.SERVICE_ID = SAQSCE.SERVICE_ID AND ISNULL(SAQSGE.GREENBOOK_RECORD_ID,'') = ISNULL(SAQSGE.GREENBOOK_RECORD_ID,'') AND SAQSCE.EQUIPMENT_ID = SAQSCO.EQUIPMENT_ID AND ISNULL(SAQSCE.PAR_SERVICE_ID,'') = ISNULL(SAQSCO.PAR_SERVICE_ID,'') AND SAQSCE.QTESRVCOB_RECORD_ID = SAQSCO.QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID
    ...
    distinct_nso = Sql.GetList("SELECT DISTINCT POSS_NSO_PART_ID FROM SAQSCN WHERE QUOTE_RECORD_ID = '{QuoteRecordId}' AND GREENBOOK = '{greenbook}' AND QTEREV_RECORD_ID = '{RevisionRecordId}' AND PAR_SERVICE_ID = '{serviceID}'".format(QuoteRecordId = OfferingRow_detail.QUOTE_RECORD_ID,greenbook = greenbook,RevisionRecordId = OfferingRow_detail.QTEREV_RECORD_ID,serviceID=serviceid))
    #A055S000P01-20739 - END - M
    Trace.Write("distinct_NSO"+str(distinct_nso))
    #INC08705445-M
    if distinct_nso:
        ...
        eqp_query = Sql.GetList("SELECT DISTINCT EQUIPMENT_ID,QUOTE_ID,CONTRACT_VALID_FROM,CONTRACT_VALID_TO,EQUIPMENT_DESCRIPTION,EQUIPMENT_RECORD_ID,EQUIPMENT_STATUS,FABLOCATION_ID,FABLOCATION_NAME,FABLOCATION_RECORD_ID,SERVICE_DESCRIPTION,SERVICE_ID,SERVICE_RECORD_ID,QTEREV_ID,QTESRVGBK_RECORD_ID,QTESRV_RECORD_ID,SERIAL_NO,TEMP_TOOL,PAR_SERVICE_DESCRIPTION,PAR_SERVICE_ID,PAR_SERVICE_RECORD_ID FROM SAQSCO WHERE QUOTE_RECORD_ID = '{QuoteRecordId}' AND QTEREV_RECORD_ID = '{RevisionRecordId}' AND SERVICE_ID = 'Z0123' AND GREENBOOK = '{greenbook}' AND EQUIPMENT_ID NOT IN (SELECT EQUIPMENT_ID FROM SAQSCN (NOLOCK) WHERE QUOTE_RECORD_ID = '{QuoteRecordId}' AND QTEREV_RECORD_ID = '{RevisionRecordId}' AND SERVICE_ID = 'Z0123')".format(QuoteRecordId = OfferingRow_detail.QUOTE_RECORD_ID,RevisionRecordId = OfferingRow_detail.QTEREV_RECORD_ID,greenbook = greenbook))
        for eqp in eqp_query:
            for nso in nso_poss_id:
                ...
                #INC08705445-M
                nso_query = Sql.GetFirst("SELECT POSS_NSO_PART_ID ,GREENBOOK FROM PRLPBE WHERE GREENBOOK = '{greenbook}' AND POSS_NSO_DESCRIPTION = '{poss_des}'".format(greenbook = greenbook,poss_des=prlpbe_query.POSS_NSO_DESCRIPTION))
                Trace.Write("nso_poss"+str(nso_query))
                #INC08705445-M
