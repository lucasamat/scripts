...
class SQL:
    ...
    def RunQuery(self, query):
        ...
        Trace.Write("SYDATABASE : RunQuery : RUNNING QUERY : " + QueryStatement)
        QueryStatement = QueryStatement.replace("'", "''")
        try:
            if  'UPDATE SAQTRV'  in QueryStatement.upper():
                # Hadoop fix - Start - A
                if 'CPQTABLEENTRYDATEMODIFIED' not in QueryStatement.upper():
                    QueryStatement = str(QueryStatement).replace("SET ", "SET CpqTableEntryDateModified = GETDATE(),")
                # Hadoop fix - End - A
    ...
    def getDomainDetails(self):
        Login_Domain = ""
        LOGIN_QUERY = SqlHelper.GetFirst("SELECT Domain FROM SYCONF (NOLOCK) where Domain like 'appliedmaterials%'")
        if LOGIN_QUERY is not None:
            Login_Domain = str(LOGIN_QUERY.Domain)
        else:
