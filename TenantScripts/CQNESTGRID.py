# =========================================================================================================================================
#   __script_name : CQNESTGRID.PY PRD
...
def GetEquipmentMaster(PerPage, PageInform, A_Keys, A_Values):
    ...
    isBookedQuote= Sql.GetFirst("SELECT REVISION_STATUS FROM SAQTRV (NOLOCK) WHERE QUOTE_RECORD_ID = '"+str(ContractRecordId)+"' AND QTEREV_RECORD_ID = '"+str(RevisionRecordId)+"'  ")
    dropdownval = "False"
    if isBookedQuote.REVISION_STATUS != "CFG-CONFIGURING":
        can_edit = dropdownval
        can_add = dropdownval
        can_delete = dropdownval
    ...
    Columns = [
        ...
        "PLATFORM",
        "MNT_PLANT_ID",
        "FABLOCATION_ID",#A055S000P01-21009
        "WARRANTY_START_DATE",
        "WARRANTY_END_DATE",
        "TEMP_TOOL"
    ]
    ...
    #     )
        ...
        if get_fab_query:
            ...
            get_fab = "= ''"
        Qstr = (
            """select top {PerPage} * from ( select  ROW_NUMBER() OVER( ORDER BY {orderby}) AS ROW, QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID,SALESORG_ID,SERIAL_NUMBER,CUSTOMER_TOOL_ID,WARRANTY_START_DATE,WARRANTY_END_DATE,EQUIPMENTCATEGORY_ID,EQUIPMENT_ID,MNT_PLANT_ID,GREENBOOK,QUOTE_NAME,SALESORG_NAME,EQUIPMENTCATEGORY_DESCRIPTION,EQUIPMENT_STATUS,PLATFORM,FABLOCATION_ID,QUOTE_ID,FABLOCATION_NAME,TEMP_TOOL,DEVICE_NODE,DEVICE_TYPE,PROCESS_TYPE,WAFER_SIZE_GROUP,GENERATION,END_PRODUCT_TECHNOLOGY,TECHNOLOGY,IBASE_ATTSTS,SUBSTRATE_SIZE_GROUP from SAQFEQ (NOLOCK) where QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}'{where_string} AND FABLOCATION_ID {get_fab}  and RELOCATION_EQUIPMENT_TYPE = '{equp_type}') m where m.ROW BETWEEN {Page_start} and {Page_End} """.format(orderby = orderby, ContractRecordId = ContractRecordId,RevisionRecordId = RevisionRecordId, get_fab = get_fab, Page_start = Page_start, Page_End = Page_End,PerPage = PerPage,equp_type = 'SENDING EQUIPMENT' if "Sending Account -" in TreeParam else "RECEIVING EQUIPMENT" if "Receiving Account -" in TreeParam else "",where_string = " AND "+str(where_string) if str(where_string)!="" else "" ) )
    ...
    #A055S000P01-20453 Start - M
    noOfRecords = str(Page_End)
    if QueryCountObj is not None:
        QueryCount = QueryCountObj.cnt
        if QueryCount < int(PerPage):
            noOfRecords = str(QueryCount)
    #A055S000P01-20453 End - M
    
    parent_obj = Sql.GetList(Qstr)
    ibase_missing_flag = False
    assembly_missing_flag = False
    #A055S000P01-20612 start A 
    req_event_missing_flag = False
    #A055S000P01-20612 end A 
    err_values = []
    for par in parent_obj:
        err_title = ''
        data_id = str(par.QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID) 
        ibase_status = str(par.IBASE_ATTSTS)
        ibase_greenbook =str(par.GREENBOOK)
        if str(ibase_status) == "INCOMPLETE" and str(ibase_greenbook) =="DISPLAY":
            ibase_attributes = {"GENERATION":par.GENERATION,"END_PRODUCT_TECHNOLOGY":par.END_PRODUCT_TECHNOLOGY,"TECHNOLOGY":par.TECHNOLOGY}
            #ibase_icon = '<a class = "cust_exc" title="Missing iBase Attributes"><img src="/mt/appliedmaterials_prd/Additionalfiles/Icons/Exclamation.svg"></a>' ibase_missing_flag = True
            for key,col in ibase_attributes.items():
                if not col:
                    err_title += "Attribute "+str(key)+" is missing from the iBase \n"
            ibase_icon = '<a class = "cust_exc" title="'+str(err_title)+'"><img src="/mt/appliedmaterials_prd/Additionalfiles/Icons/Exclamation.svg"></a>'
        elif str(ibase_status) == "INCOMPLETE" and str(ibase_greenbook) !="DISPLAY":
            ibase_attributes = {"PLATFORM":par.PLATFORM,"DEVICE_NODE":par.DEVICE_NODE,"DEVICE_TYPE":par.DEVICE_TYPE,"SUBSTRATE_SIZE_GROUP":par.SUBSTRATE_SIZE_GROUP}
            #ibase_icon = '<a class = "cust_exc" title="Missing iBase Attributes"><img src="/mt/appliedmaterials_prd/Additionalfiles/Icons/Exclamation.svg"></a>'
            ibase_missing_flag = True
            for key,col in ibase_attributes.items():
                if not col:
                    err_title += "Attribute "+str(key)+" is missing from the iBase \n"
            ibase_icon = '<a class = "cust_exc" title="'+str(err_title)+'"><img src="/mt/appliedmaterials_prd/Additionalfiles/Icons/Exclamation.svg"></a>'
        #INC08730324 A
        elif str(ibase_status) == "NO ASSEMBLY":
            ibase_icon = '<a class = "cust_exc" title="Assembly is Missing"><img src="/mt/'+str(Sql.getDomainDetails())+'/Additionalfiles/Orange1_Circle.svg"></a>'
            err_title +="Assembly is Missing"
            assembly_missing_flag = True
        #INC08730324 A
        #A055S000P01-20612 start A
        elif str(ibase_status) =="REQUIRED EVENT IS MISSING":
            ibase_icon = '<a class = "cust_exc" title="'+str(ent_status_des)+'"><img src="/mt/'+str(Sql.getDomainDetails())+'/Additionalfiles/Icons/red_tag_icon.svg"></a>'
            err_title +="Required Event Is Missing"
            req_event_missing_flag  = True
            Trace.Write("----req_event_missing_flag="+str(req_event_missing_flag))
        #A055S000P01-20612 end A
        else:
            ibase_icon = '<a class = "cust_exc" title="Complete"><img src="/mt/appliedmaterials_prd/Additionalfiles/Green_Tick.svg"></a>'
            err_title +="Complete"
        err_values.append(err_title)
        if TreeParam != 'Fab Locations':
            Action_str = (
            ...
            Action_str = ('<div class="btn-group dropdown"><div class="dropdown" id="ctr_drop"><i data-toggle="dropdown" id="dropdownMenuButton" class="fa fa-sort-desc dropdown-toggle" aria-expanded="false"></i><ul class="dropdown-menu left" aria-labelledby="dropdownMenuButton"><li><a class="dropdown-item cur_sty" href="#" id="'
            + str(data_id)
            + '" onclick="Commonteree_view_RL(this)">VIEW</a></li>'
            )
        Status_str = str(ibase_icon)
        ...
        data_dict["IBASE_ATTSTS"] = str(Status_str)
    ...
    # INC08632845 #A055S000P01-20453 - Start - M
    table_header += '<th data-field="SELECT" class="wid45" data-checkbox="true"></th>'
    # INC08632845 #A055S000P01-20453 - End - M
    ...
    #A055S000P01-20699
    if assembly_missing_flag or ibase_missing_flag or req_event_missing_flag:
        missing_text = "<img src=\'/mt/APPLIEDMATERIALS_PRD/Additionalfiles/warning1.svg\' class=\'wid20_px\'><p>The requested action could not be completed due to one or more of the following reasons:</p><p>Missing attributes from Ibase<span class ='p-5'>/</span> Missing Assembly(s) from the added tool(s)<span class ='p-5'>/</span> Assembly(s) in the quote with required event(s) missing.</p><p>Kindly remove the impacted tools for the above mentioned issues in the FAB location node and work with the FSO to correct the iBase for missing attributes/assemblies.</p><p>In the case of missing events, kindly work with the SBU to correct the event/assembly mapping in SSCM. Once corrected, you may re-add the tools and proceed.</p>"
        filter_control_function += ('$(".cpq_notification_div .cpq_cust_warning").html("<div>'+str(missing_text)+'</div>").css("display","block").parent().css("display","block").parent().prev().children("div").removeClass("collapsed");$("#notify").addClass("in");$("#notify").css("height","auto");')
    #A055S000P01-20699    
    #A055S000P01-20612 start A
    #A055S000P01-20612 end A
    else:
        filter_control_function += ('$(".cpq_notification_div .cpq_cust_warning").html("").css("display","none").parent().css("display","none").parent().prev().children("div").addClass("collapsed");$("#notify").removeClass("in");')
    filter_control_function += ('var err_values = '+str(err_values)+';console.log(err_values);var table_id = "table_equipment_parent";$("#"+table_id+" > tbody > tr ").each(function(index,row){$(this).find("td:nth-child(3) > a").attr("title",err_values[index])});')
    #A055S000P01-20453 Start - M
    filter_control_function += (
    '$("'
    + filter_class
    ...
    dbl_clk_function = (
        'localStorage.setItem("selectall","no"); var checkedRows=[]; localStorage.setItem("multiedit_checkbox_clicked", []); $("'
        + str(table_ids)
        + '\ th.bs-checkbox div.th-inner").before("<div class=\'pad0brdbt\' >SELECT</div>");  $("'
        + str(table_ids)
        + '").on("check.bs.table", function (e, row, $element) { console.log("checked00009==2233==");checkedRows.push($element.closest("tr").find("td:'
        + str(cls)
        + '").text()); checkedRows = [...new Set(checkedRows)]; localStorage.setItem("multiedit_checkbox_clicked", checkedRows); if(checkedRows.length == ' + str(noOfRecords) + '){ localStorage.setItem("selectall","yes"); } if(checkedRows.length > 1){ $("button#delete_receive_equipment").css("display","block");} else { $("button#delete_receive_equipment").css("display","none");} }); $("'
        + str(table_ids)
        + '").on("check-all.bs.table", function (e) { localStorage.setItem("selectall","yes"); localStorage.setItem("multiedit_checkbox_clicked", []); checkedRows=[]; var table = $("'
        + str(table_ids)
        + '").closest("table"); table.find("tbody tr").each(function() { rowKey = $(this).find("td:nth-child(4)").text(); checkedRows.push(rowKey); }); localStorage.setItem("multiedit_checkbox_clicked", checkedRows); $("button#delete_receive_equipment").css("display","block"); }); $("'
        + str(table_ids)
        + '").on("uncheck-all.bs.table", function (e) { localStorage.setItem("selectall","no"); localStorage.setItem("multiedit_checkbox_clicked", []); checkedRows=[]; $("button#delete_receive_equipment").css("display","none"); }); $("'
        + str(table_ids)
        + '").on("uncheck.bs.table", function (e, row, $element) { localStorage.setItem("selectall","no");var rec_ids=$element.closest("tr").find("td:'
        + str(cls)
        + '").text(); $.each(checkedRows, function(index, value) { if (value === rec_ids) { checkedRows.splice(index,1); }}); localStorage.setItem("multiedit_checkbox_clicked", checkedRows); if(checkedRows.length > 1){ $("button#delete_receive_equipment").css("display","block");} else { $("button#delete_receive_equipment").css("display","none");}});'
    )
    ##CR BULK DELETE ENDS
    ...
    dbl_clk_function += (
        '$("'
        + str(table_ids)
        + '").on("all.bs.table", function (e, name, args) { $(".bs-checkbox input").addClass("custom"); $(".bs-checkbox input").after("<span class=\'lbl\'></span>"); });$(".bs-checkbox input").addClass("custom"); $(".bs-checkbox input").after("<span class=\'lbl\'></span>"); $("'
        + str(table_ids)
    ...
    for key, col_name in enumerate(list(Columns)):
        ...
        try:
            ...
            if str(objss_obj.PICKLIST).upper() == "TRUE":
                filter_level_data = "select"
                filter_clas_name = (
                    '<div id = "'+ str(table_id)+ '_RelatedMutipleCheckBoxDrop_'+ str(key)+ '" class="form-control bootstrap-table-filter-control-'
                    + str(col_name)
                    + " RelatedMutipleCheckBoxDrop_"
                    + str(key)
                    + ' "></div>'
        ...
        if filter_level_data == "select":
            try:
                if TreeParam == 'Fab Locations':
                    ...
                    Trace.Write("###"+str(FablocationId)+"TPM="+str(TreeParentParam))
                    xcd = Sql.GetFirst(
                    "SELECT (STUFF((SELECT DISTINCT ', ' + CAST("
                    + str(col_name)
                    + " AS CHAR(100)) FROM "
    ...
    Action_Str += " of"
    if isBookedQuote.REVISION_STATUS != "CFG-CONFIGURING":
        INCLUDE_CHECK = 'DISABLE_INCLUDE_CHECK'
    else:
...
def GetEventsMaster(PerPage, PageInform, A_Keys, A_Values):
    if str(PerPage) == "" and str(PageInform) == "":
        Page_start = 1
        Page_End = 10
        PerPage = 10
    ...
    data_list = []
    #INC08833987 start A
    get_include_QueryCountObj_checkbox = ""
    #INC08833987 end A
    ...
    if TreeTopSuperParentParam == "Product Offerings":
        Qstr = (
            "select top "
            + str(PerPage)
            + " * from ( select ROW_NUMBER() OVER( ORDER BY "+str(orderby)+") AS ROW, QUOTE_REV_PO_GRNBK_PM_EVEN_ASSEMBLIES_RECORD_ID,EQUIPMENT_DESCRIPTION,EQUIPMENT_ID,ASSEMBLY_ID,GOT_CODE,PM_ID,PM_NAME,KIT_ID,KIT_NAME,KIT_NUMBER,SSCM_PM_FREQUENCY,PM_FREQUENCY,DEVICE_NODE,PROCESS_TYPE,MNTEVT_LEVEL,TKM_FLAG,INCLUDED from SAQGPA (NOLOCK) where QUOTE_RECORD_ID = '"
            + str(ContractRecordId)
            + "' and QTEREV_RECORD_ID = '"
            + str(RevisionRecordId)
            + "' and SERVICE_ID = '"
        ...
        # INC08700581 - A
        where_events = "QUOTE_RECORD_ID = '"+ str(ContractRecordId)+ "' and QTEREV_RECORD_ID = '"+ str(RevisionRecordId)+ "' and SERVICE_ID = '"+ str(TreeParentParam)+ "' and GREENBOOK = '"+str(TreeParam)+"' and ( PM_FREQUENCY_EDITABLE = 'True' OR PM_FREQUENCY_EDITABLE = '1' ) "
        # INC08700581 - A 
        QueryCountObj = Sql.GetFirst(
        ...
        # INC08700581 - A
        get_include_QueryCountObj = Sql.GetFirst(
            "select count(CpqTableEntryId) as cnt from SAQGPA (NOLOCK) where QUOTE_RECORD_ID = '"
            + str(ContractRecordId)
            + "' and QTEREV_RECORD_ID = '"
            + str(RevisionRecordId)
            + "' and SERVICE_ID = '" + str(TreeParentParam) + "' and GREENBOOK ='"
            + str(TreeParam)
            + "' and ( INCLUDED = 'True' OR INCLUDED = '1' ) and ( PM_FREQUENCY_EDITABLE = 'True' OR PM_FREQUENCY_EDITABLE = '1' ) "+str(where_string)
        )
        Trace.Write("inc"+str(get_include_QueryCountObj.cnt))
        if get_include_QueryCountObj.cnt == QueryCountObj.cnt:
            Trace.Write("included")
            get_include_QueryCountObj_checkbox =  'INCLUDED_SELECT_ALL'
        else:
            Trace.Write("no_included")
            get_include_QueryCountObj_checkbox =  'NO_INCLUDED_SELECT'
        # INC08700581 - A
    elif TreeSuperParentParam == "Product Offerings":
        Qstr = (
            "select top "
            + str(PerPage)
            + " * from ( select ROW_NUMBER() OVER( ORDER BY "+str(orderby)+") AS ROW, QUOTE_REV_PO_GRNBK_PM_EVEN_ASSEMBLIES_RECORD_ID,EQUIPMENT_DESCRIPTION,EQUIPMENT_ID,ASSEMBLY_ID,GOT_CODE,PM_ID,PM_NAME,KIT_ID,KIT_NAME,KIT_NUMBER,SSCM_PM_FREQUENCY,PM_FREQUENCY,TKM_FLAG,DEVICE_NODE,PROCESS_TYPE,MNTEVT_LEVEL,INCLUDED from SAQGPA (NOLOCK) where QUOTE_RECORD_ID = '"
            + str(ContractRecordId)
            + "' and QTEREV_RECORD_ID = '"
            + str(RevisionRecordId)
            + "' and SERVICE_ID = '"
        ...
        # INC08700581 - A
        where_events = ("QUOTE_RECORD_ID = '"+ str(ContractRecordId)+ "' and QTEREV_RECORD_ID = '"+ str(RevisionRecordId)+ "' and SERVICE_ID = '"+ str(TreeParam)+ "'  and ( PM_FREQUENCY_EDITABLE = 'True' OR PM_FREQUENCY_EDITABLE = '1' ) ")
        # INC08700581 - A
        QueryCountObj = Sql.GetFirst(
            "select count(CpqTableEntryId) as cnt from SAQGPA (NOLOCK) where QUOTE_RECORD_ID = '"
            + str(ContractRecordId)
        ...
        Qstr = (
            "select top "
            + str(PerPage)
            + " * from ( select ROW_NUMBER() OVER( ORDER BY "+str(orderby)+") AS ROW, QUOTE_REV_PO_GBK_GOT_CODE_PM_EVENTS_RECORD_ID,GREENBOOK,DEVICE_NODE,PROCESS_TYPE,GOT_CODE,KIT_ID,KIT_NUMBER,PM_ID,PM_NAME,SSCM_PM_FREQUENCY,PM_FREQUENCY from "+str(ObjectName)+" (NOLOCK) where QUOTE_RECORD_ID = '"
            + str(ContractRecordId)
            + "' and QTEREV_RECORD_ID = '"
            + str(RevisionRecordId)
            + "' and SERVICE_ID = '"
    ...
    for par in parent_obj:
        ...
        #A055S000P01-21090
        Action_str = (
            '<div class="btn-group dropdown"><div class="dropdown" id="ctr_drop"><i data-toggle="dropdown" id="dropdownMenuButton" class="fa fa-sort-desc dropdown-toggle" aria-expanded="false"></i>'
        )
        #A055S000P01-21090
        if can_edit.upper() == "TRUE":
            Action_str += (
        ...
        if str(ObjectName) == "SAQGPA":
            ...
            data_dict["MNTEVT_LEVEL"] = ('<abbr id ="" title="' + str(par.MNTEVT_LEVEL) + '">' + str(par.MNTEVT_LEVEL) + "</abbr>")
        ...
        try:
            ...
            data_dict["DEVICE_NODE"] = ('<abbr id ="" title="' + par.DEVICE_NODE + '">' + par.DEVICE_NODE + "</abbr>")
            data_dict["PROCESS_TYPE"] = ('<abbr id ="" title="' + par.PROCESS_TYPE + '">' + par.PROCESS_TYPE + "</abbr>")
            data_dict["KIT_ID"] = ('<abbr id ="" title="' + par.KIT_ID + '">' + par.KIT_ID + "</abbr>")
            data_dict["KIT_NUMBER"] = ('<abbr id ="" title="' + par.KIT_NUMBER + '">' + par.KIT_NUMBER + "</abbr>")
        #INC08784694 - end - M
    ...
    for key, invs in enumerate(list(Columns)):
        ...
        if checkbox_list is not None and invs in checkbox_list:
            ...
            elif invs in ("QUOTE_REV_PO_GRNBK_PM_EVEN_ASSEMBLIES_RECORD_ID","QUOTE_REV_PO_GBK_GOT_CODE_PM_EVENTS_RECORD_ID"):
    ...
    + '").click( function(){ var table_id = $(this).closest("table").attr("id"); ATTRIBUTE_VALUEList = []; '
    + str(values_list)
    + ' var attribute_value = $(this).val(); cpq.server.executeScript("CQNESTGRID", {"TABNAME":"Events Parent", "ACTION":"PRODUCT_ONLOAD_FILTER", "ATTRIBUTE_NAME": '
    + str(list(Columns))
    ...
    + str(tbl_id)
    + '").bootstrapTable("load", data1 ); if ($(\'.segment_revision_sale_id_text\').text() != \'CFG-CONFIGURING\'){$("#table_event_parent #INCLUDED").attr("disabled","disabled");}else{$("#table_event_parent #INCLUDED").removeAttr("disabled");} $("#noRecDisp").remove(); if (document.getElementById("'+str(tbl_id) + '___totalItemCount")){document.getElementById("'+str(tbl_id)+ '___totalItemCount").innerHTML = data2;}  if (document.getElementById("'+str(tbl_id) + '___NumberofItem")) { console.log("if_chk_j"); document.getElementById("'+str(tbl_id)+ '___NumberofItem").innerHTML = data3;}} else{ console.log("else_chk_j"); $("#' + str(tbl_id) + '").bootstrapTable("load", data1  );$("#' + str(tbl_id) + '").after("<div id=\'noRecDisp\' class=\'noRecord\'>No Records to Display</div>"); if (document.getElementById("'+str(tbl_id) + '___totalItemCount")){document.getElementById("'+str(tbl_id)+ '___totalItemCount").innerHTML = data2;}  if (document.getElementById("'+str(tbl_id) + '___NumberofItem")) {document.getElementById("'+str(tbl_id)+ '___NumberofItem").innerHTML = data3;} }} catch(err){} }); filter_search_click();$(".JColResizer").mousedown(function(){ $("thead.fullHeadFirst").css("cssText","z-index: 2;border-top: 1px solid rgb(220, 220, 220);top: 154px;border-right: 0px !important;");$("thead.fullHeadSecond").css("display","none"); });$(".JColResizer").mouseup(function(){ var th_width_resize = [];$("#table_event_parent thead.fullHeadFirst tr th").each(function(index){var wid = $(this).css("width"); if(index ==0 || index ==1){th_width_resize.push("60px");}else{th_width_resize.push(wid);}}); $("thead.fullHeadFirst").css("cssText","position: fixed;z-index: 2;border-top: 1px so	lid rgb(220, 220, 220); top: 154px;border-right: 0px !important;");$("thead.fullHeadSecond").css("display","table-header-group");$("#table_event_parent thead.fullHeadFirst tr th").each(function(index){var num = th_width_resize[index].split("px");var numsp = parseInt(num[0]);numsp = numsp - 1;var make_str =numsp+"px"; var c = "width:"+make_str+";white-space: nowrap;overflow: hidden;text-overflow: ellipsis;";var d = "width:"+make_str+";"; $(this).css("cssText",c);$(this).children("div:first-child").css("cssText",c);$(this).children("div.fht-cell").css("cssText",d);});$("#table_event_parent thead.fullHeadSecond tr th").each(function(index){var num = th_width_resize[index].split("px");var numsp = parseInt(num[0]);numsp = numsp - 1;var make_str =numsp+"px"; var c = "width:"+make_str+";white-space: nowrap;overflow: hidden;text-overflow: ellipsis;";var d = "width:"+make_str+";"; $(this).css("cssText",c);$(this).children("div:first-child").css("cssText",c);$(this).children("div.fht-cell").css("cssText",d);}); });});')
    ...
    Test = (
        ...
        + '___totalItemCount" >'
        + str(QueryCount)
        + '</span><div class="clear-padding fltltmrgtp3" ><div  class="pull-right vertmidtxtrht"><select onchange="PageFunctestChild(this,\'Quote\',\'\',\'table_event_parent\')" id="'
        + str(table_id)
        + '___PageCountValue" class="form-control wid65vermiddisinbmarl5"><option value="10" selected>10</option><option value="20">20</option><option value="50">50</option><option value="100">100</option><option value="200">200</option></select> </div></div></div><div class="col-xs-8 col-md-4  clear-padding disinpad10txtcen"  data-bind="visible: totalItemCount"><div class="clear-padding col-xs-12 col-sm-6 col-md-12 bor0" ><ul class="pagination pagination"><li class="disabled"><a href="#" onclick="FirstPageLoad_paginationChild(\'Quote\',\'\',\'table_event_parent\')"><i class="fa fa-caret-left font14whtbld" ></i><i class="fa fa-caret-left font14" ></i></a></li><li class="disabled"><a href="#" onclick="Previous12334Child(\'Quote\',\'\',\'table_event_parent\')"><i class="fa fa-caret-left font14" ></i>PREVIOUS</a></li><li class="disabled"><a href="#" class="disabledPage" onclick="Next12334Child(\'Quote\',\'\',\'table_event_parent\')">NEXT<i class="fa fa-caret-right font14" ></i></a></li><li class="disabled"><a href="#" onclick="LastPageLoad_paginationChild(\'Quote\',\'\',\'table_event_parent\')" class="disabledPage"><i class="fa fa-caret-right font14"></i><i class="fa fa-caret-right font14whtbld"></i></a></li></ul></div> </div> <div class="col-md-4 pr_page_pad"> <span id="'
        + str(table_id)
        + '___page_count" class="currentPage page_right_content">1</span><span class="page_right_content pad_rt_2">Page </span></div></div>'
    )
    if QueryCount < int(PerPage):
    ...
    #A055S000P01-20390 - Start
    if (str(isBookedQuote.REVISION_STATUS).upper() in ("CBC-CBC COMPLETED","BOK-CONTRACT CREATED","BOK-CONTRACT BOOKED","LGL-LEGAL SOW ACCEPTED","LGL-LEGAL SOW REJECTED","LGL-PREPARING LEGAL SOW","CBC-PREPARING CBC")) or (str(isBookedQuote.REVISION_STATUS).upper() in ("CFG-ACQUIRING","PRR-ON HOLD PRICING","CFG-ON HOLD - COSTING","PRI-PRICING","PRR-PRICING REVIEWED","PRR-RECALCULATING") and str(TreeParam).upper() not in ("QUOTE ITEMS","BILLING","APPROVALS","QUOTE DOCUMENTS")) or (str(isBookedQuote.REVISION_STATUS).upper() in ("APR-APPROVED","APR-REJECTED","APR-RECALLED","APR-APPROVAL PENDING","OPD-PREPARING QUOTE DOCUMENTS","OPD-CUSTOMER ACCEPTED","APR-SUBMITTED TO CUSTOMER","OPD-CUSTOMER REJECTED") and str(TreeParam).upper() not in ("APPROVALS","QUOTE DOCUMENTS")):
        Nso_Included = "True"
    else:
        Nso_Included = "False"
    #A055S000P01-20390 - End
...
def GetEquipmentChild(recid, PerPage, PageInform, A_Keys, A_Values,reckeyid):
    # This function is used to construct the table inside the Equipment Parent Table.(As Nested Table).
    Trace.Write("cpqid"+str(reckeyid))
    TreeParam = Product.GetGlobal("TreeParam")
    TreeParentParam = Product.GetGlobal("TreeParentLevel0")
    TreeSuperParentParam = Product.GetGlobal("TreeParentLevel1")
    TreeTopSuperParentParam = Product.GetGlobal("TreeParentLevel2")
    ...
    dropdownval = "False"
    if isBookedQuote.REVISION_STATUS != "CFG-CONFIGURING":
        can_edit1 = dropdownval
        can_add1 = dropdownval
        can_delete1 = dropdownval
    ...
    if sale_type != "TOOL RELOCATION":
        if TreeParam == 'Fab Locations' or TreeParam == "Customer Information":
            Parent_Equipmentid = Sql.GetFirst("select EQUIPMENT_ID,QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID from SAQFEQ (NOLOCK) where QUOTE_RECORD_ID = '{ContractRecordId}' AND QTEREV_RECORD_ID = '{RevisionRecordId}' AND EQUIPMENT_ID = '{EquipmentId}' AND CpqTableEntryId ='{key_id}'".format(ContractRecordId = ContractRecordId,RevisionRecordId = RevisionRecordId,EquipmentId=recid,key_id = reckeyid))
        elif TreeParentParam == 'Fab Locations':
            Parent_Equipmentid = Sql.GetFirst("select EQUIPMENT_ID,QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID from SAQFEQ (NOLOCK) where QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and  FABLOCATION_ID = '{FablocationId}' AND EQUIPMENT_ID = '{EquipmentId}' ".format(ContractRecordId = ContractRecordId,RevisionRecordId = RevisionRecordId,FablocationId = FablocationId,EquipmentId=recid,key_id = reckeyid))
        elif TreeSuperParentParam == 'Fab Locations':
            Parent_Equipmentid = Sql.GetFirst("select EQUIPMENT_ID,QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID from SAQFEQ (NOLOCK) where QUOTE_RECORD_ID = '{ContractRecordId}'  and QTEREV_RECORD_ID = '{RevisionRecordId}' and FABLOCATION_ID = '{FablocationId}' AND EQUIPMENT_ID = '{EquipmentId}'AND GREENBOOK = '{GreenBook}' AND CpqTableEntryId ='{key_id}' ".format(ContractRecordId = ContractRecordId,RevisionRecordId = RevisionRecordId,FablocationId = TreeParentParam,EquipmentId = recid,GreenBook = Product.GetGlobal("TreeParam"),key_id = reckeyid))
    elif sale_type == "TOOL RELOCATION":
        if TreeParam == 'Fab Locations' or TreeParam == "Customer Information":
            Parent_Equipmentid = Sql.GetFirst("select EQUIPMENT_ID,QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID from SAQFEQ (NOLOCK) where QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' AND EQUIPMENT_ID = '{EquipmentId}' AND CpqTableEntryId ='{key_id}'".format(ContractRecordId = ContractRecordId,RevisionRecordId = RevisionRecordId,EquipmentId=recid,key_id = reckeyid))
        elif TreeParentParam == 'Fab Locations':
            Parent_Equipmentid = Sql.GetFirst("select EQUIPMENT_ID,QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID from SAQFEQ (NOLOCK) where QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' AND EQUIPMENT_ID = '{EquipmentId}' AND CpqTableEntryId ='{key_id}'".format(ContractRecordId = ContractRecordId,RevisionRecordId = RevisionRecordId,FablocationId = FablocationId,EquipmentId=recid,key_id = reckeyid))
        elif TreeSuperParentParam == 'Fab Locations':
            Parent_Equipmentid = Sql.GetFirst("select EQUIPMENT_ID,QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID from SAQFEQ (NOLOCK) where QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and FABLOCATION_ID = '{FablocationId}' AND EQUIPMENT_ID = '{EquipmentId}' AND CpqTableEntryId ='{key_id}'".format(ContractRecordId = ContractRecordId,RevisionRecordId = RevisionRecordId,FablocationId = FablocationId,EquipmentId = recid,key_id = reckeyid))
        elif TreeTopSuperParentParam == 'Fab Locations':
            Parent_Equipmentid = Sql.GetFirst("select EQUIPMENT_ID,QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID from SAQFEQ (NOLOCK) where QUOTE_RECORD_ID = '{ContractRecordId}'  and QTEREV_RECORD_ID = '{RevisionRecordId}' and FABLOCATION_ID = '{FablocationId}' AND EQUIPMENT_ID = '{EquipmentId}'AND GREENBOOK = '{GreenBook}' AND CpqTableEntryId ='{key_id}'".format(ContractRecordId = ContractRecordId,RevisionRecordId = RevisionRecordId,FablocationId = TreeParentParam,EquipmentId = recid,GreenBook = Product.GetGlobal("TreeParam"),key_id = reckeyid))
    if Parent_Equipmentid:
        EquipmentID = Parent_Equipmentid.EQUIPMENT_ID
        Equipmentrecid = Parent_Equipmentid.QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID
        if sale_type != "TOOL RELOCATION":
            if TreeParam == 'Fab Locations' or TreeParam == "Customer Information":
                child_obj_recid = Sql.GetList("select top "+str(PerPage)+" * from (select ROW_NUMBER() OVER( ORDER BY QUOTE_FAB_LOC_COV_OBJ_ASSEMBLY_RECORD_ID) AS ROW, QUOTE_FAB_LOC_COV_OBJ_ASSEMBLY_RECORD_ID,EQUIPMENT_ID,INCLUDED,SERIAL_NUMBER,ASSEMBLY_ID,ASSEMBLY_DESCRIPTION,GOT_CODE,MNT_PLANT_ID,FABLOCATION_ID,WARRANTY_START_DATE,EQUIPMENTCATEGORY_ID,WARRANTY_END_DATE,SALESORG_ID,EQUIPMENTTYPE_ID as ASSEMBLYTYPE_ID from SAQFEA (NOLOCK) where EQUIPMENT_ID = '{EquipmentID}' AND QTEREVFEQ_RECORD_ID = '{Equipmentrecid}' AND QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' ) m where m.ROW BETWEEN ".format(EquipmentID = EquipmentID,Equipmentrecid = Equipmentrecid,ContractRecordId = ContractRecordId,RevisionRecordId = RevisionRecordId)+ str(Page_start)+" and "+str(Page_End))
                QueryCountObj = Sql.GetFirst("select count(CpqTableEntryId) as cnt from SAQFEA (NOLOCK) where QUOTE_RECORD_ID ='{ContractRecordId}' and QTEREV_RECORD_ID ='{RevisionRecordId}' and EQUIPMENT_ID = '{EquipmentID}' AND QTEREVFEQ_RECORD_ID = '{Equipmentrecid}'".format(EquipmentID = EquipmentID,Equipmentrecid = Equipmentrecid,ContractRecordId = ContractRecordId,RevisionRecordId = RevisionRecordId))
            elif TreeParentParam == 'Fab Locations':
                child_obj_recid = Sql.GetList("select top "+str(PerPage)+" * from (select ROW_NUMBER() OVER( ORDER BY QUOTE_FAB_LOC_COV_OBJ_ASSEMBLY_RECORD_ID) AS ROW, QUOTE_FAB_LOC_COV_OBJ_ASSEMBLY_RECORD_ID,EQUIPMENT_ID,INCLUDED,SERIAL_NUMBER,ASSEMBLY_ID,ASSEMBLY_DESCRIPTION,GOT_CODE,MNT_PLANT_ID,FABLOCATION_ID,WARRANTY_START_DATE,EQUIPMENTCATEGORY_ID,WARRANTY_END_DATE,SALESORG_ID,EQUIPMENTTYPE_ID as ASSEMBLYTYPE_ID from SAQFEA (NOLOCK) where EQUIPMENT_ID = '{EquipmentID}' AND QTEREVFEQ_RECORD_ID = '{Equipmentrecid}' and QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and FABLOCATION_ID = '{FablocationId}') m where m.ROW BETWEEN ".format(EquipmentID = EquipmentID,Equipmentrecid = Equipmentrecid,ContractRecordId = ContractRecordId,RevisionRecordId = RevisionRecordId,FablocationId=FablocationId)+ str(Page_start)+ " and "+ str(Page_End))
                QueryCountObj = Sql.GetFirst("select count(CpqTableEntryId) as cnt from SAQFEA (NOLOCK) where QUOTE_RECORD_ID = '{ContractRecordId}' and FABLOCATION_ID = '{FablocationId}' and EQUIPMENT_ID = '{EquipmentID}' AND QTEREV_RECORD_ID = '{RevisionRecordId}' AND QTEREVFEQ_RECORD_ID = '{Equipmentrecid}'".format(ContractRecordId = ContractRecordId,RevisionRecordId = RevisionRecordId,EquipmentID =EquipmentID,Equipmentrecid = Equipmentrecid,FablocationId =FablocationId))
            elif TreeSuperParentParam == 'Fab Locations':
                child_obj_recid = Sql.GetList("select top "+str(PerPage)+" * from (select ROW_NUMBER() OVER( ORDER BY QUOTE_FAB_LOC_COV_OBJ_ASSEMBLY_RECORD_ID) AS ROW, QUOTE_FAB_LOC_COV_OBJ_ASSEMBLY_RECORD_ID,EQUIPMENT_ID,SERIAL_NUMBER,INCLUDED,ASSEMBLY_ID,ASSEMBLY_DESCRIPTION,GOT_CODE,MNT_PLANT_ID,FABLOCATION_ID,WARRANTY_START_DATE,EQUIPMENTCATEGORY_ID,WARRANTY_END_DATE,SALESORG_ID,EQUIPMENTTYPE_ID as ASSEMBLYTYPE_ID from SAQFEA (NOLOCK) where EQUIPMENT_ID = '{EquipmentID}' AND QTEREVFEQ_RECORD_ID = '{Equipmentrecid}' and QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and FABLOCATION_ID = '{FablocationId}')m where m.ROW BETWEEN ".format(ContractRecordId = ContractRecordId,RevisionRecordId = RevisionRecordId,EquipmentID =EquipmentID,Equipmentrecid = Equipmentrecid, FablocationId = TreeParentParam)+ str(Page_start)+ " and "+ str(Page_End))
                QueryCountObj = Sql.GetFirst("select count(CpqTableEntryId) as cnt from SAQFEA (NOLOCK) where EQUIPMENT_ID = '{EquipmentID}' AND QTEREVFEQ_RECORD_ID = '{Equipmentrecid}' and QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and FABLOCATION_ID = '{FablocationId}'".format(ContractRecordId = ContractRecordId,RevisionRecordId = RevisionRecordId,EquipmentID =EquipmentID,Equipmentrecid = Equipmentrecid, FablocationId = TreeParentParam))
        elif sale_type == "TOOL RELOCATION":
            if TreeParam == 'Fab Locations' or TreeParam == "Customer Information":
                child_obj_recid = Sql.GetList("select top "+str(PerPage)+" * from (select ROW_NUMBER() OVER( ORDER BY QUOTE_FAB_LOC_COV_OBJ_ASSEMBLY_RECORD_ID) AS ROW, QUOTE_FAB_LOC_COV_OBJ_ASSEMBLY_RECORD_ID,EQUIPMENT_ID,INCLUDED,SERIAL_NUMBER,ASSEMBLY_ID,ASSEMBLY_DESCRIPTION,GOT_CODE,MNT_PLANT_ID,FABLOCATION_ID,WARRANTY_START_DATE,EQUIPMENTCATEGORY_ID,WARRANTY_END_DATE,SALESORG_ID,EQUIPMENTTYPE_ID as ASSEMBLYTYPE_ID from SAQFEA (NOLOCK) where EQUIPMENT_ID = '{EquipmentID}' AND QTEREVFEQ_RECORD_ID = '{Equipmentrecid}' AND QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}') m where m.ROW BETWEEN ".format(EquipmentID = EquipmentID,Equipmentrecid = Equipmentrecid,ContractRecordId = ContractRecordId,RevisionRecordId = RevisionRecordId)+ str(Page_start)+ " and "+ str(Page_End))
                QueryCountObj = Sql.GetFirst("select count(CpqTableEntryId) as cnt from SAQFEA (NOLOCK) where EQUIPMENT_ID = '{EquipmentID}' AND QTEREVFEQ_RECORD_ID = '{Equipmentrecid}' AND QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}'".format(EquipmentID = EquipmentID,Equipmentrecid = Equipmentrecid,ContractRecordId = ContractRecordId,RevisionRecordId = RevisionRecordId))
...
def GetEventsChild(recid, PerPage, PageInform, A_Keys, A_Values):
    ...
    if Parent_event:
        ...
        if str(objname)=="SAQGPA":
            child_obj_recid = Sql.GetList(
                "select distinct top 1 * from (select ROW_NUMBER() OVER( ORDER BY CpqTableEntryId DESC) AS ROW, QUOTE_SERVICE_COV_OBJ_ASS_PM_KIT_PARTS_RECORD_ID,KIT_ID,KIT_NAME,KIT_NUMBER,TKM_FLAG,ASSEMBLY_ID from SAQSKP (NOLOCK) where QTEREV_RECORD_ID = '"+str(RevisionRecordId)+"'  AND QUOTE_RECORD_ID = '"+str(ContractRecordId)+"' AND ASSEMBLY_ID = '"
            ...
            child_obj_recid = Sql.GetList(
                "select distinct top 1 * from (select ROW_NUMBER() OVER( ORDER BY CpqTableEntryId DESC) AS ROW, QUOTE_SERVICE_COV_OBJ_ASS_PM_KIT_PARTS_RECORD_ID,KIT_ID,KIT_NAME,KIT_NUMBER,TKM_FLAG,ASSEMBLY_ID from SAQSKP (NOLOCK) where QTEREV_RECORD_ID = '"+str(RevisionRecordId)+"'  AND QUOTE_RECORD_ID = '"+str(ContractRecordId)+"' AND PM_ID = '"+str(Parent_event.PM_ID)+"' AND SERVICE_ID = '"+str(Parent_event.SERVICE_ID)+"' AND (KIT_ID = '"+str(Parent_event.KIT_ID)+"' or KIT_ID IS NULL) AND (KIT_NUMBER = '"+str(Parent_event.KIT_NUMBER)+"' or KIT_NUMBER IS NULL) AND QTEGBKPME_RECORD_ID = '"+str(Parent_event.QUOTE_REV_PO_GBK_GOT_CODE_PM_EVENTS_RECORD_ID)+"' )m where m.ROW BETWEEN "+ str(Page_start)+ " and "+ str(Page_End)
            )
            QueryCountObj = Sql.GetFirst(
        ...
        QueryCount = ""
        if QueryCountObj is not None:
            QueryCount = 1 if child_obj_recid else 0
        # Data construction for table.
        for child in child_obj_recid:
...
def GetEquipmentMasterFilter(ATTRIBUTE_NAME, ATTRIBUTE_VALUE,PerPage,PageInform):
    ...
    for quer_key, quer_value in enumerate(Dict_formation):
        ...
        if Dict_formation.get(quer_value) != "":
            ...
            if str(quer_values).find(",") == -1:
                if x_picklistcheck == "TRUE" or str(quer_value)=="IBASE_ATTSTS":
    ...
    orderby = ""
    if SortColumn != '' and SortColumnOrder !='' and SortColumn!='STATUS':
        orderby = SortColumn + " " + SortColumnOrder
    else:
        orderby = "QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID"
    if ATTRIBUTE_VALUE is None or ATTRIBUTE_VALUE == "" or ATTRIBUTE_VALUE_STR is None or ATTRIBUTE_VALUE_STR == "":
        ...
        if TreeParam == 'Fab Locations':
            ...
            "select top "+str(PerPage)+" QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID,SALESORG_ID,EQUIPMENTCATEGORY_ID,EQUIPMENT_ID,MNT_PLANT_ID,CUSTOMER_TOOL_ID,SERIAL_NUMBER,GREENBOOK,QUOTE_NAME,SALESORG_NAME,EQUIPMENT_DESCRIPTION,EQUIPMENT_STATUS,PLATFORM,FABLOCATION_ID,WARRANTY_START_DATE,QUOTE_ID,FABLOCATION_NAME,WARRANTY_END_DATE,TEMP_TOOL,DEVICE_NODE,DEVICE_TYPE,PROCESS_TYPE,WAFER_SIZE_GROUP,GENERATION,END_PRODUCT_TECHNOLOGY,TECHNOLOGY,IBASE_ATTSTS,SUBSTRATE_SIZE_GROUP  from SAQFEQ (NOLOCK) where QUOTE_RECORD_ID = '"
            + str(ContractRecordId)
            + "' AND QTEREV_RECORD_ID = '{}' ".format(str(Quote.GetGlobal("quote_revision_record_id")))
            + " ORDER BY "
            + str(orderby)
            ...
            if (("Sending Account -" in TreeParam) or ("Receiving Account -" in TreeParam)) and TreeParentParam == 'Fab Locations':
                ...
                if get_fab_query:
                    ...
                    get_fab = "= ''"
                parent_obj = Sql.GetList(
                    "select top "+str(PerPage)+" QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID,SALESORG_ID,EQUIPMENTCATEGORY_ID,EQUIPMENT_ID,MNT_PLANT_ID,CUSTOMER_TOOL_ID,SERIAL_NUMBER,GREENBOOK,QUOTE_NAME,SALESORG_NAME,EQUIPMENT_DESCRIPTION,EQUIPMENT_STATUS,PLATFORM,FABLOCATION_ID,WARRANTY_START_DATE,QUOTE_ID,FABLOCATION_NAME,WARRANTY_END_DATE,TEMP_TOOL,DEVICE_NODE,DEVICE_TYPE,PROCESS_TYPE,WAFER_SIZE_GROUP,GENERATION,END_PRODUCT_TECHNOLOGY,TECHNOLOGY,IBASE_ATTSTS,SUBSTRATE_SIZE_GROUP from SAQFEQ (NOLOCK) where QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and FABLOCATION_ID {get_fab} and RELOCATION_EQUIPMENT_TYPE = '{equp_type}' ORDER BY {orderby}".format(orderby = orderby, ContractRecordId = ContractRecordId,RevisionRecordId = Quote.GetGlobal("quote_revision_record_id"), get_fab = get_fab,equp_type = 'SENDING EQUIPMENT' if "Sending Account -" in TreeParam else "RECEIVING EQUIPMENT" if "Receiving Account -" in TreeParam else "")
                )
                ...
                parent_obj = Sql.GetList(
                    "select top {PerPage} QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID,SALESORG_ID,EQUIPMENTCATEGORY_ID,EQUIPMENT_ID,MNT_PLANT_ID,CUSTOMER_TOOL_ID,SERIAL_NUMBER,GREENBOOK,QUOTE_NAME,SALESORG_NAME,EQUIPMENT_DESCRIPTION,EQUIPMENT_STATUS,PLATFORM,FABLOCATION_ID,WARRANTY_START_DATE,QUOTE_ID,FABLOCATION_NAME,WARRANTY_END_DATE,TEMP_TOOL,DEVICE_NODE,DEVICE_TYPE,PROCESS_TYPE,WAFER_SIZE_GROUP,GENERATION,END_PRODUCT_TECHNOLOGY,TECHNOLOGY,IBASE_ATTSTS,SUBSTRATE_SIZE_GROUP from SAQFEQ (NOLOCK) where QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and FABLOCATION_ID = '{TreeParam}' and RELOCATION_EQUIPMENT_TYPE = '{equp_type}' ORDER BY {orderby}".format(PerPage=str(PerPage),ContractRecordId=str(ContractRecordId),RevisionRecordId = Quote.GetGlobal("quote_revision_record_id"),TreeParam=str(TreeParam),equp_type = 'SENDING EQUIPMENT' if "Sending Account -" in TreeParentParam else "RECEIVING EQUIPMENT" if "Receiving Account -" in TreeParentParam else "",orderby=str(orderby))
                )
                ...
                if str(TreeParentParam)=="Fab Locations":
                    ...
                    Trace.Write('filter---------22222222222222222')
                    parent_obj = Sql.GetList(
                        "select top "+str(PerPage)+" QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID,SALESORG_ID,EQUIPMENTCATEGORY_ID,EQUIPMENT_ID,MNT_PLANT_ID,CUSTOMER_TOOL_ID,SERIAL_NUMBER,GREENBOOK,QUOTE_NAME,SALESORG_NAME,EQUIPMENT_DESCRIPTION,EQUIPMENT_STATUS,PLATFORM,FABLOCATION_ID,WARRANTY_START_DATE,QUOTE_ID,FABLOCATION_NAME,WARRANTY_END_DATE,TEMP_TOOL,DEVICE_NODE,DEVICE_TYPE,PROCESS_TYPE,WAFER_SIZE_GROUP,GENERATION,END_PRODUCT_TECHNOLOGY,TECHNOLOGY,IBASE_ATTSTS,SUBSTRATE_SIZE_GROUP from SAQFEQ (NOLOCK) where QUOTE_RECORD_ID = '"
                        + str(ContractRecordId)
                        + "' and QTEREV_RECORD_ID = '"
        ...
        if TreeSuperParentParam == "Fab Locations":
            if "Sending Account -" in TreeParentParam or "Receiving Account -" in TreeParentParam:
                if "Sending Account -" in TreeParentParam:
                    ...
                    equp_type = "RECEIVING EQUIPMENT"
                parent_obj = Sql.GetList(
                    "select top "+str(PerPage)+" QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID,SALESORG_ID,EQUIPMENTCATEGORY_ID,EQUIPMENT_ID,MNT_PLANT_ID,CUSTOMER_TOOL_ID,SERIAL_NUMBER,GREENBOOK,QUOTE_NAME,SALESORG_NAME,EQUIPMENT_DESCRIPTION,EQUIPMENT_STATUS,PLATFORM,FABLOCATION_ID,WARRANTY_START_DATE,QUOTE_ID,FABLOCATION_NAME,WARRANTY_END_DATE,TEMP_TOOL,DEVICE_NODE,DEVICE_TYPE,PROCESS_TYPE,WAFER_SIZE_GROUP,GENERATION,END_PRODUCT_TECHNOLOGY,TECHNOLOGY,IBASE_ATTSTS,SUBSTRATE_SIZE_GROUP from SAQFEQ (NOLOCK) where "
                ...
                parent_obj = Sql.GetList(
                    "select top "+str(PerPage)+"  QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID,EQUIPMENTCATEGORY_ID,SERIAL_NUMBER,CUSTOMER_TOOL_ID,GREENBOOK,EQUIPMENT_STATUS,PLATFORM,MNT_PLANT_ID,EQUIPMENT_ID,EQUIPMENT_DESCRIPTION,FABLOCATION_ID,FABLOCATION_NAME,QUOTE_ID,QUOTE_NAME,SALESORG_ID,SALESORG_NAME,WARRANTY_START_DATE,WARRANTY_END_DATE,WAFER_SIZE,TEMP_TOOL,DEVICE_NODE,DEVICE_TYPE,PROCESS_TYPE,WAFER_SIZE_GROUP,GENERATION,END_PRODUCT_TECHNOLOGY,TECHNOLOGY,IBASE_ATTSTS,SUBSTRATE_SIZE_GROUP from SAQFEQ (NOLOCK) where "
                    + str(ATTRIBUTE_VALUE_STR)
                    + " 1=1 and QUOTE_RECORD_ID = '"
                    + str(ContractRecordId)
                    + "' and QTEREV_RECORD_ID = '"
            ...
            if (("Sending Account -" in TreeParam) or ("Receiving Account -" in TreeParam)) and TreeParentParam == 'Fab Locations':
                ...
                if get_fab_query:
                    ...
                    get_fab = "= ''"
                parent_obj = Sql.GetList(
                    """select top {PerPage} QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID,SALESORG_ID,EQUIPMENTCATEGORY_ID,EQUIPMENT_ID,MNT_PLANT_ID,CUSTOMER_TOOL_ID,SERIAL_NUMBER,GREENBOOK,QUOTE_NAME,SALESORG_NAME,EQUIPMENT_DESCRIPTION,EQUIPMENT_STATUS,PLATFORM,FABLOCATION_ID,CONVERT(varchar,WARRANTY_START_DATE,101) AS WARRANTY_START_DATE,CONVERT(varchar,WARRANTY_END_DATE,101) WARRANTY_END_DATE,QUOTE_ID,FABLOCATION_NAME,TEMP_TOOL,DEVICE_NODE,DEVICE_TYPE,PROCESS_TYPE,WAFER_SIZE_GROUP,GENERATION,END_PRODUCT_TECHNOLOGY,TECHNOLOGY,IBASE_ATTSTS,SUBSTRATE_SIZE_GROUP from SAQFEQ (NOLOCK) where {ATTRIBUTE_VALUE_STR} QUOTE_RECORD_ID = '{ContractRecordId}'  and QTEREV_RECORD_ID = '{RevisionRecordId}' and FABLOCATION_ID {get_fab}  and RELOCATION_EQUIPMENT_TYPE = '{equp_type}' ORDER BY {orderby} """.format(orderby = orderby, ContractRecordId = ContractRecordId,RevisionRecordId = Quote.GetGlobal("quote_revision_record_id"), get_fab = get_fab,PerPage = PerPage,ATTRIBUTE_VALUE_STR=str(ATTRIBUTE_VALUE_STR) if str(ATTRIBUTE_VALUE_STR) else " ",equp_type = 'SENDING EQUIPMENT' if "Sending Account -" in TreeParam else "RECEIVING EQUIPMENT" if "Receiving Account -" in TreeParam else "" ) )
                Count = Sql.GetFirst(
...
def GetEventsMasterFilter(ATTRIBUTE_NAME, ATTRIBUTE_VALUE,PerPage,PageInform):
    ...
    if str(TreeSuperParentParam)=="Z0009" or str(TreeSuperParentParam)=="Z0010" or str(TreeSuperParentParam)=="Z0128":
        ...
        ObjectName = "SAQGPA"
    #INC08700581 start-A
    if ATTRIBUTE_NAME[0]=="INCLUDED":
        ATTRIBUTE_VALUE[0]=''
    #INC08700581 end-A
    Dict_formation = dict(zip(ATTRIBUTE_NAME, ATTRIBUTE_VALUE))
    for quer_key, quer_value in enumerate(Dict_formation):
        x_picklistcheckobj = Sql.GetFirst(
            "SELECT PICKLIST FROM SYOBJD (NOLOCK) WHERE OBJECT_NAME = '" + str(ObjectName) + "' AND API_NAME = '" + str(quer_value) + "'"
        )
        x_picklistcheck = str(x_picklistcheckobj.PICKLIST).upper()
        if Dict_formation.get(quer_value) != "":
            quer_values = str(Dict_formation.get(quer_value)).strip()
    ...
    if isBookedQuote.REVISION_STATUS != "CFG-CONFIGURING":
        ...
        can_delete = dropdownval
    #INC08700581 start-M
    orderby = include_selectall=""
    #INC08700581 start-M
    # INC08700581 - A
    where_condition_events = ""
    # INC08700581 - A
    ...
    if TreeTopSuperParentParam == "Product Offerings":
        parent_obj = Sql.GetList(
            "select top "
            + str(PerPage)
            + " * from ( select ROW_NUMBER() OVER( ORDER BY "+str(orderby)+") AS ROW, QUOTE_REV_PO_GRNBK_PM_EVEN_ASSEMBLIES_RECORD_ID,EQUIPMENT_DESCRIPTION,EQUIPMENT_ID,ASSEMBLY_ID,GREENBOOK,FABLOCATION_ID,DEVICE_NODE,PROCESS_TYPE,MNTEVT_LEVEL,GOT_CODE,PM_ID,KIT_ID,KIT_NAME,KIT_NUMBER,PM_NAME,SSCM_PM_FREQUENCY,PM_FREQUENCY,INCLUDED from SAQGPA (NOLOCK) where "+str(ATTRIBUTE_VALUE_STR)+" QUOTE_RECORD_ID = '"+str(ContractRecordId)+"' and QTEREV_RECORD_ID = '"+str(Quote.GetGlobal("quote_revision_record_id"))+"' and SERVICE_ID  = '"+str(TreeParentParam)+"' and GREENBOOK = '"+str(TreeParam)+"' and ( PM_FREQUENCY_EDITABLE = 'True' OR PM_FREQUENCY_EDITABLE = '1' ) ) m "
        )
        # INC08700581 - A
        where_condition_events = " "+str(ATTRIBUTE_VALUE_STR)+" QUOTE_RECORD_ID = '"+str(ContractRecordId)+"' and QTEREV_RECORD_ID = '"+str(Quote.GetGlobal("quote_revision_record_id"))+"' and SERVICE_ID  = '"+str(TreeParentParam)+"' and GREENBOOK = '"+str(TreeParam)+"' and ( PM_FREQUENCY_EDITABLE = 'True' OR PM_FREQUENCY_EDITABLE = '1' )"
        # INC08700581 - A
        ...
        if Count:
            QueryCount = Count.cnt
        #update saqgpa based on included start INC08700581 M
        included_value =1
        if include_selectall == "include_selectall":
            Sql.RunQuery("UPDATE {table_name} SET INCLUDED = {included_value} WHERE "+str(ATTRIBUTE_VALUE_STR)+" QUOTE_RECORD_ID= '{ContractRecordId}' AND QTEREV_RECORD_ID ='{revision_record_id}'AND SERVICE_ID ='{service_id}' AND GREENBOOK = '{greenbook}'".format(included_value =1 if included_value == True else 0 ,ContractRecordId=ContractRecordId,revision_record_id=Quote.GetGlobal("quote_revision_record_id"),table_name = 'SAQGPA', service_id =TreeParentParam,greenbook=TreeParam))
            '''Sql.RunQuery("""UPDATE SAQGPM
                        SET
                        PM_FREQUENCY = assembly.PM_FREQUENCY,
                        SSCM_PM_FREQUENCY = assembly.SSCM_PM_FREQUENCY
                        FROM SAQGPM (NOLOCK)
                        INNER JOIN (select SAQGPA.INCLUDED,SAQGPA.QTEREVPME_RECORD_ID,SAQGPA.QUOTE_RECORD_ID,SAQGPA.GREENBOOK_RECORD_ID,SAQGPA.QTEREV_RECORD_ID,SAQGPA.SERVICE_RECORD_ID,SAQGPA.GOTCODE_RECORD_ID,SAQGPA.PM_RECORD_ID,SAQGPA.DEVICE_NODE,SAQGPA.PROCESS_TYPE,SUM(ISNULL(SAQGPA.PM_FREQUENCY, 0)) as PM_FREQUENCY,SUM(ISNULL(SAQGPA.SSCM_PM_FREQUENCY, 0)) as SSCM_PM_FREQUENCY FROM SAQGPA (NOLOCK) where SAQGPA.QUOTE_RECORD_ID = '{QuoteRecordId}' and SAQGPA.QTEREV_RECORD_ID = '{RevisionRecordId}' AND SAQGPA.SERVICE_ID = '{TreeParams}' GROUP BY QUOTE_RECORD_ID,QTEREV_RECORD_ID,SERVICE_RECORD_ID,GOTCODE_RECORD_ID,PM_RECORD_ID,DEVICE_NODE,PROCESS_TYPE,GREENBOOK_RECORD_ID,QTEREVPME_RECORD_ID,INCLUDED) assembly ON SAQGPM.QUOTE_RECORD_ID = assembly.QUOTE_RECORD_ID AND SAQGPM.QTEREV_RECORD_ID = assembly.QTEREV_RECORD_ID  AND SAQGPM.QUOTE_REV_PO_GBK_GOT_CODE_PM_EVENTS_RECORD_ID = assembly.QTEREVPME_RECORD_ID WHERE SAQGPM.QUOTE_RECORD_ID = '{QuoteRecordId}' and SAQGPM.QTEREV_RECORD_ID = '{RevisionRecordId}' AND SAQGPM.SERVICE_ID = '{TreeParams}' AND assembly.INCLUDED = 'True' """.format(QuoteRecordId=ContractRecordId,RevisionRecordId=Quote.GetGlobal("quote_revision_record_id"),TreeParams=TreeParam))'''
        #update saqgpa based on included start INC08700581 M
    elif TreeSuperParentParam == "Product Offerings":
        parent_obj = Sql.GetList(
            "select top "
            + str(PerPage)
            + " * from ( select ROW_NUMBER() OVER( ORDER BY "+str(orderby)+") AS ROW, QUOTE_REV_PO_GRNBK_PM_EVEN_ASSEMBLIES_RECORD_ID,EQUIPMENT_DESCRIPTION,EQUIPMENT_ID,ASSEMBLY_ID,GREENBOOK,FABLOCATION_ID,DEVICE_NODE,PROCESS_TYPE,GOT_CODE,PM_ID,KIT_ID,KIT_NAME,MNTEVT_LEVEL,KIT_NUMBER,PM_NAME,SSCM_PM_FREQUENCY,PM_FREQUENCY,INCLUDED from SAQGPA (NOLOCK) where "+str(ATTRIBUTE_VALUE_STR)+" QUOTE_RECORD_ID = '"+str(ContractRecordId)+"' and QTEREV_RECORD_ID = '"+str(Quote.GetGlobal("quote_revision_record_id"))+"' and SERVICE_ID  = '"+str(TreeParam)+"' and ( PM_FREQUENCY_EDITABLE = 'True' OR PM_FREQUENCY_EDITABLE = '1' ))m "
        )
        ...
        # INC08700581 - A
        where_condition_events = " "+str(ATTRIBUTE_VALUE_STR)+" QUOTE_RECORD_ID = '"+str(ContractRecordId)+"' and QTEREV_RECORD_ID = '"+str(Quote.GetGlobal("quote_revision_record_id"))+"' and SERVICE_ID  = '"+str(TreeParam)+"' and ( PM_FREQUENCY_EDITABLE = 'True' OR PM_FREQUENCY_EDITABLE = '1' )"
        # INC08700581 - A
        if Count:
            QueryCount = Count.cnt
        #update saqgpa based on included start INC08700581 M
        if include_selectall == "include_selectall":
            Sql.RunQuery("UPDATE {table_name} SET INCLUDED = {included_value} WHERE "+str(ATTRIBUTE_VALUE_STR)+" QUOTE_RECORD_ID= '{ContractRecordId}' AND QTEREV_RECORD_ID ='{revision_record_id}'AND SERVICE_ID ='{service_id}'".format(included_value =1 if included_value == True else 0 ,ContractRecordId=ContractRecordId,revision_record_id=Quote.GetGlobal("quote_revision_record_id"),table_name = 'SAQGPA', service_id =TreeParam))
            Sql.RunQuery("""UPDATE SAQGPM
                        SET
                        PM_FREQUENCY = assembly.PM_FREQUENCY,
                        SSCM_PM_FREQUENCY = assembly.SSCM_PM_FREQUENCY
        ...
        parent_obj = Sql.GetList(
            "select top "
            + str(PerPage)
            + " * from ( select ROW_NUMBER() OVER( ORDER BY "+str(orderby)+") AS ROW, QUOTE_REV_PO_GRNBK_PM_EVEN_ASSEMBLIES_RECORD_ID,EQUIPMENT_DESCRIPTION,EQUIPMENT_ID,ASSEMBLY_ID,GREENBOOK,FABLOCATION_ID,DEVICE_NODE,PROCESS_TYPE,MNTEVT_LEVEL,GOT_CODE,PM_ID,PM_NAME,KIT_NUMBER,SSCM_PM_FREQUENCY,PM_FREQUENCY,INCLUDED from SAQGPA (NOLOCK) where "+str(ATTRIBUTE_VALUE_STR)+" QUOTE_RECORD_ID = '"
            + str(ContractRecordId)
            + "' and QTEREV_RECORD_ID = '"
            + str(RevisionRecordId)
            + "' and SERVICE_ID = '"
        ...
        parent_obj = Sql.GetList(
            "select top "
            + str(PerPage)
            + " * from ( select ROW_NUMBER() OVER( ORDER BY "+str(orderby)+") AS ROW, QUOTE_REV_PO_GBK_GOT_CODE_PM_EVENTS_RECORD_ID,GREENBOOK,DEVICE_NODE,PROCESS_TYPE,MNTEVT_LEVEL,GOT_CODE,KIT_ID,KIT_NUMBER,PM_ID,PM_NAME,SSCM_PM_FREQUENCY,PM_FREQUENCY,INCLUDED from "+str(ObjectName)+" (NOLOCK) where "+str(ATTRIBUTE_VALUE_STR)+" QUOTE_RECORD_ID = '"
            + str(ContractRecordId)
            + "' and QTEREV_RECORD_ID = '"
            + str(RevisionRecordId)
            + "' and SERVICE_ID = '"
    ...
    if parent_obj:
        for par in parent_obj:
            ...
            if str(ObjectName) == "SAQGPA":
                ...
                data_dict["ids"] = str(data_id)
                data_dict["ACTIONS"] = str(Action_str)
                data_dict["QUOTE_REV_PO_GBK_GOT_CODE_PM_EVENTS_RECORD_ID"] = CPQID.KeyCPQId.GetCPQId(
                    "SAQGPM", str(par.QUOTE_REV_PO_GBK_GOT_CODE_PM_EVENTS_RECORD_ID)
                )
                #data_dict["EQUIPMENT_DESCRIPTION"] = ('<abbr id ="" title="' + str(par.EQUIPMENT_DESCRIPTION) + '">' + str(par.EQUIPMENT_DESCRIPTION) + "</abbr>") 
                #data_dict["EQUIPMENT_ID"] = ('<abbr id ="" title="' + str(par.EQUIPMENT_ID) + '">' + str(par.EQUIPMENT_ID) + "</abbr>")
                #data_dict["ASSEMBLY_ID"] = ('<abbr id ="" title="' + str(par.ASSEMBLY_ID) + '">' + str(par.ASSEMBLY_ID) + "</abbr>")
    ...
    if QueryCount == 0:
        page = str(QueryCount) + " - " + str(QueryCount) + " of "
        include_selectall = 'no_include_selectall'
    elif QueryCount < int(PerPage):
        page = str(Page_start) + " - " + str(QueryCount) + " of "
        include_selectall = 'no_include_selectall'
    else:
    ...
    return data_list,QueryCount,page,include_selectall,Nso_Included,where_condition_events#INC08700581
...
def GetContractEquipmentMasterFilter(ATTRIBUTE_NAME, ATTRIBUTE_VALUE,PerPage,PageInform):
    ...
    return data_list, QueryCount, page,Nso_Included


def GetEquipmentChildFilter(ATTRIBUTE_NAME, ATTRIBUTE_VALUE, RECID,PerPage,PageInform,reckeyid):
    if str(PerPage) == "" and str(PageInform) == "":
        Page_start = 1
        Page_End = 10
        PerPage = 10
    ...
    recid = str(RECID)
    isBookedQuote= Sql.GetFirst("SELECT REVISION_STATUS FROM SAQTRV (NOLOCK) WHERE QUOTE_RECORD_ID = '"+str(ContractRecordId)+"' AND QTEREV_RECORD_ID = '"+str(RevisionRecordId)+"'  ")
    get_equipment_id = Sql.GetFirst("SELECT EQUIPMENT_ID,QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID FROM SAQFEQ(NOLOCK) WHERE QUOTE_RECORD_ID = '{ContractRecordId}' AND QTEREV_RECORD_ID = '{RevisionRecordId}' AND CpqTableEntryId = '{reckeyid}'".format(ContractRecordId = ContractRecordId,RevisionRecordId = RevisionRecordId,reckeyid = reckeyid))
    dropdownval = "False"
    if isBookedQuote.REVISION_STATUS != "CFG-CONFIGURING":
        can_edit1 = dropdownval
        can_clone1 = dropdownval
        can_delete1 = dropdownval
    ...
    for quer_key, quer_value in enumerate(Dict_formation):
        if Dict_formation.get(quer_value) != "":
            ...
            if str(quer_values).find(",") == -1:
                ...
                quer_values = tuple(list(quer_values))
                ATTRIBUTE_VALUE_STR += str(quer_value) + " in " + str(quer_values) + " and "
    Parent_Equipmentid = get_equipment_id.EQUIPMENT_ID
    Parent_Equipment_key_id = get_equipment_id.QUOTE_FAB_LOCATION_EQUIPMENTS_RECORD_ID
    if ATTRIBUTE_VALUE is None or ATTRIBUTE_VALUE == "" or ATTRIBUTE_VALUE_STR is None or ATTRIBUTE_VALUE_STR == "":
        if TreeParam == 'Fab Locations' or TreeParam == 'Customer Information':
            child_obj_recid = Sql.GetList("select top "+str(PerPage)+" QUOTE_FAB_LOC_COV_OBJ_ASSEMBLY_RECORD_ID,EQUIPMENT_ID,SERIAL_NUMBER,ASSEMBLY_ID,ASSEMBLY_DESCRIPTION,GOT_CODE,INCLUDED,MNT_PLANT_ID,FABLOCATION_ID,WARRANTY_START_DATE,EQUIPMENTCATEGORY_ID,WARRANTY_END_DATE,SALESORG_ID,EQUIPMENTTYPE_ID AS ASSEMBLYTYPE_ID from SAQFEA (NOLOCK) where EQUIPMENT_ID = '{Parent_Equipmentid}' and QTEREVFEQ_RECORD_ID = '{Parent_Equipment_key_id}' and QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' ORDER BY  {ORDER_BY}".format(ContractRecordId=ContractRecordId,RevisionRecordId = RevisionRecordId,ORDER_BY = orderby,Parent_Equipment_key_id =Parent_Equipment_key_id,Parent_Equipmentid =Parent_Equipmentid))
            
            Count = Sql.GetFirst( "select count(*) as cnt from SAQFEA (NOLOCK) where EQUIPMENT_ID = '{Parent_Equipmentid}' AND QTEREVFEQ_RECORD_ID = '{Parent_Equipment_key_id}' and QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' ".format(ContractRecordId=ContractRecordId,RevisionRecordId = RevisionRecordId,Parent_Equipmentid =Parent_Equipmentid,Parent_Equipment_key_id = Parent_Equipment_key_id))
            if Count:
                QueryCount = Count.cnt
            ...
            if (TreeParam.startswith("Sending") or TreeParam.startswith("Receiving")):
                ...
                Count = Sql.GetFirst("select count(*) as cnt from SAQFEA (NOLOCK) where EQUIPMENT_ID = '{Parent_Equipmentid}' AND QTEREVFEQ_RECORD_ID = '{Parent_Equipment_key_id}' and QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}'".format(ContractRecordId=ContractRecordId,RevisionRecordId = RevisionRecordId,Parent_Equipment_key_id =Parent_Equipment_key_id,Parent_Equipmentid =Parent_Equipmentid))
                if Count:
                    QueryCount = Count.cnt
            else:
                child_obj_recid = Sql.GetList("select top "+str(PerPage)+" QUOTE_FAB_LOC_COV_OBJ_ASSEMBLY_RECORD_ID,EQUIPMENT_ID,SERIAL_NUMBER,ASSEMBLY_ID,ASSEMBLY_DESCRIPTION,GOT_CODE,INCLUDED,MNT_PLANT_ID,FABLOCATION_ID,WARRANTY_START_DATE,EQUIPMENTCATEGORY_ID,WARRANTY_END_DATE,SALESORG_ID,EQUIPMENTTYPE_ID AS ASSEMBLYTYPE_ID from SAQFEA (NOLOCK) where EQUIPMENT_ID = '{Parent_Equipmentid}' AND QTEREVFEQ_RECORD_ID = '{Parent_Equipment_key_id}' and QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and FABLOCATION_ID = '{FabId}' ORDER BY  {ORDER_BY}".format(ContractRecordId=ContractRecordId,RevisionRecordId = RevisionRecordId,FabId=TreeParam,ORDER_BY = orderby,Parent_Equipment_key_id = Parent_Equipment_key_id,Parent_Equipmentid = Parent_Equipmentid))

                Count = Sql.GetFirst("select count(*) as cnt from SAQFEA (NOLOCK) where EQUIPMENT_ID = '{Parent_Equipmentid}' AND QTEREVFEQ_RECORD_ID = '{Parent_Equipment_key_id}' and QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and FABLOCATION_ID = '{FabId}'".format(ContractRecordId=ContractRecordId,RevisionRecordId = RevisionRecordId,FabId=TreeParam,Parent_Equipment_key_id = Parent_Equipment_key_id,Parent_Equipmentid = Parent_Equipmentid))
                if Count:
                    QueryCount = Count.cnt
        else:
            if (TreeParentParam.startswith("Sending") or TreeParentParam.startswith("Receiving")):
                child_obj_recid = Sql.GetList( "select top "+str(PerPage)+" QUOTE_FAB_LOC_COV_OBJ_ASSEMBLY_RECORD_ID,EQUIPMENT_ID,SERIAL_NUMBER,ASSEMBLY_ID,ASSEMBLY_DESCRIPTION,GOT_CODE,INCLUDED,MNT_PLANT_ID,FABLOCATION_ID,WARRANTY_START_DATE,EQUIPMENTCATEGORY_ID,WARRANTY_END_DATE,SALESORG_ID,EQUIPMENTTYPE_ID AS ASSEMBLYTYPE_ID from SAQFEA (NOLOCK) where EQUIPMENT_ID = '{Parent_Equipmentid}' AND QTEREVFEQ_RECORD_ID = '{Parent_Equipment_key_id}' and QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and FABLOCATION_ID = '{FabId}' ORDER BY  {ORDER_BY}".format(ContractRecordId=ContractRecordId,RevisionRecordId = RevisionRecordId,FabId=TreeParam,ORDER_BY = orderby,Parent_Equipmentid =Parent_Equipmentid,Parent_Equipment_key_id = Parent_Equipment_key_id))
                Count = Sql.GetFirst("select count(*) as cnt from SAQFEA (NOLOCK) where EQUIPMENT_ID = '{Parent_Equipmentid}' AND QTEREVFEQ_RECORD_ID = '{Parent_Equipment_key_id}' and QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and FABLOCATION_ID = '{FabId}'".format(ContractRecordId=ContractRecordId,RevisionRecordId = RevisionRecordId,FabId=TreeParam,Parent_Equipmentid =Parent_Equipmentid,Parent_Equipment_key_id = Parent_Equipment_key_id))
                if Count:
                    QueryCount = Count.cnt

            elif (TreeSuperParentParam.startswith("Sending") or TreeSuperParentParam.startswith("Receiving")):
                child_obj_recid = Sql.GetList( "select top "+str(PerPage)+" QUOTE_FAB_LOC_COV_OBJ_ASSEMBLY_RECORD_ID,EQUIPMENT_ID,SERIAL_NUMBER,ASSEMBLY_ID,ASSEMBLY_DESCRIPTION,GOT_CODE,INCLUDED,MNT_PLANT_ID,FABLOCATION_ID,WARRANTY_START_DATE,EQUIPMENTCATEGORY_ID,WARRANTY_END_DATE,SALESORG_ID,EQUIPMENTTYPE_ID AS ASSEMBLYTYPE_ID from SAQFEA (NOLOCK) where EQUIPMENT_ID = '{Parent_Equipmentid}' AND QTEREVFEQ_RECORD_ID = '{Parent_Equipment_key_id}' and QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and FABLOCATION_ID = '{FabId}' ORDER BY  {ORDER_BY}".format(ContractRecordId=ContractRecordId,RevisionRecordId = RevisionRecordId, FabId=TreeParentParam, ORDER_BY = orderby,Parent_Equipmentid =Parent_Equipmentid,Parent_Equipment_key_id = Parent_Equipment_key_id))
                Count = Sql.GetFirst( "select count(*) as cnt from SAQFEA (NOLOCK) where EQUIPMENT_ID = '{Parent_Equipmentid}' AND QTEREVFEQ_RECORD_ID = '{Parent_Equipment_key_id}' and QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and FABLOCATION_ID = '{FabId}'".format(ContractRecordId=ContractRecordId,RevisionRecordId = RevisionRecordId,FabId=TreeParentParam,Parent_Equipmentid =Parent_Equipmentid,Parent_Equipment_key_id = Parent_Equipment_key_id))
                if Count:
                    QueryCount = Count.cnt
        ...
        if TreeParentParam == "Fab Locations" or TreeSuperParentParam == "Fab Locations":
        ...
        child_obj_recid = Sql.GetList( "select top "+str(PerPage)+" QUOTE_FAB_LOC_COV_OBJ_ASSEMBLY_RECORD_ID,EQUIPMENT_ID,SERIAL_NUMBER,ASSEMBLY_ID,ASSEMBLY_DESCRIPTION,GOT_CODE,INCLUDED,MNT_PLANT_ID,FABLOCATION_ID,WARRANTY_START_DATE,EQUIPMENTCATEGORY_ID,WARRANTY_END_DATE,SALESORG_ID,EQUIPMENTTYPE_ID AS ASSEMBLYTYPE_ID from SAQFEA (NOLOCK) where EQUIPMENT_ID = '" + str(Parent_Equipmentid) + "' AND QTEREVFEQ_RECORD_ID = '" + str(Parent_Equipment_key_id) + "' and " + str(ATTRIBUTE_VALUE_STR) + " 1=1 and QUOTE_RECORD_ID = '" + str(ContractRecordId) + "' and QTEREV_RECORD_ID = '" + str(RevisionRecordId) + "' ORDER BY "+str(orderby)+"" )
        Count = Sql.GetFirst("select count(*) as cnt from SAQFEA (NOLOCK) where EQUIPMENT_ID = '" + str(Parent_Equipmentid) + "' and QTEREVFEQ_RECORD_ID = '" + str(Parent_Equipment_key_id) + "' and " + str(ATTRIBUTE_VALUE_STR) + " 1=1 and QUOTE_RECORD_ID = '" + str(ContractRecordId) + "' and QTEREV_RECORD_ID = '" + str(RevisionRecordId) + "' " )
        if Count:
            QueryCount = Count.cnt
    ...
    for cld in child_obj_recid:
        '''cld = Sql.GetFirst(
            "select QUOTE_FAB_LOC_COV_OBJ_ASSEMBLY_RECORD_ID,EQUIPMENT_ID,ASSEMBLY_ID,SERIAL_NUMBER,EQUIPMENTCATEGORY_ID,ASSEMBLY_DESCRIPTION,GOT_CODE,INCLUDED,MNT_PLANT_ID,FABLOCATION_ID,WARRANTY_START_DATE,WARRANTY_END_DATE,SALESORG_ID from SAQFEA (NOLOCK) where EQUIPMENT_ID = '{EquipmentID}' and QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}'  and FABLOCATION_ID = '{FablocationId}'".format(
...
def QuoteAssemblyPreventiveMaintainenceParent(PerPage, PageInform, A_Keys, A_Values,ASSEMBLYID,EQUIPMENTID):
    ...
    if str(SortColumn) == "SSCM_PM_FREQUENCY" or str(SortColumn) == "PM_FREQUENCY":
        ...
        Trace.Write('SortColumn---5049---'+str(SortColumn))
        Trace.Write('SortColumnOrder--5050----'+str(SortColumnOrder))
        sort_by = " ORDER BY "+str(SortColumn)+" "+str(SortColumnOrder)
    else:
        sort_by = ' ORDER BY QUOTE_SERVICE_COV_OBJ_ASS_PM_KIT_RECORD_ID'
    Trace.Write(str(SortColumn)+'--sort_by---'+str(sort_by))
    where_string = ""
    if A_Keys != "" and A_Values != "":
        A_Keys = list(A_Keys)
        A_Values = list(A_Values)
    ...
    if TreeSuperParentParam == "Comprehensive Services" or TreeSuperParentParam == "Complementary Products":
        ...
        QueryCountObj = Sql.GetFirst(
            "select count(QUOTE_SERVICE_COV_OBJ_ASS_PM_KIT_RECORD_ID) as cnt from SAQSAP (NOLOCK) INNER JOIN SAQSCA (NOLOCK) ON SAQSCA.QUOTE_SERVICE_COVERED_OBJECT_ASSEMBLIES_RECORD_ID = SAQSAP.QTESRVCOA_RECORD_ID where SAQSAP.QUOTE_RECORD_ID = '"
            + str(ContractRecordId)
            + "' and SAQSAP.QTEREV_RECORD_ID = '"
            + str(RevisionRecordId)
        ...
        Trace.Write('sort_by--5145---'+str(sort_by))
        Qstr = (
    ...
    if TopSuperParentParam in ('Comprehensive Services','Complementary Products'): 
        ...
        if TreeParentParam in ('Comprehensive Services','Complementary Products') or TreeSuperParentParam in ('Comprehensive Services','Complementary Products'):
            ...
            #INC08831137 - Start - M
            dbl_clk_function += (
                'function onClickCell(event, field, value, row, $element) { if(localStorage.getItem("InlineEdit")=="YES"){ return ;} reco_id = localStorage.getItem("multiedit_checkbox_clicked"); var reco_id=reco_id.split(",");var keyIndex = $("#table_Preventive_Maintainence_parent thead").find("[data-field=\'QUOTE_SERVICE_COV_OBJ_ASS_PM_KIT_RECORD_ID\']").index() || $("#table_Preventive_Maintainence_parent thead").find("[data-field=\'QUOTE_REV_PO_GRNBK_PM_EVEN_ASSEMBLIES_RECORD_ID\']").index();  keyIndex = keyIndex+1; var selectedId = $element.closest("tr").find("td:nth-child("+keyIndex+")").text().trim(); reco_id.push(selectedId); reco_id = removeDuplicates(reco_id);localStorage.setItem("multiedit_save_date", reco_id);  localStorage.setItem("PM_selectedId",selectedId);localStorage.setItem("table_id_RL_edit", "SYOBJR_00011_1E92CAAD_4EE9_4E5C_AA11_80F20D295A63");console.log("field--",field);console.log("value--",value);edit_index = $("'+str(table_ids)+'").find("[data-field="+ field +"]").index()+1;localStorage.setItem("edit_index",edit_index); debugger; cpq.server.executeScript("SYBLKETRLG", {"TITLE":field, "VALUE":value, "CLICKEDID":"SYOBJR_00011_1E92CAAD_4EE9_4E5C_AA11_80F20D295A63", "RECORDID":reco_id, "ELEMENT":"RELATEDEDIT"}, function(data) { debugger; localStorage.setItem("saqico_title",field); localStorage.setItem("PM_FrequencyBulkedit","yes");data1=data[0]; data2=data[1]; if(data1 != "NO"){ if(document.getElementById("RL_EDIT_DIV_ID") ) { document.getElementById("RL_EDIT_DIV_ID").innerHTML = data1; document.getElementById("cont_multiEditModalSection").style.display = "block"; $("#cont_multiEditModalSection").prepend("<div class=\'modal-backdrop fade in\'></div>"); var divHeight = $("#cont_multiEditModalSection").height(); $("#cont_multiEditModalSection .modal-backdrop").css("min-height", divHeight+"px"); $("#cont_multiEditModalSection .modal-dialog").css("width","550px"); $(".modal-dialog").css("margin-top","100px"); }   var divHeight = $("#cont_multiEditModalSection").height(); $("#cont_multiEditModalSection .modal-backdrop").css("min-height", divHeight+"px"); $("#cont_multiEditModalSection .modal-dialog").css("width","550px"); $(".modal-dialog").css("margin-top","100px"); } if (data2.length !== 0){ $.each( data2, function( key, values ) { onclick_datepicker(values) }); }  }); }'
            )
        dbl_clk_function += (
            'var checkedRows=[]; var keyIndex = $("#table_Preventive_Maintainence_parent thead").find("[data-field=\'QUOTE_SERVICE_COV_OBJ_ASS_PM_KIT_RECORD_ID\']").index() || $("#table_Preventive_Maintainence_parent thead").find("[data-field=\'QUOTE_REV_PO_GRNBK_PM_EVEN_ASSEMBLIES_RECORD_ID\']").index();  keyIndex = keyIndex+1; localStorage.setItem("multiedit_checkbox_clicked", []); debugger; $("'
            + str(table_ids)
            + '").on("check.bs.table", function (e, row, $element) { console.log("checked00009==");checkedRows.push($element.closest("tr").find("td:nth-child("+keyIndex+")").text()); localStorage.setItem("multiedit_checkbox_clicked", checkedRows); }); $("'
            + str(table_ids)
            + '").on("check-all.bs.table", function (e) { var table = $("'
            + str(table_ids)
            + '").closest("table"); table.find("tbody tr").each(function() { checkedRows.push($(this).find("td:nth-child("+keyIndex+")").text()); }); localStorage.setItem("multiedit_checkbox_clicked", checkedRows); }); $("'
            + str(table_ids)
            + '").on("uncheck-all.bs.table", function (e) { localStorage.setItem("multiedit_checkbox_clicked", []); checkedRows=[]; }); $("'
            + str(table_ids)
            + '").on("uncheck.bs.table", function (e, row, $element) { var rec_ids=$element.closest("tr").find("td:nth-child("+keyIndex+")").text(); $.each(checkedRows, function(index, value) { if (value === rec_ids) { checkedRows.splice(index,1); }}); localStorage.setItem("multiedit_checkbox_clicked", checkedRows); });'
        )
        ##INC08831137 - End - M
        dbl_clk_function += ('$(".bs-checkbox input").addClass("custom"); $(".bs-checkbox input").after("<span class=\'lbl\'></span>"); $("'
            + str(table_ids)
...
def QuoteAssemblyPreventiveMaintainenceKitMaterialChild(recid, PerPage, PageInform, A_Keys, A_Values,ASSEMBLYID,EQUIPMENTID,KITID,KITNUMBER):
    ...
    Trace.Write("treeparentparam"+str(TopSuperParentParam))
    ...
    Test = (
        ...
        + '___totalItemCount" >'
        + str(QueryCount)
        + '</span><div class="clear-padding fltltmrgtp3" ><div  class="pull-right vertmidtxtrht"><select onchange="PageFunctestChild(this,\'Quote\',\'\',\''+str(table_id)+'\')" id="'
        + str(table_id)
        + '___PageCountValue" class="form-control wid65vermiddisinbmarl5"><option value="10" selected>10</option><option value="20">20</option><option value="50">50</option><option value="100">100</option><option value="200">200</option></select> </div></div></div><div class="col-xs-8 col-md-4  clear-padding disinpad10txtcen"  data-bind="visible: totalItemCount"><div class="clear-padding col-xs-12 col-sm-6 col-md-12 bor0" ><ul class="pagination pagination"><li class="disabled"><a href="#" onclick="FirstPageLoad_paginationChild(\'Quote\',\'\',\''
        +str(table_id)
...
def QuoteAssemblyPreventiveMaintainenceParentFilter(ATTRIBUTE_NAME, ATTRIBUTE_VALUE,ASSEMBLYID,EQUIPMENTID,PerPage,PageInform):
    ...
    for quer_key, quer_value in enumerate(Dict_formation):
        ...
        if Dict_formation.get(quer_value) != "":
            ...
            if TreeSuperParentParam == "Comprehensive Services" or TreeSuperParentParam == "Complementary Products":
                if str(quer_values).find(",") == -1:
                    if x_picklistcheck == "TRUE":
                        ATTRIBUTE_VALUE_STR += "SAQSAP."+str(quer_value) + " = '" + str(quer_values) + "' and "
                    else:
                        ATTRIBUTE_VALUE_STR += "SAQSAP."+str(quer_value) + " like '%" + str(quer_values) + "%' and "
                else:
                    quer_values = quer_values.split(",")
                    quer_values = tuple(list(quer_values))
                    ATTRIBUTE_VALUE_STR += "SAQSAP."+str(quer_value) + " in " + str(quer_values) + " and "
                if str(quer_value) == 'QUOTE_SERVICE_COV_OBJ_ASS_PM_KIT_RECORD_ID':
                    if str(str(quer_values)).find("-") == -1:
                        ATTRIBUTE_VALUE_STR = (" SAQSAP.CpqTableEntryId = '"+ str(quer_values)+ "' and ")
                    else:
                        xa_str = str(quer_values).split("-")[1]
                        ATTRIBUTE_VALUE_STR = (" SAQSAP.CpqTableEntryId = '"+ str(xa_str)+ "' and ")
            else:
                if str(quer_values).find(",") == -1:
                    if x_picklistcheck == "TRUE":
                        ATTRIBUTE_VALUE_STR += str(quer_value) + " = '" + str(quer_values) + "' and "
    ...
    orderby = ""
    if str(SortColumn) == "SSCM_PM_FREQUENCY" or str(SortColumn) == "PM_FREQUENCY":
        orderby = " CAST("+str(SortColumn)+" AS FLOAT) "+str(SortColumnOrder)
    elif SortColumn != '' and SortColumnOrder !='':
        orderby = SortColumn + " " + SortColumnOrder
    else:
...
def QuoteAssemblyPreventiveMaintainenceKitMaterialChildFilter(ATTRIBUTE_NAME, ATTRIBUTE_VALUE, RECID,ASSEMBLYID,EQUIPMENTID,KITID,KITNUMBER,PerPage,PageInform,PM_ID):    
    FablocationId = Product.GetGlobal("TreeParam")
    ContractRecordId = Quote.GetGlobal("contract_quote_record_id")
    RevisionRecordId = Quote.GetGlobal("quote_revision_record_id")
    obj_id1 = "SYOBJ-00948"
...
def GetAssembliesMaster(PerPage, PageInform, A_Keys, A_Values):
    ...
    Test = (
        ...
        + str(QueryCount)
        + '</span><div class="clear-padding fltltmrgtp3" ><div  class="pull-right vertmidtxtrht"><select onchange="PageFunctestChild(this,\'Quotes\')" id="PageCountValue"  class="form-control wid65vermiddisinbmarl5"><option value="10" selected>10</option><option value="20">20</option><option value="50">50</option><option value="100">100</option><option value="200">200</option></select> </div></div></div><div class="col-xs-8 col-md-4  clear-padding disinpad10txtcen"  data-bind="visible: totalItemCount"><div class="clear-padding col-xs-12 col-sm-6 col-md-12 bor0" ><ul class="pagination pagination"><li class="disabled">'
        ""'<a href="#" onclick="FirstPageLoad_paginationChild(\'Quote\',\'\',\''
        +str(table_id)
...
def GetContractAssembliesMaster(PerPage, PageInform, A_Keys, A_Values):
    ...
    Test = (
        ...
        ""'<a href="#" onclick="FirstPageLoad_paginationChild(\'Quote\',\'\',\''
        +str(table_id)
        +'\')"><i class="fa fa-caret-left font14whtbld" ></i><i class="fa fa-caret-left font14" ></i></a></li><li class="disabled"><a href="#" onclick="Previous12334Child(\'Quotes\')"><i class="fa fa-caret-left font14" ></i>PREVIOUS</a></li><li class="disabled"><a href="#" class="disabledPage" onclick="Next12334Child(\'Quotes\')">NEXT<i class="fa fa-caret-right font14" ></i></a></li><li class="disabled">'
        ""'<a href="#" onclick="LastPageLoad_paginationChild(\'Quote\',\'\',\''
        +str(table_id)
        +'\')" class="disabledPage"><i class="fa fa-caret-right font14"></i><i class="fa fa-caret-right font14whtbld"></i></a></li></ul></div> </div> <div class="col-md-4 pr_page_pad"> <span id="page_count" class="currentPage page_right_content">1</span><span class="page_right_content pad_rt_2">Page </span></div></div>'
    )
...
def GetFtsAssembliesChild(recid, PerPage, PageInform, A_Keys, A_Values):
    ...
    #Columns = ["INCLUDED","QUOTE_REV_PO_GRNBK_PM_EVEN_ASSEMBLIES_RECORD_ID","ASSEMBLY_ID", "ASSEMBLY_DESCRIPTION", "EQUIPMENT_DESCRIPTION", "GOT_CODE"]
    Columns = ["INCLUDED","ASSEMBLY_ID", "ASSEMBLY_DESCRIPTION", "EQUIPMENT_DESCRIPTION", "GOT_CODE"]
    Objd_Obj = Sql.GetList(
    ...
    if Parent_Equipmentid:
        ...
        if TreeSuperParentParam == 'Product Offerings':
            child_obj_recid = Sql.GetList(
                "select distinct top 10 EQUIPMENT_ID,ASSEMBLY_ID,ASSEMBLY_DESCRIPTION,GOT_CODE, EQUIPMENT_DESCRIPTION,INCLUDED from SAQGPA (NOLOCK) where EQUIPMENT_ID = '{Parent_Equipmentid}' and QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and SERVICE_ID = '{TreeParam}' ".format(
            ...
            QueryCountObj = Sql.GetFirst(
                "select count(distinct ASSEMBLY_ID) as cnt from SAQGPA (NOLOCK) where QUOTE_RECORD_ID = '"
                + str(ContractRecordId)
                + "' and QTEREV_RECORD_ID = '"
                + str(RevisionRecordId)
                + "' and EQUIPMENT_ID ='"
        ...
        if TreeTopSuperParentParam == 'Product Offerings':
            child_obj_recid = Sql.GetList(
                "select distinct top 10 EQUIPMENT_ID,ASSEMBLY_ID,ASSEMBLY_DESCRIPTION,GOT_CODE, EQUIPMENT_DESCRIPTION,INCLUDED from SAQGPA (NOLOCK) where EQUIPMENT_ID = '{Parent_Equipmentid}' and QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and SERVICE_ID = '{TreeParentParam}' ".format(
                    ContractRecordId=Quote.GetGlobal("contract_quote_record_id"), RevisionRecordId = Quote.GetGlobal("quote_revision_record_id"),Parent_Equipmentid=recid, TreeParentParam=TreeParentParam
                )
            )
    ...
    filter_control_function += (
        ...
        + '").click( function(){ var table_id = $(this).closest("table").attr("id"); ATTRIBUTE_VALUEList = []; '
        + str(values_list)
        + ' var attribute_value = $(this).val(); cpq.server.executeScript("CQNESTGRID", {"TABNAME":"Fts Assemblies Child", "ACTION":"PRODUCT_ONLOAD_FILTER", "ATTRIBUTE_NAME": '
        + str(list(Columns))
        + ', "ATTRIBUTE_VALUE": ATTRIBUTE_VALUEList, "RECID":"'
        + str(recid)
        + '" }, function(data) { $("'
    ...
    ##INC08691509 M
    Trace.Write('8096---.')
    if isBookedQuote.REVISION_STATUS  not in ('CFG-CONFIGURING'):
        INCLUDE_CHECK = 'DISABLE_INCLUDE_CHECK'
    else:
        INCLUDE_CHECK = 'INCLUDE_CHECK'
    return (
...
def GetFtsAssembliesChildFilter(ATTRIBUTE_NAME, ATTRIBUTE_VALUE, RECID):
    ...
    if ATTRIBUTE_VALUE is None or ATTRIBUTE_VALUE == "" or ATTRIBUTE_VALUE_STR is None or ATTRIBUTE_VALUE_STR == "":
        parent_obj = Sql.GetList(
            "select distinct EQUIPMENT_ID,ASSEMBLY_ID,ASSEMBLY_DESCRIPTION,GOT_CODE, EQUIPMENT_DESCRIPTION,INCLUDED from SAQGPA (NOLOCK) where EQUIPMENT_ID = '{recid}' and QUOTE_RECORD_ID = '{ContractRecordId}'  and QTEREV_RECORD_ID = '{RevisionRecordId}' and SERVICE_ID = '{TreeParam}'".format(
        ...
        parent_obj = Sql.GetList(
            "select distinct EQUIPMENT_ID,ASSEMBLY_ID,ASSEMBLY_DESCRIPTION,GOT_CODE, EQUIPMENT_DESCRIPTION,INCLUDED from SAQGPA (NOLOCK) where  "
            + str(ATTRIBUTE_VALUE_STR)
...
def UpdateBreadcrumb():    
    ...
    ##A055S000P01-8690 code ends..      
        ...
        if qry:
            ...
            eq_id = "Line"
    elif TreeParam == "Add-On Products" and TABLENAME == "SAQSGB":
        qry = Sql.GetFirst(
    ...
    Action_Str += '</abbr></a><span class="angle_symbol"><img src="/mt/appliedmaterials_prd/images/productimages/BREADCRUMB_ICON_TRANS.PNG"></span></li>'
...
def GetCovObjMaster(PerPage, PageInform, A_Keys, A_Values):    
    ...
    if TreeSuperParentParam == "Product Offerings" or (TreeTopSuperParentParam== "Comprehensive Services" and TreeParentParam=="Add-On Products"):
        ...
        Qstr = (
            "select top "
            + str(PerPage)
            + " * from ( select ROW_NUMBER() OVER( ORDER BY "+str(orderby)+") AS ROW, QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID,EQUIPMENT_ID,EQUIPMENT_DESCRIPTION,PLATFORM,SERIAL_NO,GREENBOOK,FABLOCATION_ID,WARRANTY_END_DATE,WARRANTY_END_DATE_ALERT,WARRANTY_START_DATE,CONTRACT_VALID_FROM,CONTRACT_VALID_TO,MNT_PLANT_ID,EQUIPMENT_STATUS,CUSTOMER_TOOL_ID,EQUIPMENTCATEGORY_DESCRIPTION AS EQUIPMENTCATEGORY_DESCRIPTION,SNDFBL_ID,TEMP_TOOL,IBASE_ATTSTS,ENTINT_STATUS,ENTINT_STATUS_DESC from SAQSCO (NOLOCK) where QUOTE_RECORD_ID = '"
            + str(ContractRecordId)
            + "' and QTEREV_RECORD_ID = '"
            + str(RevisionRecordId)
            + "' and SERVICE_ID = '"
        ...
        Qstr = (
            "select top "
            + str(PerPage)
            + " * from ( select ROW_NUMBER() OVER( ORDER BY QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID) AS ROW, QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID,EQUIPMENT_ID,EQUIPMENT_DESCRIPTION,PLATFORM,SERIAL_NO,GREENBOOK,FABLOCATION_ID,WARRANTY_END_DATE,WARRANTY_END_DATE_ALERT,WARRANTY_START_DATE,CONTRACT_VALID_FROM,CONTRACT_VALID_TO,MNT_PLANT_ID,EQUIPMENT_STATUS,CUSTOMER_TOOL_ID,EQUIPMENTCATEGORY_DESCRIPTION AS EQUIPMENTCATEGORY_DESCRIPTION,SNDFBL_ID,TEMP_TOOL,IBASE_ATTSTS,ENTINT_STATUS,ENTINT_STATUS_DESC from SAQSCO (NOLOCK) where QUOTE_RECORD_ID = '"
            + str(ContractRecordId)
            + "' and QTEREV_RECORD_ID = '"
            + str(RevisionRecordId)
            + "' and SERVICE_ID = '"
        ...
        if TreeTopSuperParentParam == "Product Offerings":
            ...
            Qstr = (
                "select top "
                + str(PerPage)
                + " * from ( select ROW_NUMBER() OVER( ORDER BY QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID) AS ROW, QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID,EQUIPMENT_ID,EQUIPMENT_DESCRIPTION,SERIAL_NO,GREENBOOK,FABLOCATION_ID,WARRANTY_END_DATE,WARRANTY_END_DATE_ALERT,WARRANTY_START_DATE,CONTRACT_VALID_FROM,CONTRACT_VALID_TO,MNT_PLANT_ID,EQUIPMENT_STATUS,CUSTOMER_TOOL_ID,EQUIPMENTCATEGORY_DESCRIPTION AS EQUIPMENTCATEGORY_DESCRIPTION,SNDFBL_ID from SAQSCO (NOLOCK) where QUOTE_RECORD_ID = '"
                + str(ContractRecordId)
                + "'  and QTEREV_RECORD_ID = '"
                + str(RevisionRecordId)
                + "'and SERVICE_ID = '"
            ...
            if TreeParentParam == "Receiving Equipment":
                ...
                service_id = Product.GetGlobal("addon_service_id")
                #A055S000P01-20610 start - A
                Qstr = (
                    "select top "
                    + str(PerPage)
                    ...
                    + "'  and QTEREV_RECORD_ID = '"
                    + str(RevisionRecordId)
                    + "' and SERVICE_TYPE = 'Add-On Products' and SERVICE_ID = '"+str(service_id)+"' and PAR_SERVICE_ID = '"+str(TreeSuperParentParam)+"' and GREENBOOK = '"
                    + str(TreeParentParam)
                    + "') m where m.ROW BETWEEN "
                    + str(Page_start)
                    + " and "
    ...
    for par in parent_obj:
        ...
        #A055S000P01-20610 start - A
        ent_status_des = str(par.ENTINT_STATUS_DESC)
        #A055S000P01-20610 end - A
        Trace.Write('Qstr-WARRANTY_END_DATE_ALERT ----'+str(par.WARRANTY_END_DATE_ALERT))
        ibase_status = str(par.IBASE_ATTSTS)
        if str(ibase_status) == "INCOMPLETE":
            ibase_attributes = par.EQUIPMENT_ID
            ibase_icon = '<a class = "cust_exc" title="Missing iBase Attributes for assemblies"><img src="/mt/appliedmaterials_prd/Additionalfiles/Icons/Exclamation.svg"></a>'
            ibase_missing_flag = True
        elif str(ibase_status) =="NO ASSEMBLY":
            ibase_icon = '<a class = "cust_exc" title="Assembly is Missing"><img src="/mt/'+str(Sql.getDomainDetails())+'/Additionalfiles/Orange1_Circle.svg"></a>'
            assembly_missing_flag = True
        #A055S000P01-20610 start - A
            ...
            ibase_icon = '<a class = "cust_exc" title="'+str(ent_status_des)+'"><img src="/mt/'+str(Sql.getDomainDetails())+'/Additionalfiles/Icons/red_tag_icon.svg"></a>'
            req_event_missing_flag = True
            Trace.Write("--------------------------1-"+str(ent_status_des))
        #A055S000P01-20610 end - A
        else:
            ibase_icon = '<a class = "cust_exc" title="Complete"><img src="/mt/appliedmaterials_prd/Additionalfiles/Green_Tick.svg"></a>'
        Action_str = (
        ...
        if can_delete.upper() == "TRUE":
            Trace.Write("test7003")
            Action_str += '<li><a class="dropdown-item" id="deletebtn" data-target="#cont_CommonModalDelete" data-toggle="modal" onclick="CommonDelete(this, \'SAQSCO#'+ data_id +'\', \'WARNING\')" href="#">DELETE</a></li>'
        """if can_add.upper() == "TRUE" and par.MARKET_TYPE == "NON MARKET BASED" and par.MODEL_TYPE != "COST PLUS":
            Action_str += (
                '<li><a class="dropdown-item" id="'
                + str(data_id)
        ...
        data_dict = {}
        data_dict["ids"] = str(data_id)
        data_dict["IBASE_ATTSTS"] = str(ibase_icon)
        data_dict["ACTIONS"] = str(Action_str)
        data_dict["QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID"] = CPQID.KeyCPQId.GetCPQId(
            "SAQSCO", str(par.QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID)
        )
        data_dict["EQUIPMENT_ID"] = ('<abbr id ="" title="' + str(par.EQUIPMENT_ID) + '">' + str(par.EQUIPMENT_ID) + "</abbr>") 
        data_dict["EQUIPMENT_DESCRIPTION"] = ('<abbr id ="" title="' + par.EQUIPMENT_DESCRIPTION + '">' + par.EQUIPMENT_DESCRIPTION + "</abbr>")
        data_dict["PLATFORM"] = ('<abbr id ="" title="' + str(par.PLATFORM) + '">' + str(par.PLATFORM) + "</abbr>")
        data_dict["FABLOCATION_ID"] = ('<abbr id ="" title="' + str(par.SNDFBL_ID) + '">' + str(par.FABLOCATION_ID) + "</abbr>") 
        data_dict["CONTRACT_VALID_FROM"] = ('<abbr id ="" title="' + str(par.CONTRACT_VALID_FROM) + '">' + str(par.CONTRACT_VALID_FROM) + "</abbr>")
        data_dict["CONTRACT_VALID_TO"] = ('<abbr id ="" title="' + str(par.CONTRACT_VALID_TO) + '">' + str(par.CONTRACT_VALID_TO) + "</abbr>")
    ...
    #CR BULK DELETE STARTS
    cls = "eq(2)"
    if (str(Param.ACTION) == 'LOAD' and (str(active_subtab) == 'Equipment' or str(Param.TABNAME) == 'Covered Object Parent')):
        dbl_clk_function = (
            'localStorage.setItem("selectall","no"); var checkedRows=[]; localStorage.setItem("multiedit_checkbox_clicked", []); $("'
            + str(table_ids)
            + '").on("check.bs.table", function (e, row, $element) { console.log("checked00009==2255==");checkedRows.push($element.closest("tr").find("td:'
            + str(cls)
            + '").text()); localStorage.setItem("multiedit_checkbox_clicked", checkedRows); if(checkedRows.length == ' + str(noOfRecords) + ') { localStorage.setItem("selectall","yes"); } if(checkedRows.length > 1 && "' + str(TreeParam) + '" != "Add-On Products"){ $("#delete_receive_equipment").css("display","block");} else { $("#delete_receive_equipment").css("display","none");} }); $("'
            + str(table_ids)
            + '").on("check-all.bs.table", function (e) { localStorage.setItem("selectall","yes"); localStorage.setItem("multiedit_checkbox_clicked", []);  checkedRows=[]; var table = $("'
            + str(table_ids)
            + '").closest("table"); table.find("tbody tr").each(function() { rowKey = $(this).find("td:nth-child(3)").text(); checkedRows.push(rowKey); }); localStorage.setItem("multiedit_checkbox_clicked", checkedRows); if ("' + str(TreeParam) + '" != "Add-On Products") { $("#delete_receive_equipment").css("display","block");} }); $("'
            + str(table_ids)
            + '").on("uncheck-all.bs.table", function (e) { localStorage.setItem("selectall","no"); localStorage.setItem("multiedit_checkbox_clicked", []); checkedRows=[]; $("#delete_receive_equipment").css("display","none"); }); $("'
            + str(table_ids)
            + '").on("uncheck.bs.table", function (e, row, $element) { localStorage.setItem("selectall","no");var rec_ids=$element.closest("tr").find("td:'
            + str(cls)
            + '").text(); $.each(checkedRows, function(index, value) { if (value === rec_ids) { checkedRows.splice(index,1); }}); localStorage.setItem("multiedit_checkbox_clicked", checkedRows); if(checkedRows.length > 1 && "' + str(TreeParam) + '" != "Add-On Products"){ $("#delete_receive_equipment").css("display","block");} else { $("#delete_receive_equipment").css("display","none");}});'
        )
    #CR BULK DELETE ENDS
    dbl_clk_function += (
    ...
    for key, col_name in enumerate(list(Columns)):
        ...
        try:
            ...
            if str(objss_obj.PICKLIST).upper() == "TRUE":
                Trace.Write('type'+str(objss_obj.API_NAME))
                filter_level_data = "select"
                filter_clas_name = (
                    '<div id = "'+ str(table_id)+ '_RelatedMutipleCheckBoxDrop_'+ str(key)+ '" class="form-control bootstrap-table-filter-control-'
                    + str(col_name)
...
def GetCovObjChild(recid, PerPage, PageInform, A_Keys, A_Values, reckeyid):
    TreeParam = Product.GetGlobal("TreeParam")
    TreeParentParam = Product.GetGlobal("TreeParentLevel0")
    TreeSuperParentParam = Product.GetGlobal("TreeParentLevel1")
    TreeTopSuperParentParam = Product.GetGlobal("TreeParentLevel2")
    ...
    if Objd_Obj is not None:
        ...
        lookup_list = {ins.LOOKUP_API_NAME: ins.API_NAME for ins in Objd_Obj}
    lookup_str = ",".join(list(lookup_disply_list))
    get_tool_details =Sql.GetFirst("Select QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID,EQUIPMENT_ID FROM SAQSCO (NOLOCK) WHERE QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' AND EQUIPMENT_ID = '{Equipment_Id}' AND CpqTableEntryId = '{reckeyid}'".format(ContractRecordId = ContractRecordId,RevisionRecordId = RevisionRecordId,Equipment_Id = recid,reckeyid = reckeyid))
    Parent_Equipmentid = get_tool_details.EQUIPMENT_ID
    Parent_Equipment_key_id = get_tool_details.QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID
    if Parent_Equipmentid:
        if TreeTopSuperParentParam == "Product Offerings" or TreeSuperParentParam == "Comprehensive Services":
            child_obj_recid = Sql.GetList("select top "+str(PerPage)+" * from (select ROW_NUMBER() OVER( ORDER BY QUOTE_SERVICE_COVERED_OBJECT_ASSEMBLIES_RECORD_ID) AS ROW, QUOTE_SERVICE_COVERED_OBJECT_ASSEMBLIES_RECORD_ID,EQUIPMENT_ID,ASSEMBLY_ID,ASSEMBLY_DESCRIPTION,EQUIPMENTTYPE_ID,GOT_CODE,EQUIPMENT_DESCRIPTION,MNT_PLANT_ID,FABLOCATION_ID,WARRANTY_START_DATE,WARRANTY_END_DATE,INCLUDED from SAQSCA (NOLOCK) where EQUIPMENT_ID = '{Parent_Equipmentid}' and QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and SERVICE_ID = '{Service_Id}' and QTESRVCOB_RECORD_ID = '{Parent_Equipment_key_id}' ) m where m.ROW BETWEEN ".format(ContractRecordId = ContractRecordId,RevisionRecordId = RevisionRecordId,Parent_Equipmentid = Parent_Equipmentid,Service_Id=TreeParentParam,Parent_Equipment_key_id = Parent_Equipment_key_id)+ str(Page_start)+ " and "+ str(Page_End))

            QueryCountObj = Sql.GetFirst("select count(CpqTableEntryId) as cnt from SAQSCA (NOLOCK) where QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and EQUIPMENT_ID ='{Parent_Equipmentid}' and QTESRVCOB_RECORD_ID = '{Parent_Equipment_key_id}' and SERVICE_ID ='{Service_Id}'".format(ContractRecordId = ContractRecordId,RevisionRecordId = RevisionRecordId,Parent_Equipmentid = Parent_Equipmentid,Service_Id=TreeParentParam,Parent_Equipment_key_id = Parent_Equipment_key_id))
        elif TreeSuperParentParam == "Add-On Products":
            child_obj_recid = Sql.GetList("select top "+str(PerPage)+" * from (select ROW_NUMBER() OVER( ORDER BY QUOTE_SERVICE_COVERED_OBJECT_ASSEMBLIES_RECORD_ID) AS ROW, QUOTE_SERVICE_COVERED_OBJECT_ASSEMBLIES_RECORD_ID,EQUIPMENT_ID,ASSEMBLY_ID,ASSEMBLY_DESCRIPTION,EQUIPMENTTYPE_ID,GOT_CODE,EQUIPMENT_DESCRIPTION,MNT_PLANT_ID,FABLOCATION_ID,WARRANTY_START_DATE,WARRANTY_END_DATE,INCLUDED from SAQSCA (NOLOCK) where EQUIPMENT_ID = '{Parent_Equipmentid}' and QTESRVCOB_RECORD_ID = '{Parent_Equipment_key_id}' and QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and SERVICE_ID = '{Service_Id}') m where m.ROW BETWEEN ".format(ContractRecordId = ContractRecordId,RevisionRecordId = RevisionRecordId,Parent_Equipmentid = Parent_Equipmentid,Service_Id = TreeParentParam,Parent_Equipment_key_id = Parent_Equipment_key_id)+ str(Page_start)+ " and "+ str(Page_End))
            
            QueryCountObj = Sql.GetFirst("select count(CpqTableEntryId) as cnt from SAQSCA (NOLOCK) where QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and EQUIPMENT_ID ='{Parent_Equipmentid}' and QTESRVCOB_RECORD_ID = '{Parent_Equipment_key_id}' and SERVICE_ID ='{Service_Id}'".format(ContractRecordId = ContractRecordId,RevisionRecordId = RevisionRecordId,Parent_Equipmentid = Parent_Equipmentid,Service_Id=TreeParentParam,Parent_Equipment_key_id = Parent_Equipment_key_id))          
        elif (TreeParam.startswith("Sending") or TreeParam.startswith("Receiving")):
            child_obj_recid = Sql.GetList("select top "+str(PerPage)+" * from (select ROW_NUMBER() OVER( ORDER BY QUOTE_SERVICE_COVERED_OBJECT_ASSEMBLIES_RECORD_ID) AS ROW, QUOTE_SERVICE_COVERED_OBJECT_ASSEMBLIES_RECORD_ID,EQUIPMENT_ID,ASSEMBLY_ID,ASSEMBLY_DESCRIPTION,EQUIPMENTTYPE_ID,GOT_CODE,EQUIPMENT_DESCRIPTION,MNT_PLANT_ID,FABLOCATION_ID,WARRANTY_START_DATE,WARRANTY_END_DATE,INCLUDED from SAQSCA (NOLOCK) where EQUIPMENT_ID = '{Parent_Equipmentid}' and QTESRVCOB_RECORD_ID = '{Parent_Equipment_key_id}' and QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and SERVICE_ID = '{Service_Id}') m where m.ROW BETWEEN ".format(ContractRecordId=ContractRecordId,Parent_Equipmentid=Parent_Equipmentid,Service_Id=TreeParentParam,RevisionRecordId=RevisionRecordId,Parent_Equipment_key_id = Parent_Equipment_key_id)+ str(Page_start)+ " and "+ str(Page_End))
            
            QueryCountObj = Sql.GetFirst("select count(CpqTableEntryId) as cnt from SAQSCA (NOLOCK) where QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}'and EQUIPMENT_ID ='{Parent_Equipmentid}' and QTESRVCOB_RECORD_ID = '{Parent_Equipment_key_id}' and SERVICE_ID = '{TreeParentParam}'".format(ContractRecordId=ContractRecordId,Parent_Equipmentid=Parent_Equipmentid,Service_Id=TreeParentParam,RevisionRecordId=RevisionRecordId,Parent_Equipment_key_id = Parent_Equipment_key_id))
        elif TreeTopSuperParentParam == "Product Offerings":
            child_obj_recid = Sql.GetList("select top "+str(PerPage)+" * from (select ROW_NUMBER() OVER( ORDER BY QUOTE_SERVICE_COVERED_OBJECT_ASSEMBLIES_RECORD_ID) AS ROW, QUOTE_SERVICE_COVERED_OBJECT_ASSEMBLIES_RECORD_ID,EQUIPMENT_ID,ASSEMBLY_ID,ASSEMBLY_DESCRIPTION,EQUIPMENTTYPE_ID,GOT_CODE,EQUIPMENT_DESCRIPTION,MNT_PLANT_ID,FABLOCATION_ID,WARRANTY_START_DATE,WARRANTY_END_DATE,INCLUDED from SAQSCA (NOLOCK) where EQUIPMENT_ID = '{Parent_Equipmentid}' and QTESRVCOB_RECORD_ID = '{Parent_Equipment_key_id}' and QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and SERVICE_ID = '{Service_Id}' AND FABLOCATION_ID = '{fab}') m where m.ROW BETWEEN ".format(ContractRecordId=ContractRecordId,RevisionRecordId = RevisionRecordId,Parent_Equipmentid=Parent_Equipmentid,Parent_Equipment_key_id = Parent_Equipment_key_id,Service_Id=TreeParentParam,fab=TreeParam)+ str(Page_start)+ " and "+ str(Page_End))
            
            QueryCountObj = Sql.GetFirst( "select count(CpqTableEntryId) as cnt from SAQSCA (NOLOCK) where QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and EQUIPMENT_ID ='{Parent_Equipmentid}' and QTESRVCOB_RECORD_ID = '{Parent_Equipment_key_id}' and SERVICE_ID ='{Service_Id}' AND FABLOCATION_ID = '{fab}".format(ContractRecordId=ContractRecordId,RevisionRecordId = RevisionRecordId,Parent_Equipmentid=Parent_Equipmentid,Parent_Equipment_key_id = Parent_Equipment_key_id,Service_Id=TreeParentParam,fab=TreeParam))           
        elif TreeSuperParentParam == "Product Offerings":
            child_obj_recid = Sql.GetList("select top "+str(PerPage)+" * from (select ROW_NUMBER() OVER( ORDER BY QUOTE_SERVICE_COVERED_OBJECT_ASSEMBLIES_RECORD_ID) AS ROW, QUOTE_SERVICE_COVERED_OBJECT_ASSEMBLIES_RECORD_ID,EQUIPMENT_ID,ASSEMBLY_ID,ASSEMBLY_DESCRIPTION,EQUIPMENTTYPE_ID,GOT_CODE,EQUIPMENT_DESCRIPTION,MNT_PLANT_ID,FABLOCATION_ID,WARRANTY_START_DATE,WARRANTY_END_DATE,INCLUDED from SAQSCA (NOLOCK) where EQUIPMENT_ID = '{Parent_Equipmentid}' and QTESRVCOB_RECORD_ID = '{Parent_Equipment_key_id}' and QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and SERVICE_ID = '{Service_Id}') m where m.ROW BETWEEN ".format(ContractRecordId = ContractRecordId,RevisionRecordId = RevisionRecordId, Parent_Equipmentid=Parent_Equipmentid,Parent_Equipment_key_id =Parent_Equipment_key_id,Service_Id = TreeParam)+ str(Page_start)+ " and " + str(Page_End))
            
            QueryCountObj = Sql.GetFirst( "select count(CpqTableEntryId) as cnt from SAQSCA (NOLOCK) where QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and EQUIPMENT_ID ='{Parent_Equipmentid}' and QTESRVCOB_RECORD_ID = '{Parent_Equipment_key_id}' and SERVICE_ID ='{Service_Id}'".format(ContractRecordId = ContractRecordId,RevisionRecordId = RevisionRecordId,Parent_Equipmentid=Parent_Equipmentid,Parent_Equipment_key_id =Parent_Equipment_key_id,Service_Id = TreeParam))
        elif TreeTopSuperParentParam == "Add-On Products":
            child_obj_recid = Sql.GetList("select top "+str(PerPage)+" * from (select ROW_NUMBER() OVER( ORDER BY QUOTE_SERVICE_COVERED_OBJECT_ASSEMBLIES_RECORD_ID) AS ROW, QUOTE_SERVICE_COVERED_OBJECT_ASSEMBLIES_RECORD_ID,EQUIPMENT_ID,ASSEMBLY_ID,ASSEMBLY_DESCRIPTION,EQUIPMENTTYPE_ID,GOT_CODE,EQUIPMENT_DESCRIPTION,MNT_PLANT_ID,FABLOCATION_ID,WARRANTY_START_DATE,WARRANTY_END_DATE,INCLUDED from SAQSCA (NOLOCK) where EQUIPMENT_ID = '{Parent_Equipmentid}' and QTESRVCOB_RECORD_ID = '{Parent_Equipment_key_id}' and QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and SERVICE_ID = '{Service_Id}' AND FABLOCATION_ID = '{fab}') m where m.ROW BETWEEN ".format(ContractRecordId = ContractRecordId, RevisionRecordId = RevisionRecordId , Parent_Equipmentid=Parent_Equipmentid,Parent_Equipment_key_id = Parent_Equipment_key_id,Service_Id=TreeSuperParentParam, fab=TreeParentParam)+str(Page_start) + " and " + str(Page_End))
            
            QueryCountObj = Sql.GetFirst( "select count(CpqTableEntryId) as cnt from SAQSCA (NOLOCK) where QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and EQUIPMENT_ID ='{Parent_Equipmentid}' and QTESRVCOB_RECORD_ID = '{Parent_Equipment_key_id}' and SERVICE_ID ='{Service_Id}' AND FABLOCATION_ID = '{fab}'".format(ContractRecordId = ContractRecordId, RevisionRecordId = RevisionRecordId , Parent_Equipmentid=Parent_Equipmentid,Parent_Equipment_key_id = Parent_Equipment_key_id,Service_Id=TreeSuperParentParam, fab=TreeParentParam))
        elif (TreeTopSuperParentParam == "Comprehensive Services" or TreeTopSuperParentParam == "Complementary Products") and TreeParentParam != "Receiving Equipment" and TreeParam != "Add-On Products":
            child_obj_recid = Sql.GetList("select top "+str(PerPage)+" * from (select ROW_NUMBER() OVER( ORDER BY QUOTE_SERVICE_COVERED_OBJECT_ASSEMBLIES_RECORD_ID) AS ROW, QUOTE_SERVICE_COVERED_OBJECT_ASSEMBLIES_RECORD_ID,EQUIPMENT_ID,ASSEMBLY_ID,ASSEMBLY_DESCRIPTION,EQUIPMENTTYPE_ID,GOT_CODE,EQUIPMENT_DESCRIPTION,MNT_PLANT_ID,FABLOCATION_ID,WARRANTY_START_DATE,WARRANTY_END_DATE,INCLUDED from SAQSCA (NOLOCK) where EQUIPMENT_ID = '{Parent_Equipmentid}' and QTESRVCOB_RECORD_ID = '{Parent_Equipment_key_id}' and QUOTE_RECORD_ID = '{ContractRecordId}'  and QTEREV_RECORD_ID = '{RevisionRecordId}' and SERVICE_ID = '{Service_Id}' AND FABLOCATION_ID = '{fab}') m where m.ROW BETWEEN ".format(ContractRecordId=ContractRecordId, RevisionRecordId = RevisionRecordId, Parent_Equipmentid=Parent_Equipmentid,Parent_Equipment_key_id = Parent_Equipment_key_id,Service_Id=TreeSuperParentParam,fab=TreeParentParam) + str(Page_start) + " and " + str(Page_End))
            
            QueryCountObj = Sql.GetFirst("select count(CpqTableEntryId) as cnt from SAQSCA (NOLOCK) where QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId'and EQUIPMENT_ID ='{Parent_Equipmentid}' and QTESRVCOB_RECORD_ID = '{Parent_Equipment_key_id}' and SERVICE_ID ='{Service_Id}' AND FABLOCATION_ID = '{fab}".format(ContractRecordId=ContractRecordId, RevisionRecordId = RevisionRecordId, Parent_Equipmentid=Parent_Equipmentid,Parent_Equipment_key_id =Parent_Equipment_key_id,Service_Id=TreeSuperParentParam, fab=TreeParentParam))
        elif TreeParam == "Add-On Products" and (TreeTopSuperParentParam == "Comprehensive Services" or TreeTopSuperParentParam == "Complementary Products"):
            child_obj_recid = Sql.GetList( "select top "+str(PerPage)+" * from (select ROW_NUMBER() OVER( ORDER BY QUOTE_SERVICE_COVERED_OBJECT_ASSEMBLIES_RECORD_ID) AS ROW, QUOTE_SERVICE_COVERED_OBJECT_ASSEMBLIES_RECORD_ID,EQUIPMENT_ID,ASSEMBLY_ID,ASSEMBLY_DESCRIPTION,EQUIPMENTTYPE_ID,GOT_CODE,EQUIPMENT_DESCRIPTION,MNT_PLANT_ID,FABLOCATION_ID,WARRANTY_START_DATE,WARRANTY_END_DATE,INCLUDED from SAQSCA (NOLOCK) where EQUIPMENT_ID = '{Parent_Equipmentid}' and QTESRVCOB_RECORD_ID = '{Parent_Equipment_key_id}' and QUOTE_RECORD_ID = '{ContractRecordId}'  and QTEREV_RECORD_ID = '{RevisionRecordId}' and SERVICE_ID = '{Service_Id}') m where m.ROW BETWEEN ".format(ContractRecordId=ContractRecordId, RevisionRecordId = RevisionRecordId,Parent_Equipmentid=Parent_Equipmentid,Parent_Equipment_key_id =Parent_Equipment_key_id,Service_Id=TreeSuperParentParam) + str(Page_start) + " and " + str(Page_End))
            
            QueryCountObj = Sql.GetFirst("select count(CpqTableEntryId) as cnt from SAQSCA (NOLOCK) where QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and EQUIPMENT_ID = '{Parent_Equipmentid}' and QTESRVCOB_RECORD_ID = '{Parent_Equipment_key_id}' and SERVICE_ID ='{Service_Id}'".format(ContractRecordId=ContractRecordId, RevisionRecordId = RevisionRecordId,Parent_Equipmentid=Parent_Equipmentid,Parent_Equipment_key_id =Parent_Equipment_key_id,Service_Id=TreeSuperParentParam))
        elif TreeParentParam == "Receiving Equipment":
            child_obj_recid = Sql.GetList( "select top "+str(PerPage)+" * from (select ROW_NUMBER() OVER( ORDER BY QUOTE_SERVICE_COVERED_OBJECT_ASSEMBLIES_RECORD_ID) AS ROW, QUOTE_SERVICE_COVERED_OBJECT_ASSEMBLIES_RECORD_ID,EQUIPMENT_ID,ASSEMBLY_ID,ASSEMBLY_DESCRIPTION,EQUIPMENTTYPE_ID,GOT_CODE,EQUIPMENT_DESCRIPTION,MNT_PLANT_ID,FABLOCATION_ID,WARRANTY_START_DATE,WARRANTY_END_DATE,INCLUDED from SAQSCA (NOLOCK) where EQUIPMENT_ID = '{Parent_Equipmentid}' and QTESRVCOB_RECORD_ID = '{Parent_Equipment_key_id}' and QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and SERVICE_ID = '{Service_Id}' AND FABLOCATION_ID = '{fab}') m where m.ROW BETWEEN ".format(ContractRecordId=ContractRecordId, RevisionRecordId = RevisionRecordId, Parent_Equipmentid=Parent_Equipmentid, Service_Id=TreeSuperParentParam,fab=TreeParam,Parent_Equipment_key_id = Parent_Equipment_key_id) + str(Page_start) + " and " + str(Page_End) )

            QueryCountObj = Sql.GetFirst("select count(CpqTableEntryId) as cnt from SAQSCA (NOLOCK) where QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and EQUIPMENT_ID ='{Parent_Equipmentid}' and QTESRVCOB_RECORD_ID = '{Parent_Equipment_key_id}' and SERVICE_ID ='{Service_Id}' AND FABLOCATION_ID = '{fab}'".format(ContractRecordId=ContractRecordId, RevisionRecordId = RevisionRecordId, Parent_Equipmentid=Parent_Equipmentid, Service_Id=TreeSuperParentParam, fab=TreeParam,Parent_Equipment_key_id = Parent_Equipment_key_id)) 
        elif TreeSuperParentParam == "Receiving Equipment":
            child_obj_recid = Sql.GetList("select top "+str(PerPage)+" * from (select ROW_NUMBER() OVER( ORDER BY QUOTE_SERVICE_COVERED_OBJECT_ASSEMBLIES_RECORD_ID) AS ROW, QUOTE_SERVICE_COVERED_OBJECT_ASSEMBLIES_RECORD_ID,EQUIPMENT_ID,ASSEMBLY_ID,ASSEMBLY_DESCRIPTION,EQUIPMENTTYPE_ID,GOT_CODE,EQUIPMENT_DESCRIPTION,MNT_PLANT_ID,FABLOCATION_ID,WARRANTY_START_DATE,WARRANTY_END_DATE,INCLUDED from SAQSCA (NOLOCK) where EQUIPMENT_ID = '{Parent_Equipmentid}' and QTESRVCOB_RECORD_ID = '{Parent_Equipment_key_id}' and QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and SERVICE_ID = '{Service_Id}' AND FABLOCATION_ID = '{fab}') m where m.ROW BETWEEN ".format(ContractRecordId=ContractRecordId,RevisionRecordId = RevisionRecordId,Parent_Equipmentid=Parent_Equipmentid,Service_Id=TreeTopSuperParentParam,fab=TreeParentParam,Parent_Equipment_key_id = Parent_Equipment_key_id) + str(Page_start) + " and " + str(Page_End))

            QueryCountObj = Sql.GetFirst("select count(CpqTableEntryId) as cnt from SAQSCA (NOLOCK) where QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and EQUIPMENT_ID ='{Parent_Equipmentid}' and QTESRVCOB_RECORD_ID = '{Parent_Equipment_key_id}' and SERVICE_ID ='{Service_Id}' AND FABLOCATION_ID = '{fab)}".format(ContractRecordId=ContractRecordId,RevisionRecordId = RevisionRecordId,Parent_Equipmentid=Parent_Equipmentid,Service_Id=TreeTopSuperParentParam,fab=TreeParentParam,Parent_Equipment_key_id = Parent_Equipment_key_id))       
        if QueryCountObj is not None:
            QueryCount = QueryCountObj.cnt
        # Data construction for table.
        for child in child_obj_recid:
    ...
    for key, col_name in enumerate(list(Columns)):
        ...
        try:
            if col_name == "INCLUDED":
                FORMULA_table = ObjectName
                FORMULA_col = col_name
            else:
                FORMULA_LOGIC = objss_obj.FORMULA_LOGIC.strip()
                FORMULA_col = FORMULA_LOGIC.split(" ")[1].strip()
                FORMULA_table = FORMULA_LOGIC.split(" ")[3].strip()
            ...
            if str(ins_obj.PICKLIST).upper() == "TRUE":
                filter_level_data = "checkbox"
                filter_clas_name = (
                    '<div id = "'
                    + str(table_id)
                    + "_RelatedMutipleCheckBoxDrop_"
    ...
    RelatedDrop_str = (
        ...
        + "').getElementsByClassName('filter-control');  for (i = 0; i < listws.length; i++) { document.getElementById('"
        + str(table_id)
        + "').getElementsByClassName('filter-control')[i].innerHTML = datachld6[i];  } for (j = 0; j < listws.length; j++) { if (datachld7[j] == 'checkbox') { if (datachld8[j]) { var dataAdapter = new $.jqx.dataAdapter(datachld8[j]); $('#"
        + str(table_id)
        + "_RelatedMutipleCheckBoxDrop_' + j.toString() ).jqxDropDownList( { checkboxes: true, source: dataAdapter, autoDropDownHeight: true }); } } } } }  catch(err) { setTimeout(function() { var listws = document.getElementById('"
        + str(table_id)
        + "').getElementsByClassName('filter-control');  for (i = 0; i < listws.length; i++) { document.getElementById('"
...
def GetCovObjMasterFilter(ATTRIBUTE_NAME, ATTRIBUTE_VALUE,PerPage,PageInform):
    ...
    for quer_key, quer_value in enumerate(Dict_formation):
        if Dict_formation.get(quer_value) != "":            
            ...
            if str(quer_values).upper() == "TRUE":
                ...
                quer_values = "FALSE"
            if str(quer_values).find(",") == -1:
                if str(quer_value) == "IBASE_ATTSTS":
                    ATTRIBUTE_VALUE_STR += str(quer_value) + " = '" + str(quer_values) + "' and "
                else:
                    ATTRIBUTE_VALUE_STR += str(quer_value) + " like '%" + str(quer_values) + "%' and "
            else:
    ...
    if ATTRIBUTE_VALUE is None or ATTRIBUTE_VALUE == "" or ATTRIBUTE_VALUE_STR is None or ATTRIBUTE_VALUE_STR == "":
        ...
        if TreeSuperParentParam == "Product Offerings" or TreeParentParam == "Add-On Products":
            parent_obj = Sql.GetList(
                "SELECT TOP "+str(PerPage)+" QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID,EQUIPMENTCATEGORY_DESCRIPTION,EQUIPMENT_ID,EQUIPMENT_DESCRIPTION,SERIAL_NO,GREENBOOK,FABLOCATION_ID, WARRANTY_END_DATE,WARRANTY_START_DATE,MNT_PLANT_ID,EQUIPMENT_STATUS,CUSTOMER_TOOL_ID,EQUIPMENTCATEGORY_ID AS DESCRIPTION,SNDFBL_ID,WARRANTY_END_DATE_ALERT,TEMP_TOOL,PLATFORM, CONTRACT_VALID_FROM, CONTRACT_VALID_TO,IBASE_ATTSTS,ENTINT_STATUS,ENTINT_STATUS_DESC from SAQSCO (NOLOCK) where QUOTE_RECORD_ID = '"
                + str(ContractRecordId)
                + "' and QTEREV_RECORD_ID = '"
                + str(RevisionRecordId)
                + "' AND SERVICE_ID = '"
            ...
            if TreeTopSuperParentParam == "Product Offerings":
                ...
                if TreeParam == "Receiving Equipment":
                    parent_obj = Sql.GetList(
                        "SELECT top "+str(PerPage)+" QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID,EQUIPMENTCATEGORY_DESCRIPTION,EQUIPMENT_ID,EQUIPMENT_DESCRIPTION,SERIAL_NO,GREENBOOK,FABLOCATION_ID, WARRANTY_END_DATE,WARRANTY_START_DATE,MNT_PLANT_ID,EQUIPMENT_STATUS,CUSTOMER_TOOL_ID,EQUIPMENTCATEGORY_ID AS DESCRIPTION,WARRANTY_END_DATE_ALERT,TEMP_TOOL,PLATFORM, CONTRACT_VALID_FROM, CONTRACT_VALID_TO,IBASE_ATTSTS,ENTINT_STATUS,ENTINT_STATUS_DESC from SAQSCO (NOLOCK) where  QUOTE_RECORD_ID = '"
                        + str(ContractRecordId)
                        + "' and QTEREV_RECORD_ID = '"
                        + str(RevisionRecordId)
                        + "' AND SERVICE_ID = '"
                    ...
                    if str(TreeSuperParentParam)=="Comprehensive Services" or str(TreeSuperParentParam)=="Complementary Products":
                        equipment_column = " EQUIPMENTCATEGORY_ID AS DESCRIPTION "
                        level = "GREENBOOK"
                    else:
                        equipment_column = " EQUIPMENTCATEGORY_ID "
                        level = "FABLOCATION_ID"
                    #A055S000P01-20612 start A 
                    parent_obj = Sql.GetList(
                        "SELECT TOP "+str(PerPage)+" QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID,EQUIPMENTCATEGORY_DESCRIPTION,EQUIPMENT_ID,EQUIPMENT_DESCRIPTION,SERIAL_NO,GREENBOOK,FABLOCATION_ID, WARRANTY_END_DATE,WARRANTY_START_DATE,MNT_PLANT_ID,EQUIPMENT_STATUS,CUSTOMER_TOOL_ID,"+str(equipment_column)+",WARRANTY_END_DATE_ALERT,TEMP_TOOL,PLATFORM, CONTRACT_VALID_FROM, CONTRACT_VALID_TO,IBASE_ATTSTS,ENTINT_STATUS,ENTINT_STATUS_DESC from SAQSCO (NOLOCK) where QUOTE_RECORD_ID = '"
                        + str(ContractRecordId)
                        + "' and QTEREV_RECORD_ID = '"
                        + str(RevisionRecordId)
                        + "'AND SERVICE_ID = '"
                ...
                if TreeParam == "Receiving Equipment":
                    #A055S000P01-20612 start A 
                    parent_obj = Sql.GetList(
                        "SELECT top "+str(PerPage)+" QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID,EQUIPMENTCATEGORY_DESCRIPTION,EQUIPMENT_ID,EQUIPMENT_DESCRIPTION,SERIAL_NO,GREENBOOK,FABLOCATION_ID, WARRANTY_END_DATE,WARRANTY_START_DATE,MNT_PLANT_ID,EQUIPMENT_STATUS,CUSTOMER_TOOL_ID,"+str(equipment_column)+",WARRANTY_END_DATE_ALERT,TEMP_TOOL,PLATFORM, CONTRACT_VALID_FROM, CONTRACT_VALID_TO,IBASE_ATTSTS,ENTINT_STATUS,ENTINT_STATUS_DESC from SAQSCO (NOLOCK) where "+ str(ATTRIBUTE_VALUE_STR)+ " 1=1 and QUOTE_RECORD_ID = '"+ str(ContractRecordId)+ "' and QTEREV_RECORD_ID = '"+ str(RevisionRecordId)+ "' AND SERVICE_TYPE = '"+ str(TreeTopSuperParentParam)+ "' AND SERVICE_ID ='"+ str(TreeSuperParentParam)+ "' ORDER BY "  + str(orderby))
                    ...
                    #A055S000P01-20612 start A 
                    parent_obj = Sql.GetList(
                        "SELECT top "+str(PerPage)+" QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID,EQUIPMENTCATEGORY_DESCRIPTION,EQUIPMENT_ID,EQUIPMENT_DESCRIPTION,SERIAL_NO,GREENBOOK,FABLOCATION_ID, WARRANTY_END_DATE,WARRANTY_START_DATE,MNT_PLANT_ID,EQUIPMENT_STATUS,CUSTOMER_TOOL_ID,"+str(equipment_column)+",WARRANTY_END_DATE_ALERT,TEMP_TOOL,PLATFORM, CONTRACT_VALID_FROM, CONTRACT_VALID_TO,IBASE_ATTSTS,ENTINT_STATUS,ENTINT_STATUS_DESC from SAQSCO (NOLOCK) where "
                        + str(ATTRIBUTE_VALUE_STR)
                        + " 1=1 and QUOTE_RECORD_ID = '"
                        + str(ContractRecordId)
                    ...
                    parent_obj = Sql.GetList(
                        ...
                        + str(TreeParam)
                        + "' AND SERVICE_ID ='"
                        + str(Quote.GetGlobal("addon_service_id"))
                        + "' AND PAR_SERVICE_ID = '"
                        + str(TreeSuperParentParam)
                        + "' AND GREENBOOK = '"
                        + str(TreeParentParam)
                    ...
                    QueryCountObj = Sql.GetFirst(
                        ...
                        + str(TreeParam)
                        + "' AND SERVICE_ID ='"
                        + str(Product.GetGlobal("addon_service_id"))
                        + "' AND PAR_SERVICE_ID = '"
                        + str(TreeSuperParentParam)
                        + "' AND GREENBOOK = '"
                        + str(TreeParentParam)
        ...
        if TreeSuperParentParam == "Product Offerings" or TreeParentParam == "Add-On Products":
            #A055S000P01-20612 start A 
            parent_obj = Sql.GetList(
                "SELECT top "+str(PerPage)+" QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID,EQUIPMENTCATEGORY_DESCRIPTION,EQUIPMENT_ID,EQUIPMENT_DESCRIPTION,SERIAL_NO,GREENBOOK,FABLOCATION_ID, WARRANTY_END_DATE,WARRANTY_START_DATE,MNT_PLANT_ID,EQUIPMENT_STATUS,CUSTOMER_TOOL_ID,EQUIPMENTCATEGORY_ID,EQUIPMENTCATEGORY_ID AS DESCRIPTION,SNDFBL_ID,WARRANTY_END_DATE_ALERT,TEMP_TOOL,PLATFORM, CONTRACT_VALID_FROM, CONTRACT_VALID_TO,IBASE_ATTSTS,ENTINT_STATUS,ENTINT_STATUS_DESC from SAQSCO (NOLOCK) where "
                + str(ATTRIBUTE_VALUE_STR)
                + " 1=1 and QUOTE_RECORD_ID = '"
                + str(ContractRecordId)
                + "' and QTEREV_RECORD_ID = '"
            ...
            if TreeTopSuperParentParam == "Product Offerings":
                if TreeParam == "Receiving Equipment":
                    #A055S000P01-20612 start A 
                    parent_obj = Sql.GetList(
                        "SELECT top "+str(PerPage)+" QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID,EQUIPMENTCATEGORY_DESCRIPTION,EQUIPMENT_ID,EQUIPMENT_DESCRIPTION,SERIAL_NO,GREENBOOK,FABLOCATION_ID, WARRANTY_END_DATE,WARRANTY_START_DATE,MNT_PLANT_ID,EQUIPMENT_STATUS,CUSTOMER_TOOL_ID,EQUIPMENTCATEGORY_ID AS DESCRIPTION,WARRANTY_END_DATE_ALERT,TEMP_TOOL,PLATFORM, CONTRACT_VALID_FROM, CONTRACT_VALID_TO,IBASE_ATTSTS,ENTINT_STATUS,ENTINT_STATUS_DESC from SAQSCO (NOLOCK) where "
                        + str(ATTRIBUTE_VALUE_STR)
                        + " 1=1 and QUOTE_RECORD_ID = '"
                        + str(ContractRecordId)
                        + "' and QTEREV_RECORD_ID = '"
                ...
                if TreeParam == "Receiving Equipment":
                    #A055S000P01-20612 start A 
                    parent_obj = Sql.GetList(
                        "SELECT top "+str(PerPage)+" QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID,TEMP_TOOL,EQUIPMENTCATEGORY_DESCRIPTION,EQUIPMENT_ID,EQUIPMENT_DESCRIPTION,SERIAL_NO,GREENBOOK,FABLOCATION_ID, WARRANTY_END_DATE,WARRANTY_START_DATE,MNT_PLANT_ID,EQUIPMENT_STATUS,CUSTOMER_TOOL_ID,"+str(equipment_column)+",WARRANTY_END_DATE_ALERT,PLATFORM, CONTRACT_VALID_FROM, CONTRACT_VALID_TO,IBASE_ATTSTS,ENTINT_STATUS,ENTINT_STATUS_DESC from SAQSCO (NOLOCK) where "+ str(ATTRIBUTE_VALUE_STR)+ " 1=1 and QUOTE_RECORD_ID = '"+ str(ContractRecordId)+ "' and QTEREV_RECORD_ID = '"+ str(RevisionRecordId)+ "' AND SERVICE_TYPE = '"+ str(TreeTopSuperParentParam)+ "' AND SERVICE_ID ='"+ str(TreeSuperParentParam)+ "' ORDER BY "  + str(orderby))
                    ...
                    #A055S000P01-20612 start A 
                    parent_obj = Sql.GetList(
                        "SELECT top "+str(PerPage)+" QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID,TEMP_TOOL,EQUIPMENTCATEGORY_DESCRIPTION,EQUIPMENT_ID,EQUIPMENT_DESCRIPTION,SERIAL_NO,GREENBOOK,FABLOCATION_ID, WARRANTY_END_DATE,WARRANTY_START_DATE,MNT_PLANT_ID,EQUIPMENT_STATUS,CUSTOMER_TOOL_ID,"+str(equipment_column)+",WARRANTY_END_DATE_ALERT,PLATFORM, CONTRACT_VALID_FROM, CONTRACT_VALID_TO,IBASE_ATTSTS,ENTINT_STATUS,ENTINT_STATUS_DESC from SAQSCO (NOLOCK) where "
                        + str(ATTRIBUTE_VALUE_STR)
                        + " 1=1 and QUOTE_RECORD_ID = '"
                        + str(ContractRecordId)
                        + "' and QTEREV_RECORD_ID = '"
    ...
    for par in parent_obj:
        ...
        data_dict["WARRANTY_END_DATE"] = ('<abbr id ="" title="' + str(par.WARRANTY_END_DATE) + '">' + str(par.WARRANTY_END_DATE) + "</abbr>")
        data_dict["WARRANTY_END_DATE_ALERT"] = str(par.WARRANTY_END_DATE_ALERT)
        data_dict["TEMP_TOOL"] = str(par.TEMP_TOOL) if par.TEMP_TOOL else ""
        data_dict["PLATFORM"] = str(par.PLATFORM) if par.PLATFORM else ""
        data_dict["CONTRACT_VALID_FROM"] = str(par.CONTRACT_VALID_FROM) if par.CONTRACT_VALID_FROM else ""
        data_dict["CONTRACT_VALID_TO"] = str(par.CONTRACT_VALID_TO) if par.CONTRACT_VALID_TO else ""
        data_list.append(data_dict)
        """page = ""
        if QueryCount < int(PerPage):
            page = str(Page_start) + " - " + str(QueryCount)
...
def GetCovObjChildFilter(ATTRIBUTE_NAME, ATTRIBUTE_VALUE,RECID,PerPage,PageInform,reckeyid):
    if str(PerPage) == "" and str(PageInform) == "":
        Page_start = 1
        Page_End = 10
        PerPage = 10
    ...
    Dict_formation = dict(zip(ATTRIBUTE_NAME, ATTRIBUTE_VALUE))
    for quer_key, quer_value in enumerate(Dict_formation):
        #A055S000P01-21089 -M- Start
        x_picklistcheckobj = Sql.GetFirst(
            "SELECT PICKLIST,DATA_TYPE,FORMULA_DATA_TYPE FROM SYOBJD (NOLOCK) WHERE OBJECT_NAME ='SAQSCA' AND API_NAME = '" + str(quer_value) + "'"
        )
        x_picklistcheck = str(x_picklistcheckobj.PICKLIST).upper()
        if Dict_formation.get(quer_value) != "":
            quer_values = str(Dict_formation.get(quer_value)).strip()
            if str(quer_values).upper() == "TRUE":
                ...
                quer_values = "FALSE"
            if str(quer_values).find(",") == -1:
                if (x_picklistcheckobj.DATA_TYPE == "CHECKBOX" or x_picklistcheckobj.FORMULA_DATA_TYPE == "CHECKBOX") and x_picklistcheck == "TRUE":
                    if x_picklistcheck == "TRUE":
                        ATTRIBUTE_VALUE_STR += "ISNULL("+str(quer_value)+",0) = '" + str(quer_values) + "' and "
                    else:
                        ATTRIBUTE_VALUE_STR += "ISNULL("+str(quer_value)+",0) like '%" + str(quer_values) + "%' and "
                else:     
                    if x_picklistcheck == "TRUE":
                        ATTRIBUTE_VALUE_STR += str(quer_value)+ "= '" + str(quer_values) + "' and "
                    else:
                ...
                quer_values = quer_values.split(",")
                quer_values = tuple(list(quer_values))
                if (x_picklistcheckobj.DATA_TYPE == "CHECKBOX" or x_picklistcheckobj.FORMULA_DATA_TYPE == "CHECKBOX"):
                    ATTRIBUTE_VALUE_STR += "ISNULL("+str(quer_value) +",0)"+ " in " + str(quer_values) + " and "
                else:
                    ATTRIBUTE_VALUE_STR += str(quer_value) + " in " + str(quer_values) + " and "
            #A055S000P01-21089 -M- End

    data_list = []
    rec_id = "SYOBJ_00929"
    obj_id = "SYOBJ-00929"
    objh_getid = Sql.GetFirst("SELECT TOP 1  RECORD_ID  FROM SYOBJH (NOLOCK) WHERE SAPCPQ_ATTRIBUTE_NAME = '"+str(obj_id)+"'")
    if objh_getid:
        obj_id = objh_getid.RECORD_ID
    objs_obj = Sql.GetFirst("select CAN_ADD,CAN_EDIT,COLUMNS,CAN_DELETE from SYOBJR (NOLOCK) where OBJ_REC_ID = '"+str(obj_id) +"'")
    ...
    if ATTRIBUTE_VALUE is None or ATTRIBUTE_VALUE == "" or ATTRIBUTE_VALUE_STR is None or ATTRIBUTE_VALUE_STR == "":
        Trace.Write("empty search")
        if TreeSuperParentParam == "Product Offerings":
            parent_obj = Sql.GetList("select top "+str(PerPage)+"  QUOTE_SERVICE_COVERED_OBJECT_ASSEMBLIES_RECORD_ID,EQUIPMENT_ID,ASSEMBLY_ID,ASSEMBLY_DESCRIPTION,GOT_CODE, EQUIPMENTTYPE_ID, EQUIPMENT_DESCRIPTION, MNT_PLANT_ID,FABLOCATION_ID,WARRANTY_START_DATE,WARRANTY_END_DATE,INCLUDED from SAQSCA (NOLOCK) where EQUIPMENT_ID = '{Parent_Equipmentid}' and QTESRVCOB_RECORD_ID = '{Parent_Equipment_key_id}' and QUOTE_RECORD_ID = '{ContractRecordId}'  and QTEREV_RECORD_ID = '{RevisionRecordId}' and SERVICE_ID = '{Service_Id}' ORDER BY {ord_by} ".format(ContractRecordId=ContractRecordId,RevisionRecordId = RevisionRecordId, Parent_Equipmentid = Parent_Equipmentid,Parent_Equipment_key_id = Parent_Equipment_key_id, Service_Id=TreeParam,ord_by = orderby))
            
            QueryCountObj = Sql.GetFirst( "select count(*) as cnt from SAQSCA (NOLOCK) where EQUIPMENT_ID = '{Parent_Equipmentid}' and QTESRVCOB_RECORD_ID = '{Parent_Equipment_key_id}' and QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and SERVICE_ID = '{Service_Id}' ".format( ContractRecordId=ContractRecordId,RevisionRecordId = RevisionRecordId, Parent_Equipmentid = Parent_Equipmentid,Parent_Equipment_key_id = Parent_Equipment_key_id, Service_Id=TreeParam))
            if QueryCountObj is not None:
                QueryCount = QueryCountObj.cnt         
        else:
            if TreeTopSuperParentParam == "Product Offerings":
                parent_obj = Sql.GetList( "select top "+str(PerPage)+"  QUOTE_SERVICE_COVERED_OBJECT_ASSEMBLIES_RECORD_ID,EQUIPMENT_ID,ASSEMBLY_ID,ASSEMBLY_DESCRIPTION,GOT_CODE, EQUIPMENTTYPE_ID, EQUIPMENT_DESCRIPTION, MNT_PLANT_ID,FABLOCATION_ID,WARRANTY_START_DATE,WARRANTY_END_DATE,INCLUDED from SAQSCA (NOLOCK) where EQUIPMENT_ID = '{Parent_Equipmentid}' and QTESRVCOB_RECORD_ID = '{Parent_Equipment_key_id}' and QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and SERVICE_ID = '{Service_Id}' ORDER BY {ord_by} ".format( ContractRecordId=ContractRecordId,RevisionRecordId = RevisionRecordId, Parent_Equipmentid = Parent_Equipmentid,Parent_Equipment_key_id = Parent_Equipment_key_id, Service_Id=TreeParentParam,ord_by = orderby ) )
                
                QueryCountObj = Sql.GetFirst( "select count(*) as cnt from SAQSCA (NOLOCK) where EQUIPMENT_ID = '{Parent_Equipmentid}' and QTESRVCOB_RECORD_ID = '{Parent_Equipment_key_id}' and QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and SERVICE_ID = '{Service_Id}' ".format( ContractRecordId=ContractRecordId,RevisionRecordId = RevisionRecordId,Parent_Equipmentid = Parent_Equipmentid,Parent_Equipment_key_id = Parent_Equipment_key_id, Service_Id=TreeParentParam))
                if QueryCountObj is not None:
                    QueryCount = QueryCountObj.cnt
            else:
                parent_obj = Sql.GetList( "select top "+str(PerPage)+"  QUOTE_SERVICE_COVERED_OBJECT_ASSEMBLIES_RECORD_ID,EQUIPMENT_ID,ASSEMBLY_ID,ASSEMBLY_DESCRIPTION,GOT_CODE, EQUIPMENTTYPE_ID, EQUIPMENT_DESCRIPTION, MNT_PLANT_ID,FABLOCATION_ID,WARRANTY_START_DATE,WARRANTY_END_DATE,INCLUDED from SAQSCA (NOLOCK) where   QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and EQUIPMENT_ID = '{Parent_Equipmentid}' and QTESRVCOB_RECORD_ID = '{Parent_Equipment_key_id}' and SERVICE_ID = '{service_id}'and FABLOCATION_ID = '{fablocation_id}' ORDER BY {ord_by} ".format( ContractRecordId=ContractRecordId, RevisionRecordId = RevisionRecordId, Parent_Equipmentid = Parent_Equipmentid,Parent_Equipment_key_id = Parent_Equipment_key_id, service_id=TreeSuperParentParam, fablocation_id = TreeParentParam, ord_by = orderby ) )
                
                QueryCountObj = Sql.GetFirst( "select count(*) as cnt from SAQSCA (NOLOCK) where   QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and EQUIPMENT_ID = '{Parent_Equipmentid}' and QTESRVCOB_RECORD_ID = '{Parent_Equipment_key_id}' and SERVICE_ID = '{service_id}'and FABLOCATION_ID = '{fablocation_id}'".format( ContractRecordId=ContractRecordId, RevisionRecordId = RevisionRecordId, Parent_Equipmentid = Parent_Equipmentid,Parent_Equipment_key_id = Parent_Equipment_key_id, service_id=TreeSuperParentParam, fablocation_id = TreeParentParam, ))
                if QueryCountObj is not None:
                    QueryCount = QueryCountObj.cnt
                
    else:
        if TreeSuperParentParam == "Product Offerings":
            parent_obj = Sql.GetList( "select top "+str(PerPage)+"  QUOTE_SERVICE_COVERED_OBJECT_ASSEMBLIES_RECORD_ID,EQUIPMENT_ID,ASSEMBLY_ID,ASSEMBLY_DESCRIPTION,GOT_CODE, EQUIPMENTTYPE_ID, EQUIPMENT_DESCRIPTION, MNT_PLANT_ID,FABLOCATION_ID,WARRANTY_START_DATE,WARRANTY_END_DATE,INCLUDED from SAQSCA (NOLOCK) where  " + str(ATTRIBUTE_VALUE_STR) + " 1=1 and QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}'  and EQUIPMENT_ID = '{Parent_Equipmentid}' and QTESRVCOB_RECORD_ID = '{Parent_Equipment_key_id}' and SERVICE_ID = '{Service_Id}' ORDER BY {ord_by} ".format( ContractRecordId=ContractRecordId,RevisionRecordId = RevisionRecordId, Parent_Equipmentid = Parent_Equipmentid,Parent_Equipment_key_id = Parent_Equipment_key_id, Service_Id=TreeParam,ord_by = orderby ) )
            
            QueryCountObj = Sql.GetFirst( "select count(*) as cnt from SAQSCA (NOLOCK) where  " + str(ATTRIBUTE_VALUE_STR) + " 1=1 and QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and EQUIPMENT_ID = '{Parent_Equipmentid}' and QTESRVCOB_RECORD_ID = '{Parent_Equipment_key_id}' and SERVICE_ID = '{Service_Id}'".format( ContractRecordId=ContractRecordId,RevisionRecordId = RevisionRecordId, Parent_Equipmentid = Parent_Equipmentid,Parent_Equipment_key_id = Parent_Equipment_key_id, Service_Id=TreeParam ))
            if QueryCountObj is not None:
                QueryCount = QueryCountObj.cnt
        else:
            if TreeTopSuperParentParam == "Product Offerings":
                ...
                QueryCountObj = Sql.GetFirst( "select count(*) as cnt from SAQSCA (NOLOCK) where  " + str(ATTRIBUTE_VALUE_STR) + " 1=1 and QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and EQUIPMENT_ID = '{Parent_Equipmentid}' and QTESRVCOB_RECORD_ID = '{Parent_Equipment_key_id}' and SERVICE_ID = '{Service_Id}'".format( ContractRecordId=ContractRecordId, RevisionRecordId = RevisionRecordId, Parent_Equipmentid = Parent_Equipmentid,Parent_Equipment_key_id = Parent_Equipment_key_id, Service_Id=TreeParentParam, ))
                if QueryCountObj is not None:
                    QueryCount = QueryCountObj.cnt
            else:   
                Trace.Write("5 level coditional search --->")
                parent_obj = Sql.GetList( "select top "+str(PerPage)+"  QUOTE_SERVICE_COVERED_OBJECT_ASSEMBLIES_RECORD_ID,EQUIPMENT_ID,ASSEMBLY_ID,ASSEMBLY_DESCRIPTION,GOT_CODE, EQUIPMENTTYPE_ID, EQUIPMENT_DESCRIPTION, MNT_PLANT_ID,FABLOCATION_ID,WARRANTY_START_DATE,WARRANTY_END_DATE,INCLUDED from SAQSCA (NOLOCK) where  " + str(ATTRIBUTE_VALUE_STR) + " 1=1 and QUOTE_RECORD_ID = '{ContractRecordId}' and QTEREV_RECORD_ID = '{RevisionRecordId}' and EQUIPMENT_ID = '{Parent_Equipmentid}' and QTESRVCOB_RECORD_ID = '{Parent_Equipment_key_id}' and SERVICE_ID = '{service_id}'and FABLOCATION_ID = '{fablocation_id}' ORDER BY {ord_by}".format( ContractRecordId=ContractRecordId, RevisionRecordId = RevisionRecordId, Parent_Equipmentid = Parent_Equipmentid,Parent_Equipment_key_id = Parent_Equipment_key_id, service_id=TreeSuperParentParam, fablocation_id = TreeParentParam, ord_by = orderby ) )
                
                QueryCountObj = Sql.GetFirst( "select count(*) as cnt from SAQSCA (NOLOCK) where  " + str(ATTRIBUTE_VALUE_STR) + " 1=1 and QUOTE_RECORD_ID = '{ContractRecordId}'  and QTEREV_RECORD_ID = '{RevisionRecordId}' and EQUIPMENT_ID = '{Parent_Equipmentid}' and QTESRVCOB_RECORD_ID = '{Parent_Equipment_key_id}' and SERVICE_ID = '{service_id}'and FABLOCATION_ID = '{fablocation_id}'".format( ContractRecordId=ContractRecordId, RevisionRecordId = RevisionRecordId, Parent_Equipmentid = Parent_Equipmentid,Parent_Equipment_key_id = Parent_Equipment_key_id, service_id=TreeSuperParentParam, fablocation_id = TreeParentParam, ))
                if QueryCountObj is not None:
                    QueryCount = QueryCountObj.cnt
    for par in parent_obj:
        data_dict = {}
...
def GetCommonParentContract(PerPage, PageInform, A_Keys, A_Values):    
    ...
    imgstr = '<img title="Acquired" src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/Green_Tick.svg>'
    acquiring_img_str = '<img title="Acquiring" src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/Cloud_Icon.svg>'
    exclamation = '<img title="Approval Required" src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/clock_exe.svg>'
    error = '<img title="Error" src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/exclamation_icon.svg>'
    partially_priced = '<img title="Partially Priced" src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/Red1_Circle.svg>'
    assembly_missing = '<img title="Assembly Missing" src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/Orange1_Circle.svg>'
    ...
    for par in parent_obj:
        ...
        if 'BUNDLE' in str(par.SERVICE_ID):            
            ...
            if 'ACQUIRING' in status_list:
                icon = '<img title="Acquiring" src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/Cloud_Icon.svg>'
                ...
                icon = '<img title="Error" src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/exclamation_icon.svg>'
                ...
                icon = '<img title="Approval Required" src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/clock_exe.svg>'
                ...
                icon = '<img title="Acquired" src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/Green_Tick.svg>'
            ...
            if 'SPARE' not in GetQuoteType.CONTRACT_TYPE:
                ...
                if item_lines_obj:
                    ...
                    if "ACQUIRED" in line_items_status_list and ('ACQUIRING' not in line_items_status_list and 'APPROVAL REQUIRED' not in line_items_status_list and 'ERROR' not in line_items_status_list):
                        ...
                        icon = '<img title="Acquiring" src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/clock_exe.svg>'
                        status_update=SqlHelper.GetFirst("sp_executesql @statement = N'UPDATE CTCITM SET PRICING_STATUS=''ACQUIRING'' WHERE CONTRACT_RECORD_ID =''"+str(Quote.GetGlobal("contract_record_id"))+"'' AND SERVICE_ID LIKE ''%BASE%'' '")
            else:
    ...
    for key, col_name in enumerate(list(Columns)):
        ...
        if filter_level_data == "select":
            ...
            for string in StringValue_list:
                string = string.strip()
                if string == "ACQUIRED":
                    string_value = string.replace("ACQUIRED","<img title='Acquired' src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/Green_Tick.svg> ACQUIRED")
                if string == "APPROVAL REQUIRED":
                    string_value = string.replace("APPROVAL REQUIRED","<img title='Approval Required' src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/clock_exe.svg> APPROVAL REQUIRED")
                if string == "ACQUIRING":                        
                    string_value = string.replace("ACQUIRING","<img title='Acquiring' src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/Cloud_Icon.svg> ACQUIRING")
                if string == "ERROR":
                    string_value = string.replace("ERROR","<img title='Error' src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/exclamation_icon.svg> ERROR")
                    status_update=SqlHelper.GetFirst("sp_executesql @statement = N'UPDATE CTCITM SET PRICING_STATUS=''ERROR'' WHERE CONTRACT_RECORD_ID =''"+str(Quote.GetGlobal("contract_record_id"))+"'' AND SERVICE_ID LIKE ''%BUNDLE%'' '")
                if string == "ASSEMBLY MISSING":
                    string_value = string.replace("ASSEMBLY MISSING","<img title='Assembly Missing' src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/Orange1_Circle.svg> ASSEMBLY MISSING")
                if string == "PARTIALLY PRICED":
                    string_value = string.replace("PARTIALLY PRICED","<img title='Partially Priced' src=/mt/APPLIEDMATERIALS_PRD/Additionalfiles/Red1_Circle.svg> PARTIALLY PRICED")
...
try:
    ...
    ATTRIBUTE_VALUE = ""
try:
    CPQ_TAB_ID = Param.CPQ_TAB_ID
except:
    CPQ_TAB_ID = ""    
Trace.Write("ATTRIBUTE_VALUE" + str(ATTRIBUTE_VALUE))
...
if ACTION == "LOAD":
    ...
    if TABNAME == "Equipment Parent":
        ...
        ApiResponse = ApiResponseFactory.JsonResponse(QuoteAssemblyPreventiveMaintainenceKitMaterialChild(PM_ID,PerPage, PageInform, A_Keys, A_Values,ASSEMBLYID,EQUIPMENTID,KITID,KITNUMBER))    
        ...
        PerPage = Param.PerPage
        PageInform = Param.PageInform
        ApiResponse = ApiResponseFactory.JsonResponse(GetEquipmentChild(CHILDEQUIPMENT, PerPage, PageInform, A_Keys, A_Values,CPQ_TAB_ID))
        ...
        PerPage = Param.PerPage
        PageInform = Param.PageInform
        ApiResponse = ApiResponseFactory.JsonResponse(GetCovObjChild(EQUIPMENTID, PerPage, PageInform, A_Keys, A_Values,CPQ_TAB_ID))
        Trace.Write("Current Rec Id is " + str(CURR_REC_ID)) 
    ...
    if TABNAME == "Equipment Parent Filter":
        ...
        RECID = Param.REC_ID
        PM_ID=Param.PM_ID
        ApiResponse = ApiResponseFactory.JsonResponse(QuoteAssemblyPreventiveMaintainenceKitMaterialChildFilter(ATTRIBUTE_NAME, ATTRIBUTE_VALUE, RECID,ASSEMBLYID,EQUIPMENTID,KITID,KITNUMBER,SortPerPage,SortPageInform,PM_ID))      
    elif TABNAME == "Events Child Filter":
        ApiResponse = ApiResponseFactory.JsonResponse(GetEventsChild(CHILDEQUIPMENT, PerPage, PageInform, A_Keys, A_Values))  
        ...
        ApiResponse = ApiResponseFactory.JsonResponse(GetEquipmentChildFilter(ATTRIBUTE_NAME, ATTRIBUTE_VALUE, RECID,SortPerPage,SortPageInform,CPQ_TAB_ID))
    elif TABNAME == "Covered Object Child":
        REC_ID = Param.REC_ID
        RECID = REC_ID.split("_")[-1]
        ApiResponse = ApiResponseFactory.JsonResponse(GetCovObjChildFilter(ATTRIBUTE_NAME, ATTRIBUTE_VALUE, RECID,SortPerPage,SortPageInform,CPQ_TAB_ID))
        ...
        REC_ID = Param.REC_ID
        RECID = REC_ID.split("_")[-1]
        ApiResponse = ApiResponseFactory.JsonResponse(GetSendEupChildFilter(ATTRIBUTE_NAME, ATTRIBUTE_VALUE, RECID,SortPerPage,SortPageInform))    
    elif TABNAME == "Common Parent":
        ApiResponse = ApiResponseFactory.JsonResponse(GetCommonParentFilter(ATTRIBUTE_NAME, ATTRIBUTE_VALUE,SortPerPage,SortPageInform))
    ...
    if TABNAME == "Equipment child":
        ApiResponse = ApiResponseFactory.JsonResponse(GetEquipmentChild(ATTRIBUTE_NAME, PerPage, PageInform, A_Keys, A_Values,CPQ_TAB_ID))
    elif TABNAME == "Covered Object Child":
        ApiResponse = ApiResponseFactory.JsonResponse(GetCovObjChild(ATTRIBUTE_NAME, PerPage, PageInform, A_Keys, A_Values,CPQ_TAB_ID))
    elif TABNAME == "Events child":
        ApiResponse = ApiResponseFactory.JsonResponse(GetEventsChild(ATTRIBUTE_NAME, PerPage, PageInform, A_Keys, A_Values))
    elif TABNAME == "Sending Equipment child":
        ApiResponse = ApiResponseFactory.JsonResponse(GetSendingEquipmentChild(ATTRIBUTE_NAME, PerPage, PageInform, A_Keys, A_Values))
    elif TABNAME == "Assemblies Child":
        ApiResponse = ApiResponseFactory.JsonResponse(GetAssembliesChild(ATTRIBUTE_NAME, PerPage, PageInform, A_Keys, A_Values))
    elif TABNAME == "Fts Assemblies Child":
        ApiResponse = ApiResponseFactory.JsonResponse(GetFtsAssembliesChild(ATTRIBUTE_NAME, PerPage, PageInform, A_Keys, A_Values))
    elif TABNAME == "Preventive Maintainence child":
        ApiResponse = ApiResponseFactory.JsonResponse(QuoteAssemblyPreventiveMaintainenceKitMaterialChild(ATTRIBUTE_NAME, PerPage, PageInform, A_Keys, A_Values,ASSEMBLYID,EQUIPMENTID,KITID,KITNUMBER))    
