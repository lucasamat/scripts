	...
	if 'Param' in globals(): 	
		if hasattr(Param, 'CPQ_Columns'): 
			...
			for crmifno in input_data:	
				...
				UpdateTable1=SqlHelper.GetFirst("sp_executesql @T=N'update SAQICO set STATUS =  '''' FROM SAQICO (NOLOCK) JOIN SAQRIO (NOLOCK) ON SAQICO.QUOTE_ID = SAQRIO.QUOTE_ID AND SAQICO.QTEREV_ID = SAQRIO.QTEREV_ID where SAQICO.QUOTE_ID+''-''+ CONVERT(VARCHAR,SAQICO.QTEREV_ID)=''"+ str(QUOTE_ID)+ "'' and SAQRIO.EQUIPMENT_ID = ''"+ str(EQUIPMENT_ID)+ "'' AND SAQICO.SERVICE_ID IN (SELECT DISTINCT SERVICE_ID FROM PRSPRV(NOLOCK) WHERE ISNULL(SSCM_COST,''FALSE'')=''TRUE'' )  '")
				
				UpdateTable1=SqlHelper.GetFirst("sp_executesql @T=N'update SAQRIT set STATUS =  '''' FROM SAQRIT (NOLOCK) JOIN SAQRIO (NOLOCK) ON SAQRIT.QUOTE_ID = SAQRIO.QUOTE_ID AND SAQRIT.QTEREV_ID = SAQRIO.QTEREV_ID where SAQRIT.QUOTE_ID+''-''+ CONVERT(VARCHAR,SAQRIT.QTEREV_ID)=''"+ str(QUOTE_ID)+ "'' and SAQRIO.EQUIPMENT_ID = ''"+ str(EQUIPMENT_ID)+ "'' AND SAQRIT.SERVICE_ID IN (SELECT DISTINCT SERVICE_ID FROM PRSPRV(NOLOCK) WHERE ISNULL(SSCM_COST,''FALSE'')=''TRUE'' ) '")
				Log.Info('76-to check cfg configure---')
				...
				sqlqueryinfo = SqlHelper.GetList("SELECT DISTINCT QTEREV_ID,QUOTE_ID FROM SAQICO(NOLOCK) WHERE QUOTE_ID+'-'+ CONVERT(VARCHAR,QTEREV_ID)='"+ str(QUOTE_ID)+ "' ")
