...
class TreeView:
    ...
    def CommonDynamicLeftTreeView(self):
                ...
                for getParentObj in getParentObjQuery:
                    ...
                    try:
                        if str(TabName) == "Quote":
                            if (str(get_ohold_pricing_status).upper() in ("CBC-CBC COMPLETED","BOK-CONTRACT CREATED","BOK-CONTRACT BOOKED","LGL-LEGAL SOW ACCEPTED","LGL-LEGAL SOW REJECTED","LGL-PREPARING LEGAL SOW","CBC-PREPARING CBC")) or (str(get_ohold_pricing_status).upper() in ("CFG-ACQUIRING","PRR-ON HOLD PRICING","CFG-ON HOLD - COSTING","PRI-PRICING","PRR-PRICING REVIEWED","PRR-RECALCULATING") and str(getParentObj.NODE_NAME).upper() not in ("QUOTE ITEMS","BILLING","APPROVALS","QUOTE DOCUMENTS")) or (str(get_ohold_pricing_status).upper() in ("APR-APPROVED","APR-REJECTED","APR-RECALLED","APR-APPROVAL PENDING","OPD-PREPARING QUOTE DOCUMENTS","OPD-CUSTOMER ACCEPTED","OPD-CUSTOMER REJECTED") and str(getParentObj.NODE_NAME).upper() not in ("APPROVALS","QUOTE DOCUMENTS")):
                    ...
                    if str(getParentObj.TREEIMAGE_URL):
                        ...
                        image_url = '<img class="leftside-bar-icons" src="/mt/appliedmaterials_prd/Additionalfiles/AMAT/Quoteimages/{image_url}"/>'.format(
                            image_url=image_url
                        )
                        active_image_url = str(getParentObj.ACTIVE_TREEIMAGE_URL)
                        active_image_url = '<img class="activeimage-leftside-bar-icons" src="/mt/appliedmaterials_prd/Additionalfiles/AMAT/Quoteimages/{image_url}"/>'.format(
        ...
        Log.Info(str(self.check_count)+"===== "+str(self.contract_quote_record_id)+" ====Recursive==========>>> "+str(returnList))
        ...
        if str(NodeType) == "DYNAMIC":
                            ...
                            if NodeText == "True":
                                ...
                                if str(ObjName).strip() == "SAQTSV" and str(NodeName) == "SERVICE_ID":
                                    ...
                                    try:
                                        ...
                                        image_url = ""
                                    if image_url:
                                        image_url = '<img class="leftside-bar-status_icon" src="/mt/appliedmaterials_prd/Additionalfiles/AMAT/Quoteimages/{image_url}"/>'.format(
                                            image_url=image_url
                                        )
                                        ...
                                        for ent_table in ent_table_list:
                                            ...
                                            if get_entitlement_xml:
                                                ...
                                                if subtab_temp_variable == "Events" and ent_table == "SAQTSE" and service_id != "Z0010":
                                                    #INC08621345 - M
                                                    pattern_id = re.compile(r"<ENTITLEMENT_ID>(?:AGS_"+str(service_id)+"_NET_PRMALB|AGS_"+str(service_id)+"_NET_CRMALB)</ENTITLEMENT_ID>")
                                                    pattern_name = re.compile(
                                                        r"<ENTITLEMENT_DISPLAY_VALUE>(?:Included - All PM|Included - Monthly and Above|Included - Quarterly and Above|Included - All PM (PDC/MPS)|Included - Qtrly and Above|Included - &lt; Quarterly|Included)</ENTITLEMENT_DISPLAY_VALUE>"
                                                    )
                                                    #INC08621345 - M
                                                    subtab_temp = "Events"
            ...
            if findChildOneObj is not None and len(findChildOneObj) > 0:
                for findChildOne in findChildOneObj:
                    ...
                    try:
                        if str(TabName) == "Quote":
                            if (str(get_ohold_pricing_status).upper() in ("CBC-CBC COMPLETED","BOK-CONTRACT CREATED","BOK-CONTRACT BOOKED","LGL-LEGAL SOW ACCEPTED","LGL-LEGAL SOW REJECTED","LGL-PREPARING LEGAL SOW","CBC-PREPARING CBC")) or (str(get_ohold_pricing_status).upper() in ("CFG-ACQUIRING","PRR-ON HOLD PRICING","CFG-ON HOLD - COSTING","PRI-PRICING","PRR-PRICING REVIEWED","PRR-RECALCULATING") and str(findChildOne.NODE_NAME).upper() not in ("QUOTE ITEMS","BILLING","APPROVALS","QUOTE DOCUMENTS")) or (str(get_ohold_pricing_status).upper() in ("APR-APPROVED","APR-REJECTED","APR-RECALLED","APR-APPROVAL PENDING","OPD-PREPARING QUOTE DOCUMENTS","OPD-CUSTOMER ACCEPTED","OPD-CUSTOMER REJECTED") and str(findChildOne.NODE_NAME).upper() not in ("APPROVALS","QUOTE DOCUMENTS")):
                                lock_icon = '<span class="icon fa fa-lock" aria-hidden="true"></span>'
                            else:
                                lock_icon = ""
