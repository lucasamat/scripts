...
class AncillaryProductOperation:
    ...
    def _do_opertion(self):
        if self.action_type == "INSERT_SERVICE":
            ...
            except Exception as e:#INC08817707 - M
                Log.Info("entitlement error-"+str(e))
        ...
        Sql.RunQuery(
                ...
                INSERT SAQSCO (
                    ...
                    TEMP_TOOL 
                        ...
                        TEMP_TOOL
                        FROM SAQSCO (NOLOCK)
                        WHERE QUOTE_RECORD_ID = '{QuoteRecordId}'  AND QTEREV_RECORD_ID = '{RevisionRecordId}'  AND SERVICE_ID ='{par_service_id}' {addtional_where} AND SAQSCO.QTEREVFEQ_RECORD_ID not in (SELECT QTEREVFEQ_RECORD_ID FROM SAQSCO (NOLOCK) WHERE QUOTE_RECORD_ID = '{QuoteRecordId}'  AND QTEREV_RECORD_ID = '{RevisionRecordId}'  AND PAR_SERVICE_ID ='{par_service_id}' AND SERVICE_ID ='{serviceid}')
    ...
    def _insert_assembly(self):
        ...
        if self.equip_insert_flag == 'True':
            ...
            addtional_where = " AND GREENBOOK in {}".format(existing_greenbook_list)
        #2127 ends
        Sql.RunQuery("DELETE FROM SAQSCA WHERE QUOTE_RECORD_ID = '{}' AND QTEREV_RECORD_ID = '{}' AND SERVICE_ID ='{}' AND PAR_SERVICE_ID = '{}' {} ".format(self.contract_quote_record_id, self.contract_quote_revision_record_id ,self.ancillary_obj, self.service_id, addtional_where))
        Sql.RunQuery(
            """
                INSERT SAQSCA (
                    ...
                    QTEREVFEQ_RECORD_ID
                    FROM SAQSCA (NOLOCK)
                    WHERE QUOTE_RECORD_ID = '{QuoteRecordId}'  AND QTEREV_RECORD_ID = '{RevisionRecordId}'  AND SERVICE_ID ='{par_service_id}' {addtional_where} 
                """.format(
                        par_service_id = self.service_id,
                        serviceid = self.ancillary_obj ,
                        QuoteRecordId = self.contract_quote_record_id,
    ...
    def _insert_service_ent(self):
        ...
        for addon in get_service_details:
            ...
            if ProductVersionObj and get_existing_record.cnt == 0:
                ...
                tbrow["QUOTE_NAME"]=addon.QUOTE_NAME.replace("'","''")#INC08958263
    ...
    def _update_entitlement_values(self,anc_service = '',ent_table = ''):
        Log.Info("_update_entitlement_values-> "+str(self.contract_quote_record_id)+'-'+str(anc_service))#INC08739271 
        try:
            ...
            if check_ancillary.CNT > 0 :#INC08817707 - M
    ...
    def _entitlement_rolldown(self):
        try:
            ...
            if get_ancillaryservice :				
                ...
                # 
                #end-
                Log.Info("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaadon")
                #INC08622381 M
                qtqsge_query_anc= """INSERT SAQSGE (KB_VERSION,QUOTE_ID,QUOTE_NAME,QUOTE_RECORD_ID,QTEREV_RECORD_ID,QTEREV_ID,SERVICE_DESCRIPTION,SERVICE_ID,SERVICE_RECORD_ID,SALESORG_ID,SALESORG_NAME,SALESORG_RECORD_ID,	
                    CPS_CONFIGURATION_ID, CPS_MATCH_ID,GREENBOOK,GREENBOOK_RECORD_ID,QTESRVENT_RECORD_ID,ENTITLEMENT_XML,CONFIGURATION_STATUS,PAR_SERVICE_ID,PAR_SERVICE_RECORD_ID,PAR_SERVICE_DESCRIPTION,QTESRVGBK_RECORD_ID, QUOTE_SERVICE_GREENBOOK_ENTITLEMENT_RECORD_ID,CPQTABLEENTRYADDEDBY,CPQTABLEENTRYDATEADDED )
                    ...
                    SAQTSE.CPS_CONFIGURATION_ID, SAQTSE.CPS_MATCH_ID,SAQSCO.GREENBOOK,SAQSCO.GREENBOOK_RECORD_ID,SAQTSE.QUOTE_SERVICE_ENTITLEMENT_RECORD_ID as QTESRVENT_RECORD_ID,SAQTSE.ENTITLEMENT_XML,SAQTSE.CONFIGURATION_STATUS,SAQTSE.PAR_SERVICE_ID,SAQTSE.PAR_SERVICE_RECORD_ID,SAQTSE.PAR_SERVICE_DESCRIPTION,SAQSGB.QUOTE_SERVICE_GREENBOOK_RECORD_ID as QTESRVGBK_RECORD_ID
                    FROM
                    SAQTSE (NOLOCK) 
                ...
                for offering in ["Z0046","Z0100","Z0101","Z0048"]:
                ...
                qtqsce_anc_query="""
                    ...
                    JOIN SAQSCE (NOLOCK) ON SAQTSE.PAR_SERVICE_ID = SAQSCE.SERVICE_ID AND SAQSCO.QUOTE_RECORD_ID = SAQSCE.QUOTE_RECORD_ID AND SAQSCO.QTEREV_RECORD_ID = SAQSCE.QTEREV_RECORD_ID  AND SAQSCO.EQUIPMENT_ID = SAQSCE.EQUIPMENT_ID AND SAQSCO.QTEREVFEQ_RECORD_ID = SAQSCE.QTEREVFEQ_RECORD_ID

                    WHERE SAQTSE.QUOTE_RECORD_ID = '{QuoteRecordId}' AND ISNULL(SAQSCE.CONFIGURATION_STATUS,'') = 'COMPLETE' AND SAQTSE.QTEREV_RECORD_ID = '{revision_rec_id}' AND SAQTSE.PAR_SERVICE_ID = '{par_service_id}' AND SAQSCO.QUOTE_SERVICE_COVERED_OBJECTS_RECORD_ID not in (SELECT QTESRVCOB_RECORD_ID FROM SAQSCE (NOLOCK) WHERE QUOTE_RECORD_ID = '{QuoteRecordId}' AND QTEREV_RECORD_ID = '{revision_rec_id}' AND SERVICE_ID = SAQTSE.SERVICE_ID AND PAR_SERVICE_ID = '{par_service_id}')) IQ""".format(UserId=self.user_id, QuoteRecordId=self.contract_quote_record_id, ServiceId=self.ancillary_obj, revision_rec_id = self.contract_quote_revision_record_id,par_service_id = self.service_id)
                #A055S000P01-21104 - End - M
    ...
    def _delete_operation(self):
        ...
        #INC08621047 - Start - M
        if self.tablename == "SAQTSE": 
            delete_obj_list = ["SAQTSV","SAQSGB","SAQSCO","SAQSCA","SAQTSE","SAQSGE","SAQSCE","SAQSAE"]#INC08739271 
        elif self.tablename == "SAQSGE":
