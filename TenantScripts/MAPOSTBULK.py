...
try:	
	...
	while Check_flag1 == 1:
		primaryQueryItems = SqlHelper.GetFirst(
			""
			+ str(Parameter.QUERY_CRITERIA_1)
			+ " TEMP_PROCESS (SESSION_ID)SELECT DISTINCT TOP 100 MAMSOP_INBOUND.SAP_PART_NUMBER FROM MAMSOP_INBOUND(NOLOCK) LEFT JOIN TEMP_PROCESS(NOLOCK) ON MAMSOP_INBOUND.SAP_PART_NUMBER = TEMP_PROCESS.SESSION_ID WHERE MAMSOP_INBOUND.PROCESS_STATUS=''READY FOR UPLOAD'' AND ISNULL(ERROR,'''')=''''  AND MAMSOP_INBOUND.TIMESTAMP='"
			+ str(timestamp_sessionid)
			+ "' AND TEMP_PROCESS.SESSION_ID IS NULL  ' "
		)
