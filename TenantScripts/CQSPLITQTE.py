...
def _insert_equipment_entitlement():
    ...
    # Set BILLING_CYCLE and BILLING_TYPE from parent service
    Sql.RunQuery("""UPDATE SAQSCE SET SAQSCE.BILLING_CYCLE = SAQSCE_SELF.BILLING_CYCLE, SAQSCE.BILTYP = SAQSCE_SELF.BILTYP FROM 
                        SAQSCE LEFT JOIN SAQSCE (NOLOCK) SAQSCE_SELF ON SAQSCE_SELF.QUOTE_RECORD_ID = SAQSCE.QUOTE_RECORD_ID
                                                AND SAQSCE_SELF.QTEREV_RECORD_ID = SAQSCE.QTEREV_RECORD_ID
                                                AND SAQSCE_SELF.SERVICE_ID = SAQSCE.PAR_SERVICE_ID
                                                AND SAQSCE_SELF.EQUIPMENT_ID = SAQSCE.EQUIPMENT_ID
                                                AND SAQSCE_SELF.GREENBOOK = SAQSCE.GREENBOOK
                                                AND SAQSCE_SELF.FABLOCATION_ID = SAQSCE.FABLOCATION_ID
                        WHERE SAQSCE.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQSCE.QTEREV_RECORD_ID = '{QuoteRevisionRecordId}' AND SAQSCE.SERVICE_ID = '{ServiceId}'""".format(QuoteRecordId=contract_quote_rec_id, QuoteRevisionRecordId=quote_revision_rec_id, ServiceId='Z0105'))
...
def splitserviceinsert():
    ...
    ##HPQC DEFECT 809 CODE STARTS...
    get_existing_record = Sql.GetList("SELECT DISTINCT SERVICE_ID FROM SAQITE WHERE QUOTE_RECORD_ID = '{contract_quote_rec_id}' AND QTEREV_RECORD_ID = '{quote_revision_rec_id}' AND SPLIT_QUOTE = 'YES'".format(contract_quote_rec_id = contract_quote_rec_id,quote_revision_rec_id =quote_revision_rec_id))
    ...
    for par_service in service_list:
        get_split_service_object = Sql.GetFirst("SELECT SERVICE_ID FROM SAQRIS WHERE QUOTE_RECORD_ID = '{QuoteRecordId}' AND QTEREV_RECORD_ID = '{RevisionRecordId}' AND SERVICE_ID = 'Z0105' and PAR_SERVICE_ID = '{ParentServiceId}'".format(QuoteRecordId=contract_quote_rec_id,RevisionRecordId=quote_revision_rec_id,ParentServiceId = par_service))
        
        ##To get the parent service details...
        service_object = Sql.GetFirst("SELECT SERVICE_ID,SERVICE_DESCRIPTION,SERVICE_RECORD_ID FROM SAQTSV WHERE QUOTE_RECORD_ID = '{QuoteRecordId}' AND QTEREV_RECORD_ID = '{RevisionRecordId}' AND SERVICE_ID = '{ParentServiceId}'".format(QuoteRecordId=contract_quote_rec_id,RevisionRecordId=quote_revision_rec_id,ParentServiceId = par_service))
        ...
        if not get_split_service_object:
            ...
            tablerow = {
                ...
                "PAR_SERVICE_ID":service_object.SERVICE_ID if service_object else null,
                "PAR_SERVICE_DESCRIPTION":service_object.SERVICE_DESCRIPTION if service_object else null,
                "PAR_SERVICE_RECORD_ID":service_object.SERVICE_RECORD_ID if service_object else null,
    ...
    #VALUES UPDATING FOR SPLIT SERVICES:
    service_tax_update = "UPDATE A SET A.SRV_SPT_ENT = B.SRV_SPT_ENT,A.SPR_SPT_ENT = B.SPR_SPT_ENT FROM SAQTSE A JOIN SAQTSE B ON A.QUOTE_RECORD_ID = B.QUOTE_RECORD_ID AND A.PAR_SERVICE_ID=B.SERVICE_ID AND A.QTEREV_RECORD_ID = B.QTEREV_RECORD_ID WHERE A.QUOTE_RECORD_ID='{contract_quote_rec_id}' AND A.QTEREV_RECORD_ID='{quote_revision_rec_id}' AND A.SERVICE_ID = '{splitservice_id}' ".format(contract_quote_rec_id=contract_quote_rec_id,quote_revision_rec_id = quote_revision_rec_id,splitservice_id=splitservice_id)
    Sql.RunQuery(service_tax_update)
    
    service_tax_update_equipment = "UPDATE A SET A.SRV_SPT_ENT = B.SRV_SPT_ENT,A.SPR_SPT_ENT = B.SPR_SPT_ENT FROM SAQSCE A JOIN SAQSCE B ON A.QUOTE_RECORD_ID = B.QUOTE_RECORD_ID AND A.PAR_SERVICE_ID=B.SERVICE_ID AND A.QTEREV_RECORD_ID = B.QTEREV_RECORD_ID AND A.EQUIPMENT_ID =B.EQUIPMENT_ID WHERE A.QUOTE_RECORD_ID='{contract_quote_rec_id}' AND A.QTEREV_RECORD_ID='{quote_revision_rec_id}' AND A.SERVICE_ID = '{splitservice_id}' ".format(contract_quote_rec_id=contract_quote_rec_id,quote_revision_rec_id = quote_revision_rec_id,splitservice_id=splitservice_id)
    Sql.RunQuery(service_tax_update_equipment)
    ...
    ##updating saqtrv@808 hpqc defect
    LOGIN_CREDENTIALS = SqlHelper.GetFirst("SELECT USER_NAME as Username,Password,Domain FROM SYCONF where Domain='AMAT_PRD'")
    Log.Info('412-contract_quote_rec_id---'+str(contract_quote_rec_id))
    ...
    Sql.RunQuery("""UPDATE SAQTRV
                        ...
                        WHERE SAQTRV.QUOTE_RECORD_ID = '{quote_rec_id}' AND SAQTRV.QUOTE_REVISION_RECORD_ID = '{quote_revision_rec_id}' 	""".format(quote_rec_id = contract_quote_rec_id ,quote_revision_rec_id = quote_revision_rec_id ) )
    ##updating saqtrv@808 hpqc defect end
    # INC08787871 - Start - M
    ##Calling the iflow for quote header writeback to cpq to c4c code starts..
    CQCPQC4CWB.writeback_to_c4c("quote_header",contract_quote_rec_id,quote_revision_rec_id)
    CQCPQC4CWB.writeback_to_c4c("opportunity_header",contract_quote_rec_id,quote_revision_rec_id)
...
def servicelevel_split_equip(seid):
    ...
    updatesaqritchild ="""UPDATE A SET A.SPLIT_PERCENT = ISNULL(SPSPCT, 0) FROM SAQRIT A JOIN SAQSCE B ON A.QUOTE_RECORD_ID =B.QUOTE_RECORD_ID  AND A.QTEREV_RECORD_ID  =B.QTEREV_RECORD_ID AND A.SERVICE_ID =B.SERVICE_ID AND A.FABLOCATION_ID = B.FABLOCATION_ID AND A.GREENBOOK  = B.GREENBOOK AND A.OBJECT_ID = B.EQUIPMENT_ID WHERE A.QUOTE_RECORD_ID ='{contract_quote_rec_id}' AND A.QTEREV_RECORD_ID='{quote_revision_rec_id}' AND A.SERVICE_ID ='{seid}'""".format(contract_quote_rec_id =contract_quote_rec_id,quote_revision_rec_id =quote_revision_rec_id,seid =seid)
    Sql.RunQuery(updatesaqritchild)
    ...
    SELECT DISTINCT SAQRIT.CONTRACT_VALID_FROM,SAQRIT.CONTRACT_VALID_TO,SAQRIT.DOC_CURRENCY,SAQRIT.DOCURR_RECORD_ID,SAQRIT.EXCHANGE_RATE,SAQRIT.EXCHANGE_RATE_DATE,SAQRIT.EXCHANGE_RATE_RECORD_ID,SAQRIT.GL_ACCOUNT_NO,SAQRIT.GLOBAL_CURRENCY,SAQRIT.GLOBAL_CURRENCY_RECORD_ID,(({equipments_count} + ROW_NUMBER()OVER(ORDER BY(SAQRIT.CpqTableEntryId))) * {item_number_saqrit_inc}) AS LINE,SAQRIT.OBJECT_ID,SAQRIT.OBJECT_TYPE,'{splitservice_name}' as SERVICE_DESCRIPTION,'{splitservice_id}' as SERVICE_ID,'{splitservice_recid}' as SERVICE_RECORD_ID,SAQRIT.PROFIT_CENTER,SAQRIT.QUANTITY,SAQRIT.QUOTE_ID,SAQRIT.QUOTE_RECORD_ID,SAQRIT.QTEREV_ID,SAQRIT.QTEREV_RECORD_ID,SAQRIT.REF_SALESORDER,SAQRIT.TAX_PERCENTAGE,SAQRIT.TAX_AMOUNT,SAQRIT.TAX_AMOUNT_INGL_CURR,SAQRIT.TAXCLASSIFICATION_DESCRIPTION,SAQRIT.TAXCLASSIFICATION_ID,SAQRIT.TAXCLASSIFICATION_RECORD_ID,SAQRIT.FABLOCATION_ID,SAQRIT.FABLOCATION_NAME,SAQRIT.FABLOCATION_RECORD_ID,SAQRIT.GREENBOOK,SAQRIT.GREENBOOK_RECORD_ID,SAQRIT.NET_PRICE,SAQRIT.NET_PRICE_INGL_CURR,SAQRIT.PLANT_ID,SAQRIT.PLANT_NAME,SAQRIT.PLANT_RECORD_ID,SAQRIT.COMVAL_INGL_CURR,SAQRIT.ESTVAL_INGL_CURR,SAQRIT.NET_VALUE ,SAQRIT.NET_VALUE_INGL_CURR,SAQRIT.UNIT_PRICE,SAQRIT.UNIT_PRICE_INGL_CURR,SAQRIS.QUOTE_REV_ITEM_SUMMARY_RECORD_ID,SAQRIT.MODULE_ID,SAQRIT.MODULE_NAME,SAQRIT.MODULE_RECORD_ID,SAQRIT.LINE AS PARQTEITM_LINE,SAQRIT.QUOTE_REVISION_CONTRACT_ITEM_ID AS PARQTEITM_LINE_RECORD_ID,SAQRIT.BILLING_TYPE,SAQRIT.COMMITTED_VALUE,SAQRIT.ESTIMATED_VALUE,SAQRIT.SPLIT_PERCENT,SAQRIT.SPLIT,'ACQUIRED' AS STATUS, SAQRIT.EQUIPMENT_ID, SAQRIT.EQUIPMENT_RECORD_ID FROM SAQRIT (NOLOCK) JOIN SAQRIS (NOLOCK) ON SAQRIS.QUOTE_RECORD_ID = SAQRIT.QUOTE_RECORD_ID AND SAQRIS.QTEREV_RECORD_ID = SAQRIT.QTEREV_RECORD_ID AND SAQRIS.PAR_SERVICE_ID = SAQRIT.SERVICE_ID AND SAQRIS.SERVICE_ID = '{splitservice_id}' WHERE SAQRIT.QUOTE_RECORD_ID = '{contract_quote_rec_id}' AND SAQRIT.QTEREV_RECORD_ID = '{quote_revision_rec_id}' AND SAQRIT.SERVICE_ID = '{seid}' AND ISNULL(SAQRIT.SPLIT,'')='')A""".format(contract_quote_rec_id = contract_quote_rec_id , quote_revision_rec_id = quote_revision_rec_id,item_number_saqrit_inc =item_number_saqrit_inc,equipments_count =equipments_count,splitservice_recid = splitservice_recid,splitservice_id=splitservice_id,splitservice_name = splitservice_name,seid = seid,UserId=user_id,UserName=user_name))
    
    #UPDATE PRICING TO CLONE RECORD AS WELL AS MASTER
    update_pricing = SqlHelper.GetFirst("sp_executesql @T=N'UPDATE A SET A.NET_PRICE = (CASE WHEN ISNULL(A.PARQTEITM_LINE_RECORD_ID,'''')='''' AND ISNULL(A.PARQTEITM_LINE,''0'') = ''0'' THEN NET_PRICE * XY.SPLIT_PERCENT/100 WHEN A.PARQTEITM_LINE_RECORD_ID <> A.QUOTE_REVISION_CONTRACT_ITEM_ID AND ISNULL(A.PARQTEITM_LINE,''0'') <> ''0''  and ISNULL(A.PARQTEITM_LINE_RECORD_ID,'''') <> '''' THEN NET_PRICE * (100-XY.SPLIT_PERCENT)/100 END),A.NET_PRICE_INGL_CURR = (CASE WHEN ISNULL(A.PARQTEITM_LINE_RECORD_ID,'''')='''' AND ISNULL(A.PARQTEITM_LINE,''0'') = ''0'' THEN NET_PRICE_INGL_CURR * XY.SPLIT_PERCENT/100 WHEN A.PARQTEITM_LINE_RECORD_ID <> A.QUOTE_REVISION_CONTRACT_ITEM_ID AND ISNULL(A.PARQTEITM_LINE,''0'') <> ''0''  and ISNULL(A.PARQTEITM_LINE_RECORD_ID,'''') <> '''' THEN NET_PRICE_INGL_CURR * (100-XY.SPLIT_PERCENT)/100 END),A.NET_VALUE = (CASE WHEN ISNULL(A.PARQTEITM_LINE_RECORD_ID,'''')='''' AND ISNULL(A.PARQTEITM_LINE,''0'') = ''0'' THEN NET_VALUE * XY.SPLIT_PERCENT/100 WHEN A.PARQTEITM_LINE_RECORD_ID <> A.QUOTE_REVISION_CONTRACT_ITEM_ID AND ISNULL(A.PARQTEITM_LINE,''0'') <> ''0''  and ISNULL(A.PARQTEITM_LINE_RECORD_ID,'''') <> '''' THEN NET_VALUE * (100-XY.SPLIT_PERCENT)/100 END) + TAX_AMOUNT,A.NET_VALUE_INGL_CURR = (CASE WHEN ISNULL(A.PARQTEITM_LINE_RECORD_ID,'''')='''' AND ISNULL(A.PARQTEITM_LINE,''0'') = ''0'' THEN NET_VALUE_INGL_CURR * XY.SPLIT_PERCENT/100 WHEN A.PARQTEITM_LINE_RECORD_ID <> A.QUOTE_REVISION_CONTRACT_ITEM_ID AND ISNULL(A.PARQTEITM_LINE,''0'') <> ''0''  and ISNULL(A.PARQTEITM_LINE_RECORD_ID,'''') <> '''' THEN NET_VALUE_INGL_CURR * (100-XY.SPLIT_PERCENT)/100 END) + TAX_AMOUNT,A.SPLIT = ''YES'',A.SPLIT_PERCENT = (CASE WHEN ISNULL(A.PARQTEITM_LINE_RECORD_ID,'''')='''' AND ISNULL(A.PARQTEITM_LINE,''0'') = ''0'' THEN XY.SPLIT_PERCENT WHEN A.PARQTEITM_LINE_RECORD_ID <> A.QUOTE_REVISION_CONTRACT_ITEM_ID AND ISNULL(A.PARQTEITM_LINE,''0'') <> ''0''  and ISNULL(A.PARQTEITM_LINE_RECORD_ID,'''') <> '''' THEN 100-XY.SPLIT_PERCENT END),A.UNIT_PRICE_INGL_CURR = (CASE WHEN A.PARQTEITM_LINE_RECORD_ID <> A.QUOTE_REVISION_CONTRACT_ITEM_ID AND ISNULL(A.PARQTEITM_LINE,''0'') <> ''0''  and ISNULL(A.PARQTEITM_LINE_RECORD_ID,'''') <> '''' THEN NET_PRICE_INGL_CURR * (100-XY.SPLIT_PERCENT)/100 END),A.UNIT_PRICE = (CASE WHEN A.PARQTEITM_LINE_RECORD_ID <> A.QUOTE_REVISION_CONTRACT_ITEM_ID AND ISNULL(A.PARQTEITM_LINE,''0'') <> ''0''  and ISNULL(A.PARQTEITM_LINE_RECORD_ID,'''') <> '''' THEN NET_PRICE * (100-XY.SPLIT_PERCENT)/100 END) FROM SAQRIT(NOLOCK)A INNER JOIN(SELECT DISTINCT B.SPLIT_PERCENT,B.QUOTE_RECORD_ID,B.QTEREV_RECORD_ID,B.OBJECT_ID FROM SAQRIT B(NOLOCK) WHERE B.QUOTE_RECORD_ID = ''"+str(contract_quote_rec_id)+"'' AND B.SERVICE_ID = ''"+str(seid)+"'') AS XY ON A.QUOTE_RECORD_ID = XY.QUOTE_RECORD_ID AND A.QTEREV_RECORD_ID = XY.QTEREV_RECORD_ID AND A.OBJECT_ID = XY.OBJECT_ID  WHERE A.QUOTE_RECORD_ID = ''"+str(contract_quote_rec_id)+"'' AND ISNULL(A.SPLIT,'''')<>''YES'' AND A.SERVICE_ID IN( ''"+str(seid)+"'' ,''"+str(splitservice_id)+"'')'".format(contract_quote_rec_id =contract_quote_rec_id,seid =seid,splitservice_id = splitservice_id))
    ...
    # A055S000P01-17876 - End
    tax_calculation_master = ScriptExecutor.ExecuteGlobal('CQCPSTAXRE',{'service_id':seid, 'Fun_type':'CPQ_TO_CPS'})
    annaul_line_insert = ScriptExecutor.ExecuteGlobal("CQINSQTITM",{"ContractQuoteRecordId":contract_quote_rec_id, "ContractQuoteRevisionRecordId":quote_revision_rec_id, "ServiceId":splitservice_id,"ParentServiceId":seid, "ActionType":'INSERT_LINE_ITEMS', 'TriggeredFrom':'Split'})
    tax_calculation = ScriptExecutor.ExecuteGlobal('CQCPSTAXRE',{'service_id':splitservice_id, 'Fun_type':'CPQ_TO_CPS'})

    Sql.RunQuery("""UPDATE SAQICO SET SPQTEV = 'No'	
                    FROM SAQICO (NOLOCK) 
                    WHERE SAQICO.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQICO.QTEREV_RECORD_ID = '{QuoteRevisionRecordId}' AND SAQICO.SERVICE_ID = '{ServiceId}' AND QTEITM_RECORD_ID IN ('{ItemRecordIds}')
    ...
    Sql.RunQuery("""UPDATE SAQICO SET 
                    ...
                    JOIN (SELECT QTEITM_RECORD_ID,QUOTE_RECORD_ID,QTEREV_RECORD_ID,FABLOCATION_ID,GREENBOOK,EQUIPMENT_ID,SPSPCT,SPCTPR,SPCTCS,SPCTMG,SVSPCT,SVCTPR,SVCTCS,SVCTMG,STATUS,CNTYER  FROM SAQICO WHERE SAQICO.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQICO.QTEREV_RECORD_ID = '{QuoteRevisionRecordId}' AND SAQICO.SERVICE_ID = '{ParentServiceId}' AND ISNULL(SAQICO.SPQTEV,'No') = 'Yes') P_SAQICO ON P_SAQICO.QUOTE_RECORD_ID = SAQICO.QUOTE_RECORD_ID AND P_SAQICO.QTEREV_RECORD_ID = SAQICO.QTEREV_RECORD_ID AND P_SAQICO.FABLOCATION_ID = SAQICO.FABLOCATION_ID AND P_SAQICO.GREENBOOK = SAQICO.GREENBOOK AND P_SAQICO.EQUIPMENT_ID = SAQICO.EQUIPMENT_ID AND P_SAQICO.QTEITM_RECORD_ID = SAQRIT.PARQTEITM_LINE_RECORD_ID AND P_SAQICO.CNTYER = SAQICO.CNTYER
    ...
    Sql.RunQuery("""UPDATE SAQICO SET TNTVGC = CASE WHEN ISNULL(SAQICO.SPQTEV,'No') = 'No' THEN ROUND((CNTPRC * ISNULL(DCCRFX,1)) ,CONVERT(INT,IQ.DECIMAL_PLACES),CONVERT(INT,IQ.ROUNDING_METHOD)) ELSE ROUND((SPCTPR * ISNULL(DCCRFX,1)) ,CONVERT(INT,IQ.DECIMAL_PLACES),CONVERT(INT,IQ.ROUNDING_METHOD)) END,TNTMGC = CASE WHEN ISNULL(SAQICO.SPQTEV,'No') = 'No' THEN CNTPRC - CNTCST ELSE SPCTPR - SPCTCS END,TNTMPC = CASE WHEN ISNULL(SAQICO.SPQTEV,'No') = 'No' THEN (CNTPRC - CNTCST) / ISNULL(NULLIF(CNTPRC,0), 1) ELSE (SPCTPR - SPCTCS) / ISNULL(NULLIF(SPCTPR,0), 1) END
    FROM SAQICO (NOLOCK) 
    JOIN (SELECT DISTINCT SAQRIT.QUOTE_RECORD_ID, SAQRIT.QTEREV_RECORD_ID, SAQRIT.SERVICE_ID, SAQRIT.LINE, CASE WHEN ROUNDING_DECIMAL_PLACES = '' THEN 0 ELSE ROUNDING_DECIMAL_PLACES END AS DECIMAL_PLACES, CASE WHEN ROUNDING_METHOD='ROUND DOWN' THEN 1 ELSE 0 END AS ROUNDING_METHOD FROM SAQRIT(NOLOCK) JOIN PRCURR (NOLOCK) ON SAQRIT.DOC_CURRENCY = PRCURR.CURRENCY WHERE SAQRIT.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQRIT.QTEREV_RECORD_ID = '{QuoteRevisionRecordId}') IQ ON IQ.QUOTE_RECORD_ID = SAQICO.QUOTE_RECORD_ID AND IQ.QTEREV_RECORD_ID = SAQICO.QTEREV_RECORD_ID AND IQ.SERVICE_ID = SAQICO.SERVICE_ID AND IQ.LINE = SAQICO.LINE
    ...
    # Year 1
    Sql.RunQuery("UPDATE SAQRIT SET YEAR_1 = SAQICO.TNTVDC, YEAR_1_INGL_CURR = SAQICO.TNTVGC FROM SAQRIT (NOLOCK) JOIN SAQICO (NOLOCK) ON SAQICO.QUOTE_RECORD_ID = SAQRIT.QUOTE_RECORD_ID AND SAQICO.QTEREV_RECORD_ID = SAQRIT.QTEREV_RECORD_ID AND SAQICO.SERVICE_ID = SAQRIT.SERVICE_ID AND SAQICO.LINE = SAQRIT.LINE WHERE SAQRIT.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQRIT.QTEREV_RECORD_ID = '{QuoteRevisionRecordId}' AND ISNULL(SAQICO.CNTYER,'') = 'YEAR 1' AND SAQICO.SERVICE_ID <> 'Z0116'".format(QuoteRecordId=contract_quote_rec_id,QuoteRevisionRecordId=quote_revision_rec_id))

    # Year 2
    Sql.RunQuery("UPDATE SAQRIT SET YEAR_2 = SAQICO.TNTVDC, YEAR_2_INGL_CURR = SAQICO.TNTVGC FROM SAQRIT (NOLOCK) JOIN SAQICO (NOLOCK) ON SAQICO.QUOTE_RECORD_ID = SAQRIT.QUOTE_RECORD_ID AND SAQICO.QTEREV_RECORD_ID = SAQRIT.QTEREV_RECORD_ID AND SAQICO.SERVICE_ID = SAQRIT.SERVICE_ID AND SAQICO.LINE = SAQRIT.LINE WHERE SAQRIT.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQRIT.QTEREV_RECORD_ID = '{QuoteRevisionRecordId}' AND ISNULL(SAQICO.CNTYER,'') = 'YEAR 2' AND SAQICO.SERVICE_ID <> 'Z0116'".format(QuoteRecordId=contract_quote_rec_id,QuoteRevisionRecordId=quote_revision_rec_id))

    # Year 3
    Sql.RunQuery("UPDATE SAQRIT SET YEAR_3 = SAQICO.TNTVDC, YEAR_3_INGL_CURR = SAQICO.TNTVGC FROM SAQRIT (NOLOCK) JOIN SAQICO (NOLOCK) ON SAQICO.QUOTE_RECORD_ID = SAQRIT.QUOTE_RECORD_ID AND SAQICO.QTEREV_RECORD_ID = SAQRIT.QTEREV_RECORD_ID AND SAQICO.SERVICE_ID = SAQRIT.SERVICE_ID AND SAQICO.LINE = SAQRIT.LINE WHERE SAQRIT.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQRIT.QTEREV_RECORD_ID = '{QuoteRevisionRecordId}' AND ISNULL(SAQICO.CNTYER,'') = 'YEAR 3' AND SAQICO.SERVICE_ID <> 'Z0116'".format(QuoteRecordId=contract_quote_rec_id,QuoteRevisionRecordId=quote_revision_rec_id))

    # Year 4
    Sql.RunQuery("UPDATE SAQRIT SET YEAR_4 = SAQICO.TNTVDC, YEAR_4_INGL_CURR = SAQICO.TNTVGC FROM SAQRIT (NOLOCK) JOIN SAQICO (NOLOCK) ON SAQICO.QUOTE_RECORD_ID = SAQRIT.QUOTE_RECORD_ID AND SAQICO.QTEREV_RECORD_ID = SAQRIT.QTEREV_RECORD_ID AND SAQICO.SERVICE_ID = SAQRIT.SERVICE_ID AND SAQICO.LINE = SAQRIT.LINE WHERE SAQRIT.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQRIT.QTEREV_RECORD_ID = '{QuoteRevisionRecordId}' AND ISNULL(SAQICO.CNTYER,'') = 'YEAR 4' AND SAQICO.SERVICE_ID <> 'Z0116'".format(QuoteRecordId=contract_quote_rec_id,QuoteRevisionRecordId=quote_revision_rec_id))

    # Year 5
    Sql.RunQuery("UPDATE SAQRIT SET YEAR_5 = SAQICO.TNTVDC, YEAR_5_INGL_CURR = SAQICO.TNTVGC FROM SAQRIT (NOLOCK) JOIN SAQICO (NOLOCK) ON SAQICO.QUOTE_RECORD_ID = SAQRIT.QUOTE_RECORD_ID AND SAQICO.QTEREV_RECORD_ID = SAQRIT.QTEREV_RECORD_ID AND SAQICO.SERVICE_ID = SAQRIT.SERVICE_ID AND SAQICO.LINE = SAQRIT.LINE WHERE SAQRIT.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQRIT.QTEREV_RECORD_ID = '{QuoteRevisionRecordId}' AND ISNULL(SAQICO.CNTYER,'') = 'YEAR 5' AND SAQICO.SERVICE_ID <> 'Z0116'".format(QuoteRecordId=contract_quote_rec_id,QuoteRevisionRecordId=quote_revision_rec_id))
    ...
    Sql.RunQuery("""UPDATE SAQRIT 
                    ...
                    FROM SAQRIT
                        INNER JOIN (SELECT SAQICO.QUOTE_RECORD_ID, SAQICO.QTEREV_RECORD_ID,SAQICO.SERVICE_ID,SAQICO.GRNBOK,SAQICO.EQUIPMENT_ID,SAQICO.LINE,
                                ...
                                FROM SAQICO WHERE SAQICO.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQICO.QTEREV_RECORD_ID = '{QuoteRevisionRecordId}' GROUP BY SAQICO.QUOTE_RECORD_ID, SAQICO.QTEREV_RECORD_ID,SAQICO.SERVICE_ID,SAQICO.GRNBOK,SAQICO.EQUIPMENT_ID, SAQICO.LINE
                            ...
                            WHERE SAQRIT.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQRIT.QTEREV_RECORD_ID='{QuoteRevisionRecordId}'""".format( QuoteRecordId=contract_quote_rec_id ,QuoteRevisionRecordId=quote_revision_rec_id))
    # A055S000P01-17876 - End	
    ...
    Sql.RunQuery("""UPDATE SAQRIS 
                            ...
                            INNER JOIN (SELECT SAQRIT.QUOTE_RECORD_ID, SAQRIT.QTEREV_RECORD_ID,SAQRIT.SERVICE_ID,SAQRIT.QTEITMSUM_RECORD_ID,
                                ...
                                FROM SAQRIT (NOLOCK)
                                WHERE SAQRIT.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQRIT.QTEREV_RECORD_ID = '{QuoteRevisionRecordId}' AND SAQRIT.SERVICE_ID = '{ServiceId}' GROUP BY SAQRIT.QUOTE_RECORD_ID, SAQRIT.QTEREV_RECORD_ID,SAQRIT.SERVICE_ID,SAQRIT.QTEITMSUM_RECORD_ID
                        ...
                        WHERE SAQRIS.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQRIS.QTEREV_RECORD_ID='{QuoteRevisionRecordId}' AND SAQRIS.SERVICE_ID = '{ServiceId}'""".format(QuoteRecordId= contract_quote_rec_id,QuoteRevisionRecordId=quote_revision_rec_id,ServiceId=seid))
    Sql.RunQuery("""UPDATE SAQRIS 
                            SET UNIT_PRICE_INGL_CURR = IQ.UNIT_PRICE_INGL_CURR, 
                            NET_PRICE_INGL_CURR=IQ.NET_PRICE_INGL_CURR,
                            UNIT_PRICE = IQ.UNIT_PRICE, 
                        ...
                        WHERE SAQRIS.QUOTE_RECORD_ID = '{QuoteRecordId}' AND SAQRIS.QTEREV_RECORD_ID='{QuoteRevisionRecordId}' AND SAQRIS.SERVICE_ID = '{ServiceId}'""".format(QuoteRecordId= contract_quote_rec_id,QuoteRevisionRecordId=quote_revision_rec_id,ServiceId=splitservice_id))
    # If SAQICO status is ON HOLD TKM for Z0105 then we update it us Acquired
    Sql.RunQuery("UPDATE SAQICO SET STATUS = 'ACQUIRED' FROM SAQICO (NOLOCK) WHERE QUOTE_RECORD_ID = '{QuoteRecordId}' AND QTEREV_RECORD_ID = '{QuoteRevisionRecordId}' AND SERVICE_ID = '{ServiceId}' AND STATUS = 'CFG-ON HOLD TKM'".format(QuoteRecordId=contract_quote_rec_id,QuoteRevisionRecordId=quote_revision_rec_id, ServiceId=splitservice_id))

    # If SAQRIT status is ON HOLD TKM for Z0105 then we update it us Acquired
    Sql.RunQuery("UPDATE SAQRIT SET STATUS = 'ACQUIRED' FROM SAQRIT (NOLOCK) WHERE QUOTE_RECORD_ID = '{QuoteRecordId}' AND QTEREV_RECORD_ID = '{QuoteRevisionRecordId}' AND SERVICE_ID = '{ServiceId}' AND STATUS = 'CFG-ON HOLD TKM'".format(QuoteRecordId=contract_quote_rec_id,QuoteRevisionRecordId=quote_revision_rec_id, ServiceId=splitservice_id))
    
    # update_service_parent_summary = """UPDATE A  SET A.NET_PRICE = B.NET_PRICE,A.NET_PRICE_INGL_CURR = B.NET_PRICE_INGL_CURR,A.NET_VALUE = B.NET_VALUE,A.NET_VALUE_INGL_CURR = B.NET_VALUE_INGL_CURR,A.UNIT_PRICE = B.UNIT_PRICE,A.UNIT_PRICE_INGL_CURR = B.UNIT_PRICE_INGL_CURR FROM SAQRIS A(NOLOCK) JOIN (SELECT SUM(NET_PRICE) AS NET_PRICE,SUM(NET_PRICE_INGL_CURR) AS NET_PRICE_INGL_CURR,SUM(NET_VALUE) AS NET_VALUE,SUM(NET_VALUE_INGL_CURR) AS NET_VALUE_INGL_CURR,SUM(UNIT_PRICE) AS UNIT_PRICE,SUM(UNIT_PRICE_INGL_CURR) AS UNIT_PRICE_INGL_CURR,QUOTE_RECORD_ID,QTEREV_RECORD_ID,SERVICE_ID,QTEITMSUM_RECORD_ID from SAQRIT(NOLOCK) WHERE QUOTE_RECORD_ID ='{contract_quote_rec_id}' AND QTEREV_RECORD_ID = '{quote_revision_rec_id}'  AND SERVICE_ID = '{seid}' GROUP BY QUOTE_RECORD_ID,QTEREV_RECORD_ID,SERVICE_ID,QTEITMSUM_RECORD_ID) B ON A.QUOTE_RECORD_ID = B.QUOTE_RECORD_ID AND A.SERVICE_ID=B.SERVICE_ID AND A.QTEREV_RECORD_ID = B.QTEREV_RECORD_ID AND A.QUOTE_REV_ITEM_SUMMARY_RECORD_ID = B.QTEITMSUM_RECORD_ID""".format(contract_quote_rec_id= contract_quote_rec_id,quote_revision_rec_id = quote_revision_rec_id,seid =seid)
    # Sql.RunQuery(update_service_parent_summary)
    # update_service_child_summary = """UPDATE A  SET A.NET_PRICE = B.NET_PRICE,A.NET_PRICE_INGL_CURR = B.NET_PRICE_INGL_CURR,A.NET_VALUE = B.NET_VALUE,A.NET_VALUE_INGL_CURR = B.NET_VALUE_INGL_CURR,A.UNIT_PRICE = B.UNIT_PRICE,A.UNIT_PRICE_INGL_CURR = B.UNIT_PRICE_INGL_CURR FROM SAQRIS A(NOLOCK) JOIN (SELECT SUM(NET_PRICE) AS NET_PRICE,SUM(NET_PRICE_INGL_CURR) AS NET_PRICE_INGL_CURR,SUM(NET_VALUE) AS NET_VALUE,SUM(NET_VALUE_INGL_CURR) AS NET_VALUE_INGL_CURR,SUM(UNIT_PRICE) AS UNIT_PRICE,SUM(UNIT_PRICE_INGL_CURR) AS UNIT_PRICE_INGL_CURR,QUOTE_RECORD_ID,QTEREV_RECORD_ID,SERVICE_ID,QTEITMSUM_RECORD_ID from SAQRIT(NOLOCK) WHERE QUOTE_RECORD_ID ='{contract_quote_rec_id}' AND QTEREV_RECORD_ID = '{quote_revision_rec_id}'  AND SERVICE_ID = '{splitservice_id}' GROUP BY QUOTE_RECORD_ID,QTEREV_RECORD_ID,SERVICE_ID,QTEITMSUM_RECORD_ID) B ON A.QUOTE_RECORD_ID = B.QUOTE_RECORD_ID AND A.SERVICE_ID=B.SERVICE_ID AND A.QTEREV_RECORD_ID = B.QTEREV_RECORD_ID AND A.QUOTE_REV_ITEM_SUMMARY_RECORD_ID = B.QTEITMSUM_RECORD_ID""".format(contract_quote_rec_id= contract_quote_rec_id,quote_revision_rec_id = quote_revision_rec_id,seid =seid,splitservice_id =splitservice_id)
...
def servicelevel_split_green(seid):
    ...
    #2162 bug start
    #TAXVGC - Tax Amount (Documnet Currency) 
    Sql.RunQuery("UPDATE SAQICO SET TAXVDC = TNTVDC * (ISNULL(TAXVTP,0)/100) FROM SAQICO (NOLOCK) WHERE QUOTE_RECORD_ID = '{QuoteRecordId}' AND QTEREV_RECORD_ID = '{QuoteRevisionRecordId}'".format(QuoteRecordId=contract_quote_rec_id,QuoteRevisionRecordId=quote_revision_rec_id))
    #2162 bug end
