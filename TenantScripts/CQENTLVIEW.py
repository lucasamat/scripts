                                ...
                                for val in GetXMLsecField:
                                    ...
                                    if DType == "Drop Down":
                                        ...
                                        if standard_attr_values and val.ENTITLEMENT_ID == str(attrSysId):
                                            # A055S000P01-20694
                                            approval_status = Sql.GetFirst("SELECT APPROVAL_REQUIRED FROM PRENVL(NOLOCK) WHERE ENTITLEMENT_ID = '{}' AND ENTITLEMENT_DISPLAY_VALUE = '{}'".format(str(attrSysId),str(val.ENTITLEMENT_DISPLAY_VALUE).replace("'","''")) )
                                            if approval_status:
                                                #Trace.Write("imgstr--1-"+str(approval_status.APPROVAL_REQUIRED))
                                                if approval_status.APPROVAL_REQUIRED == True:
                                                    imgstr = '<img title=Acquired src=/mt/'+str(Sql.getDomainDetails())+'/Additionalfiles/clock_exe.svg>'
                                            # A055S000P01-20747
                                            # A055S000P01-20776
                                            elif attrSysId in ('AGS_Z0035_KPI_TGTKPI','AGS_Z0035W_KPI_TGTKPI','AGS_Z0009_PQB_PMQCPC') or (attrSysId=='AGS_Z0092W_KPI_TGTKPI' and str(val.ENTITLEMENT_DISPLAY_VALUE).strip() != 'Excluded') or (attrSysId=='AGS_Z0092W_CVR_CNTCOV' and str(val.ENTITLEMENT_DISPLAY_VALUE).strip() == '7x24'):
                                                # A055S000P01-20776
                                                #Trace.Write("inside add target kpi img str")
                                            ...
                                            for value in standard_attr_values:
                                                #INC08621345 - M
                                                if value.SYSTEM_ID in dropdowndisallowlist or (attrSysId in ('AGS_Z0091_NET_PRMALB','AGS_Z0091W_NET_PRMALB','AGS_Z0035_NET_PRMALB','AGS_Z0035W_NET_PRMALB') and self.treeparam in ('PDC','MPS') and value.STANDARD_ATTRIBUTE_DISPLAY_VAL == 'Included - Monthly and Above') or (attrSysId in ('AGS_Z0091_KPI_TGTKPI','AGS_Z0091W_KPI_TGTKPI','AGS_Z0092W_KPI_TGTKPI','AGS_Z0092_KPI_TGTKPI','AGS_Z0090_KPI_TGTKPI','AGS_Z0035W_KPI_TGTKPI' ) and self.treeparam != 'IMPLANT' and value.STANDARD_ATTRIBUTE_DISPLAY_VAL in ('Max wafer Output <= 4%','Max Wafer Output >4%')) or(attrSysId in ('AGS_Z0091_KPI_TGTKPI','AGS_Z0091W_KPI_TGTKPI','AGS_Z0092W_KPI_TGTKPI','AGS_Z0092_KPI_TGTKPI','AGS_Z0090_KPI_TGTKPI','AGS_Z0035W_KPI_TGTKPI' ) and self.treeparam not in ('MDP','DDP') and value.STANDARD_ATTRIBUTE_DISPLAY_VAL in ('Excursion Detection') ) or (attrSysId in ('AGS_Z0010_PQB_USRFEE') and self.treeparam not in ('CMP') and value.STANDARD_ATTRIBUTE_DISPLAY_VAL == 'AGS_{}_PQB_USRFEE_002'.format(ProductPartnumber) ) or (attrSysId in ('AGS_Z0091_KPI_PRPFGT','AGS_Z0091W_KPI_PRPFGT') and self.treeparam in ('PDC') and value.STANDARD_ATTRIBUTE_DISPLAY_VAL == "Std Srvc + All PM's" ) or (attrSysId == 'AGS_Z0009_TSC_CUOWPN' and self.treeparam  != 'DISPLAY' and value.STANDARD_ATTRIBUTE_DISPLAY_VAL == 'Yes' ) or (attrSysId == 'AGS_Z0099_TSC_CONSUM' and self.treeparam  not in ('PDC','MPS') and value.STANDARD_ATTRIBUTE_DISPLAY_VAL == 'Included' ) or (attrSysId == 'AGS_Z0099_NET_PRMALB' and self.treeparam  in ('PDC','MPS') and value.STANDARD_ATTRIBUTE_DISPLAY_VAL in ('Included - Quarterly and Above','Included - Monthly and Above','Excluded') ):
                                                #INC08621345 - M
                                                    disallow_style = "style = 'display:none'"
                                                else:
                                                    disallow_style = ""
                                        ...
                                        if standard_attr_values and val.ENTITLEMENT_ID == str(attrSysId):
                                            ...
                                            for value in standard_attr_values:
                                                ...
                                                try:
                                                    # A055S000P01-20694
                                                    flag = 0
                                                    for x in str(val.ENTITLEMENT_DISPLAY_VALUE).replace("'","''").split(","):
                                                        approval_status = Sql.GetFirst("SELECT APPROVAL_REQUIRED FROM PRENVL WHERE ENTITLEMENT_ID = '{}' AND ENTITLEMENT_DISPLAY_VALUE = '{}'".format(str(attrSysId),x) )
                                                        if approval_status:
                                                            flag = 1
                                                            break
                                                    if flag == 1:
                                        ...
                                        # A055S000P01-20694
                                        approval_status = Sql.GetFirst("SELECT APPROVAL_REQUIRED FROM PRENVL WHERE ENTITLEMENT_ID = '{}' AND ENTITLEMENT_DISPLAY_VALUE = ''".format(str(attrSysId)) )
                                        if approval_status:
                                            if approval_status.APPROVAL_REQUIRED == True:
                                                imgstr = ('<img title=Acquired src=/mt/appliedmaterials_prd/Additionalfiles/clock_exe.svg>')
                                        # A055S000P01-20694
