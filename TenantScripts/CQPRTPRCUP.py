...
if getPricingProc:
	...
	if exch == '':
		...
		#Hadoop Fix , A055S000P01-20862 - Start - M Added revision id condition to avoid updating CpqTableEntryDateModified in SAQTRV with same date for all revisions
		update_SAQTRV = "UPDATE SAQTRV  SET PRICINGPROCEDURE_ID = '{prc}', EXCHANGE_RATE_TYPE = '{EXCH}' WHERE SAQTRV.QUOTE_ID = '{quote}' AND SAQTRV.QTEREV_RECORD_ID = '{revision}' ".format(prc=str(PricingProcedure),EXCH=str(exch), quote=QUOTE, revision = revision)
...
if part_query:
	...
	while L == 1:
		...
		if part_number != '':
			...
			L = 0
		if not get_part_query and part_number != '': #INC08733295 - Start - M
			get_part_query = Sql.GetList("SELECT DISTINCT PART_NUMBER, QUANTITY as ANNUAL_QUANTITY,ODCC_FLAG,SHPACCOUNT_ID,SALESUOM_ID,SALESUOM_CONVERSION_FACTOR,BASEUOM_ID FROM (SELECT PART_NUMBER, QUANTITY,null as ODCC_FLAG, null as SHPACCOUNT_ID, SALESUOM_ID, SALESUOM_CONVERSION_FACTOR, BASEUOM_ID, ROW_NUMBER() OVER(ORDER BY PART_NUMBER) AS SNO FROM SAQRSP (NOLOCK) WHERE QUOTE_ID = '"+str(QUOTE)+"' AND QTEREV_RECORD_ID = '"+str(revision)+"'AND PART_NUMBER = '"+str(part_number)+"' AND INCLUDED = 1 AND SERVICE_ID IN('Z0100') )A WHERE SNO>="+str(start)+" AND SNO<="+str(end)+"  ")
			L = 0 #INC08733295 - END - M
		...
		if not get_part_query: #INC08733295 - Start - M
			get_part_query = Sql.GetList("SELECT DISTINCT PART_NUMBER, QUANTITY as ANNUAL_QUANTITY,ODCC_FLAG,SHPACCOUNT_ID,SALESUOM_ID,SALESUOM_CONVERSION_FACTOR,BASEUOM_ID FROM (SELECT PART_NUMBER, QUANTITY,null as ODCC_FLAG, null as SHPACCOUNT_ID, SALESUOM_ID, SALESUOM_CONVERSION_FACTOR, BASEUOM_ID, ROW_NUMBER() OVER(ORDER BY PART_NUMBER) AS SNO FROM SAQRSP (NOLOCK) WHERE QUOTE_ID = '"+str(QUOTE)+"' AND QTEREV_RECORD_ID = '"+str(revision)+"' AND INCLUDED = 1 AND SERVICE_ID IN('Z0100') )A WHERE SNO>="+str(start)+" AND SNO<="+str(end)+"  ") #INC08733295 - END - M
		if not get_part_query:
			get_part_query = Sql.GetList("SELECT DISTINCT PART_NUMBER, CUSTOMER_ANNUAL_QUANTITY as ANNUAL_QUANTITY,ODCC_FLAG, SHPACCOUNT_ID,SALESUOM_ID,SALESUOM_CONVERSION_FACTOR,BASEUOM_ID FROM (SELECT PART_NUMBER, CUSTOMER_ANNUAL_QUANTITY,ODCC_FLAG,SHPACCOUNT_ID ,SALESUOM_ID,SALESUOM_CONVERSION_FACTOR,BASEUOM_ID,ROW_NUMBER() OVER(ORDER BY PART_NUMBER) AS SNO FROM SAQSPT (NOLOCK) WHERE QUOTE_ID = '"+str(QUOTE)+"' AND QTEREV_RECORD_ID = '"+str(revision)+"' )A WHERE SNO>="+str(start)+" AND SNO<="+str(end)+"  ")
